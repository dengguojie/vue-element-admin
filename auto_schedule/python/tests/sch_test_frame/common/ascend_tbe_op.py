...
from sch_test_frame.runtime import AscendRTSApi
from sch_test_frame.common import dtype_trans
from sch_test_frame.utils import shape_utils
...
def calc_op_param_size(shape_size, dtype):
    if not isinstance(dtype,
                      str) and dtype not in dtype_trans.get_all_str_dtypes():
        raise TypeError("dtype must be str and in [%s]" %
                        ",".join(dtype_trans.get_all_str_dtypes()))
...
class AscendOp:
    ...
    def _load_op_func(self):
        ...
        if not op_func:
            raise RuntimeError(
                "can't get op function in op module, op module name: %s, op function name: %s"
    ...
    def _pick_kernel_args(args):
        ...
        for input_info in input_list:
            ...
            size = -1 if shape_size < 0 else calc_op_param_size(
                shape_size, dtype)
            info = {
                "shape": shape,
                "dtype": dtype,
        ...
        try:
            with tbe.common.context.op_context.OpContext("dynamic"):
                ...
                tbe.common.context.op_context.get_context().add_op_info(
                    op_info)
                op_func(*args, **kwargs)
                compile_info = tbe.common.context.op_context.get_context(
                ).get_compile_info()
...
class AscendOpKernelParam:
    ...
    @staticmethod
    def build_op_param_by_data_file(data_file_path: str, dtype: str,
                                    shape: List[int]):
        if not os.path.exists(data_file_path):
            raise IOError("data_file_path is not exist, path: %s" %
                          data_file_path)
        np_dtype = dtype_trans.str_to_np_dtype(dtype)
        if not np_dtype:
            raise RuntimeError("dtype must in [%s]" %
                               ",".join(dtype_trans.get_all_str_dtypes()))
        ...
        if shape_size > len(np_data):
            raise RuntimeError("Data size(%d) in data_file < shape size(%d)" %
    ...
    def sync_from_device(self):
        if self._ascend_device and self._hbm_pointer:
            byte_data, _ = self._ascend_device.get_data_from_hbm(
                self._hbm_pointer, self.size)
            np_data = np.frombuffer(byte_data,
                                    dtype=dtype_trans.str_to_np_dtype(
                                        self.dtype))
...
class AscendOpKernelRunner:
    ...
    def __init__(self,
        ...
        self.ascend_device = AscendRTSApi(
    ...
    def build_kernel_param(self,
                           data,
                           shape=None,
                           dtype=None) -> AscendOpKernelParam:
    ...
    def _fill_inputs(self, inputs: List[Union[AscendOpKernelParam]],
                     kernel_args: List, input_params: List):
    ...
    def _fill_workspace(self, kernel: AscendOpKernel, wksp_hbm_pointers: List,
                        kernel_args: List):
    ...
    def _fill_outputs(self, kernel: AscendOpKernel,
                      ...
                      kernel_args: List):
        output_idx = 0
        output_input_ref_map = dict(
            output_input_ref) if output_input_ref else {}
            ...
            if output_info is not None:
                if output_idx in output_input_ref_map.keys():
                    output_param = input_params[
                        output_input_ref_map[output_idx]].create_ref()
                else:
                    ...
                    if not out_size:
                        shape_size = shape_utils.calc_shape_size(shape)
                        out_size = -1 if shape_size < 0 else calc_op_param_size(
                            shape_size, dtype)
                    out_hbm_pointer = self.ascend_device.malloc(out_size)
                    self.ascend_device.memset(out_hbm_pointer, out_size, 0,
                                              out_size)
                    output_param = AscendOpKernelParam(
                        shape=shape,
                        dtype=dtype,
    ...
    def _fill_tiling(self, kernel: AscendOpKernel, tiling_data: bytes,
                     tiling_hbm: List, kernel_args: List):
    ...
    def _execute_kernel(self, kernel: AscendOpKernel, kernel_args, block_dim):
        ...
        def _execute_kernel():
            self.ascend_device.launch_kernel(kernel.stub_func_p,
                                             block_dim, kernel_args,
                                             len(kernel_args), None,
                                             self._stream)
        ...
        if self.profiling:
            profiling_data = self.ascend_device.get_online_profiling_data(
                self._stream, self.profiling_times)
            cost_time = [
                float(profiling_data[i].totalcycle)
    ...
    def run(
        ...
        inputs,
        output_input_ref: List[List[int]] = None,
        tiling=None,
        block_dim=None,
        actual_output_info=None
    ) -> Union[AscendOpKernelParam, List[AscendOpKernelParam], None]:
        ...
        self._fill_outputs(kernel, output_input_ref, actual_output_info,
                           input_params, output_params, kernel_args)
