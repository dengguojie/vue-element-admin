...
def dsl_reduce_min(x, _, axis, keep_dim, kernel_name='dsl_reduce_min'):
    input_shape = x.get("shape")
    input_dtype = x.get("dtype")
    data1 = tvm.placeholder(input_shape, name='data1', dtype=input_dtype)
    res = tbe.reduce_min(data1, axis, keep_dim)
...
ut_case = OpUT("reduce_min", "reduce_min.test_reduce_min_impl", "dsl_reduce_min")


def test_axis_in_none(_):
    try:
        input1 = tvm.placeholder((1, 128), name="input1", dtype="float16")
        tbe.reduce_min(input1, [None])
...
def calc_expect_func(x, _, axis, keep_dims):
    x_value = x.get("value")
    res = np.minimum.reduce(x_value, axis=axis, keepdims=keep_dims)
    return (res, )
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_reduce_min_precision_1",
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_reduce_min_precision_2",
        "calc_expect_func": calc_expect_func,
        "precision_standard": precision_info.PrecisionStandard(0.001, 0.001)
    })
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_reduce_min_precision_3",
        "calc_expect_func": calc_expect_func,
        "precision_standard": precision_info.PrecisionStandard(0.001, 0.001)
    })
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_reduce_min_precision_4",
        "calc_expect_func": calc_expect_func,
        "precision_standard": precision_info.PrecisionStandard(0.001, 0.001)
    })
