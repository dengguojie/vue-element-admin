...
def test_h_out_1_case_910A(test_arg):
    try:
        with tbe.common.context.op_context.OpContext("dynamic"):
            with tbe.dsl.base.operation.compute():
                ...
                config = {
                    "name": "conv2d_tilingcase_h_out_1_910A",
...
def test_h_out_1_case_710(test_arg):
    try:
        with tbe.common.context.op_context.OpContext("dynamic"):
            with tbe.dsl.base.operation.compute():
                shape_x = [-1, 96, -1, -1, 16]
                ...
                with te.tvm.target.cce():
                    sch = te.utils.cce.auto_schedule(quant_out)
                config = {
                    "name": "conv2d_tilingcase_h_out_1_710",
                    "tensor_list": tensor_list,
                    "build_args": {"constant_realize_extent_in_infer_bound": False}
                }
                tbe.dsl.unify_schedule.build.build(sch, config)
...
print("adding test_h_out_1_case_910A")
ut_case.add_cust_test_func("Ascend910A", test_func=test_h_out_1_case_910A)

print("adding test_h_out_1_case_710")
ut_case.add_cust_test_func("Ascend310P3", test_func=test_h_out_1_case_710)
