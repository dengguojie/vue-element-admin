...
def dsl_vmins(x, _, scalar, is_tvm_const, kernel_name='dsl_vmins'):
    input_shape = x.get("shape")
    input_dtype = x.get("dtype")
    data1 = tvm.placeholder(input_shape, name='data1', dtype=input_dtype)
    if is_tvm_const:
        data2 = tvm.const(scalar, dtype=input_dtype)
        res = tbe.vmins(data1, data2)
    else:
        res = tbe.vmins(data1, scalar)
...
ut_case = OpUT("vmins", "vmins.test_vmins_impl", "dsl_vmins")
...
case1 = {"params": [{"shape": (5, 8, 16, 16), "dtype": "float16", "format": "ND"},
         ...
         "case_name": "test_vmins_scalar_is_float",
         "expect": "success",
         "support_expect": True
         }
...
case2 = {"params": [{"shape": (30000, 1), "dtype": "float32", "format": "ND"},
         ...
         "case_name": "test_vmins_scalar_is_int",
         "expect": "success",
         "support_expect": True
         }
...
case3 = {"params": [{"shape": (128,), "dtype": "float32", "format": "ND"},
         ...
         "case_name": "test_vmins_scalar_is_const",
         "expect": "success",
         "support_expect": True
         }
...
def calc_expect_func(x, _, scalar, __):
    x_value = x.get("value")
    output = np.minimum(x_value, scalar)
    return output
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_vmins_precision_1",
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_vmins_precision_2",
        "calc_expect_func": calc_expect_func,
        "precision_standard": precision_info.PrecisionStandard(0.001, 0.001)
    })
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_vmins_precision_3",
        "calc_expect_func": calc_expect_func,
        "precision_standard": precision_info.PrecisionStandard(0.001, 0.001)
    })
