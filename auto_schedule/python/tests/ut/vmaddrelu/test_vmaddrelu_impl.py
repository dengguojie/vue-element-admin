...
def dsl_vmaddrelu(x, y, z, _, kernel_name='dsl_vmaddrelu'):
    ...
    res = tbe.vmaddrelu(data1, data2, data3)
...
ut_case = OpUT("vmaddrelu", "vmaddrelu.test_vmaddrelu_impl", "dsl_vmaddrelu")
...
def test_tensor1_in_not_tensor(_):
    try:
        ...
        tbe.vmaddrelu(input1, input2, input3)
...
def calc_expect_func(x, y, z, _):
    ...
    y_value = y.get("value")
    z_value = z.get("value")
    res = np.maximum(np.add(np.multiply(x_value, z_value),  y_value), 0)
    return (res, )
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_vmaddrelu_precision_1",
...
ut_case.add_precision_case(
    "all", {
        ...
        "case_name": "test_vmaddrelu_precision_2",
        "calc_expect_func": calc_expect_func,
        "precision_standard": precision_info.PrecisionStandard(0.001, 0.001)
    })
