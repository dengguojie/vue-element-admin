...
ut_case = OpUT("vsqrt", "vsqrt.test_vsqrt_impl", "dsl_vsqrt")
...
def calc_expect_func(x, _, __):
    x_value = x.get("value")
    output = np.sqrt(x_value)
    return output


ut_case.add_precision_case(
    "Ascend310", {
        "params": [{"shape": (12, ), "dtype": "float16", "param_type": "input"},
                   {"shape": (12, ), "dtype": "float16", "param_type": "output"},
                   1
                   ],
        "case_name": "test_vsqrt_precision_frozen_inference_graph",
...
ut_case.add_precision_case(
    "Ascend910A", {
        "params": [{"shape": (3, ), "dtype": "float16", "param_type": "input"},
                   {"shape": (3, ), "dtype": "float16", "param_type": "output"},
                   0.0
                   ],
        "case_name": "test_vsqrt_precision_cascade_rcnn_inference_model",
        "calc_expect_func": calc_expect_func,
        "precision_standard": precision_info.PrecisionStandard(0.001, 0.001)
    })
