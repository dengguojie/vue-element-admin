# # -*- coding:utf-8 -*-
from sch_test_frame.ut import OpUT
from sch_test_frame.common import precision_info
import numpy as np
import tbe

ut_case = OpUT("SoftmaxCrossEntropyWithLogits", "impl.dynamic.softmax_cross_entropy_with_logits",
               "softmax_cross_entropy_with_logits")

case1 = {"params": [{"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1,), "dtype": "float16", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, None),)},
                    {"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))}],
         "case_name": "softmax_cross_entropy_with_logits_1",
         "expect": "success",
         "support_expect": True}
case2 = {"params": [{"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1, 3), "dtype": "float16", "format": "ND", "ori_shape": (-1, 3), "ori_format": "ND",
                     "range": ((1, None), (3, 3))},
                    {"shape": (-1,), "dtype": "float16", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, None),)},
                    {"shape": (-1, 3), "dtype": "float16", "format": "ND", "ori_shape": (-1, 3), "ori_format": "ND",
                     "range": ((1, None), (3, 3))}],
         "case_name": "softmax_cross_entropy_with_logits_2",
         "expect": "success",
         "support_expect": True}
case3 = {
    "params": [{"shape": (-1, 2, 3), "dtype": "float32", "format": "ND", "ori_shape": (-1, 2, 3), "ori_format": "ND",
                "range": ((1, None), (2, 2), (3, 3))},
               {"shape": (-1, 2), "dtype": "float32", "format": "ND", "ori_shape": (-1, 2), "ori_format": "ND",
                "range": ((1, None), (2, 2))},
               {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                "range": ((1, None),)},
               {"shape": (-1, 2), "dtype": "float32", "format": "ND", "ori_shape": (-1, 2), "ori_format": "ND",
                "range": ((1, None), (2, 2))}],
    "case_name": "softmax_cross_entropy_with_logits_3",
    "expect": ValueError,
    "support_expect": True}
case4 = {"params": [{"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1,), "dtype": "float16", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, None),)},
                    {"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))}],
         "case_name": "softmax_cross_entropy_with_logits_4",
         "expect": "success",
         "support_expect": True}
case5 = {"params": [{"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (2, -1), "dtype": "float16", "format": "ND", "ori_shape": (2, -1), "ori_format": "ND",
                     "range": ((2, 2), (1, None))},
                    {"shape": (2,), "dtype": "float16", "format": "ND", "ori_shape": (2,), "ori_format": "ND",
                     "range": ((2, 2),)},
                    {"shape": (2, -1), "dtype": "float16", "format": "ND", "ori_shape": (2, -1), "ori_format": "ND",
                     "range": ((2, 2), (1, None))}],
         "case_name": "softmax_cross_entropy_with_logits_5",
         "expect": "success",
         "support_expect": True}
case6 = {"params": [{"shape": (2, 24), "dtype": "float16", "format": "ND", "ori_shape": (2, 24), "ori_format": "ND",
                     "range": ((2, 2), (24, 24))},
                    {"shape": (-1, -1), "dtype": "float16", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, None), (22, None))},
                    {"shape": (2,), "dtype": "float16", "format": "ND", "ori_shape": (2,), "ori_format": "ND",
                     "range": ((2, 2),)},
                    {"shape": (2, 24), "dtype": "float16", "format": "ND", "ori_shape": (2, 24), "ori_format": "ND",
                     "range": ((2, 2), (24, 24))}],
         "case_name": "softmax_cross_entropy_with_logits_6",
         "expect": "success",
         "support_expect": True}
case7 = {"params": [{"shape": (8, -1), "dtype": "float16", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, 24))},
                    {"shape": (8, -1), "dtype": "float16", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, 24))},
                    {"shape": (8,), "dtype": "float16", "format": "ND", "ori_shape": (8,), "ori_format": "ND",
                     "range": ((8, 8),)},
                    {"shape": (8, -1), "dtype": "float16", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, 24))}],
         "case_name": "softmax_cross_entropy_with_logits_7",
         "expect": "success",
         "support_expect": True}
case8 = {"params": [{"shape": (8, -1), "dtype": "float16", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, None))},
                    {"shape": (8, -1), "dtype": "float16", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, None))},
                    {"shape": (8,), "dtype": "float16", "format": "ND", "ori_shape": (8,), "ori_format": "ND",
                     "range": ((8, 8),)},
                    {"shape": (8, -1), "dtype": "float16", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, None))}],
         "case_name": "softmax_cross_entropy_with_logits_8",
         "expect": "success",
         "support_expect": True}

case9 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, None),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))}],
         "case_name": "softmax_cross_entropy_with_logits_9",
         "expect": "success",
         "support_expect": True}

case10 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1, 3), "dtype": "float32", "format": "ND", "ori_shape": (-1, 3), "ori_format": "ND",
                     "range": ((1, None), (3, 3))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, None),)},
                    {"shape": (-1, 3), "dtype": "float32", "format": "ND", "ori_shape": (-1, 3), "ori_format": "ND",
                     "range": ((1, None), (3, 3))}],
         "case_name": "softmax_cross_entropy_with_logits_10",
         "expect": "success",
         "support_expect": True}

case11 = {
    "params": [{"shape": (-1, 2, 3), "dtype": "float32", "format": "ND", "ori_shape": (-1, 2, 3), "ori_format": "ND",
                "range": ((1, None), (2, 2), (3, 3))},
               {"shape": (-1, 2), "dtype": "float32", "format": "ND", "ori_shape": (-1, 2), "ori_format": "ND",
                "range": ((1, None), (2, 2))},
               {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                "range": ((1, None),)},
               {"shape": (-1, 2), "dtype": "float32", "format": "ND", "ori_shape": (-1, 2), "ori_format": "ND",
                "range": ((1, None), (2, 2))}],
    "case_name": "softmax_cross_entropy_with_logits_11",
    "expect": ValueError,
    "support_expect": True}
case12 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, None),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))}],
         "case_name": "softmax_cross_entropy_with_logits_12",
         "expect": "success",
         "support_expect": True}
case13 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, None), (1, None))},
                    {"shape": (2, -1), "dtype": "float32", "format": "ND", "ori_shape": (2, -1), "ori_format": "ND",
                     "range": ((2, 2), (1, None))},
                    {"shape": (2,), "dtype": "float32", "format": "ND", "ori_shape": (2,), "ori_format": "ND",
                     "range": ((2, 2),)},
                    {"shape": (2, -1), "dtype": "float32", "format": "ND", "ori_shape": (2, -1), "ori_format": "ND",
                     "range": ((2, 2), (1, None))}],
         "case_name": "softmax_cross_entropy_with_logits_13",
         "expect": "success",
         "support_expect": True}
case14 = {"params": [{"shape": (2, 24), "dtype": "float32", "format": "ND", "ori_shape": (2, 24), "ori_format": "ND",
                     "range": ((2, 2), (24, 24))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, None), (22, None))},
                    {"shape": (2,), "dtype": "float32", "format": "ND", "ori_shape": (2,), "ori_format": "ND",
                     "range": ((2, 2),)},
                    {"shape": (2, 24), "dtype": "float32", "format": "ND", "ori_shape": (2, 24), "ori_format": "ND",
                     "range": ((2, 2), (24, 24))}],
         "case_name": "softmax_cross_entropy_with_logits_14",
         "expect": "success",
         "support_expect": True}
case15 = {"params": [{"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, 24))},
                    {"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, 24))},
                    {"shape": (8,), "dtype": "float32", "format": "ND", "ori_shape": (8,), "ori_format": "ND",
                     "range": ((8, 8),)},
                    {"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, 24))}],
         "case_name": "softmax_cross_entropy_with_logits_15",
         "expect": "success",
         "support_expect": True}
case16 = {"params": [{"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, None))},
                    {"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, None))},
                    {"shape": (8,), "dtype": "float32", "format": "ND", "ori_shape": (8,), "ori_format": "ND",
                     "range": ((8, 8),)},
                    {"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (1, None))}],
         "case_name": "softmax_cross_entropy_with_logits_16",
         "expect": "success",
         "support_expect": True}
case17 = {"params": [{"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (2, 5000))},
                    {"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (2, 5000))},
                    {"shape": (8,), "dtype": "float32", "format": "ND", "ori_shape": (8,), "ori_format": "ND",
                     "range": ((8, 8),)},
                    {"shape": (8, -1), "dtype": "float32", "format": "ND", "ori_shape": (8, -1), "ori_format": "ND",
                     "range": ((8, 8), (2, 5000))}],
         "case_name": "softmax_cross_entropy_with_logits_17",
         "expect": "success",
         "support_expect": True}
case18 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 5000), (2, 5000))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 5000), (2, 5000))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((2, 5000),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 5000), (2, 5000))}],
         "case_name": "softmax_cross_entropy_with_logits_18",
         "expect": "success",
         "support_expect": True}
case19 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 5000), (1, 5000))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 5000), (1, 5000))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, 5000),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 5000), (1, 5000))}],
         "case_name": "softmax_cross_entropy_with_logits_19",
         "expect": "success",
         "support_expect": True}
case20 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 5000), (2, 5000))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 5000), (2, 5000))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, 5000),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 5000), (2, 5000))}],
         "case_name": "softmax_cross_entropy_with_logits_20",
         "expect": "success",
         "support_expect": True}
case21 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 5000), (1, 5000))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 5000), (1, 5000))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((2, 5000),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 5000), (1, 5000))}],
         "case_name": "softmax_cross_entropy_with_logits_21",
         "expect": "success",
         "support_expect": True}

case22 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 30000), (2, 30000))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 30000), (2, 30000))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((2, 30000),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 30000), (2, 30000))}],
         "case_name": "softmax_cross_entropy_with_logits_22",
         "expect": "success",
         "support_expect": True}
case23 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 30000), (1, 30000))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 30000), (1, 30000))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, 30000),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 30000), (1, 30000))}],
         "case_name": "softmax_cross_entropy_with_logits_23",
         "expect": "success",
         "support_expect": True}
case24 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 30000), (2, 30000))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 30000), (2, 30000))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((1, 30000),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((1, 30000), (2, 30000))}],
         "case_name": "softmax_cross_entropy_with_logits_24",
         "expect": "success",
         "support_expect": True}
case25 = {"params": [{"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 30000), (1, 30000))},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 30000), (1, 30000))},
                    {"shape": (-1,), "dtype": "float32", "format": "ND", "ori_shape": (-1,), "ori_format": "ND",
                     "range": ((2, 30000),)},
                    {"shape": (-1, -1), "dtype": "float32", "format": "ND", "ori_shape": (-1, -1), "ori_format": "ND",
                     "range": ((2, 30000), (1, 30000))}],
         "case_name": "softmax_cross_entropy_with_logits_25",
         "expect": "success",
         "support_expect": True}



compile_case_list = [
    case1,
    case2,
    case3,
    case4,
    case5,
    case6,
    case7,
    case8,
    case9,
    case10,
    case11,
    case12,
    case13,
    case14,
    case15,
    case16,
    case17,
    case18,
    case19,
    case20,
    case21,
    case22,
    case23,
    case24,
    case25
]

for item in compile_case_list:
    ut_case.add_case(["Ascend910A"], case=item)

if __name__ == '__main__':
    with tbe.common.context.op_context.OpContext("dynamic"):
        ut_case.run("Ascend910A")