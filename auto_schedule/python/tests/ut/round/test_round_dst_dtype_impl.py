...
@register_operator("dsl_round")
def dsl_round(x, y, _, kernel_name='dsl_round'):
    with tbe.common.context.op_context.OpContext("static") as f:
        ...
        for (x,) in ins:
            with tbe.dsl.compute():
                ...
                res = tbe.dsl.round(data1, dtype=output_dtype)
...
ut_case = OpUT("round", "round.test_round_dst_dtype_impl", "dsl_round")


def test_shape_value_less_then_zero(_):
    try:
        input1 = tvm.placeholder((0,), name="input1", dtype="float32")
        tbe.dsl.round(input1, dtype="int64")
...
for item in test_func_list:
    ut_case.add_cust_test_func(support_soc=["Ascend910B2"], test_func=item)
...
case4 = {"params": [{"shape": (17, 11, 19, 23, 7, 3), "dtype": "bfloat16", "format": "ND"},
         ...
         "case_name": "test_round_bf16_int32",
         "expect": "success",
         "support_expect": True
         }
