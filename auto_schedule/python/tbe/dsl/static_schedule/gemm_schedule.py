...
class GemmScheduleV2:
    ...
    def get_al1_bound(tiling, tensor_map):
        ...
        l0c_parts = util.int_ceil_div(util.get_value(tensor_map.get("a_l0a").shape[-4]) // tiling.get("block_dim")[2],
        ...
        if tiling.get("AL1_shape"):
            ...
            k_bound = util.get_value(tensor_map.get("a_l0a").shape[-3]) * \
                      util.get_value(tensor_map.get("a_l0a").shape[-1])
        # parts from from gm to l0c ignore multi core
        l0c_bounds = util.int_ceil_div(util.get_value(tensor_map.get("a_l0a").shape[-4]), tiling.get("CL0_matrix")[1])
    ...
    def get_bl1_bound(tiling, tensor_map):
        ...
        """
        l0c_parts = util.int_ceil_div(util.get_value(tensor_map.get("b_l0b").shape[-3]) // tiling.get("block_dim")[1],
        ...
        if tiling.get("BL1_shape"):
            ...
            bl1_parts = 1
            k_bound = util.get_value(tensor_map.get("b_l0b").shape[-4]) * \
                      util.get_value(tensor_map.get("b_l0b").shape[-1])
        l0c_bounds = util.int_ceil_div(util.get_value(tensor_map.get("b_l0b").shape[-3]), tiling.get("CL0_matrix")[0])
    ...
    def gemm_schedule(self):
        ...
        """
        sch = self.sch
        tensor_map, all_tensor, leaf_tensor = self.tiling_case_para["tensor_list"]
        tiling = self.tiling_case_para["tiling_strategy"]

        # set scope for matmul
        util.set_matmul_scope(sch, tensor_map)
        util.set_out_scope(all_tensor, leaf_tensor, sch, tensor_map)
