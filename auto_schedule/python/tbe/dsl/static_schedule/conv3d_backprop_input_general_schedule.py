...
def general_schedule(tensor, sch_list, tiling_case=None, var_range=None):
    ...
    def _al1_and_bl1_process():
        if kd_reduce_flag:
            (reduce_axis_kd_outer_outer,
             reduce_axis_kd_outer_inner) = sch[c_col].split(reduce_axis_kd_outer, kd_tiling_l1_factor)
    ...
    def _fetch_tensor_info(var_map):
        ...
        kernel_d, kernel_h, kernel_w = [i.value for i in c_ub_exact_hw.op.attrs["kernels"]]
        a_col_before = a_col.op.input_tensors[0]  # im2col_row_major in L1
        dilation = [i.value for i in a_col_before.op.attrs["dilation"]]
        padding = cube_util.shape_to_list(a_col_before.op.attrs["padding"])
        padding_var = a_col_before.op.attrs["padding_var"]
        ...
        if "a_filling" in tensor_map.keys():
            ...
            stride_h, stride_w = [i.value for i in a_filling.op.attrs["stride_expand"]]
            if not var_map:
    ...
    def _tiling_l0_process():
        if tiling.get("BL0_matrix"):
            ...
            _, bl0_tiling_kd, bl0_tiling_kb, bl0_tiling_nb, _, _ = [i.value for i in b_col.shape]
    ...
    def _emit_insn_aub():
        ...
        if stride_h > 1 or stride_w > 1:
            if aub_fusion_flag is False and not var_map:
    ...
    aub_input_tensors, cub_input_tensors, \
        aub_body_tensors, cub_body_tensors = _get_ubfusion_tensors(input_ops, body_ops)
    mean_flag = False
    ...
    if not var_map:
        _, _, _, _, _, kernel_h, kernel_w, _ = [i.value for i in a_col_before.shape]
    else:
    ...
    b_ddr_n1, b_ddr_k1, b_ddr_k0, b_ddr_n0 = [i.value for i in b_ddr.shape]
    ...
    _, _, bl1_k1, bl1_co1, bl1_co0, _ = [i.value for i in b_l1.shape]
