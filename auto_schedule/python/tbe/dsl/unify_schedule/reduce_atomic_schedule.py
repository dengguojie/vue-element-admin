...
class ReduceAtomicSchedule(_VectorSchedule):
    ...
    def _do_block_tiling(self):
        ...
        if tiling_axis not in self.reduce_info.reduce_axis_indices:
            dict_args = dict()
            ...
            dict_args["detailed_cause"] = "Atomic schedule block tiling can " \
                                          ...
                                          % (tiling_axis, self.reduce_info.reduce_axis_indices)
            raise RuntimeError(dict_args, get_error_message(dict_args))

        tiling_axis = self.reduce_info.reduce_axis_indices.index(tiling_axis)
    ...
    def _do_ub_tiling(self):
        ...
        if ub_split_axis in self.reduce_info.reduce_axis_indices:
            axis = self.reduce_info.reduce_axis_indices.index(ub_split_axis)
    ...
    def _atomic_additonal_schedule(self):
        ...
        fused_list = []
        block_split_axis = self.reduce_info.reduce_axis_indices.index(block_split_axis)
    ...
    def _do_set_constraint(self):
        ...
        reduce_axis_index = self.reduce_info.reduce_axis_indices
    ...
    def _calculate_storage_align(self):
        ...
        def _storage_align_tensors_before_reduce(_align_axis):
            tensors_before_reduce = self.get_all_producers_stages(self.reduce_rfs)
        ...
        if self._reduce_case == 2:
            ...
            align_factor = int(BLOCK_SIZE_BYTE // DTYPE_BYTE_MAPPING[self.reduce_rfs.dtype])
            para = {"align_axis_var": self.reduce_rfs.op.axis[res_align_axis + self._axis_offset],
                    "align_factor": align_factor,
                    "offset": 0}
    ...
    def _calculate_compute_at(self):
        ...
        for tensor in tensors_before_reduce:
            if tensor not in self.graph_info.input_tensor_set:
                para = {"parent": self.schedule[ub_tiling_tensor], "scope": res_ub_outer}
                self.compute_at_map[tensor] = para

        para = {"parent": self.schedule[self.reduce_repls],
                "scope": self.reduce_repls.op.reduce_axis[0]}
        self.compute_at_map[self.reduce_rfs] = para
    ...
    def _reorder_atomic_reduce_all(self, out_tensor_ub_rf, out_tensor_global):
        ...
        reduce_block_axis = self.reduce_info.reduce_axis_indices.index(block_split_axis)
        reduce_ub_axis = self.reduce_info.reduce_axis_indices.index(ub_split_axis)
