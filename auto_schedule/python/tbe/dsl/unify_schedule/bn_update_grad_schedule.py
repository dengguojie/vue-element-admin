...
class BNUpdateGradSchedule():
    ...
    def _gen_reversed_subgraph_list(self):
        ...
        self.max_ub_count = self._get_max_ub_count(dtype, self.shape_x) // 2
        add_compile_info("max_ub_count", self.max_ub_count)
    ...
    def _do_cache_write(self):
        ...
        self.final_out_buffer = sch.cache_write(self.out, cce.scope_ubuf)
    ...
    def _do_storage_bound(self):
        ...
        for stage_tensor in tensor_storage_bound_set:
            self.schedule[stage_tensor].set_storage_bound(self.max_ub_count)
        self.schedule[self.final_out_buffer].set_storage_bound(self.max_ub_count)
        self.schedule[self.out].set_storage_bound(self.max_ub_count)
    ...
    def _do_tiling(self):
        ...
        final_out_buffer = self.final_out_buffer
        ...
        if block_split_axis_index != 0:
            self.schedule[final_out_tensor].reorder(block_split_axis, final_out_tensor.op.axis[0])
        self.sum_x_block_outer, self.sum_x_block_inner = self.schedule[final_out_tensor].split(block_split_axis, nparts=core_num)

        # ub tiling
        ub_split_axis = final_out_buffer.op.axis[ub_split_axis_index]
        if ub_split_axis_index == 0:
            self.sum_x_ub_outer, self.sum_x_ub_inner = self.schedule[final_out_buffer].split(final_out_buffer.op.reduce_axis[0], factor=1)
            ...
            self.sum_x_ub_outer, self.sum_x_ub_inner = self.schedule[final_out_buffer].split(final_out_buffer.op.reduce_axis[1], factor=1)
            self.schedule[final_out_buffer].reorder(final_out_buffer.op.axis[0],
                                                    final_out_buffer.op.axis[1],
    ...
    def _do_compute_at(self):
        ...
        for tensor in self.input_tensor_buffer_map:
            buffer_tensor = self.input_tensor_buffer_map[tensor]
            sch[buffer_tensor].compute_at(sch[final_out_buffer], self.sum_x_ub_inner)
        
        for tensor in self.mid_tensor_buffer_map:
            buffer_tensor = self.mid_tensor_buffer_map[tensor]
            sch[buffer_tensor].compute_at(sch[final_out_buffer], self.sum_x_ub_inner)

        sch[final_out_buffer].compute_at(sch[self.out], self.sum_x_block_inner)
