...
class ExtractImagePatchesSchedule:
    ...
    def _do_tiling(self):
        funcs = {TilingStrategy.SPECIAL_DIMEN_KEY: self._do_tiling_special_dimen,
                 TilingStrategy.FOUR_DIMEN: self._do_tiling_four_dimen}
        funcs.get(self._tiling_strategy)()

    def _do_tiling_special_dimen(self):
        self.multi_core_factor_0 = 16
    ...
    def _do_tiling_four_dimen(self):
        self.multi_core_factor_0 = operation.get_context().get("multi_core_factor_0")
        fmap_shape = self.extract_params.get("fmap_shape")
        self.fmap_n = fmap_shape[0]
        self._sch.set_var_range(self.fmap_n, 2, 513)
    ...
    # 'pylint: disable=too-many-arguments
    def _get_tiling_param(self, setfmatrix_dict, used_ub_size, type_size, avg_split_ub_size,
                          align_block_size):
        out_w = self.extract_params.get('out_w')
        fmap_shape = self.extract_params.get('fmap_shape')
        c_in_real = self.extract_params.get("c_in_real")
        lcm_out_w = self.extract_params.get('lcm_out_w')
        cut_h_col = self.extract_params.get('cut_h_col')
        cut_w_row = self.extract_params.get('cut_w_row')
        cut_h_row = self.extract_params.get('cut_h_row')
        dilated_kernel_h = self.extract_params.get('dilated_kernel_h')
        dilated_kernel_w = self.extract_params.get('dilated_kernel_w')
    ...
    def _split_multi_core_32b_not_aligned(self, multi_core_factor, dma_split_axis_id, dma_split_factor,
                                          workspace_res):
        """
    ...
    @staticmethod
    def _add_compile_info(device_core_num, in_special_device, workspace_shape, c_in_real,
                          kernel_h_w, stride_h_w, dilate_h_w):
        add_compile_info("coreNum", device_core_num)
        add_compile_info("inSpecialDevice", in_special_device)
        add_compile_info("workspaceDimen", workspace_shape)
        add_compile_info("realC", c_in_real)
        add_compile_info("ksizeHW", kernel_h_w)
        add_compile_info("strideHW", stride_h_w)
        add_compile_info("dilateHW", dilate_h_w)
    ...
    def do_schedule(self):
        ...
        self._sch[fmap_in_l1].set_scope(tbe_platform.scope_cbuf)
        self._sch[fmap_im2col].set_scope(tbe_platform.scope_cbuf)
        self._sch[fmap_fractal].set_scope(tbe_platform.scope_ubuf)
        self._sch[split_c1_ub].set_scope(tbe_platform.scope_ubuf)
        self._sch[transpose_ub].set_scope(tbe_platform.scope_ubuf)
        self._sch[merge_hw_ub].set_scope(tbe_platform.scope_ubuf)
        self._sch[merge_co_ub].set_scope(tbe_platform.scope_ubuf)
        self._sch[workspace_res].set_scope(tbe_platform.scope_gm)
        self._sch[ub_res].set_scope(tbe_platform.scope_ubuf)

        workspace_shape = [int(workspace_res.shape[i]) for i in range(1, 4)]
        kernel_h_w = [kernel_h, kernel_w]
        stride_h_w = [stride_h, stride_w]
        dilate_h_w = [dilate_h, dilate_w]
        ...
        if cut_h_col > self.fmap_h:
            cut_h_col = self.fmap_h
        # `cut_h_col while cut_hw = Constant.BLOCK_SIZE`
        cut_w_row_s = (Constant.BLOCK_SIZE - 1) * stride_w + 1
        cut_h_row_s = max(stride_h, (((cut_w_row_s - 1) // self.fmap_w + 1) - 1) * stride_h + 1)
        ...
        self.extract_params['lcm_out_w'] = lcm_out_w
        self.extract_params['cut_h_col'] = cut_h_col
        self.extract_params['cut_w_row'] = cut_w_row
        self.extract_params['cut_h_row'] = cut_h_row
        self.extract_params['dilated_kernel_h'] = dilated_kernel_h
        self.extract_params['dilated_kernel_w'] = dilated_kernel_w

        self._sch[ub_res].buffer_align((1, 1), (1, 1), (1, 1), (1, align_block_size))
        self._sch[fmap_im2col].buffer_align((1, 1), (out_w, out_w), (1, 1), (1, 1), (1, 1), (1, align_block_size))
        self._sch[fmap_fractal].buffer_align((1, 1), (1, 1), (1, 1), (1, Constant.BLOCK_SIZE), (1, align_block_size))

        used_ub_size = Constant.SIZE_UB // type_size // Constant.DOUBLE_BUFFER
        avg_split_ub_size = used_ub_size // Constant.NEED_UB_SPACE_NUM
        howo = out_h * out_w
        khkw = kernel_h * kernel_w
        c_out = khkw * self.fmap_c1 * self.fmap_c0

        out_shape = [self.fmap_n, howo, khkw, c_in_real]
        self.device_core_num = util.get_core_num()
        in_special_device = False
        if self.device_core_num == 8:
            in_special_device = True
        ...
        def _get_multi_core_factor(dma_split_axis_id, tiling_factor):
            ...
            if Constant.SIZE_L1 >= self.fmap_h * self.fmap_w * self.fmap_c0 * self.fmap_c1 * \
                    type_size * Constant.DOUBLE_BUFFER:
                howo_align = Constant.BLOCK_SIZE
            elif Constant.SIZE_L1 >= cut_h_col * self.fmap_w * self.fmap_c0 * self.fmap_c1 * \
                    type_size * Constant.DOUBLE_BUFFER:
        ...
        def _schedule_32b_not_aligned(dma_split_axis_id, dma_split_factor, allow_multi_core, reg_mov=True):
            ...
            khkw_factor = khkw
            c_factor = c_in_real
            tiling_param = self._get_tiling_param(setfmatrix_dict, used_ub_size, type_size, avg_split_ub_size,
                                                  align_block_size)
            ...
            if lcm_out_w * c_out <= avg_split_ub_size and khkw * self.fmap_c1 <= Constant.LOAD3D_REPEAT_TIME_LIMIT \
                    and max_vm_cut_col > 0 and max_vm_cut_row > 0 and Constant.SIZE_L1 >= self.fmap_h * \
                    self.fmap_w * self.fmap_c0 * self.fmap_c1 * type_size * Constant.DOUBLE_BUFFER:
            ...
            workspace_res_n_outer, workspace_res_n_inner = self._sch[workspace_res].split(
                workspace_axis_list[0], factor=tiling_factor[0])
            workspace_res_howo_outer, workspace_res_howo_inner = self._sch[workspace_res].split(
                workspace_axis_list[1], factor=tiling_factor[1])
            workspace_res_khkw_outer, workspace_res_khkw_inner = self._sch[workspace_res].split(
                workspace_axis_list[2], factor=tiling_factor[2])
            workspace_res_c1_inner_outer, workspace_res_c1_inner = self._sch[workspace_res].split(
                workspace_axis_list[3], factor=align_block_size)
            workspace_res_c1_outer, workspace_res_c1_inner_outer = \
                self._sch[workspace_res].split(workspace_res_c1_inner_outer,
            ...
            if Constant.SIZE_L1 >= self.fmap_h * self.fmap_w * self.fmap_c0 * self.fmap_c1 * \
                    type_size * Constant.DOUBLE_BUFFER:
                compute_at_id = 0
            elif Constant.SIZE_L1 >= cut_h_row * self.fmap_w * self.fmap_c0 * self.fmap_c1 * \
                    type_size * Constant.DOUBLE_BUFFER and move_rate != move_rate_cut_col:
                compute_at_id = 1
            elif Constant.SIZE_L1 >= cut_h_col * self.fmap_w * self.fmap_c0 * self.fmap_c1 * \
                    type_size * Constant.DOUBLE_BUFFER and move_rate == move_rate_cut_col:
                compute_at_id = 1
            elif Constant.SIZE_L1 >= cut_h_col * self.fmap_w * self.fmap_c0 * \
                    type_size * Constant.DOUBLE_BUFFER and move_rate == move_rate_cut_col:
                compute_at_id = 1
                workspace_c_out_outer, workspace_axis_outer_list[3] = self._sch[workspace_res].split(
                ...
                workspace_c_out_outer, workspace_axis_outer_list[3] = self._sch[workspace_res].split(
                    workspace_axis_outer_list[3], factor=1)
                workspace_bind_list.append(workspace_c_out_outer)
                workspace_axis_outer_list[2], _ = self._sch[workspace_res].split(
                    workspace_axis_outer_list[2], factor=max(kernel_w // tiling_factor[2], 1))

            self._sch[workspace_res].reorder(
                *(workspace_bind_list + workspace_axis_outer_list + workspace_axis_inner_list))

            self._sch[fmap_im2col].compute_at(self._sch[workspace_res], workspace_axis_outer_list[compute_at_id])
            self._sch[fmap_in_l1].compute_at(self._sch[workspace_res], workspace_axis_outer_list[compute_at_id])

            self._sch[merge_co_ub].compute_at(self._sch[workspace_res], workspace_axis_outer_list[3])
            self._sch[merge_hw_ub].compute_at(self._sch[workspace_res], workspace_axis_outer_list[3])
            self._sch[transpose_ub].compute_at(self._sch[workspace_res], workspace_axis_outer_list[3])
            self._sch[split_c1_ub].compute_at(self._sch[workspace_res], workspace_axis_outer_list[3])
            self._sch[fmap_fractal].compute_at(self._sch[workspace_res], workspace_axis_outer_list[3])

            self._sch[ub_res].compute_at(self._sch[self.output_res], res_axis_list[dma_split_axis_id])
            if reg_mov:
            ...
            self._sch[workspace_res].bind(workspace_bind_list[0], block)

            self._sch[split_c1_ub].compute_inline()
            self._sch[merge_co_ub].compute_inline()

            self._sch[fmap_in_l1].emit_insn(fmap_in_l1.op.axis[0], te_platform.DMA_COPY)
            self._sch[fmap_im2col].emit_insn(fmap_im2col.op.axis[0], te_platform.SET_FMATRIX, setfmatrix_dict)
            self._sch[fmap_fractal].emit_insn(fmap_fractal.op.axis[0], te_platform.IM2COL)
            self._sch[split_c1_ub].emit_insn(split_c1_ub.op.axis[0], te_platform.DMA_COPY)

            if dtype_input in ("int8", "uint8"):
                self._sch[transpose_ub].emit_insn(transpose_ub.op.axis[0], te_platform.DMA_COPY)
                self._sch[merge_hw_ub].emit_insn(merge_hw_ub.op.axis[0], te_platform.DMA_COPY)
            else:
                self._sch[transpose_ub].emit_insn(transpose_ub.op.axis[0], te_platform.insn_cmd.ADDVS)
                self._sch[merge_hw_ub].emit_insn(merge_hw_ub.op.axis[0], te_platform.insn_cmd.ADDVS)

            self._sch[merge_co_ub].emit_insn(merge_co_ub.op.axis[0], te_platform.DMA_COPY)
            self._sch[workspace_res].emit_insn(workspace_axis_inner_list[0], te_platform.DMA_COPY)
            self._sch[ub_res].emit_insn(ub_res.op.axis[0], te_platform.DMA_COPY)
            if reg_mov:
        ...
        if c_in_real % align_block_size == 0:
            ...
            khkw_factor = khkw
            c_factor = c_in_real
            tiling_param = self._get_tiling_param(setfmatrix_dict, used_ub_size, type_size,
                                                  avg_split_ub_size, align_block_size)

            max_vm_cut_col, max_vm_cut_row, max_vm_cut_col_p, max_vm_cut_min, move_rate_cut_col,\
            ...
            if lcm_out_w * c_out <= avg_split_ub_size and khkw * self.fmap_c1 <= Constant.LOAD3D_REPEAT_TIME_LIMIT \
                    ...
                    and Constant.SIZE_L1 >= self.fmap_h * self.fmap_w * self.fmap_c0 * self.fmap_c1 * \
                    type_size * Constant.DOUBLE_BUFFER:
                max_v = avg_split_ub_size // lcm_out_w // c_out
                if lcm_out_w * max_v < howo:
                    # if True cut n howo else only cut n
            ...
            res_n_outer, res_n_inner = self._sch[self.output_res].split(res_axis_list[0], factor=tiling_factor[0])
            res_howo_outer, res_howo_inner = self._sch[self.output_res].split(res_axis_list[1],
                                                                              factor=tiling_factor[1])
            res_khkw_outer, res_khkw_inner = self._sch[self.output_res].split(res_axis_list[2],
                                                                              factor=tiling_factor[2])
            res_c_inner_outer, res_c_inner = self._sch[self.output_res].split(res_axis_list[3],
                                                                              factor=align_block_size)
            res_c_outer, res_c_outer_inner = self._sch[self.output_res].split(res_c_inner_outer,
                                                                              factor=tiling_factor[3] //
                                                                              align_block_size)
            ...
            if Constant.SIZE_L1 >= self.fmap_h * self.fmap_w * self.fmap_c0 * self.fmap_c1 * \
                ...
                compute_at_id = 1
                    ...
                    type_size * Constant.DOUBLE_BUFFER and move_rate == move_rate_cut_col:
                compute_at_id = 1
            elif Constant.SIZE_L1 >= cut_h_col * self.fmap_w * self.fmap_c0 * type_size * \
                    Constant.DOUBLE_BUFFER and move_rate == move_rate_cut_col:
                compute_at_id = 1
            ...
            elif Constant.SIZE_L1 >= cut_h_row_s * self.fmap_w * self.fmap_c0 * type_size * Constant.DOUBLE_BUFFER \
                    and split_khkw_mode:
                compute_at_id = 2
            ...
            self._sch[self.output_res].reorder(*(res_bind_list + res_axis_outer_list +
                            [res_n_inner, res_c_outer_inner, res_howo_inner, res_khkw_inner, res_c_inner]))

            self._sch[fmap_im2col].compute_at(self._sch[self.output_res], res_axis_outer_list[compute_at_id])
            self._sch[fmap_in_l1].compute_at(self._sch[self.output_res], res_axis_outer_list[compute_at_id])
            self._sch[transpose_ub].compute_at(self._sch[self.output_res], res_axis_outer_list[3])
            self._sch[fmap_fractal].compute_at(self._sch[self.output_res], res_axis_outer_list[3])

            self._sch[workspace_res].compute_inline()
            self._sch[ub_res].compute_inline()
            self._sch[merge_co_ub].compute_inline()
            self._sch[merge_hw_ub].compute_inline()
            self._sch[split_c1_ub].compute_inline()
            ...
            self._sch[fmap_fractal].emit_insn(fmap_fractal.op.axis[0], te_platform.IM2COL)
            self._sch[transpose_ub].emit_insn(transpose_ub.op.axis[0], te_platform.DMA_COPY)
            self._sch[self.output_res].emit_insn(res_n_inner, te_platform.DMA_COPY)
        else:
        ...
        self._sch[fmap_in_l1].double_buffer()
        self._sch[fmap_im2col].double_buffer()
        self._sch[fmap_fractal].double_buffer()
        self._sch[transpose_ub].double_buffer()
        self._sch[ub_res].double_buffer()
        self._add_compile_info(self.device_core_num, in_special_device, workspace_shape, c_in_real, kernel_h_w,
                               stride_h_w, dilate_h_w)
        return self._sch
