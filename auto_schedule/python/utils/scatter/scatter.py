...
class Scatter:
    ...
    def calc_process(self, mask, dest_addr, src_addr1, src_addr2, repeat_times,
        ...
        """
        need_vconv_dtype = ("int8", "uint8")
        compute_repeat_strid, dst_ub, mask, src1_ub, src2_ub = \
            self._convert_before_calc(dest_addr, is_tile, mask,
                                      need_vconv_dtype, repeat_times,
                                      src_addr1, src_addr2)
        ...
        if self.compute_type == "vadd":
            ...
            raise RuntimeError("the operater [%s] is not supported" %
                               self.compute_type)
        self._convert_after_calc(dest_addr, is_tile, mask, need_vconv_dtype,
                                 repeat_times, src_addr1)

    def _convert_after_calc(self, dest_addr, is_tile, mask, need_vconv_dtype,
                            repeat_times, src_addr1):
        if self.var_dtype in need_vconv_dtype:
            if is_tile:
    ...
    def _convert_before_calc(self, dest_addr, is_tile, mask, need_vconv_dtype,
                             repeat_times, src_addr1, src_addr2):
        if self.var_dtype in need_vconv_dtype:
            if is_tile:
        ...
        return compute_repeat_strid, dst_ub, mask, src1_ub, src2_ub
