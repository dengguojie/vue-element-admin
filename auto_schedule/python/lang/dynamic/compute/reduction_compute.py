...
@decorator
def check_support_of_reduce(func, *args, **kwargs):
    '''
    ...
    if len(args) == 3 or len(args) == 4:
        ...
        if dtype not in supported_dtypes:
            soc_ver = cceconf.get_soc_spec("SOC_VERSION")
            if dtype == "int32" and is_last_axis:
                raise RuntimeError(
                    "%s do not support %s in %s for reduce last axis !"
                    % (intr, dtype, soc_ver))
            else:
                raise RuntimeError("%s do not support %s in %s !"
                                   % (intr,  dtype, soc_ver))
