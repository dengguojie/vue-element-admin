...
class DeConvKernelSize1Pattern(CubeDslPattern):  # pylint:disable=R0902
    ...
    def generate_a(self, dedy):  # pylint:disable=R0914
        ...
        if from_l1_flag:
            pat_conv = ConvDslPattern(kernel_h=1, kernel_w=1,
                                      stride=[1, 1],
                                      pad=[0, 0, 0, 0],
                                      dilations=(1, 1), offset_x=0)
            dedy_col = pat_conv.generate_a(dedy, slice_offset=h_offset,
                                           valid_shape=valid_shape)
        elif from_ddr_flag:
            if not valid_shape:
                valid_shape = (batch_dim, co1_dim, ho_dim, wo_dim, co0_dim)
            dedy_col = tvm.compute(valid_shape, lambda n, co1, h, w, co0: dedy(
                n, co1, h + h_offset, w, co0), name=dedy.name + "_col")
            pat_conv = ConvDslPattern(kernel_h=1, kernel_w=1,
                                      stride=[1, 1],
                                      pad=[0, 0, 0, 0],
                                      dilations=(1, 1), offset_x=0)
            dedy_col = pat_conv.generate_a(dedy_col, slice_offset=0,
                                           valid_shape=valid_shape)
        else:
            shape = (batch_dim, co1_dim, real_hwdim, co0_dim)
            dedy_col = tvm.compute(shape,
                                   lambda n, co1, m1, co0:
                                   dedy(n,
                                        co1,
                                        tvm.floordiv(m1, wo_dim),
                                        tvm.floormod(m1, wo_dim), co0),
                                   name=dedy.name + "_col")
            shape = (batch_dim, hw_dim, co1_dim, self._m0, co0_dim)
            dedy_col = tvm.compute(shape,
                                   lambda n, m1, co1, m0, co0:
                                   dedy_col(n, co1, m0 + self._m0 * m1, co0),
    ...
    def generate_c(self,  # pylint:disable=R0914
        ...
        def _inner_generate_mmad(matrix_a, matrix_b):  # pylint:disable=R0914
            n_dim, hw_dim, k1_dim, m0_dim, k0_dim = \
                shape_to_list(matrix_a.shape)
            bk1_dim, co1_dim, co0_dim, bk0_dim = \
                list(i.value for i in matrix_b.shape)
            if bk1_dim != k1_dim or bk0_dim != k0_dim:
                raise RuntimeError("invaild shape",
                                   bk1_dim, k1_dim, bk0_dim, k0_dim)
