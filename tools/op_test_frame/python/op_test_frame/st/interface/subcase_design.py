...
class SubCaseDesign:
    ...
    def _deal_with_value(self, input_desc, one_input_desc, for_fuzz=False):
        ...
        const_input = utils.ConstInput(input_desc.get(utils.IS_CONST))
    ...
    def _check_attr_valid(self, json_obj, fuzz_dict=None):
        attr_list = []
        if CD.ATTR not in json_obj:
            return attr_list
        if not isinstance(json_obj[CD.ATTR], (tuple, list)):
            utils.print_error_log(
                'The value (%s) of "attr" is not list. Please modify it in file'
                ' %s.' % (json_obj[CD.ATTR], self.current_json_path))
            raise utils.OpTestGenException(
                utils.OP_TEST_GEN_INVALID_DATA_ERROR)
        name_list = []
        for attr in json_obj[CD.ATTR]:
            CD.check_required_key_valid(attr, ATTR_REQUIRED_KEYS, CD.ATTR,
                                        self.current_json_path)
