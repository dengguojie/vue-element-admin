...
def _get_factor(ele_zero, ele_cnt, total_ele, no_remainder):
    ...
    no_remainder: bool
        when split_axis == 0,
        the value of shape[0] whether divided by split_factor
        without remainder.
...
def _tilling_axis(shape, dtype, no_remainder):
    ...
    dtype_bytes_size = platform_info.get_bit_len(dtype) // 8
...
def _check_shape(ref_shape, value_shape):
    ...
    para_check.check_shape_rule(ref_shape)
    para_check.check_shape_size(ref_shape, SHAPE_SIZE_LIMIT)


def _check_params(ref_shape, value_shape, dtype, kernel_name):
    """
    check the parameters including ref_shape, value_shape,
    dtype and kernel_name
...
def assign(ref, value, output, kernel_name="assign"):
    ...
    _tilling_axis(ref_shape, dtype, False)
    ...
    if core_bind_axis < split_axis:
        ...
        if split_axis == 0:
            ...
            if split_axis == core_bind_axis and core_bind_split_factor > \
                    split_factor:
                ...
                if core_bind_axis == split_axis:
                    ...
                    core_bind_axis_outer = axis_outer
                    axis_inner = tilling_axis_inner
    ...
    sch[data_b_ub].emit_insn(data_b_ub.op.axis[split_axis], 'dma_copy')
    sch[data_a].emit_insn(tilling_axis_inner, 'dma_copy')
