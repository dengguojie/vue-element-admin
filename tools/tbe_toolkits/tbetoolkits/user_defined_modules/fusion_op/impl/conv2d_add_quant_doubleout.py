...
def conv2d_add_quant_doubleout(inputs, weights, bias,fm2_place, offset_w, outputs,output, strides, pads, dilations=[1, 1, 1, 1],
           groups=1, data_format='NHWC', offset_x=0, kernel_name="conv2d_add_quant_doubleout"):
    print("inputs, weights, bias, offset_w=============================",inputs, weights, bias, offset_w) 
    #groups = int(group)
    #c_org_channel = w_shape[1]//groups
    ...
    #shape_w = ((in_channel_weight * kh * kw + block_size_k - 1) // block_size_k,Co1, block_size_n, block_size_k)
    ...
    with tvm.target.cce():
        ...
        fm = tvm.placeholder(shape_in, name='fm', dtype='float16', attrs={'ori_shape': inputs['ori_shape'],'ori_format': 'NCHW'})
        ...
        add_res = add_compute(fm2,conv_res,None)        
        out = ascend_quant_compute(add_res, None, quant_dict["scale"],quant_dict["offset"],quant_dict["sqrt_mode"])
        out = [add_res,out]

        sch = generic.auto_schedule(out)
        tensor_list = [fm, filter_w,fm2]
        tensor_list.extend(out)
        
        if bias:
            tensor_list = [fm, filter_w, bias_tensor,fm2]
            tensor_list.extend(out)
