...
def lcm(wout, factor):
    ...
    return tmp // factor
def leaky_relun0_conv2d(inputs, weights, bias, offset_w, outputs, strides, pads,
                        dilations=[1, 1, 1, 1], groups=1, data_format='NHWC',
                        offset_x=0, kernel_name="leaky_relun0_conv2d"):
    ...
    enlarge = min(lcm(lcm(cin_per_group, block_size_k)//cin_per_group,
    ...
    with tvm.target.cce():
        ...
        if bias:
            ...
            bias_tensor = None
        leaky_relu_res = leaky_relu_compute(fm, None, 0) #, negative_slope=0.1
        out = conv2d_compute(leaky_relu_res, filter_w, bias_tensor, None, None,
    ...
    config = {"print_ir": False, "need_build": True, "name": kernel_name,
