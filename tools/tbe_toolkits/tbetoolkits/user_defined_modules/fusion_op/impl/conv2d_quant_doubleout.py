...
def conv2d_quant_doubleout(inputs, weights, bias, offset_w, outputs, output,strides, pads, dilations=[1, 1, 1, 1],
           groups=1, data_format='NHWC', offset_x=0, kernel_name="conv2d_quant_doubleout"):
...
#        dequant_res = ascend_dequant_compute(conv_res, deq_reg, None, sqrt_mode=False, relu_flag=False)
        out = ascend_quant_compute(conv_res, None, quant_dict["scale"],quant_dict["offset"],quant_dict["sqrt_mode"])
        out = [conv_res,out]
        ...
        if bias:
            tensor_list = [fm, filter_w, bias_tensor]
            tensor_list.extend(out)
