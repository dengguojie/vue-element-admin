...
def conv2d_backprop_input_vadd_drelu(filter, out_backprop, y, input_size, strides, pads, dilations=(1, 1, 1, 1),
    ...
    dedy = tvm.placeholder(shape_dedy, name="dedy", dtype="float16",
                           attrs={"ori_shape": out_backprop.get('ori_shape'), "dtype": "float16", "ori_format": out_backprop.get('ori_format')})
    # pylint: disable=redefined-builtin
    filter = tvm.placeholder(shape_filter_frac, name="filter", dtype="float16",
                             attrs={"ori_shape": filter.get('ori_shape'), "dtype": "float16", "ori_format": filter.get('ori_format')})
    ...
    vadd_shape = [i.value for i in dedx.shape]
    vadd_tensor = tvm.placeholder(vadd_shape, name="vadd_tensor", dtype='float16')
    datas = [dedx, vadd_tensor]
    vadd_res = add_n_compute(datas, "", 2)
    mask_shape = [i.value for i in dedx.shape]
    mask = tvm.placeholder(mask_shape, name="mask", dtype='bool')
    res = relu_grad_v2_compute(vadd_res, mask, {}, kernel_name="relu_grad_v2")
    tensor_list = [filter, dedy, vadd_tensor, mask, res]
    with cce():
