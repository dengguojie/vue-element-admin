...
def lcm(wout, factor):
    ...
    return tmp // factor
def conv2d_leaky_relu_add(inputs, weights, bias,fm2_place, offset_w, outputs, strides, pads, dilations=[1, 1, 1, 1],
           groups=1, data_format='NHWC', offset_x=0, kernel_name="conv2d_leaky_relu_add"):
...
#            Co1, block_size_n, block_size_k)
    with tvm.target.cce():
        ...
        leaky_relu_res = leaky_relu_compute(conv_res, None, 0.1) #, negative_slope=0.1
        fm2 = tvm.placeholder(leaky_relu_res.shape, name='fmap2', dtype="float16", attrs={'ori_format': 'NCHW'})
        out =  add_compute(fm2,leaky_relu_res, None)
