...
def conv2d_add(inputs, weights, bias, fm2_place,offset_w, outputs, strides, pads, dilations=[1, 1, 1, 1],
           groups=1, data_format='NHWC', offset_x=0, kernel_name="conv2d_add"):
    ...
    with tvm.target.cce():
        ...
        out = add_compute(fm2,conv_res,None)

#        out = add_compute(conv_res,conv_res, None) #, negative_slope=0.1
        sch = generic.auto_schedule(out)
        #tensor_list = [fm, fm2, filter_w, out]
