...
def conv2d_relu_add(inputs,weights,bias,fm2_place,offset_w,outputs,strides,pads,dilations,groups, data_format, offset_x,kernel_name='conv2d_relu_add_dynamic'):
...
#        weights_fraz = (Cin*filter_h*filter_w, Cout, block_size_n, block_size_k)
        ...
        relu_res = relu(conv_res, relu_outputs, "relu")
        fm2 = tvm.placeholder(relu_res.shape,name="fm2",dtype="float16",attrs={'ori_format': 'NCHW'})
        add_res = add(fm2, relu_res,"add")
        with tvm.target.cce():
