...
def conv2d_dequant_quant_doubleout(inputs,weights,bias,deq_scale,offset_w,outputs,output,strides,pads,dilations,
                                  groups, data_format, offset_x,kernel_name='conv2d_dequant_quant_doubleout_dynamic'):
    ...
    with tbe.dsl.base.operation.compute():
        ...
        if bias:
            bias_tensor = tvm.placeholder(
                bias.get("ori_shape"),name="bias", dtype="int32",
        ...
        quant_res = ascend_quant_compute(dequant_res, None, quant_dict["scale"], quant_dict["offset"], quant_dict["sqrt_mode"])
        out = [dequant_res,quant_res]

        with tvm.target.cce():
            sch = te.utils.cce.auto_schedule(out)
        tensor_list = [input_tensor,weight_tensor,deq_reg]
        tensor_list.extend(out)
        if bias:
            tensor_list = [input_tensor,weight_tensor,bias_tensor,deq_reg]
            tensor_list.extend(out)

        config = {"name": kernel_name,
            "tensor_list": tensor_list,
            "build_args": {"constant_realize_extent_in_infer_bound": False}}
        print("info__________________________",sch,config,tensor_list)
