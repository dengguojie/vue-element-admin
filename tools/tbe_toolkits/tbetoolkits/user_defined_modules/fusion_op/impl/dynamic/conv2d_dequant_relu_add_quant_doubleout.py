...
def conv2d_dequant_relu_add_quant_doubleout(inputs,weights,bias,deq_scale,fm2_place,offset_w,outputs,output,strides,pads,
                            dilations,groups, data_format, offset_x,kernel_name='conv2d_dequant_relu_add_quant_doubleout_dynamic'):
...
#        weights_fraz = (Cin*filter_h*filter_w, Cout, block_size_n, block_size_k)
        ...
        tensor_list = [input_tensor,weight_tensor,deq_reg,fm2]
        tensor_list.extend(out)
        if bias:
            tensor_list = [input_tensor,weight_tensor,bias_tensor,deq_reg,fm2]
            tensor_list.extend(out)
