...
def conv2d_relu(inputs,weights,bias,offset_w,outputs,strides,pads,dilations,groups, data_format, offset_x,kernel_name='conv2d_relu_dynamic'):
    ...
    with tbe.dsl.base.operation.compute():
        ...
        if inputs.get("ori_format") == "NCHW" and \
                inputs.get("format") == "NC1HWC0":
            n_dim = 0
            h_dim = 2
            w_dim = 3
            dynamic_flag = -1
        ...
        if C0 == 16:
            Cout = (Cout + 15) // 16
            # without quant or dequant  C0=16
        ...
        with tvm.target.cce():
            sch = te.utils.cce.auto_schedule(relu_res)

        tensor_list = [input_tensor, weight_tensor, bias_tensor, relu_res]
