...
def conv2d_dequant(inputs,weights,bias,deq_scale,offset_w,outputs,strides,pads,dilations,groups, data_format, offset_x,kernel_name='conv2d_dequant_dynamic'):
    ...
    with tbe.dsl.base.operation.compute():
            ...
            out = ascend_dequant_compute(conv_res, deq_reg, None, sqrt_mode=False, relu_flag=False)
    
            with tvm.target.cce():
                sch = te.utils.cce.auto_schedule(out)
            tensor_list = [input_tensor,weight_tensor,deq_reg,out]
            if bias:
                tensor_list = [input_tensor,weight_tensor,bias_tensor,deq_reg,out]
