...
from impl.dynamic.relu6 import relu6_compute as relu6
...
def conv2d_relu6(inputs,weights,bias,offset_w,outputs,strides,pads,dilations,groups, data_format, offset_x,kernel_name='conv2d_relu6_dynamic'):
...
#        weights_fraz = (Cin*filter_h*filter_w, Cout, block_size_n, block_size_k)
        ...
        relu6_res = relu6(conv_res, relu_outputs, "relu")

        print(conv_res)
        with tvm.target.cce():
            sch = te.utils.cce.auto_schedule(relu6_res)
        tensor_list = [input_tensor, weight_tensor,  relu6_res]
        if bias:
            tensor_list = [input_tensor, weight_tensor, bias_tensor, relu6_res]
