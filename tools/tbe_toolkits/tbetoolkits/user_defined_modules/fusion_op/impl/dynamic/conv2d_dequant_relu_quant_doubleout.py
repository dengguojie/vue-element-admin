...
def conv2d_dequant_relu_quant_doubleout(inputs,weights,bias,deq_scale,offset_w,outputs,output,strides,pads,
                                   dilations,groups, data_format, offset_x,kernel_name='conv2d_dequant_relu_quant_doubleout_dynamic'):
...
#        weights_fraz = (Cin*filter_h*filter_w, Cout, block_size_n, block_size_k)
        ...
        out = [relu_res,out]
        print("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk",out)
        # 待coding

#        tmp_res = tbe.vmaxs(out, tvm.const(0, out.dtype))
#        final_res = tbe.vmins(tmp_res, tvm.const(6, input_x.dtype))
#        out = {'op_placeholder': out}
