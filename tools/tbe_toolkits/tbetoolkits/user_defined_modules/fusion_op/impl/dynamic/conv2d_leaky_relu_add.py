...
def conv2d_leaky_relu_add(inputs,weights,bias,fm2_place,offset_w,outputs,strides,pads,dilations,groups, data_format, offset_x,kernel_name='conv2d_leaky_relu_add_dynamic'):
...
#        weights_fraz = (Cin*filter_h*filter_w, Cout, block_size_n, block_size_k)
        ...
        leakyrelu_res = leakyrelu(conv_res, relu_outputs, 0.1, "leaky_relu")
        fm2 = tvm.placeholder(leakyrelu_res.shape, name='fmap2', dtype="float16", attrs={'ori_format': 'NCHW'})
        add_res = add(fm2,leakyrelu_res, "add")
        with tvm.target.cce():
            sch = te.utils.cce.auto_schedule(add_res)
        tensor_list = [input_tensor, weight_tensor, fm2,add_res]
        if bias:
            tensor_list = [input_tensor, weight_tensor, bias_tensor, fm2,add_res]
