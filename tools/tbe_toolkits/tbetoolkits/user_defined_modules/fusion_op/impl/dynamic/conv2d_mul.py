...
from impl.dynamic.mul import mul_compute as mul
...
def conv2d_mul(inputs,weights,bias,offset_w,outputs,strides,pads,dilations,groups, data_format, offset_x,kernel_name='conv2d_mul_dynamic'):
    mul_outputs={"dtype": "float16"}
    print("###########I,m coming###############")
    print(locals())
...
#        weights_fraz = (Cin*filter_h*filter_w, Cout, block_size_n, block_size_k)
        ...
        # 输入是5轴
        # input_tensor = tvm.placeholder(
        #     inputs.get("shape"), name="fmap", dtype=inputs.get("dtype"),
        #     attrs={'shape':  inputs.get('shape'),
        #            "ori_shape": inputs.get("ori_shape"),
        #            "format": inputs.get("format"),
        #            "ori_format": inputs.get("ori_format"),
        #            "range": inputs.get("range")})
        #
        # weight_tensor = tvm.placeholder(
        #     weights.get("shape"),name="weight_map", dtype=weights.get("dtype"),
        #     attrs={'shape':  weights_fraz,
        #            "ori_shape": weights.get("ori_shape"),
        #            "format": weights.get("format"),
        #            "ori_format": weights.get("ori_format"),
        #            "range": weights.get("range")})
        #
        # bias_tensor = tvm.placeholder(
        #     bias.get("shape"),name="bias", dtype=bias.get("dtype"),
        #     attrs={'shape':  bias.get('shape'),
        #            "ori_shape": bias.get("ori_shape"),
        #            "format": "ND",
        #            "ori_format": "ND"})
        ...
        # Tensor(shape=[batch_n, 4, (16*wo), 16], op.name=bias_mul_vector_cc_1) <tvm.tensor.TensorSlice object at 0x7f164159a1d0>
        # mul_res = mul(conv_res['op_res'][0], mul_outputs, "mul")
        mul_res = mul(conv_res, conv_res, "mul")
        print("ABABABABBBBBBBBBBBBBBBBBBBBBBBBBBB")
        print(conv_res)
        with tvm.target.cce():
            sch = te.utils.cce.auto_schedule(mul_res)
        print("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")
        tensor_list = [input_tensor, weight_tensor, mul_res]
        if bias:
            tensor_list = [input_tensor, weight_tensor, bias_tensor, mul_res]
