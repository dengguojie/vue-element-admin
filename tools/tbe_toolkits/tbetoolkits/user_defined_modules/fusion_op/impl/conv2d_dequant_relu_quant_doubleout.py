...
def conv2d_dequant_relu_quant_doubleout(inputs, weights, bias, deq_scale,offset_w, outputs,output, strides, pads, dilations=[1, 1, 1, 1],
           groups=1, data_format='NHWC', offset_x=0, kernel_name="conv2d_dequant_relu_quant_doubleout"):
    print("stc=====shape_info===============",inputs, weights, bias,offset_w, outputs, strides, pads, dilations,groups)
...
#    c_org_channel = w_shape[1]//groups
    ...
    #in_channel_weight = ((Cin_k + block_size_k - 1) // block_size_k) * block_size_k
    ...
    with tvm.target.cce():
        ...
        dequant_res = ascend_dequant_compute(conv_res, deq_reg, None, sqrt_mode=False, relu_flag=True)
        relu_res = leaky_relu_compute(dequant_res,None) 
        ...
        tensor_list = [fm, filter_w, deq_reg]
        tensor_list.extend(out)

        if bias:
            tensor_list = [fm, filter_w, bias_tensor, deq_reg]
            tensor_list.extend(out)
