...
Status ParseParamsConv3D(const Message* op_src, ge::Operator& op) {
  ...
  OP_LOGI(TbeGetName(op).c_str(), "update filter format success, now is %d", op.GetInputDesc(kPos1).GetFormat());
  ...
  OP_LOGI(TbeGetName(op).c_str(), "update input x format success, now is %d", op.GetInputDesc(kPos0).GetFormat());
  ...
  OP_LOGI(TbeGetName(op).c_str(), "update output y format success, now is %d", op.GetInputDesc(kPos0).GetFormat());
  return SUCCESS;
}
