...
def _check_topk_param_region_sort_shape(output_args):
...
def _check_topk_param_k(input_args):
    data_type = input_args["regions_orig"].dtype
    val_k = input_args["k"]
    if data_type == "float32" and val_k > 3000:
        raise RuntimeError("k > 3000")
    if data_type == "float16" and val_k > 6000:
        raise RuntimeError("k > 6000")


def _check_topk_param(input_args, output_args):
    """
    check topk param
    ...
    _check_topk_param_k(input_args)
    _check_topk_param_region_sort_shape(output_args)


def _set_ub_size_by_product_type(data_type):
    """
...
def _tik_topk_min(tik_inst, val_a, val_b, result_):
    """
    get min val
...
def tik_topk(tik_inst, topk_in, topk_out):
    ...
    NA
    """
    _check_topk_param(topk_in, topk_out)
    data_type = topk_in["regions_orig"].dtype
    _set_ub_size_by_product_type(data_type)
    ...
    with tik_inst.new_stmt_scope():
        _tik_topk_filter_by_score_threshold(
            tik_inst,
            (n_required, topk_in["k"], topk_in["score_threshold"], data_type),
            topk_out)


def _tik_topk_filter_by_score_threshold(tik_inst, filter_input, top_output):
    """
    ...
    vsel_score_ub = tik_inst.Tensor(data_type, (n_required_apply, 8), name="vsel_score_ub", scope=tik.scope_ubuf)

    required_score_ub = tik_inst.Tensor(data_type, (n_required_apply, REGION_SIZE_INFP16),
                                        name="required_score_ub", scope=tik.scope_ubuf)
    tik_inst.data_move(
        required_score_ub[0, 0], top_output["regions_sorted"][top_output["batch_id"], 0, 0],
        sid=0, nburst=1, burst=math.ceil(REGION_SIZE_INBYTE * n_required / 32), src_stride=0, dst_stride=0)
    ...
    if tbe_platform.cce_conf.get_soc_spec("SOC_VERSION") in ("Ascend310", "Ascend910"):
        ...
        zeros_ub = tik_inst.Tensor(data_type, (ceil_num,), name="zeros_ub", scope=tik.scope_ubuf)
        tik_inst.vector_dup(ceil_num, zeros_ub, 0, 1, 1, 8)
        threshold_ub = tik_inst.Tensor(data_type, (16, 8), name="threshold_ub", scope=tik.scope_ubuf)
        tik_inst.vector_dup([0x1010101010101010, 0x1010101010101010], threshold_ub, score_threshold, 1, 1, 8)
        ...
        with tik_inst.for_range(0, n_required_apply // (ceil_num // 8)) as cmp_index:
            cmp_mask = tik_inst.vcmp_gt([0x1010101010101010, 0x1010101010101010],
                                        required_score_ub[cmp_index * (ceil_num // 8), 0], threshold_ub, 1, 1)
            tik_inst.vsel(ceil_num, 0, vsel_score_ub[cmp_index * (ceil_num // 8), 0], cmp_mask,
                          ones_ub, zeros_ub, 1, 1, 1, 1)

        count_dst_ub = tik_inst.Tensor("int32", (8,), name="count_dst_ub", scope=tik.scope_ubuf)
        count_src_ub = tik_inst.Tensor("float16", (16,), name="count_src_ub", scope=tik.scope_ubuf)
        vec_reduce_add_ub = tik_inst.Tensor("float16", (n_required_apply // (ceil_num // 8),),
                                            name="vec_reduce_add_ub", scope=tik.scope_ubuf)
        ...
        _tik_topk_min(tik_inst, k, count_scalar, top_output["proposal_actual_num"])
    else:
        raise RuntimeError("only support Ascend310 or Ascend910!\n")


def _tik_topk_get_mem_out(merge_count, sorting, n_remains, regions_orig, interm):
    mem_out = sorting if n_remains == 0 else interm
    if merge_count % 2 == 0:
        mem_out = sorting if n_remains == 0 else regions_orig
    return mem_out
...
def _tik_topk_split_proposal_group(tik_inst, proposal_store, split_param):
    ...
    """
    idx, mem_ub, sorting, orig, interm = \
        proposal_store[0], proposal_store[1], proposal_store[2], proposal_store[3], proposal_store[4]
    n_regions, n_req, src_pos = split_param[0], split_param[1], split_param[2]
    dest_pos = src_pos

    if n_regions <= HALF_UB_REGION_CAPACITY:
        _tik_topk_merge_sort(tik_inst, (idx, mem_ub, sorting, orig), ([n_regions], n_req, [src_pos], dest_pos))
        return
    ...
    merge_count = 0
    while n_remains > 0:
        split_num = VMS4_ELEMENT_NUM - len(sub_list)
        if n_remains <= HALF_UB_REGION_CAPACITY * split_num:
            n_regions_subtsk = math.ceil(n_remains / split_num)
            n_regions_subtsk = math.ceil(n_regions_subtsk / VBS_SORT_NUM) * VBS_SORT_NUM
            n_required_subtsk = min(n_regions_subtsk, n_req)
            for i in range(split_num):
                subtsk_n_regions = min(n_regions_subtsk, n_remains)
                subtsk_n_required = min(n_required_subtsk, subtsk_n_regions)
                _tik_topk_merge_sort(tik_inst, (idx, mem_ub, mem_in, orig),
                                     ([subtsk_n_regions], subtsk_n_required, [src_pos], dest_pos))
                sub_list.append(dest_pos)
                req_list.append(subtsk_n_required)
                src_pos += subtsk_n_regions
                dest_pos += subtsk_n_required
                n_remains -= subtsk_n_regions
        else:
            tmp_reg = min(HALF_UB_REGION_CAPACITY, n_remains)
            tmp_req = min(n_req, tmp_reg)
            _tik_topk_merge_sort(tik_inst, (idx, mem_ub, mem_in, orig), ([tmp_reg], tmp_req, [src_pos], dest_pos))
            sub_list.append(dest_pos)
            req_list.append(tmp_req)
            src_pos += tmp_reg
            dest_pos += tmp_req
            n_remains -= tmp_reg
        if len(sub_list) == VMS4_ELEMENT_NUM:
            mem_out = _tik_topk_get_mem_out(merge_count, sorting, n_remains, orig, interm)
            _tik_topk_merge_sort_external(tik_inst, (mem_ub, mem_out, mem_in, idx, n_req), (sub_list, req_list, 0))
            mem_in = mem_out
            sub_list.clear()
            req_list.clear()
            sub_list.append(0)
            req_list.append(n_req)
            dest_pos = n_req
            merge_count += 1


def _tik_topk_internal_vbs(tik_inst, vbs_src, dest_pos_ub):
    """
    do vbs
    ...
    region_info_list = [{"offset": 0, "length": 1,
                         "repeat": src_region_num_list[0]}]

    if region_info_list[0]["length"] == 1:
        if n_total_regions % 16 != 0:
            raise RuntimeError("n_total_regions should be multiple of 16.", n_total_regions)
...
def _tik_topk_check_region_info(region_info):
    if region_info["repeat"] < 0:
        raise RuntimeError("repeat < 0")


def _tik_topk_check_region_info_list(region_info_list):
    if len(region_info_list) == 1 and region_info_list[0]["repeat"] == 1:
        return True
    return False


def _tik_topk_internal_vms4(tik_inst, mem_ub, dest_pos_ub, region_info_list):
    """
    do vms4
    ...
    n_vms4 = 0
    while True:
        if _tik_topk_check_region_info_list(region_info_list) is True:
            break
        ...
        while region_info_list:
            ...
            _tik_topk_check_region_info(region_info)
            repeat = region_info_list[0]["repeat"]
            length = region_info_list[0]["length"]
            if (repeat >= VMS4_ELEMENT_NUM) and (length * VMS4_ELEMENT_NUM < MAX_INPUT_LIST_LENGTH):
                region_info_list = _tik_topk_merge_sort_same_subgroup(
                    tik_inst, (mem_ub, dest_pos_ub, src_pos_ub),
                    region_info_list, new_region_info_list)
                continue
            elif repeat >= VMS4_ELEMENT_NUM:
                region_info_list = _tik_topk_merge_sort_reassign_subgroup(
                    tik_inst, (mem_ub, dest_pos_ub, src_pos_ub),
                    region_info_list, new_region_info_list)
                continue

            region_info_list = _tik_topk_merge_sort_tail_subgroup(tik_inst, (mem_ub, dest_pos_ub, src_pos_ub),
                                                                  region_info_list, new_region_info_list)
        region_info_list = new_region_info_list
    return dest_pos_ub


def _tik_topk_merge_sort_valid_src_region_num_list(src_region_num_list, src_pos_list):
...
def _tik_topk_merge_sort(tik_inst, inter_sort_store, inter_split_param):
    """
    sort internal
    ...
    _tik_topk_merge_sort_valid_src_region_num_list(src_region_num_list, src_pos_list)
    ...
    for i in range(n_input_list):
        ...
        tik_inst.data_move(
            ...
            nburst=1, burst=math.ceil(REGION_SIZE_INBYTE * 4 / 32) * (src_region_num_list[i] // 4),
            src_stride=0, dst_stride=0)

    # 2. vbs
    dest_pos_ub = HALF_UB_REGION_CAPACITY if n_input_list == 1 else sum(src_region_num_list)
    region_info_list = _tik_topk_internal_vbs(tik_inst, (mem_ub, src_region_num_list, n_total_regions), dest_pos_ub)
    # 3. vms4
    dest_pos_ub = _tik_topk_internal_vms4(tik_inst, mem_ub, dest_pos_ub, region_info_list)
    ...
    tik_inst.data_move(regions_sorted[batch_id, dest_pos, 0], mem_ub[0, dest_pos_ub, 0], sid=0, nburst=1,
                       burst=math.ceil(REGION_SIZE_INBYTE * 4 / 32) * (nburst_val // 4), src_stride=0, dst_stride=0)


def _tik_topk_merge_sort_same_subgroup(tik_inst, ub_info, region_info_list,
                                       new_region_info_list):
    """
...
def _tik_topk_merge_sort_reassign_subgroup_valid_region_info(region_info):
    if region_info["repeat"] < 0:
        raise RuntimeError("repeat < 0")
...
def _tik_topk_merge_sort_reassign_subgroup(tik_inst, ub_info, region_info_list,
                                           new_region_info_list):
    """
    ...
    region_info = region_info_list[0]
    _tik_topk_merge_sort_reassign_subgroup_valid_region_info(region_info)
...
def _tik_topk_merge_sort_tail_subgroup_valid_region_info(region_info):
    if region_info["repeat"] <= 0:
        raise RuntimeError("repeat <= 0")
    if region_info["repeat"] >= VMS4_ELEMENT_NUM:
        raise RuntimeError("repeat >= VMS4_ELEMENT_NUM")
...
def _tik_topk_merge_sort_tail_subgroup(tik_inst, ub_info, region_info_list, new_region_info_list):
    """
    sort tail sub group
    ...
    region_info = region_info_list[0]
    _tik_topk_merge_sort_tail_subgroup_valid_region_info(region_info)
    ...
    if valid_bit > 1:
        ...
        tik_inst.data_move(dst, src_list[0], sid=0, nburst=1,
                           burst=math.ceil(REGION_SIZE_INBYTE * 4 / 32) * (src_list_lengths[0] // 4),
                           src_stride=0, dst_stride=0)
    new_region_info_list.append(new_region_info)
    return region_info_list


def _tik_topk_get_n_burst(tik_inst, ms_src_list_len_):
    with tik_inst.if_scope((ms_src_list_len_ % 4) > 0):
        n_burst_len = (ms_src_list_len_ // 4 + 1)
    with tik_inst.else_scope():
        n_burst_len = (ms_src_list_len_ // 4)
    return n_burst_len


def _tik_topk_external_out_to_ub_set_value(tik_inst, n_src_list, src_list_rem_, ms_src_list_len_, ms_src_list,
                                           regions_orig, src_pos_, var_idx, list_idx, n_burst_, ms_valid_bit_,
                                           slot_capacity, batch_id):
    for slot_idx in range(n_src_list):
        with tik_inst.if_scope(slot_idx == var_idx):
            _tik_topk_min(tik_inst, slot_capacity, src_list_rem_[list_idx], ms_src_list_len_[slot_idx])
            n_burst_val = _tik_topk_get_n_burst(tik_inst, ms_src_list_len_[slot_idx])
            n_burst_.set_as(n_burst_val)
            ms_valid_bit_.set_as(ms_valid_bit_ + 2 ** slot_idx)
            tik_inst.data_move(ms_src_list[slot_idx], regions_orig[batch_id, src_pos_[list_idx], 0],
                               sid=0, nburst=1, burst=math.ceil(REGION_SIZE_INBYTE * 4 / 32) * n_burst_,
                               src_stride=0, dst_stride=0)


def _tik_topk_external_out_to_ub(tik_inst, data_src, slot_val):
    """
    ...
    var_idx = tik_inst.Scalar("int64", "var_idx", 0)

    for list_idx in range(n_src_list):
        with tik_inst.if_scope(src_list_rem_[list_idx] > 0):
            list_slot_map_[list_idx].set_as(var_idx)
            _tik_topk_external_out_to_ub_set_value(tik_inst, n_src_list, src_list_rem_, ms_src_list_len_, ms_src_list,
                                                   regions_orig, src_pos_, var_idx, list_idx, n_burst_, ms_valid_bit_,
                                                   slot_capacity, batch_id)
            var_idx.set_as(var_idx + 1)

    return ms_valid_bit_, list_slot_map_, n_burst_, ms_src_list


def _tik_top_external_set_slot_data_set_value(tik_inst, n_src_list, src_pos_, src_list_rem_, list_slot_map_,
                                              num_exhausted_, slot_idx):
    for list_idx in range(n_src_list):
        with tik_inst.if_scope(list_slot_map_[list_idx] == slot_idx):
            src_pos_[list_idx].set_as(src_pos_[list_idx] + num_exhausted_[slot_idx])
            src_list_rem_[list_idx].set_as(src_list_rem_[list_idx] - num_exhausted_[slot_idx])


def _tik_top_external_set_slot_data(tik_inst, src_list_set):
    """
    ...
    for slot_idx in range(n_src_list):
        value = (0x01 << slot_idx)
        with tik_inst.if_scope(ms_valid_bit_ & value):
            n_selected_.set_as(n_selected_ + num_exhausted_[slot_idx])
            _tik_top_external_set_slot_data_set_value(
                tik_inst, n_src_list, src_pos_, src_list_rem_, list_slot_map_, num_exhausted_, slot_idx)
    return n_selected_


def _tik_top_external_ub_to_out(tik_inst, sorted_data, move_data_param):
    """
    ...
    _tik_topk_min(tik_inst, n_selected_, n_total_rem_, n_selected_)
...
def _tik_topk_merge_sort_external(tik_inst, data_store, sub_list):
    """
    external sort
    ...
    max_iteration = math.ceil(n_required / int(min(slot_capacity, min_input_length)))
    ...
    with tik_inst.for_range(0, max_iteration):
        with tik_inst.if_scope(n_total_selected_ < n_required):
            ...
            ms_valid_bit_, list_slot_map_, n_burst_, ms_src_list = \
                _tik_topk_external_out_to_ub(tik_inst, (regions_orig, n_src_list, src_list_rem_, slot_capacity,
            ...
            with tik_inst.if_scope(ms_valid_bit_ > 0):
                ...
                vms4_sr = tik_inst.vmrgsort4(ms_dest, ms_src_list, element_lengths=ms_src_list_len_,
                                             if_exhausted_suspension=True, valid_bit=ms_valid_bit_, repeat_times=1)

                # Step-3: Move result from UB to OUT
                n_selected_ = _tik_top_external_set_slot_data(
                    tik_inst, (vms4_sr, n_src_list, num_exhausted_, n_selected_,
                               ms_valid_bit_, list_slot_map_, src_pos_, src_list_rem_))
                _tik_top_external_ub_to_out(
                    tik_inst, (regions_sorted, batch_id, dest_pos, ms_dest),
                    (dest_pos_, n_selected_, n_total_rem_, n_burst_, n_total_selected_))
