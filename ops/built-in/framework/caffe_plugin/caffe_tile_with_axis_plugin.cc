...
/*!
 * \file caffe_tile_with_axis_plugin.cpp
 * \brief
 */
...
Status ParseParamsTileWithAxis(const Message* op_src, ge::Operator& op_dst) {
  ...
  if (layer == nullptr) {
    OP_LOGI("TileWithAxis", "[TileWithAxis_Plugin] cast op_src to LayerParameter failed.");
    return FAILED;
  }
  ...
  if (param.has_axis()) {
    ...
    op_dst.SetAttr("axis", static_cast<int64_t>(1));
  }

  if (param.has_tiles()) {
    op_dst.SetAttr("tiles", static_cast<int64_t>(param.tiles()));
  }
