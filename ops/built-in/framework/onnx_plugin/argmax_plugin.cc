...
Status ParseParamsArgMax(const Message* op_src, ge::Operator& op_dest) {
  ...
  for (const auto& attr : node->attribute()) {
    ...
    if (attr.name() == "select_last_index" && attr.type() == ge::onnx::AttributeProto::INT && attr.i() == 1) {
      ONNX_PLUGIN_LOGE(op_dest.GetName().c_str(), "Only support select_last_index=0, but 1 is obtained now");
      return FAILED;
    }
  }
...
static Status ParseOpToGraphArgMax(const Operator& op, Graph& graph) {
  ...
  auto ArgMaxV2 = op::ArgMaxV2().set_input_x(data0)
  ...
  if (keep_dims == 1) {
    ...
    auto expandDims = op::ExpandDims().set_input_x(ArgMaxV2).set_input_axis(data1);
