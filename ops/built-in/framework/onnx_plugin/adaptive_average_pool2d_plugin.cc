...
Status ParseParamsAdaptiveAvgPool2d(const Message* op_src, ge::Operator& op_dest) {
  ...
  if (node == nullptr) {
    OP_LOGE("AdaptiveAvgPool2d", "Dynamic cast op_src to NodeProto failed.");
    return FAILED;
  }
  ...
  for (const auto& attr : node->attribute()) {
    if (attr.name() == "output_size" && attr.type() == ge::onnx::AttributeProto::INTS) {
      if (attr.ints_size() == 2) {
        ...
        OP_LOGE("AdaptiveAvgPool2d", "length of output_size must be 2.");
      }
  ...
  if (set_output_size_flag) {
    ...
    OP_LOGE("AdaptiveAvgPool2d", "onnx AdaptiveAvgPool2d op has no output_size attr.");
  }
  return SUCCESS;
}
