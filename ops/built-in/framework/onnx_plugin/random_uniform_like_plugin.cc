...
Status ParseParamsRandomUniformLike(const Message* op_src, ge::Operator& op_dest) {
  ...
  if (node == nullptr) {
    OP_LOGE("ParseParamsRandomUniformLike", "Dynamic cast op_src to NodeProto failed.");
    return FAILED;
  }
...
Status ParseOpToGraphRandomUniformLike(const ge::Operator &op, ge::Graph &graph) {
  ...
  if (kvlist.find(dtype) == kvlist.end()){
    OP_LOGE("RandomUniformLike", "only support float32/float16/double, but got %d", dtype);
    return FAILED;
  }
  ...
  ge::Tensor tensor_mean(tensorDesc, reinterpret_cast<uint8_t*>(&low), sizeof(float));
  ge::Tensor tensor_scale(tensorDesc, reinterpret_cast<uint8_t*>(&delta), sizeof(float));
