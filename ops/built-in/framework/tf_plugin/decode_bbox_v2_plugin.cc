...
Status DecodeBboxV2Params(const std::vector<const google::protobuf::Message*> insideNodes, ge::Operator& op) {
  ...
  for (auto node : insideNodes) {
    ...
    OP_LOGD(TbeGetName(op).c_str(), "DecodeBoxV2 NodeDef is %s ", node_def->name().c_str());
  ...
  if (!ge::AttrUtils::SetListFloat(opDesc, "scales", scales_list)) {
    OP_LOGE(TbeGetName(op).c_str(), "Set scales_list failed.");
    return FAILED;
  }
  OP_LOGD(TbeGetName(op).c_str(), "DecodeBboxV2's attr scales is [%.1f, %.1f, %.1f, %.1f]", scale, scale_1, scale_2,
          scale_3);
