...
Status DynamicLSTMParserParams(const std::vector<const google::protobuf::Message*> inside_nodes, ge::Operator& op) {
  ...
  ge::GeTensorDesc input_desc = op_desc->GetInputDesc(wInputPosition);
  input_desc.SetOriginFormat(ge::FORMAT_HWCN);
  input_desc.SetFormat(ge::FORMAT_HWCN);

  if (op_desc->UpdateInputDesc(wInputPosition, input_desc) != ge::GRAPH_SUCCESS) {
    OP_LOGE(TbeGetName(op).c_str(), "Update input desc fail, index:%u.", wInputPosition);
    return FAILED;
  }
