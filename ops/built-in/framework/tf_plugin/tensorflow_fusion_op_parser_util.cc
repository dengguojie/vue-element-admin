    ...
    if (attr_value.has_list()) {                                                                              \
      if (attr_value.list().name##_size() > 0) {                                                              \
        if (type != "list(" type_string ")") {                                                                \
          OP_LOGE("GeAttrValue had value with type 'list(" type_string ")'when '%s' expected", type.c_str()); \
          return FAILED;                                                                                      \
        }                                                                                                     \
        ++num_set;                                                                                            \
      }                                                                                                       \
      ...
      if (type != type_string) {                                                                              \
        OP_LOGE("GeAttrValue had value with type '" type_string "' when '%s' expected", type.c_str());        \
        return FAILED;                                                                                        \
      }                                                                                                       \
      ++num_set;                                                                                              \
    }                                                                                                         \
...
#undef VALIDATE_FIELD
  ...
  if (attr_value.value_case() == domi::tensorflow::AttrValue::kPlaceholder) {
    OP_LOGE("GeAttrValue had value with unexpected type 'placeholder'");
    return FAILED;
  }
