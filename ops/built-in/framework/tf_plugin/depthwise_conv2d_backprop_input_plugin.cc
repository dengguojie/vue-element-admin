...
Status DepthwiseConv2DBackpropInputMappingFn(const ge::Operator& op_src, ge::Operator& op) {
  ...
  CHECK(op_dsc == nullptr, CUBE_CALL_ERR_REPORT(TbeGetName(op), "GetOpDescFromOperator got nullptr failed."),
        return FAILED);
  auto tensor_desc_w = op_dsc->MutableInputDesc("filter");
  tensor_desc_w->SetOriginFormat(ge::FORMAT_HWCN);
  tensor_desc_w->SetFormat(ge::FORMAT_HWCN);
  OP_LOGD(TbeGetName(op), "Update filter format success.");
