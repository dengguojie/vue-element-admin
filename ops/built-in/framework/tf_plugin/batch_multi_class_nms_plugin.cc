...
Status BatchMultiClassNonMaxSuppressionParams(const std::vector<const google::protobuf::Message *> insideNodes, ge::Operator &op) {
    ...
    for (auto node : insideNodes) {
        ...
        if ((scoreConstNodeName == "") && (nodeDef->op() == "Greater") && 
           ((nodeName.find(scoreRetinanetConstKey) != std::string::npos) || (nodeName.find(scoreConstKey) != std::string::npos))) {
        ...
        if (((nodeName.find(sizeConstKey) != std::string::npos) || (nodeName.find(sizeRetinanetConstKey) != std::string::npos))
            && (nodeDef->op() == "Minimum")) {
        ...
        if ((sizeOutputConstNodeName == "") && (nodeDef->op() == nms_v3) && (nodeName.find(retinanet_out_num) != std::string::npos)) {
