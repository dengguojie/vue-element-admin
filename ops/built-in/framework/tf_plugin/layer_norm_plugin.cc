...
Status LayerNormParserParams(const std::vector<const google::protobuf::Message*> inside_nodes, ge::Operator& op) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter layer norm fusion parser.");
  ...
  for (auto inside_node : inside_nodes) {
    ...
    if (node_def->op() == "Mean") {
      if (node_def->input().size() < kMeanInputSize) {
        OP_LOGE(TbeGetName(op).c_str(), "Input size of node mean is invalid, which is %d.", node_def->input().size());
        return FAILED;
      }
  ...
  if (mean_const_input_node_name.empty()) {
    OP_LOGE(TbeGetName(op).c_str(), "cann't find Mean op node in layernorm");
    return FAILED;
  }
  ...
  OP_LOGI(TbeGetName(op).c_str(), "begin_norm_axis = %d, begin_params_axis= %d", begin_norm_axis, begin_params_axis);

  if (!ge::AttrUtils::SetInt(op_desc, "begin_norm_axis", begin_norm_axis)) {
    OP_LOGE(TbeGetName(op).c_str(), "Set begin_norm_axis failed");
    return PARAM_INVALID;
  }
  if (!ge::AttrUtils::SetInt(op_desc, "begin_params_axis", begin_params_axis)) {
    OP_LOGE(TbeGetName(op).c_str(), "Set begin_params_axis failed");
    return PARAM_INVALID;
  }
