...
void ScopeToAbsoluteBBoxPass::GenerateFusionResult(const vector<Scope *> &scopes,
...
string ScopeToAbsoluteBBoxPass::to_string(const vector<Scope *> &scopes) const {
  string result;
  for (auto &scope : scopes) {
    result += scope->Name();
    result += " ";
...
bool ScopeToAbsoluteBBoxPass::MatchedSubScopes(const Scope *root_scope) const {
  ...
  const vector<string> scopes2check = {
      "while/", "ToAbsoluteCoordinates/", "Scale/"
  };
