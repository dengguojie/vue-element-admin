...
namespace ge {
namespace {
const char *const kScopeType = "FastrcnnPredictions";
const char *const kScopeTypeBoolmask = "boolean_mask";
const char *const kScopeTypeMap = "map";
const char *const kScopeTypeFastrcnnPredictions = "FastrcnnPredictions";
const char *const kOpType = "FastrcnnPredictions";
}  // namespace
...
/*
  ...
  ScopeFusionPatterns patterns_boolmask;
  ScopeFusionPatterns patterns_map;
  GenBoolmaskScopePatterns(patterns_boolmask);  // match batchnorm Scope
  GenMapScopePatterns(patterns_map);      // match moments Scope
  patterns_list.push_back(patterns_boolmask);
  patterns_list.push_back(patterns_map);
  return patterns_list;*/
}

std::string ScopeFastrcnnPredictionsPass::PassName() { return std::string("ScopeFastrcnnPredistionsPass"); }
...
/*
void ScopeFastrcnnPredictionsPass::GenBoolmaskScopePatterns(ScopeFusionPatterns &patterns) {
  // boolean mask
  vector<ScopePattern *> batch;
  ScopePattern *boolmask_cell = new (std::nothrow) ScopePattern();
  if (boolmask_cell == nullptr) {
    OP_LOGE(kOpType, "Alloc an object failed.");
    return;
  }
  boolmask_cell->SetSubType(kScopeTypeBoolmask);
  boolmask_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("ConcatV2", 1));    // ConcatV2 num is 1
  boolmask_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("Squeeze", 1));  // Squeeze num is 1
  boolmask_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("Where", 1));  // Where num is 1
  boolmask_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("Reshape", 2));  // Reshape num is 1
  boolmask_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("Shape", 3));    // Shape num is 3
  boolmask_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("Gather", 1));    // Gather num is 1
  boolmask_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("StridedSlice", 3));    // StridedSlice num is 3
  boolmask_cell->AddScopeFeature(ScopeFeature("", -1, "boolean_mask"));

  batch.push_back(boolmask_cell);
  patterns.push_back(batch);
  OP_LOGI(kOpType, "Add GenBoolmaskScopePatterns boolmask_cell.");
}

void ScopeFastrcnnPredictionsPass::GenMapScopePatterns(ScopeFusionPatterns &patterns) {
  // boolean mask
  vector<ScopePattern *> batch;
  ScopePattern *map_cell = new (std::nothrow) ScopePattern();
  if (map_cell == nullptr) {
    OP_LOGE(kOpType, "Alloc an object failed.");
    return;
  }
  map_cell->SetSubType(kScopeTypeMap);
  map_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("NonMaxSuppressionV2", 1));  // NonMaxSuppressionV2 num is 1
  map_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("TopKV2", 1));  // TopKV2 num is 1
  map_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("Less", 1));  // Less num is 1
  map_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("LoopCond", 1));  // LoopCond num is 1
  map_cell->AddNodeOpTypeFeature(NodeOpTypeFeature("Where", 1));  // LoopCond num is 1

  map_cell->AddScopeFeature(ScopeFeature("", -1, "map"));

  batch.push_back(map_cell);
  patterns.push_back(batch);
  OP_LOGI(kOpType, "Add GenMapScopePatterns map_cell.");
}
*/
...
void ScopeFastrcnnPredictionsPass::GenScopePatterns(ScopeFusionPatterns &patterns) {
  ...
  fastPredictionPattern->AddNodeOpTypeFeature(NodeOpTypeFeature("NonMaxSuppressionV2", 1));  // NonMaxSuppressionV2 num is 1
...
void ScopeFastrcnnPredictionsPass::GenerateFusionResult(const std::vector<Scope *> &scopes,
  ...
  for (auto &scope : scopes) {
	...
	fusion_rlt->InsertInputs("fastrcnn_predictions/strided_slice", {1, kFusionDisableIndex, kFusionDisableIndex, kFusionDisableIndex});	  // Input fastrcnn_all_probs
