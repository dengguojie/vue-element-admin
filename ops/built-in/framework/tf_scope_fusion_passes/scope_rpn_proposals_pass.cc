...
/*!
 * \file scope_rpn_proposals_pass.cc
 * \brief
 */
...
void ScopeRpnProposalsPass::GenScopePatterns(ScopeFusionPatterns& patterns) {
  ...
  rpnProposalsPattern->AddNodeOpTypeFeature(
      NodeOpTypeFeature("NonMaxSuppressionV2", 1, 0));  // The number of NonMaxSuppressionV2 is 1.
  rpnProposalsPattern->AddNodeOpTypeFeature(NodeOpTypeFeature("TopKV2", 1, 0));  // The number of TopKV2 is 1.
  rpnProposalsPattern->AddNodeOpTypeFeature(
      NodeOpTypeFeature("Where", 0, 4));  // The number of Where is a multiple of 2.  actual 4
  rpnProposalsPattern->AddNodeOpTypeFeature(
      NodeOpTypeFeature("Gather", 0, 6));  // The number of Gather is a multiple of 2. actual  6

  rpnProposalsPattern->AddNodeOpTypeFeature(
      NodeOpTypeFeature("ExpandDims", -1, 0));                                    // The ExpandDims op is not included
...
Status ScopeRpnProposalsPass::LastMatchScopesAndOPs(std::shared_ptr<ScopeGraph>& scopeGraph,
  ...
  for (auto& scope : scopes) {
    if (scope->SubType() == kRpnProposalsType && scope->Name().find("generate_rpn_proposals") != std::string::npos) {
