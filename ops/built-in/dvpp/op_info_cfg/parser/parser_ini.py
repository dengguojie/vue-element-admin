    ...
    with open(ini_file) as ini_file:
        ...
        for line in lines:
            ...
            if line.startswith("["):
                ...
                arr_num = len(key1.split("."))
                if arr_num == 2:
                    key1_0, key1_1 = key1.split(".")
                    if not key1_0 in op:
                        op[key1_0] = {}
                    if key1_1 in op.get(key1_0):
                        raise RuntimeError("Op:" + op_name + " " + key1_0 + " " + key1_1 + " is repeated!")
                    op[key1_0][key1_1] = key2
                elif arr_num == 3:
                    key1_0, key1_1, key1_2 = key1.split(".")
                    if not key1_0 in op:
                        op[key1_0] = {}
                    if not key1_1 in op.get(key1_0):
                        op[key1_0][key1_1] = {}
                    if key1_2 in op.get(key1_0).get(key1_1):
                        raise RuntimeError("Op:" + op_name + " " + key1_0 + " " + key1_1 + " " + key1_2 +
                                           " is repeated!")
                    op[key1_0][key1_1][key1_2] = key2
