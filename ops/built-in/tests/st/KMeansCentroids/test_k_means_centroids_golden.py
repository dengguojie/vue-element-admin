...
def deal_with_large_data(samples, centroids, sum_square_centroids, sum_square_samples, use_actual_distance):
    """ golden data
    """
    m = samples.shape[0]
    n, d = centroids.shape
...
def calc_expect_func(x, y, sum_square_y, sum_square_x,
                     segment_sum, segment_count, kmean_total_distance, use_actual_distance):
    """ golden data
    """
    shape_x = x.get("shape")
    shape_y = y.get("shape")
    ...
    if m >= 65536 and n >= 1024:
        return deal_with_large_data(data_x, data_y, data_sum_square_y, data_sum_square_x, use_actual_distance)

    matmul_res = np.matmul(data_x, data_y.T).astype("float32")

    np_distance = -2 * matmul_res
    if use_actual_distance:
        np_distance += data_sum_square_x
