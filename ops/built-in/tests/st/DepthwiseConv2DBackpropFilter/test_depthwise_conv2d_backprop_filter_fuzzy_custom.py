...
def test_depthwise_conv2d_backprop_filter_fuzz_build_static():
    input_list = [
        {'shape': (1, 1, 8, 3051, 16), 'ori_shape': (1, 8, 3051, 1), 'ori_format': 'NHWC', 'format': 'NC1HWC0', 'dtype': 'float16'},
        ...
        'test_depthwise_conv2d_backprop_filter_generalization_static_mode_general_case', {"mode": "keep_rank"}
    ]
    with tbe.common.context.op_context.OpContext("dynamic"):
        op_info = operator_info.OpInfo("DepthwiseConv2dBackpropFilter", "DepthwiseConv2dBackpropFilter")
        tbe.common.context.op_context.get_context().add_op_info(op_info)
        depthwise_conv2d_backprop_filter_generalization(*input_list)

def test_depthwise_conv2d_backprop_filter_fuzz_build_dynamic():
    input_list = [
        ...
        'test_depthwise_conv2d_backprop_filter_generalization_dynamic_mode_general_case', {"mode": "keep_rank"}
    ]
    with tbe.common.context.op_context.OpContext("dynamic"):
        op_info = operator_info.OpInfo("DepthwiseConv2dBackpropFilter", "DepthwiseConv2dBackpropFilter")
        tbe.common.context.op_context.get_context().add_op_info(op_info)
...
if __name__ == '__main__':
    test_depthwise_conv2d_backprop_filter_fuzz_build_static()
    test_depthwise_conv2d_backprop_filter_fuzz_build_dynamic()