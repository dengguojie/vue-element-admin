...
def generic_fresnel_asymp(x):
    ...
    res_1 = np.multiply(data_f, data_s)
    res_2 = np.multiply(data_g, data_c)
    res_sub = np.subtract(res_1, res_2)
    res_3 = np.multiply(res_sub, data_y)
    res = np.add(0.5, res_3)
    return res
...
def _generic_fresnel_cos_interval(x):
    ...
    data_cn_polevl = polevl(tmp_x4, Constant.COEF_CN, Constant.CN_COUNT)
    data_cd_polevl = polevl(tmp_x4, Constant.COEF_CD, Constant.CD_COUNT)
    res = np.multiply(x, data_cn_polevl)
    res = np.true_divide(res, data_cd_polevl)
...
def calc_expect_func(x, y):
    ...
    for i in np.nditer(input_x, op_flags=['readwrite']):
        ...
        if i > Constant.THRESHOLD_B:
            ...
            res = _generic_fresnel_cos_interval(i)
        else:
