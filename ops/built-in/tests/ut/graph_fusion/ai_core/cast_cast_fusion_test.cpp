...
TEST_F(cast_cast_fusion_test, cast_cast_fusion_test_3) {
    ge::Graph graph("cast_cast_fusion_test_3");
    ...
    for (auto node: compute_graph_ptr->GetAllNodes()) {
        if (node->GetType() == "Cast") {
            ...
            DataType castInputDataType = castInputTensor.GetDataType();
            DataType castOutputDataType = castOutputTensor.GetDataType();
            if (castInputDataType == ge::DT_INT32 && castOutputDataType == ge::DT_FLOAT16) {
                findFusionCast = true;
            }
        }
    }
