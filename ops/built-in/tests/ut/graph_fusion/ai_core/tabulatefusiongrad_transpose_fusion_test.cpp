...
class tabulatefusiongrad_transpose_fusion_test : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "tabulatefusiongrad_transpose_fusion_test SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "tabulatefusiongrad_transpose_fusion_test TearDown" << std::endl;
  }
};
...
TEST_F(tabulatefusiongrad_transpose_fusion_test, tabulatefusiongrad_transpose_fusion_test_01) {
  std::string testCaseName = "tabulatefusiongrad_transpose_fusion_test_01";
  int64_t nloc = 8192;
  int64_t nnei = 92;
  ...
  auto dy = CreateDataNode("dy", {nloc, 4, last_layer_size}, FORMAT_ND, DT_FLOAT);
  auto descriptor = CreateDataNode("descriptor", {nloc, 4, last_layer_size}, FORMAT_ND, DT_FLOAT);
  
  std::string tabulatefusiongradOpName = "TabulateFusionGrad_01";
  auto tabulatefusiongradOp = TabulateFusionGrad(tabulatefusiongradOpName.c_str());
  tabulatefusiongradOp.set_input_table(table)
      .set_input_table_info(table_info)
      .set_input_em_x(em_x)
      .set_input_em(em)
      .set_input_dy(dy)
      .set_input_descriptor(descriptor);
  tabulatefusiongradOp.update_output_desc_dy_dem_x(SimpleTensorDesc("dy_dem_x", {nloc, nnei}, FORMAT_ND, DT_FLOAT));
  tabulatefusiongradOp.update_output_desc_dy_dem(SimpleTensorDesc("dy_dem", {nloc, nnei, 4}, FORMAT_ND, DT_FLOAT));
  
  std::vector<Operator> inputs{table_info, em_x, em, dy, descriptor};
  std::vector<Operator> outputs{tabulatefusiongradOp};
  ...
  fe::FusionPassTestUtils::RunGraphFusionPass("ATabulateFusionGradTransposeFusionPass", fe::BUILT_IN_GRAPH_PASS,
                                              *computeGraph);
  ...
  for (auto iNode : computeGraph->GetAllNodes()) {
    if (iNode->GetType() == "TabulateFusionGrad") {
      ...
      ge::GeTensorDesc inputTable = iOpDesc->GetInputDesc(0);
      ge::GeShape inputTableShape = inputTable.GetShape();
      EXPECT_EQ(inputTableShape.GetDim(1), 100 * 6);
    }
  }
}
