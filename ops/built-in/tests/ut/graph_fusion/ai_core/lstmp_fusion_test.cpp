...
TEST_F(lstmp_fusion_test, lstmp_fusion_test_3) {
    ...
    auto xData = op::Data("xData");
    std::vector<int64_t> dims_x{16,1,16};
    ge::Shape shape_x(dims_x);
    ge::TensorDesc tensorDescX(shape_x, FORMAT_ND, DT_FLOAT);
    xData.update_input_desc_x(tensorDescX);
    xData.update_output_desc_y(tensorDescX);
...
TEST_F(lstmp_fusion_test, lstmp_fusion_test_4) {
    ge::Graph graph("lstmp_fusion_test");

    auto xData = op::Data("xData");
    std::vector<int64_t> dims_x{16,8,16};
    ge::Shape shape_x(dims_x);
    ge::TensorDesc tensorDescX(shape_x, FORMAT_ND, DT_FLOAT);
    xData.update_input_desc_x(tensorDescX);
    xData.update_output_desc_y(tensorDescX);
    ...
    for (auto node: compute_graph_ptr->GetAllNodes()) {
        ...
        else if (node->GetType() == "SplitV") {
            findSplitV = true;
        }
    }
    EXPECT_EQ(findDynamicRNNV3, true);
    EXPECT_EQ(findSplitV, true);
}
