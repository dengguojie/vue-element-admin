...
#include "buffer_fusion/ub_fusion/ai_core/pooling/tbe_antiquant_maxpooling_fusion_pass.h"
#undef protected
#undef private
using namespace ge;

class TbeAntiquantMaxpoolingFusionTest : public testing::Test {
protected:
  static void SetUpTestCase() {
    std::cout << "Tbe antiquant maxpooling fusion pass SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "Tbe antiquant maxpooling fusion pass TearDown" << std::endl;
  }
};

TEST_F(TbeAntiquantMaxpoolingFusionTest, TbeAntiquantMaxpoolingFusionTest_1) {
  OpDescPtr antiquant_op = std::make_shared<OpDesc>("antiquant", "AscendAntiQuant");
  ...
  antiquant_op->AddInputDesc("x", tensor_desc);
  antiquant_op->AddOutputDesc("y", tensor_desc);
  ...
  NodePtr antiquant_node = graph->AddNode(antiquant_op);
  NodePtr pool_node = graph->AddNode(pool_op);
  NodePtr quant_node = graph->AddNode(quant_op);
  ge::GraphUtils::AddEdge(antiquant_node->GetOutDataAnchor(0), pool_node->GetInDataAnchor(0));
  ge::GraphUtils::AddEdge(pool_node->GetOutDataAnchor(0), quant_node->GetInDataAnchor(0));

  fe::AntiquantMaxpoolingFusionPass fusion_pass;
  vector<fe::BufferFusionPattern*> patterns = fusion_pass.DefinePatterns();
  
  fe::BufferFusionOpDesc antiquant_desc;
  antiquant_desc.desc_name = "antiquant";
  const fe::BufferFusionOpDesc *antiquant_desc_ptr = &antiquant_desc;
  fe::BufferFusionOpDesc pool_desc;
  pool_desc.desc_name = "maxpooling";
  ...
  vector<NodePtr> antiquant_nodes = {antiquant_node};
  mapping.emplace(antiquant_desc_ptr, antiquant_nodes);
  ...
  vector<NodePtr> fusion_nodes = {antiquant_node, pool_node, quant_node};
