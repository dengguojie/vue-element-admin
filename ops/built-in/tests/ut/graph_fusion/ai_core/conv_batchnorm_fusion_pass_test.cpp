...
TEST_F(conv_batchnorm_fusion_pass_test, biasadd_conv_fusion_test_2) {
  OpDescPtr data = std::make_shared<OpDesc>("DATA0", "Data");
  OpDescPtr filter_const = std::make_shared<OpDesc>("filter_const", "Const");
  OpDescPtr scale_const = std::make_shared<OpDesc>("scale_const", "Const");
  OpDescPtr offset_const = std::make_shared<OpDesc>("offset_const", "Const");
  ...
  vector<int64_t> input_dim = {10, 320, 320, 3};
  GeShape input_shape(input_dim);
  GeTensorDesc input_tenosr_desc(input_shape, FORMAT_NHWC, DT_FLOAT);
  input_tenosr_desc.SetOriginFormat(FORMAT_NHWC);
  input_tenosr_desc.SetOriginDataType(DT_FLOAT);
  input_tenosr_desc.SetOriginShape(input_shape);

  vector<int64_t> output_dim = {10, 320, 320, 3};
  GeShape output_shape(output_dim);
  GeTensorDesc output_tenosr_desc(output_shape, FORMAT_NHWC, DT_FLOAT);
  output_tenosr_desc.SetOriginFormat(FORMAT_NHWC);
  output_tenosr_desc.SetOriginDataType(DT_FLOAT);
  output_tenosr_desc.SetOriginShape(output_shape);

  vector<int64_t> filter_dim = {40, 3, 3, 3};
  GeShape filter_shape(filter_dim);
  GeTensorDesc filter_tenosr_desc(filter_shape, FORMAT_NHWC, DT_FLOAT);
  filter_tenosr_desc.SetOriginFormat(FORMAT_NHWC);
  filter_tenosr_desc.SetOriginDataType(DT_FLOAT);
  filter_tenosr_desc.SetOriginShape(filter_shape);

  vector<int64_t> const_dim = {40, 3, 3, 3};
  GeShape const_shape(const_dim);
  GeTensorDesc const_tenosr_desc(const_shape, FORMAT_NHWC, DT_FLOAT);
  const_tenosr_desc.SetOriginFormat(FORMAT_NHWC);
  const_tenosr_desc.SetOriginDataType(DT_FLOAT);
  const_tenosr_desc.SetOriginShape(const_shape);
  ...
  ge::AttrUtils::SetListInt(conv, "dilations", {1, 1, 1, 1});
  ge::AttrUtils::SetListInt(conv, "pads", {1, 1, 1, 1});
  ge::AttrUtils::SetListInt(conv, "strides", {1, 2, 2, 1});
...
TEST_F(conv_batchnorm_fusion_pass_test, biasadd_conv_fusion_test_3) {
  OpDescPtr data = std::make_shared<OpDesc>("DATA0", "Data");
  OpDescPtr filter_const = std::make_shared<OpDesc>("filter_const", "Const");
  OpDescPtr scale_const = std::make_shared<OpDesc>("scale_const", "Const");
  OpDescPtr offset_const = std::make_shared<OpDesc>("offset_const", "Const");
  ...
  vector<int64_t> input_dim = {320, 320, 3, 10};
  GeShape input_shape(input_dim);
  GeTensorDesc input_tenosr_desc(input_shape, FORMAT_HWCN, DT_FLOAT);
  input_tenosr_desc.SetOriginFormat(FORMAT_HWCN);
  input_tenosr_desc.SetOriginDataType(DT_FLOAT);
  input_tenosr_desc.SetOriginShape(input_shape);

  vector<int64_t> output_dim = {320, 320, 3, 10};
  GeShape output_shape(output_dim);
  GeTensorDesc output_tenosr_desc(output_shape, FORMAT_HWCN, DT_FLOAT);
  output_tenosr_desc.SetOriginFormat(FORMAT_HWCN);
  output_tenosr_desc.SetOriginDataType(DT_FLOAT);
  output_tenosr_desc.SetOriginShape(output_shape);

  vector<int64_t> filter_dim = {3, 3, 3, 40};
  GeShape filter_shape(filter_dim);
  GeTensorDesc filter_tenosr_desc(filter_shape, FORMAT_HWCN, DT_FLOAT);
  filter_tenosr_desc.SetOriginFormat(FORMAT_HWCN);
  filter_tenosr_desc.SetOriginDataType(DT_FLOAT);
  filter_tenosr_desc.SetOriginShape(filter_shape);

  vector<int64_t> const_dim = {3, 3, 3, 40};
  GeShape const_shape(const_dim);
  GeTensorDesc const_tenosr_desc(const_shape, FORMAT_HWCN, DT_FLOAT);
  const_tenosr_desc.SetOriginFormat(FORMAT_HWCN);
  const_tenosr_desc.SetOriginDataType(DT_FLOAT);
  const_tenosr_desc.SetOriginShape(const_shape);
  ...
  ge::AttrUtils::SetListInt(conv, "dilations", {1, 1, 1, 1});
  ge::AttrUtils::SetListInt(conv, "pads", {1, 1, 1, 1});
  ge::AttrUtils::SetListInt(conv, "strides", {2, 2, 1, 1});
...
TEST_F(conv_batchnorm_fusion_pass_test, biasadd_conv_fusion_test_4) {
  OpDescPtr data = std::make_shared<OpDesc>("DATA0", "Data");
  OpDescPtr filter_const = std::make_shared<OpDesc>("filter_const", "Const");
  OpDescPtr scale_const = std::make_shared<OpDesc>("scale_const", "Const");
  OpDescPtr offset_const = std::make_shared<OpDesc>("offset_const", "Const");
  ...
  vector<int64_t> input_dim = {3, 320, 320, 10};
  GeShape input_shape(input_dim);
  GeTensorDesc input_tenosr_desc(input_shape, FORMAT_CHWN, DT_FLOAT);
  input_tenosr_desc.SetOriginFormat(FORMAT_CHWN);
  input_tenosr_desc.SetOriginDataType(DT_FLOAT);
  input_tenosr_desc.SetOriginShape(input_shape);

  vector<int64_t> output_dim = {3, 320, 320, 10};
  GeShape output_shape(output_dim);
  GeTensorDesc output_tenosr_desc(output_shape, FORMAT_CHWN, DT_FLOAT);
  output_tenosr_desc.SetOriginFormat(FORMAT_CHWN);
  output_tenosr_desc.SetOriginDataType(DT_FLOAT);
  output_tenosr_desc.SetOriginShape(output_shape);

  vector<int64_t> filter_dim = {3, 3, 3, 40};
  GeShape filter_shape(filter_dim);
  GeTensorDesc filter_tenosr_desc(filter_shape, FORMAT_CHWN, DT_FLOAT);
  filter_tenosr_desc.SetOriginFormat(FORMAT_CHWN);
  filter_tenosr_desc.SetOriginDataType(DT_FLOAT);
  filter_tenosr_desc.SetOriginShape(filter_shape);

  vector<int64_t> const_dim = {3, 3, 3, 40};
  GeShape const_shape(const_dim);
  GeTensorDesc const_tenosr_desc(const_shape, FORMAT_CHWN, DT_FLOAT);
  const_tenosr_desc.SetOriginFormat(FORMAT_CHWN);
  const_tenosr_desc.SetOriginDataType(DT_FLOAT);
  const_tenosr_desc.SetOriginShape(const_shape);
