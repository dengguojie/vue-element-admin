...
class layernorm_grad_fusion_pass_test : public testing::Test {
protected:
...
TEST_F(layernorm_grad_fusion_pass_test, layernorm_grad_fusion_pass_test_1) {

  ge::Graph graph("layernorm_grad_fusion_pass_test_1");
  ...
  auto end_op_pd_gamma = op::Cast("const1_op");
  end_op_pd_gamma.set_input_x(layer_norm_grad_op, "pd_gamma")
                 .set_attr_dst_type(0);

  auto end_op_pd_beta = op::Cast("const2_op");
  ...
  std::vector<Operator> outputs{end_op_pd_x, end_op_pd_gamma, end_op_pd_beta};
  graph.SetInputs(inputs).SetOutputs(outputs);
  ge::ComputeGraphPtr compute_graph_ptr = ge::GraphUtils::GetComputeGraph(graph);
  fe::FusionPassTestUtils::InferShapeAndType(compute_graph_ptr);
  fe::FusionPassTestUtils::RunGraphFusionPass("LayerNormGradFusionPass", fe::BUILT_IN_GRAPH_PASS, *compute_graph_ptr);
  ...
  EXPECT_EQ(graphFusionInfoMap["LayerNormGradFusionPass"].GetMatchTimes(), 1);
  EXPECT_EQ(graphFusionInfoMap["LayerNormGradFusionPass"].GetEffectTimes(), 1);
}

TEST_F(layernorm_grad_fusion_pass_test, layernorm_grad_fusion_pass_test_2) {
  ge::Graph graph("layernorm_grad_fusion_pass_test_2");
  auto input_data = op::Data("input_data");
  std::vector<int64_t> dims{-1, 32};
  ge::Shape shape(dims);
  ge::TensorDesc tensorDesc(shape, FORMAT_ND, DT_FLOAT16);
  input_data.update_input_desc_x(tensorDesc);
  input_data.update_output_desc_y(tensorDesc);
  ...
  auto input_data_dy = op::Data("input_data_dy");
  std::vector<int64_t> dims_dy{-1, 32};
  ge::Shape shape_dy(dims_dy);
  ge::TensorDesc tensorDescDy(shape_dy, FORMAT_ND, DT_FLOAT16);
  input_data_dy.update_input_desc_x(tensorDescDy);
  input_data_dy.update_output_desc_y(tensorDescDy);
