...
class group_norm_fusion_test : public testing::Test {
protected:
  static void SetUpTestCase() {
    std::cout << "group_norm_fusion_test SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "group_norm_fusion_test TearDown" << std::endl;
  }
};

TEST_F(group_norm_fusion_test, group_norm_fusion_test1) {
  ge::Graph graph("group_norm_fusion_test");
  ...
  uint64_t *shape0_tensor_value = new uint64_t[3]{8, 32, 4096};
  ...
  uint64_t *shape1_tensor_value = new uint64_t[4]{8, 64, 128, 16};
  ...
  ge::TensorDesc data0_desc(ge::Shape({8, 64, 128, 16}), FORMAT_NCHW,  DT_INT64);
  ge::TensorDesc reshape_y_desc(ge::Shape({8, 32, 4096}), FORMAT_ND,  DT_INT64);
  ...
  fe::FusionPassTestUtils::RunGraphFusionPass("GroupNormFusionPass", fe::BUILT_IN_GRAPH_PASS, *compute_graph_ptr);

  bool findOp = false;
  bool shapeMatch = false;
  vector<int64_t> expectShape{8, 64, 128, 16};
