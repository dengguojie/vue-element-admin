...
TEST_F(resize_bilinear_cast_fusion_test, resize_bilinear_cast_fusion_test_3) {
  ge::Graph graph("resize_bilinear_cast_fusion_test_3");
  ge::Tensor size_tensor;
  std::vector<int64_t> size_vec{2};
  ge::Shape size_shape(size_vec);
  ge::TensorDesc size_desc(size_shape, FORMAT_ND, DT_INT32);
  ...
  *(size_data + 1) = 10;
  ...
  auto add = op::Add("add")
        .set_input_x1(resize_bilinear_op)
        .set_input_x2(cast_op);
  ...
  ge::TensorDesc tensorDescAddOut(output_shape, ge::FORMAT_NC1HWC0, ge::DT_FLOAT16);
  add.update_input_desc_x1(output_desc);
  add.update_input_desc_x2(output_desc);
  add.update_output_desc_y(tensorDescCastOut);
