...
class batchnormgrad_bninfergrad_fusion_pass_test : public testing::Test {
 protected:
...
TEST_F(batchnormgrad_bninfergrad_fusion_pass_test, batchnormgrad_bninfergrad_fusion_pass_test_1) {
  ...
  *dataValue = 1.1;
  ...
  input1.update_output_desc_y(TensorDesc(ge::Shape({1}), FORMAT_NCHW, DT_FLOAT));
  delete[] dataValue;

  auto input2 = op::Const("input2");
  input2.set_attr_value(axis);
  input2.update_output_desc_y(TensorDesc(ge::Shape({1}), FORMAT_NCHW, DT_FLOAT));

  auto input3 = op::Const("input3");
  input3.set_attr_value(axis);
  input3.update_output_desc_y(TensorDesc(ge::Shape({1}), FORMAT_NCHW, DT_FLOAT));

  auto input4 = op::Const("input4");
  input4.set_attr_value(axis);
  input4.update_output_desc_y(TensorDesc(ge::Shape({1}), FORMAT_NCHW, DT_FLOAT));

  auto batchNormGrad = op::BatchNormGrad("batchNormGrad");
  batchNormGrad.set_input_y_backprop(input0);
  batchNormGrad.set_input_x(input1);
  batchNormGrad.set_input_scale(input2);
  batchNormGrad.set_input_reserve_space_1(input3);
  batchNormGrad.set_input_reserve_space_2(input4);
  batchNormGrad.set_attr_is_training(false);

  std::vector<Operator> inputs{input0, input1, input2, input3, input4};
  std::vector<Operator> outputs{batchNormGrad};
  graph.SetInputs(inputs).SetOutputs(outputs);
  ge::ComputeGraphPtr compute_graph_ptr = ge::GraphUtils::GetComputeGraph(graph);
  fe::FusionPassTestUtils::InferShapeAndType(compute_graph_ptr);
  GE_DUMP(compute_graph_ptr, "batchnorm_bninfer_fusion_test_1_before");
  fe::FusionPassTestUtils::RunGraphFusionPass("BatchNormGradBnInferGradFusion", fe::BUILT_IN_GRAPH_PASS,
                                              *compute_graph_ptr);
  GE_DUMP(compute_graph_ptr, "batchnorm_bninfer_fusion_test_1_after");
}

TEST_F(batchnormgrad_bninfergrad_fusion_pass_test, batchnormgrad_bninfergrad_fusion_pass_test_2) {
...
TEST_F(batchnormgrad_bninfergrad_fusion_pass_test, batchnormgrad_bninfergrad_fusion_pass_test_3) {
  ...
  ge::OpDescPtr batchNormGrad = std::make_shared<ge::OpDesc>("batchNormGrad", "BatchNormGrad");
  ...
  batchNormGrad->AddInputDesc(bias_desc);
  batchNormGrad->AddInputDesc(bias_desc);
  batchNormGrad->AddInputDesc(bias_desc);
  batchNormGrad->AddInputDesc(bias_desc);
  batchNormGrad->AddInputDesc(bias_desc);
  batchNormGrad->AddOutputDesc(bias_desc);
  batchNormGrad->AddOutputDesc(bias_desc);
  batchNormGrad->AddOutputDesc(bias_desc);
  batchNormGrad->AddOutputDesc(bias_desc);
  batchNormGrad->AddOutputDesc(bias_desc);
  ...
  ge::NodePtr batchNormGrad_node = graph->AddNode(batchNormGrad);
  ge::NodePtr netoutput_node = graph->AddNode(netoutput);

  ge::AttrUtils::SetBool(batchNormGrad_node->GetOpDesc(), "is_training", false);
  ge::AttrUtils::SetFloat(batchNormGrad_node->GetOpDesc(), "epsilon", 1.1);

  ge::GraphUtils::AddEdge(x1_node->GetOutDataAnchor(0), batchNormGrad_node->GetInDataAnchor(0));
  ge::GraphUtils::AddEdge(scale_node->GetOutDataAnchor(0), batchNormGrad_node->GetInDataAnchor(1));
  ge::GraphUtils::AddEdge(offset_node->GetOutDataAnchor(0), batchNormGrad_node->GetInDataAnchor(2));
  ge::GraphUtils::AddEdge(mean_node->GetOutDataAnchor(0), batchNormGrad_node->GetInDataAnchor(3));
  ge::GraphUtils::AddEdge(variance_node->GetOutDataAnchor(0), batchNormGrad_node->GetInDataAnchor(4));
  ge::GraphUtils::AddEdge(batchNormGrad_node->GetOutDataAnchor(0), netoutput_node->GetInDataAnchor(0));
  ge::GraphUtils::AddEdge(batchNormGrad_node->GetOutControlAnchor(), netoutput_node->GetInControlAnchor());

  std::cout << batchNormGrad_node->GetAllOutDataAnchorsSize() << std::endl;

  EXPECT_EQ(batchNormGrad_node->GetOutDataAnchor(0)->GetPeerInDataAnchors().size(), 1);

  std::cout << batchNormGrad_node->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() << std::endl;
  fe::FusionPassTestUtils::InferShapeAndType(graph);
  GE_DUMP(graph, "batchnorm_bninfer_fusion_test_2_before");
  fe::FusionPassTestUtils::RunGraphFusionPass("BatchNormGradBnInferGradFusion", fe::BUILT_IN_GRAPH_PASS, *graph);
  GE_DUMP(graph, "batchnorm_bninfer_fusion_test_2_after");
}

TEST_F(batchnormgrad_bninfergrad_fusion_pass_test, batchnormgrad_bninfergrad_fusion_pass_test_4) {
