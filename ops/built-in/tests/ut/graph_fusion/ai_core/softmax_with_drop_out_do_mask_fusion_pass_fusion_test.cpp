...
TEST_F(softmax_with_drop_out_do_mask_fusion_pass_test, softmax_with_drop_out_do_mask_fusion_pass_test_1) {
    ...
    std::vector<int64_t> ori_dims{4,16,512,512};
    ge::Shape shape(dims);
    ...
    tensorDesc1.SetOriginShape(ori_shape); 
...
TEST_F(softmax_with_drop_out_do_mask_fusion_pass_test, softmax_with_drop_out_do_mask_fusion_pass_test_2) {
    ge::Graph graph("softmax_with_drop_out_do_mask_fusion_pass_test_2");

    auto in_input_x_data = op::Data("diag_input_data");
    std::vector<int64_t> dims{4,16,32,32,16,16};
    std::vector<int64_t> ori_dims{4,16,500,500};
    ge::Shape shape(dims);
    ge::Shape ori_shape(ori_dims);
    ge::TensorDesc tensorDesc(shape, FORMAT_FRACTAL_NZ, ge::DT_FLOAT16);
    tensorDesc.SetOriginShape(ori_shape);
