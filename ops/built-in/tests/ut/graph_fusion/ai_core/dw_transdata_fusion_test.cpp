  ...
  if (iter != createFns.end()) {
    if (passType == fe::BUILT_IN_AI_CORE_BUFFER_FUSION_PASS) {
      auto BufferFusionPassBasePtr =
          std::unique_ptr<TbeDwTransDataFusionPass>(dynamic_cast<TbeDwTransDataFusionPass *>(iter->second()));
...
// dw check not supported case
TEST_F(dw_transdata_fusion_test, dw_transdata_fusion_test_1) {
    ge::Graph graph("dw_transdata_fusion_test_1");
    ...
    EXPECT_EQ(find_dw, false);
    EXPECT_EQ(find_td, false);
    EXPECT_EQ(res, SUCCESS);
}

// transdata check not supported case
TEST_F(dw_transdata_fusion_test, dw_transdata_fusion_test_2) {
    ge::Graph graph("dw_transdata_fusion_test_2");

    auto fmap_shape = vector<int64_t>({-1, -1, -1, -1});
    std:vector<std::pair<int64_t, int64_t>> unlimited_range = {{1, -1}, {1, -1}, {1, -1}, {1, -1}};
    ge::TensorDesc fmap_desc(ge::Shape(fmap_shape), ge::FORMAT_NHWC, ge::DT_FLOAT16);
    fmap_desc.SetShapeRange(unlimited_range);
    auto data_fmap = op::Data("data_fmap");
    auto data_dedy = op::Data("data_dedy");
    data_fmap.update_input_desc_x(fmap_desc);
    ...
    auto filter_data = op::Const("filter_size");
    auto filter_size_shape = vector<int64_t>({4});
    ge::TensorDesc filter_size_desc(ge::Shape(filter_size_shape), ge::FORMAT_NHWC, ge::DT_INT32);
    Tensor filter_size;
    int32_t *filter_size_value = new int32_t[4];
    filter_size.SetTensorDesc(filter_size_desc);
    filter_size.SetData((uint8_t *)filter_size_value, 4 * sizeof(int32_t));
...
// check not support range case
TEST_F(dw_transdata_fusion_test, dw_transdata_fusion_test_3) {
    ge::Graph graph("dw_transdata_fusion_test_3");

    auto fmap_shape = vector<int64_t>({-1, -1, -1, -1});
    std:vector<std::pair<int64_t, int64_t>> unlimited_range = {{1, 32}, {1, 32}, {1, 32}, {1, 32}};
    ge::TensorDesc fmap_desc(ge::Shape(fmap_shape), ge::FORMAT_NCHW, ge::DT_FLOAT16);
    fmap_desc.SetShapeRange(unlimited_range);
    auto data_fmap = op::Data("data_fmap");
    auto data_dedy = op::Data("data_dedy");
