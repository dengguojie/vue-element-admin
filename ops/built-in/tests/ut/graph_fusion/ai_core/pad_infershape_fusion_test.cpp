...
class pad_fusion_pass_test : public testing::Test {
 protected:
...
TEST_F(pad_fusion_pass_test, pad_fusion_pass_test_int32) {
  ge::Graph graph("pad_fusion_pass_test_1");
  // input x info
...
TEST_F(pad_fusion_pass_test, pad_fusion_pass_test_int64) {
  ge::Graph graph("pad_fusion_pass_test_1");
  // input x info
  auto input_x_shape = vector<int64_t>({3, 5});
  std::vector<std::pair<int64_t, int64_t>> range_x1 = {{3, 3}, {5, 5}};
  vector<uint64_t> paddings = {1, 1, 2, 2};
  auto dtype = DT_FLOAT;
  // expect info
  std::vector<int64_t> expected_shape = {5, 9};
  std::vector<std::pair<int64_t, int64_t>> expected_range = {};
  ...
  Tensor depth_tensor(desc_input_size_1);
  uint64_t* depth_tensor_value = new uint64_t[input_x_shape.size() * 2];
  for (size_t dim = 0; dim < input_x_shape.size() * 2; dim++) {
    *(depth_tensor_value + dim) = paddings[dim];
  }
  depth_tensor.SetData((uint8_t*)depth_tensor_value, input_x_shape.size() * 2 * sizeof(uint64_t));
  auto depth_const_op = op::Const("depth").set_attr_value(depth_tensor);
  // new op
  auto test_op = op::Pad("Pad");
  test_op.set_input_x(data);
...
TEST_F(pad_fusion_pass_test, pad_fusion_pass_test_dynamic) {
  ge::Graph graph("pad_fusion_pass_test_2");
  // input x info
