...
class dynamic_gru_v2_grad_fusion_test : public testing::Test {
protected:
  static void SetUpTestCase() {
      std::cout << "dynamic_gru_v2_grad_fusion_test SetUp" << std::endl;
  }

  static void TearDownTestCase() {
      std::cout << "dynamic_gru_v2_grad_fusion_test TearDown" << std::endl;
  }
};

TEST_F(dynamic_gru_v2_grad_fusion_test, dynamic_gru_v2_grad_fusion_test_1) {
  ge::Graph graph("dynamic_gru_v2_grad_fusion_test_1");
  int64_t tSize = 5;
  ...
  for (auto node: computeGraphPtr->GetAllNodes()) {
    if (node->GetType() == "GRUV2HiddenGradCell") {
      findGRUV2HiddenGradCell = true;
    }
  }
  EXPECT_EQ(findGRUV2HiddenGradCell, true);
}


TEST_F(dynamic_gru_v2_grad_fusion_test, dynamic_gru_v2_grad_fusion_test_2) {
  ge::Graph graph("dynamic_gru_v2_grad_fusion_test_2");
  int64_t tSize = 1;
  int64_t batchSize = 128;
  int64_t inputSize = 64;
  int64_t hiddenSize = 32;
