...
class DepthwiseconvDequantFusionTest : public testing::Test {
protected:
  static void SetUpTestCase() {
    std::cout << "tbe_depthwiseconv_dequant_fusion_pass_test SetUp" << std::endl;
  ...
  static void TearDownTestCase() {
    std::cout << "tbe_depthwiseconv_dequant_fusion_pass_test TearDown" << std::endl;
  }
  static fe::BufferFusionMapping ConstructFusionMappingOfDepthwiseconvDequant(const ComputeGraphPtr compute_graph_ptr);
...
vector<fe::BufferFusionPattern *> DepthwiseconvDequantFusionTest::patterns;
std::unique_ptr<fe::BufferFusionPassBase> DepthwiseconvDequantFusionTest::ptr_buffer_fusion_pass_func;
const string DepthwiseconvDequantFusionTest::name_fusion_pass = "TbeDepthwiseConvDequantFusionPass";
const fe::BufferFusionPassType DepthwiseconvDequantFusionTest::type_fusion_pass = fe::BUILT_IN_AI_CORE_BUFFER_FUSION_PASS;

fe::BufferFusionMapping DepthwiseconvDequantFusionTest::ConstructFusionMappingOfDepthwiseconvDequant(const ComputeGraphPtr compute_graph_ptr) {
  ...
  for (const auto &ptr_node : compute_graph_ptr->GetAllNodes()) {
    ...
    } else if (desc->GetName() == "AscendQuant") {
      nodes_quant.push_back(ptr_node);
    }
  }

  fe::BufferFusionPattern *pattern;
  EXPECT_TRUE(fe::FusionPassTestUtils::GetBufferFusionPattern(patterns, "TbeDepthwiseConvDequantMulQuantBroadcastFusion", &pattern));
...
TEST_F(DepthwiseconvDequantFusionTest, depthwiseconv_dequant_fusion_pass_test_1) {
  ...
  builder.AddDataEdge(data1, 0, mul_node, 1);
  builder.AddDataEdge(dequant_node, 0, mul_node, 0);
  builder.AddDataEdge(mul_node, 0, quant_node, 0);

  auto graph = builder.GetGraph();
  // ge::ComputeGraphPtr graph_ptr = ge::GraphUtils::GetComputeGraph(graph);
  auto mapping = ConstructFusionMappingOfDepthwiseconvDequant(graph);
  GraphUtils::DumpGEGraphToOnnx(*graph, "depthwiseconv_dequant_fusion_pass_before");
  Status res = fe::FusionPassTestUtils::RunBufferFusionPass(ptr_buffer_fusion_pass_func.get(), patterns,
                                                            graph, mapping);
  EXPECT_EQ(res, fe::SUCCESS);                                                       
}

TEST_F(DepthwiseconvDequantFusionTest, depthwiseconv_dequant_fusion_pass_test_2) {
  ut::GraphBuilder builder = ut::GraphBuilder(this->test_info_->name());
  auto data = builder.AddNode("Data1", "Data", 0, 1, {1, 1, 56, 56, 16}, FORMAT_NC1HWC0, ge::DT_FLOAT16);
  auto data1 = builder.AddNode("Data2", "Data", 0, 1, {1, 1, 1, 1, 16}, FORMAT_ND, ge::DT_FLOAT16);
  auto conv_node = builder.AddNode("Conv2D", "DepthwiseConv2D",
  ...
  builder.AddDataEdge(conv_node, 0, dequant_node, 0);
  builder.AddDataEdge(data, 0, dequant_node, 1);
  builder.AddDataEdge(data1, 0, mul_node, 0);
...
TEST_F(DepthwiseconvDequantFusionTest, depthwiseconv_dequant_fusion_pass_test_3) {
  ut::GraphBuilder builder = ut::GraphBuilder(this->test_info_->name());
  auto data = builder.AddNode("Data1", "Data", 0, 1, {1, 1, 56, 56, 16}, FORMAT_NC1HWC0, ge::DT_FLOAT16);
