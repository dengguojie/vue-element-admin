...
TEST_F(concat_fusion_test, concat_fusion_test_2) {
  ...
  *(crops_data + 0) = 0;
  ...
  auto concat_layer = op::ConcatD("concat");
  concat_layer.create_dynamic_input_x(69);
  for (int64_t n = 0; n < 69; n++) {
    concat_layer.set_dynamic_input_x(n, inputx0Data);
  }
  ...
  ge::ComputeGraphPtr compute_graph_ptr = ge::GraphUtils::GetComputeGraph(graph);
  fe::FusionPassTestUtils::InferShapeAndType(compute_graph_ptr);
  fe::FusionPassTestUtils::RunGraphFusionPass("ZConcatDFusionPass", fe::BUILT_IN_GRAPH_PASS, *compute_graph_ptr);

  bool findConcat = false;
  for (auto node: compute_graph_ptr->GetAllNodes()) {
    if (node->GetType() == "ConcatD") {
      auto name = node->GetName();
      if (name.find("ConcatDBase_node") != string::npos) {
        findConcat = true;
      }
    }
  }
...
TEST_F(concat_fusion_test, concat_fusion_test_3) {
  ge::Graph graph("concat_fusion_test_3");
...
TEST_F(concat_fusion_test, concat_fusion_test_4) {
  ge::Graph graph("concat_fusion_test_4");
...
TEST_F(concat_fusion_test, concat_fusion_test_5) {
  ge::Graph graph("concat_fusion_test_5");
