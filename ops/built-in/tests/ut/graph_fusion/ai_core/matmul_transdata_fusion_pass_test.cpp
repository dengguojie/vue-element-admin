...
TEST_F(gemm_transdata_fusion_pass_test, gemm_transdata_fusion_pass_test_2) {
    ge::Graph graph("gemm_to_matmul_fusion_fusion_pass_test_1");

    DESC_DATA(data_a, ge::GeShape({16, 32}), FORMAT_ND, ge::GeShape({16, 32}), FORMAT_ND, DT_INT8);
    DESC_DATA(data_b, ge::GeShape({32, 64}), FORMAT_ND, ge::GeShape({32, 64}), FORMAT_ND, DT_INT8);
    DESC_DATA(data_c, ge::GeShape({16, 64}), FORMAT_ND, ge::GeShape({16, 64}), FORMAT_ND, DT_FLOAT);
    DESC_DATA(data_alpha, ge::GeShape({1}), FORMAT_ND, ge::GeShape({1}), FORMAT_ND, DT_FLOAT);
    DESC_DATA(data_beta, ge::GeShape({1}), FORMAT_ND, ge::GeShape({1}), FORMAT_ND, DT_FLOAT);
    DESC_DATA(data_y, ge::GeShape({16, 64}), FORMAT_ND, ge::GeShape({16, 64}), FORMAT_ND, DT_FLOAT);

    DESC_DATA(data_a_nz, ge::GeShape({16, 32}), FORMAT_ND, ge::GeShape({1, 1, 16, 32}), FORMAT_FRACTAL_NZ, DT_INT8);
    DESC_DATA(data_b_nz, ge::GeShape({32, 64}), FORMAT_ND, ge::GeShape({2, 2, 16, 32}), FORMAT_FRACTAL_NZ, DT_INT8);
    DESC_DATA(data_m_nz, ge::GeShape({16, 64}), FORMAT_ND, ge::GeShape({4, 1, 16, 16}), FORMAT_FRACTAL_NZ, DT_INT32);
    DESC_DATA(data_c_nz, ge::GeShape({16, 64}), FORMAT_ND, ge::GeShape({4, 1, 16, 16}), FORMAT_FRACTAL_NZ, DT_FLOAT);
    DESC_DATA(data_y_nz, ge::GeShape({16, 64}), FORMAT_ND, ge::GeShape({4, 1, 16, 16}), FORMAT_FRACTAL_NZ, DT_FLOAT);
    ...
    matmul->AddInputDesc("x1", desc_data_a_nz);
    matmul->AddInputDesc("x2", desc_data_b_nz);
    matmul->AddOutputDesc(desc_data_m_nz);
    // update mul, add and cast
    cast->AddInputDesc(desc_data_m_nz);
    cast->AddOutputDesc(desc_data_y_nz);
    mul_1->AddInputDesc("x1", desc_data_y_nz);
    mul_1->AddInputDesc("x2", desc_data_alpha);
    mul_1->AddOutputDesc(desc_data_y_nz);
    mul_2->AddInputDesc("x1", desc_data_c_nz);
    ...
    ge::GraphUtils::AddEdge(cast_node->GetOutDataAnchor(0), mul_1_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(data_alpha_node->GetOutDataAnchor(0), mul_1_node->GetInDataAnchor(1));
    ge::GraphUtils::AddEdge(trans_c_node->GetOutDataAnchor(0), mul_2_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(data_beta_node->GetOutDataAnchor(0), mul_2_node->GetInDataAnchor(1));
    ge::GraphUtils::AddEdge(mul_1_node->GetOutDataAnchor(0), add_node->GetInDataAnchor(0));
