...
class Conv2dDequantVaddReluQuantFusionTest : public testing::Test {
protected:
  static void SetUpTestCase() {
    std::cout << "conv2d_dequant_vadd_relu_quant_pass_test SetUp" << std::endl;
  ...
  static void TearDownTestCase() {
    std::cout << "conv2d_dequant_vadd_relu_quant_pass_test TearDown" << std::endl;
  }
  static fe::BufferFusionMapping ConstructFusionMappingOfConvDequantVaddReluQuant(const ComputeGraphPtr compute_graph_ptr);
...
vector<fe::BufferFusionPattern *> Conv2dDequantVaddReluQuantFusionTest::patterns;
std::unique_ptr<fe::BufferFusionPassBase> Conv2dDequantVaddReluQuantFusionTest::ptr_buffer_fusion_pass_func;
const string Conv2dDequantVaddReluQuantFusionTest::name_fusion_pass = "TbeConvDequantVaddReluQuantFusionPass";
const fe::BufferFusionPassType Conv2dDequantVaddReluQuantFusionTest::type_fusion_pass = fe::BUILT_IN_AI_CORE_BUFFER_FUSION_PASS;

fe::BufferFusionMapping Conv2dDequantVaddReluQuantFusionTest::ConstructFusionMappingOfConvDequantVaddReluQuant(const ComputeGraphPtr compute_graph_ptr) {
  ...
  EXPECT_TRUE(fe::FusionPassTestUtils::GetBufferFusionPattern(patterns, "TbeConvDequantVaddReluQuantBroadcastFusion", &pattern));
...
TEST_F(Conv2dDequantVaddReluQuantFusionTest, conv2d_dequant_vadd_relu_pass_test_1) {
  ...
  builder.AddDataEdge(relu_node, 0, quant_node, 0);

  auto graph = builder.GetGraph();
  // ge::ComputeGraphPtr graph_ptr = ge::GraphUtils::GetComputeGraph(graph);
  auto mapping = ConstructFusionMappingOfConvDequantVaddReluQuant(graph);
  GraphUtils::DumpGEGraphToOnnx(*graph, "tbe_conv_dequant_vadd_relu_quant_pass_before");
  Status res = fe::FusionPassTestUtils::RunBufferFusionPass(ptr_buffer_fusion_pass_func.get(), patterns,
                                                            graph, mapping);
  EXPECT_EQ(res, fe::SUCCESS);                                                       
}

TEST_F(Conv2dDequantVaddReluQuantFusionTest, conv2d_dequant_vadd_relu_pass_test_2) {
  fe::PlatformInfo platformInfo;
  fe::OptionalInfo optiCompilationInfo;
  platformInfo.soc_info.ai_core_cnt = 1;
  platformInfo.str_info.ccec_aic_version = "dav-s200";
...
TEST_F(Conv2dDequantVaddReluQuantFusionTest, conv2d_dequant_vadd_relu_pass_test_3) {
  fe::PlatformInfo platformInfo;
  fe::OptionalInfo optiCompilationInfo;
  platformInfo.soc_info.ai_core_cnt = 1;
  platformInfo.str_info.ccec_aic_version = "dav-s200";
