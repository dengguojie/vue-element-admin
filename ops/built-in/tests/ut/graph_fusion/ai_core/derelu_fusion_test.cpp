...
TEST_F(derelu_fusion_test, derelu_fusion_test_2) {
  ge::Graph graph("derelu_fusion_test_2");
  std::vector<int64_t> dims{3, 32,2,3};
  ge::Shape shape(dims);
  ge::TensorDesc tensorDesc(shape,FORMAT_NHWC, DT_FLOAT);
  ...
  std::vector<int64_t> grad_dims{3,32,2,3 };
  ge::Shape grad_shape(grad_dims);
  ge::TensorDesc tensorDesc2(grad_shape);
  auto relu_grad_data = op::Data("relu_grad_data");
  relu_grad_data.update_input_desc_x(tensorDesc2);
  ...
  auto reluGrad_op = op::ReluGrad("relugrad");
  reluGrad_op.set_input_gradients(relu_grad_data).set_input_features(relu_op);
