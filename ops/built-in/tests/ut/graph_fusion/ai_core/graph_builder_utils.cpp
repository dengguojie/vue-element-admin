...
NodePtr GraphBuilder::AddNode(const std::string &name, const std::string &type, int in_cnt, int out_cnt,
                              const std::vector<int64_t>& shape, Format format, DataType data_type) {
...
NodePtr GraphBuilder::AddNode(const string &name, const string &type, std::vector<std::string> input_names,
...
NodePtr GraphBuilder::AddNode(const std::string &name,
                              const std::string &type,
                              const std::vector<UtOpDesc> &inputs,
                              const std::vector<UtOpDesc> &outputs) {
  auto op_desc = std::make_shared<OpDesc>(name, type);
  for (const auto& input : inputs) {
    op_desc->AddInputDesc(input.GetGeTensorDesc());
  }

  for (const auto& output : outputs) {
    op_desc->AddOutputDesc(output.GetGeTensorDesc());
  }
