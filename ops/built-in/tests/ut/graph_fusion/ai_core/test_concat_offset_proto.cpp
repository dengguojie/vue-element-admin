...
TEST_F(concat_offset_test, concat_offset_test_1){
  ...
  data_x1.update_input_desc(desc_x1);
  ...
  data_x2.update_input_desc(desc_x2);
  ...
  data_x3.update_input_desc(desc_x3);
...
TEST_F(concat_offset_test, concat_offset_test_2){
  ge::Graph graph("concat_offset_2");
  auto shape_x1 = vector<int64_t>({-1,});
  TensorDesc desc_x1(ge::Shape(shape_x1), FORMAT_ND, DT_INT32);
  std::vector<std::pair<int64_t, int64_t>>x1_range;
  x1_range.push_back(std::pair<int64_t, int64_t>){1,9};
  ...
  auto shape_x3 = vector<int64_t>({5,});
  TensorDesc desc_x3(ge::Shape(shape_x3), FORMAT_ND, DT_INT32);
  ...
  //new op
  auto test_layer = op::ConcatOffset("ConcatOffset_2");
  test_layer.set_input_concat_dim(input_concat_dim_data)
            .create_dynamic_input_x(3)
            .set_dynamic_input_x(0,data_x1)
            .set_dynamic_input_x(1,data_x2)
  ...
  bool ShapeMatch2 = false;
  bool ShapeMatch3 = false;
  bool ShapeRangeMatch1 = false;
  bool ShapeRangeMatch2 = false;
  bool ShapeRangeMatch3 = false;
  //except_shape
  std::vector<std::pair<int64_t, int64_t>>expect_range1;
  std::vector<std::pair<int64_t, int64_t>>expect_range2;
  std::vector<std::pair<int64_t, int64_t>>expect_range3;

  auto expect_shape1 = vector<int64_t>({5,});
  auto expect_shape2 = vector<int64_t>({5,});
  auto expect_shape3 = vector<int64_t>({5,});
  expect_range1.push_back(std::pair<int64_t,int64_t>{5,5});
  expect_range2.push_back(std::pair<int64_t,int64_t>{5,5});
  expect_range3.push_back(std::pair<int64_t,int64_t>{5,5});

  for(auto node: compute_graph_ptr->GetAllNodes()){
      if(node->GetType() == "ConcatOffset"){
          std::cout<< "ConcatOffset test_2!!!"<<std::endl;
          findOp = true;
          auto outputDesc1 = node->GetOpDesc()->GetOutputDesc(0);
          auto outputDesc2 = node->GetOpDesc()->GetOutputDesc(1);
          auto outputDesc3 = node->GetOpDesc()->GetOutputDesc(2);
          ...
          if(output_range1 == expect_range1){
              shapeRangeMatch1 = true;
          }
          if(output_range2 == expect_range2){
              shapeRangeMatch1 = true;
          }
          if(output_range3 == expect_range3){
              shapeRangeMatch1 = true;
          }
      }
  }
  ...
  EXPECT_EQ(ShapeMatch2, true);
  EXPECT_EQ(ShapeMatch3, true);
  EXPECT_EQ(ShapeRangeMatch1, true);
  EXPECT_EQ(ShapeRangeMatch2, true);
  EXPECT_EQ(ShapeRangeMatch3, true);
}

TEST_F(concat_offset_test, concat_offset_test_3{
  ge::Graph graph("concat_offset_3");
  auto shape_x1 = vector<int64_t>({3,});
  TensorDesc desc_x1(ge::Shape(shape_x1), FORMAT_ND, DT_INT32);
  ...
  auto shape_x2 = vector<int64_t>({3,});
  TensorDesc desc_x2(ge::Shape(shape_x2), FORMAT_ND, DT_INT32);
  ...
  std::vector<Operator> inputs{input_concat_dim_data, data_x1, data_x2};
  std::vector<Operator> outputs{test_layer};
  graph.SetInputs(inputs).SetOutputs(outputs);
  ...
  auto expect_shape1 = vector<int64_t>({3,});
  auto expect_shape2 = vector<int64_t>({3,});
  for(auto node: compute_graph_ptr->GetAllNodes()){
      if(node->GetType() == "ConcatOffset"){
          std::cout<< "ConcatOffset test_3!!!"<<std::endl;
          findOp = true;
          auto outputDesc1 = node->GetOpDesc()->GetOutputDesc(0);
          auto outputDesc2 = node->GetOpDesc()->GetOutputDesc(1);
          std::vector<int64_t> output_shape1 = outputDesc1.GetShape().GetDims();
...
TEST_F(concat_offset_test, concat_offset_test_4) {
  ge::Graph graph("concat_offset_3");
  auto shape_x1 = vector<int64_t>({3,});
  TensorDesc desc_x1(ge::Shape(shape_x1), FORMAT_ND, DT_INT32);
  ...
  auto expect_shape1 = vector<int64_t>({4,});
  auto expect_shape2 = vector<int64_t>({3,});
  for(auto node: compute_graph_ptr->GetAllNodes()){
      if(node->GetType() == "ConcatOffset"){
          std::cout<< "ConcatOffset test_3!!!"<<std::endl;
