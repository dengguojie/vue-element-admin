...
TEST_F(confusion_softmax_grad_fusion_test, confusion_softmax_grad_fusion_test_3) {
    ge::Graph graph("confusion_softmax_grad_fusion_test_3");
    ...
    for (auto node: compute_graph_ptr->GetAllNodes()) {
        if (node->GetType() == "ConfusionSoftmaxGrad") {
            findFused = true;
        }
    }
