...
class softmax_grad_ext_fusion_test : public testing::Test {
protected:
    static void SetUpTestCase() {
        std::cout << "softmax_grad_ext_fusion_test SetUp" << std::endl;
    }

    static void TearDownTestCase() {
        std::cout << "softmax_grad_ext_fusion_test TearDown" << std::endl;
    }
};

TEST_F(softmax_grad_ext_fusion_test, softmax_grad_ext_fusion_test_1) {
    ge::Graph graph("softmax_grad_ext_fusion_test_1");
    
    ge::Tensor mul0_tensor;
    std::vector<int64_t> mul0_vec{-1};
    ge::Shape mul0_shape(mul0_vec);
    ge::TensorDesc mul0_desc(mul0_shape, FORMAT_ND, DT_FLOAT);
    int64_t mul0_size = 32;
    ...
    fe::FusionPassTestUtils::RunGraphFusionPass("SoftmaxGradExtFusion", fe::BUILT_IN_GRAPH_PASS, *compute_graph_ptr);
...
TEST_F(softmax_grad_ext_fusion_test, softmax_grad_ext_fusion_test_2) {
    ge::Graph graph("softmax_grad_ext_v2_fusion_test_2");
    ...
    bool findOp = false;
    for (auto node: compute_graph_ptr->GetAllNodes()) {
        if (node->GetType() == "SoftmaxGradExtFusionV2") {
            findOp = true;
        }
    }
    EXPECT_EQ(false, false);
