...
class in_training_update_grad : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "in_training_update_grad SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "in_training_update_grad TearDown" << std::endl;
  }
};

TEST_F(in_training_update_grad, in_training_update_grad_infershape_diff_test_1) {
  ...
  auto out_y_desc = op.GetOutputDesc("res_gamma");
  EXPECT_EQ(out_y_desc.GetDataType(), ge::DT_FLOAT);
  std::vector<int64_t> expected_y_output_shape = {2, 1, 1, 1, 32};
  EXPECT_EQ(out_y_desc.GetShape().GetDims(), expected_y_output_shape);

  auto out_batch_mean_desc = op.GetOutputDesc("res_beta");
  EXPECT_EQ(out_batch_mean_desc.GetDataType(), ge::DT_FLOAT);
  std::vector<int64_t> expected_batch_mean_output_shape = {2, 1, 1, 1, 32};
