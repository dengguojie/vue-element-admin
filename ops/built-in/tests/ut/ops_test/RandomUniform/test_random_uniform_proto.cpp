...
class RandomUniform : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "RandomUniform SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "RandomUniform TearDown" << std::endl;
  }
};

TEST_F(RandomUniform, random_uniform_infer_shape) {
  ge::op::RandomUniform op;
...
TEST_F(RandomUniform, random_uniform_infer_shape_failed) {
  ge::op::RandomUniform op;
  std::vector<std::pair<int64_t,int64_t>> shape_range = {{2, 2}};
  auto tensor_desc = create_desc_shape_range({2},
                                             ge::DT_INT64, ge::FORMAT_ND,
