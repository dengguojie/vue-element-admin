...
class RotatedOverlapsProtoTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "RotatedOverlaps Proto Test SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "RotatedOverlaps Proto Test TearDown" << std::endl;
  }
};


TEST_F(RotatedOverlapsProtoTest, rotated_overlaps_proto_test_succ) {
    ge::op::RotatedOverlaps op;
    ...
    auto output_desc = op.GetOutputDesc("overlaps");
    EXPECT_EQ(output_desc.GetDataType(), ge::DT_FLOAT);
    std::vector<int64_t> expected_output_shape = {B, N, K};
    EXPECT_EQ(output_desc.GetShape().GetDims(), expected_output_shape);
}

TEST_F(RotatedOverlapsProtoTest, rotated_overlaps_proto_test_failed_shape) {
    ge::op::RotatedOverlaps op;
