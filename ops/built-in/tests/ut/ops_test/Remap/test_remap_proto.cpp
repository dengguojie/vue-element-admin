...
class Remap : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "Remap SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "Remap TearDown" << std::endl;
  }
};

TEST_F(Remap, remap_infer_shape_ok) {
  ge::op::Remap op;
  ...
  op.UpdateInputDesc("map_offset", tensor_desc);
  auto ret = op.InferShapeAndType();
  EXPECT_EQ(ret, ge::GRAPH_SUCCESS);
  auto output_desc = op.GetOutputDesc("map_img");
  EXPECT_EQ(output_desc.GetDataType(), ge::DT_FLOAT16);
  std::vector<int64_t> expected_output_shape = {1, 100, 4, 3};
  EXPECT_EQ(output_desc.GetShape().GetDims(), expected_output_shape);
}

TEST_F(Remap, remap_infer_shape_img_failed) {
  ge::op::Remap op;
  std::vector<std::pair<int64_t, int64_t>> shape_range = {{2, 2}, {100, 200}, {4, 8}};
...
TEST_F(Remap, remap_infer_shape_img_failed_1) {
  ge::op::Remap op;
  std::vector<std::pair<int64_t, int64_t>> shape_range = {{2, 2}, {100, 200}, {4, 8}};
  ...
  op.UpdateInputDesc("map_offset", tensor_desc_not_ok);
  auto ret = op.InferShapeAndType();
  EXPECT_EQ(ret, ge::GRAPH_FAILED);
}
