...
class equal : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "equal SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "equal TearDown" << std::endl;
  }
};

TEST_F(equal, equal_infershape_diff_test_4) {
  ge::op::Equal op;
  ...
  std::vector<std::pair<int64_t, int64_t>> expected_output_shape_range = {{1,3},{1,4},{1,5},{1,6},{4,7}};
  std::vector<std::pair<int64_t, int64_t>> output_shape_range;
  output_desc.GetShapeRange(output_shape_range);
  EXPECT_EQ(output_shape_range, expected_output_shape_range);
}

TEST_F(equal, equal_infershape_diff_test_5) {
  ge::op::Equal op;
...
TEST_F(equal, equal_infershape_diff_test_6) {
  ge::op::Equal op;
...
TEST_F(equal, equal_infershape_diff_test_7) {
  ge::op::Equal op;
  ...
  std::vector<int64_t> expected_output_shape = {-2};
  EXPECT_EQ(output_desc.GetShape().GetDims(), expected_output_shape);
  std::vector<std::pair<int64_t, int64_t>> expected_output_shape_range = {{1,10}, {2,7},{1,7}};
  std::vector<std::pair<int64_t, int64_t>> output_shape_range;
  output_desc.GetShapeRange(output_shape_range);
  EXPECT_EQ(output_shape_range, expected_output_shape_range);
}

TEST_F(equal, equal_infershape_diff_test_8) {
  ge::op::Equal op;
...
TEST_F(equal, equal_infershape_diff_test_10) {
  ge::op::Equal op;
  op.UpdateInputDesc("x1", create_desc_shape_range({-1}, ge::DT_INT32, ge::FORMAT_ND, {-1}, ge::FORMAT_ND, {{1,55}}));
