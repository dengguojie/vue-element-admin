...
TEST_F(relu_v2, relu_v2_infer_shape_03) {
  ge::op::ReluV2 op;
  std::vector<std::pair<int64_t,int64_t>> shape_range = {{1, 20}, {1, 400}, {1, 400}, {1, 400}, {1,400}};
  auto tensor_desc = create_desc_shape_and_origin_shape_range({-1, -1, -1, -1, 96},
                                                              ge::DT_FLOAT16, ge::FORMAT_HWCN,
                                                              {16, 112, 112, 96},
                                                              ge::FORMAT_HWCN, shape_range);
  ...
  auto mask_desc = op.GetOutputDesc("mask");
  EXPECT_EQ(output_desc.GetDataType(), ge::DT_FLOAT16);
  std::vector<int64_t> expected_output_shape = {16, 112, 112, 96};
  EXPECT_EQ(output_desc.GetShape().GetDims(), expected_output_shape);
  std::vector<std::pair<int64_t, int64_t>> output_shape_range;
  EXPECT_EQ(output_desc.GetShapeRange(output_shape_range), ge::GRAPH_SUCCESS);
  std::vector<std::pair<int64_t, int64_t>> expected_shape_range = {};
  EXPECT_EQ(output_shape_range, expected_shape_range);
  std::vector<int64_t> expected_mask_shape = {16, 112, 7, 96, 2};
  EXPECT_EQ(mask_desc.GetShape().GetDims(), expected_mask_shape);
}

TEST_F(relu_v2, relu_v2_infer_shape_04) {
  ge::op::ReluV2 op;
  std::vector<std::pair<int64_t,int64_t>> shape_range = {{1, 20}, {1, 400}, {1, 400}, {1, 400}, {1,400}};
  auto tensor_desc = create_desc_shape_and_origin_shape_range({-1, -1, -1, -1, 96},
                                                              ge::DT_UINT8, ge::FORMAT_HWCN,
                                                              {16, 112, 112, 96},
                                                              ge::FORMAT_HWCN, shape_range);
  ...
  std::vector<std::pair<int64_t, int64_t>> expected_shape_range = {};
  EXPECT_EQ(output_shape_range, expected_shape_range);
  std::vector<int64_t> expected_mask_shape = {16, 112, 4, 96, 4};
  EXPECT_EQ(mask_desc.GetShape().GetDims(), expected_mask_shape);
}
