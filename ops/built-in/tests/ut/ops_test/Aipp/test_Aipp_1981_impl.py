...
def test_aipp_static_1(test_arg):
    ...
    try:
        new_aipp_compute(input_data, input_dync_param, output_data, aipp_config_dict, "Ascend910B", kernel_name="aipp")
...
def test_aipp_format_support(test_arg):
    ...
    for cce_product in aipp_comm.Const.SUPPORT_IMAGE_FORMAT_MAP:
        try:
            aipp_comm.check_aipp_static_config(input_data.get("shape"), input_data.get("format"), output_data,
                                               aipp_config_dict_tmp, cce_product)
        except Exception as e:
