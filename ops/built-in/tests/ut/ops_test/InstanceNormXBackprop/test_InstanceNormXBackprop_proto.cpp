...
class x_backprop : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "x_backprop SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "x_backprop TearDown" << std::endl;
  }
};

TEST_F(x_backprop, x_backprop_infershape_diff_test_1) {
  ge::op::InstanceNormXBackprop op;
  ...
  auto out_batch_mean_desc = op.GetOutputDesc("res_for_gamma");
  EXPECT_EQ(out_batch_mean_desc.GetDataType(), ge::DT_FLOAT);
  std::vector<int64_t> expected_batch_mean_output_shape = {4, 64, 64, 64, 16};
