...
class batch_to_space_d_infer_test : public testing::Test {
  protected:
  static void SetUpTestCase() {
    std::cout << "batch_to_space_d_infer_test SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "batch_to_space_d_infer_test TearDown" << std::endl;
  }
};

TEST_F(batch_to_space_d_infer_test, batch_to_space_d_infer_test_1) {

  // set input info
  auto input_shape = vector<int64_t>({12, 11, 20, 16});
  std::vector<std::pair<int64_t,int64_t>> input_range = {{12, 12}, {11, 11}, {20, 20}, {16, 16}};
  auto block_size = 2;
  auto crops = vector<int64_t>({1,2,2,1});
  auto test_format = ge::FORMAT_NHWC;

  // expect result
  std::vector<int64_t> expected_shape = {3, 19, 37, 16};
...
TEST_F(batch_to_space_d_infer_test, batch_to_space_d_infer_test_4) {
  ...
  // expect result
  std::vector<int64_t> expected_shape = {3, 16, 19, 37};
...
TEST_F(batch_to_space_d_infer_test, batch_to_space_d_infer_test_2) {

  // set input info
  auto input_shape = vector<int64_t>({40, -1, -1, 16});
  std::vector<std::pair<int64_t,int64_t>> input_range = {{40, 40}, {7, 7}, {11, 11}, {16, 16}};
  auto block_size = 2;
  auto crops = vector<int64_t>({1,1,1,1});
  auto test_format = ge::FORMAT_NHWC;

  // expect result
  std::vector<int64_t> expected_shape = {10, -1, -1, 16};
  std::vector<std::pair<int64_t,int64_t>> expected_range = {{10, 10}, {12, 12}, {20, 20}, {16, 16}};
...
TEST_F(batch_to_space_d_infer_test, batch_to_space_d_infer_test_5) {
  ...
  // expect result
  std::vector<int64_t> expected_shape = {10, 16, -1, -1};
  std::vector<std::pair<int64_t,int64_t>> expected_range = {{10, 10}, {16, 16}, {12, 12}, {20, 20}};
...
TEST_F(batch_to_space_d_infer_test, batch_to_space_d_infer_test_3) {
