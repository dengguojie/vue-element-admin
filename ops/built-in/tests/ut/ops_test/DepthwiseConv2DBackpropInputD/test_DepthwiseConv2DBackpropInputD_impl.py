...
ut_case = OpUT("DepthwiseConv2dBackpropInput", "impl.depthwise_conv2d_backprop_input_d",
               "depthwise_conv2d_backprop_input_d")

dp_conv2d_bp_input_op_testcase = [
    # fp16 without bias
    ((3, 3, 960, 1), (32, 960, 7, 7), (32, 960, 7, 7), 1, 1, (1, 1, 1, 1), "NCHW", "success"),
    ((5, 5, 192, 1), (16, 192, 14, 14), (16, 192, 28, 28), 2, 1, (2, 2, 2, 2), "NCHW", "success"),
    ((3, 3, 96, 1), (8, 96, 56, 56), (8, 96, 56, 56), 1, 1, (1, 1, 1, 1), "NCHW", "success"),
    ((3, 3, 32, 1), (1, 32, 56, 56), (1, 32, 112, 112), 2, 1, (1, 1, 1, 1), "NCHW", "success"),
    ((3, 3, 256, 1), (2, 334, 502, 256), (2, 336, 504, 256), 1, 1, (0, 0, 0, 0), "NHWC", "success"),
    ((3, 3, 2048, 1), (648, 2, 3, 2048), (648, 4, 5, 2048), 1, 1, (0, 0, 0, 0), "NHWC", "success"),
    ((5, 5, 240, 1), (2, 14, 14, 240), (2, 28, 28, 240), 2, 1, (2, 2, 2, 2), "NHWC", "success"),
]
...
def _get_kernel_name(x_shape, filter_shape, bias_shape, stride, dilation, pads, dtype):
    ...
    kernel_name = 'dp_conv2d_' + '_'.join(map(str, x_shape)) + '_' + '_'.join(map(str, filter_shape)) + '_' + '_'.join(
        map(str, bias_shape)) + '_' + str(stride) + '_' + str(dilation) + "_" + '_'.join(map(str, pads)) + '_' + dtype
    return kernel_name
...
def _gen_trans_data_case(param):
    ...
    kernel_name = _get_kernel_name(filter_shape, out_backprop_shape, input_size, stride, dilation, pads, dtype)
    return {
