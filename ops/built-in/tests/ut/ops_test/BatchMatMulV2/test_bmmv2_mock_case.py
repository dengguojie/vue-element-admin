...
def test_matmul_ND2ND_fp16():
    with cce():
        ...
        dx_res = batch_matmul_compute(x1_trans, x2_trans, None, None, y, False, False)
...
def test_matmul_ND2ND_int8():
    with cce():
        ...
        matmul_out = batch_matmul_compute(tensor_a, tensor_b, None, None, output_y, True, False)
...
def test_matmul_NZ2ND_fp16():
    with cce():
        ...
        dx_res = batch_matmul_compute(x1, x2, None, None, y, False, True, 0)
...
def test_matmul_ND2NZ_fp16():
    with cce():
        ...
        out = batch_matmul_compute(x1_trans, x2_trans, bias, None, y, False, False, 0)
        sch = auto_schedule(out)
...
def test_matmul_NZ2NZ_fp16():
    with cce():
        ...
        out = batch_matmul_compute(x1, x2, None, None, y, True, True, 0)
        sch = auto_schedule(out)
...
def test_matmul_NZ2NZ_int8():
    with cce():
        ...
        matmul_out = batch_matmul_compute(tensor_a_ori, tensor_b_ori, bias, None, output_y, True, False, 0)
        sch = auto_schedule(matmul_out)

def test_matmul_hf32():
    with op_context.OpContext():
        with cce():
            precision_mode = "hign_performance"
            op_info = OpInfo("BatchMatMulV2", "BatchMatMulV2")
            context = op_context.get_context()
            op_info._precision_mode = precision_mode
...
def test_matmul_fixpipe_3():
    import tbe
    with tbe.common.context.op_context.OpContext("pre-static"):
        with cce():
            x1 = tvm.placeholder((9, 4, 2, 16, 16), name="tensor_a", dtype="float16", attrs={"ori_shape": (3, 3, 32, 64), "format": "FRACTAL_NZ", "ori_format": "ND"})
            x2 = tvm.placeholder((9, 2, 4, 16, 16), name="tensor_b", dtype="float16", attrs={"ori_shape": (3, 3, 64, 32), "format": "FRACTAL_NZ", "ori_format": "ND"})
            bias = tvm.placeholder((32,), name="tensor_bias", dtype="float32", attrs={"format": "ND", "ori_format": "ND", "ori_shape": (32,)})
            output_y = {"shape": (3, 3, 2, 2, 16, 16), "dtype": "float16", "ori_shape": (3, 3, 32, 32), "format": "FRACTAL_NZ", "ori_format": "ND"}
            matmul_out = batch_matmul_compute(x1, x2, bias, None, output_y, False, False, 0)
            y = {"shape": (3, 3, 32, 32), "dtype": "float16", "ori_shape": (3, 3, 32, 32), "format": "ND", "ori_format": "ND"}
            res = fixpipe_compute(matmul_out, None, None, None, None, None, None, None, None, None, y, [], [], "")
            tensor_list = [x1, x2, bias, res]
            sch = auto_schedule(res)

def test_matmul_fixpipe_4():
    import tbe
    with tbe.common.context.op_context.OpContext("pre-static"):
        with cce():
            x1 = tvm.placeholder((6, 2, 2, 16, 32), name="tensor_a", dtype="int8", attrs={"ori_shape": (2, 3, 32, 64), "format": "FRACTAL_NZ", "ori_format": "ND"})
            x2 = tvm.placeholder((6, 1, 4, 16, 32), name="tensor_b", dtype="int8", attrs={"ori_shape": (2, 3, 64, 32), "format": "FRACTAL_NZ", "ori_format": "ND"})
            bias = tvm.placeholder((32,), name="tensor_bias", dtype="int32", attrs={"format": "ND", "ori_format": "ND", "ori_shape": (32,)})
