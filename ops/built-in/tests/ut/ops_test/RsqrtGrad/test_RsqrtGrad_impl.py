...
def calc_expect_func(y, dy, z):   
    ...
    input1 = tf.placeholder(y_data.dtype, y_data.shape)
    input2 = tf.placeholder(dy_data.dtype, dy_data.shape)
    output_var = gen_math_ops.rsqrt_grad(input1, input2)
    with tf.Session(config=session_config) as session:
        result = session.run(output_var, feed_dict={input1: y_data, input2: dy_data})
    return (result,)
