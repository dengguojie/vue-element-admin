...
class FractionalMaxPool_UT : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "FractionalMaxPool_UT SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "FractionalMaxPool_UT TearDown" << std::endl;
  }
};

TEST_F(FractionalMaxPool_UT, InfershapeFractionalMaxPool_001) {
  ge::op::FractionalMaxPool op;
...
TEST_F(FractionalMaxPool_UT, InfershapeFractionalMaxPool_002) {
  ge::op::FractionalMaxPool op;
...
TEST_F(FractionalMaxPool_UT, InfershapeFractionalMaxPool_003) {
  ge::op::FractionalMaxPool op;
  op.UpdateInputDesc("x", create_desc({-1, 2, 2, -4}, ge::DT_FLOAT));
...
TEST_F(FractionalMaxPool_UT, InfershapeFractionalMaxPool_004) {
  ge::op::FractionalMaxPool op;
  op.UpdateInputDesc("x", create_desc({2, 2, 2, 2}, ge::DT_FLOAT));
  std::vector<float> pooling_ratio = {1.0, 1.0, 1.0, 1.0};
  op.SetAttr("pooling_ratio", pooling_ratio);
