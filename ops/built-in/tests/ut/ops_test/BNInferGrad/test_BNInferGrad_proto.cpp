...
class bn_infer_grad:public testing::Test{
    protected:
        static void SetUpTestCase(){
            std::cout<<"bn_infer_grad Proto Test SetUp"<<std::endl;
        }

        static void TearDownTestCase(){
            std::cout<<"bn_infer_grad Proto Test TearDown"<<std::endl;
        }
};


TEST_F(bn_infer_grad,bn_infer_grad_infershape_diff_test){
    ...
    op.UpdateInputDesc("batch_variance", tensor_desc);
    op.SetAttr("epsilon", (float)0.0001);
    auto ret = op.InferShapeAndType();
    EXPECT_EQ(ret, ge::GRAPH_SUCCESS);
    auto output_y1_desc = op.GetOutputDesc("x_backprop");
