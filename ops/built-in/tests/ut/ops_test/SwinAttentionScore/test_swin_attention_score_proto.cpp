...
class SwinAttentionScoreTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "SwinAttentionScore Proto Test SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "SwinAttentionScore Proto Test TearDown" << std::endl;
  }
};


TEST_F(SwinAttentionScoreTest, SwinAttentionScoreInferShapeTest_1) {
  int32_t batch_dim0 = 32;
  int32_t batch_dim1 = 16;
  int32_t seq_dim = 384;
  int32_t n_dim = 64;

  ge::op::SwinAttentionScore op;
  ...
  op.UpdateInputDesc("value", create_desc_shape_range({batch_dim0, batch_dim1, seq_dim, n_dim},
                                                      ...
                                                      {{batch_dim0, batch_dim0}, {batch_dim1, batch_dim1},
                                                       {seq_dim, seq_dim}, {n_dim, n_dim}}));
  op.UpdateInputDesc("padding_mask1", create_desc_shape_range({1, batch_dim1, seq_dim, seq_dim},
                                                         ge::DT_FLOAT16, ge::FORMAT_ND,
                                                      {batch_dim0, 1, seq_dim, seq_dim}, ge::FORMAT_ND,
                                                      {{batch_dim0, batch_dim0}, {1, 1},
                                                       {seq_dim, seq_dim}, {seq_dim, seq_dim}}));
  op.UpdateInputDesc("padding_mask2", create_desc_shape_range({batch_dim0, 1, seq_dim, seq_dim},
                                                         ge::DT_FLOAT16, ge::FORMAT_ND,
                                                      {batch_dim0, 1, seq_dim, seq_dim}, ge::FORMAT_ND,
                                                      {{batch_dim0, batch_dim0}, {1, 1},
                                                       {seq_dim, seq_dim}, {seq_dim, seq_dim}}));
