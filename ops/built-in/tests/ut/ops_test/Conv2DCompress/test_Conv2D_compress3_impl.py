...
def test_conv2d_compress_impl1(test_arg):
    ...
    def requant_compute_fusion(x, sqrt_mode=False, relu_flag=False, platform="v100"):
        if len(x.shape) == 5:
            ...
            raise RuntimeError("requant x shape is not support")
        deqscale_shape = [1, channel_in1, 1, 1, channel_in0]
        deqscale_data = tvm.placeholder(deqscale_shape, name="requant_data",
                                        dtype="float16" if platform == "v100" else "uint64",
                                        attrs={"ori_shape": [channel_in1*channel_in0]})
        input_tensors = []
        input_tensors.append(deqscale_data)
        requant_res = ascend_requant_compute(x, deqscale_data, None, relu_flag=relu_flag)
    ...
    def fusion_conv2dcompress_compute(testcases):
        ...
        for testcase in testcases:
            ...
            total_input_tensors.append(requant_res)
            res = requant_res
