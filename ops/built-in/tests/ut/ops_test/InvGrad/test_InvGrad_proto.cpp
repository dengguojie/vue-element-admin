...
class InvGrad : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "InvGrad SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "InvGrad TearDown" << std::endl;
  }
};

TEST_F(InvGrad, inv_grad_infer_shape_fp16) {
  ge::op::InvGrad op;
...
TEST_F(InvGrad, inv_grad_infer_shape_1) {
  ge::op::InvGrad op;
  ...
  op.UpdateInputDesc("grad", tensor_desc_x2);
   auto status = op.VerifyAllAttr(true);
  EXPECT_EQ(status, ge::GRAPH_SUCCESS);
