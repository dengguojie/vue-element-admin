...
def test_conv2d_dynamic_transdata_fusion(test_arg):
    ...
    def conv2d_transdata_fusion_compile(mode):
        bias_flag = False
        strides = [-1, -1, -1, -1]
        pads = [1, 1, 1, 1]
        dilations = [1, 1, 1, 1]
    ...
    testcase = ["online", "opc"]
    for mode in testcase:
        print("conv2d_transdata_fusion_compile mode:", mode)
        conv2d_transdata_fusion_compile(mode)
