...
def test_conv2d_aipp_maxpool_quant_fusion(test_arg):
    ...
    def aipp_conv_relu_maxpooling_template( shape_in_ori, shape_w_ori, \
        ...
        with tvm.target.cce():
            ...
            if soc_version in ["Ascend610", "Ascend310P3", "Ascend310B1"
                               ] and C04:
        ...
        #     "print_ir": False,
        #     "need_build": True,
        #     "name": "aipp_conv2d_fixpipe_maxpool",
    ...
    def aipp_conv_relu_maxpooling_test_cases2():
        with tvm.target.cce():
            ...
            bias = tvm.placeholder((64, ), name="bias", dtype="float16")
            offset_w = None
            outputs = None
            strides = [1, 2, 2, 1]
            pads = [2, 3, 2, 3]
            dilations = [1, 1, 1, 1]
            conv_res = conv2d_compute(inputs,
                                      weights,
                                      bias,
                                      offset_w,
                                      outputs,
            ...
            unit_list = ["pre_act"]
            eltwise_mode = ""
            relu = fixpipe_compute(conv_res, x2, quant_scale_0, relu_weight_0,
                                   clip_value_0, quant_scale_1, relu_weight_1,
                                   clip_value_1, anti_quant_scale,
                                   anti_quant_offset, output, fusion_op_list,
                                   unit_list, eltwise_mode)
            ...
            out = ascend_quant_compute(pool_res,
                                       None,
                                       scale=0.5,
                                       offset=0.5,
                                       sqrt_mode=False)
            ...
            tensor_list = [aipp_input_data, weights, bias, out]
...
print("adding Conv2D v300 aipp maxpool quant fusion testcases")
ut_case.add_cust_test_func('Ascend310B1',
                           test_func=test_conv2d_aipp_maxpool_quant_fusion)
