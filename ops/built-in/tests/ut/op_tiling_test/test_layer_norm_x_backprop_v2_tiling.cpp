...
TEST_F(LayerNormXBackpropV2Tiling, LayerNormXBackpropV2_tiling_test_1) {
  ...
  std::string compileInfo = R"({
                        ...
                        "CORE_NUM":32,
                        "MAX_DTYPE": 4,
                        "COEXISTING_QUANTITY": 8,
                        "_vars": {"10000": ["dim_0", "dim_1", "dim_2"]},
                        "_normal_vars": {"10000": []},
                        "_attr_vars": {"10000": []},
                        "_custom_vars": {"10000": ["dim_0", "dim_1", "dim_2"]}
                        })";
...
TEST_F(LayerNormXBackpropV2Tiling, LayerNormXBackpropV2_tiling_test_2) {
  ...
  std::string compileInfo = R"({
                        ...
                        "MAX_DTYPE": 4,
                        "COEXISTING_QUANTITY": 8,
                        "_vars": {"20000": ["dim_0", "dim_1", "dim_2"]},
                        "_normal_vars": {"20000": []},
                        "_attr_vars": {"20000": []},
                        "_custom_vars": {"20000": ["dim_0", "dim_1", "dim_2"]}
                        })";

  std::vector<std::vector<int64_t>> inputs{{64, 114, 16, 16}, {64, 114, 16, 16}, {1824, 1}, {1824, 1}, {1024}};
  std::vector<std::vector<int64_t>> outputs{{64, 1824, 16}, {64, 1824, 16}};
  std::vector<ge::DataType> input_types{ge::DT_FLOAT, ge::DT_FLOAT, ge::DT_FLOAT, ge::DT_FLOAT, ge::DT_FLOAT};
  std::vector<ge::DataType> output_types{ge::DT_FLOAT, ge::DT_FLOAT};
  ge::Format data_format = ge::FORMAT_ND;
  ge::Format data_format_fractal_nz = ge::FORMAT_FRACTAL_NZ;

  TensorDesc tensor_inputA;
  tensor_inputA.SetShape(ge::Shape(inputs[0]));
  tensor_inputA.SetDataType(input_types[0]);
  tensor_inputA.SetFormat(data_format_fractal_nz);
  TensorDesc tensor_inputB;
  tensor_inputB.SetShape(ge::Shape(inputs[1]));
  tensor_inputB.SetDataType(input_types[1]);
  tensor_inputB.SetFormat(data_format_fractal_nz);
  TensorDesc tensor_inputC;
  tensor_inputC.SetShape(ge::Shape(inputs[2]));
  tensor_inputC.SetDataType(input_types[2]);
  tensor_inputC.SetFormat(data_format);
  ...
  EXPECT_EQ(to_string(runInfo.GetAllTilingData()), "64 114 1824 ");
}