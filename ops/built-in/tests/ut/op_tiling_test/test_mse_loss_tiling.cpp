...
class MseLossTiling : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "MseLossTiling SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "MseLossTiling TearDown" << std::endl;
  }
};
...
TEST_F(MseLossTiling, MseLoss_tiling_test_1) {
  auto iter = optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().find("MseLoss");
  ASSERT_TRUE(iter != optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().end());
  auto opParas = op::MseLoss("MseLoss");
...
TEST_F(MseLossTiling, MseLoss_tiling_test_2) {
  auto iter = optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().find("MseLoss");
  ASSERT_TRUE(iter != optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().end());
  auto opParas = op::MseLoss("MseLoss");
  ...
  TENSOR_INPUT_WITH_SHAPE(opParas, predict, input_shapes[0], dtypes[0], ge::FORMAT_ND, {});
  TENSOR_INPUT_WITH_SHAPE(opParas, label, input_shapes[1], dtypes[1], ge::FORMAT_ND, {});
  TENSOR_OUTPUT_WITH_SHAPE(opParas, y, {1}, ge::DT_FLOAT16, ge::FORMAT_ND, {});
  std::string compileInfo = R"({"reduction": "mean",
   "_reduce_vars": {"4294966896": [20000,20001,30000, 40000]},
