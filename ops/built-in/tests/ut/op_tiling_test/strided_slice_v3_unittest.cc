...
class StridedSliceV3UT : public testing::Test {
 protected:
  ...
  template <typename T>
  gert::TilingContextFaker ConstructContextFaker(gert::StorageShape &x_shape, gert::StorageShape &y_shape,
                                                 std::map<std::string, std::vector<T>> &value_dict,
                                                 optiling::StridedSliceV3CompileInfo &compile_info,
                                                 std::unique_ptr<uint8_t[]>& param) {
...
TEST_F(StridedSliceV3UT, stried_slice_v3_normal) {
  ...
  std::map<std::string, std::vector<int32_t>> value_dict = {
      ...
      {"axis", {0, 1, 2, 3}},
      {"stride", {1, 1, 1, 1}}};
  gert::StorageShape x_shape = {{4, 4, 4, 4}, {4, 4, 4, 4}};
  gert::StorageShape y_shape = {{3, 3, 3, 3}, {3, 3, 3, 3}};

  auto param = gert::TilingData::CreateCap(2048);
  auto faker = ConstructContextFaker<int32_t>(x_shape, y_shape, value_dict, compile_info, param);
  ...
  EXPECT_EQ(to_string<int64_t>(tiling_data->GetData(), tiling_data->GetDataSize()),
            "1 4 4 4 4 4 3 3 3 3 0 0 0 0 3 3 3 3 1 1 1 1 ");
}
