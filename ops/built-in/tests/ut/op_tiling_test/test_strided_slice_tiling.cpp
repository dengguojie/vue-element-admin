...
TEST_F(stried_slice_tiling, stried_slice_tiling_mode_7_new_axis) {
  auto iter = optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().find("StridedSlice");
  ASSERT_TRUE(iter != optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().end());
  auto opParas = op::StridedSlice("StridedSlice");
  vector<vector<int64_t>> input_shapes = {
  ...
  opParas.SetAttr("end_mask", 0);
  opParas.SetAttr("ellipsis_mask", 0);
  opParas.SetAttr("new_axis_mask", 2);
  opParas.SetAttr("shrink_axis_mask", 0);

  std::string compileInfo =
      R"({"vars": {"block_dim": 32, "begin_mask": 0, "end_mask": 0, "ellipsis_mask": 0, "new_axis_mask": 2, "shrink_axis_mask": 0, "ub_size": 261762}})";
  optiling::utils::OpCompileInfo op_compile_info(this->test_info_->name(), compileInfo);
  optiling::utils::OpRunInfo runInfo;
...
TEST_F(stried_slice_tiling, stried_slice_empty_input) {
  auto iter = optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().find("StridedSlice");
  ASSERT_TRUE(iter != optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().end());
  auto opParas = op::StridedSlice("StridedSlice");
  vector<vector<int64_t>> input_shapes = {
  ...
  std::string compileInfo =
      R"({"vars": {"block_dim": 32, "begin_mask": 0, "end_mask": 0, "ellipsis_mask": 1, "new_axis_mask": 2, "shrink_axis_mask": 0, "ub_size": 261762}})";
  optiling::utils::OpCompileInfo op_compile_info(this->test_info_->name(), compileInfo);
  optiling::utils::OpRunInfo runInfo;

  auto ret = iter->second.tiling_func_v2_(opParas, op_compile_info, runInfo);
  ASSERT_TRUE(ret);
  EXPECT_EQ(to_string(runInfo.GetAllTilingData()), "7 1 1 1 0 1 1 ");
}

TEST_F(stried_slice_tiling, stried_slice_empty_input1) {
  auto iter = optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().find("StridedSlice");
  ASSERT_TRUE(iter != optiling::OpTilingFuncRegistry::RegisteredOpFuncInfo().end());
  auto opParas = op::StridedSlice("StridedSlice");
  vector<vector<int64_t>> input_shapes = {
  ...
  opParas.SetAttr("end_mask", 0);
  opParas.SetAttr("ellipsis_mask", 1);
  opParas.SetAttr("new_axis_mask", 3);
  opParas.SetAttr("shrink_axis_mask", 0);

  std::string compileInfo =
      R"({"vars": {"block_dim": 32, "begin_mask": 0, "end_mask": 0, "ellipsis_mask": 1, "new_axis_mask": 3, "shrink_axis_mask": 0, "ub_size": 261762}})";
  optiling::utils::OpCompileInfo op_compile_info(this->test_info_->name(), compileInfo);
  optiling::utils::OpRunInfo runInfo;
