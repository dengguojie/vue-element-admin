...
TEST_F(GEMMTiling, GEMM_op_tiling_obj_matmul) {
  ...
  const ge::AscendString compileInfo = R"({"_pattern": "Matmul", "dynamic_mode":"dynamic_mkn", "repo_seeds": {}, "repo_range": {}, "cost_range": {"10000": [1, 3, 1, 3, 1, 4]}, "block_dim": {"10000": 2}, "attrs":{"transpose_a": false, "transpose_b": false}})";
...
TEST_F(GEMMTiling, GEMM_op_tiling_obj_batchmatmul) {
  ...
  const ge::AscendString compileInfo = R"({"_pattern": "BatchMatMul", "dynamic_mode":"dynamic_mknb", "repo_seeds": {"10000": [1, 3, 3, 2]}, "repo_range": {"10000": [1, 3, 1, 3, 1, 4]}, "cost_range": {}, "block_dim": {"10000": 2}, "attrs":{"transpose_a": true, "transpose_b": true}})";

  ge::Graph graph("batchmatmul_op_tiling_test_0");

  auto x1_shape = vector<int64_t>({3, 2});
  ge::TensorDesc desc_x1(ge::Shape(x1_shape), ge::FORMAT_ND, ge::DT_FLOAT16);
  desc_x1.SetOriginShape(ge::Shape(x1_shape));
  auto x1 = op::Data("x1");
  x1.update_input_desc_x(desc_x1);
  ...
  auto x2_shape = vector<int64_t>({3, 4, 3});
  ge::TensorDesc desc_x2(ge::Shape(x2_shape), ge::FORMAT_ND, ge::DT_FLOAT16);
  desc_x2.SetOriginShape(ge::Shape(x2_shape));
  auto x2 = op::Data("x2");
  x2.update_input_desc_x(desc_x2);
  ...
  auto y_shape = vector<int64_t>({3, 2, 4});
  ge::TensorDesc output_desc_y(ge::Shape(y_shape), ge::FORMAT_ND, ge::DT_FLOAT16);
  matmul.update_output_desc_y(output_desc_y);
  ...
  optiling::utils::OpCompileInfo op_compile_info("BatchMatMul", compileInfo);
  optiling::utils::OpRunInfo runInfo;
  ASSERT_TRUE(iter->second(matmul, op_compile_info, runInfo));
  EXPECT_EQ(runInfo.GetBlockDim(), 2);
  EXPECT_EQ(runInfo.GetTilingKey(), 10000);
