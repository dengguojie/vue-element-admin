...
TEST_F(Conv2DTransposeTiling, Conv2d_transpose_tiling_dynamic_nhw) {
  ...
  std::vector<std::string> input_types{"int32", "float16", "float16"};
  std::string output_dtype = "float16";
  std::vector<std::string> input_formats{"ND", "NCHW", "NCHW"};
  ...
  op_compile_info.key = "Conv2d_transpose_tiling_dynamic_nhw";
  OpRunInfo runInfo;
...
TEST_F(Conv2DTransposeTiling, Conv2d_transpose_tiling_fuzz_build_list_input) {
  ...
  std::string compileInfo = R"([{"_pattern": "Conv2d_backprop_input", "push_status": 0, "tiling_type": "dynamic_tiling", "repo_seeds": {}, "repo_range": {}, "cost_range": {"0": [1, 10, 10, 25, 10, 25]}, "block_dim": {"0": 2}, "_vars": {"0": ["batch_n", "dedy_h", "dx_h", "dedy_w", "dx_w"]}}, {"_pattern": "Conv2d_backprop_input", "push_status": 0, "tiling_type": "dynamic_tiling", "repo_seeds": {}, "repo_range": {}, "cost_range": {"1": [10, 20, 15, 35, 14, 35]}, "block_dim": {"1": 2}, "_vars": {"1": ["batch_n", "dedy_h", "dx_h", "dedy_w", "dx_w"]}}])";
  std::vector<std::vector<int64_t>> inputs {
    {4},
    {1, 64, 16, 16},
    {64, 32, 3, 3},
  ...
  OpCompileInfo op_compile_info;
  op_compile_info.str = compileInfo;
  op_compile_info.key = "Conv2d_transpose_tiling_fuzz_build_list_input";
  OpRunInfo runInfo;
  ASSERT_TRUE(iter->second(opParas, op_compile_info, runInfo));
  EXPECT_EQ(runInfo.block_dim, 2);
