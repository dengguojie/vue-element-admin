...
TEST_F(RoiAlignGradTiling, roi_align_grad_tiling_0) {
  ...
  EXPECT_EQ(to_string(runInfo.tiling_data), "2 32 1024 5 16 24 40 ");
}
...
TEST_F(RoiAlignGradTiling, roi_align_grad_tiling_1) {
  ...
  OpRunInfo runInfo;
  ASSERT_TRUE(iter->second.tiling_func_(opParas, op_compile_info, runInfo));
  EXPECT_EQ(to_string(runInfo.tiling_data), "1 32 1024 5 16 24 90 ");
}
...
TEST_F(RoiAlignGradTiling, roi_align_grad_tiling_2) {
  ...
  OpRunInfo runInfo;
  ASSERT_TRUE(iter->second.tiling_func_(opParas, op_compile_info, runInfo));
  EXPECT_EQ(to_string(runInfo.tiling_data), "3 32 1024 5 8 24 40 ");
}
