...
inline void RunKernelAtanh(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelAtanh(
  ...
  aicpu::NodeDefBuilder(node_def.get(), "Atanh", "Atanh")
      .Input({"x", data_type_in, dims_in, input0})
      .Output({"output", data_type_out, dims_out, output});
  RunKernelAtanh(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}
...
void CreateAndRunKernelAtanh(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelAtanh(dims, dims, input0, output, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAtanhParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelAtanh(dims_in, dims_out, input0, output,
                          aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAtanhParamInvalid(const std::vector<std::int64_t> &dims,
                                         Tin *input0, Tout *output) {
  CreateAndRunKernelAtanhParamInvalid(dims, dims, input0, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAtanhInnerError(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelAtanh(dims_in, dims_out, input0, output,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelAtanhInnerError(const std::vector<std::int64_t> &dims,
                                       Tin *input0, Tout *output) {
  CreateAndRunKernelAtanhInnerError(dims, dims, input0, output);
}
...
void RunTestAtanh() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "atanh/data/atanh_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "atanh/data/atanh_data_" +
...
TEST_F(TEST_ATANH_UT, DATA_TYPE_DT_FLOAT16) { RunTestAtanh<Eigen::half>(); }

TEST_F(TEST_ATANH_UT, DATA_TYPE_DT_FLOAT) { RunTestAtanh<std::float_t>(); }

TEST_F(TEST_ATANH_UT, DATA_TYPE_DT_DOUBLE) { RunTestAtanh<std::double_t>(); }

TEST_F(TEST_ATANH_UT, DATA_TYPE_DT_COMPLEX64) {
  RunTestAtanh<std::complex<std::float_t>>();
}

TEST_F(TEST_ATANH_UT, DATA_TYPE_DT_COMPLEX128) {
  RunTestAtanh<std::complex<std::double_t>>();
}
// exception instance
TEST_F(TEST_ATANH_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelAtanhInnerError({2000, 6000}, float_0_, float_0_);
}

TEST_F(TEST_ATANH_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelAtanhParamInvalid({2, 6}, {2, 8}, float_12_, float_16_);
}

TEST_F(TEST_ATANH_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelAtanhParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_ATANH_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAtanhParamInvalid({2, 11}, float_null_, float_null_);
}

TEST_F(TEST_ATANH_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAtanhParamInvalid({0, 0}, float_0_, float_null_);
}

TEST_F(TEST_ATANH_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "Atanh", "Atanh")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelAtanh(node_def, aicpu::DeviceType::HOST,
                 aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_ATANH_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelAtanhParamInvalid({2, 11}, bool_22_, bool_22_);
}
