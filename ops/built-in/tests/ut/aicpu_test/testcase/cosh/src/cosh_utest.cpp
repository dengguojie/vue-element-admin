...
inline void RunKernelCosh(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelCosh(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelCosh(dims, dims, input0, output, status, bad_kernel);
}
...
void CreateAndRunKernelCoshParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelCosh(dims_in, dims_out, input0, output,
                         aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelCoshParamInvalid(const std::vector<std::int64_t> &dims,
                                        Tin *input0, Tout *output) {
  CreateAndRunKernelCoshParamInvalid(dims, dims, input0, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelCoshInnerError(const std::vector<std::int64_t> &dims_in,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelCoshInnerError(const std::vector<std::int64_t> &dims,
                                      Tin *input0, Tout *output) {
  CreateAndRunKernelCoshInnerError(dims, dims, input0, output);
}
...
void RunTestCosh() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "cosh/data/cosh_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "cosh/data/cosh_data_" +
...
TEST_F(TEST_COSH_UT, DATA_TYPE_DT_FLOAT16) { RunTestCosh<Eigen::half>(); }

TEST_F(TEST_COSH_UT, DATA_TYPE_DT_FLOAT) { RunTestCosh<std::float_t>(); }

TEST_F(TEST_COSH_UT, DATA_TYPE_DT_DOUBLE) { RunTestCosh<std::double_t>(); }

TEST_F(TEST_COSH_UT, DATA_TYPE_DT_COMPLEX64) {
  RunTestCosh<std::complex<std::float_t>>();
}

TEST_F(TEST_COSH_UT, DATA_TYPE_DT_COMPLEX128) {
  RunTestCosh<std::complex<std::double_t>>();
}
// exception instance
TEST_F(TEST_COSH_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelCoshInnerError({2000, 6000}, float_0_, float_0_);
}
...
TEST_F(TEST_COSH_UT, NO_OUTPUT_EXCEPTION) {
  ...
  RunKernelCosh(node_def, aicpu::DeviceType::HOST,
                aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}
