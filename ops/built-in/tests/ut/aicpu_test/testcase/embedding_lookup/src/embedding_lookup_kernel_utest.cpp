...
TEST_F(TEST_EmbeddingLookup_UTest, EmbeddingLookup_INT8_Success) {
  // raw data
  int8_t expect_out[8] = {10, 11, 0, 0, 0, 0, 10, 11};
  int8_t input_params[8] = {8, 9, 10, 11, 12, 13, 14, 15};
  int8_t input_indices[4] = {5, 2, 8, 5};
  int8_t offset[1] = {4};
  int8_t output[8] = {0};
  ...
  std::vector<int64_t> shapes0 = {4, 2};
  aicpuShape0->SetDimSizes(shapes0);
  inputTensor0->SetDataType(DT_INT8);
  inputTensor0->SetData(input_params);
  inputTensor0->SetDataSize(8 * sizeof(int8_t));
  ...
  std::vector<int64_t> shapes1 = {2, 2};
  aicpuShape1->SetDimSizes(shapes1);
  inputTensor1->SetDataType(DT_INT8);
  inputTensor1->SetData(input_indices);
  ...
  std::vector<int64_t> shapes2 = {4};
  aicpuShape2->SetDimSizes(shapes2);
  inputTensor2->SetDataType(DT_INT8);
  inputTensor2->SetData(offset);
  inputTensor2->SetDataSize(1 * sizeof(int8_t));

  // set output
  auto outputTensor1 = nodeDef->AddOutputs();
  EXPECT_NE(outputTensor1, nullptr);
  outputTensor1->SetDataType(DT_INT8);
  outputTensor1->SetData(output);
  outputTensor1->SetDataSize(8 * sizeof(int8_t));
  ...
  for (int i = 0; i < 8; ++i)
    std::cout << output[i] << " ";
  std::cout << std::endl;
  EXPECT_EQ(0, std::memcmp(output, expect_out, 8 * sizeof(int8_t)));
}

TEST_F(TEST_EmbeddingLookup_UTest, EmbeddingLookup_INT16_Success) {
  // raw data
  int16_t expect_out[8] = {10, 11, 0, 0, 0, 0, 10, 11};
  int16_t input_params[8] = {8, 9, 10, 11, 12, 13, 14, 15};
  int16_t input_indices[4] = {5, 2, 8, 5};
  int16_t offset[1] = {4};
  int16_t output[8] = {0};
  ...
  std::vector<int64_t> shapes0 = {4, 2};
  aicpuShape0->SetDimSizes(shapes0);
  inputTensor0->SetDataType(DT_INT16);
  inputTensor0->SetData(input_params);
  inputTensor0->SetDataSize(8 * sizeof(int16_t));
  ...
  std::vector<int64_t> shapes1 = {2, 2};
  aicpuShape1->SetDimSizes(shapes1);
  inputTensor1->SetDataType(DT_INT16);
  inputTensor1->SetData(input_indices);
  ...
  std::vector<int64_t> shapes2 = {4};
  aicpuShape2->SetDimSizes(shapes2);
  inputTensor2->SetDataType(DT_INT16);
  inputTensor2->SetData(offset);
  inputTensor2->SetDataSize(1 * sizeof(int16_t));

  // set output
  auto outputTensor1 = nodeDef->AddOutputs();
  EXPECT_NE(outputTensor1, nullptr);
  outputTensor1->SetDataType(DT_INT16);
  outputTensor1->SetData(output);
  outputTensor1->SetDataSize(8 * sizeof(int16_t));
  ...
  for (int i = 0; i < 8; ++i)
    std::cout << output[i] << " ";
  std::cout << std::endl;
  EXPECT_EQ(0, std::memcmp(output, expect_out, 8 * sizeof(int16_t)));
}