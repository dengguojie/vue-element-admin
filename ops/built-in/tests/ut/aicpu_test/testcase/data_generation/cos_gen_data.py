...
def gen_random_data(nptype, shape, rand_type, low, high):
    ...
    write_file_txt("cos/data/cos_data_input_0_" + nptype.__name__ + ".txt", x_batch)
    x_batch.tofile("cos/data/cos_data_input_0_" + nptype.__name__ + ".bin")
    x_placeholder = tf.compat.v1.placeholder(nptype, shape=shape)

    write_file_txt(
        "cos/data/cos_data_" + nptype.__name__ + "_dim.txt", np.array([len(shape)])
    )
    write_file_txt(
        "cos/data/cos_data_" + nptype.__name__ + "_shape.txt", np.array(shape)
    )
    result = tf.cos(x_placeholder)
    with tf.compat.v1.Session(config=config("cpu")) as session:
        data = session.run(result, feed_dict={x_placeholder: x_batch})
    write_file_txt("cos/data/cos_data_output_" + nptype.__name__ + "_expect.txt", data)
    data.tofile("cos/data/cos_data_output_" + nptype.__name__ + "_expect.bin")
