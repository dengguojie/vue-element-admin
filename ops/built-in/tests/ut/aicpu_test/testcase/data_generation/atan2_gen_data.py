...
def gen_random_data(nptype, shape, rand_type, low, high):
    ...
    y_batch = gen_data(shape, nptype, rand_type, low, high)
    write_file_txt("atan2/data/atan2_data_input_0_" + nptype.__name__ + ".txt", y_batch)
    y_batch.tofile("atan2/data/atan2_data_input_0_" + nptype.__name__ + ".bin")
    y_placeholder = tf.compat.v1.placeholder(nptype, shape=shape)
    x_batch = gen_data(shape, nptype, rand_type, low, high)
    write_file_txt("atan2/data/atan2_data_input_1_" + nptype.__name__ + ".txt", x_batch)
    x_batch.tofile("atan2/data/atan2_data_input_1_" + nptype.__name__ + ".bin")
    x_placeholder = tf.compat.v1.placeholder(nptype, shape=shape)

    write_file_txt("atan2/data/atan2_data_" + nptype.__name__ + "_dim.txt", np.array([len(shape)]))
    write_file_txt("atan2/data/atan2_data_" + nptype.__name__ + "_shape.txt", np.array(shape))
    result = tf.atan2(y_placeholder, x_placeholder)
    with tf.compat.v1.Session(config=config('cpu')) as session:
        data = session.run(result, feed_dict={y_placeholder: y_batch,x_placeholder: x_batch})
    write_file_txt("atan2/data/atan2_data_output_" + nptype.__name__ + "_expect.txt", data)
    data.tofile("atan2/data/atan2_data_output_" + nptype.__name__ + "_expect.bin")
