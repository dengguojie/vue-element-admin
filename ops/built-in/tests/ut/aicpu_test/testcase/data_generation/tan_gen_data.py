...
def gen_random_data(nptype, shape, rand_type, low, high):
    ...
    write_file_txt("tan/data/tan_data_input_" + nptype.__name__ + ".txt", x_batch, fmt="%s")
    write_file_txt("tan/data/tan_data_" + nptype.__name__ + "_dim.txt", np.array([len(shape)]), fmt="%s")
    write_file_txt("tan/data/tan_data_" + nptype.__name__ + "_shape.txt", np.array(shape), fmt="%s")
    x_batch.tofile("tan/data/tan_data_input_" + nptype.__name__ + ".bin")
    x_placeholder = tf.compat.v1.placeholder(nptype, shape=shape)
    result = tf.tan(x_placeholder)
    with tf.compat.v1.Session(config=config('cpu')) as session:
        data = session.run(result, feed_dict={x_placeholder: x_batch})
    write_file_txt("tan/data/tan_data_output_" + nptype.__name__ + ".txt", data, fmt="%s")
    data.tofile("tan/data/tan_data_output_" + nptype.__name__ + ".bin")
