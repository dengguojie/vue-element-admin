...
def gen_random_data(nptype, shape, rand_type, low, high):
    ...
    write_file_txt("atanh/data/atanh_data_input_0_" + nptype.__name__ + ".txt", x_batch)
    x_batch.tofile("atanh/data/atanh_data_input_0_" + nptype.__name__ + ".bin")
    x_placeholder = tf.compat.v1.placeholder(nptype, shape=shape)

    write_file_txt("atanh/data/atanh_data_" + nptype.__name__ + "_dim.txt", np.array([len(shape)]))
    write_file_txt("atanh/data/atanh_data_" + nptype.__name__ + "_shape.txt", np.array(shape))
    result = tf.atanh(x_placeholder)
    with tf.compat.v1.Session(config=config('cpu')) as session:
        data = session.run(result, feed_dict={x_placeholder: x_batch})
    write_file_txt("atanh/data/atanh_data_output_" + nptype.__name__ + "_expect.txt", data)
    data.tofile("atanh/data/atanh_data_output_" + nptype.__name__ + "_expect.bin")
...
def run():
    ...
    gen_random_data(np.complex64, [12, 130, 3], "uniform", -1, 1)
    gen_random_data(np.complex128, [12, 130, 3], "uniform", -1, 1)
