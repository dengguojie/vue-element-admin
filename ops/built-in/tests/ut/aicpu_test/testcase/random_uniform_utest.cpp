...
#define CREATE_NODEDEF(shapes, data_types, datas, seed1, seed2)        \
  ...
  if (seed1 == -1) {                                                   \
    NodeDefBuilder(node_def.get(), "RandomUniform", "RandomUniform")   \
...
#define RANDOM_UNIFORM_CASE_WITH_TYPE(case_name, base_type, aicpu_data_type,             \
  ...
  TEST_F(TEST_RANDOM_UNIFORM_UT, TestRandomUniform_##case_name) {                        \
...
RANDOM_UNIFORM_CASE_WITH_TYPE(float16_with_seed, Eigen::half, DT_FLOAT16, DT_FLOAT16, 10, -1)

RANDOM_UNIFORM_CASE_WITH_TYPE(float_with_seed2, float, DT_FLOAT, DT_FLOAT, 10, 5)

RANDOM_UNIFORM_CASE_WITH_TYPE(double_with_no_seed, double, DT_DOUBLE, DT_DOUBLE, -1, -1)
