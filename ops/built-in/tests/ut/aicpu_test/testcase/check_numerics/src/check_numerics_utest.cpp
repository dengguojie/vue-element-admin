...
class TEST_CHECK_NUMERICS_UT : public testing::Test {
 protected:
...
inline void RunKernelCheckNumerics(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelCheckNumerics(
  ...
  aicpu::NodeDefBuilder(node_def.get(), "CheckNumerics", "CheckNumerics")
      .Input({"x", data_type_in, dims_in, input0})
      .Output({"output", data_type_out, dims_out, output});
  RunKernelCheckNumerics(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}
...
void CreateAndRunKernelCheckNumerics(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelCheckNumerics(dims, dims, input0, output, status,
                                  bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelCheckNumericsParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelCheckNumerics(
      dims_in, dims_out, input0, output,
...
void CreateAndRunKernelCheckNumericsParamInvalid(
    const std::vector<std::int64_t> &dims, Tin *input0, Tout *output) {
  CreateAndRunKernelCheckNumericsParamInvalid(dims, dims, input0, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelCheckNumericsInnerError(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelCheckNumerics(
      dims_in, dims_out, input0, output,
...
void CreateAndRunKernelCheckNumericsInnerError(
    const std::vector<std::int64_t> &dims, Tin *input0, Tout *output) {
  CreateAndRunKernelCheckNumericsInnerError(dims, dims, input0, output);
}
...
template <typename Tin, typename Tout>
void RunTestCheckNumerics() {
  const auto data_name{ToDataName<Tin>()};

  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath +
                            "check_numerics/data/check_numerics_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath +
                              "check_numerics/data/check_numerics_data_" +
  ...
  std::string data_path0{ktestcaseFilePath +
                         "check_numerics/data/check_numerics_data_input_0_" +
                         data_name + ".bin"};
  EXPECT_EQ(ReadBinFile(data_path0, data0, input_size), true);

  Tout output[output_size];
  CreateAndRunKernelCheckNumerics(dims, data0, output);
  std::string out_data_actual_path{
      ktestcaseFilePath + "check_numerics/data/check_numerics_data_output_" +
      data_name + "_actual.txt"};
  EXPECT_EQ(WriteFile(out_data_actual_path, output, output_size), true);
  std::string out_data_actual_bin_path{
      ktestcaseFilePath + "check_numerics/data/check_numerics_data_output_" +
  ...
  std::string out_data_path{ktestcaseFilePath +
                            "check_numerics/data/check_numerics_data_output_" +
...
template <typename T>
void RunTestCheckNumerics() {
  RunTestCheckNumerics<T, T>();
}

TEST_F(TEST_CHECK_NUMERICS_UT, DATA_TYPE_DT_FLOAT16) {
  RunTestCheckNumerics<Eigen::half>();
}

TEST_F(TEST_CHECK_NUMERICS_UT, DATA_TYPE_DT_FLOAT) {
  RunTestCheckNumerics<std::float_t>();
}

TEST_F(TEST_CHECK_NUMERICS_UT, DATA_TYPE_DT_DOUBLE) {
  RunTestCheckNumerics<std::double_t>();
}
// exception instance
TEST_F(TEST_CHECK_NUMERICS_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelCheckNumericsInnerError({2000, 6000}, float_0_, float_0_);
}

TEST_F(TEST_CHECK_NUMERICS_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelCheckNumericsParamInvalid({2, 6}, {2, 8}, float_12_,
                                              float_16_);
}

TEST_F(TEST_CHECK_NUMERICS_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelCheckNumericsParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_CHECK_NUMERICS_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelCheckNumericsParamInvalid({2, 11}, float_null_,
                                              float_null_);
}

TEST_F(TEST_CHECK_NUMERICS_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelCheckNumericsParamInvalid({0, 0}, float_0_, float_null_);
}

TEST_F(TEST_CHECK_NUMERICS_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "CheckNumerics", "CheckNumerics")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelCheckNumerics(node_def, aicpu::DeviceType::HOST,
                         aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_CHECK_NUMERICS_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelCheckNumericsParamInvalid({2, 11}, bool_22_, bool_22_);
}

TEST_F(TEST_CHECK_NUMERICS_UT, INPUT_DTYPE_NAN) {
  CreateAndRunKernelCheckNumericsParamInvalid({2, 6}, float_12_nan_, float_12_);
}
