...
inline void RunKernelAddN(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelAddN(
  ...
  aicpu::NodeDefBuilder(node_def.get(), "AddN", "AddN")
      .Input({"input0", data_type_in, dims_in, input0})
      .Input({"input1", data_type_in, dims_in, input0})
      .Output({"output", data_type_out, dims_out, output})
      .Attr("N", 2);
  RunKernelAddN(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}
...
void CreateAndRunKernelAddN(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelAddN(dims, dims, input0, output, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAddNParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelAddN(dims_in, dims_out, input0, output,
                         aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAddNParamInvalid(const std::vector<std::int64_t> &dims,
                                        Tin *input0, Tout *output) {
  CreateAndRunKernelAddNParamInvalid(dims, dims, input0, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAddNInnerError(const std::vector<std::int64_t> &dims_in,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelAddNInnerError(const std::vector<std::int64_t> &dims,
                                      Tin *input0, Tout *output) {
  CreateAndRunKernelAddNInnerError(dims, dims, input0, output);
}
...
void RunTestAddN() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "add_n/data/add_n_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "add_n/data/add_n_data_" +
...
TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_INT8) { RunTestAddN<std::int8_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_INT16) { RunTestAddN<std::int16_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_INT32) { RunTestAddN<std::int32_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_INT64) { RunTestAddN<std::int64_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_UINT8) { RunTestAddN<std::uint8_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_UINT16) { RunTestAddN<std::uint16_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_UINT32) { RunTestAddN<std::uint32_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_UINT64) { RunTestAddN<std::uint64_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_FLOAT16) { RunTestAddN<Eigen::half>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_FLOAT) { RunTestAddN<std::float_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_DOUBLE) { RunTestAddN<std::double_t>(); }

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_COMPLEX64) {
  RunTestAddN<std::complex<std::float_t>>();
}

TEST_F(TEST_ADD_N_UT, DATA_TYPE_DT_COMPLEX128) {
  RunTestAddN<std::complex<std::double_t>>();
}
// exception instance
TEST_F(TEST_ADD_N_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelAddNInnerError({2000, 6000}, float_0_, float_0_);
}

TEST_F(TEST_ADD_N_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelAddNParamInvalid({2, 6}, {2, 8}, float_12_, float_16_);
}

TEST_F(TEST_ADD_N_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelAddNParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_ADD_N_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAddNParamInvalid({2, 11}, float_null_, float_null_);
}

TEST_F(TEST_ADD_N_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAddNParamInvalid({2, 11}, int32_22_, float_null_);
}

TEST_F(TEST_ADD_N_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "AddN", "AddN")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelAddN(node_def, aicpu::DeviceType::HOST,
                aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_ADD_N_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelAddNParamInvalid({2, 11}, bool_22_, bool_22_);
}
