...
inline void RunKernelAsin(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelAsin(
  ...
  RunKernelAsin(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}
...
void CreateAndRunKernelAsin(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelAsin(dims, dims, input, output, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAsinParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input, Tout *output) {
  CreateAndRunKernelAsin(dims_in, dims_out, input, output,
                         aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAsinParamInvalid(const std::vector<std::int64_t> &dims,
                                        Tin *input, Tout *output) {
  CreateAndRunKernelAsinParamInvalid(dims, dims, input, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAsinInnerError(const std::vector<std::int64_t> &dims_in,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelAsinInnerError(const std::vector<std::int64_t> &dims,
                                      Tin *input, Tout *output) {
  CreateAndRunKernelAsinInnerError(dims, dims, input, output);
}
...
void RunTestAsin() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "asin/data/asin_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "asin/data/asin_data_" +
  ...
  T data1[input1_size];
  std::string data_path{ktestcaseFilePath + "asin/data/asin_data_input_" +
                        data_name + ".bin"};
  EXPECT_EQ(ReadBinFile(data_path, data1, input1_size), true);

  T output[input1_size];
  CreateAndRunKernelAsin(dims, data1, output);
  std::string out_data_actual_path{ktestcaseFilePath +
                                   "asin/data/asin_data_output_" + data_name +
                                   "_actual.txt"};
  EXPECT_EQ(WriteFile(out_data_actual_path, output, input1_size), true);
  std::string out_data_actual_bin_path{ktestcaseFilePath +
                                       "asin/data/asin_data_output_" +
                                       data_name + "_actual.bin"};
  EXPECT_EQ(WriteBinFile(out_data_actual_bin_path, output, input1_size), true);

  T expect_out[input1_size];
  std::string out_data_path{ktestcaseFilePath + "asin/data/asin_data_output_" +
                            data_name + ".bin"};
...
#define ADD_CASE(base_type, aicpu_type)          \
  TEST_F(TEST_ASIN_UT, DATA_TYPE_##aicpu_type) { \
    RunTestAsin<base_type>();                    \
  }

// exception instance
TEST_F(TEST_ASIN_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelAsinInnerError({2, 6}, float_12_, float_12_);
}

TEST_F(TEST_ASIN_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelAsinParamInvalid({2, 6}, {2, 8}, float_12_, float_16_);
}

TEST_F(TEST_ASIN_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelAsinParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_ASIN_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAsinParamInvalid({2, 11}, float_null_, float_null_);
}

TEST_F(TEST_ASIN_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAsinParamInvalid({0, 0}, float_0_, float_null_);
}

TEST_F(TEST_ASIN_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "Asin", "Asin")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelAsin(node_def, aicpu::DeviceType::HOST,
                aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_ASIN_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelAsinParamInvalid({2, 11}, bool_22_, bool_22_);
}
