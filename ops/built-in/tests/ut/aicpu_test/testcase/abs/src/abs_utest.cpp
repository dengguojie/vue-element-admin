...
class TEST_ABS_UT : public testing::Test {
protected:
...
inline void RunKernelAbs(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelAbs(
  ...
  aicpu::NodeDefBuilder(node_def.get(), "Abs", "Abs")
      .Input({"x", data_type_in, dims_in, input0})
      .Output({"output", data_type_out, dims_out, output});
  RunKernelAbs(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}
...
void CreateAndRunKernelAbs(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelAbs(dims, dims, input0, output, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAbsParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelAbs(dims_in, dims_out, input0, output,
                        aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAbsParamInvalid(const std::vector<std::int64_t> &dims,
                                       Tin *input0, Tout *output) {
  CreateAndRunKernelAbsParamInvalid(dims, dims, input0, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAbsInnerError(const std::vector<std::int64_t> &dims_in,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelAbsInnerError(const std::vector<std::int64_t> &dims,
                                     Tin *input0, Tout *output) {
  CreateAndRunKernelAbsInnerError(dims, dims, input0, output);
}
...
void RunTestAbs() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "abs/data/abs_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "abs/data/abs_data_" +
  ...
  std::string data_path0{ktestcaseFilePath + "abs/data/abs_data_input_0_" +
                         data_name + ".bin"};
  EXPECT_EQ(ReadBinFile(data_path0, data0, input_size), true);

  Tout output[output_size];
  CreateAndRunKernelAbs(dims, data0, output);
  std::string out_data_actual_path{ktestcaseFilePath +
                                   "abs/data/abs_data_output_" + data_name +
                                   "_actual.txt"};
  ...
  std::string out_data_path{ktestcaseFilePath + "abs/data/abs_data_output_" +
...
TEST_F(TEST_ABS_UT, DATA_TYPE_DT_INT8) { RunTestAbs<std::int8_t>(); }

TEST_F(TEST_ABS_UT, DATA_TYPE_DT_INT16) { RunTestAbs<std::int16_t>(); }

TEST_F(TEST_ABS_UT, DATA_TYPE_DT_INT32) { RunTestAbs<std::int32_t>(); }

TEST_F(TEST_ABS_UT, DATA_TYPE_DT_INT64) { RunTestAbs<std::int64_t>(); }

TEST_F(TEST_ABS_UT, DATA_TYPE_DT_FLOAT16) { RunTestAbs<Eigen::half>(); }

TEST_F(TEST_ABS_UT, DATA_TYPE_DT_FLOAT) { RunTestAbs<std::float_t>(); }

TEST_F(TEST_ABS_UT, DATA_TYPE_DT_DOUBLE) { RunTestAbs<std::double_t>(); }
// exception instance
TEST_F(TEST_ABS_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelAbsInnerError({2000, 6000}, float_0_, float_0_);
}

TEST_F(TEST_ABS_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelAbsParamInvalid({2, 6}, {2, 8}, float_12_, float_16_);
}

TEST_F(TEST_ABS_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelAbsParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_ABS_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAbsParamInvalid({2, 11}, float_null_, float_null_);
}

TEST_F(TEST_ABS_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "Abs", "Abs")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelAbs(node_def, aicpu::DeviceType::HOST,
               aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_ABS_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelAbsParamInvalid({2, 11}, bool_22_, bool_22_);
}
