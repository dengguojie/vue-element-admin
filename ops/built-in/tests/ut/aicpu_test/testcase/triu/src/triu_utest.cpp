...
class TEST_TRIU_UT : public testing::Test {};

#define CREATE_NODEDEF(shapes, data_types, datas, diagonal)        \
  auto node_def = CpuKernelUtils::CpuKernelUtils::CreateNodeDef(); \
  NodeDefBuilder(node_def.get(), "Triu", "Triu")                   \
      .Input({"x", data_types[0], shapes[0], datas[0]})            \
      .Output({"y", data_types[1], shapes[1], datas[1]})           \
      .Attr("diagonal", diagonal);

template <typename T>
void RunTriuKernel1(vector<string> data_files, vector<DataType> data_types,
...
template <typename T>
void RunTriuKernel2(vector<DataType> data_types,
                    vector<vector<int64_t>> &shapes, int diagonal = 0) {
  uint64_t input_size = CalTotalElements(shapes, 0);
  T *input = new T[input_size];
...
TEST_F(TEST_TRIU_UT, DATA_TYPE_INT32_SUCC) {
  vector<DataType> data_types = {DT_INT32, DT_INT32};
  vector<vector<int64_t>> shapes = {{3, 4}, {3, 4}};
  vector<string> files{"triu/data/triu_data_input1_1.txt",
                       "triu/data/triu_data_output1_1.txt"};
  RunTriuKernel1<int32_t>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_INT64_SUCC) {
  vector<DataType> data_types = {DT_INT64, DT_INT64};
  vector<vector<int64_t>> shapes{{3, 5}, {3, 5}};
  vector<string> files{"triu/data/triu_data_input1_2.txt",
                       "triu/data/triu_data_output1_2.txt"};
  RunTriuKernel1<int64_t>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_FLOAT_SUCC) {
  vector<DataType> data_types = {DT_FLOAT, DT_FLOAT};
  vector<vector<int64_t>> shapes{{6, 4}, {6, 4}};
  vector<string> files{"triu/data/triu_data_input1_3.txt",
                       "triu/data/triu_data_output1_3.txt"};
  RunTriuKernel1<float>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_DOUBLE_SUCC) {
  vector<DataType> data_types = {DT_DOUBLE, DT_DOUBLE};
  vector<vector<int64_t>> shapes{{64, 64, 64}, {64, 64, 64}};
  vector<string> files{"triu/data/triu_data_input1_4.txt",
                       "triu/data/triu_data_output1_4.txt"};
  RunTriuKernel1<double>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_INT8_SUCC) {
  vector<DataType> data_types = {DT_INT8, DT_INT8};
  vector<vector<int64_t>> shapes{{12, 8}, {12, 8}};
  vector<string> files{"triu/data/triu_data_input1_5.txt",
                       "triu/data/triu_data_output1_5.txt"};
  RunTriuKernel1<int8_t>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_UINT8_SUCC) {
  vector<DataType> data_types = {DT_UINT8, DT_UINT8};
  vector<vector<int64_t>> shapes{{10, 20}, {10, 20}};
  vector<string> files{"triu/data/triu_data_input1_6.txt",
                       "triu/data/triu_data_output1_6.txt"};
  RunTriuKernel1<uint8_t>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_INT16_SUCC) {
  vector<DataType> data_types = {DT_INT16, DT_INT16};
  vector<vector<int64_t>> shapes{{9, 6}, {9, 6}};
  vector<string> files{"triu/data/triu_data_input1_7.txt",
                       "triu/data/triu_data_output1_7.txt"};
  RunTriuKernel1<int16_t>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_FLOAT16_SUCC) {
  vector<DataType> data_types = {DT_FLOAT16, DT_FLOAT16};
  vector<vector<int64_t>> shapes{{4, 5}, {4, 5}};
  vector<string> files{"triu/data/triu_data_input1_8.txt",
                       "triu/data/triu_data_output1_8.txt"};
  RunTriuKernel1<Eigen::half>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_BOOL_SUCC) {
  vector<DataType> data_types = {DT_BOOL, DT_BOOL};
  vector<vector<int64_t>> shapes{{6, 5}, {6, 5}};
  vector<string> files{"triu/data/triu_data_input1_9.txt",
                       "triu/data/triu_data_output1_9.txt"};
  RunTriuKernel1<bool>(files, data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_UINT16_SUCC) {
  vector<DataType> data_types = {DT_UINT16, DT_UINT16};
  vector<vector<int64_t>> shapes = {{6, 13}, {6, 13}};
  RunTriuKernel2<uint16_t>(data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_UINT32_SUCC) {
  vector<DataType> data_types = {DT_UINT32, DT_UINT32};
  vector<vector<int64_t>> shapes = {{6, 13}, {6, 13}};
  RunTriuKernel2<uint32_t>(data_types, shapes);
}

TEST_F(TEST_TRIU_UT, DATA_TYPE_UINT64_SUCC) {
  vector<DataType> data_types = {DT_UINT64, DT_UINT64};
  vector<vector<int64_t>> shapes = {{2, 3}, {2, 3}};
  RunTriuKernel2<uint64_t>(data_types, shapes);
}

TEST_F(TEST_TRIU_UT, ATTR_DIAGONAL_POSITIVE) {
  vector<DataType> data_types = {DT_DOUBLE, DT_DOUBLE};
  vector<vector<int64_t>> shapes{{8, 10}, {8, 10}};
  vector<string> files{"triu/data/triu_data_input1_10.txt",
                       "triu/data/triu_data_output1_10.txt"};
  RunTriuKernel1<double>(files, data_types, shapes, 7);
}

TEST_F(TEST_TRIU_UT, ATTR_DIAGONAL_NEGATIVE) {
  vector<DataType> data_types = {DT_INT32, DT_INT32};
  vector<vector<int64_t>> shapes{{12, 10}, {12, 10}};
  vector<string> files{"triu/data/triu_data_input1_11.txt",
                       "triu/data/triu_data_output1_11.txt"};
  RunTriuKernel1<int32_t>(files, data_types, shapes, -9);
}

TEST_F(TEST_TRIU_UT, INPUT_BATCH_MATRIXS) {
  vector<DataType> data_types = {DT_FLOAT, DT_FLOAT};
  vector<vector<int64_t>> shapes{{2, 3, 6, 8}, {2, 3, 6, 8}};
  vector<string> files{"triu/data/triu_data_input1_12.txt",
                       "triu/data/triu_data_output1_12.txt"};
  RunTriuKernel1<float>(files, data_types, shapes);
}

// exception instance
TEST_F(TEST_TRIU_UT, INPUT_SHAPE_EXCEPTION) {
  vector<DataType> data_types = {DT_INT32, DT_INT32};
...
TEST_F(TEST_TRIU_UT, INPUT_DTYPE_EXCEPTION) {
...
TEST_F(TEST_TRIU_UT, INPUT_NULL_EXCEPTION) {
  vector<DataType> data_types = {DT_INT32, DT_INT32};
...
TEST_F(TEST_TRIU_UT, INPUT_DIM_EXCEPTION) {
  vector<DataType> data_types = {DT_DOUBLE, DT_DOUBLE};
