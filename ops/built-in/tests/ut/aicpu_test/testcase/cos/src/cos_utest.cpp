...
class TEST_COS_UT : public testing::Test {
 protected:
...
inline void RunKernelCos(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelCos(
  ...
  aicpu::NodeDefBuilder(node_def.get(), "Cos", "Cos")
      .Input({"x", data_type_in, dims_in, input0})
      .Output({"output", data_type_out, dims_out, output});
  RunKernelCos(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}
...
void CreateAndRunKernelCos(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelCos(dims, dims, input0, output, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelCosParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelCos(dims_in, dims_out, input0, output,
                        aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelCosParamInvalid(const std::vector<std::int64_t> &dims,
                                       Tin *input0, Tout *output) {
  CreateAndRunKernelCosParamInvalid(dims, dims, input0, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelCosInnerError(const std::vector<std::int64_t> &dims_in,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelCosInnerError(const std::vector<std::int64_t> &dims,
                                     Tin *input0, Tout *output) {
  CreateAndRunKernelCosInnerError(dims, dims, input0, output);
}
...
void RunTestCos() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "cos/data/cos_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "cos/data/cos_data_" +
  ...
  std::string data_path0{ktestcaseFilePath + "cos/data/cos_data_input_0_" +
                         data_name + ".bin"};
  EXPECT_EQ(ReadBinFile(data_path0, data0, input_size), true);

  Tout output[output_size];
  CreateAndRunKernelCos(dims, data0, output);
  std::string out_data_actual_path{ktestcaseFilePath +
                                   "cos/data/cos_data_output_" + data_name +
                                   "_actual.txt"};
  ...
  std::string out_data_path{ktestcaseFilePath + "cos/data/cos_data_output_" +
...
TEST_F(TEST_COS_UT, DATA_TYPE_DT_FLOAT16) { RunTestCos<Eigen::half>(); }

TEST_F(TEST_COS_UT, DATA_TYPE_DT_FLOAT) { RunTestCos<std::float_t>(); }

TEST_F(TEST_COS_UT, DATA_TYPE_DT_DOUBLE) { RunTestCos<std::double_t>(); }

TEST_F(TEST_COS_UT, DATA_TYPE_DT_COMPLEX64) {
  RunTestCos<std::complex<std::float_t>>();
}

TEST_F(TEST_COS_UT, DATA_TYPE_DT_COMPLEX128) {
  RunTestCos<std::complex<std::double_t>>();
}
// exception instance
TEST_F(TEST_COS_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelCosInnerError({2000, 6000}, float_0_, float_0_);
}

TEST_F(TEST_COS_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelCosParamInvalid({2, 6}, {2, 8}, float_12_, float_16_);
}

TEST_F(TEST_COS_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelCosParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_COS_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelCosParamInvalid({2, 11}, float_null_, float_null_);
}

TEST_F(TEST_COS_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "Cos", "Cos")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelCos(node_def, aicpu::DeviceType::HOST,
               aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_COS_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelCosParamInvalid({2, 11}, bool_22_, bool_22_);
}
