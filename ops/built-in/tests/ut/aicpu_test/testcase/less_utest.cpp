...
void CalcExpectWithDiffShape(const NodeDef &node_def, bool expect_out[]) {
  ...
  if (input0_num > input1_num) {
    for (int64_t j = 0; j < input0_num; ++j) {
      expect_out[j] = input0_data[j] < input1_data[0] ? true : false;
    }
  }
}
...
#define ADD_CASE(base_type, aicpu_type)                                     \
  TEST_F(TEST_LESS_UT, TestLessBroad_##aicpu_type) {                        \
    ...
    CalcExpectWithSameShape<base_type>(*node_def.get(), expect_out);        \
    CompareResult<bool>(output, expect_out, 22);                            \
  }

#define ADD_CASE_WITH_BROADCAST(base_type, aicpu_type)                      \
  TEST_F(TEST_LESS_UT, TestLess_##aicpu_type) {                             \
    vector<DataType> data_types = {aicpu_type, aicpu_type, DT_BOOL};        \
    ...
    RUN_KERNEL(node_def, HOST, KERNEL_STATUS_OK);                           \
    bool expect_out[22] = {(bool)0};                                        \
    CalcExpectWithDiffShape<base_type>(*node_def.get(), expect_out);        \
    CompareResult<bool>(output, expect_out, 22);                            \
  }

TEST_F(TEST_LESS_UT, ExpInput) {
...
TEST_F(TEST_LESS_UT, ExpInputDiffDtype) {
...
TEST_F(TEST_LESS_UT, ExpInputNull) {
  vector<DataType> data_types = {DT_INT32, DT_INT32, DT_BOOL};
  vector<vector<int64_t>> shapes = {{2, 11}, {2, 11}, {2, 11}};
  bool output[22] = {(bool)0};
...
TEST_F(TEST_LESS_UT, ExpInputBool) {
