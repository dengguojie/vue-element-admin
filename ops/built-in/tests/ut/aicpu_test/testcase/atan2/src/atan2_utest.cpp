...
class TEST_ATAN2_UT : public testing::Test {
 protected:
...
inline void RunKernelAtan2(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelAtan2(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tin *input1,
    Tout *output,
  ...
  aicpu::NodeDefBuilder(node_def.get(), "Atan2", "Atan2")
      .Input({"y", data_type_in, dims_in, input0})
      .Input({"x", data_type_in, dims_in, input1})
      .Output({"output", data_type_out, dims_out, output});
  RunKernelAtan2(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAtan2(
    const std::vector<std::int64_t> &dims, Tin *input0, Tin *input1,
    Tout *output,
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelAtan2(dims, dims, input0, input1, output, status,
                          bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAtan2ParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tin *input1,
    Tout *output) {
  CreateAndRunKernelAtan2(dims_in, dims_out, input0, input1, output,
                          aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAtan2ParamInvalid(const std::vector<std::int64_t> &dims,
                                         Tin *input0, Tin *input1,
                                         Tout *output) {
  CreateAndRunKernelAtan2ParamInvalid(dims, dims, input0, input1, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAtan2InnerError(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tin *input1,
    Tout *output) {
  CreateAndRunKernelAtan2(dims_in, dims_out, input0, input1, output,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelAtan2InnerError(const std::vector<std::int64_t> &dims,
                                       Tin *input0, Tin *input1, Tout *output) {
  CreateAndRunKernelAtan2InnerError(dims, dims, input0, input1, output);
}
...
void RunTestAtan2() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "atan2/data/atan2_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "atan2/data/atan2_data_" +
  ...
  std::string data_path0{ktestcaseFilePath + "atan2/data/atan2_data_input_0_" +
                         data_name + ".bin"};
  EXPECT_EQ(ReadBinFile(data_path0, data0, input_size), true);
  Tin data1[input_size];
  std::string data_path1{ktestcaseFilePath + "atan2/data/atan2_data_input_1_" +
                         data_name + ".bin"};
  EXPECT_EQ(ReadBinFile(data_path1, data1, input_size), true);

  Tout output[output_size];
  CreateAndRunKernelAtan2(dims, data0, data1, output);
  std::string out_data_actual_path{ktestcaseFilePath +
                                   "atan2/data/atan2_data_output_" + data_name +
                                   "_actual.txt"};
  EXPECT_EQ(WriteFile(out_data_actual_path, output, output_size), true);
  std::string out_data_actual_bin_path{ktestcaseFilePath +
                                       "atan2/data/atan2_data_output_" +
...
TEST_F(TEST_ATAN2_UT, DATA_TYPE_DT_FLOAT16) { RunTestAtan2<Eigen::half>(); }

TEST_F(TEST_ATAN2_UT, DATA_TYPE_DT_FLOAT) { RunTestAtan2<std::float_t>(); }

TEST_F(TEST_ATAN2_UT, DATA_TYPE_DT_DOUBLE) { RunTestAtan2<std::double_t>(); }
// exception instance
TEST_F(TEST_ATAN2_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelAtan2InnerError({2000, 6000}, float_0_, float_0_, float_0_);
}

TEST_F(TEST_ATAN2_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelAtan2ParamInvalid({2, 6}, {2, 8}, float_12_, float_12_,
                                      float_16_);
}

TEST_F(TEST_ATAN2_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelAtan2ParamInvalid({2, 11}, int32_22_, int32_22_, int64_22_);
}

TEST_F(TEST_ATAN2_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAtan2ParamInvalid({2, 11}, float_null_, float_null_,
                                      float_null_);
}

TEST_F(TEST_ATAN2_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAtan2ParamInvalid({0, 0}, float_0_, float_0_, float_null_);
}

TEST_F(TEST_ATAN2_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "Atan2", "Atan2")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelAtan2(node_def, aicpu::DeviceType::HOST,
                 aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_ATAN2_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelAtan2ParamInvalid({2, 11}, bool_22_, bool_22_, bool_22_);
}
