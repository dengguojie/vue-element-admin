...
class TEST_SOFTPLUS_UT : public testing::Test {
protected:
...
inline void RunKernelSoftplus(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelSoftplus(
  ...
  aicpu::NodeDefBuilder(node_def.get(), "Softplus", "Softplus")
      .Input({"x", data_type_in, dims_in, input})
      .Output({"y", data_type_out, dims_out, output});
  RunKernelSoftplus(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}
...
void CreateAndRunKernelSoftplus(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelSoftplus(dims, dims, input, output, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelSoftplusParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input, Tout *output) {
  CreateAndRunKernelSoftplus(dims_in, dims_out, input, output,
...
void CreateAndRunKernelSoftplusParamInvalid(
    const std::vector<std::int64_t> &dims, Tin *input, Tout *output) {
  CreateAndRunKernelSoftplusParamInvalid(dims, dims, input, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelSoftplusInnerError(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input, Tout *output) {
  CreateAndRunKernelSoftplus(dims_in, dims_out, input, output,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelSoftplusInnerError(const std::vector<std::int64_t> &dims,
                                          Tin *input, Tout *output) {
  CreateAndRunKernelSoftplusInnerError(dims, dims, input, output);
}
...
void RunTestSoftplus(int flag) {
  if (flag == 0) {
    ...
    T data1[input1_size];
    std::string data_path{ktestcaseFilePath +
                          "softplus/data/softplus_data_input_" + data_name +
                          ".bin"};
    EXPECT_EQ(ReadBinFile(data_path, data1, input1_size), true);

    T output[input1_size];
    CreateAndRunKernelSoftplus(dims, data1, output);

    std::string out_data_actual_path{ktestcaseFilePath +
                                     "softplus/data/softplus_data_output_" +
    ...
    std::uint64_t dim[1];
    std::string data_dim_path{ktestcaseFilePath +
                              "softplus/data/softplus_bigdata_" + data_name +
                              "_dim.txt"};
    EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);
    ...
    T data1[input1_size];
    std::string data_path{ktestcaseFilePath +
                          "softplus/data/softplus_bigdata_input_" + data_name +
                          ".bin"};
    EXPECT_EQ(ReadBinFile(data_path, data1, input1_size), true);
    ...
    std::string out_data_actual_path{ktestcaseFilePath +
                                     "softplus/data/softplus_bigdata_output_" +
                                     data_name + "_actual.txt"};
    EXPECT_EQ(WriteFile(out_data_actual_path, output, input1_size), true);
...
#define ADD_CASE(base_type, aicpu_type)              \
  TEST_F(TEST_SOFTPLUS_UT, DATA_TYPE_##aicpu_type) { \
    RunTestSoftplus<base_type>(0);                   \
  }

#define ADD_BIGDATA_CASE(base_type, aicpu_type)         \
  TEST_F(TEST_SOFTPLUS_UT, BIGDATA_TYPE_##aicpu_type) { \
    RunTestSoftplus<base_type>(1);                      \
  }

// exception inssoftplusce
TEST_F(TEST_SOFTPLUS_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelSoftplusInnerError({2, 6}, float_12_, float_12_);
}

TEST_F(TEST_SOFTPLUS_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelSoftplusParamInvalid({2, 6}, {2, 8}, float_12_, float_16_);
}

TEST_F(TEST_SOFTPLUS_UT, INPUT_DIM_EXCEPTION) {
  CreateAndRunKernelSoftplusParamInvalid({2, 6}, {6, 2}, float_12_, float_12_);
}

TEST_F(TEST_SOFTPLUS_UT, INPUT_DIMSIZE_EXCEPTION) {
  CreateAndRunKernelSoftplusParamInvalid({2, 2, 3}, {6, 2}, float_12_,
                                         float_12_);
}

TEST_F(TEST_SOFTPLUS_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelSoftplusParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_SOFTPLUS_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelSoftplusParamInvalid({2, 11}, float_null_, float_null_);
}

TEST_F(TEST_SOFTPLUS_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelSoftplusParamInvalid({0, 0}, float_0_, float_null_);
}

TEST_F(TEST_SOFTPLUS_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "Softplus", "Softplus")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelSoftplus(node_def, aicpu::DeviceType::HOST,
                    aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_SOFTPLUS_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelSoftplusParamInvalid({2, 11}, bool_22_, bool_22_);
}
