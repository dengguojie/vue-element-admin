...
void CreateAndRunKernelAdjustSaturation(
  ...
  std::float_t delta[1]{0.5};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "AdjustSaturation", "AdjustSaturation")
      .Input({"image", data_type_in, dims_in, input0})
      .Input({"delta", aicpu::DataType::DT_FLOAT, {1}, delta})
      .Output({"output", data_type_out, dims_out, output});
  RunKernelAdjustSaturation(node_def, aicpu::DeviceType::HOST, status,
                            bad_kernel);
}
...
void CreateAndRunKernelAdjustSaturation(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelAdjustSaturation(dims, dims, input0, output, status,
                                     bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAdjustSaturationParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelAdjustSaturation(
...
void CreateAndRunKernelAdjustSaturationParamInvalid(
    const std::vector<std::int64_t> &dims, Tin *input0, Tout *output) {
  CreateAndRunKernelAdjustSaturationParamInvalid(dims, dims, input0, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAdjustSaturationInnerError(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelAdjustSaturation(
      dims_in, dims_out, input0, output,
...
void CreateAndRunKernelAdjustSaturationInnerError(
    const std::vector<std::int64_t> &dims, Tin *input0, Tout *output) {
  CreateAndRunKernelAdjustSaturationInnerError(dims, dims, input0, output);
}
...
void RunTestAdjustSaturation() {
  ...
  std::string data_path0{
      ktestcaseFilePath +
      "adjust_saturation/data/adjust_saturation_data_input_0_" + data_name +
      ".bin"};
  EXPECT_EQ(ReadBinFile(data_path0, data0, input_size), true);

  Tout output[output_size];
  CreateAndRunKernelAdjustSaturation(dims, data0, output);
  std::string out_data_actual_path{
      ktestcaseFilePath +
      "adjust_saturation/data/adjust_saturation_data_output_" + data_name +
      "_actual.txt"};
...
template <typename T>
void RunTestAdjustSaturation() {
  RunTestAdjustSaturation<T, T>();
}

TEST_F(TEST_ADJUST_SATURATION_UT, DATA_TYPE_DT_FLOAT16) {
  RunTestAdjustSaturation<Eigen::half>();
}

TEST_F(TEST_ADJUST_SATURATION_UT, DATA_TYPE_DT_FLOAT) {
  RunTestAdjustSaturation<std::float_t>();
}
// exception instance
TEST_F(TEST_ADJUST_SATURATION_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelAdjustSaturationInnerError({2000, 6000}, float_0_,
                                               float_0_);
}

TEST_F(TEST_ADJUST_SATURATION_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelAdjustSaturationParamInvalid({2, 6}, {2, 8}, float_12_,
                                                 float_16_);
}

TEST_F(TEST_ADJUST_SATURATION_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelAdjustSaturationParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_ADJUST_SATURATION_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAdjustSaturationParamInvalid({2, 11}, float_null_,
                                                 float_null_);
}

TEST_F(TEST_ADJUST_SATURATION_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelAdjustSaturationParamInvalid({2, 11}, int32_22_,
                                                 float_null_);
}
...
TEST_F(TEST_ADJUST_SATURATION_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelAdjustSaturationParamInvalid({2, 11}, bool_22_, bool_22_);
}
