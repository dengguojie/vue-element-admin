...
inline void RunKernelZeta(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelZeta(
  ...
  aicpu::NodeDefBuilder(node_def.get(), "Zeta", "Zeta")
      .Input({"x", data_type_in, dims_in, input0})
      .Input({"q", data_type_in, dims_in, input1})
      .Output({"output", data_type_out, dims_out, output});
  RunKernelZeta(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}
...
void CreateAndRunKernelZeta(
    ...
    aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
    bool bad_kernel = false) {
  CreateAndRunKernelZeta(dims, dims, input0, input1, output, status,
                         bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelZetaParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tin *input1,
    Tout *output) {
  CreateAndRunKernelZeta(dims_in, dims_out, input0, input1, output,
                         aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelZetaParamInvalid(const std::vector<std::int64_t> &dims,
                                        Tin *input0, Tin *input1,
                                        Tout *output) {
  CreateAndRunKernelZetaParamInvalid(dims, dims, input0, input1, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelZetaInnerError(const std::vector<std::int64_t> &dims_in,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelZetaInnerError(const std::vector<std::int64_t> &dims,
                                      Tin *input0, Tin *input1, Tout *output) {
  CreateAndRunKernelZetaInnerError(dims, dims, input0, input1, output);
}
...
void RunTestZeta() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "zeta/data/zeta_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "zeta/data/zeta_data_" +
...
TEST_F(TEST_ZETA_UT, DATA_TYPE_DT_FLOAT) { RunTestZeta<std::float_t>(); }

TEST_F(TEST_ZETA_UT, DATA_TYPE_DT_DOUBLE) { RunTestZeta<std::double_t>(); }
// exception instance
TEST_F(TEST_ZETA_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelZetaInnerError({2000, 6000}, float_0_, float_0_, float_0_);
}

TEST_F(TEST_ZETA_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelZetaParamInvalid({2, 6}, {2, 8}, float_12_, float_12_,
                                     float_16_);
}

TEST_F(TEST_ZETA_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelZetaParamInvalid({2, 11}, int32_22_, int32_22_, int64_22_);
}

TEST_F(TEST_ZETA_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelZetaParamInvalid({2, 11}, float_null_, float_null_,
                                     float_null_);
}

TEST_F(TEST_ZETA_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelZetaParamInvalid({0, 0}, float_0_, float_0_, float_null_);
}

TEST_F(TEST_ZETA_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "Zeta", "Zeta")
      .Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelZeta(node_def, aicpu::DeviceType::HOST,
                aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_ZETA_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelZetaParamInvalid({2, 11}, bool_22_, bool_22_, bool_22_);
}
