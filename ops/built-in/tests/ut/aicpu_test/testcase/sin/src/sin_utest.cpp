...
class TEST_SIN_UT : public testing::Test {
protected:
...
inline void RunKernelSin(std::shared_ptr<aicpu::NodeDef> node_def, aicpu::DeviceType device_type, uint32_t expect,
...
void CreateAndRunKernelSin(const std::vector<std::int64_t>& dims_in, const std::vector<std::int64_t>& dims_out,
                           Tin* input, Tout* output, aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
                           bool bad_kernel = false) {
  ...
  aicpu::NodeDefBuilder(node_def.get(), "Sin", "Sin")
      .Input({"x", data_type_in, dims_in, input})
      .Output({"y", data_type_out, dims_out, output});
  RunKernelSin(node_def, aicpu::DeviceType::HOST, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelSin(const std::vector<std::int64_t>& dims, Tin* input, Tout* output,
                           aicpu::KernelStatus status = aicpu::KernelStatus::KERNEL_STATUS_OK,
                           bool bad_kernel = false) {
  CreateAndRunKernelSin(dims, dims, input, output, status, bad_kernel);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelSinParamInvalid(const std::vector<std::int64_t>& dims_in,
                                       const std::vector<std::int64_t>& dims_out, Tin* input, Tout* output) {
  CreateAndRunKernelSin(dims_in, dims_out, input, output, aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelSinParamInvalid(const std::vector<std::int64_t>& dims, Tin* input, Tout* output) {
  CreateAndRunKernelSinParamInvalid(dims, dims, input, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelSinInnerError(const std::vector<std::int64_t>& dims_in,
                                     const std::vector<std::int64_t>& dims_out, Tin* input, Tout* output) {
  CreateAndRunKernelSin(dims_in, dims_out, input, output, aicpu::KernelStatus::KERNEL_STATUS_INNER_ERROR, true);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelSinInnerError(const std::vector<std::int64_t>& dims, Tin* input, Tout* output) {
  CreateAndRunKernelSinInnerError(dims, dims, input, output);
}
...
void RunTestSin() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "sin/data/sin_data_" + data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "sin/data/sin_data_" + data_name + "_shape.txt"};
  ...
  T data1[input1_size];
  std::string data_path{ktestcaseFilePath + "sin/data/sin_data_input_" + data_name + ".bin"};
  EXPECT_EQ(ReadBinFile(data_path, data1, input1_size), true);

  T output[input1_size];
  CreateAndRunKernelSin(dims, data1, output);
  std::string out_data_actual_path{ktestcaseFilePath + "sin/data/sin_data_output_" + data_name + "_actual.txt"};
  EXPECT_EQ(WriteFile(out_data_actual_path, output, input1_size), true);

  T expect_out[input1_size];
  std::string out_data_path{ktestcaseFilePath + "sin/data/sin_data_output_" + data_name + ".bin"};
...
#define ADD_CASE(base_type, aicpu_type)         \
  TEST_F(TEST_SIN_UT, DATA_TYPE_##aicpu_type) { \
    RunTestSin<base_type>();                    \
  }

// exception instance
TEST_F(TEST_SIN_UT, BAD_KERNEL_EXCEPTION) {
  CreateAndRunKernelSinInnerError({2, 6}, float_12_, float_12_);
}

TEST_F(TEST_SIN_UT, INPUT_SHAPE_EXCEPTION) {
  CreateAndRunKernelSinParamInvalid({2, 6}, {2, 8}, float_12_, float_16_);
}

TEST_F(TEST_SIN_UT, INPUT_DIM_EXCEPTION) {
  CreateAndRunKernelSinParamInvalid({2, 6}, {6, 2}, float_12_, float_12_);
}

TEST_F(TEST_SIN_UT, INPUT_DIMSIZE_EXCEPTION) {
  CreateAndRunKernelSinParamInvalid({2, 2, 3}, {6, 2}, float_12_, float_12_);
}

TEST_F(TEST_SIN_UT, INPUT_DTYPE_EXCEPTION) {
  CreateAndRunKernelSinParamInvalid({2, 11}, int32_22_, int64_22_);
}

TEST_F(TEST_SIN_UT, INPUT_NULL_EXCEPTION) {
  CreateAndRunKernelSinParamInvalid({2, 11}, float_null_, float_null_);
}

TEST_F(TEST_SIN_UT, OUTPUT_NULL_EXCEPTION) {
  CreateAndRunKernelSinParamInvalid({0, 0}, float_0_, float_null_);
}

TEST_F(TEST_SIN_UT, NO_OUTPUT_EXCEPTION) {
  const auto data_type_in{ToDataType<std::float_t>()};
  auto node_def{aicpu::CpuKernelUtils::CreateNodeDef()};
  aicpu::NodeDefBuilder(node_def.get(), "Sin", "Sin").Input({"x", data_type_in, {2, 6}, float_12_});
  RunKernelSin(node_def, aicpu::DeviceType::HOST, aicpu::KernelStatus::KERNEL_STATUS_PARAM_INVALID);
}

TEST_F(TEST_SIN_UT, INPUT_BOOL_UNSUPPORT) {
  CreateAndRunKernelSinParamInvalid({2, 11}, bool_22_, bool_22_);
}
