...
class reduceL2_onnx_plugin_test : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "reduceL2_onnx_plugin_test SetUp" << std::endl;
  }


  static void TearDownTestCase() {
    std::cout << "reduceL2_onnx_plugin_test TearDown" << std::endl;
  }
};
...
TEST_F(reduceL2_onnx_plugin_test, ReduceL2_default) {
  ...
  std::string modelFile = caseDir + "/ReduceL2_default.onnx";
...
TEST_F(reduceL2_onnx_plugin_test, ReduceL2_keepdims) {
  ...
  std::size_t idx = caseDir.find_last_of("/");
  caseDir = caseDir.substr(0, idx);
  std::string modelFile = caseDir + "/ReduceL2_keepdims.onnx";
  std::map<ge::AscendString, ge::AscendString> parser_params;
...
TEST_F(reduceL2_onnx_plugin_test, ReduceL2_negative) {
  ...
  std::size_t idx = caseDir.find_last_of("/");
  caseDir = caseDir.substr(0, idx);
  std::string modelFile = caseDir + "/ReduceL2_negative.onnx";
  std::map<ge::AscendString, ge::AscendString> parser_params;
...
TEST_F(reduceL2_onnx_plugin_test, ReduceL2_not_keepdims) {
  ...
  std::size_t idx = caseDir.find_last_of("/");
  caseDir = caseDir.substr(0, idx);
  std::string modelFile = caseDir + "/ReduceL2_not_keepdims.onnx";
  std::map<ge::AscendString, ge::AscendString> parser_params;
