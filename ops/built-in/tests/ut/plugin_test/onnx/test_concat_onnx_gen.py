...
def make_concat_two_input():
    ...
    graph = helper.make_graph(
        ...
        name="test_concat",
        inputs=[helper.make_tensor_value_info("A", onnx.TensorProto.FLOAT, [2, 3, 4]),
                helper.make_tensor_value_info("A1", onnx.TensorProto.FLOAT, [2, 3, 4])],
        outputs=[helper.make_tensor_value_info("C", onnx.TensorProto.FLOAT, [2, 3, 4])]
    ...
    onnx.save(model, "./test_concat_case_V11_1.onnx")
    onnx.checker.check_model(model)


if __name__ == '__main__':
    concat_list = (9, 11, 12, 13)
    make_concat_two_input()
