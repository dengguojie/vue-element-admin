...
def make_flatten_v12():
    x = helper.make_tensor_value_info('x', TensorProto.FLOAT, [2, 3, 4, 5])
    y = helper.make_tensor_value_info(
        'y', TensorProto.FLOAT, [2 * 3 * 4 * 5, 1])
    node = helper.make_node(
        'Flatten',
        inputs=['x'],
        outputs=['y'],
    ...
    onnx.save(model, "./test_flatten_v12.onnx")
    onnx.checker.check_model(model)


def make_flatten_v13():
    x = helper.make_tensor_value_info('x', TensorProto.FLOAT, [2, 3, 4, 5])
    y = helper.make_tensor_value_info(
        'y', TensorProto.FLOAT, [2 * 3 * 4 * 5, 1])
    node = helper.make_node(
    ...
    onnx.save(model, "./test_flatten_v13.onnx")
    onnx.checker.check_model(model)
...
def make_flatten_v9():
    ...
    onnx.save(model, "./test_flatten_v9.onnx")
    onnx.checker.check_model(model)
