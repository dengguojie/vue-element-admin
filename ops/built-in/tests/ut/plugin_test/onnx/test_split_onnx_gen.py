...
def make_split_v13_no_default():
    ...
    graph = helper.make_graph(
        ...
        initializer=[helper.make_tensor('split', onnx.TensorProto.INT64, (3, ), const_tensor )]
    )
...
def make_split_v13_default_split():
    node = helper.make_node(
        'Split',
        inputs=['input'],
        outputs=['output_0', 'output_1', 'output_2'],
    ...
    model = helper.make_model(graph, producer_name='onnx')
    model.opset_import[0].version = 13
    onnx.save(model, './test_split_v13_default_split.onnx')
