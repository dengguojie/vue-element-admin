...
TEST_F(reshape_onnx_plugin_test, reshape_onnx_plugin_test_case_2)
{
  CleanGlobal();
  ge::Graph graph;
  ...
  std::size_t idx = caseDir.find_last_of("/");
  caseDir = caseDir.substr(0, idx);
  std::string modelFile = caseDir + "/test_reshape_case_v12.onnx";
  std::map<ge::AscendString, ge::AscendString> parser_params;
...
TEST_F(reshape_onnx_plugin_test, reshape_onnx_plugin_test_case_3)
{
  CleanGlobal();
  ge::Graph graph;
  ...
  std::size_t idx = caseDir.find_last_of("/");
  caseDir = caseDir.substr(0, idx);
  std::string modelFile = caseDir + "/test_reshape_case_v13.onnx";
  std::map<ge::AscendString, ge::AscendString> parser_params;
...
TEST_F(reshape_onnx_plugin_test, reshape_onnx_plugin_test_case_4)
{
  CleanGlobal();
  ge::Graph graph;
  ...
  std::size_t idx = caseDir.find_last_of("/");
  caseDir = caseDir.substr(0, idx);
  std::string modelFile = caseDir + "/test_reshape_case_v9.onnx";
  std::map<ge::AscendString, ge::AscendString> parser_params;
