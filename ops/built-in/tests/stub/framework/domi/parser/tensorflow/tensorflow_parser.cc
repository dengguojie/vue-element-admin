...
const std::vector<std::string> kMakeOperatorNotByIr = {ge::ARG, ge::VARIABLE, ge::VARHANDLEOP, ge::FRAMEWORKOP,
                                                       ge::DATA};
const std::map<string, domiTensorFormat_t> kNeedMarkFormatNodes = {{"ExtractImagePatches", DOMI_TENSOR_NHWC},
                                                                   {"ExtractVolumePatches", DOMI_TENSOR_NHWC},
                                                                   {"LogSoftmax", DOMI_TENSOR_NHWC},
                                                                   {"ResizeBilinear", DOMI_TENSOR_NHWC},
                                                                   {"ResizeBilinearGrad", DOMI_TENSOR_NHWC},
                                                                   {"ResizeNearestNeighbor", DOMI_TENSOR_NHWC},
                                                                   {"Softmax", DOMI_TENSOR_NHWC},
                                                                   {"SoftmaxCrossEntropyWithLogits", DOMI_TENSOR_NHWC},
                                                                   {"SoftmaxGrad", DOMI_TENSOR_NHWC},
                                                                   {"SpaceToBatch", DOMI_TENSOR_NHWC}};
...
Status PostOpProcessForSubgraph(const ParseArg &arg) {
  ...
  auto post_func = OpRegistry::Instance()->GetParseSubgraphPostFunc(arg.parent_node->GetType());
...
Status TensorFlowModelParser::AddNode(const domi::tensorflow::NodeDef *node_def, ge::ComputeGraphPtr &graph,
  ...
  if (type_it == tensorflow_op_map.end()) {
    ...
    GE_CHK_STATUS(AutoMappingFn(node_def, op));
    op.BreakConnect();
  ...
  if (!needFusion) {
    ...
    shared_ptr<TensorFlowOpParser> tensorflow_op_parser = std::static_pointer_cast<TensorFlowOpParser>(op_parser);
    GE_CHECK_NOTNULL(tensorflow_op_parser);
    status = tensorflow_op_parser->ParseParams(node_def, op);
    GE_CHK_STATUS_EXEC(status, DeleteFuisonNodeDef();
                       return status, "Parse Params for node %s failed", node_name.c_str());
  }
  ...
  if (needFusion) {
    ...
    shared_ptr<TensorFlowFusionOpParser> tensorflow_fusion_op_parser =
        std::static_pointer_cast<TensorFlowFusionOpParser>(fusion_op_parser);
    GE_CHECK_NOTNULL(tensorflow_fusion_op_parser);
    ...
    if (iter == fusion_op_nodedef_map_.end()) {
      DOMI_LOGE("FusionOp node %s has no children node!", node_name.c_str());
      DeleteFuisonNodeDef();
    ...
    GE_CHK_STATUS_EXEC(status, DeleteFuisonNodeDef();
                       return status, "Parse Params for fusionop node %s failed", node_name.c_str());
  ...
  // Resolve whether the input of opnode is a variable or a constant, and record it in opdef.is_input_const.
  if (node_def->op() != TENSORFLOWF_NODE_OP_CONST) {
    GE_IF_BOOL_EXEC(GetOpNodeInputMode(node_def, op) != SUCCESS, DeleteFuisonNodeDef(); return INTERNAL_ERROR);
  }
  return SUCCESS;
}

Status TensorFlowModelParser::GetOpNodeInputMode(const domi::tensorflow::NodeDef *node_def, ge::OpDescPtr &op) {
  GE_CHECK_NOTNULL(node_def);
  GE_CHECK_NOTNULL(op);
  vector<bool> v_is_input_const;
  for (const string &input_name : node_def->input()) {
    string tmp_node_name;
    GE_RETURN_IF_ERROR(CheckInputNodeName(input_name, &tmp_node_name, nullptr, nullptr));

    auto iter = nodedef_map_.find(tmp_node_name);
    if (iter != nodedef_map_.end()) {
      if (domi::GetContext().train_flag) {
        v_is_input_const.push_back(false);
      } else {
        const NodeDef *input_node = iter->second;
        GE_CHECK_NOTNULL(input_node);
        if (input_node->op() == TENSORFLOWF_NODE_OP_CONST) {
          // Input is constant
          v_is_input_const.push_back(true);
        } else if (input_node->op() == TENSORFLOWF_NODE_OP_IDENTITY) {
          // The input is identity operator. Need to see whether the input of identity operator is constant
          string tmp_node_input_name;
          GE_IF_BOOL_EXEC(input_node->input_size() == 0,
                          DOMI_LOGE("the input size of node %s is 0", input_node->name().c_str()));
          GE_RETURN_IF_ERROR(CheckInputNodeName(input_node->input(0), &tmp_node_input_name, nullptr, nullptr));
          const domi::tensorflow::NodeDef *identity_input_node = nodedef_map_[tmp_node_input_name];
          GE_CHECK_NOTNULL(identity_input_node);
          v_is_input_const.push_back(identity_input_node->op() == TENSORFLOWF_NODE_OP_CONST);
        } else {
          // Input is others
          v_is_input_const.push_back(false);
        }
  ...
  op->SetIsInputConst(v_is_input_const);
...
Status TensorFlowModelParser::CheckoutInputNum(ge::OpDescPtr &op_desc, const domi::tensorflow::NodeDef *node) {
  ...
  if (input_tensor_num != factory_input_size) {
    DOMI_LOGE("op [%s], type[%s], The input number of tensorflow[%zu] should be equal to factory size[%zu]",
...
Status TensorFlowModelParser::CheckOpType(const domi::tensorflow::NodeDef *node_def, string &op_type) {
  ...
  static char *parserPriority = std::getenv("PARSER_PRIORITY");
  std::map<std::string, set<int>> check_dims = {
      {ge::MATMUL, {{3136}, {512}, {1024}}}, {ge::SPARSESOFTMAXCROSSENTROPYWITHLOGITS, {10}},
  };
  ...
  GE_IF_BOOL_EXEC(
      (op_type == ge::MATMUL) && (parserPriority != nullptr) && (string(parserPriority) == "cce"),
      GE_CHK_STATUS_RET(CheckOpShapeDim(node_def, check_dims[op_type], valid), "failed to check op shape");
      GE_IF_BOOL_EXEC(!valid, op_type = ge::FRAMEWORKOP; GELOGI("Set op %s to frameworkop", node_name.c_str());
                      framework_ops_[node_name] = node_def;););
...
Status TensorFlowModelParser::ParseNodeDef(TensorFlowModelParser *parser, ge::ComputeGraphPtr &graph,
  ...
  domi::ImplyType implyType = OpRegistry::Instance()->GetImplyType(op_type);
  ...
  if (op_factory.GetName() != node_name) {
    ...
    op = ge::OpDescUtils::GetOpDescFromOperator(op_factory);
    GE_CHECK_NOTNULL(op);
    GELOGD("After GetOpDescFromOperator op[%s] type[%s] have input size: %zu, outpu tsize: %zu", op->GetName().c_str(),
           op->GetType().c_str(), op->GetInputsSize(), op->GetOutputsSize());
  ...
  if (!needFusion) {
    shared_ptr<OpParser> op_parser = factory->CreateOpParser(op_type);
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(op_parser == nullptr, return FAILED, "get op parser failed, node name = %s",
                                   node_name.c_str());
    shared_ptr<TensorFlowOpParser> tensorflow_op_parser = std::dynamic_pointer_cast<TensorFlowOpParser>(op_parser);
    GE_IF_BOOL_EXEC(tensorflow_op_parser == nullptr, DOMI_LOGE("node :%s type :%s, can not get parser , please check!",
  ...
  if (node_def->op() != TENSORFLOWF_NODE_OP_CONST) {
    GE_IF_BOOL_EXEC(parser->GetOpNodeInputMode(node_def, op) != SUCCESS, return INTERNAL_ERROR);
  }
...
Status TensorFlowModelParser::AddFmkNode(ge::ComputeGraphPtr &graph, shared_ptr<ge::ScopeGraph> &scope_graph,
                                         const vector<string> &op_node_name_list, bool is_dataset_init) {
  GE_CHECK_NOTNULL(graph);
  GE_CHECK_NOTNULL(scope_graph);
  ...
  for (size_t i = 0; i < op_node_list_size; ++i) {
    ...
    GE_RETURN_IF_ERROR(AddFusionNodeDef(op_node_name));
...
Status TensorFlowModelParser::ParseFromMemory(const char *data, uint32_t size, ge::ComputeGraphPtr &graph) {
  ...
  if (domi::GetContext().input_dims.empty()) {
    graph_def = OriDef;
  } else {
    GELOGI("Before Trim, the Graph Node size is:%d", OriDef.node_size());
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(GenerateGraphByInputShape(OriDef, &graph_def),
                                   return INTERNAL_ERROR, "Trim Graph fail.");
  ...
  for (size_t i = 0; i < op_node_name_list.size(); i++) {
    ...
    GE_RETURN_IF_ERROR(AddNode(node_def, graph, scope_graph));
  }
...
Status TensorFlowModelParser::GetFunctionProto(const string &file,
  ...
  if (!read) {
    GELOGE(INTERNAL_ERROR,
           ...
           "Try to use scritp in atc package to save the subgraph to graph_def_library.pbtxt");
    return FAILED;
  }
...
Status TensorFlowModelParser::Parse(const char *model_path, ge::ComputeGraphPtr &root_graph) {
  ...
  if (domi::GetContext().input_dims.empty() && domi::GetContext().out_nodes_map.empty()) {
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(GenerateGraphByInputShape(ori_def, &graph_def),
                                   return INTERNAL_ERROR, "Trim Graph fail.");
  ...
  while (!tasks.empty()) {
    ...
    auto model_parser = ModelParserFactory::Instance()->CreateModelParser(domi::FrameworkType::TENSORFLOW);
    auto ret = model_parser->ParseAllGraph(arg.proto, arg.graph);
    if (ret != SUCCESS) {
      ErrorManager::GetInstance().ATCReportErrMessage("E12016", {"functionname", "graphname"},
                                                      {arg.function_name, arg.graph->GetName()});
...
Status TensorFlowModelParser::ParseAllGraph(const google::protobuf::Message *proto, ge::ComputeGraphPtr &graph) {
  ...
  for (size_t i = 0; i < op_node_name_list.size(); i++) {
    ...
    if (ret != SUCCESS) {
      ErrorManager::GetInstance().ATCReportErrMessage("E12007", {"opname"}, {node_def->name()});
...
Status TensorFlowModelParser::CheckInputNodeName(const string &input_node_name, string *node_name, int32_t *index,
  ...
  *node_name = tmp_input_node_name.substr(0, find);
  ...
  try {
    int32_t tmp_index = static_cast<int32_t>(std::stoi(indexstr.c_str(), nullptr, 10));
    *index = tmp_index;
  } catch (std::invalid_argument &) {
    ErrorManager::GetInstance().ATCReportErrMessage("E12022", {"input", "index"}, {input_node_name, indexstr});
    GELOGE(INTERNAL_ERROR, "stl[stoi] input_node_name[%s] indexstr[%s] is invalid argument!", input_node_name.c_str(),
           indexstr.c_str());
    return INTERNAL_ERROR;
  } catch (std::out_of_range &) {
    ErrorManager::GetInstance().ATCReportErrMessage("E12023", {"input", "index"}, {input_node_name, indexstr});
    GELOGE(INTERNAL_ERROR, "stl[stoi] input_node_name[%s] indexstr[%s] is out of range!", input_node_name.c_str(),
           indexstr.c_str());
    return INTERNAL_ERROR;
  } catch (...) {
    ErrorManager::GetInstance().ATCReportErrMessage("E12024", {"input", "index"}, {input_node_name, indexstr});
...
Status TensorFlowModelParser::RunScopeFusionPass(const vector<string> &scope_passes_list,
  ...
  if (ret != SUCCESS && ret != SCOPE_NOT_CHANGED) {
    ErrorManager::GetInstance().ATCReportErrMessage("E12003", {"ret"}, {std::to_string(ret)});
...
Status TensorFlowModelParser::UpdateNormalOpContext(shared_ptr<ge::ScopeGraph> &scope_graph,
                                                    const string &op_node_name, OpNodeContext &normal_op_node_context) {
  GE_CHECK_NOTNULL(scope_graph);
...
Status TensorFlowModelParser::ParseProtoWithSubgraph(const google::protobuf::Message *root_proto,
                                                     GetGraphCallback callback, ge::ComputeGraphPtr &root_graph) {
...
Status TensorFlowModelParser::OptimizeSnapShot(domi::tensorflow::NodeDef *curr_mode_def,
  ...
  if (curr_mode_def == nullptr) {
    DOMI_LOGE("input param is nullptr.");
    return PARAM_INVALID;
  }
  ...
  for (auto &output_iter : output_map) {
    ...
    for (auto &input : *inputs) {
      ...
      if (CheckInputNodeName(input, &node_name, nullptr, &is_control) != SUCCESS) {
        DOMI_LOGE("parse node input info failed, node %s, input %s.", output_node_def->name().c_str(), input.c_str());
        return FAILED;
      }
  ...
  for (auto &curr_node_def : nodedef_to_optimize) {
    ...
    for (auto &input : curr_node_def->input()) {
      ...
      if (CheckInputNodeName(input, &node_name, &input_index, &is_control) != SUCCESS) {
        DOMI_LOGE("parse SnapShot input info failed, node %s, input %s.", curr_node_def->name().c_str(), input.c_str());
        return FAILED;
      }
      if (is_control) {
    ...
    if (data_input_cnt != 1) {
      DOMI_LOGE("%s op data input size %u invalid", curr_node_def->name().c_str(), data_input_cnt);
      return FAILED;
    }
  ...
  for (int w = 0; w < graph_def->node_size(); w++) {
    ...
    for (int k = 0; k < nodeDst->input_size(); k++) {
      ...
      if (CheckInputNodeName(nodeDstInputName, &nodeDstInputNameTmp, nullptr, &isControl) != SUCCESS) {
        DOMI_LOGE("CheckInputNodeName failed, node is: %s", nodeDstInputName.c_str());
        return;
      }
      if (nodeDstInputNameTmp == nodeCurrent->name()) {
        ...
        if (isControl) {
          ...
          if (CheckInputNodeName(nodeCurrentName, &nodeCurrentNameTmp, nullptr, nullptr) != SUCCESS) {
            DOMI_LOGE("CheckInputNodeName failed, node is: %s", nodeCurrentName.c_str());
            return;
          }
...
Status TensorFlowModelParser::GraphDefOptimizeTranspose(GraphDef *graph_def) {
  // The caller guarantees that the pointer is not null
  char *useMul4DEnv = std::getenv("USE_MUL_4D");
  int bUseMul4DEnv = (useMul4DEnv != nullptr) ? std::strtol(useMul4DEnv, nullptr, 10) : 0;
  GE_IF_BOOL_EXEC(bUseMul4DEnv == 0, return SUCCESS)
  ...
  std::map<std::string, DelTransposeInfo> transposeInfo;
  std::map<std::string, std::string> softmaxInfo;

  GE_CHK_STATUS_RET(GetTransposeInfo(graph_def, softmaxInfo, transposeInfo), "GetTransposeInfo failed.");
  GE_IF_BOOL_EXEC(SUCCESS != EraseTransposeNode(softmaxInfo, transposeInfo), return SUCCESS);
  // Optimize Transpose Node
  OptimizeTranspose(transposeInfo);

  SoftmaxAddAttr(graph_def);
...
Status TensorFlowModelParser::GraphDefOptimize(domi::tensorflow::GraphDef *graph_def) {
  ...
  GE_CHK_STATUS_RET(GraphDefOptimizeTranspose(graph_def));
...
domiTensorFormat_t TensorFlowModelParser::InferInputFormats() {
  ...
  return global_input_format == DOMI_TENSOR_RESERVED ? DOMI_TENSOR_NHWC : global_input_format;
}
...
Status TensorFlowModelParser::GetNodeFormat(const NodeDef *node, TfTranspose pred_transpose, domiTensorFormat_t &format,
  ...
  if (ge::TensorFlowUtil::FindAttrValue(node, TENSORFLOW_ATTR_DATA_FORMAT, attr) && node->op() != ge::BIASADD) {
    ...
    format = (attr.s() == TENSORFLOWF_TENSOR_NCHW) ? DOMI_TENSOR_NCHW : DOMI_TENSOR_NHWC;

    GE_IF_BOOL_EXEC(format == DOMI_TENSOR_NCHW && pred_transpose == TO_NCHW, format = DOMI_TENSOR_NHWC);
    GE_IF_BOOL_EXEC(format == DOMI_TENSOR_NHWC && pred_transpose == TO_NHWC, format = DOMI_TENSOR_NCHW);
    GE_IF_BOOL_EXEC((format == DOMI_TENSOR_NCHW && pred_transpose == TO_NHWC) ||
                        (format == DOMI_TENSOR_NHWC && pred_transpose == TO_NCHW),
...
Status TensorFlowModelParser::GenerateGraphByInputShape(const domi::tensorflow::GraphDef &input_graph_def,
...
Status TensorFlowModelParser::AddFusionNodeDef(const string &op_node_name) {
  auto iter = fusion_op_nodedef_map_.find(op_node_name);
  if (iter != fusion_op_nodedef_map_.end()) {
    NodeDef *node_def = new (std::nothrow) NodeDef();
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(node_def == nullptr, return FAILED, "new an object failed.");
    vector<string> funsion_op_info = fusion_op_type_map_[op_node_name];
    node_def->set_name(op_node_name);
    node_def->set_op(funsion_op_info[0]);
...
Status TensorFlowModelParser::FusionNodeParseParams(shared_ptr<OpParser> &op_parser,
  ...
  GE_IF_BOOL_EXEC(tensorflow_fusion_op_parser == nullptr,
                  DOMI_LOGE("node :%s can not get fusion parser, please check!", node_def->name().c_str());
                  return INTERNAL_ERROR);
  ...
  (void)ge::AttrUtils::SetStr(node->GetOpDesc(), ge::ATTR_NAME_FUSIONOP_ORIGINAL_TYPE, node_def->op());
  vector<const domi::tensorflow::NodeDef *> node_def_v = iter->second;
  Status status = tensorflow_fusion_op_parser->ParseParams(node_def_v, node);
...
Status TensorFlowModelParser::OptimizeConstNodes4CustomOp(domi::tensorflow::GraphDef *graph_def) {
  ...
  for (auto &it_node_map : all_nodedef_map) {
    ...
    const std::vector<RemoveInputConfigure> &move_input_vec =
        OpRegistry::Instance()->GetRemoveInputConfigure(current_op_name);
    ...
    GE_CHK_BOOL_EXEC_INFO(OpRegistry::Instance()->GetImplyTypeByOriOpType(current_op_name) == domi::ImplyType::TVM,
    ...
    for (const auto &it : move_input_vec) {
      ...
      if (it.moveType == OMG_REMOVE_TYPE_WITH_COND) {
        domi::tensorflow::AttrValue attr_value;
      ...
      } else if (it.moveType == OMG_REMOVE_INPUT_WITH_ORIGINAL_TYPE && it.originalType == current_op_name) {
        GELOGD("Input %s:%d will be removed.", current_op_name.c_str(), move_index);
        unused_inputs.insert(move_index);
      } else if (it.moveType == OMG_INPUT_REORDER) {
...
Status TensorFlowModelParser::RemoveIsolateNode(domi::tensorflow::GraphDef *graph_def) {
  ...
  for (int i = 0; i < graph_def->node_size(); i++) {
    ...
    if (node_inputs_outputs_map_.find(node_name) == node_inputs_outputs_map_.end()) {
      DOMI_LOGE("Can not find input output context, node:%s.", node_name.c_str());
      return FAILED;
    }
...
REGISTER_MODEL_PARSER_CREATOR(TENSORFLOW, TensorFlowModelParser);
REGISTER_WEIGHTS_PARSER_CREATOR(TENSORFLOW, TensorFlowWeightsParser);
}  // namespace domi
