...
bool GetCceTbeTransInfo(string opType, OpSupportTranInfo &opSupportInfo) {
  ...
  GE_IF_BOOL_EXEC(
    ...
    if (OpRegistry().Instance()->GetImplyType(ge::DEPTHWISEWEIGHT4D26D) == ImplyType::TVM) {
  ...
  GE_IF_BOOL_EXEC(OpRegistry().Instance()->GetImplyType(opType) == ImplyType::BUILDIN, cceTbeOpType = "CCE";)
...
void SetIntAttr(const string &originalType, OpDescPtr &opDesc,
  ...
  if (originalType == "Pack" && (attr.first == "axis" || attr.first == "N")) {
    ...
    (*tfAttr)[attr.first].set_type((tensorflow::DataType)i);
  }
}
...
void SetListIntAttr(const string &originalType, OpDescPtr &opDesc,
  ...
  if (iter != is_dataset_op_vec.end()) {
    if (attr.first == "Toutput_types" || attr.first == "output_types") {
      for (auto i : intList) {
        list->add_type((tensorflow::DataType)i);
      }
      (*tfAttr)[attr.first] = attrValue;
...
void SetListListIntAttr(const string &originalType, OpDescPtr &opDesc,
  ...
  if ((originalType == "IteratorV2" || originalType == "BatchDatasetV2" || originalType == "IteratorGetNext" ||
    ...
    tensorflow::TensorShapeProto *tensorShape = list->add_shape();
    tensorflow::TensorShapeProto_Dim *dim = tensorShape->add_dim();
    dim->set_size(0);
    (*tfAttr)[attr.first] = attrValue;
  }
}

void SetTensorValue(const ge::ConstGeTensorPtr &geTensor, tensorflow::TensorProto *tfTensor, int32_t dataCount) {
  if (dataCount > 1) {
...
Status SetTensorAttr(ge::OpDescPtr &opDesc, google::protobuf::Map<string, domi::tensorflow::AttrValue> *tfAttr,
  ...
  tensorflow::TensorProto *tf_tensor = (*tfAttr)[attr.first].mutable_tensor();
  ...
  tensorflow::TensorShapeProto *tf_shape = tf_tensor->mutable_tensor_shape();
  for (auto dim : ge_tensor->GetTensorDesc().GetShape().GetDims()) {
    tensorflow::TensorShapeProto_Dim *tf_dims = tf_shape->add_dim();
...
Status LessIOList(ge::GeAttrValue::LIST_INT &input_list, ge::GeAttrValue::LIST_INT &output_list,
  ...
  input_list.push_back(dtype);
  output_list.push_back(tensorflow::DataType::DT_BOOL);
...
Status SelectIOList(ge::GeAttrValue::LIST_INT &input_list, ge::GeAttrValue::LIST_INT &output_list,
  ...
  input_list.push_back(tensorflow::DataType::DT_BOOL);
  input_list.push_back(t);
  input_list.push_back(t);
...
Status DropOutGenMaskIOList(ge::GeAttrValue::LIST_INT &input_list, ge::GeAttrValue::LIST_INT &output_list,
                            ge::OpDescPtr &opDesc) {
  input_list.push_back(tensorflow::DT_INT64);
  input_list.push_back(tensorflow::DT_FLOAT);
  output_list.push_back(tensorflow::DT_UINT8);
...
Status TopKV2IOList(ge::GeAttrValue::LIST_INT &inputList, ge::GeAttrValue::LIST_INT &outputList,
  ...
  inputList.push_back(tensorflow::DataType::DT_INT32);

  // output_list - eg:{1, 3}
  outputList.push_back(t);
  outputList.push_back(tensorflow::DataType::DT_INT32);
...
Status CreateNodeDefBytes(ge::NodePtr n, string originalType, map<string, PIOListHandle> &mOpIOListFuncMap) {
  ...
  for (auto ge_desc : opDesc->GetAllOutputsDescPtr()) {
    ...
    for (uint32_t j = 0; j < ge_desc->GetShape().GetDimNum(); ++j) {
      ...
      GE_CHECK_LE(tmp_dim, UINT32_MAX);
      FMK_UINT32_MULCHECK(real_size, tmp_dim);
      real_size *= tmp_dim;
    }
...
Status CreateOpDefBytes(ge::NodePtr n, string original_type) {
  ...
  if (original_type == "Const") {
    ...
    tensorflow::OpDef_AttrDef *attr3 = proto.add_attr();
    ...
    // Set domi::AttrDef
    tensorflow::OpDef_AttrDef *attr0 = proto.add_attr();
    attr0->set_name("f");
    attr0->set_type("func");
    ...
    tensorflow::OpDef_AttrDef *attr4 = proto.add_attr();
    ...
    // Set input_arg & output_arg
    tensorflow::OpDef::ArgDef *input_argdef_1 = proto.add_input_arg();
    input_argdef_1->set_name("iterator");
    input_argdef_1->set_type(::domi::tensorflow::DataType::DT_RESOURCE);

    tensorflow::OpDef::ArgDef *output_argdef = proto.add_output_arg();
    ...
    tensorflow::OpDef::ArgDef *inputArgdef4 = proto.add_input_arg();
    inputArgdef4->set_name("num_parallel_calls");
    inputArgdef4->set_type(::domi::tensorflow::DataType::DT_INT64);

    tensorflow::OpDef::ArgDef *inputArgdef5 = proto.add_input_arg();
...
Status CreateFuncDefBytes(ge::NodePtr n, string original_type, string func_bin_path) {
  ...
  if (funcDefBytes.GetSize() > 0 && funcDefBytes.GetData() != nullptr) {
    tensorflow::FunctionDefLibrary funcdeflib;
...
Status GraphOptimizer::MakeTfProtoDef() {
...
Status GraphOptimizer::FusionFmkop() {
...
Status GraphOptimizer::MarkForFusion(unordered_map<string, vector<NodePtr>> &node_cluser_Map) {
  GE_CHECK_NOTNULL(graph_);
...
// find frameworkOP
Status GraphOptimizer::FindFmkNodeCluser(unordered_map<string, vector<NodePtr>> &node_cluser_Map) {
  vector<NodePtr> temp_node_cluser;
...
Status GraphOptimizer::UpdateGraph(vector<NodePtr> &nodes) {
...
Status GraphOptimizer::InsertNode(ge::ComputeGraphPtr sub_graph, vector<ge::NodePtr> &nodes,
...
Status GraphOptimizer::LinkInnerAnchor(unordered_map<string, ge::NodePtr> &node_map) {
  for (auto node : graph_->GetDirectNode()) {
...
Status GraphOptimizer::RebuildOutputAnchors(vector<ge::OutDataAnchorPtr> &output_anchors,
...
// rebuild input desc
Status GraphOptimizer::RebuildInputAnchors(vector<ge::InDataAnchorPtr> &input_anchors, ge::OpDescPtr fusion_op_desc) {
...
Status GraphOptimizer::RebuildFusionNode(vector<ge::InDataAnchorPtr> &input_anchors,
                                         vector<ge::OutDataAnchorPtr> &output_anchors,
                                         map<ge::OutDataAnchorPtr, vector<ge::InDataAnchorPtr>> &output_in_map,
                                         vector<ge::InControlAnchorPtr> &input_control_anchors,
                                         vector<ge::OutControlAnchorPtr> &output_control_anchors,
...
Status GraphOptimizer::Insert4DTo5DTransOp(OutDataAnchorPtr src_anchor, InDataAnchorPtr dst_anchor,
...
Status GraphOptimizer::InsertFZ2HWCK(OutDataAnchorPtr src_anchor, InDataAnchorPtr dst_anchor,
...
Status GraphOptimizer::InsertVar5DTo4D(ge::OutDataAnchorPtr src_anchor, ge::InDataAnchorPtr dst_anchor,
                                       enum ge::Format srcOutFormat, enum ge::DataType srcOutDatatype,
                                       enum ge::Format dstInFormat, enum ge::DataType dstInDatatype) {
...
Status GraphOptimizer::InsertHWCK2FZ(OutDataAnchorPtr src_anchor, InDataAnchorPtr dst_anchor,
                                     enum ge::Format srcOutFormat, enum ge::DataType srcOutDatatype,
                                     enum ge::Format dstInFormat, enum ge::DataType dstInDatatype) {
...
Status GraphOptimizer::Insert5DTo4DTransOp(OutDataAnchorPtr src_anchor, InDataAnchorPtr dst_anchor,
                                           enum ge::Format src_out_format, enum ge::DataType src_out_data_type,
                                           enum ge::Format dst_in_format, enum ge::DataType dst_in_data_type) {
  // Status ret;
...
Status GraphOptimizer::NewNodeAddEdges(OutDataAnchorPtr src_anchor, InDataAnchorPtr dst_anchor, NodePtr first,
...
OpDescPtr GraphOptimizer::CreateTranslateOp(enum ge::Format inFormat, enum ge::DataType inDatatype,
  ...
  AttrUtils::SetInt(op_def, ge::ATTR_NAME_INPUT_FORMAT, inFormat);
  AttrUtils::SetInt(op_def, ATTR_NAME_INPUT_DATATYPE, inDatatype);
  AttrUtils::SetInt(op_def, ge::ATTR_NAME_OUTPUT_FORMAT, outFormat);
  AttrUtils::SetInt(op_def, ATTR_NAME_OUTPUT_DATATYPE, outDatatype);
...
OpDescPtr GraphOptimizer::CreatePermuteOp(enum ge::Format input_format, enum ge::Format output_format) {
  static uint32_t transop_count = 0;
  ...
  AttrUtils::SetInt(op_desc, ge::ATTR_NAME_INPUT_FORMAT, (int64_t)input_format);
  AttrUtils::SetInt(op_desc, ge::ATTR_NAME_OUTPUT_FORMAT, (int64_t)output_format);

  GE_IF_BOOL_EXEC(input_format == FORMAT_NCHW, AttrUtils::SetInt(op_desc, "NCHW_to_NHWC", (int64_t)1));
  GE_IF_BOOL_EXEC(input_format == FORMAT_NHWC, AttrUtils::SetInt(op_desc, "NHWC_to_NCHW", (int64_t)1));
...
OpDescPtr GraphOptimizer::CreateCastOp(enum ge::DataType input_data_type, enum ge::DataType output_data_type,
...
OpDescPtr GraphOptimizer::CreateTransDataOp(enum ge::Format input_format) {
  static uint32_t transop_count = 0;
  std::stringstream sstmp;
