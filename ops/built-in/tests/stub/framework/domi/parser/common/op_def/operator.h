...
class FMK_FUNC_HOST_VISIBILITY Operator {
 public:
  explicit Operator(const std::string &type);
  Operator() { op_schema_ = nullptr; }

  virtual ~Operator() { op_schema_ = nullptr; }
  ...
  Operator &AttrVector(std::string key, std::vector<int64_t> &value);

  Operator &Name(const std::string &name);

  Operator &Type(const std::string &type);

  Operator &InputTensorDesc(const ge::GeTensorDesc &input_tensordesc);

  Operator &OutputTensorDesc(const ge::GeTensorDesc &output_tensordesc);
...
#define ATTR_SETTER_WITH_VALUE(TypeName)                                       \
  ...
  Operator &Attr(const std::string &name, const ge::Tuple<TypeName> &value)
