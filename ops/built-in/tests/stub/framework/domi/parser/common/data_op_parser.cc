...
Status DataOpParser::Init5DInputTensor(const vector<int64_t> &shape, ge::GeTensorDesc &tensor_desc) {
  ...
  if (graph_status != ge::GRAPH_SUCCESS) {
    DOMI_LOGE("GetTensorSizeInBytes failed!");
    return domi::FAILED;
  }
...
Status DataOpParser::Init5DOutputTensor(const vector<int64_t> &shape, ge::GeTensorDesc &output) {
  ...
  if (graph_status != ge::GRAPH_SUCCESS) {
    DOMI_LOGE("GetTensorMemorySizeInBytes failed!");
    return domi::FAILED;
  }
  // Set the actual occupied space size
...
Status DataOpParser::InitOutputTensor(const vector<int64_t> &shape, ge::GeTensorDesc &output) {
  ...
  if (graph_status != ge::GRAPH_SUCCESS) {
    DOMI_LOGE("CalcTensorMemSize failed!");
    return FAILED;
  }
  ...
  if ((output_size > UINT32_MAX) || (output_size < 0)) {
    DOMI_LOGE("After calc tensor memory size, output_mem_size = %ld, out of data range [0, %u]", output_size,
              UINT32_MAX);
    return FAILED;
  }

  int64_t size = output_size;
  if (size > (INT64_MAX - kTwoTimesAlign * kDataMemAlignSize)) {
    DOMI_LOGE("The updated mem size ie bigger than UINT32_MAX");
    return FAILED;
  } else {
