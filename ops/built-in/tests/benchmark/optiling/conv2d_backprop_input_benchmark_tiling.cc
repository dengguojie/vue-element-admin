...
static void Conv2DBpInputTilingBenchmark(benchmark::State &state, const char *compile_info) {
  ...
  for (auto _ : state) {
    optiling::Conv2DBpInputTiling(op_name, conv2dbackpropinput, nlohmann::json::parse(compile_info), run_info);
  }
}

static void Conv2DBpInputTilingRepoSize100(benchmark::State &state) {
  ...
  Conv2DBpInputTilingBenchmark(state, compile_info);
}
BENCHMARK(Conv2DBpInputTilingRepoSize100);

static void Conv2DBpInputTilingCostSize100(benchmark::State &state) {
  const char *compile_info =
...
BENCHMARK(Conv2DBpInputTilingCostSize100);

static void Conv2DBpInputTilingBinary(benchmark::State &state) {
  const char *compile_info =
...
BENCHMARK(Conv2DBpInputTilingBinary);