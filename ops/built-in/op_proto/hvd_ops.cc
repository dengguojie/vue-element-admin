...
bool CheckSupportDateTpye(ge::DataType type){
    const std::vector<ge::DataType> SUPPORTED_TYPE = {
        DT_INT32, DT_FLOAT16, DT_FLOAT, DT_INT8
    };
...
IMPLEMT_INFERFUNC(HorovodAllgather, HorovodAllgatherInferShape) {
    ...
    outDims = inDims;
    if (!(outDims.size() > 0)) {
        OP_LOGE(op.GetName().c_str(), "out tensor is empty, expected: > 0, actual: %zu.", \
            outDims.size());
        return GRAPH_FAILED;
    }
...
IMPLEMT_VERIFIER(HorovodAllreduce, HorovodAllreduceVerify) {
    ...
    if (it == SUPPORTED_REDUCTION.end()) {
        OP_LOGE(op.GetName().c_str(), "Attr reduction [%d] is not supported. expecttd: min, max, prod, sum", \
            reduction);
        return GRAPH_FAILED;
    }
    // check supported data type in HCCL
