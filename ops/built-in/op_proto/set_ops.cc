...
IMPLEMT_INFERFUNC(DenseToSparseSetOperation, DenseToSparseSetOperationInfer)
{
    ...
    graphStatus status =
        ValidateSparseTensor(x2_indices_tensor, x2_values_tensor, x2_shape_tensor, op.GetName().c_str());
    if (status != GRAPH_SUCCESS) {
...
IMPLEMT_INFERFUNC(SparseToSparseSetOperation, SparseToSparseSetOperationInfer)
{
    ...
    graphStatus status =
        ValidateSparseTensor(x1_indices_tensor, x1_values_tensor, x1_shape_tensor, op.GetName().c_str());
    if (status != GRAPH_SUCCESS) {
