...
bool BroadCastTwoShape(const Operator& op, const ge::Shape& shape_x, const ge::Shape& shape_y,
  ...
  for (size_t i = 0UL; i < dim_x.size(); i++) {
    if ((dim_x[i] != dim_y[i]) && (dim_x[i] != 1) && (dim_y[i] != 1)) {
      OP_LOGE(TbeGetName(op).c_str(), "The %s's dimensions does not match the broadcast rule(%lu %lu).",
              TbeGetName(op).c_str(), dim_x[i], dim_y[i]);
      return false;
    }
...
bool InferShapeForMaximumAndMinimum(Operator& op) {
  ...
  if (op.GetAttr("grad_x", attr_grad_x) == GRAPH_FAILED) {
    OP_LOGE(TbeGetName(op).c_str(), "get attr grad_x failed");
  }
  if (op.GetAttr("grad_y", attr_grad_y) == GRAPH_FAILED) {
    OP_LOGE(TbeGetName(op).c_str(), "get attr grad_y failed");
  }
  if (attr_grad_x == false && attr_grad_y == false) {
    OP_LOGE(TbeGetName(op).c_str(), "the grad_x and grad_y is not support all false");
    return false;
  }
  if (attr_grad_x) {
...
IMPLEMT_VERIFIER(FusedMulAdd, FusedMulAddVerify) {
  ...
  DataType input_type_x3 = op.GetInputDescByName("x3").GetDataType();
  if (input_type_x1 != input_type_x2) {
    string err_msg1 = ConcatString("The ", TbeGetName(op).c_str(),
                                   " op dtype is not same, type1:", input_type_x1, ", type2:", input_type_x2);
  ...
  if (input_type_x2 != input_type_x3) {
    string err_msg1 = ConcatString("The ", TbeGetName(op).c_str(),
                                   " op dtype is not same, type2:", input_type_x2, ", type3:", input_type_x3);
    std::string err_msg = OtherErrMsg(err_msg1);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return false;
  }
...
IMPLEMT_COMMON_INFERFUNC(FusedMulAddInferShape) {
  ...
  CHECK(op_desc == nullptr ||
        ...
        OP_LOGE(TbeGetName(op).c_str(), "invalid OpDesc."), return GRAPH_FAILED);
  ...
  OP_LOGI(TbeGetName(op).c_str(), "shape %s: %s, shape %s: %s, shape %s: %s.",
  ...
  if (!TwoShapeAndRangeBroadcastIntegration(op, dimVec, Vec_range, dimsZ, shape_range_z, "x1_broadcast", "x3")) {
    return GRAPH_FAILED;
  }
...
IMPLEMT_VERIFIER(FusedMulAddAdd, FusedMulAddAddVerify) {
  ...
  DataType input_type_x2 = op.GetInputDescByName("x2").GetDataType();
  DataType input_type_x3 = op.GetInputDescByName("x3").GetDataType();
  DataType input_type_x4 = op.GetInputDescByName("x4").GetDataType();
  if (input_type_x1 != input_type_x2 || input_type_x2 != input_type_x3 || input_type_x3 != input_type_x4) {
    string err_msg1 = ConcatString("The ", TbeGetName(op).c_str(), " op dtype is not same, type1:", input_type_x1,
...
IMPLEMT_COMMON_INFERFUNC(FusedMulAddAddInferShape) {
  ...
  CHECK(yDesc == nullptr, OP_LOGE(TbeGetName(op).c_str(), "Failed to get y desc"), return GRAPH_FAILED);

  yDesc->SetDataType(x1Desc->GetDataType());

  if (IsUnknownVec(x1Dims) || IsUnknownVec(x2Dims) || IsUnknownVec(x3Dims) || IsUnknownVec(x4Dims)) {
    OP_LOGW(TbeGetName(op).c_str(), "Inputs do not support dynamic shape!");
    return GRAPH_FAILED;
  }
...
IMPLEMT_COMMON_INFERFUNC(ReciprocalGradInferShape) {
  ...
  if (IsUnknownRankShape(vec_y) || IsUnknownVec(vec_y)) {
    if (!InferShapeRangeTwoInOneOutBroadcast(op, "y", "dy", "z")) {
      return GRAPH_FAILED;
    }
  }
  return GRAPH_SUCCESS;
...
IMPLEMT_COMMON_INFERFUNC(RsqrtGradInferShape) {
  Shape y_shape = op.GetInputDescByName("y").GetShape();
  DataType input_dtype = op.GetInputDescByName("y").GetDataType();
  std::vector<std::pair<int64_t, int64_t>> shape_range_y;
  auto status = op.GetInputDescByName("y").GetShapeRange(shape_range_y);
...
IMPLEMT_VERIFIER(ApproximateEqual, ApproximateEqualVerify) {
  ...
  if (tolerance_data < 0) {
    std::string err_msg = GetAttrValueErrMsg("tolerance_data", std::to_string(tolerance_data), "tolerance_data >= 0");
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
bool CheckDynamicInputDtype(const Operator& op, const string& input_name1) {
  const char *op_name = "AccumulateNV2";
  DataType first_input_dtype = op.GetDynamicInputDesc(input_name1.c_str(), 0).GetDataType();
  auto input_dynamic_size = op.GetInputsSize();
  for (size_t i = 0; i < input_dynamic_size; ++i) {
    DataType input_dtype = op.GetDynamicInputDesc(input_name1.c_str(), i).GetDataType();
    if (first_input_dtype != input_dtype) {
      std::string err_msg = OtherErrMsg(ConcatString("the op type is not same, type1:", input_dtype,
                                                     ", type2:", first_input_dtype));
      VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op_name, err_msg);
      return false;
    }
  }
...
IMPLEMT_VERIFIER(AccumulateNV2, AccumulateNV2Verify) {
  ...
  if (GRAPH_SUCCESS != op.GetAttr("N", num)) {
    ...
    if (op.GetInputsSize() != static_cast<uint64_t>(num)) {
      string err_msg1 = ConcatString("The ", TbeGetName(op).c_str(), " op size is not same, op.GetInputsSize():",
                                     op.GetInputsSize(), ", num:", static_cast<uint64_t>(num));
      std::string err_msg = OtherErrMsg(err_msg1);
      VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op_name, err_msg);
      return GRAPH_FAILED;
    }
...
static int64_t GenerateOutShape(std::vector<int64_t> shape1, std::vector<int64_t> shape2, int64_t index) {
        ...
        if (!shape_vector.empty() && !IsUnknownRankShape(shape_vector)) {
            if (!IsUnknownRankShape(temp_vector)) {
                ...
                for (size_t j = 0UL; j < shape_vector.size(); j++) {
                    if (shape_vector[j] == -1) {
                        std::string err_msg =
                          OtherErrMsg("Operands could not be broadcast together with these shapes.");
                        VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op_name, err_msg);
                        return GRAPH_FAILED;
                    }
                }
...
IMPLEMT_VERIFIER(FakeQuantWithMinMaxVarsPerChannel, FakeQuantWithMinMaxVarsPerChannelVerify) {
  ...
  if (num_bits < 2 || num_bits > 16) {
    std::string err_msg = GetParamOutRangeErrMsg("num_bits", "2, 16", std::to_string(num_bits));
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
  Shape shape_x = op.GetInputDescByName("x").GetShape();
  Shape shape_min = op.GetInputDescByName("min").GetShape();
  Shape shape_max = op.GetInputDescByName("max").GetShape();
  ...
  if (dims_x.size() < 1) {
    std::string err_msg = GetAttrValueErrMsg("dims_x", std::to_string(dims_x.size()), "dims_x.size() >= 1");
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
IMPLEMT_COMMON_INFERFUNC(FakeQuantWithMinMaxVarsPerChannelInferShape) {
  Shape shape_input = op.GetInputDescByName("x").GetShape();
  DataType dtype_input = op.GetInputDescByName("x").GetDataType();
...
IMPLEMT_VERIFIER(FakeQuantWithMinMaxArgs, FakeQuantWithMinMaxArgsVerify) {
  ...
  if (numBits < 2 || numBits > 16) {
    std::string err_msg = GetParamOutRangeErrMsg("numBits", "2, 16", std::to_string(numBits));
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
IMPLEMT_VERIFIER(FakeQuantWithMinMaxArgsGradient, FakeQuantWithMinMaxArgsGradientVerify) {
  ...
  Shape shape_x = op.GetInputDescByName("x").GetShape();
  Shape shape_y = op.GetInputDescByName("gradients").GetShape();
  std::vector<int64_t> dims_x = shape_x.GetDims();
...
IMPLEMT_COMMON_INFERFUNC(FakeQuantWithMinMaxVarsInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter FakeQuantWithMinMaxVarsInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_VERIFIER(FakeQuantWithMinMaxVarsGradient, FakeQuantWithMinMaxVarsGradientVerify) {
  ...
  std::vector<int64_t> dims_y = shape_y.GetDims();
  if (dims_x.size() != dims_y.size()) {
    string err_msg1 = ConcatString("The dim size is not same, dims_x.size():", dims_x.size(),
                                   ", dims_y.size():", dims_y.size());
    std::string err_msg = OtherErrMsg(err_msg1);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  } else {
...
IMPLEMT_COMMON_INFERFUNC(FakeQuantWithMinMaxVarsGradientInferShape) {
  Shape shape_input_gradients = op.GetInputDescByName("gradients").GetShape();
  Shape shape_input_min = op.GetInputDescByName("min").GetShape();
  Shape shape_input_max = op.GetInputDescByName("max").GetShape();
  DataType dtype_input_gradients = op.GetInputDescByName("gradients").GetDataType();
  DataType dtype_input_min = op.GetInputDescByName("min").GetDataType();
  DataType dtype_input_max = op.GetInputDescByName("max").GetDataType();
  TensorDesc tensordesc_output = op.GetOutputDescByName("backprops_wrt_x");
  TensorDesc tensordesc_output_min = op.GetOutputDescByName("backprops_wrt_min");
  TensorDesc tensordesc_output_max = op.GetOutputDescByName("backprops_wrt_max");
...
IMPLEMT_VERIFIER(FakeQuantWithMinMaxVarsPerChannelGradient, FakeQuantWithMinMaxVarsPerChannelGradientVerify) {
  ...
  if ((dims_min.size() != 1) || (dims_max.size() != 1)) {
    string err_msg1 = ConcatString("shape of min and max must be rank 1, dims_min.size():", dims_min.size(),
                                   ", dims_max.size():", dims_max.size());
    std::string err_msg = OtherErrMsg(err_msg1);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
  if (dims_min[0] != dims_max[0]) {
    string err_msg1 = ConcatString("shape of min and max must be same, dims_min[0]:", dims_min[0],
                                   ", dims_max[0]:", dims_max[0]);
    std::string err_msg = OtherErrMsg(err_msg1);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
  if (dims_x[dims_x.size() - 1] != dims_min[0]) {
    string err_msg1 = ConcatString("The last dimension of x must be the same as min, dims_min[0]:", dims_min[0],
                                   ", dims_x[dims_x.size() - 1]:", dims_x[dims_x.size() - 1]);
    std::string err_msg = OtherErrMsg(err_msg1);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
IMPLEMT_COMMON_INFERFUNC(ArgMaxWithValueInferShape) {
  ...
  CHECK(op_info == nullptr, VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), OtherErrMsg("invalid OpDesc.")),
        return GRAPH_FAILED);
  auto input_desc = op_info->MutableInputDesc(0);
  CHECK(input_desc == nullptr, VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), OtherErrMsg("invalid InputDesc.")),
        return GRAPH_FAILED);
  const GeShape& input_shape = input_desc->MutableShape();
  ...
  CHECK(indice_desc == nullptr, VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), OtherErrMsg("invalid OutputDesc.")),
        return GRAPH_FAILED);
  auto values_desc = op_info->MutableOutputDesc(values_output_idx);
  CHECK(values_desc == nullptr, VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), OtherErrMsg("invalid OutputDesc.")),
        return GRAPH_FAILED);
  ...
  CHECK(GRAPH_SUCCESS != op.GetAttr("dimension", dimension),
        VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), GetInputInvalidErrMsg("dimension")), return GRAPH_FAILED);
  // get attr keep_dims
  bool keep_dims;
  CHECK(GRAPH_SUCCESS != op.GetAttr("keep_dims", keep_dims),
        VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), GetInputInvalidErrMsg("keep_dims")), return GRAPH_FAILED);

  PROFILING_PROTO_AFTER_GET_SHAPE_REG();
  // if input_shape == -2, set output -2
  if (input_shape.IsUnknownDimNum() || input_shape.IsScalar()) {
    OP_LOGD(TbeGetName(op).c_str(), "input is UnknownDimNum, set the output is UnknownDimNum");
...
int64_t GetEltwiseConstValue(ge::Operator& op) {
  int64_t tensor_num;
  if (ge::GRAPH_SUCCESS != op.GetAttr("N", tensor_num)) {
    OP_LOGE(TbeGetName(op).c_str(), "The eltwise op GetOpAttr failed!");
  }
  return tensor_num;
}
...
bool InferShapeAndTypeErfinv(Operator& op, const string& input_name, const string& output_name) {
    ...
    ge::Shape shape = op.GetInputDescByName(input_name.c_str()).GetShape();
...
IMPLEMT_COMMON_INFERFUNC(ErfinvInferShape) {
    ...
    OP_LOGE(TbeGetName(op).c_str(), "The InferShapeAndTypeErfinv is one input and one output.");
    return GRAPH_FAILED;
}
...
IMPLEMT_COMMON_INFERFUNC(LambApplyOptimizerAssignInferShape) {
  Shape x_shape = op.GetInputDescByName("grad").GetShape();
  DataType input_dtype = op.GetInputDescByName("grad").GetDataType();
  TensorDesc tensordesc_output = op.GetOutputDescByName("output0");
  tensordesc_output.SetShape(x_shape);
  tensordesc_output.SetDataType(input_dtype);
  (void)op.UpdateOutputDesc("output0", tensordesc_output);

  Shape v_shape = op.GetInputDescByName("inputv").GetShape();
  DataType inputv_dtype = op.GetInputDescByName("inputv").GetDataType();
  TensorDesc tensordesc_outputv = op.GetOutputDescByName("inputv");
  ...
  Shape m_shape = op.GetInputDescByName("inputm").GetShape();
  DataType inputm_dtype = op.GetInputDescByName("inputm").GetDataType();
  TensorDesc tensordesc_outputm = op.GetOutputDescByName("inputm");
...
IMPLEMT_COMMON_INFERFUNC(LambApplyWeightAssignInferShape) {
  Shape x_shape = op.GetInputDescByName("input_param").GetShape();
  DataType input_dtype = op.GetInputDescByName("input_param").GetDataType();
  TensorDesc tensordesc_output = op.GetOutputDescByName("input_param");
  tensordesc_output.SetShape(x_shape);
  tensordesc_output.SetDataType(input_dtype);
...
IMPLEMT_COMMON_INFERFUNC(SquareSumV2InferShape) {
  auto shape = op.GetInputDescByName("input_x").GetShape();
  DataType input_dtype = op.GetInputDescByName("input_x").GetDataType();
  TensorDesc tensordesc_output = op.GetOutputDescByName("output1");
  ...
  tensordesc_output.SetDataType(input_dtype);
  TensorDesc tensordesc_output1 = op.GetOutputDescByName("output2");
...
IMPLEMT_COMMON_INFERFUNC(SquareSumALlInferShape) {
  ...
  DataType input_x1_dtype = op.GetInputDescByName("x1").GetDataType();
  DataType input_x2_dtype = op.GetInputDescByName("x2").GetDataType();
  TensorDesc tensor_desc_y1 = op.GetOutputDescByName("y1");
  TensorDesc tensor_desc_y2 = op.GetOutputDescByName("y2");
...
IMPLEMT_COMMON_INFERFUNC(FusedMulAddNInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter FusedMulAddNInferShape");
...
IMPLEMT_COMMON_INFERFUNC(FusedMulAddNL2lossInferShape) {
  std::vector<int64_t> o_shape_vector;
  Shape o_shape(o_shape_vector);
  auto shape_x = op.GetInputDescByName("x1").GetShape();
...
IMPLEMT_INFERFUNC(Bias, BiasInferShape) {
  OP_LOGI("Bias", "bias infer shape begin---%d", op.GetInputDescByName("bias").GetShape().GetDims().size());
  DataType dtype_x = op.GetInputDescByName("x").GetDataType();
  ge::Shape shape_x = op.GetInputDescByName("x").GetShape();
  std::vector<int64_t> dims_x = shape_x.GetDims();
  std::vector<std::pair<int64_t, int64_t>> input_range;
  ...
  ge::Shape shape_bias = op.GetInputDescByName("bias").GetShape();
  ...
  if (dims_x.size() == 4 && bias_dim_num != 0) {
    ...
    op.GetInputDescByName("bias").GetShapeRange(range_bias_new);
    if (bias_from_blob) {
      if (num_axes == -1) {
    ...
    TensorDesc bias_desc = op.GetInputDescByName("bias");
...
IMPLEMT_VERIFIER(Bias, BiasVerify) {
  ...
  if (num_axes < -1) {
    std::string err_msg = GetAttrValueErrMsg("num_axes", std::to_string(num_axes), "num_axes >= -1");
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
  ...
  if (bias_from_blob) {
    if (num_axes == -1) {
      ...
      if (length_bias != bias_num) {
        string err_msg1 = ConcatString("length_bias and bias_num must be equal, length_bias:", length_bias,
                                       ", bias_num:", bias_num);
        std::string err_msg = OtherErrMsg(err_msg1);
        VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
        return GRAPH_FAILED;
      ...
      if (num_axis > length_x) {
        string err_msg1 = ConcatString("bias shape extends x shape when applied, num_axis:", num_axis,
                                       ", length_x:", length_x);
        std::string err_msg = OtherErrMsg(err_msg1);
        VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
        return GRAPH_FAILED;
    ...
    if (bias_dim_num != 0) {
      ...
      if (bias_num > length_x) {
        string err_msg1 = ConcatString("bias shape extends x shape when applied, bias_num:", bias_num,
                                       ", length_x:", length_x);
        std::string err_msg = OtherErrMsg(err_msg1);
        VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
        return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(ConfusionMulGradInferShape) {
  auto shape = op.GetInputDescByName("input0").GetShape();
  auto shape1 = op.GetInputDescByName("input1").GetShape();
  DataType input_dtype = op.GetInputDescByName("input0").GetDataType();
  DataType input_dtype1 = op.GetInputDescByName("input1").GetDataType();
  TensorDesc tensordesc_output = op.GetOutputDescByName("output0");
  TensorDesc tensordesc_output1 = op.GetOutputDescByName("output1");
  ...
  if (input_format == FORMAT_NC1HWC0) {
    if (origin_shape.GetDimNum() == 4) {
      if (origin_format == FORMAT_NCHW) {
        ...
        OP_LOGE(TbeGetName(op).c_str(), "5D tensor's origin format should in [NCHW, NHWC]");
        return GRAPH_FAILED;
      }
    } else {
      OP_LOGE(TbeGetName(op).c_str(), "5D tensor's origin shape should be 4D tensor");
      return GRAPH_FAILED;
    }
    ...
    if (axis == 10000 || axis == 1 || axis == 4) {
      OP_LOGE(TbeGetName(op).c_str(), "5D tensor's axis is invalid");
      return GRAPH_FAILED;
    }
  } else if (axis < 0) {
...
IMPLEMT_VERIFIER(ArgMaxWithK, ArgMaxWithKVerify) {
  int topk = op.get_attr_topk();
  if (topk < 1) {
    OP_LOGE(TbeGetName(op).c_str(), "topk must be greater than 0, current topk is %d", topk);
    return GRAPH_FAILED;
  }
  return GRAPH_SUCCESS;
}
...
IMPLEMT_COMMON_INFERFUNC(FillsInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter FillsInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_VERIFIER(MulNoNan, MulNoNanVerify) {
  ...
  DataType input_type_x2 = op.GetInputDescByName("x2").GetDataType();
  if (input_type_x1 != input_type_x2) {
    string err_msg1 = ConcatString("the dtype of input_type_x1 and input_type_x2 must be same! input_type_x1:",
                                   input_type_x1, ", input_type_x2:", input_type_x2);
    std::string err_msg = OtherErrMsg(err_msg1);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
	}
...
IMPLEMT_VERIFIER(CosineEmbeddingLoss, CosineEmbeddingLossVerify) {
  Shape shape_x1 = op.GetInputDescByName("x1").GetShape();
  Shape shape_x2 = op.GetInputDescByName("x2").GetShape();
  if ((shape_x1.GetDimNum() < 2) && (shape_x2.GetDimNum() < 2)) {
    string err_msg1 = ConcatString("input x1 or x2 dims must bigger than 1, shape_x1.GetDimNum():",
                                   shape_x1.GetDimNum(), ", shape_x2.GetDimNum():", shape_x2.GetDimNum());
    std::string err_msg = OtherErrMsg(err_msg1);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
IMPLEMT_COMMON_INFERFUNC(CosineEmbeddingLossInferShape) {
  Shape shape_x1 = op.GetInputDescByName("x1").GetShape();
  Shape shape_x2 = op.GetInputDescByName("x2").GetShape();
  Shape shape_tgt = op.GetInputDescByName("target").GetShape();
  ...
  OP_LOGI(TbeGetName(op).c_str(), "setting margin:%f, reduction:%s\n", margin, reduction.c_str());
...
bool InferShapeAndTypeTensorEqual(Operator &op, const string &input_name1,
  ...
  if (shape_x.GetShapeSize() != shape_y.GetShapeSize()) {
    OP_LOGE(TbeGetName(op).c_str(), "The ShapeSize of input_x does not match input_y.");
    return false;
  }
  return true;
...
IMPLEMT_VERIFIER(TensorEqual, TensorEqualVerify) {
  ...
  if (op.GetInputDescByName("input_x").GetDataType() !=
      op.GetInputDescByName("input_y").GetDataType()) {
    OP_LOGE(TbeGetName(op).c_str(), "input_x input_y tensor dtype does not match.");
    return GRAPH_FAILED;
  }
  return GRAPH_SUCCESS;
}
...
IMPLEMT_VERIFIER(Addcdiv, AddcdivVerify) {
  ...
  if (op.GetInputDescByName("x1").GetDataType() != op.GetInputDescByName("input_data").GetDataType() ||
      op.GetInputDescByName("x2").GetDataType() != op.GetInputDescByName("input_data").GetDataType()) {
    OP_LOGE(op_name, "input_data data type and x1, x2 match failed");
    return GRAPH_FAILED;
  }
  return GRAPH_SUCCESS;
}
...
// ----------------StrideAdd Begin-------------------
bool InferShapeAndTypeStrideAdd(Operator &op, const string &input_name1, const string &input_name2,
                                const string &outputName) {
  TensorDesc output_desc = op.GetOutputDescByName(outputName.c_str());
  ...
  op.UpdateOutputDesc(outputName.c_str(), output_desc);
...
IMPLEMT_COMMON_INFERFUNC(StrideAddInferShape) {
  ...
  OP_LOGE(TbeGetName(op).c_str(), "IMPLEMT_COMMON_INFERFUNC FAILED.");
  return GRAPH_FAILED;
}
...
// ----------------MaskedScale Begin-------------------
bool VerifyMaskedScaleShapeAndType(const Operator &op, DataType x_dtype, DataType mask_dtype) {
    if ((x_dtype != DT_FLOAT) && (x_dtype != DT_FLOAT16)) {
        OP_LOGE(TbeGetName(op).c_str(), "The input dtype of x is invalid, please check!");
        return false;
    }

    if ((mask_dtype != DT_INT8) && (mask_dtype != DT_FLOAT) && (mask_dtype != DT_FLOAT16)) {
        OP_LOGE(TbeGetName(op).c_str(), "The input dtype of mask is invalid, please check!");
        return false;
    }
...
IMPLEMT_VERIFIER(MaskedScale, MaskedScaleVerify) {
    TensorDesc x_tensordesc = op.GetInputDescByName("x");
    DataType x_dtype = x_tensordesc.GetDataType();
    TensorDesc mask_tensordesc = op.GetInputDescByName("mask");
...
IMPLEMT_COMMON_INFERFUNC(MaskedScaleInferShape) {
    ...
    if (input_shape.GetShapeSize() != mask_shape.GetShapeSize()) {
        OP_LOGE(TbeGetName(op).c_str(), "shapesize of x not match mask");
        return GRAPH_FAILED;
    }
...
IMPLEMT_COMMON_INFERFUNC(AddsInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter AddsInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(CeilInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter CeilInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(CosInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter CosInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(CoshInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter CoshInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(SinInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter SinInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(SinhInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter SinhInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(TanInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter TanInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_VERIFIER(Lerp, LerpVerify) {
  DataType start_type = op.GetInputDescByName("start").GetDataType();
  DataType end_type = op.GetInputDescByName("end").GetDataType();
  DataType weight_type = op.GetInputDescByName("weight").GetDataType();
...
IMPLEMT_COMMON_INFERFUNC(FloorInferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter FloorInferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(Expm1InferShape) {
  OP_LOGI(TbeGetName(op).c_str(), "Enter Expm1InferShape");
  if (OneInOneOutDynamicInfer(op, "x", {"y"})) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
IMPLEMT_VERIFIER(DataCompare, DataCompareVerify) {
  ...
  if (ge::GRAPH_SUCCESS != op.GetAttr("atol", atol_data)) {
    OP_LOGE(TbeGetName(op).c_str(), "GetOpAttr failed of DataCompare!");
    return GRAPH_FAILED;
  }
  if (atol_data < 0) {
    OP_LOGE(TbeGetName(op).c_str(), "atol should >= 0!");
    return GRAPH_FAILED;
  }
  ...
  if (rtol_data < 0) {
    OP_LOGE(TbeGetName(op).c_str(), "rtol should >= 0!");
    return GRAPH_FAILED;
  }
...
bool InferShapeDot(Operator& op,
  ...
  TensorDesc input1_desc = op.GetInputDescByName(input1_name.c_str());
  TensorDesc input2_desc = op.GetInputDescByName(input2_name.c_str());
  ...
  if (dims_input1.size() != dims_input2.size()) {
      OP_LOGE(TbeGetName(op).c_str(), "The dim of input_x and input_y not match.");
      return false;
  }

  if (dims_input1.size() != 1) {
      OP_LOGE(TbeGetName(op).c_str(), "The dim of input must be 1");
      return false;
  }

  if (dims_input1[0] != dims_input2[0]) {
      OP_LOGE(TbeGetName(op).c_str(), "The 0-dim of input_x and input_y not match.");
      return false;
  }
...
IMPLEMT_VERIFIER(Dot, DotVerify) {
  if (op.GetInputDescByName("input_x").GetDataType() != op.GetInputDescByName("input_y").GetDataType()) {
      OP_LOGE(TbeGetName(op).c_str(), "The dataType of input_x and input_y not match.");
      return GRAPH_FAILED;
  }
  return GRAPH_SUCCESS;
}
...
IMPLEMT_COMMON_INFERFUNC(ArgMaxGradInferShape) {
    Shape shape = op.GetInputDescByName("var").GetShape();
    DataType input_dtype = op.GetInputDescByName("var").GetDataType();
    Format input_format = op.GetInputDescByName("var").GetFormat();
...
bool IsArgMaxGradCheckPass(const Operator& op,
                           const string& var_name,
                           const string& indices_name,
                           const string& updates_name,
                           const string& dimmension_name) {
    TensorDesc input_var_desc = op.GetInputDescByName(var_name.c_str());
    TensorDesc input_indices_desc = op.GetInputDescByName(indices_name.c_str());
    TensorDesc input_updates_desc = op.GetInputDescByName(updates_name.c_str());
    ...
    auto dim = 0;
    if (op.GetAttr(dimmension_name.c_str(), dim) == GRAPH_FAILED) {
        OP_LOGE(TbeGetName(op).c_str(), "get attr dimension failed");
        return false;
    }
    ...
    if (dims < 0) {
        if (dims < (0 - max_shape_len)) {
            OP_LOGE(TbeGetName(op).c_str(), "attr dimension invalid.should bigger than -max_shape_len");
            return false;
        }
        dims = dims + max_shape_len;
    } else if (dims >= max_shape_len) {
        OP_LOGE(TbeGetName(op).c_str(), "attr dimension invalid. should less than max_shape_len");
        return false;
    }

    if ((shape_var_list.size() > 1) &&
        (shape_var_list.size() != shape_updates_list.size() + 1)) {
        OP_LOGE(TbeGetName(op).c_str(), "The dim size of var should biger than updates(indices) 1.");
        return false;
    }

    if ((1 == shape_var_list.size()) && (1 != shape_updates_list.size())) {
        OP_LOGE(TbeGetName(op).c_str(), "The dim size of var should equal updates(indices) when size=1.");
        return false;
    }

    if (shape_indices_list.size() != shape_updates_list.size()) {
        OP_LOGE(TbeGetName(op).c_str(), "The dim size of indices and updates not match.");
        return false;
    }

    for (size_t i = 0; i < shape_indices.GetDimNum(); i++) {
        if (shape_indices.GetDim(i) != shape_updates.GetDim(i)) {
            OP_LOGE(TbeGetName(op).c_str(), "The dim value of indices and updates not match.");
            return false;
        }
    }
    ...
    if (shape_var_list.size() > 1) {
        for (size_t i = 0; i < shape_indices.GetDimNum(); i++) {
            if (((static_cast<int32_t>(i) < dims) && (shape_indices.GetDim(i) != shape_var.GetDim(i))) ||
                ...
                OP_LOGE(TbeGetName(op).c_str(), "The dim value of var and updates not match.");
                return false;
            }
        }
    }
    ...
    if (var_dtype != updates_dtype) {
        OP_LOGE(TbeGetName(op).c_str(), "The dtype of var and updates not match.");
        return false;
    }
...
IMPLEMT_VERIFIER(ArgMaxGrad, ArgMaxGradVerify) {
    if (true != IsArgMaxGradCheckPass(op, "var", "indices", "updates", "dimension")) {
        OP_LOGE(TbeGetName(op).c_str(), "the ArgMaxGrad op inputs check fail!\n");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}
...
bool IsArgMaxGradDCheckPass(const Operator& op,
                            const string& var_name,
                            const string& indices_name,
                            const string& updates_name,
                            const string& dimmension_name,
    ...
    TensorDesc input_indices_desc = op.GetInputDescByName(indices_name.c_str());
    TensorDesc input_updates_desc = op.GetInputDescByName(updates_name.c_str());
    TensorDesc input_assist_desc = op.GetInputDescByName(assist_name.c_str());
    ...
    if (shape_var_list.size() != shape_assist_list.size()) {
        OP_LOGE(TbeGetName(op).c_str(), "shape of var and assist mot match.");
        return false;
    }
    ...
    for (size_t i = 0; i < shape_var.GetDimNum(); i++) {
        if (shape_var.GetDim(i) != shape_assist.GetDim(i)) {
            OP_LOGE(TbeGetName(op).c_str(), "The dim value of var and assist not match.");
            return false;
        }
    }
...
IMPLEMT_VERIFIER(ArgMaxGradD, ArgMaxGradDVerify) {
    if (true != IsArgMaxGradDCheckPass(op, "var", "indices", "updates", "dimension", "assist")) {
        OP_LOGE(TbeGetName(op).c_str(), "the ArgMaxGradD op inputs check fail!\n");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}
                                       ...
                                       const string &output_name)
{
    ...
    DataType input_dtype = op.GetInputDescByName(input_name.c_str()).GetDataType();
    Format input_format = op.GetInputDescByName(input_name.c_str()).GetFormat();
    ge::Shape shape_x = op.GetInputDescByName(input_name.c_str()).GetShape();
    std::vector<int64_t> dims_x = shape_x.GetDims();
    std::vector<int64_t> dim_vec;
    int64_t attr_dim;
    op.GetAttr(attr_name.c_str(), attr_dim);
...
IMPLEMT_VERIFIER(CosineSimilarity, CosineSimilarityVerify) {
    if (op.GetInputDescByName("input_x1").GetDataType() != op.GetInputDescByName("input_x2").GetDataType())
    {
        OP_LOGE(TbeGetName(op).c_str(), "the op two inputs dtype need equal!\n");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}
...
IMPLEMT_COMMON_INFERFUNC(CosineSimilarityInferShape) {
    ...
    OP_LOGE(TbeGetName(op).c_str(), "Obtains the processing function of the output tensor fail!\n");
    return GRAPH_FAILED;
}
...
IMPLEMT_COMMON_INFERFUNC(ApplyAdamV2InferShape) {
  TensorDesc tensordesc_output_0 = op.GetOutputDescByName("var");
  TensorDesc tensordesc_output_1 = op.GetOutputDescByName("m");
  TensorDesc tensordesc_output_2 = op.GetOutputDescByName("v");
  TensorDesc tensordesc_input = op.GetInputDescByName("var");
