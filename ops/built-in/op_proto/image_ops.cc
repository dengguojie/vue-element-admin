...
IMPLEMT_INFERFUNC(CropAndResize, CropAndResizeInfer)
{
    ...
    if (boxes_dims[0] != UNKNOWN_DIM && box_index_dims[0] != UNKNOWN_DIM &&
...
IMPLEMT_INFERFUNC(ResizeNearestNeighborV2GradD, ResizeNearestNeighborV2GradDInfer)
{
    ...
    if (size_out.size() != DIM_SIZE2) {
        OpsAttrValueErrReport(op.GetName(), "size", Strcat(DIM_SIZE2), Strcat(size_out.size()));
    ...
    if (input_format == FORMAT_NHWC) {
        ...
        string expected_format_list = Strcat("FORMAT_NHWC, FORMAT_NHWC");
        OpsInputFormatErrReport(op.GetName(), "grads", expected_format_list, Strcat(input_format));
        OP_LOGE(op.GetName().c_str(), "Not supported this format%d", input_format);
    }
...
IMPLEMT_INFERFUNC(SampleDistortedBoundingBoxExt2, SampleDistortedBoundingBoxExt2Infer)
{
    ...
    judge = (WithRank(op.get_input_desc_min_object_covered(), 0, min_object_covered, op.GetName().c_str()) !=
        GRAPH_SUCCESS);
    if (judge) {
    ...
    if ((image_size_dim_value != 3) || (bounding_boxes_dim2_value != 4)) {
        OP_LOGE(op.GetName().c_str(), "DimValue0 of input image_size must be 3 and DimValue2 of "
...
IMPLEMT_COMMON_INFERFUNC(ResizeBilinearV2DInferShape)
{
    ...
    if (size_out.size() != DIM_SIZE2) {
        OpsAttrValueErrReport(op.GetName(), "size's length", "2", Strcat(size_out.size()));
        OP_LOGE(op.GetName().c_str(), "length of size_out must be equal to 2");
        return GRAPH_FAILED;
    }
    Format input_format = op.GetInputDesc("x").GetFormat();
    ...
    if (input_format == FORMAT_NHWC) {
        ...
        string expected_format_list = Strcat("FORMAT_NHWC, FORMAT_NCHW");
        OpsInputFormatErrReport(op.GetName(), "x", expected_format_list, Strcat(input_format));
        OP_LOGE(op.GetName().c_str(), "Not supported this format");
    }
    td.SetShape(ge::Shape(y_shape));
    td.SetDataType(DT_FLOAT);
...
// ----------------------KeepRationResizeBilinear----------------------
IMPLEMT_VERIFIER(KeepRationResizeBilinear, KeepRationResizeBilinearVerify) {
    return GRAPH_SUCCESS;
}

IMPLEMT_COMMON_INFERFUNC(KeepRationResizeBilinearInferShape) {
    ...
    if (inputFormat == FORMAT_NHWC) {
        ...
        string expectedFormatList = Strcat("FORMAT_NHWC, FORMAT_NCHW");
        OpsInputFormatErrReport(op.GetName(), "images", expectedFormatList, Strcat(inputFormat));
        OP_LOGE(op.GetName().c_str(), "Not supported this format");
        return GRAPH_FAILED;
    }
...
COMMON_INFER_FUNC_REG(KeepRationResizeBilinear, KeepRationResizeBilinearInferShape);
VERIFY_FUNC_REG(KeepRationResizeBilinear, KeepRationResizeBilinearVerify);
// ----------------------KeepRationResizeBilinear END----------------------
...
IMPLEMT_INFERFUNC(CombinedNonMaxSuppression, CombinedNonMaxSuppressionInfer)
{
    ...
    if (MakeDimForScalarInput(maxTotalSizeTensor, maxTotalSize, op.GetName().c_str()) != GRAPH_SUCCESS) {
        OP_LOGE(op.GetName().c_str(), "input maxTotalSize MakeDimForScalarInput error, real value is %ld.",
            maxTotalSize);
        return GRAPH_FAILED;
    }
    if (maxTotalSize <= 0) {
