...
uint64_t GetSrcImageSizeDtype(::domi::AippOpParams* aipp_op_params, uint64_t batch,
    uint64_t c1, uint64_t height, uint64_t width, ge::DataType *src_img_dtype) {
    uint64_t size = 0;
...
std::vector<int32_t> GetAclInputDims(::domi::AippOpParams* aipp_op_params, uint64_t batch,
                     uint64_t srcImageHeight, uint64_t srcImageWidth) {
    uint64_t channel = 3;
...
IMPLEMT_INFERFUNC(Aipp, AippInfer) {
    ...
    if (output_height != height || output_width != width) {
        OpsAippErrReport(Strcat(output_height), Strcat(output_width), Strcat(height), Strcat(width));
    ...
    if (aclInputDims.size() >= 4) {
        OP_LOGI(op.GetName().c_str(), "aclInputDims: %d, %d, %d, %d",
            aclInputDims[0], aclInputDims[1], aclInputDims[2], aclInputDims[3]);
