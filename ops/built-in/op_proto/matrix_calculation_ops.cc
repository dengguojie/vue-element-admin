...
IMPLEMT_VERIFIER(FullyConnection, FullyConnectionVerify)
{
    ...
    if (axis_new != 1 && axis_new != 2) {
        string realvalue = Strcat(axis_new);
    ...
    // check wShape size
    if (wShape.size() != 2) {
        string realvalue = Strcat(wShape.size());
...
IMPLEMT_COMMON_INFERFUNC(MatMulV2InferShape)
{
    ...
    auto input_format_1 = FORMAT_NHWC;
...
IMPLEMT_COMMON_INFERFUNC(MatrixDiagDInferShape)
{
    ...
    for (size_t i = 0; i < dims_x.size(); i++) {
        if ((dims_x[i] != dims_assist[i]) && (dims_x[i] != 1) && (dims_assist[i] != 1)) {
            OpsInputShapeBroadcastErrReport(op.GetName(), "x", "assist", Strcat(dims_x[i]), Strcat(dims_assist[i]));
...
IMPLEMT_VERIFIER(MatrixDiagD, MatrixDiagDVerify)
{
    ...
    if (input_diagonal_dtype != input_help_dtype) {
        OpsTwoInputDtypeErrReport(op.GetName(), "input_diagonal", "input_help", Strcat(input_diagonal_dtype),
            Strcat(input_help_dtype));
        OP_LOGE(op.GetName().c_str(), "the matrix_diag op inputs "
...
IMPLEMT_VERIFIER(MatrixDiagPartD, MatrixDiagPartDVerify)
{
    ...
    if (input_diagonal_dtype != input_help_dtype) {
        OpsTwoInputDtypeErrReport(op.GetName(), "input_diagonal", "input_help", Strcat(input_diagonal_dtype),
            Strcat(input_help_dtype));
        OP_LOGE(op.GetName().c_str(), "the matrix_diag_part op inputs "
            "should have the same dtype!\n");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
...
IMPLEMT_VERIFIER(MatrixSetDiagD, MatrixSetDiagDVerify)
{
    ...
    if ((input_matrix_dtype != input_diagonal_dtype) || (input_matrix_dtype != input_help_dtype)) {
        OpsTwoInputDtypeErrReport(op.GetName(), "input_matrix", "input_help", Strcat(input_matrix_dtype),
            Strcat(input_help_dtype));
        OP_LOGE(op.GetName().c_str(), "the matrix_set_part op inputs "
            "should have the same dtype!\n");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
...
IMPLEMT_VERIFIER(ScatterDiv, ScatterDivVerify)
{
    ...
    if (var_dtype != updates_dtype) {
        OpsTwoInputDtypeErrReport(op.GetName(), "var", "updates", Strcat(var_dtype), Strcat(updates_dtype));
        OP_LOGE(op.GetName().c_str(), "the scatter_div op inputs "
            "should have the same dtype!");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
...
IMPLEMT_COMMON_INFERFUNC(ConfusionMatrixInferShape)
{
    ...
    if (output_dtype_str == "float32") {
        ...
        string expected_data_type_list = Strcat("float32, int32, int8, float16, uint8");
...
IMPLEMT_VERIFIER(ScatterMul, ScatterMulVerify)
{
    ...
    if (var_dtype != updates_dtype) {
        OpsTwoInputDtypeErrReport(op.GetName(), "var", "updates", Strcat(var_dtype), Strcat(updates_dtype));
        OP_LOGE(op.GetName().c_str(), "the scatter_mul op inputs "
            "should have the same dtype!\n");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
...
IMPLEMT_VERIFIER(ScatterMin, ScatterMinVerify)
{
    ...
    if (var_dtype != updates_dtype) {
        OpsTwoInputDtypeErrReport(op.GetName(), "var", "updates", Strcat(var_dtype), Strcat(updates_dtype));
        OP_LOGE(op.GetName().c_str(), "the scatter_min op inputs "
            "should have the same dtype!\n");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
...
IMPLEMT_VERIFIER(ScatterMax, ScatterMaxVerify)
{
    ...
    if (var_dtype != updates_dtype) {
        OpsTwoInputDtypeErrReport(op.GetName(), "var", "updates", Strcat(var_dtype), Strcat(updates_dtype));
        OP_LOGE(op.GetName().c_str(), "the scatter_max op inputs "
            "should have the same dtype!\n");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
    ...
    if (op.GetInputConstData("k", k_tensor) == GRAPH_SUCCESS && FullyDefined(k_shape)) {
        ...
        if (k_shape.GetDimNum() == 0) {
            ...
            if (num_elements == 1) {
                ...
                OP_LOGE(op.GetName().c_str(),
                    "diag_index must be a vector with one or two elements. It has %lld elements", num_elements);
                return GRAPH_PARAM_INVALID;
            }
        }
    ...
    if (RankKnown(input_shape)) {
        ...
        if (k_index_known) {
            if (WithRank(diagonal_tensor_desc, (lower_diag_index == upper_diag_index ? input_rank - 1 : input_rank),
                ...
                OP_LOGE(op.GetName().c_str(),
                    "input diagonal must be fit with input matrix rank %lld, real rank is %lld.", input_rank,
                    diagonal_shape.GetDimNum());
                return GRAPH_FAILED;
            } else {
                if (WithRankAtLeast(diagonal_tensor_desc, input_rank - 1, diagonal_shape, op.GetName().c_str()) !=
                    GRAPH_SUCCESS) {
                    OP_LOGE(op.GetName().c_str(), "input diagonal must be at least %lld-D, real rank is %lld.",
                        input_rank - 1, diagonal_shape.GetDimNum());
                    return GRAPH_FAILED;
                }

                if (WithRankAtMost(diagonal_tensor_desc, input_rank, diagonal_shape, op.GetName().c_str()) !=
                    GRAPH_SUCCESS) {
                    OP_LOGE(op.GetName().c_str(), "input diagonal must be at most %lld-D, real rank is %lld.",
...
IMPLEMT_COMMON_INFERFUNC(MatrixDiagV2InferShape)
{
    ...
    if (lower_diag_index < upper_diag_index) {
        ...
        if (num_diags != (upper_diag_index - lower_diag_index + 1)) {
            OP_LOGE(op.GetName().c_str(), "The number of rows of `diagonal` doesn't match the number of \
    ...
    if (num_rows != min_num_rows && num_cols != min_num_cols) {
        OP_LOGE(op.GetName().c_str(), "num_rows and num_cols are not consistent with lower_diag_index, \
    ...
    if (lower_diag_index == upper_diag_index) {
        if (ReplaceDim(diagonal_shape, diagonal_rank - 1, num_rows, output_shape, op.GetName().c_str()) !=
            GRAPH_SUCCESS) {
        ...
        if (ReplaceDim(diagonal_shape, diagonal_rank - 2, num_rows, output_shape, op.GetName().c_str()) !=
            GRAPH_SUCCESS) {
            OP_LOGE(op.GetName().c_str(), "failed to replacedim from diagonal_shape.");
            return GRAPH_FAILED;
        }
        if (ReplaceDim(output_shape, diagonal_rank - 1, num_cols, output_shape, op.GetName().c_str()) !=
            GRAPH_SUCCESS) {
