...
IMPLEMT_INFERFUNC(CompareAndBitpack, CompareAndBitpackInfer)
{
    ...
    if ((!dims.empty()) && (dims != UNKNOWN_SHAPE)) {
        ...
        if (Divide(last_dim, int64_t(8), true, inferred_dim, op.GetName().c_str()) != GRAPH_SUCCESS) {
            string err_msg = Strcat("the last dim[", last_dim, "] of 0th input must be the multiple of 8");
...
IMPLEMT_INFERFUNC(SparseSegmentMeanGrad, SparseSegmentMeanGradInfer)
{
    ...
    if(INPUT_NUM != op.GetInputsSize()){
        OP_LOGE(op.GetName().c_str(), "the SparseSegmentMeanGrad op's input_num should be %z, real input_num is %z",
    ...
    if(OUTPUT_NUM != op.GetOutputsSize()){
        OP_LOGE(op.GetName().c_str(), "the SparseSegmentMeanGrad op's output_num should be %z, real output_num is %z",
                OUTPUT_NUM, op.GetInputsSize());
        return GRAPH_FAILED;
    }
    ...
    if (WithRankAtLeast(x_desc, 1, x_ge_shape) != GRAPH_SUCCESS) {
        OP_LOGE(op.GetName().c_str(), "input x should be at least 1-D, real rank is %lld",
    ...
    if (WithRank(indices_desc, 1, indices_shape) != GRAPH_SUCCESS) {
        OP_LOGE(op.GetName().c_str(), "input indices must be 1-D, real rank is %lld",
                indices_desc->GetShape().GetDimNum());
        return GRAPH_FAILED;
    }
...
IMPLEMT_COMMON_INFERFUNC(HistogramFixedWidthInferShape)
{
    ...
    if (op.GetAttr("dtype", dtype_attr) == GRAPH_SUCCESS) {
        if (dtype_attr != "int32") {
            OP_LOGE(op.GetName().c_str(), "dtype_attr only "
                "support int32.");
            return GRAPH_FAILED;
        }
    }
...
IMPLEMT_VERIFIER(HistogramFixedWidth, HistogramFixedWidthVerify)
{
    ...
    if (x_dtype != range_dtype) {
        OP_LOGE(op.GetName().c_str(), "the HistogramFixedWidth op inputs "
...
IMPLEMT_COMMON_INFERFUNC(HistogramFixedWidthDInferShape)
{
    ...
    if (nbins <= 0) {
        OpsAttrValueErrReport(op.GetName(), "nbins", ">0", Strcat(nbins));
...
IMPLEMT_COMMON_INFERFUNC(NLLLossInferShape)
{
    ...
    if (x_shape.GetDimNum() == 2 and reduction == "none") {
