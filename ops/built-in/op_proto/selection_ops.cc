...
static graphStatus GetStridedSliceGradValue(const ge::Operator& op, const std::string& keyName,
  ...
  if (ge::GRAPH_SUCCESS != op.GetAttr(keyName.c_str(), multiples)) {
...
IMPLEMT_COMMON_INFERFUNC(StridedSliceGradDInferShape) {
  ge::Shape outputShape = op.GetInputDescByName("dy").GetShape();
  DataType input_dtype = op.GetInputDescByName("dy").GetDataType();
...
static graphStatus IsMasksAllZero(const ge::Operator& op, struct SsgMasks& slice_masks, bool& no_mask) {
  ...
  for (auto& item : mask_maps) {
    ...
    if (ge::GRAPH_SUCCESS != op.GetAttr(item.first.c_str(), mask_value)) {
...
IMPLEMT_COMMON_INFERFUNC(StridedSliceGradInferShape) {
  ...
  DataType input_dtype = op.GetInputDescByName("dy").GetDataType();
  Shape shape_dy = op.GetInputDescByName("dy").GetShape();
  ...
  op.GetInputDescByName("dy").GetShapeRange(in_range);

  auto op_desc = OpDescUtils::GetOpDescFromOperator(op);
  op_desc->SetOpInferDepends({"shape", "begin", "end", "strides"});
  auto dim_vector = op.GetInputDescByName("shape").GetShape().GetDims();
  if (dim_vector.empty()) {
  ...
  if (op.GetInputConstData("shape", output_shape_tensor) != GRAPH_SUCCESS) {
    ...
    if (shape_dy.GetDims() == UNKNOWN_RANK) {
      ...
      if (dim_vector != UNKNOWN_RANK and dim_vector != UNKNOWN_SHAPE) {
        ...
        OP_LOGD(TbeGetName(op).c_str(), "dim_num is %d.", dim_num);
    ...
    if (dim_vector == UNKNOWN_SHAPE || dim_vector == UNKNOWN_RANK) {
      OP_LOGD(TbeGetName(op).c_str(), "shape is UNKNOWN_SHAPE or UNKNOWN_RANK");
      vector<int64_t> shape(1, -2);
      Shape out_shape(shape);
      TensorDesc tensordesc_output = op.GetOutputDescByName("output");
      tensordesc_output.SetShape(out_shape);
    ...
    for (const auto& param : list_names) {
      begin_len = std::max(op.GetInputDescByName(param.c_str()).GetShape().GetDim(0), begin_len);
    }
  ...
  DataType dtype = op.GetInputDescByName("shape").GetDataType();
...
static void GetTileConstValue(const Operator& op, const Tensor& const_tensor, const DataType& dtype,
  ...
  if (dtype == ge::DT_INT32) {
    ...
    OP_LOGE(TbeGetName(op).c_str(), "not support this type");
  }
}

static graphStatus TileInferShapeAndType(ge::Operator& op, std::vector<int64_t>& multiples) {
  OP_LOGI(TbeGetName(op).c_str(), "Get into multiples attr branch.");
  ...
  constexpr uint64_t max_input_len = 8;
  constexpr uint64_t min_input_len = 2;

  std::vector<std::pair<int64_t, int64_t>> ori_shape_range;
  op.GetInputDescByName("x").GetShapeRange(ori_shape_range);
  ...
  if (multiples_len < input_len) {
    OP_LOGE(TbeGetName(op).c_str(), "the tile multiples len %lu is less than the input len %lu!", multiples_len,
            input_len);
    return GRAPH_FAILED;
  }
  ...
  if (IsUnknownRankShape(input_vector)) {
    OP_LOGI(TbeGetName(op).c_str(), "Get into multiples attr and input unknown rank.");
  ...
  if (input_len != multiples_len) {
    OP_LOGI(TbeGetName(op).c_str(), "Get into align input shape with multiples.");
  ...
  if (input_len == 0) {
    ...
    OP_LOGI(TbeGetName(op).c_str(), "Get into input and multiples all empty.");
    ...
    if (input_shape.GetDim(0) >= 0 && (!multiples.empty())) {
      OP_LOGI(TbeGetName(op).c_str(), "Get into align_input len 1 and input shape >= 0.");
      ...
      OP_LOGI(TbeGetName(op).c_str(), "Get into align_input len 1 and input shape == -1.");
      if (ori_shape_range[0].second == -1 || ori_shape_range[0].second * multiples[0] >= shape_threshold) {
        ...
        output_range.push_back(
            std::make_pair(ori_shape_range[0].first * multiples[0], ori_shape_range[0].second * multiples[0]));
      }
      output_desc.SetShape(input_shape);
      output_desc.SetDataType(input_dtype);
      output_desc.SetShapeRange(output_range);
      ...
      OP_LOGE(TbeGetName(op).c_str(), "Illegal input dim value when multiples len is 1.");
      return GRAPH_FAILED;
    }

  } else if (input_len <= max_input_len && input_len >= min_input_len) {
    for (uint64_t i = 0; i < input_len; i++) {
      if (input_shape.GetDim(i) >= 0) {
        ...
        if (ori_shape_range[i].second == -1 || ori_shape_range[i].second * multiples[i] >= shape_threshold) {
          ...
          output_range.push_back(
              std::make_pair(ori_shape_range[i].first * multiples[i], ori_shape_range[i].second * multiples[i]));
        }
      }
    }
    output_desc.SetShape(input_shape);
    ...
    OP_LOGE(TbeGetName(op).c_str(), "Illegal input len while the input_len is %lu", input_len);
    return GRAPH_FAILED;
  }
}
...
IMPLEMT_COMMON_INFERFUNC(TileInferShape) {
  ...
  if (op.GetInputConstData("multiples", multiples_tensor) != GRAPH_SUCCESS) {
    ...
    OP_LOGI(TbeGetName(op).c_str(), "Get constValue failed of [multiples]");
    std::vector<int64_t> multiples_vector = op.GetInputDescByName("multiples").GetShape().GetDims();
    ...
    if (multiples_vector.empty() || (!multiples_vector.empty() && multiples_vector[0] == 0)) {
      OP_LOGE(TbeGetName(op).c_str(), "Get illegal multiples vector value empty or 0!");
      return GRAPH_FAILED;
    } else if (multiples_vector.size() == 1 && multiples_vector[0] > 0) {
      // output_vector length equals to multiples, and all the range is (1, -1)
      OP_LOGI(TbeGetName(op).c_str(), "Get into the branch multiples tensor shape bigger than zero.");
      ...
      if (input_vector.empty() || UNKNOWN_RANK == input_vector) {
        OP_LOGI(TbeGetName(op).c_str(), "Get into branch multiples unknown and input empty or unknown rank.");
        ...
        OP_LOGI(TbeGetName(op).c_str(), "Get into branch multiples unknown and input static or unknown dims.");
      ...
      OP_LOGE(TbeGetName(op).c_str(), "Illegal input of tensor multiples!");
      return GRAPH_FAILED;
    }
  }

  DataType dtype = op.GetInputDescByName("multiples").GetDataType();
...
vector<int64_t> GetTileDConstValue(const ge::Operator& op, const std::string& key_name) {
  std::vector<int64_t> multiples;
  if (ge::GRAPH_SUCCESS != op.GetAttr(key_name.c_str(), multiples)) {
...
IMPLEMT_COMMON_INFERFUNC(RangeInferShape) {
  ...
  if ((input_start_tensor == nullptr) ||
    ...
    if (assist_num_one < 1e-6) {
      AICPU_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), string("the value of input[delta] should not be zero"));
      return GRAPH_FAILED;
    }
    ...
    OP_LOGD(TbeGetName(op).c_str(), "output shape: [%s].", to_string(dimsIn).c_str());
    return GRAPH_SUCCESS;
  }
}
...
IMPLEMT_COMMON_INFERFUNC(RangeDInferShape) {
  ...
  (void)op.UpdateOutputDesc("y", op.GetInputDescByName("x"));
  return GRAPH_SUCCESS;
}
...
static graphStatus GatherV2InferOptimize(ge::Operator &op, int64_t &axis, GeTensorDescPtr &x_desc,
  ...
  if (IsUnknownRankShape(indices_shape) || IsUnknownRankShape(x_shape)) {
    y_shape.push_back(UNKNOWN_RANK_DIM);
...
IMPLEMT_COMMON_INFERFUNC(GatherV2InferShape) {
    ...
    if (ge::GRAPH_SUCCESS != static_cast<int64_t>(op.GetAttr("batch_dims", batch_dims))) {
      batch_dims = 0;
      OP_LOGW(TbeGetName(op).c_str(), "GetAttr(batch_dims) failed, set default value to 0.");
    }
    ...
    if (IsUnknownRankShape(indices_shape) || IsUnknownRankShape(x_shape)) {
      ...
      // unknown shape
      OP_LOGI(TbeGetName(op).c_str(), "GetInputConstData(axis) [%d]", result);
...
static graphStatus GatherV2InferShapeAndType(ge::Operator& op, int32_t& axis) {
  ...
  if (axis < 0) {
    ...
    std::string err_msg =
        OtherErrMsg(ConcatString("x_desc RealDimCnt[", x_real_dim_cnt, "] < axis + 1[", axis + 1, "]"));
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
IMPLEMT_COMMON_INFERFUNC(GatherInferShape) {
  ...
  OP_LOGI(TbeGetName(op).c_str(), "output shape range is:%s", to_string(y_shape_range).c_str());
  return GRAPH_SUCCESS;
}
...
IMPLEMT_COMMON_INFERFUNC(GatherDInferShape) {
    auto op_info = OpDescUtils::GetOpDescFromOperator(op);
    if (op_info == nullptr) {
      OP_LOGE(TbeGetName(op).c_str(), "op_info should not be nullptr");
      return GRAPH_FAILED;
    }
...
bool InferShapeAndTypeLogSpaceD(Operator& op, const string& input_name, const string& output_name,
  ...
  Format output_format = op.GetInputDescByName(input_name.c_str()).GetFormat();
  ...
  int64_t dtype = 1;
  if (op.GetAttr(attr_name.c_str(), dtype) != GRAPH_SUCCESS) {
...
{
    if (op.GetInputDescByName("assist").GetShape().GetDims().size() != 1) {
        OP_LOGE(TbeGetName(op).c_str(), "Input size must be 1.");
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}
...
IMPLEMT_COMMON_INFERFUNC(LogSpaceDInferShape) {
  ...
  OP_LOGE(TbeGetName(op).c_str(), "should have the assist, y and dtype.");
  return GRAPH_FAILED;
}
...
IMPLEMT_COMMON_INFERFUNC(UnsortedSegmentSumDInferShape) {
  ...
  if (input_num_segments <= 0) {
    std::string err_msg =
        GetAttrValueErrMsg("num_segments", ConcatString(input_num_segments), ConcatString("greater than 0"));
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
static graphStatus GetStridedSliceListConstValues(const ge::Operator& op, struct SliceParameters& slice_params) {
  ...
  for (auto& item: const_values) {
    if (op.GetInputConstData(item.first.c_str(), const_tensor) != GRAPH_SUCCESS) {
      OP_LOGI(TbeGetName(op).c_str(), "[%s] is not constant.", item.first.c_str());
      all_const = false;
      continue;
    }

    item.second.clear();
    auto dtype = op.GetInputDescByName(item.first.c_str()).GetDataType();
...
IMPLEMT_COMMON_INFERFUNC(StridedSliceDInferShape) {
  ...
  // Get input shape
  ge::Shape shape = op.GetInputDescByName("x").GetShape();
  DataType input_dtype = op.GetInputDescByName("x").GetDataType();
  ...
  if (!StridedSliceCommonInferShape(TbeGetName(op), input_params, output_shape, output_ranges)) {
    return GRAPH_FAILED;
  }
  ...
  OP_LOGD(TbeGetName(op).c_str(), "output_shape:%s", to_string(tensor_desc_output.GetShape()).c_str());
...
IMPLEMT_INFER_DATA_SLICE(StridedSliceD, StridedSliceDInferDataSlice) {
  ...
  if (!output_data_slice.empty()) {
    ...
    OP_LOGE_IF(!AttrUtils::SetListListInt(desc, ge::ATTR_NAME_DATA_SLICE, output_data_slice), GRAPH_FAILED,
                 TbeGetName(op), "Set input(%s) data slice failed", desc->GetName().c_str());
...
IMPLEMT_COMMON_INFERFUNC(StridedSliceInferShape) {
  ...
  OP_LOGD(TbeGetName(op).c_str(), "begin_len:%lld", begin_len);
  ...
  OP_LOGD(TbeGetName(op).c_str(), "begin_list:%s", to_string(slice_params.begin_list).c_str());
  OP_LOGD(TbeGetName(op).c_str(), "end_list:%s", to_string(slice_params.end_list).c_str());
  OP_LOGD(TbeGetName(op).c_str(), "stride_list:%s", to_string(slice_params.stride_list).c_str());
  ...
  ge::Shape outputShape = ge::Shape(output_shape);
  tensor_desc_output.SetShape(outputShape);
  OP_LOGD(TbeGetName(op).c_str(), "output_ranges:%s", to_string(output_ranges).c_str());
  OP_LOGD(TbeGetName(op).c_str(), "output_shape:%s", to_string(tensor_desc_output.GetShape()).c_str());
  (void) op.UpdateOutputDesc("y", tensor_desc_output);
...
bool BroadCastTwoinOneout(const Operator& op, std::vector<int64_t>& shape_x, std::vector<int64_t>& shape_y,
  ...
  for (size_t i = 0; i < dim_x.size(); i++) {
    if (dim_x[i] == UNKNOWN_RANK_DIM || dim_y[i] == UNKNOWN_RANK_DIM) {
      dim_out.push_back(UNKNOWN_RANK_DIM);
      return true;
    }
  }
  // set out dims
...
static bool SegmentDShapeVerify(const Operator& op, const std::string& input_name,
  ...
  if (GRAPH_SUCCESS != op.GetAttr(segment_ids_name.c_str(), segment_ids)) {
...
IMPLEMT_COMMON_INFERFUNC(SegmentMaxDInferShape) {
  ...
  for (size_t dim = 0; dim < segment_ids.size(); dim++) {
    if (dim == 0 && segment_ids[dim] < 0) {
      OP_LOGE(TbeGetName(op).c_str(), "segment_ids must be positive integer");
      return GRAPH_FAILED;
    }
  ...
  auto output_shape_dims = input_desc.GetShape().GetDims();
  if (output_shape_dims.empty()) {
    OP_LOGE(TbeGetName(op).c_str(), "The dims of input is empty!");
    return GRAPH_FAILED;
  }
  output_shape_dims[0] = first_axis_dims;
...
IMPLEMT_COMMON_INFERFUNC(SliceDInferShape) {
  ...
  if ((int64_t)input_size.size() != (int64_t)dimNum) {
    std::string err_msg =
        GetAttrSizeErrMsg("input_size", ConcatString((int64_t)input_size.size()), ConcatString((int64_t)dimNum));
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
  if ((int64_t)input_begin.size() != (int64_t)dimNum) {
    std::string err_msg =
        GetAttrSizeErrMsg("input_begin", ConcatString((int64_t)input_begin.size()), ConcatString((int64_t)dimNum));
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
IMPLEMT_COMMON_INFERFUNC(OneHotDInferShape) {
  ...
  if (ge::GRAPH_SUCCESS != op.GetAttr("depth", depth)) {
    OP_LOGE(TbeGetName(op).c_str(), "OneHot GetOpAttr depth failed!");
    return GRAPH_FAILED;
  }
  if (depth < 1) {
    OP_LOGE(TbeGetName(op).c_str(), "depth need greater than or equals to 1");
    return GRAPH_FAILED;
  }
  ...
  int32_t axis = -1;
  if (ge::GRAPH_SUCCESS != op.GetAttr("axis", axis)) {
    OP_LOGE(TbeGetName(op).c_str(), "Get const axis failed from op of 'OneHotD'!\n");
    return GRAPH_FAILED;
  }
  if (axis < -dim_nums || axis > dim_nums) {
    OP_LOGE(TbeGetName(op).c_str(), "attr axis is not in range");
    return GRAPH_FAILED;
  }
  if (axis < -1) {
    OP_LOGE(TbeGetName(op).c_str(), "attr axis must be >= -1");
    return GRAPH_FAILED;
  }

  DataType input_type = op.GetInputDescByName("on_value").GetDataType();
...
IMPLEMT_COMMON_INFERFUNC(TopKV2DInferShape) {
  ...
  if (!unkonwn_dim_flag && k_tensor.GetData() != nullptr) {
    DataType dtype = op.GetInputDescByName("k").GetDataType();
    if (dtype == DT_INT32) {
...
IMPLEMT_COMMON_INFERFUNC(ScatterNdDInferShape) {
  ...
  if (GRAPH_SUCCESS != op.GetAttr(shape_name.c_str(), shape_out_list)) {
...
bool InTopKDCheckInputX1AndX2(const Operator& op) {
  ...
  if (input_prediction == nullptr) {
    OP_LOGE(TbeGetName(op).c_str(), "Get constValue failed of [x1].");
    return false;
  }
  auto input_target = op_info->MutableInputDesc("x2");
  if (input_target == nullptr) {
    OP_LOGE(TbeGetName(op).c_str(), "Get constValue failed of [x2].");
    return false;
  }
  auto output_desc = op_info->MutableOutputDesc("y");
  if (output_desc == nullptr) {
    OP_LOGE(TbeGetName(op).c_str(), "Get constValue failed of [y].");
    return false;
  }
  ...
  if (target_dim != 1) {
    OP_LOGE(TbeGetName(op).c_str(), "Target must be 1-dimensional, but get %lu", target_dim);
    return false;
  }
  if (!unknown_rank) {
    size_t prediction_dim = input_prediction->GetShape().GetDimNum();
    constexpr size_t dim_num_2d = 2;
    if (prediction_dim != dim_num_2d) {
      OP_LOGE(TbeGetName(op).c_str(), "Predictions must be 2-dimensional, but get %lu", prediction_dim);
      return false;
    }
...
IMPLEMT_COMMON_INFERFUNC(InTopKDInferShape) {
  ...
  if (input_prediction == nullptr) {
    AICPU_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op),
                                       std::string("get input[x1] desc failed, input[x1] desc is nullptr."));
    return GRAPH_FAILED;
  }
  auto input_target = op_info->MutableInputDesc("x2");
  if (input_target == nullptr) {
    AICPU_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op),
                                       std::string("get input[x2] desc failed, input[x2] desc is nullptr."));
    return GRAPH_FAILED;
  }
  auto output_desc = op_info->MutableOutputDesc("y");
  if (output_desc == nullptr) {
    AICPU_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op),
                                       std::string("get output[y] desc failed, output[y] desc is nullptr."));
    return GRAPH_FAILED;
  }
  ...
  if (unknown_rank) {
    ...
    if (input_target->GetShape().GetDim(0) == -1 || input_prediction->GetShape().GetDim(0) == -1 ||
      ...
      if (output_desc->SetShapeRange(input1_range) != GRAPH_SUCCESS) {
        AICPU_INFER_SHAPE_CALL_ERR_REPORT(TbeGetName(op), std::string("set output[y] shape range failed."));
        return GRAPH_FAILED;
      }
    }
  }
...
IMPLEMT_COMMON_INFERFUNC(StridedSliceAssignInferShape) {
  TensorDesc tensordesc_output = op.GetOutputDescByName("var");
  tensordesc_output.SetShape(op.GetInputDescByName("var").GetShape());
  tensordesc_output.SetDataType(op.GetInputDescByName("var").GetDataType());
...
IMPLEMT_COMMON_INFERFUNC(StridedSliceAssignDInferShape) {
  ...
  if (!CheckListEmpty(TbeGetName(op), begin, "begin")) {
    std::string err_msg = GetInputInvalidErrMsg("begin");
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
  ...
  if (!CheckListEmpty(TbeGetName(op), end, "end")) {
...
IMPLEMT_COMMON_INFERFUNC(CumminInferShape) {
  ...
  TensorDesc output_desc_argmin = op.GetOutputDescByName("indices");
...
IMPLEMT_COMMON_INFERFUNC(CummaxInferShape) {
  TensorDesc output_desc_y = op.GetOutputDescByName("y");
  TensorDesc output_desc_indices = op.GetOutputDescByName("indices");
  DataType predict_dtype = op.GetInputDescByName("x").GetDataType();
  Format predict_format = op.GetInputDescByName("x").GetFormat();
  ge::Shape output_shape = op.GetInputDescByName("x").GetShape();
  output_desc_y.SetDataType(predict_dtype);
...
IMPLEMT_COMMON_INFERFUNC(InplaceUpdateInferShape) {
  ...
  VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), OtherErrMsg("InplaceUpdate OneInOneOutDynamicInfer failed"));
  return GRAPH_FAILED;
}
...
IMPLEMT_COMMON_INFERFUNC(InplaceUpdateDInferShape) {
  auto output_desc = op.GetInputDescByName("x");
  auto input_v_desc = op.GetInputDescByName("v");
  int64_t dim_value_v;
  ...
  if ((int64_t)indices.size() != dim_value_v) {
    string err_msg = ConcatString("The length of rank 0 of tensor v must be the same as length of indices. indices:",
                                  (int64_t)indices.size(), ", dim_value_v:", dim_value_v);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
  (void)op.UpdateOutputDesc("y", output_desc);
...
IMPLEMT_COMMON_INFERFUNC(InplaceAddInferShape) {
  ...
  Tensor indices;
  graphStatus ret = op.GetInputConstData(indices_name.c_str(), indices);
  OP_LOGI(TbeGetName(op).c_str(), "InplaceAddInferShape get const data from input[%s] ret = %d",
...
IMPLEMT_COMMON_INFERFUNC(InplaceAddDInferShape) {
  ...
  if ((int64_t)indices.size() != dim_value_v) {
    string err_msg1 = ConcatString("bias shape extends x shape when applied, indices:", (int64_t)indices.size(),
                                   ", dim_value_v:", dim_value_v);
...
IMPLEMT_COMMON_INFERFUNC(InplaceSubInferShape) {
  ...
  graphStatus ret = op.GetInputConstData(indices_name.c_str(), indices);
  OP_LOGI(TbeGetName(op).c_str(), "InplaceSubInferShape get const data from input[%s] ret = %d",
          indices_name.c_str(), static_cast<int>(ret));
...
IMPLEMT_COMMON_INFERFUNC(InplaceSubDInferShape) {
  ...
  if ((int64_t)indices.size() != dim_value_v) {
    string err_msg1 = ConcatString("The length of rank 0 of tensor v must be the same as length of indices., indices:",
                                   (int64_t)indices.size(), ", dim_value_v:", dim_value_v);
    std::string err_msg = OtherErrMsg(err_msg1);
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(ProposalInferShape) {
  ...
  auto cls_prob_shape = op.GetInputDescByName("cls_prob").GetShape().GetDims();
  int64_t batch = cls_prob_shape[0];
  DataType input_dtype = op.GetInputDescByName("cls_prob").GetDataType();
  ...
  TensorDesc rois_desc = op.GetOutputDescByName("rois");
  rois_desc.SetShape(out_shape_rois);
  rois_desc.SetDataType(input_dtype);

  Shape out_shape_actual_rois_num({batch, 8});
  TensorDesc actual_rois_num_desc = op.GetOutputDescByName("actual_rois_num");
...
IMPLEMT_VERIFIER(PassThrough, PassThroughVerify) {
  ...
  if (inputFormat != FORMAT_NCHW && inputFormat != FORMAT_NHWC) {
    ...
    std::string err_msg =
        GetInputFormatNotSupportErrMsg("inputFormat", expected_format_list, ConcatString(inputFormat));
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
    return GRAPH_FAILED;
  }
...
IMPLEMT_COMMON_INFERFUNC(CropInferShape) {
  ...
  auto outputDesc = op.GetInputDescByName("size");
  ...
  if (inputFormat == FORMAT_NC1HWC0) {
    if (originShape.GetDimNum() == 4) {
      if (originFormat == FORMAT_NCHW) {
        ...
        std::string err_msg =
            GetInputFormatNotSupportErrMsg("5D tensor's origin", expected_format_list, ConcatString(originFormat));
        VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg);
        return GRAPH_FAILED;
      }
    } else {
...
IMPLEMT_INFER_DATA_SLICE(StridedWrite, StridedWriteInferDataSlice) {
  ...
  OP_LOGD(TbeGetName(op).c_str(), "Enter StridedWrite InferDataSlice");
  ...
  for (size_t i = 0; i < y_data_slice.size(); i++) {
    if (y_data_slice[i].size() > 0) {
      ...
      OP_LOGD(TbeGetName(op).c_str(), "Set y_data_slice is on No %d axis", i + 1);
    }
  }
  if (have_slice == false) {
    return GRAPH_FAILED;
  ...
  if (!AttrUtils::SetListListInt(tensor_desc_x, ge::ATTR_NAME_DATA_SLICE, x_data_slice)) {
    OP_LOGD(TbeGetName(op).c_str(), "Set x_data slice failed!");
    return GRAPH_FAILED;
  }
  OP_LOGD(TbeGetName(op).c_str(), "Calc StridedWrite InferDataSlice end!");
  return GRAPH_SUCCESS;
}
...
IMPLEMT_COMMON_INFERFUNC(InferMaskedFillShape) {
  // ge::Operator op;
  OP_LOGD(TbeGetName(op).c_str(), "InferMaskedFillShape Begin.");
  ...
  OP_LOGD(TbeGetName(op).c_str(), "InferMaskedFillShape End.");

  return GRAPH_SUCCESS;
}

IMPLEMT_VERIFIER(MaskedFill, MaskedFillVerify) {
  auto input_type_mask = op.GetInputDescByName("mask").GetDataType();
...
bool InferShapeAndTypeMaskedSelectV2(Operator& op, const string& input_name1,
  ...
  for (size_t i = 0; i < dims_x.size(); i++) {
    if ((dims_x[i] != dims_y[i]) && (dims_x[i] != 1) && (dims_y[i] != 1)) {
      OP_LOGE(TbeGetName(op).c_str(), "The shape of x1 and x2 can not broadcast.");
      return false;
    }
...
IMPLEMT_VERIFIER(MaskedSelectV2, MaskedSelectV2Verify) {
  if (op.GetInputDescByName("x").GetDataType() !=
      op.GetOutputDescByName("y").GetDataType()) {
    OP_LOGE(TbeGetName(op).c_str(), "x y tensor dtype does not match.");
    return GRAPH_FAILED;
  }
  return GRAPH_SUCCESS;
}
...
static graphStatus GetStridedSliceInferConstData(const ge::Operator& op, struct SliceParameters& slice_params,
  ...
  for (auto& item: const_values) {
    ...
    if (slice_desc->MutableInputDesc(item.first) != nullptr && \
        op.GetInputConstData(item.first.c_str(), const_tensor) == GRAPH_SUCCESS) {
      item.second.clear();
      auto dtype = op.GetInputDescByName(item.first.c_str()).GetDataType();
      GetConstValue(op, const_tensor, dtype, item.second);
    } else {
...
IMPLEMT_COMMON_INFERFUNC(StridedSliceV2InferShape) {
  ...
  if (axes_valid) {
    ...
    auto clamp = [=](int64_t x) { return x = x > MAX_INT ? MAX_INT : (x < MIN_INT ? MIN_INT : x); };
    ...
    for (size_t i = 0; i < axes_list.size(); ++i) {
      ...
      if (axes_index < 0) {
        axes_index = 0;
        OP_LOGD(TbeGetName(op).c_str(), "Neg Value Out Of Boudary:%s", to_string(axes_list).c_str());
      }
  ...
  if (dim_num == 0) {
    std::string err_msg = GetParamOutRangeErrMsg("input x's dimnum", ConcatString("[", DIM_SIZE1, ", ", DIM_SIZE8, "]"),
...
bool InferShapeAndTypeSliceLastDim(Operator& op, const string& x_name,
  ...
  DataType input_dtype = op.GetInputDescByName(x_name.c_str()).GetDataType();
  Format input_format = op.GetInputDescByName(x_name.c_str()).GetFormat();

  ge::Shape shape_x = op.GetInputDescByName(x_name.c_str()).GetShape();
  std::vector<int64_t> dims_x = shape_x.GetDims();

  std::vector<int64_t> dims_output;
  int64_t begin, end, stride, length;
  if (GRAPH_SUCCESS != op.GetAttr("start", begin)) {
    OP_LOGE(TbeGetName(op).c_str(), "Failed to get the length");
    return false;
  }
  ...
  if (stride == 0) {
    OP_LOGE(TbeGetName(op).c_str(), "stride[%ld] error.", stride);
    return GRAPH_FAILED;
  }
...
IMPLEMT_COMMON_INFERFUNC(SliceLastDimInferShape) {
  ...
  OP_LOGE(TbeGetName(op).c_str(), "infer shape failed!");
  return GRAPH_FAILED;
}
...
bool InferShapeAndTypeIndexFillD(Operator &op, const string &input_name, const string &output_name)
{
  ...
  ge::Shape shape_input = op.GetInputDescByName(input_name.c_str()).GetShape();
...
// ----------------AddRowRanges Begin-------------------
bool InferShapeAndTypeAddRowRanges(Operator& op, const string& input_name1, const string& output_name) {
  TensorDesc v_output_desc = op.GetOutputDescByName(output_name.c_str());
...
IMPLEMT_VERIFIER(AddRowRanges, AddRowRangesVerify) {
  ...
  DataType src_dtype = op.GetInputDescByName("src").GetDataType();
  DataType x_out_dtype = op.GetOutputDescByName("x").GetDataType();
  ...
  if (indices_dtype != DT_INT32) {
    OP_LOGE(TbeGetName(op).c_str(), "indices dtype is not int32, please check!");
    return GRAPH_FAILED;
  }
  return GRAPH_SUCCESS;
}
...
IMPLEMT_COMMON_INFERFUNC(MaskedFillRangeInferShape)
{
  ...
  auto input_type = op.GetInputDescByName("x").GetDataType();
...
IMPLEMT_VERIFIER(InplaceTopKDistance, InplaceTopKDistanceVerify) {
  std::vector<int64_t> topk_pq_distance_dims = op.GetInputDescByName("topk_pq_distance").GetShape().GetDims();
  std::vector<int64_t> topk_pq_index_dims = op.GetInputDescByName("topk_pq_index").GetShape().GetDims();
  std::vector<int64_t> topk_pq_ivf_dims = op.GetInputDescByName("topk_pq_ivf").GetShape().GetDims();
  ...
  std::vector<int64_t> pq_distance_dims = op.GetInputDescByName("pq_distance").GetShape().GetDims();
  std::vector<int64_t> pq_index_dims = op.GetInputDescByName("pq_index").GetShape().GetDims();
  ...
  OP_LOGE(TbeGetName(op).c_str(), "GRAPH_SUCCESS.");
  return GRAPH_SUCCESS;
}
// Registered inferfunction
...
IMPLEMT_COMMON_INFERFUNC(StridedSliceV3InferShape) {
  ...
  if (axes_valid) {
    ...
    for (size_t i = 0; i < input_axes_values.size(); ++i) {
      ...
      if (axes_index < 0) {
        axes_index = 0;
        OP_LOGD(TbeGetName(op).c_str(), "Neg Value Out Of Boudary:%s", to_string(input_axes_values).c_str());
      }
      axes_mask = (1 << axes_index) | axes_mask;
...
IMPLEMT_COMMON_INFERFUNC(MovingSumWithSigmoidInferShape) {
  OP_LOGD(TbeGetName(op).c_str(), "MovingSumWithSigmoidInferShape start.");
  ...
  if (data != nullptr) {
    ...
    OP_LOGD(TbeGetName(op).c_str(), "failed to get constValue of [offset].");
    std::vector<int64_t> energy_dims = energy_shape.GetDims();
    if (!IsUnknown(energy_dims)) {
      OP_LOGE(TbeGetName(op).c_str(), "static case is not supported.");
      return GRAPH_FAILED;
    }
  ...
  OP_LOGD(TbeGetName(op).c_str(), "MovingSumWithSigmoidInferShape finish.");
  return GRAPH_SUCCESS;
}
...
IMPLEMT_INFERFUNC(DynSeqOuter, DynSeqOuterInferShape) {
  OP_LOGD(TbeGetName(op).c_str(), "DynSeqOuterInferShape start.");
  ...
  if (data1 != nullptr && data2 != nullptr) {
    ...
    OP_LOGD(TbeGetName(op).c_str(), "failed to get constValue of [seq_len1, seq_len2].");
    std::vector<int64_t> energy_dims = energy_shape.GetDims();
    if (!IsUnknown(energy_dims)) {
      OP_LOGE(TbeGetName(op).c_str(), "static case is not supported.");
      return GRAPH_FAILED;
...
IMPLEMT_COMMON_INFERFUNC(NonMaxSuppressionBucketizeInferShape) {
  std::vector<int64_t> boxes_shape = op.GetInputDescByName("input_nmsed_boxes").GetShape().GetDims();
  std::vector<int64_t> score_class_shape = op.GetInputDescByName("input_nmsed_score").GetShape().GetDims();
  ...
  // update output desc
  TensorDesc tensor_desc_boxes = op.GetOutputDescByName("output_nmsed_boxes");
  TensorDesc tensor_desc_score = op.GetOutputDescByName("output_nmsed_score");
  TensorDesc tensor_desc_class = op.GetOutputDescByName("output_nmsed_class");
