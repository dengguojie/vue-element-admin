...
IMPLEMT_COMMON_INFERFUNC(ActULQClampMinGradInferShape) {
    ...
    Shape x_clamped_loss_shape = op.GetInputDescByName("x_clamped_loss").GetShape();
    ...
    TensorDesc clamp_min_grad = op.GetOutputDescByName("clamp_min_grad");
    clamp_min_grad.SetShape({});
    clamp_min_grad.SetDataType(op.GetInputDescByName("y_grad").GetDataType());
    if (op.UpdateOutputDesc("clamp_min_grad", clamp_min_grad) != GRAPH_SUCCESS) {
        OP_LOGE(TbeGetName(op).c_str(), "Update output[clamp_min_grad] failed.");
        return GRAPH_FAILED;
    }
