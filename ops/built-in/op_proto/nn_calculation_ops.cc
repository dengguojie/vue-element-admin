...
IMPLEMT_INFERFUNC(Deconvolution, DeconvolutionInfer) {
  ...
  if (isDynamic) {
    ...
    if (!x_range.empty() && x_range.size() > idxW) {
      y_range[idx].first = (int64_t)kc * groups;
      y_range[idx].second = (int64_t)kc * groups;
      if (ih == -1) {
...
static graphStatus VerifyDeformableOffsetsInput(const ge::Operator& op) {
  ...
  if (x_c % dg != 0) {
    OP_LOGE(op.GetName().c_str(),
            "Input x's channel can not divide deformable_groups");
    return GRAPH_FAILED;
  }
  ...
  if (modulated != true) {
    OP_LOGE(op.GetName().c_str(), "Currently modulated must be true.");
    return GRAPH_FAILED;
  }
