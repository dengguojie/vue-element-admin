...
bool GetInputDataType(const ge::DataType &data_type,
                      const std::vector<ge::DataType> &supportList)
{
  std::vector<ge::DataType>::const_iterator supportIter =
      find(supportList.begin(), supportList.end(), data_type);
...
bool CheckInputDtypeAndShape(const Operator &op,
                             const std::map<std::string, std::vector<DataType> > &inputTensorMap)
{
  ...
  for (; iter != inputTensorMap.end(); ++iter) {
    ...
    if (input_type != first_input_dtype) {
      OP_LOGE(op.GetName().c_str(),
              "the op type of param %s must equal with param %s", iter->first.c_str(), first_name.c_str());
      return false;
    }
    auto dims = input_desc.GetShape().GetDims();
    if (dims != first_shape_dims) {
      OP_LOGE(op.GetName().c_str(),
              "the op shape of param %s must equal with param %s", iter->first.c_str(), first_name.c_str());
      return false;
    }
  }
  return true;
...
bool CheckInputDataType(const Operator& op, const std::string& input_name,
  ...
  if (!valid) {
    OpsInputDtypeErrReport(op.GetName(), input_name, DebugString(support_list), Strcat(input_type));
...
bool CheckTwoInputDtypeSame(const Operator &op, const string &input_name1,
  ...
  if (input_type_x1 != input_type_x2) {
    OpsTwoInputDtypeErrReport(op.GetName(), input_name1, input_name2, Strcat(input_type_x1), Strcat(input_type_x2));
    OP_LOGE(op.GetName().c_str(),
            "The %s op dtype is not same, type1:%s, type2:%s",
             op.GetName().c_str(),
...
bool CheckInputDtypeSame(const Operator &op,
  ...
  for (const string &input_name : input_tensors) {
    ...
    if (input_dtype != first_input_dtype) {
       OP_LOGE(op.GetName().c_str(),
              "the op type of param %s must equal with param %s",
               input_name.c_str(), (*first_name).c_str());
       return false;
    }
  }
  return true;
...
bool CheckInputsShapeDtypeSame(const Operator& op,
  ...
  for (++input_name; input_name != input_names.end(); ++input_name) {
    ...
    input_des.GetShape().GetDims() != first_input_des.GetShape().GetDims()) {
      OpsAttrValueErrReport(op.GetName(), Strcat(input_name->c_str(), "'s dtype and shape"),
      Strcat("same as",  first_input_name->c_str(), "[", first_input_des.GetDataType(),"]","[", DebugString(first_input_des.GetShape().GetDims()),"]"),
      Strcat("[", input_des.GetDataType(),"]","[", DebugString(input_des.GetShape().GetDims()),"]"));
      OP_LOGE(op.GetName().c_str(),
              "the dtype and shape of param %s must be same as param %s",
...
bool InferShapeAndTypeTwoInOneOutBroadcast(Operator &op,
  ...
  if (IsEmptyTensor(dimsX) && !IsEmptyTensor(dimsY)) {
    ...
    OP_LOGE(op.GetName().c_str(),
            "One input is empty tensor and another is not.");
    return false;
  ...
  if (IsUnknownRankShape(dimsX) || IsUnknownRankShape(dimsY)) {
    ...
    OP_LOGI(op.GetName().c_str(),
            ...
            to_string(ge::Shape(dimVec)).c_str(),input_dtype);
  ...
  for (size_t i = 0; i < dimsX.size(); i++) {
    if ((dimsX[i] != dimsY[i]) && (dimsX[i] != 1) && (dimsY[i] != 1) && (dimsX[i] != -1) && (dimsY[i] != -1)) {
      OpsInputShapeBroadcastErrReport(op.GetName(), input_name1, input_name2, Strcat(dimsX[i]), Strcat(dimsY[i]));
      OP_LOGE(op.GetName().c_str(),
  ...
  tensordesc_output->SetDataType(input_dtype);
  OP_LOGI(op.GetName().c_str(),
          "output shape is: %s, output dtype is:%s.",
          to_string(outputShape).c_str(), ge::TypeUtils::DataTypeToSerialString(input_dtype).c_str());

  return true;
}

bool InferShapeRangeTwoInOneOutBroadcase(Operator& op,
                                         const string& input_name1,
                                         const string& input_name2,
                                         const string& output_name) {
  auto op_desc = OpDescUtils::GetOpDescFromOperator(op);
  ...
  if (!IsUnknownRankShape(dims_out)) {
    ...
    while (dims_x.size() > shape_range_y.size()){
      shape_range_y.insert(shape_range_y.begin(),
    ...
    for (size_t i=0; i<size_shape_out; i++) {
      ...
      if (i<shape_range_x.size() && i<shape_range_y.size()) {
        if (shape_range_x[i].second == -1 && shape_range_y[i].second == 1) {
          ...
          out_range.push_back(std::pair<int64_t,int64_t>(
              std::min(shape_range_x[i].first, shape_range_y[i].first),
    ...
    while (shape_range_x.size() < max_size) {
      shape_range_x.insert(shape_range_x.begin(),
          std::pair<int64_t,int64_t>(1, 1));
    }
...
bool GetInputDataType(const ge::DataType& dataType,
                      const std::vector<ge::DataType>& supportList,
                      std::string& dType) {
  std::vector<ge::DataType>::const_iterator supportIter =
    find(supportList.begin(), supportList.end(), dataType);
  if (supportIter == supportList.end()) {
    return false;
  }

  std::map<ge::DataType, std::string>::const_iterator totalIter =
    DTYPE_STR_MAP.find(dataType);
...
bool CheckInputDataType(const Operator& op, std::string* data_type,
                        const std::string& input_name,
                        const std::vector<ge::DataType>& supportList) {
  DataType input_type = op.GetInputDesc(input_name).GetDataType();
  if (false == GetInputDataType(input_type, supportList, *data_type)) {
    LOG_ERROR("[ERROR]op [%s] [%s] do not supported dtype [%s]!\n",
              op.GetName().c_str(), input_name.c_str(), data_type->c_str());
    return false;
  }
  return true;
}

bool GetConstValue(const ge::Operator& op,
                   const std::string& key_name, float& attr_value) {
...
bool GetConstValue(const ge::Operator& op,
                   const std::string& key_name, int64_t& attr_value) {
  if (ge::GRAPH_SUCCESS != op.GetAttr(key_name, attr_value)) {
    LOG_ERROR("[ERROR]op [%s] GetOpAttr [%s] failed!\n",
              op.GetName().c_str(), key_name.c_str());
    return false;
...
bool GetConstValue(const ge::Operator& op,
                   const std::string& key_name, bool& attr_value) {
  if (ge::GRAPH_SUCCESS != op.GetAttr(key_name, attr_value)) {
    LOG_ERROR("[ERROR]op [%s] GetOpAttr [%s] failed!\n",
              op.GetName().c_str(), key_name.c_str());
    return false;
...
bool GetScalerValue(const Operator& op, const Tensor& const_tensor, const DataType& dtype,
                   std::int64_t& const_data) {
  if (dtype == ge::DT_INT32) {
    int32_t* const_data_ptr = (int32_t*) const_tensor.GetData();
...
bool DynamicShapeInfer::CatchFormatAndShape() {
  ...
  for(map<std::string, uint32_t>::iterator it=inputs.begin(); it!=inputs.end(); it++) {
    ...
    map_dtype.insert(std::pair<std::string, DataType>(
        input_name, tensor_desc_input->GetDataType()));
  ...
  for(map<std::string, uint32_t>::iterator it=outputs.begin(); it!=outputs.end(); it++) {
    ...
    map_dtype.insert(std::pair<std::string, DataType>(
        output_name, tensor_desc_output->GetDataType()));
...
bool DynamicShapeInfer::UpdateFormatAndShape() {
  ...
  for(map<std::string, uint32_t>::iterator it=inputs.begin(); it!=inputs.end(); it++) {
    ...
    // print some info
    OP_LOGI(op.GetName().c_str(), "origin input shape %s is %s",
            input_name.c_str(), to_string(ori_infer_shape).c_str());

    ShapeAndFormat shapeAndFormatInfoInput = {
        ori_infer_shape, current_shape, ori_input_format,
    ...
    if (ori_input_format == cur_input_format) {
      ...
      // print some info
      OP_LOGI(op.GetName().c_str(), "current input shape %s is %s",
              input_name.c_str(), to_string(current_shape).c_str());
  ...
  for(map<std::string, uint32_t>::iterator it=outputs.begin(); it!=outputs.end(); it++) {
    ...
    // print some info
    OP_LOGI(op.GetName().c_str(), "origin output shape %s is %s",
            output_name.c_str(), to_string(ori_infer_out_shape).c_str());

    ShapeAndFormat shapeAndFormatInfoOutput = {
        ori_infer_out_shape, current_out_shape, ori_output_format,
    ...
    if (ori_output_format == cur_output_format) {
      ...
      // print some info
      OP_LOGI(op.GetName().c_str(), "current output shape %s is %s",
              output_name.c_str(), to_string(current_out_shape).c_str());
...
bool IsUnknownRankShape(std::vector<int64_t>& shape_vec) {
  if (shape_vec.size() == 1 && shape_vec[0] == -2) {
    return true;
  }
  return false;
...
bool MakeUpShapeRange(std::vector<int64_t>& shape, std::vector<std::pair<int64_t,int64_t>>& range) {
