...
template <typename Ta, typename Tb> bool FastBoundsCheck(const Ta index, const Tb limit)
{
...
static graphStatus GetShapeDataFromConstData(const Tensor &tensor, int64_t rank, std::vector<int64_t> &data, const char* op_name)
{
...
graphStatus Divide(const int64_t dividend, const int64_t divisor, const bool evenlyDivisible,
                   int64_t &out, const char* op_name)
{
    if (divisor == 1) {
        out = dividend;
...
graphStatus ValidateSparseTensor(const TensorDesc &indices, const TensorDesc &values,
                                 const TensorDesc &shape, const char* op_name)
{
    // Validate ranks
    Shape unused_shape;
...
graphStatus ValidateVariableResourceHandle(Operator &op,
