...
namespace ge {
    ...
    IMPLEMT_VERIFIER(Yolo, YoloVerify) {
        ...
        if (boxNum < 1) {
            ...
            string realvalue =  Strcat(boxNum);
        ...
        if (coordNum != 4) {
            ...
            string realvalue =  Strcat(coordNum);
        ...
        if (classNum < 1 || classNum > 1024)
        {
            ...
            string realvalue =  Strcat(classNum);
        ...
        if (boxNum * (1 + coordNum + classNum) != channelNum) {
            OP_LOGE(op.GetName().c_str(),
                    "channels(%d) should be (boxes(%d) * (1 + coords(%d) + classes(%d)))",
                    channelNum, boxNum, coordNum, classNum);
            string realvalue =  Strcat(channelNum);
            OpsAttrValueErrReport(op.GetName().c_str(),
                                  "channels", "must equal with boxes*(1+coords+classes)",
                                  realvalue);
            return GRAPH_FAILED;
        }
    ...
    IMPLEMT_VERIFIER(SPP, SPPVerify) {
        ...
        if (pyramidHeight < 1 || pyramidHeight >= 7) {
            ...
            string realvalue =  Strcat(pyramidHeight);
        ...
        if (pyramidHeight > 1 && (bottomH > 510 || bottomW > 510)) {
            ...
            string realvalue =  Strcat(bottomH);
    ...
    IMPLEMT_VERIFIER(DecodeBbox, DecodeBboxVerify) {
        ...
        if (box_predictions_format != FORMAT_ND &&
            box_predictions_format != FORMAT_NCHW &&
        ...
        if (box_predictions_shape_dimension == 3) {
            ...
            if (box_predictions_shape_D != 4 || anchors_shape_D != 4) {
                OP_LOGE(op.GetName().c_str(),
                "last dimension of box_predictions and anchors should be FOUR");
                return GRAPH_FAILED;
            }
        }
        if (box_predictions_shape_dimension == 4) {
            ...
            if (box_predictions_shape_N != 4 || anchors_shape_N != 4) {
                OP_LOGE(op.GetName().c_str(),
                "first dimension of box_predictions and anchors should be FOUR");
                return GRAPH_FAILED;
            }
        }
    ...
    IMPLEMT_VERIFIER(FastrcnnPredictions, FastrcnnPredictionsVerify) {
        ...
        if (rois_shape_N != num * classes) {
            OP_LOGE(op.GetName().c_str(), "first dimension of rois should be consistent to first dimension mul second dimension of score");
            return GRAPH_FAILED;
        }
        // check attr k
        int64_t k = 0;
...
IMPLEMT_VERIFIER(BatchMultiClassNonMaxSuppression, BatchMultiClassNonMaxSuppressionVerify) {
    ...
    if (inputShape[inputShape.size() - 1] != 4) {
        ...
        OP_LOGE(op.GetName().c_str(), "the last dim of boxes shape must be 4, while is %d", inputShape[inputShape.size() - 1]);
        ErrorManager::GetInstance().ReportErrMessage(report_error_code, err_map);
    ...
    if (inputShape.size() == 4) {
      ...
      if ((boxesClassNum != scoreClassNum) && (boxesClassNum != 1)) {
        OP_LOGE(op.GetName().c_str(), "the ClassNum in input score is %d, while ClassNum in boxes is %d", scoreClassNum, boxesClassNum);
        map<string, string> err_map;
...
// ----------------NormalizeBBox-------------------
COMMON_INFER_FUNC_REG(NormalizeBBox,
                      ELMTWISE_INFER_SHAPEANDTYPE("boxes", "y"));
// ----------------NormalizeBBox-------------------

// ----------------ToAbsoluteBBox-------------------
COMMON_INFER_FUNC_REG(ToAbsoluteBBox,
                      ELMTWISE_INFER_SHAPEANDTYPE("normalized_boxes", "y"));
// ----------------ToAbsoluteBBox-------------------
...
//Registered inferfunction
COMMON_INFER_FUNC_REG(DecodeBboxV2,
                      ELMTWISE_INFER_SHAPEANDTYPE("boxes", "y"));
