...
IMPLEMT_COMMON_INFERFUNC(ReluV2InferShape) {
  ...
  if (dims.size() != 4 ){
    OpsAttrValueErrReport(op.GetName(), "x's origin shape dim", "4", Strcat(dims.size()));
  ...
  if (xDesc.GetOriginFormat() == FORMAT_NHWC) {
    ...
    OpsInputFormatErrReport(op.GetName(), "x", expected_format_list, Strcat(xDesc.GetOriginFormat()));
...
IMPLEMT_COMMON_INFERFUNC(SoftplusGradInferShape) {
  if(InferShapeAndTypeTwoInOneOutBroadcast(op, "gradients",
                                           "features", "backprops")) {
    return GRAPH_SUCCESS;
  }
  return GRAPH_FAILED;
...
//----------------ThresholdGradV2D-------------------
bool InferShapeAndTypeThresholdGradV2D(Operator& op, const string& input_name1, const string& input_name2, const string& output_name) {
  ...
  // ���shapeά�ȴ�С���н�����dimsX��ÿ��ά��ȡ����tensor�ж�Ӧά�ȵĴ�ֵ
  ...
  // ���tensor��shape��ÿһ��ά�ȵ�ֵȡ���������ж�Ӧά�ȵĴ�ֵ
  ...
  op.UpdateOutputDesc(output_name, vOutputDesc);   // �������tensor������Ϣ
