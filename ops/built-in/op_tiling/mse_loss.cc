...
bool MseLossIsInAxis(std::vector<int32_t>& input, int32_t value) {
...
static bool ParseJsonCompileInfo(const std::string& op_type, const nlohmann::json& compile_info,
                                 MseLossCompileInfo& parsed_info) {
...
REGISTER_OP_TILING_V3_CUSTOM(MseLoss, MseLossTiling, ParseJsonCompileInfo, MseLossCompileInfo);
}  // namespace optiling