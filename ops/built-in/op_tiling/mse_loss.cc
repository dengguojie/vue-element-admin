...
bool IsInAxisMseLoss(std::vector<int32_t>& input, int32_t value) {
...
bool MseLossTiling(const std::string& op_type, const TeOpParas& op_paras, const nlohmann::json& op_info,
                   OpRunInfo& run_info) {
  ...
  if (op_info.count("reduce_mean_cof_dtype") > 0) {
    ...
    if (reduce_mean_cof_dtype == "float32") {
      for (uint32_t i = 0; i < input_shape.size(); i++) {
        if (IsInAxisMseLoss(reduce_axis, i)) {
...
REGISTER_OP_TILING_FUNC_BUFFERED(MseLoss, MseLossTiling);
}  // namespace optiling