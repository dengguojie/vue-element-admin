...
std::string CheckTilingInCostModel(const std::string& op_type, const json& compile_info, const BatchmatmulParas& params,
                                   bool isBatchMatmulMode) {
  string tiling_id("-1");
...
void FillRunInfoParas(OpRunInfoParas& runinfoparas, Tiling& tiling) {
  ...
  if (!aL1_shape.empty()) {
    ...
    runinfoparas.m_al1 = std::ceil(static_cast<double>(runinfoparas.params.m / runinfoparas.m_dim) / runinfoparas.m_l0);
  }
  ...
  if (!bL1_shape.empty()) {
    ...
    runinfoparas.n_bl1 = std::ceil(static_cast<double>(runinfoparas.params.n / runinfoparas.n_dim) / runinfoparas.n_l0);
  }
  ...
  runinfoparas.m_single_core =
      std::max(runinfoparas.params.m / (runinfoparas.m_dim * runinfoparas.m_al1 * runinfoparas.m_l0), int64_t(1));
  runinfoparas.n_single_core = std::max(runinfoparas.params.n / (runinfoparas.n_dim * runinfoparas.n_bl1 *
                                                                 runinfoparas.n_ub_l0_time * runinfoparas.cub_n1),
                                        int64_t(1));
}

void SetRunInfo(const bool& isBatchMatmulMode, const std::string& tiling_id, const json& compile_info,
...
std::string GEMMTilingSelect(const std::string& op_type, const ge::Operator& op_paras, const json& compile_info,
  ...
  if (tiling_id == "-1") {
    ...
    if (tiling_id == "-1") {
      GenTiling(runinfoparas.params.batch, runinfoparas.params.m, runinfoparas.params.k, runinfoparas.params.n, tiling,
                tiling_id);
      FillRunInfoParas(runinfoparas, tiling);
      if (tiling_id != "-1") {
        OP_LOGD(op_type.c_str(), "match tiling_id(%s)" in formula tiling model, tiling_id.c_str());
      }
    }
  }
  if (tiling_id != "-1") {
