...
bool GetScatterSubParams(const nlohmann::json &opCompileInfo,
                     uint32_t &coreNum,
                     uint32_t &ubSize) {
    using namespace nlohmann;
    auto allVars = opCompileInfo["vars"];
    if (allVars.count("core_num") == 0) {
        GE_LOGE("op [ScatterSubTiling] : GetCompileParams, get core_num error");
        return false;
    }
    coreNum = allVars["core_num"].get<std::uint32_t>();
    if (allVars.count("ub_size") == 0) {
        GE_LOGE("op [ScatterSubTiling] : GetCompileParams, get ub_size error");
        return false;
    }
...
bool ScatterSubTiling(const std::string &opType, const TeOpParas &opParas, const nlohmann::json &opCompileInfo, OpRunInfo &runInfo)
{
    using namespace nlohmann;
    ...
    uint32_t ubSize;
    uint32_t coreNum;
    bool flag = GetScatterSubParams(opCompileInfo, coreNum, ubSize);
    if (!flag) {
        return false;
    }
