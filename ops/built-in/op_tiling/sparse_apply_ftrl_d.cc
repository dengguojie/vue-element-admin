 ...
 *
 * dynamic SparseApplyFtrl op tiling
 */
#include <string>
...
bool CheckTensorShape(const std::string &opType, const TeOpParas &opParas,
        int32_t &varRowElem) {
    ...
    if (indicesShape[0] != gradShape[0]) {
        ...
        GE_LOGE("op [SparseApplyFtrlTiling] : grad shape[0] must be equal to indices shape[0]");
        return false;
    }
    ...
    for (int32_t i=0; i < varDims; i++) {
        if (varShape[i] != accumShape[i] || varShape[i] != linearShape[i]) {
            ...
            GE_LOGE("op [SparseApplyFtrlTiling] : accum and linear shape must be equal to var shape");
            return false;
        }
        if (i > 0) {
            if (varShape[i] != gradShape[i]) {
                ...
                GE_LOGE("op [SparseApplyFtrlTiling] : grad shape is invalid");
                return false;
            }
...
bool GetCompileParameters(const std::string &opType, const nlohmann::json &opCompileInfoJson,
                          int32_t &coreNum, int32_t &ubSize, int32_t &indicesDSize)
{
    ...
    if (allVars.count("core_num") == 0) {
        ...
        GE_LOGE("op [SparseApplyFtrlTiling] : GetCompileParams, get core_num error");
        return false;
    }
    ...
    if (allVars.count("ub_size") == 0) {
        ...
        GE_LOGE("op [SparseApplyFtrlTiling] : GetCompileParams, get ub_size error");
        return false;
    }
 ...
 */
bool SparseApplyFtrlDTiling(const std::string &opType, const TeOpParas &opParas,
                            const nlohmann::json &op_info, OpRunInfo &runInfo)
{
    GELOGI("op[%s] tiling running.", opType.c_str());
    if (op_info == nullptr) {
        GE_LOGE("op[%s] SparseApplyFtrlTiling: op_info json error.", opType.c_str());
        return false;
    }
    if (opParas.inputs.empty() || opParas.inputs.size() < 5 || opParas.inputs[0].tensor.empty() ||
        ...
        opParas.inputs[3].tensor.empty() || opParas.inputs[4].tensor.empty()) {
        ...
        GE_LOGE("op[%s] SparseApplyFtrlTiling: input shape error.", opType.c_str());
        return false;
    }
    ...
    if (!ret) {
        GE_LOGE("op[%s] SparseApplyFtrlTiling: inputs shape are invalid.", opType.c_str());
        return ret;
    }
    ...
    if (!flag) {
        GE_LOGE("op[%s] SparseApplyFtrlTiling: GetCompileParams error.", opType.c_str());
        return false;
    }
    ...
    if (varRowElem > onePartElem) {
        ...
        GE_LOGE("op[%s] SparseApplyFtrlTiling: inputs var row elements is too large, is not support yet.",
                opType.c_str());
        return false;
    }
    ...
    if (varRowElem < varElemBlock) {
        ...
        ge::OpsOneInputShapeErrReport(opType.c_str(), "var",
            "the one row elements of var is not 32 bytes align");
        GE_LOGE("op[%s] SparseApplyFtrlTiling: inputs var row elements is not 32B aligned.", opType.c_str());
        return false;
    }
    ...
    GELOGD("op [SparseApplyFtrlTiling] : indicesNumEachCore=%d, varRowElem=%d, indicesNums=%d",
           indicesNumEachCore, varRowElem, indicesNums);
