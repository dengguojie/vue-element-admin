...
const std::int32_t DTYPE_UINT1 = 100;
...
const std::int32_t ELEMENT_IN_BLOCK_UINT1 = 256;

const std::int32_t ONLY_CONST_TILING_INDEX = 0;
const std::int32_t IS_SUPPORT_BROADCAST_INDEX = 2;
const std::int32_t USE_SPECIAL_PATTERN_INDEX = 3;
const std::int32_t IS_SUPPORT_ABSORBABLE_BROADCAST_INDEX = 4;
const std::int32_t IS_UNKNOWN_RANK_INDEX = 5;
const std::int32_t HAS_ALL_UNKNOWN_INDEX = 6;

const std::int32_t MAX_UB_INDEX = 2;
const std::int32_t CUR_CORE_INDEX = 0;

const std::int32_t MAX_AVAILABLE_UB_INDEX = 2;
const std::int32_t MAX_AVAILABLE_UB_DB_INDEX = 3;

const std::int32_t NUM_TEN = 10;
const std::int32_t NUM_TWO = 2;
const std::int32_t NUM_ONE_HUNDRED = 100;

const std::int32_t MIN_SPLIT_FACTOR = 2;
const std::int32_t SPLIT_FACTOR_STEP = 2;

const std::int32_t BASE_KEY_NUM = 200000000;
const std::int32_t ORIGINAL_NO_DB_TILING_LEN = 7;

const std::int32_t MIN_BLOCK_CUT_INDEX = 20000;

static constexpr int64_t BLOCK_SIZE = 32;
static constexpr size_t MAX_UNKNOWN_RANK = 8;
static constexpr int64_t DOUBLE_BUFFER_SIZE = 2;
static constexpr int64_t BLOCK_NUM = 8;
static constexpr int64_t MAX_REPEAT_TIMES = 8;
static constexpr int64_t N_LAST_BROADCAST_THRESHOLD = 1024;
static constexpr int64_t LAST_AND_N_LAST_FACTOR = 7;
static constexpr int64_t MAX_PATTERN_DIM = 3;
static constexpr int64_t SPECIAL_BROADCAST_INPUT_NUMS = 2;
static constexpr int64_t BROADCAST_BASE_KEY = 2;
static constexpr int64_t ELEWISE_REPEATE_NUMS = 128;
static constexpr int64_t ELEWISE_UINT1_REPEATE_NUMS = 256;
static constexpr int64_t NONE_BRC_AXIS_OPTIMIZE_BLOCK_NUMS = 3;
...
bool Broadcast::Init() {
  ...
  if (!only_const_tiling) {
    ...
    is_support_absorbable_broadcast_compile = broadcast_compile_info.flag_info_compile[IS_SUPPORT_ABSORBABLE_BROADCAST_INDEX];
...
BroadcastCompileInfo::BroadcastCompileInfo(const std::string& op_type,
                                                   const nlohmann::json& outer_compile_info) {
  ...
  //pure_elewise cases: only_const_tiling && pure_elewise shape, or base_info contains 100 pattern && pure_elewise shape
...
bool Broadcast::BroadcastTiling() {
  ...
  if (broadcast_compile_info.flag_info_compile.size() > flag_info_threshold) {
    is_const = broadcast_compile_info.flag_info_compile[1];
    is_support_broadcast = broadcast_compile_info.flag_info_compile[support_broadcast_index];
...
bool Broadcast::BroadcastTiling(const OpInfo& op_info) {
  ...
  bool is_support_broadcast = true;
  bool use_special_pattern = true;
  if (broadcast_compile_info.flag_info_compile.size() > 3) {
    is_const = broadcast_compile_info.flag_info_compile[1];
    is_support_broadcast = broadcast_compile_info.flag_info_compile[2];
    use_special_pattern = broadcast_compile_info.flag_info_compile[3];
  }
  if (is_const) {
    ret = CalcConstKey(is_support_broadcast);
    ret = ret && WriteConstTiling();
