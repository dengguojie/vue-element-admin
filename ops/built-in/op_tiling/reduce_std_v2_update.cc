...
bool ReduceStdV2UpdateIsInAxis(const std::vector<int32_t>& input, int32_t value) {
...
bool ReduceStdV2UpdateTiling(const std::string& op_type, const ge::Operator& op_paras,
                             const ReduceStdV2UpdateCompileInfo& parsed_info, utils::OpRunInfo& run_info) {
  ...
  for (uint32_t i = 0; i < input_shape.GetDimNum(); i++) {
    if (ReduceStdV2UpdateIsInAxis(reduce_axis, i)) {
...
static bool ParseJsonCompileInfo(const std::string& op_type, const nlohmann::json& compile_info,
                                 ReduceStdV2UpdateCompileInfo& parsed_info) {
  parsed_info.attr_unbiased = true;
  std::string attr_unbiased;
  if (GetCompileValue(compile_info, "attr_unbiased", attr_unbiased)) {
    if (attr_unbiased == "false") {
...
REGISTER_OP_TILING_V3_CUSTOM(ReduceStdV2Update, ReduceStdV2UpdateTiling,
                             ParseJsonCompileInfo, ReduceStdV2UpdateCompileInfo);
}  // namespace optiling