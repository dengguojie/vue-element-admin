...
constexpr int32_t REDUCE_MAX_VAR_NUMS = 40;
...
struct ReduceCompileInfo: AutoTilingCompileInfo {
  public:
    ...
    std::unordered_map<uint64_t, std::vector<int32_t>> reduce_vars;
  ...
  private:
    ...
    bool GetCompileCommonInfo(const char* op_type, const nlohmann::json& json_info);
};
...
class Reduce {
  ...
  private:
    ...
    bool GetVarValue(uint64_t tiling_key);
    void DealDisableFuseAxes(std::vector<int32_t>& pos);
    bool GetOriginInputShape();
  private:
    std::string op_type;
    T* context;
    ...
    std::vector<int64_t> original_input_shape;
    ...
    std::array<uint64_t, REDUCE_MAX_VAR_NUMS> var_value{};
};
