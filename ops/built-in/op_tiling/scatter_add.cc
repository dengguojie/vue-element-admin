/*
ght (C) 2020. Huawei Technologies Co., Ltd. All rights reserved.
...
bool CheckScatterAddTensorShape(std::vector<int64_t> varShape, std::vector<int64_t> indicesShape,
                                std::vector<int64_t> updateShape, std::vector<int64_t> outputShape) {
  if (indicesShape.size() == 0) {
    ge::OpsOneInputShapeErrReport("ScatterAdd", "indices",
          "the size of indices's shape is empty");
    GE_LOGE("op [ScatterAddTiling] : the input of indices's shape is empty");
    return false;
  }
  ...
  if (varDims != outputDims) {
    ge::OpsOneInputShapeErrReport("ScatterAdd", "var",
        "the length of var must be same as the length of output");
    GE_LOGE("op [ScatterAddTiling] : the length of var must be same as the length of output");
    return false;
  }
  for (size_t i = 0; i < varDims; i++) {
    if (varShape[i] != outputShape[i]) {
      ge::OpsOneOutputShapeErrReport("ScatterAdd", "var_out",
          "the var's shape is not equal to var_out's shape");
      GE_LOGE("op [ScatterAddTiling] : the output of shape is invalid");
      return false;
    }
  }
  if (indicesShape[indicesDims - 1] == 0 && indicesDims == 1) {
    ...
    GE_LOGE("op [ScatterAddtiling] : the indices of shape is invalid");
    return false;
  }
  if (indicesDims >= updateDims) {
    ge::OpsOneInputShapeErrReport("ScatterAdd", "indices",
        "the length of indices be more than updates");
    GE_LOGE("op [ScatterAddTiling] : the length of indices must be less than updates");
    return false;
  }
  for (size_t i = 0; i < indicesDims; i++) {
    if (indicesShape[i] != updateShape[i]) {
      ge::OpsOneInputShapeErrReport("ScatterAdd", "update",
          "the indices's shape is not equal to update's shape");
      GE_LOGE("op [ScatterAddTiling] : the input update is invalid");
      return false;
    }
  }
  for (size_t i = 1; i < varDims; i++) {
    if (varShape[i] != updateShape[indicesDims + i -1]) {
      ge::OpsOneInputShapeErrReport("ScatterAdd", "update",
          "update does not satisfy the relation expression with var");
      GE_LOGE("op [ScatterAddTiling] : the input update is invalid");
      return false;
    }
  }
...
bool GetSocParamsOne(const std::string &opType,
    ...
    if (allVars.count("core_num") == 0) {
        ...
        GE_LOGE("op [ScatterAddTiling] : GetCompileParams, get core_num error");
        return false;
    }
    coreNum = allVars["core_num"].get<std::uint32_t>();
    if (allVars.count("ub_size") == 0) {
        ge::OpsGetCompileParamsErrReport(opType.c_str(), "ub_size");
        GE_LOGE("op [ScatterAddTiling] : GetCompileParams, get ub_size error");
        return false;
    }
...
bool ScatterAddTiling(const std::string &opType, const TeOpParas &opParas, const nlohmann::json &opCompileInfo, OpRunInfo &runInfo)
{
    using namespace nlohmann;
    GELOGI("op[%s] ScatterAddTiling running.", opType.c_str());
    if (opCompileInfo == nullptr) {
      GE_LOGE("op[%s] ScatterAddTiling: opCompileInfo json error.", opType.c_str());
      return false;
    }
    ...
    bool ret = CheckScatterAddTensorShape(inputShape0, inputShape1, inputShape2, outputShape);
    if (!ret) {
      GE_LOGE("op[%s] ScatterAddTiling : checktensorshape is failed",  opType.c_str());
      return false;
    }
    ScatterAdd params;
    ...
    if(params.indices_ub_number >= BYTE_BLOCK) {
        ...
        if (params.final_one_core_data_num <= MAX_UB_CORE_INDICES) {
            params.select_mode = TILING_MODE_1;
            params.select_params = SELECT_LESS_THAN_PARAMS;
            params.updates_burst_len = ceil(float(params.final_one_core_data_num * params.updates_data_num) / params.block_number);
            params.tail_updates_burst_len = ceil(float(params.last_core_data_num * params.updates_data_num) / params.block_number);
