...
namespace optiling {
  ...
  bool SliceDsl::DoBaseTiling() {
    ...
    OP_LOGD(op_type.c_str(),
            ...
            slice_compile_info.coex_list[DEPAD_MODE_IDX], slice_compile_info.coex_list[STRIDE_ALIGN_MODE_IDX], ub_available);
  ...
  bool SliceDsl::IsLRDePadTiling() {
    return (shape_len == TWO_DIMS && size_list[0] == x_shape[0] && begin_list[1] == 0 && IsDePadTiling(x_last_dim_align));
  }
  ...
  bool SliceDsl::TryImproveTiling() {
    ...
    if ((ub_axis == (shape_len - TWO_DIMS)) && (ub_factor > IMPROVE_TILING_THRESHOLD * slice_compile_info.x_align) && !(ub_axis == block_axis && ub_factor == block_factor)) {
