...
struct ResizeBilinearV2TilingParams {
    int64_t tiling_key;
    int64_t input_batch;
    int64_t input_c1;
...
bool GetResizeBilinearV2CompileParams(const nlohmann::json& compile_info,
                                             int64_t& core_num,
                                             int64_t& max_w_len,
  ...
  if (allVars.count("core_num") == 0) {
    OP_LOGE("op [ResizeBilinearV2Tiling] : GetCompileParams, get core_num error");
    return false;
  }
  core_num = allVars["core_num"].get<std::int64_t>();
  if (allVars.count("max_w_len") == 0) {
    OP_LOGE("op [ResizeBilinearV2Tiling] : GetCompileParams, get max_w_len error");
    return false;
  }
  max_w_len = allVars["max_w_len"].get<std::int64_t>();
  if (allVars.count("align_corners") == 0) {
    OP_LOGE("op [ResizeBilinearV2Tiling] : GetCompileParams, get align_corners error");
    return false;
  }
  align_corners = allVars["align_corners"].get<std::int64_t>();
  if (allVars.count("half_pixel_centers") == 0) {
    OP_LOGE("op [ResizeBilinearTiling] : GetCompileParams, get half_pixel_centers error");
    return false;
  }
...
bool ResizeBilinearV2Tiling(const std::string& op_type,
  ...
  ResizeBilinearV2TilingParams tiling_params;
  int64_t core_num = 0;
  int64_t max_w_len = 0;
  int64_t align_corners = 0;
  int64_t half_pixel_centers = 0;
  if (!GetResizeBilinearV2CompileParams(op_info, core_num, max_w_len,
  ...
  // calcu tiling
  // calcu output_height//input_height and output_weight//input_weight
  int height_ratio = static_cast<float>(tiling_params.input_height) / tiling_params.output_height;
  int weight_ratio = static_cast<float>(tiling_params.input_weight) / tiling_params.output_weight;
  if (height_ratio <= 2 && weight_ratio <= 2) {
      tiling_params.tiling_key = 1;
  } else if (height_ratio <= 2 && weight_ratio > 2) {
  ...
  auto h_max_temp = (tiling_params.output_height + core_num - 1) / core_num;
  auto h_max = (tiling_params.output_height + h_max_temp - 1) / h_max_temp;
  auto w_max_temp = (tiling_params.output_weight + core_num - 1) / core_num;
  auto w_max = (tiling_params.output_weight + w_max_temp - 1) / w_max_temp;
  ...
  int64_t cut_height_num_temp = min(left_core_num, h_max);
  tiling_params.cut_height_num = cut_height_num_temp != 0 ? cut_height_num_temp : 1;
...
REGISTER_OP_TILING_FUNC_BUFFERED(ResizeBilinearV2, ResizeBilinearV2Tiling);
}  // namespace optiling
