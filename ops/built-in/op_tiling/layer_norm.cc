...
namespace optiling
{
    ...
    int32_t CalcPatternKey(std::vector<int64_t> input, std::vector<int32_t> reduce_axis, int32_t block_split_axis,
                           int32_t ub_split_axis_index_reduce, int32_t ub_split_axis, bool is_normal, bool is_fuse_axis)
    {
        int32_t pattern = 0;
                          ...
                          std::vector<int32_t> reduce_axis)
    {
        ...
        int32_t pattern = CalcPatternKey(input_x, reduce_axis, block_split_axis, ub_split_axis_index_reduce,
                                         ub_split_axis, is_normal, is_fuse_axis);
    ...
    std::vector<int32_t> GetUbTilingData(int32_t block_inner, size_t i, std::vector<int64_t> input_x,
                                         int32_t max_ub_size, std::vector<int32_t> reduce_axis, int32_t block)
    {
        int32_t x_size = 1;
        ...
        for (size_t j = input_x.size() - 1; j > i; j--)
        {
            ...
            if (x_size > max_ub_size)
            {
                ...
                if ((ub_factor * ub_mul_num / axis_num < block) && !IsInVector(reduce_axis, j))
                {
                    ub_tiling_axis = j + 1;
    ...
    int32_t GetUnblockAxisOutputMul(int32_t block_axis, std::vector<int64_t> input_x, std::vector<int32_t> reduce_axis)
    {
        int32_t mul_num = 1;
    ...
    std::vector<int32_t> GetBlockTilingData(int32_t block_axis, int32_t block_factor, int32_t ub_tiling_axis,
                                            int32_t ub_outer, std::vector<int64_t> input_x,
        ...
        if (input_x.size() == reduce_axis.size())
        {
            ...
            if (isworkspace && isexceedub)
            {
                ...
                std::vector<int32_t> ubtilingdata =
                    GetUbTilingData(input_x[0], 0, input_x, max_ub_size, reduce_axis, block);
            ...
            else
            {
                if (input_x[i] > core_num)
                {
                    ...
                    int32_t block_inner_core =
                        input_x[i] % core_num == 0 ? input_x[i] / core_num : (input_x[i] + core_num + 1) / core_num;
                    ...
                    if (isworkspace && isexceedub)
                    {
                        ...
                        // not open and open multi-core block
                        std::vector<int32_t> ubtilingdata =
                            GetUbTilingData(block_inner, i, input_x, max_ub_size, reduce_axis, block);
                        tilingparams.ub_tiling_axis = ubtilingdata[0];
                        tilingparams.ub_factor = ubtilingdata[1];
                        int32_t ub_tiling_axis_reduce = GetUbReduceAxis(input_x, max_ub_size, reduce_axis);
                        ...
                        else
                        {
                            ...
                            else if (tilingparams.ub_tiling_axis != i)
                            {
                                std::vector<int32_t> blocktilingdata = GetBlockTilingData(
                                    i, block_inner, ubtilingdata[0], ubtilingdata[4], input_x, reduce_axis, core_num);
                ...
                else
                {
                    ...
                    if (isworkspace && isexceedub)
                    {
                        ...
                        // not open and open multi-core block
                        std::vector<int32_t> ubtilingdata =
                            GetUbTilingData(block, i, input_x, max_ub_size, reduce_axis, block);
                        tilingparams.ub_tiling_axis = ubtilingdata[0];
                        tilingparams.ub_factor = ubtilingdata[1];
                        int32_t ub_tiling_axis_reduce = GetUbReduceAxis(input_x, max_ub_size, reduce_axis);
                        tilingparams.ub_tiling_axis_reduce = ub_tiling_axis_reduce;
