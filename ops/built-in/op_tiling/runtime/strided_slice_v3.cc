...
static int64_t GetConstIndexValue(const gert::Tensor* tensor, int32_t idx, int64_t input_size) {
  // idx must be valid
  int64_t value = 0;
  if (tensor->GetDataType() == ge::DT_INT32) {
    const int32_t* data = tensor->GetData<int32_t>();
...
static void ConstructStrideList(const gert::Tensor* stride_tensor, int64_t dim_num, const std::vector<int64_t>& axes,
  ...
  // init stride_vec with 1
  stride_vec.assign(dim_num, 1);
  if (!stride_tensor) {
    OP_LOGD(OP_NAME.c_str(), "Stride tensor is null. Set stride as 1.");
    return;
  }

  // update stride_vec with const value of stride_tensor
  const int32_t stride_size = static_cast<int32_t>(stride_tensor->GetShapeSize());
  const int32_t axes_size = static_cast<int32_t>(axes.size());
  for (int32_t i = 0; i < axes_size && i < stride_size; i++) {
    int64_t axes_value = axes[i];
    stride_vec[axes_value] = GetConstIndexValue(stride_tensor, i);
  }
  return;
}
...
static void ConstructBeginList(const gert::Tensor* begin_tensor, const gert::Shape& x_shape,
  ...
  // init begin_vec with 0
  const int32_t dim_num = static_cast<int32_t>(x_shape.GetDimNum());
  begin_vec.assign(dim_num, 0);

  // update begin_vec with const value of begin_tensor
  const int32_t begins_size = static_cast<int32_t>(begin_tensor->GetShapeSize());
  const int32_t axes_size = static_cast<int32_t>(axes.size());
  for (int32_t i = 0; i < axes_size && i < begins_size; i++) {
    int64_t axes_value = axes[i];
    begin_vec[axes_value] = GetConstIndexValue(begin_tensor, i, x_shape.GetDim(axes_value));
  }
  return;
}
...
static void ConstructEndList(const gert::Tensor* end_tensor, const gert::Shape& x_shape,
  ...
  // init end_vec with input_shape
  const int32_t dim_num = static_cast<int32_t>(x_shape.GetDimNum());
  end_vec.resize(dim_num);
  for (int32_t i = 0; i < dim_num; i++) {
    end_vec[i] = x_shape.GetDim(i);
  }

  // update end_vec with const value of end_tensor
  const int32_t end_size = static_cast<int32_t>(end_tensor->GetShapeSize());
  const int32_t axes_size = static_cast<int32_t>(axes.size());
  for (int32_t i = 0;  i < axes_size && i < end_size; i++) {
    int64_t axes_value = axes[i];
    end_vec[axes_value] = GetConstIndexValue(end_tensor, i, x_shape.GetDim(axes_value));
  }
  return;
}
