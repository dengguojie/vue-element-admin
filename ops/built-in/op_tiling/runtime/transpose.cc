...
static RuntimeInfoV2 runtimeInfoList[MAX_INFO_NUM];
...
static RuntimeInfoV2& AcquireRuntimeInfoV2(int64_t id) {
...
static vector<vector<int64_t>> InitSpecificShape() {
  ...
  ADD_SPECIFIC(PTA({5,        4,   2,            1000, 5, 64, 64, -1, -1, -1, -1,   0, 2, 1, 3, -1, -1, -1, -1}));
...
static bool Is32BAligned(const RuntimeInfoV2& ri, const ShapeInfo& shapeInfo) {
  const vector<int64_t>& reducedOutShape = shapeInfo.reducedOutShape;
  int64_t dim = shapeInfo.dim;
  return reducedOutShape[dim - 1] % ElementNumPerBlock(ri.dType) == 0;
}
...
static bool IsStrideTooHuge(const ShapeInfo& shapeInfo, const RuntimeInfoV2& runtimeInfo) {
...
static bool IsOverSize(const TransposeCompilerInfo& compilerInfo,
                       const ShapeInfo& shapeInfo,
                       const RuntimeInfoV2& ri,
                       int64_t repeatAxis) {
  return shapeInfo.lastAxisBurstLen * repeatAxis > ri.ubSizeCouldUse;
}

static bool IsCouldUseFullCoreDst(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo) {
  int64_t vol = 1;
...
static bool IsCouldUseFullCoreSrc(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo) {
  int64_t vol = 1;
  for (int64_t i = 0; i < shapeInfo.dim - 2; i++) {
    vol *= shapeInfo.reducedInShape[i];
  }
...
static bool IsLastTwoAlignedAndTrans(const TransposeCompilerInfo& ci,
                                     const ShapeInfo& shapeInfo,
                                     const RuntimeInfoV2& ri) {
  int dim = shapeInfo.dim;
  if (dim < 3) {
  ...
  if (shapeInfo.reducedInShape[dim - 1] * shapeInfo.reducedInShape[dim - 2] * ri.fp16Times >=
...
template<typename T>
static bool GetPerm(TilingContext* context, const gert::Tensor* permTensor, vector<int64_t>& perm) {
  const T* permValue = permTensor->GetData<T>();
  if (!permValue) {
    VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "Perm GetData is nullptr");
    return false;
  }
  for (int64_t i = 0; i < permTensor->GetShapeSize(); i++) {
    perm[i] = permValue[i];
  }
  return true;
}

static bool GetShapePerm(TilingContext* context,
                         const TransposeCompilerInfo& ci,
                         ShapeInfo& shapeInfo,
                         RuntimeInfoV2& ri) {
  OP_LOGD(context->GetNodeName(), "Entering GetShapePerm.");
  ...
  OPS_CHECK_NULL_WITH_CONTEXT(context, permTensor)
  ...
  shapeInfo.permSize = permTensor->GetShapeSize();
  if (shapeInfo.permSize > TRANSPOSE_MAX_AXIS_NUM) {
    VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "Invalid axis num %ld", shapeInfo.permSize);
  }
  
  if (permDtype == ge::DT_INT32) {
    if (!GetPerm<int32_t>(context, permTensor, shapeInfo.perm)) {
      return false;
    }
  } else if (permDtype == ge::DT_INT64) {
    if (!GetPerm<int64_t>(context, permTensor, shapeInfo.perm)) {
      return false;
    }
  } else {
    VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "Invalid dtype");
    return false;
  }

  auto xDesc = context->GetInputDesc(0);
  OPS_CHECK_NULL_WITH_CONTEXT(context, xDesc);
  ri.dType = xDesc->GetDataType();
  ri.fp16Times = (GetSizeByDataType(ri.dType) + 1) / 2;  // add 1 for int8
  ri.ubSizeCouldUse = ci.ubSize - UB_RESERVED_BLOCK_SIZE;
  OP_LOGD(context->GetNodeName(), "GetCompileParams, coreNum[%ld], ubSize[%ld] blocks, dType[%s].",
          ci.coreNum, ci.ubSize, to_string(ri.dType).c_str());

  shapeInfo.inShapeSize = (int64_t)inShape->GetStorageShape().GetDimNum();
  shapeInfo.outShapeSize = (int64_t)outShape->GetStorageShape().GetDimNum();
  ...
  if (shapeInfo.inShapeSize > TRANSPOSE_MAX_AXIS_NUM || shapeInfo.outShapeSize > TRANSPOSE_MAX_AXIS_NUM) {
    VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "Invalid axis num, %ld, %ld",
                                    shapeInfo.inShapeSize, shapeInfo.outShapeSize);
    return false;
  }

  for (int64_t i = 0; i < shapeInfo.inShapeSize; i++) {
    shapeInfo.inShape[i] = inShape->GetStorageShape().GetDim(i);
  }
  for (int64_t i = 0; i < shapeInfo.outShapeSize; i++) {
    shapeInfo.outShape[i] = outShape->GetStorageShape().GetDim(i);
  }
  OP_LOGI(context->GetNodeName(), "input_shape: [%s], output_shape: [%s], perm: [%s].",
          vec_to_string(shapeInfo.inShape, shapeInfo.inShapeSize).c_str(),
          vec_to_string(shapeInfo.outShape, shapeInfo.outShapeSize).c_str(),
          vec_to_string(shapeInfo.perm, shapeInfo.permSize).c_str());

  return true;
}

/*
static bool AddShapePerm(const string& opType, const ge::Operator& paras,
                         const TransposeCompilerInfo& info, ShapeInfo& shapeInfo, RuntimeInfoV2& ri) {
  ...
  auto dType = input_desc->GetDataType();
  ri.fp16Times = (GetSizeByDataType(dType) + 1) / 2;  // add 1 for int8
  OP_LOGD(opType.c_str(), "GetCompileParams, coreNum[%d], ubSize[%d] blocks, dType[%s].", info.coreNum, info.ubSize,
          to_string(dType).c_str());
...
static bool SetElePerBlock(const RuntimeInfoV2& ri, ShapeInfo& si) {
  si.elePerBlock = ElementNumPerBlock(ri.dType);
  si.eleLenInBytes = GetSizeByDataType(ri.dType);
  return true;
}
...
void UpdateCoreNum(const TransposeCompilerInfo& compilerInfo,
                   const ShapeInfo& shapeInfo,
                   RuntimeInfoV2& ri) {
  if (shapeInfo.totalVolumeActual >= shapeInfo.elePerBlock * compilerInfo.coreNum) {
    ri.usedCoreNum = compilerInfo.coreNum;
    return;
  }
  if (shapeInfo.totalVolumeActual < shapeInfo.elePerBlock) {
    ri.usedCoreNum = 1;
  } else {
    ri.usedCoreNum = shapeInfo.totalVolumeActual / shapeInfo.elePerBlock;
  }
}
...
static bool IsScenario9(const TransposeCompilerInfo& compilerInfo, ShapeInfo& shapeInfo, const RuntimeInfoV2& ri) {
  if (shapeInfo.dim < 3) {
    return false;
  }
  if ((shapeInfo.lastAxisLen % shapeInfo.elePerBlock) != 0) {
  ...
  do {
    ...
    if (IsOverSize(compilerInfo, shapeInfo, ri, repeatAxisDst)) {
      break;
    }
    if (repeatAxisDst > NBURST_BOUNDARY) {
      break;
    }
    if (repeatAxisDst * shapeInfo.lastAxisBurstLen > ri.ubSizeCouldUse) {
      break;
    }
    if (repeatAxisDst * shapeInfo.lastAxisBurstLen < 32) {
  ...
  do {
    ...
    if (IsOverSize(compilerInfo, shapeInfo, ri, repeatAxisSrc)) {
      break;
    }
    if (repeatAxisSrc > NBURST_BOUNDARY) {
      break;
    }
    if (repeatAxisSrc * shapeInfo.lastAxisBurstLen > ri.ubSizeCouldUse) {
      break;
    }
    if (repeatAxisSrc * shapeInfo.lastAxisBurstLen < 32) {
      // By verification, borrow axis is better than using repeat
      break;
    }
    srcMode = true;
...
static bool IsScenario11(const ShapeInfo& shapeInfo) {
  ...
  if ((shapeInfo.elePerBlock != EPB16) && (shapeInfo.elePerBlock != EPB8) && (shapeInfo.elePerBlock != EPB32)) {
    return false;
  }
...
static bool IsScenario12(const TransposeCompilerInfo& compilerInfo, ShapeInfo& shapeInfo) {
  if (shapeInfo.dim < 3) {
    return false;
  }
  if ((shapeInfo.lastAxisLen % shapeInfo.elePerBlock) != 0) {
...
static void SetScenario(const string& opType,
                        const TransposeCompilerInfo& compilerInfo,
                        ShapeInfo& shapeInfo,
                        RuntimeInfoV2& ri) {
  ...
  if (IsLastTwoAlignedAndTrans(compilerInfo, shapeInfo, ri)) {
  ...
  if (IsIdentical(shapeInfo)) {
    ...
    Reshape(shapeInfo);
    UpdateCoreNum(compilerInfo, shapeInfo, ri);
  } else if (IsScenario9(compilerInfo, shapeInfo, ri)) {
    shapeInfo.scenario = SCENARIO_9;
  } else if (IsScenario12(compilerInfo, shapeInfo)) {
    shapeInfo.scenario = SCENARIO_12;
  ...
  OP_LOGD(opType.c_str(), "Leaving SetScenario, scenario = %d", shapeInfo.scenario);
  return;
}
 ...
 */
void  ReduceAxisV2(const char* opType,
                   const TransposeCompilerInfo& compilerInfo,
                   ShapeInfo& shapeInfo,
                   RuntimeInfoV2& ri) {
  OP_LOGD(opType, "Entering ReduceAxisV2.");

  SetElePerBlock(ri, shapeInfo);
  ...
  SetScenario(opType, compilerInfo, shapeInfo, ri);
  return;
}

static void CalcUbReorderFactor(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                                RuntimeInfoV2& runtimeInfo) {
  if (Is32BAligned(runtimeInfo, shapeInfo)) {
...
static void PrintTransposeCompilerInfo(const TransposeCompilerInfo& ci, const RuntimeInfoV2& ri, string& logStr) {
  logStr += "coreNum    usedCoreNum    ubSize    ubSizeCouldUse\n";
  logStr += "--------------------------------------------------\n";
  logStr += to_string(ci.coreNum, 11);
  logStr += to_string(ri.usedCoreNum, 15);
  logStr += to_string(ci.ubSize, 10);
  logStr += to_string(ri.ubSizeCouldUse, 14);
  logStr += "\n\n";
}

static string PrintScenario0(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  PrintShapeInfo(shapeInfo, logStr);
  PrintTransposeCompilerInfo(compilerInfo, runtimeInfo, logStr);
...
static string PrintScenario1(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  PrintShapeInfo(shapeInfo, logStr);
  PrintTransposeCompilerInfo(compilerInfo, runtimeInfo, logStr);
...
static string PrintScenario2(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  PrintShapeInfo(shapeInfo, logStr);
  PrintTransposeCompilerInfo(compilerInfo, runtimeInfo, logStr);
...
static string PrintScenario3(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  PrintShapeInfo(shapeInfo, logStr);
  PrintTransposeCompilerInfo(compilerInfo, runtimeInfo, logStr);
...
static string PrintScenario4(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  string logStr;
...
static string PrintScenario5(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  const BorrowInfo& borrowInfo = runtimeInfo.borrowInfo;
  PrintShapeInfo(shapeInfo, logStr);
...
static string PrintScenario6(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  return PrintScenario2(compilerInfo, shapeInfo, runtimeInfo);
}

static string PrintScenario7(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  PrintShapeInfo(shapeInfo, logStr);
  PrintTransposeCompilerInfo(compilerInfo, runtimeInfo, logStr);
...
static string PrintScenario9(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                             const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  PrintShapeInfo(shapeInfo, logStr);
  PrintTransposeCompilerInfo(compilerInfo, runtimeInfo, logStr);
...
static string PrintScenario11(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                              const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  PrintShapeInfo(shapeInfo, logStr);
  PrintTransposeCompilerInfo(compilerInfo, runtimeInfo, logStr);
...
static string PrintScenario12(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                              const RuntimeInfoV2& runtimeInfo) {
  string logStr;
  PrintShapeInfo(shapeInfo, logStr);
  PrintTransposeCompilerInfo(compilerInfo, runtimeInfo, logStr);
  ...
  logStr += "-----------------------------------------------------------------------------------------------------\n";
  logStr += to_string((int64_t)shapeInfo.mteMode, 9);
  logStr += arr_to_string(runtimeInfo.srcJumpStride, shapeInfo.dim - 1, 28);
  logStr += arr_to_string(runtimeInfo.dstJumpStride, shapeInfo.dim - 1, 28);
  logStr += arr_to_string(runtimeInfo.srcJumpFactor, shapeInfo.dim - 1, 28);
  logStr += arr_to_string(runtimeInfo.dstJumpFactor, shapeInfo.dim - 1, 28);
  logStr += arr_to_string(runtimeInfo.srcJumpFactorMod, shapeInfo.dim - 1, 28);
  logStr += arr_to_string(runtimeInfo.dstJumpFactorMod, shapeInfo.dim - 1, 28);
  logStr += to_string(runtimeInfo.srcStride, 11);
  logStr += to_string(runtimeInfo.dstStride, 11);
  logStr += "\n\n";

  logStr += "loopUnit        loopMain        loopTail        loopSrcStride   loopDstStride\n";
  logStr += "---------------------------------------------------------------------------------\n";
  logStr += to_string((int64_t)runtimeInfo.loopUnit, 16);
  logStr += to_string((int64_t)runtimeInfo.loopMain, 16);
  logStr += to_string((int64_t)runtimeInfo.loopTail, 16);
  logStr += to_string((int64_t)runtimeInfo.loopSrcStride, 16);
  logStr += to_string((int64_t)runtimeInfo.loopDstStride, 16);
  logStr += "\n\n";
...
static void CalcTuple(const TransposeCompilerInfo& compilerInfo,
                      const ShapeInfo& shapeInfo,
                      RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t vol = 1;
  ...
  if (shapeInfo.scenario == SCENARIO_6) {
    SplitEvenly(runtimeInfo.usedCoreNum, vol, v1Num, v2Num, v1, v2);
  } else {
...
static void CalcTupleDstS9(const TransposeCompilerInfo& compilerInfo,
                           const ShapeInfo& shapeInfo,
                           RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t vol = 1;
...
static void CalcTupleSrcS9(const TransposeCompilerInfo& compilerInfo,
                           const ShapeInfo& shapeInfo,
                           RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t vol = 1;
...
static void CalcTupleDstS12(const TransposeCompilerInfo& compilerInfo,
                            const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t vol = 1;
  int64_t v1 = 0;
  int64_t v2 = 0;
  ...
  for (i = 0; i < v1Num; i++) {
    ...
    infoPerCore.initTuple[j] = (vol / runtimeInfo.dstJumpFactorMod[j]) % runtimeInfo.loopMain;
    vol += v1;
  }
  for (i = 0; i < v2Num; i++) {
    InfoPerCoreLastAxisNT& infoPerCore = runtimeInfo.infoPerCoreLastAxisNT[v1Num + i];
  ...
  for (i = v1Num + v2Num; i < compilerInfo.coreNum; i++) {
    InfoPerCoreLastAxisNT infoPerCore;
    runtimeInfo.infoPerCoreLastAxisNT[v1Num + v2Num + i] = infoPerCore;
  }
}

static void CalcTupleSrcS12(const TransposeCompilerInfo& compilerInfo,
                            const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t vol = 1;
  int64_t v1 = 0;
  ...
  for (i = 0; i < v1Num; i++) {
    ...
    infoPerCore.initTuple[j] = (vol / runtimeInfo.srcJumpFactorMod[j]) % runtimeInfo.loopMain;
    vol += v1;
  }
...
static void CalcHugeInfo(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
...
static void CalcLoopInfo(const TransposeCompilerInfo& compilerInfo,
                         const ShapeInfo& shapeInfo,
                         RuntimeInfoV2& runtimeInfo) {
...
static void CalcMte2LoopNum(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri) {
  if (shapeInfo.lastAxisBurstLen > ri.ubSizeCouldUse) {
    return;
  }
  for (int64_t i = 0; i < compilerInfo.coreNum; i++) {
    InfoPerCoreLastAxisNT& info = ri.infoPerCoreLastAxisNT[i];

    if (info.num < 1) {
      continue;
    }

    info.aggregateLoopUnit = ri.ubSizeCouldUse / shapeInfo.lastAxisBurstLen;
...
bool TilingDataScenario0(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri) {
  ...
  IdenticalInfo* identicalInfo = ri.infoPerCoreIdentical;
...
bool TilingDataScenario1(const TransposeCompilerInfo& compilerInfo,
                         const ShapeInfo& shapeInfo,
                         RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t index = 0;
...
static void CalcBackNum(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
...
bool TilingDataScenario2(const TransposeCompilerInfo& compilerInfo,
                         const ShapeInfo& shapeInfo,
                         RuntimeInfoV2& runtimeInfo) {
...
bool TilingDataScenario3(const TransposeCompilerInfo& compilerInfo,
                         const ShapeInfo& shapeInfo,
                         RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t index = 0;
...
static void CalcStrideS4(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
...
static void CalcSrcDstPerm(RuntimeInfoV2& runtimeInfo) {
...
static void SplitCore(const TransposeCompilerInfo& compilerInfo,
                      const ShapeInfo& shapeInfo,
                      RuntimeInfoV2& runtimeInfo) {
  int64_t sum = 1;
...
static void MergeDupAxis(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  BorrowInfo& borrowInfo = runtimeInfo.borrowInfo;
  for (int64_t i = 0; i < borrowInfo.srcNum; i++) {
...
static void CalcLeftVol(const TransposeCompilerInfo& ci, const ShapeInfo& si, RuntimeInfoV2& ri, int64_t& ubSize) {
  bool lastAxisTrans = si.isLastAxisTranspose;
  int64_t reservedVol = si.lastAxisLen * ri.fp16Times;
  ...
  if (si.alignElement == 0 && (!lastAxisTrans)) {
    ...
    if (si.isLastTwoAlignedAndTrans) {
      ...
      reservedVol = si.reducedInShape[si.dim - 2] * si.reducedInShape[si.dim - 1] * ri.fp16Times;
    } else {
      ubSize = CalcVnchwconvPartialUbSize(ci.ubSize);
      if (lastAxisTrans) {
        reservedVol = ri.fp16Times;
      }
    }
  }
  ...
  if (si.isLastTwoAlignedAndTrans) {
    ...
    ri.borrowInfo.srcVol = vol;
    ri.borrowInfo.dstVol = vol;
    return;
  }
  ...
  for (int i = 0; i < LOOP_FOR_UB_PADDING; i++) {
    ...
    if (vol * AlignX(vol * (lastAxisTrans ? 1 : si.lastAxisLen), si.elePerBlock) * ri.fp16Times <= ubSize) {
      ri.borrowInfo.srcVol = vol - 1;
      ri.borrowInfo.dstVol = vol - 1;
      ri.borrowInfo.srcVol = ri.borrowInfo.srcVol > 0 ? ri.borrowInfo.srcVol : 1;
      ri.borrowInfo.dstVol = ri.borrowInfo.dstVol > 0 ? ri.borrowInfo.dstVol : 1;
      break;
    } else {
      continue;
    }
...
static void MakeSrcIndexAsInShape(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
...
static void MakeSrcIndexAsOutShape(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  BorrowInfo& borrowInfo = runtimeInfo.borrowInfo;
  int k = 0;
...
static void MakeDstIndexAsInShape(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  IndexInfo indexInfo[BORROW_MAX_AXIS_NUM_LT];
  BorrowInfo& borrowInfo = runtimeInfo.borrowInfo;
  int k = 0;
  for (int64_t i = 0; i < shapeInfo.dim; i++) {
...
static void MakeDstIndexAsOutShape(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  BorrowInfo& borrowInfo = runtimeInfo.borrowInfo;
  int k = 0;
  for (int64_t i = shapeInfo.dim - 1; i >= 0; i--) {
    for (int64_t j = 0; j < borrowInfo.dstNum; j++) {
 ...
 */
static void MakeDiscreteBeContiguous(RuntimeInfoV2& runtimeInfo) {
  const BorrowInfo& borrowInfo = runtimeInfo.borrowInfo;
...
static void CalcPermInUb(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  BorrowInfo& borrowInfo = runtimeInfo.borrowInfo;
  for (int64_t i = 0; i < shapeInfo.dim; i++) {
    bool exist = false;
...
static int64_t GetDupAxisInSrc(const RuntimeInfoV2& runtimeInfo, int64_t index) {
...
static int64_t GetDupAxisInDst(const RuntimeInfoV2& runtimeInfo, int64_t index) {
  const BorrowInfo& bi = runtimeInfo.borrowInfo;
  for (int64_t i = 0; i < bi.dstNum; i++) {
...
static void CalcSrcBorrowAxisIndex(const ShapeInfo& si, RuntimeInfoV2& runtimeInfo, int borrowSrcAxisNum) {
  int64_t dim = si.dim;
  int64_t borrowed = 1;
...
static bool IsSrcAxisFullyUsed(const ShapeInfo& si, const RuntimeInfoV2& ri) {
...
static bool IsSrcAxisInDstFullyUsed(const ShapeInfo& si, RuntimeInfoV2& runtimeInfo, int64_t axis) {
  const BorrowInfo& bi = runtimeInfo.borrowInfo;
  for (int64_t i = 0; i < bi.dstNum; i++) {
 ...
 */
static void ExtendSrcAxisIndex(const ShapeInfo& si, RuntimeInfoV2& ri, int borrowSrcAxisNum) {
...
static void CalcBorrowLoop(RuntimeInfoV2& runtimeInfo) {
  BorrowInfo& bi = runtimeInfo.borrowInfo;
...
static void CalcBorrowLoopS5(RuntimeInfoV2& runtimeInfo) {
  BorrowInfo& bi = runtimeInfo.borrowInfo;
...
static void CalcBorrowBurstLen(const ShapeInfo& si, RuntimeInfoV2& ri) {
  BorrowInfo& bi = ri.borrowInfo;
...
static void CalcBorrowBurstLenAllIn(const ShapeInfo& si, RuntimeInfoV2& ri, int64_t vol) {
  BorrowInfo& bi = ri.borrowInfo;

  bi.majorInEle = vol;
  bi.tailInEle = 0;
  bi.majorInTailEle = vol % si.elePerBlock;
  bi.tailInTailEle = 0;
  bi.majorBurstLen_in = (vol + si.elePerBlock - 1) / si.elePerBlock;
  bi.tailBurstLen_in = 0;

  bi.majorOutEle = vol;
  bi.tailOutEle = 0;
  bi.majorOutTailEle = vol % si.elePerBlock;
  bi.tailOutTailEle = 0;
  bi.majorBurstLen_out = (vol + si.elePerBlock - 1) / si.elePerBlock;
  bi.tailBurstLen_out = 0;
}

static bool IsDstUbOverFlow(const TransposeCompilerInfo& ci,
                            const ShapeInfo& si,
                            const RuntimeInfoV2& ri,
                            BorrowInfo& bi,
                            int64_t ubSize) {
  ...
  if (dstOnlyVol * AlignX(srcVol * (lastAxisTrans ? 1 : si.lastAxisLen), si.elePerBlock) * ri.fp16Times >= ubSize) {
    overflowFlag = true;
  }
  return overflowFlag;
}
...
static bool IsSrcUbOverFlow(const TransposeCompilerInfo& ci,
                            const ShapeInfo& si,
                            const RuntimeInfoV2& ri,
                            BorrowInfo& bi,
                            int64_t ubSize) {
  ...
  if (srcOnlyVol * AlignX(dstVol * (lastAxisTrans ? 1 : si.lastAxisLen), si.elePerBlock) * ri.fp16Times >= ubSize) {
    overflowFlag = true;
  }
  return overflowFlag;
}
...
static bool UpdateStep(const TransposeCompilerInfo& ci,
                       const ShapeInfo& si,
                       const RuntimeInfoV2& ri,
                       BorrowInfo& bi,
                       int64_t ubSize) {
...
static bool CalcDstBorrowAxisIndex(const TransposeCompilerInfo& ci,
                                   const ShapeInfo& si,
                                   RuntimeInfoV2& ri,
...
static void ReorderIndexInfo(const ShapeInfo& si, RuntimeInfoV2& ri) {
...
static void CalcOtherAxisIndex(const ShapeInfo& si, RuntimeInfoV2& ri) {
 ...
 * 0 1            n
 */
static int64_t GetPermHex(const RuntimeInfoV2& runtimeInfo) {
  const BorrowInfo& bi = runtimeInfo.borrowInfo;
  int64_t perm = 0;
 ...
 * 0123 -> 1203 -> 2103 -> 3210
 */
static void RepeatStride3210(const TransposeCompilerInfo& ci, const ShapeInfo& si, RuntimeInfoV2& ri,
                             int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t bl = si.alignElement == 0 ? si.lastAxisBurstLen : si.lastAxisLen * ri.fp16Times;
 ...
 * 3201:   0123  ->  2301 -> 3201
 */
static void RepeatStride3201(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri,
                             int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t bl =
      shapeInfo.alignElement == 0 ? shapeInfo.lastAxisBurstLen : shapeInfo.lastAxisLen * ri.fp16Times;
 ...
 * 2301:   0123  ->  2301
 */
static void RepeatStride2301(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri,
                             int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t bl =
      shapeInfo.alignElement == 0 ? shapeInfo.lastAxisBurstLen : shapeInfo.lastAxisLen * ri.fp16Times;
 ...
 * 2310:   0123  ->  1023 -> 2310
 */
static void RepeatStride2310(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri,
                             int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t bl =
      shapeInfo.alignElement == 0 ? shapeInfo.lastAxisBurstLen : shapeInfo.lastAxisLen * ri.fp16Times;
 ...
 * 210: 012 -> 102 ->  210
 */
static void RepeatStride210(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri,
                            int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t bl =
      shapeInfo.alignElement == 0 ? shapeInfo.lastAxisBurstLen : shapeInfo.lastAxisLen * ri.fp16Times;
 ...
 * 201: 012 -> 201
 */
static void RepeatStride201(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri,
                            int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t bl =
      shapeInfo.alignElement == 0 ? shapeInfo.lastAxisBurstLen : shapeInfo.lastAxisLen * ri.fp16Times;
 ...
 * 120: 012 -> 120
 */
static void RepeatStride120(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri,
                            int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t bl =
      shapeInfo.alignElement == 0 ? shapeInfo.lastAxisBurstLen : shapeInfo.lastAxisLen * ri.fp16Times;
 ...
 * 01 -> 10
 */
static void RepeatStride10(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri,
                           int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t bl =
      shapeInfo.alignElement == 0 ? shapeInfo.lastAxisBurstLen : shapeInfo.lastAxisLen * ri.fp16Times;
...
static void RepeatStride(const TransposeCompilerInfo& ci, const ShapeInfo& si, RuntimeInfoV2& ri,
                         int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  int64_t burstLen = si.lastAxisLen * ri.fp16Times;
  if (si.isLastAxisTranspose) {
    if (si.isLastTwoAlignedAndTrans) {
      burstLen = si.reducedInShape[si.dim - 2] * si.reducedInShape[si.dim - 1] / si.elePerBlock;
    } else {
      burstLen = ri.fp16Times;
    }
  } else {
...
static void SetVol(const ShapeInfo& si, RuntimeInfoV2& ri,
                   int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
...
static void SetVolAllIn(const ShapeInfo& si, RuntimeInfoV2& ri,
                        int64_t step[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT]) {
  BorrowInfo& bi = ri.borrowInfo;
  for (int64_t j = 0; j < BORROW_MAX_AXIS_NUM_LT; j++) {
    if (step[0][j] != 0) {
      bi.xdxsVol[0] *= step[0][j];
    }
  }
}
...
static void ConstructStep(const RuntimeInfoV2& ri, int64_t step[][BORROW_MAX_AXIS_NUM_LT]) {
  const BorrowInfo& bi = ri.borrowInfo;
...
static void ConstructStepS5(const RuntimeInfoV2& ri, int64_t step[][BORROW_MAX_AXIS_NUM_LT]) {
  const BorrowInfo& bi = ri.borrowInfo;
...
static void ConstructStepS5AllIn(const ShapeInfo& si,
                                 const RuntimeInfoV2& ri,
                                 int64_t step[][BORROW_MAX_AXIS_NUM_LT],
                                 int64_t leftMostAxis) {
  int64_t st[UB_REORDER_COMBINATION][BORROW_MAX_AXIS_NUM_LT] = {0};
  int axis = 0;
  int64_t offset = GetOffset(si);
  for (int64_t i = leftMostAxis; i <= si.dim - offset; i++) {
    st[0][axis++] = si.reducedInShape[i];
  }
...
static void CalcRepetStride(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri) {
  int perm = GetPermHex(ri);
  ri.borrowInfo.axisPerm = perm;
  ...
  ConstructStep(ri, step);

  switch (perm) {
    case 0x3210:
      RepeatStride3210(compilerInfo, shapeInfo, ri, step);
      break;
    case 0x03201:
      RepeatStride3201(compilerInfo, shapeInfo, ri, step);
      break;
    case 0x2301:
      RepeatStride2301(compilerInfo, shapeInfo, ri, step);
      break;
    case 0x2310:
      RepeatStride2310(compilerInfo, shapeInfo, ri, step);
      break;
    case 0x210:
      RepeatStride210(compilerInfo, shapeInfo, ri, step);
      break;
    case 0x201:
      RepeatStride201(compilerInfo, shapeInfo, ri, step);
      break;
    case 0x120:
      RepeatStride120(compilerInfo, shapeInfo, ri, step);
      break;
    case 0x10:
      RepeatStride10(compilerInfo, shapeInfo, ri, step);
      break;
    default:
      break;
  }
...
static void CalcRepetStrideS5(const TransposeCompilerInfo& compilerInfo,
                              const ShapeInfo& shapeInfo,
                              RuntimeInfoV2& ri) {
  int perm = GetPermHex(ri);
  ri.borrowInfo.axisPerm = perm;
  ...
  ConstructStepS5(ri, step);

  RepeatStride(compilerInfo, shapeInfo, ri, step);

  SetVol(shapeInfo, ri, step);
}


static void CalcRepetStrideS5AllIn(const TransposeCompilerInfo& ci,
                                   const ShapeInfo& si,
                                   RuntimeInfoV2& ri,
                                   int64_t leftMostAxis) {
  int perm = GetPermHex(ri);
  ...
  ConstructStepS5AllIn(si, ri, step, leftMostAxis);

  RepeatStride(ci, si, ri, step);

  SetVolAllIn(si, ri, step);
}

static bool IsLastAxisAllIn(const TransposeCompilerInfo& ci,
                            const ShapeInfo& si,
                            RuntimeInfoV2& ri,
                            int64_t& leftMostAxis,
                            int64_t& vol) {
  ...
  // scenario5 has specific optimization for this scenario
  if (si.isLastTwoAlignedAndTrans) {
    return false;
  }

  // scenario5 has specific optimization for this scenario
  if (si.alignElement == 0 && (!si.isLastAxisTranspose)) {
    return false;
  }

  if (si.dim < BORROW_SRC_AXIS_NUM_LT + offset) {
    return false;
  }

  for (int64_t i = si.dim - 1 ; i >= si.dim - BORROW_SRC_AXIS_NUM_LT; i--) {
    if (si.reducedPerm[i] < leftMostAxis) {
      leftMostAxis = si.reducedPerm[i];
    }
  }

  for (int64_t i = si.dim - 1; i >= leftMostAxis; i--) {
    if (si.reducedPerm[i] < leftMostAxis) {
      // it means left of leftMostAxis insert to the output
      return false;
    }
  }

  // limited by our framework, support 4-5 transpose axis switch
  // eg: (A,B,C,D,E,F,G) -> (B,A,F,D,E,G,C), leftMostAxis = C, src = (E,F,G), dst = (E,G,C), D is missing
  if (si.isLastAxisTranspose) {
    if (si.dim - leftMostAxis > 4) {
      return false;
    }
  } else {
    if (si.dim - leftMostAxis > 5) {
      return false;
    }
  }

  if (si.dim - leftMostAxis > BORROW_SRC_AXIS_NUM_LT + BORROW_DST_AXIS_NUM_LT) {
    return false;
  }

  for (int64_t i = leftMostAxis; i < si.dim; i++) {
    vol *= si.reducedInShape[i];
  }

  if (vol < si.elePerBlock) {
    return false;
  }

  if (((vol + vol % si.elePerBlock) * ri.fp16Times) > CalcVnchwconvPartialUbSize(ci.ubSize)) {
    return false;
  }
...
static void CalcScenario5AllIn(const ShapeInfo& si, RuntimeInfoV2& ri, int64_t leftMostAxis, int64_t vol) {
  int64_t offset = GetOffset(si);
  BorrowInfo& bi = ri.borrowInfo;
  int64_t srcNum = 0;
  int64_t dstNum = 0;
  for (int64_t i = si.dim; i > si.dim - BORROW_SRC_AXIS_NUM_LT; i--) {
    if (i < leftMostAxis) {
      break;
    }
    bi.srcIndexIn[srcNum].idx_in = i - offset;
    bi.srcIndexIn[srcNum].idx_out = GetPermIndex(si.reducedPerm, si.dim, bi.srcIndexIn[srcNum].idx_in);
    bi.srcIndexIn[srcNum].step = si.reducedInShape[i - offset];
    bi.srcIndexIn[srcNum].loop = 1;
    bi.srcIndexIn[srcNum].tail = 0;
    srcNum++;
    bi.srcNum = srcNum;
  }

  for (int64_t i = si.dim; i > si.dim - BORROW_DST_AXIS_NUM_LT; i--) {
    if (i < leftMostAxis) {
      break;
    }
    int64_t id = i - offset;
...
static bool TilingDataScenario4(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                                RuntimeInfoV2& runtimeInfo) {
  int64_t ubSize = 0;
...
static bool TilingDataScenario5(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                                RuntimeInfoV2& runtimeInfo) {
  int64_t leftMostAxis = shapeInfo.dim - 1;
  int64_t vol = 1;
  int64_t ubSize = 0;
  if (IsLastAxisAllIn(compilerInfo, shapeInfo, runtimeInfo, leftMostAxis, vol)) {
    CalcScenario5AllIn(shapeInfo, runtimeInfo, leftMostAxis, vol);
    MergeDupAxis(shapeInfo, runtimeInfo);
    ReorderIndexInfo(shapeInfo, runtimeInfo);
    CalcSrcDstPerm(runtimeInfo);
    CalcBorrowBurstLenAllIn(shapeInfo, runtimeInfo, vol);
    CalcOtherAxisIndex(shapeInfo, runtimeInfo);
    CalcPermInUb(shapeInfo, runtimeInfo);
    SplitCore(compilerInfo, shapeInfo, runtimeInfo);
    CalcRepetStrideS5AllIn(compilerInfo, shapeInfo, runtimeInfo, leftMostAxis);
    CalcStrideS4(shapeInfo, runtimeInfo);
  } else {
    CalcLeftVol(compilerInfo, shapeInfo, runtimeInfo, ubSize);
...
static bool TilingDataScenario6(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                                RuntimeInfoV2& runtimeInfo) {
...
static void SplitNByFactor(RuntimeInfoV2& runtimeInfo) {
...
static bool SplitColByFactor(RuntimeInfoV2& runtimeInfo, int64_t elePerBlock) {
  const TilingModel& tm = runtimeInfo.tilingModel;
...
static bool SplitRowByFactor(RuntimeInfoV2& runtimeInfo, int64_t elePerBlock) {
  const TilingModel& tm = runtimeInfo.tilingModel;
...
static void CalcJumpInfo(RuntimeInfoV2& runtimeInfo, int64_t dim, const vector<int64_t>& inShape,
...
static void FindLongestColPerm(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
...
static void DispatchNCR(const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
...
static void MakeNCRDecision(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, RuntimeInfoV2& ri) {
#define ADD_MODEL(SpecificModel) \
  do { \
      SpecificModel model(compilerInfo.coreNum, compilerInfo.ubSize); \
      if (model.Decision(ri.ncrs[i], shapeInfo.dim)) { \
        if (ri.tilingModel.priority == 0) { \
          ri.tilingModel.copy(model); \
        } else { \
          if (model.priority < ri.tilingModel.priority) { \
            ri.tilingModel.copy(model); \
          } \
        } \
      } \
  ...
  if (ri.fp16Times == 2) {
    for (int64_t i = 0; i < ri.ncrsSize; i++) {
      ADD_MODEL(Model001);
      ADD_MODEL(Model002);
      ADD_MODEL(Model003);
      ADD_MODEL(Model004);
...
static void Composite(RuntimeInfoV2& runtimeInfo) {
  const TilingModel& tm = runtimeInfo.tilingModel;
...
static bool IsScenario7Accept(const RuntimeInfoV2& runtimeInfo) {
...
static bool TilingDataScenario7(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                                RuntimeInfoV2& runtimeInfo) {
  bool res = false;
...
bool TilingDataScenario9(const TransposeCompilerInfo& compilerInfo,
                         const ShapeInfo& shapeInfo,
                         RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t index = 0;
...
static bool TilingDataScenario10(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo,
                                 RuntimeInfoV2& runtimeInfo) {
  int64_t ubSize = 0;
  CalcLeftVol(compilerInfo, shapeInfo, runtimeInfo, ubSize);
  CalcSrcBorrowAxisIndex(shapeInfo, runtimeInfo, BORROW_SRC_AXIS_NUM_LT);
...
static void Composite3D(const TransposeCompilerInfo& compilerInfo, const ShapeInfo& shapeInfo, 
                        RuntimeInfoV2& runtimeInfo,
...
static int64_t GetColUnit(const TransposeCompilerInfo& ci, const ShapeInfo& si) {
    if (ci.ubSize == BLOCK_NUM_256K || ci.ubSize == BLOCK_NUM_248K) {
      if (si.elePerBlock == EPB32) {
          return 224;
      } else {
...
static int64_t CalcNUnit(const TransposeCompilerInfo& ci, const ShapeInfo& si, RuntimeInfoV2& ri) {
  int64_t dim = si.dim;
  if (si.reducedInShape[dim - 2] > GetColUnit(ci, si)) {
    return 1;
  }
  if (si.reducedInShape[dim - 1] > ROW_UNIT) {
    return 1;
  }
  int64_t nUnit = (GetColUnit(ci, si) * ROW_UNIT) / \
                  (si.reducedInShape[dim - 2] * si.reducedInShape[dim - 1] * ri.fp16Times);
  return nUnit == 0 ? 1 : nUnit;
}

static bool TilingDataScenario11(const TransposeCompilerInfo & compilerInfo,
                                 const ShapeInfo & shapeInfo,
                                 RuntimeInfoV2 & runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  ...
  int64_t cFactor = 1;
  int64_t rFactor = 1;
  int64_t colUnit = GetColUnit(compilerInfo, shapeInfo);
  int64_t rowUnit = ROW_UNIT;
  int64_t colUnitNum = 1;
  int64_t rowUnitNum = 1;
  int64_t colTail = 0;
  int64_t rowTail = 0;
  int64_t nUnit = CalcNUnit(compilerInfo, shapeInfo, runtimeInfo);
...
bool TilingDataScenario12(const TransposeCompilerInfo& compilerInfo,
                          const ShapeInfo& shapeInfo, RuntimeInfoV2& runtimeInfo) {
  int64_t dim = shapeInfo.dim;
  int64_t vol = 1;
  int64_t i = 0;
  ...
  if (shapeInfo.mteMode == MTE_MODE_DST) {
    int64_t splitAxis = shapeInfo.reducedPerm[dim - 2];

    // 1. split axis info
    runtimeInfo.loopUnit = runtimeInfo.ubSizeCouldUse / shapeInfo.lastAxisBurstLen;
    runtimeInfo.loopMain = shapeInfo.reducedInShape[splitAxis] / runtimeInfo.loopUnit;
    runtimeInfo.loopTail = shapeInfo.reducedInShape[splitAxis] % runtimeInfo.loopUnit;

    // 2.1 dst stride
    for (i = 0; i < dim - 2; i++) {
      runtimeInfo.dstJumpStride[i] = CalcStride(shapeInfo.reducedOutShape, dim, i);
    }
    // 2.2 logic src stride for split axis
    runtimeInfo.dstJumpStride[i] = shapeInfo.reducedInShape[dim - 1] * runtimeInfo.loopUnit;
    runtimeInfo.loopDstStride = shapeInfo.reducedInShape[dim - 1] * runtimeInfo.loopUnit;

    // 3.1 src stride
    for (i = 0; i < dim - 2; i++) {
      runtimeInfo.srcJumpStride[i] = CalcStride(shapeInfo.reducedInShape, dim, shapeInfo.reducedPerm[i]);
    }
    // 3.2 logic src stride for split axis
    int64_t stride = 1;
    for (int j = splitAxis + 1; j < dim; j++) {
      stride *= shapeInfo.reducedInShape[j];
    }
    runtimeInfo.srcJumpStride[i] = stride * runtimeInfo.loopUnit;
    runtimeInfo.loopSrcStride = stride * runtimeInfo.loopUnit;

    // 4.1 dst factor
    for (i = 0; i < dim - 2; i++) {
      runtimeInfo.dstJumpFactor[i] = shapeInfo.reducedOutShape[i];
    }
    // 4.2 ligic dst factor
    runtimeInfo.dstJumpFactor[i] = runtimeInfo.loopMain;

    // 5. stride for repeat in data_move
    for (i = splitAxis + 1; i < shapeInfo.dim - 1; i++) {
      vol *= shapeInfo.reducedInShape[i];
    }
    runtimeInfo.srcStride = (vol - 1) * shapeInfo.lastAxisBurstLen;

    // 6. init tuple
    CalcTupleDstS12(compilerInfo, shapeInfo, runtimeInfo);
  } else {
    int64_t splitAxis = shapeInfo.dim - 2;
    ...
    // 2.2 logic dst stride for split axis
    int64_t stride = 1;
    int64_t permIndex = GetPermIndex(shapeInfo.reducedPerm, dim, dim - 2);
    for (int j = permIndex + 1; j < dim; j++) {
      stride *= shapeInfo.reducedOutShape[j];
    }
    runtimeInfo.dstJumpStride[i] = stride * runtimeInfo.loopUnit;
    runtimeInfo.loopDstStride = stride * runtimeInfo.loopUnit;

    // 3.1 src stride
    for (i = 0; i < dim - 2; i++) {
      runtimeInfo.srcJumpStride[i] = CalcStride(shapeInfo.reducedInShape, dim, i);
    }
    // 3.2 logic src stride
    runtimeInfo.srcJumpStride[i] = shapeInfo.reducedOutShape[dim - 1] * runtimeInfo.loopUnit;
    runtimeInfo.loopSrcStride = shapeInfo.reducedOutShape[dim - 1] * runtimeInfo.loopUnit;

    // 4.1 src factor
    for (i = 0; i < dim - 2; i++) {
      runtimeInfo.srcJumpFactor[i] = shapeInfo.reducedInShape[i];
    }
    // 4.2 logic src factor
    runtimeInfo.srcJumpFactor[i] = runtimeInfo.loopMain;

    // 5. stride for repeat in data_move
    for (i = permIndex + 1; i < shapeInfo.dim - 1; i++) {
      vol *= shapeInfo.reducedOutShape[i];
    }
    runtimeInfo.dstStride = (vol - 1) * shapeInfo.lastAxisBurstLen;

    // 6. init tuple
    CalcTupleSrcS12(compilerInfo, shapeInfo, runtimeInfo);
  }
...
static bool IsSpecificShape(const TransposeCompilerInfo& compilerInfo,
                            const ShapeInfo& shapeInfo,
  ...
  for (uint64_t i = 0; i < gSpecificShape.size(); i++) {
    ...
    if (gSpecificShape[i][2] != ri.fp16Times * 2) {
      continue;
    }
    for (int64_t j = 0; j < dim; j++) {
...
static void UpdateScenarios(RuntimeInfoV2& ri, TransposeScenario scenario) {
  ri.scenarios[ri.scenarioSize++] = scenario;
}

bool TransposeCalcTilingData(const string& opType, const TransposeCompilerInfo& compilerInfo, ShapeInfo& shapeInfo,
                             RuntimeInfoV2& runtimeInfo) {
  bool res = false;
  TransposeScenario scenario;
  if (IsSpecificShape(compilerInfo, shapeInfo, scenario, runtimeInfo)) {
  ...
  while (!res) {
    switch (shapeInfo.scenario) {
      ...
      case SCENARIO_10:
        ...
        OP_LOGD(opType.c_str(), "%s", PrintScenario5(compilerInfo, shapeInfo, runtimeInfo).c_str());
        break;
      case SCENARIO_11:
        ...
        break;
      case SCENARIO_12:
        res = TilingDataScenario12(compilerInfo, shapeInfo, runtimeInfo);
        UpdateScenarios(runtimeInfo, SCENARIO_12);
        OP_LOGI(opType.c_str(), "%s", PrintScenario12(compilerInfo, shapeInfo, runtimeInfo).c_str());
        PrintScenario12(compilerInfo, shapeInfo, runtimeInfo);
        break;
      default:
        break;
    }
...
#define ADD_TILING_DATA_TO_RUN_INFO(vec, size) \
  do { \
      ...
      if (!td) { \
        OP_LOGE(context->GetNodeName(), "GetRawTilingData is nullptr"); \
      } \
      int64_t cap = td->GetCapacity(); \
      void * ptr = td->GetData(); \
      if (memcpy_s(ptr, cap, (void*)(&(vec[0])), vecSize * sizeof(int64_t)) == -1) { \
        OP_LOGE(context->GetNodeName(), "Add tiling data failed, cap = %ld, actual = %ld", cap, vecSize); \
...
static ge::graphStatus SerializeScenario0(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
...
static ge::graphStatus SerializeScenario1(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
...
static ge::graphStatus SerializeScenario2(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
...
static ge::graphStatus SerializeScenario3(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
...
static ge::graphStatus SerializeScenario4(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
...
static ge::graphStatus SerializeScenario5(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
...
static ge::graphStatus SerializeScenario6(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  return SerializeScenario2(context, compilerInfo, shapeInfo, runtimeInfo);
}

static ge::graphStatus SerializeScenario7(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  const TilingModel& tm = runtimeInfo.tilingModel;
...
static ge::graphStatus SerializeScenario8(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
  ...
  for (int64_t i = 0; i < vecSize; i++) {
    tiling_data->Append(tilingVec[i]);
  }
...
static ge::graphStatus SerializeScenario9(TilingContext* context,
                                          const TransposeCompilerInfo& compilerInfo,
                                          const ShapeInfo& shapeInfo,
                                          const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
...
static ge::graphStatus SerializeScenario11(TilingContext* context,
                                           const TransposeCompilerInfo& compilerInfo,
                                           const ShapeInfo& shapeInfo,
                                           const RuntimeInfoV2& runtimeInfo) {
  DEFINE_PARAMETERS;
...
static ge::graphStatus SerializeScenario12(TilingContext* context,
                                           const TransposeCompilerInfo& compilerInfo,
                                           const ShapeInfo& shapeInfo,
                                           const RuntimeInfoV2& runtimeInfo) {
  ...
  WRITE_DATA(shapeInfo.lastAxisBurstLen);
  WRITE_DATA(shapeInfo.dim - 1);
  WRITE_DATA(runtimeInfo.loopUnit);
  WRITE_DATA(runtimeInfo.loopMain);
  WRITE_DATA(runtimeInfo.loopTail);
  WRITE_DATA(runtimeInfo.loopSrcStride);
  WRITE_DATA(runtimeInfo.loopDstStride);
  WRITE_DATA(runtimeInfo.srcStride);
  WRITE_DATA(runtimeInfo.dstStride);
...
ge::graphStatus SerializeTilingData(TilingContext* context,
                                    const TransposeCompilerInfo& compilerInfo,
                                    const ShapeInfo& shapeInfo,
                                    const RuntimeInfoV2& runtimeInfo) {
  ...
  switch (shapeInfo.scenario) {
    case SCENARIO_0:
      ret = SerializeScenario0(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_1:
      ret = SerializeScenario1(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_2:
      ret = SerializeScenario2(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_3:
      ret = SerializeScenario3(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_4:
      ret = SerializeScenario4(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_5:
      ret = SerializeScenario5(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_6:
      ret = SerializeScenario6(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_7:
      ret =  SerializeScenario7(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_8:
      ret = SerializeScenario8(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_9:
      ret = SerializeScenario9(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_10:
      ret = SerializeScenario5(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_11:
      ret = SerializeScenario11(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    case SCENARIO_12:
      ret = SerializeScenario12(context, compilerInfo, shapeInfo, runtimeInfo);
      break;
    default:
      break;
  }
...
ge::graphStatus TilingForTranspose(TilingContext* context) {
  PROFILING_TILING_INIT("Transpose");
  OP_LOGI(context->GetNodeName(), "Tiling is running.");

  auto compilerInfo = (reinterpret_cast<const TransposeCompilerInfo*>(context->GetCompileInfo()));
  OPS_CHECK_NULL_WITH_CONTEXT(context, compilerInfo)
  ...
  RuntimeInfoV2& runtimeInfo = AcquireRuntimeInfoV2(id);
  shapeInfo.id = id;
  runtimeInfo.id = id;
  ...
  if (strcmp(context->GetNodeName(), "DepthToSpace") == 0 || strcmp(context->GetNodeName(), "SpaceToDepth") == 0) {
    /*
    if (AddShapePerm(context->GetNodeName(), opParas, *compilerInfo, shapeInfo) == false) {
      ReleaseID(id);
      return false;
    }
    */
  } else {
    if (GetShapePerm(context, *compilerInfo, shapeInfo, runtimeInfo) == false) {
      ReleaseID(id);
      return ge::GRAPH_FAILED;
    }
  }
  PROFILING_TILING_AFTER_GET_COMPILE_INFO_REG();
  if (CheckTensorShape(context->GetNodeName(), shapeInfo) == false) {
    ReleaseID(id);
    return ge::GRAPH_FAILED;
  }

  ReduceAxisV2(context->GetNodeName(), *compilerInfo, shapeInfo, runtimeInfo);
  if (TransposeCalcTilingData(context->GetNodeName(), *compilerInfo, shapeInfo, runtimeInfo) == false) {
    ReleaseID(id);
    return ge::GRAPH_FAILED;
  }

  PROFILING_TILING_AFTER_CALCU_TILING_REG();
  if (SerializeTilingData(context, *compilerInfo, shapeInfo, runtimeInfo) == ge::GRAPH_FAILED) {
    ReleaseID(id);
    return ge::GRAPH_FAILED;
  }
...
static bool CheckCompileInfo(TilingParseContext* context, TransposeCompilerInfo* ci) {
  OP_LOGD(context->GetNodeName(),
          "Parsed TransposeTiling compile info(core_num, ub_size, block_size): %ld, %ld, %ld",
          ci->coreNum, ci->ubSize, ci->blockSize);
    if (ci->coreNum <= 0 || ci->ubSize <=0 ) {
        VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(),
                                        "CheckCompileInfo failed, coreNum = %ld, ubSize = %ld, blockSize = %ld",
                                        ci->coreNum, ci->ubSize, ci->blockSize);
    return false;
  }
...
IMPL_OP(Transpose).Tiling(TilingForTranspose).TilingParse<TransposeCompilerInfo>(TilingPrepareForTranspose);
}  // namespace optiling
