...
void PrintROIAlignParams(const ROIAlignTilingParams& params) {
  OP_LOGD("op [ROIAlignTiling] : tilingMode=%d.", params.tilingMode);
  OP_LOGD("op [ROIAlignTiling] : tilingMode=%d.", params.real_core_num);
  OP_LOGD("op [ROIAlignTiling] : tilingMode=%d.", params.rois_n);
  OP_LOGD("op [ROIAlignTiling] : tilingMode=%d.", params.rois_row_lenth);
  OP_LOGD("op [ROIAlignTiling] : tilingMode=%d.", params.c1_num);
  OP_LOGD("op [ROIAlignTiling] : tilingMode=%d.", params.x_height);
  OP_LOGD("op [ROIAlignTiling] : tilingMode=%d.", params.x_width);
}
...
static bool CheckTensorShape(const std::string& opType, std::vector<int64_t> x_diff_shape,
  ...
  if (x_diff_shape_dims != 5) {
    OP_LOGE(opType.c_str(), "op [ROIAlignTiling] : CheckTensorShape, shape of x_diff check failed.");
    return false;
  }

  if (rois_shape_dims != 2) {
    OP_LOGE(opType.c_str(), "op [ROIAlignTiling] : CheckTensorShape, dims of rois must be 2.");
    return false;
  }
...
static bool GetCompileParams(const std::string& opType, const nlohmann::json& opCompileInfoJson,
  ...
  if (allVars.count("core_num") == 0) {
    OP_LOGE(opType.c_str(), "op [ROIAlignTiling] : GetCompileParams, get core_num error");
    return false;
  }
  coreNum = allVars["core_num"].get<std::int64_t>();

  if (allVars.count("ub_size") == 0) {
    OP_LOGE(opType.c_str(), "op [ROIAlignTiling] : GetCompileParams, get ub_size error");
    return false;
  }
...
bool ROIAlignTiling(const std::string& opType, const TeOpParas& opParas, const nlohmann::json& op_info,
  ...
  if (op_info == nullptr) {
    OP_LOGE(opType.c_str(), "op ROIAlignTiling: op_info json error.");
    return false;
  }
  ...
  if (!flag) {
    OP_LOGE("op[%s] ROIAlignTiling: GetCompileParams error.", opType.c_str());
    return false;
  }
  ...
  if (!flag) {
    OP_LOGE("op[%s] ROIAlignTiling: params check failed.", opType.c_str());
    return false;
  }
