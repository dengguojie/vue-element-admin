...
void CalRunningParams(ScatterUpdateTilingParams& runParams, int32_t indicesNum,
    int32_t updateDataNum, int32_t maxIndice, int32_t ubSize, int32_t coreNum) {
    runParams.updatesUbNumber = updateDataNum;
    runParams.indicesUbNumber = ubSize * 0.9 / FP32_INT32_SIZE - updateDataNum;
    runParams.indicesUbNumber = ceil(float(runParams.indicesUbNumber) / INDICES_DATA_EACH_BLOCK) * INDICES_DATA_EACH_BLOCK;
...
bool CheckTensorShape(std::vector<int64_t> varShape, std::vector<int64_t> indicesShape,
  std::vector<int64_t> updatesShape, std::vector<int64_t> outShape) {
    if (varShape != outShape) {
      ge::OpsOneInputShapeErrReport("ScatterUpdate", "var",
          "the length of var must be same as the length of output");
      GE_LOGE("[ScatterUpdateTiling] : var_out's shape must be the same as var's shape.");
        return false;
    }
    ...
    if (updatesShape != actualUpdatesShape) {
        ...
        GE_LOGE("op [ScatterUpdateTiling] : updates's shape is illegal.");
        return false;
    }
    return true;
}

bool GetScatteUpdateCompileParams(const std::string &opType,
    ...
    if (allVars.count("core_num") == 0) {
        ...
        GE_LOGE("op [ScatterUpdateTiling] : GetCompileParams, get core_num error");
        return false;
    }
    coreNum = allVars["core_num"].get<std::int32_t>();
    if (allVars.count("ub_size") == 0) {
        ge::OpsGetCompileParamsErrReport(opType.c_str(), "ub_size");
        GE_LOGE("op [ScatterUpdateTiling] : GetCompileParams, get ub_size error");
        return false;
    }
...
bool ScatterUpdateTiling(const std::string &opType, const TeOpParas &opParas,
  const nlohmann::json &opCompileInfo, OpRunInfo &runInfo) {
    ...
    if (opCompileInfo == nullptr) {
      GE_LOGE("op[%s] ScatterUpdateTiling: opCompileInfo json error.", opType.c_str());
      return false;
    }
    ...
    if (opParas.inputs.empty() || varShape.size() < 2 || opParas.inputs[0].tensor.empty()
      ...
      GE_LOGE("op[%s] ScatterUpdateTiling: input shape error", opType.c_str());
      return false;
    }
    if (opParas.outputs.empty() || outShape.size() < 2 || opParas.outputs[0].tensor.empty()) {
      ...
      GE_LOGE("op[%s] ScatterUpdateTiling: output shape error", opType.c_str());
      return false;
    }

    bool is_valid_shape = CheckTensorShape(varShape, indicesShape, updatesShape, outShape);
    if (!is_valid_shape) {
      GE_LOGE("op[%s] ScatterUpdateTiling: CheckTensorShape failed.", opType.c_str());
      return false;
    }
    ...
    if (!can_get_params) {
      GE_LOGE("op[%s] ScatterUpdateTiling: GetScatteUpdateCompileParams error.", opType.c_str());
      return false;
    }
    ...
    int32_t updateDataNum = (varShape.size() > 1) ? (std::accumulate(varShape.begin()+1, varShape.end(), 1, std::multiplies<int>())) : 1;
