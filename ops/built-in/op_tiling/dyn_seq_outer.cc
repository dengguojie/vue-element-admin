...
static void InitRunningParams(DynSeqOuterTilingParam& params) {
  params.batch_size = 1;
  params.feature_dim = 1;
}

static bool GetCoreNum(const std::string& op_type, const std::vector<int64_t>& compile_info, int32_t& core_num) {
  OP_LOGD(op_type.c_str(), "GetCompileParams is running.");
  OP_TILING_CHECK(
      compile_info.size() != 1,
      VECTOR_INNER_ERR_REPORT_TILIING(op_type, "the compile info num must == 1, is %zu", compile_info.size()),
      return false);
  core_num = compile_info[0];
  return true;
}

static void SetRunningInfo(const std::string& op_type, const DynSeqOuterTilingParam& tiling_params,
                           utils::OpRunInfo& run_info) {
  OP_LOGD(op_type.c_str(), "SetRunningInfo is running.");
  run_info.AddTilingData(tiling_params.batch_size);
  run_info.AddTilingData(tiling_params.feature_dim);
}

static void PrintTilingParams(const std::string& op_type, const DynSeqOuterTilingParam& tiling_params) {
  OP_LOGD(op_type.c_str(), "PrintTilingParams is running.");
  OP_LOGD(op_type.c_str(), "op [DynSeqOuterTilingParam] : batch_size=%d.", tiling_params.batch_size);
  OP_LOGD(op_type.c_str(), "op [DynSeqOuterTilingParam] : feature_dim=%d.", tiling_params.feature_dim);
}

bool DynSeqOuterTiling(const std::string& op_type, const ge::Operator& op_paras,
                       const std::vector<int64_t>& op_compile_info, utils::OpRunInfo& run_info) {
  OP_LOGD(op_type.c_str(), "DynSeqOuterTiling is running.");
  ...
  SetRunningInfo(op_type, run_params, run_info);
  PrintTilingParams(op_type, run_params);
...
REGISTER_OP_TILING_V3_WITH_VECTOR(DynSeqOuter, DynSeqOuterTiling, DynSeqOuter_COMPILE_INFO_KEY, NO_OPTIONAL_VALUE);
}  // namespace optiling
