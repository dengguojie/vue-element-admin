...
bool GetResizeNearestNeighborV2CompileParams(const nlohmann::json& compile_info, int64_t& core_num,
                                             int64_t& max_w_len,  int64_t& align_corners,
...
bool ResizeNearestNeighborV2Tiling(const std::string& op_type, const TeOpParas& op_paras,
  ...
  if (!GetResizeNearestNeighborV2CompileParams(op_info, core_num, max_w_len, align_corners, half_pixel_centers)) {
  ...
  tiling_params.tiling_key = 100000;
  ...
  is_w_nw = (is_h_nh && tiling_params.output_weight > tiling_params.input_weight * max_w_len) ? false : is_w_nw;
  auto image_batch_c1 = tiling_params.input_batch * tiling_params.input_c1;
  tiling_params.tiling_key = is_w_nw ? tiling_params.tiling_key + 1000 : tiling_params.tiling_key;

  if (is_h_nh && is_w_nw) {
    // process h * w, so cut by nc1 first from input
    tiling_params.tiling_key = tiling_params.tiling_key + 10000;
    if (tiling_params.output_weight == tiling_params.input_weight) {
      tiling_params.tiling_key = tiling_params.tiling_key + 2000;
    }
    ...
    left_core_num = core_num / (tiling_params.cut_batch_c1_num * tiling_params.cut_weight_num);
    // continue cut height
    if (left_core_num > 1) {
    ...
    auto w_max = (cut_weight_total + core_num - 1) / core_num;
