...
bool BinaryCrossEntropyGradTiling(const std::string& op_type, const ge::Operator& op_paras,
                                  const BinaryCrossEntropyGradCompileInfo& parsed_info, utils::OpRunInfo& run_info) {
  ...
  const uint32_t INPUT_AXIS_GRAD_OUTPUT = 2;
  input_desc = operator_info->MutableInputDesc(INPUT_AXIS_GRAD_OUTPUT);
  OP_TILING_CHECK(input_desc == nullptr, VECTOR_INNER_ERR_REPORT_TILIING(op_type, "get input_desc failed."),
                  return false);
  const std::vector<int64_t> input_shape2 = input_desc->MutableShape().GetDims();
  vector<vector<int64_t>> input_shapes = {input_shape, input_shape2};
...
REGISTER_OP_TILING_V3_CUSTOM(BinaryCrossEntropyGrad, BinaryCrossEntropyGradTiling, ParseJsonCompileInfo,
                             BinaryCrossEntropyGradCompileInfo);
}  // namespace optiling
