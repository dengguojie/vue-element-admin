...
void InitRunningParams(ScatterMinTilingParams& params) {
...
void SetRuningParams(const ScatterMinTilingParams& params, OpRunInfo& runInfo) {
...
void PrintTilingParams(const ScatterMinTilingParams& params) {
  GELOGD("op [ScatterMinTiling] : tilingMode=%ld.", params.tilingMode);
  GELOGD("op [ScatterMinTiling] : indiceStep=%ld.", params.indiceStep);
  GELOGD("op [ScatterMinTiling] : coreNum=%ld.", params.coreNum);
  GELOGD("op [ScatterMinTiling] : updatesDataNum=%ld.", params.updatesDataNum);
  GELOGD("op [ScatterMinTiling] : indicesLoopNum=%ld.", params.indicesLoopNum);
  GELOGD("op [ScatterMinTiling] : indicesLastNum=%ld.", params.indicesLastNum);
  GELOGD("op [ScatterMinTiling] : updatesNum=%ld.", params.updatesNum);
  GELOGD("op [ScatterMinTiling] : updatesLoopNum=%ld.", params.updatesLoopNum);
  GELOGD("op [ScatterMinTiling] : updatesLastNum=%ld.", params.updatesLastNum);
  GELOGD("op [ScatterMinTiling] : varNum=%ld.", params.varNum);
  GELOGD("op [ScatterMinTiling] : varLoopNum=%ld.", params.varLoopNum);
  GELOGD("op [ScatterMinTiling] : varLastNum=%ld.", params.varLastNum);
  GELOGD("op [ScatterMinTiling] : varEachCoreBurstLen=%ld.", params.varEachCoreBurstLen);
  GELOGD("op [ScatterMinTiling] : varLastCoreBurstLen=%ld.", params.varLastCoreBurstLen);
  GELOGD("op [ScatterMinTiling] : maxIndice=%ld.", params.maxIndice);
  GELOGD("op [ScatterMinTiling] : varEachCoreData=%ld.", params.varEachCoreData);
}

bool CheckScatterMinShape(const std::string& opType, std::vector<int64_t> varShape, std::vector<int64_t> indicesShape,
                          std::vector<int64_t> updatesShape, std::vector<int64_t> outShape) {
  if (varShape != outShape) {
    ge::OpsOneInputShapeErrReport("ScatterMin", "var", "the length of var must be same as the length of output");
    OP_LOGE(opType.c_str(), "[ScatterMinTiling] : var_out's shape must be the same as var's shape.");
    return false;
  }
  ...
  if (updatesShape != actualUpdatesShape) {
    ge::OpsOneInputShapeErrReport("ScatterMin", "updates",
                                  "updates does not satisfy the relation expression with actualUpdatesShape");
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : updates's shape is illegal.");
    return false;
  }
  return true;
}

bool GetScatterMinCompileParams(const std::string& opType, const nlohmann::json& opCompileInfo, int64_t& coreNum,
  ...
  if (allVars.count("core_num") == 0) {
    ...
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : GetCompileParams, get core_num error");
    return false;
  }
  ...
  if (allVars.count("ub_size") == 0) {
    ...
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : GetCompileParams, get ub_size error");
    return false;
  }
  ...
  if (allVars.count("var_size") == 0) {
    ...
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : GetCompileParams, get var_size error");
    return false;
  }
  ...
  if (allVars.count("indices_size") == 0) {
    ...
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : GetCompileParams, get indices_size error");
    return false;
  }
...
bool ScatterMinTiling(const std::string& opType, const TeOpParas& opParas, const nlohmann::json& opCompileInfo,
                      OpRunInfo& runInfo) {
  using namespace ge;

  GELOGI("op[%s] ScatterMinTiling running.", opType.c_str());
  if (opCompileInfo == nullptr) {
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : opCompileInfo json error.");
    return false;
  }
  ...
  if (opParas.inputs.empty() || opParas.inputs[0].tensor.empty() ||
    ...
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : input shape error");
    return false;
  }
  ...
  if (opParas.outputs.empty() || opParas.outputs[0].tensor.empty()) {
    ...
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : output shape error");
    return false;
  }
  ...
  bool is_valid_shape = CheckScatterMinShape(opType, varShape, indicesShape, updatesShape, outShape);
  if (!is_valid_shape) {
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : CheckScatterMinShape is failed.");
    return false;
  }
  ...
  if (!can_get_params) {
    OP_LOGE(opType.c_str(), "op [ScatterMinTiling] : GetScatterMinCompileParams error.");
    return false;
  }

  ScatterMinTilingParams runParams;
  ...
  GELOGD("op [ScatterMinTiling] : indicesNum=%ld.", indicesNum);
...
REGISTER_OP_TILING_FUNC_BUFFERED(ScatterMin, ScatterMinTiling);
}  // namespace optiling
