...
bool ScatterMinTiling(const std::string& opType, const TeOpParas& opParas, const nlohmann::json& opCompileInfo,
  ...
  if (opCompileInfo == nullptr) {
    VECTOR_INNER_ERR_REPORT_TILIING(opType, "op [ScatterMinTiling] : opCompileInfo json error.");
    return false;
  }
  ...
  if (!is_valid_shape) {
    VECTOR_INNER_ERR_REPORT_TILIING(opType, "CheckScatterMinShape is failed.");
    return false;
  }
