...
bool GetV2CompileParams(const std::string& opType, const nlohmann::json& opCompileInfoJson, int32_t& coreNum,
...
bool GatherV2Tiling(const std::string& opType, const TeOpParas& opParas, const nlohmann::json& op_info,
  ...
  if (paramsRow * paramsDSize < BLOCK_SIZE) {
    ...
    if (paramsRow * paramsDSize % BLOCK_SIZE != 0) {  // not 32B aligned
      ...
      OP_LOGE("op[%s] GatherV2Tiling: inputs shape is not support now, x row size is not 32 bytes align.",
              opType.c_str());
      return false;
    } else {  // 32B aligned
