...
int32_t ComputeDivRemainders(const int32_t& num,
...
int32_t UssCeil(const int32_t& num,
                const int32_t& factor)
{
    int32_t  res;
...
int32_t UssCeilDiv(const int32_t& num,
                   const int32_t& factor)
{
    int32_t  res;
    res = (num % factor == 0) ? num / factor : num / factor + 1;
...
bool GetUssCompileParams(const std::string& op_type,
                         ...
                         int32_t& ub_tensor_num)
{
    ...
    if (op_compile_info_json == nullptr) {
        ...
        GE_LOGE("op [%s] op_compile_info_json is null", op_type.c_str());
        return false;
    }
    ...
    GELOGD("op [%s] : GetCompileParams, core_num[%d], ub_size[%d].", UNSORTED_SEGMENT_SUM_OP_TYPE.c_str(), core_num, ub_size);
    return true;
}

bool GetTilingMode(const std::vector<int64_t>& input_shape,
                   ...
                   int32_t& select_key)
{
    ...
    if (input_dtype == DTYPE_FP32) {
        if (input_dim == 1) {
            select_key = SELECT_KEY_MODE_FP32_INPUT_ONE_DIM;
        } else if (input_dim > 1) {
            if (e_size == 1) {
                select_key = SELECT_KEY_MODE_FP32_INPUT_LAST_AXIS_ONE;
...
bool GetEleDtype(const std::string& dtype,
                 EleByte& elebyte)
{
    if (dtype == "float32") {
        elebyte = FP32_BYTE;
...
bool IsUsingAllCore(const int32_t& ids_size,
...
bool IsUsingAllCoreByNumSegments(const int32_t& num_segments,
                                 const int32_t& core_num,
                                 int32_t& need_core_num)
{
...
void ComputeUbTensorSize(const int32_t& ub_size,
...
void ComputeEleNumOneCore(const int32_t& min_ele_num,
                          const int32_t& ids_num,
                          const int32_t& core_num,
                          const int32_t& e_size,
                          int32_t& ids_ele_num_front_core,
                          int32_t& ids_ele_num_last_core,
                          int32_t& input_ele_num_front_core,
                          int32_t& input_ele_num_last_core)
{
...
void ComputeInputParamsMovGm2ub(const int32_t& ub_tensor_size,
                                const EleByte& input_ele_byte,
                                const int32_t& input_ele_num,
                                const int32_t& e_size,
                                const int32_t& e_mov_times_gm2ub,
                                const int32_t& ids_ele_num_ub,
...
void ComputeIdsParamsMovGm2ub(const int32_t& ids_ele_num_one_core,
                              const int32_t& ub_tensor_size,
                              const EleByte& ids_ele_byte,
                              int32_t& ids_mov_times_gm2ub,
                              int32_t& ids_front_burst_len,
                              int32_t& ids_last_burst_len,
                              int32_t& ids_ele_num_ub_front_part,
                              int32_t& ids_ele_num_ub_last_part)
{
    ...
    if (ids_ele_num_one_core <= max_ids_ele_num_one_ub_tensor) {
        ...
        if (ids_ele_num_one_core % max_ids_ele_num_one_ub_tensor == 0) {
            ...
            ids_ele_num_ub_front_part = max_ids_ele_num_one_ub_tensor;
            ids_ele_num_ub_last_part = ComputeDivRemainders(ids_ele_num_one_core, max_ids_ele_num_one_ub_tensor, ids_mov_times_gm2ub - 1);
        }
    }
...
void ComputeUb2gmParams(const EleByte& ele_byte,
                        const int32_t e_num,
                        const int32_t ub_tensor_size,
                        int32_t& e_mov_times_gm2ub,
                        int32_t& e_ub2gm_front_burst_len,
                        int32_t& e_num_front_part,
                        int32_t& e_ub2gm_last_burst_len,
                        int32_t& e_num_last_part)
{
...
void ComputeInitOutputUbParams(const int32_t& ids_ele_num,
                               const int32_t& output_ub_ele_num_one_row,
                               int32_t& output_ub_init_last_repeat_time,
                               int32_t& output_ub_init_times)
{
    ...
    if (repeat_times % MAX_REPEAT_TIME == 0) {
        ...
        output_ub_init_last_repeat_time = ComputeDivRemainders(repeat_times, repeat_times / MAX_REPEAT_TIME, MAX_REPEAT_TIME);
    }
}

void ComputeNumSegmentsParams(const int32_t& need_core_num,
                              const int32_t& num_segmens,
                              int32_t& num_segmens_front_core,
...
void ComputeENumParams(const int32_t& e_num,
                       const EleByte& ele_byte,
                       const int32_t& ub_tensor_size,
                       int32_t& e_mov_times_gm2ub_input_scalar,
                       int32_t& e_ub2gm_front_burst_len_input_scalar,
                       int32_t& e_num_front_part_input_scalar,
                       ...
                       int32_t& repeat_time_last_part_input_scalar)
{
    ...
    if (e_num >= max_ele_num_one_ub_tensor) {
        ...
        // last part
        e_num_last_part_input_scalar = ComputeDivRemainders(e_num, e_num_front_part_input_scalar, e_mov_times_gm2ub_input_scalar - 1);
...
void WriteTilingParams(const TilingParamsFp32& params,
                       OpRunInfo& run_info)
{
    // common params
...
void WriteTilingParams(const TilingParamsInt32& params,
                       OpRunInfo& run_info)
{
    // common params
    ByteBufferPut(run_info.tiling_data, params.select_key_input_scalar);
                       ...
                       const TilingParamsFp32& params)
{
    ...
    GELOGD("op [%s] : params.input_ele_num_front_core_input_scalar=%d", op_type.c_str(), params.input_ele_num_front_core_input_scalar);
    GELOGD("op [%s] : params.input_mov_times_gm2ub_front_part_front_core_input_scalar=%d", op_type.c_str(), params.input_mov_times_gm2ub_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_front_burst_len_front_part_front_core_input_scalar=%d", op_type.c_str(), params.input_front_burst_len_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_last_burst_len_front_part_front_core_input_scalar=%d", op_type.c_str(), params.input_last_burst_len_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_front_ele_num_ub_front_part_front_core_input_scalar=%d", op_type.c_str(), params.input_front_ele_num_ub_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_last_ele_num_ub_front_part_front_core_input_scalar=%d", op_type.c_str(), params.input_last_ele_num_ub_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_front_rows_front_part_front_core_input_scalar=%d", op_type.c_str(), params.input_front_rows_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_last_rows_front_part_front_core_input_scalar=%d", op_type.c_str(), params.input_last_rows_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_mov_times_gm2ub_last_part_front_core_input_scalar=%d", op_type.c_str(), params.input_mov_times_gm2ub_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_front_burst_len_last_part_front_core_input_scalar=%d", op_type.c_str(), params.input_front_burst_len_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_last_burst_len_last_part_front_core_input_scalar=%d", op_type.c_str(), params.input_last_burst_len_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_front_ele_num_ub_last_part_front_core_input_scalar=%d", op_type.c_str(), params.input_front_ele_num_ub_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_last_ele_num_ub_last_part_front_core_input_scalar=%d", op_type.c_str(), params.input_last_ele_num_ub_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_front_rows_last_part_front_core_input_scalar=%d", op_type.c_str(), params.input_front_rows_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_last_rows_last_part_front_core_input_scalar=%d", op_type.c_str(), params.input_last_rows_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.input_ele_num_last_core_input_scalar=%d", op_type.c_str(), params.input_ele_num_last_core_input_scalar);
    GELOGD("op [%s] : params.input_mov_times_gm2ub_front_part_last_core_input_scalar=%d", op_type.c_str(), params.input_mov_times_gm2ub_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_front_burst_len_front_part_last_core_input_scalar=%d", op_type.c_str(), params.input_front_burst_len_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_last_burst_len_front_part_last_core_input_scalar=%d", op_type.c_str(), params.input_last_burst_len_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_front_ele_num_ub_front_part_last_core_input_scalar=%d", op_type.c_str(), params.input_front_ele_num_ub_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_last_ele_num_ub_front_part_last_core_input_scalar=%d", op_type.c_str(), params.input_last_ele_num_ub_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_front_rows_front_part_last_core_input_scalar=%d", op_type.c_str(), params.input_front_rows_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_last_rows_front_part_last_core_input_scalar=%d", op_type.c_str(), params.input_last_rows_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_mov_times_gm2ub_last_part_last_core_input_scalar=%d", op_type.c_str(), params.input_mov_times_gm2ub_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_front_burst_len_last_part_last_core_input_scalar=%d", op_type.c_str(), params.input_front_burst_len_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_last_burst_len_last_part_last_core_input_scalar=%d", op_type.c_str(), params.input_last_burst_len_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_front_ele_num_ub_last_part_last_core_input_scalar=%d", op_type.c_str(), params.input_front_ele_num_ub_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_last_ele_num_ub_last_part_last_core_input_scalar=%d", op_type.c_str(), params.input_last_ele_num_ub_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_front_rows_last_part_last_core_input_scalar=%d", op_type.c_str(), params.input_front_rows_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_last_rows_last_part_last_core_input_scalar=%d", op_type.c_str(), params.input_last_rows_last_part_last_core_input_scalar);
    ...
    GELOGD("op [%s] : params.e_ub2gm_front_burst_len_input_scalar=%d", op_type.c_str(), params.e_ub2gm_front_burst_len_input_scalar);
    GELOGD("op [%s] : params.e_num_front_part_input_scalar=%d", op_type.c_str(), params.e_num_front_part_input_scalar);
    GELOGD("op [%s] : params.e_ub2gm_last_burst_len_input_scalar=%d", op_type.c_str(), params.e_ub2gm_last_burst_len_input_scalar);
    ...
    GELOGD("op [%s] : params.ids_ele_num_front_core_input_scalar=%d", op_type.c_str(), params.ids_ele_num_front_core_input_scalar);
    GELOGD("op [%s] : params.ids_mov_times_gm2ub_front_core_input_scalar=%d", op_type.c_str(), params.ids_mov_times_gm2ub_front_core_input_scalar);
    GELOGD("op [%s] : params.ids_front_burst_len_front_core_input_scalar=%d", op_type.c_str(), params.ids_front_burst_len_front_core_input_scalar);
    GELOGD("op [%s] : params.ids_last_burst_len_front_core_input_scalar=%d", op_type.c_str(), params.ids_last_burst_len_front_core_input_scalar);
    GELOGD("op [%s] : params.ids_ele_num_ub_front_part_front_core_input_scalar=%d", op_type.c_str(), params.ids_ele_num_ub_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.ids_ele_num_ub_last_part_front_core_input_scalar=%d", op_type.c_str(), params.ids_ele_num_ub_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.ids_ele_num_last_core_input_scalar=%d", op_type.c_str(), params.ids_ele_num_last_core_input_scalar);
    GELOGD("op [%s] : params.ids_mov_times_gm2ub_last_core_input_scalar=%d", op_type.c_str(), params.ids_mov_times_gm2ub_last_core_input_scalar);
    GELOGD("op [%s] : params.ids_front_burst_len_last_core_input_scalar=%d", op_type.c_str(), params.ids_front_burst_len_last_core_input_scalar);
    GELOGD("op [%s] : params.ids_last_burst_len_last_core_input_scalar=%d", op_type.c_str(), params.ids_last_burst_len_last_core_input_scalar);
    GELOGD("op [%s] : params.ids_ele_num_ub_front_part_last_core_input_scalar=%d", op_type.c_str(), params.ids_ele_num_ub_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.ids_ele_num_ub_last_part_last_core_input_scalar=%d", op_type.c_str(), params.ids_ele_num_ub_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.output_ub_init_last_repeat_time_front_part_front_core_input_scalar=%d", op_type.c_str(), params.output_ub_init_last_repeat_time_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.output_ub_init_times_front_part_front_core_input_scalar=%d", op_type.c_str(), params.output_ub_init_times_front_part_front_core_input_scalar);
    GELOGD("op [%s] : params.output_ub_init_last_repeat_time_last_part_front_core_input_scalar=%d", op_type.c_str(), params.output_ub_init_last_repeat_time_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.output_ub_init_times_last_part_front_core_input_scalar=%d", op_type.c_str(), params.output_ub_init_times_last_part_front_core_input_scalar);
    GELOGD("op [%s] : params.output_ub_init_last_repeat_time_front_part_last_core_input_scalar=%d", op_type.c_str(), params.output_ub_init_last_repeat_time_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.output_ub_init_times_front_part_last_core_input_scalar=%d", op_type.c_str(), params.output_ub_init_times_front_part_last_core_input_scalar);
    GELOGD("op [%s] : params.output_ub_init_last_repeat_time_last_part_last_core_input_scalar=%d", op_type.c_str(), params.output_ub_init_last_repeat_time_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.output_ub_init_times_last_part_last_core_input_scalar=%d", op_type.c_str(), params.output_ub_init_times_last_part_last_core_input_scalar);
    GELOGD("op [%s] : params.input_last_axis_align_front_part_ele_num_input_scalar=%d", op_type.c_str(), params.input_last_axis_align_front_part_ele_num_input_scalar);
    GELOGD("op [%s] : params.input_last_axis_align_floor_ele_num_input_scalar=%d", op_type.c_str(), params.input_last_axis_align_floor_ele_num_input_scalar);
    GELOGD("op [%s] : params.last_part_vadd_mask_input_scalar=%d", op_type.c_str(), params.last_part_vadd_mask_input_scalar);
}

void PrintTilingParams(const std::string& op_type,
                       const TilingParamsInt32& params)
{
    GELOGD("op [%s] : params.select_key_input_scalar=%d", op_type.c_str(), params.select_key_input_scalar);
    GELOGD("op [%s] : params.need_core_num_input_scalar=%d", op_type.c_str(), params.need_core_num_input_scalar);
    GELOGD("op [%s] : params.num_segments_front_core_input_scalar=%d", op_type.c_str(), params.num_segments_front_core_input_scalar);
    GELOGD("op [%s] : params.num_segments_last_core_input_scalar=%d", op_type.c_str(), params.num_segments_last_core_input_scalar);
    GELOGD("op [%s] : params.ids_size_input_scalar=%d", op_type.c_str(), params.ids_size_input_scalar);
    GELOGD("op [%s] : params.ids_mov_times_gm2ub_input_scalar=%d", op_type.c_str(), params.ids_mov_times_gm2ub_input_scalar);
    GELOGD("op [%s] : params.ids_ele_num_ub_front_part_input_scalar=%d", op_type.c_str(), params.ids_ele_num_ub_front_part_input_scalar);
    GELOGD("op [%s] : params.ids_front_burst_len_input_scalar=%d", op_type.c_str(), params.ids_front_burst_len_input_scalar);
    GELOGD("op [%s] : params.ids_ele_num_ub_last_part_input_scalar=%d", op_type.c_str(), params.ids_ele_num_ub_last_part_input_scalar);
    GELOGD("op [%s] : params.ids_last_burst_len_input_scalar=%d", op_type.c_str(), params.ids_last_burst_len_input_scalar);
    GELOGD("op [%s] : params.e_num_input_scalar=%d", op_type.c_str(), params.e_num_input_scalar);
    GELOGD("op [%s] : params.e_mov_times_gm2ub_input_scalar=%d", op_type.c_str(), params.e_mov_times_gm2ub_input_scalar);
    GELOGD("op [%s] : params.e_ub2gm_front_burst_len_input_scalar=%d", op_type.c_str(), params.e_ub2gm_front_burst_len_input_scalar);
    GELOGD("op [%s] : params.e_num_front_part_input_scalar=%d", op_type.c_str(), params.e_num_front_part_input_scalar);
    GELOGD("op [%s] : params.repeat_time_front_part_input_scalar=%d", op_type.c_str(), params.repeat_time_front_part_input_scalar);
    GELOGD("op [%s] : params.e_ub2gm_last_burst_len_input_scalar=%d", op_type.c_str(), params.e_ub2gm_last_burst_len_input_scalar);
    GELOGD("op [%s] : params.e_num_last_part_input_scalar=%d", op_type.c_str(), params.e_num_last_part_input_scalar);
    GELOGD("op [%s] : params.repeat_time_last_part_input_scalar=%d", op_type.c_str(), params.repeat_time_last_part_input_scalar);
}

// tiling function
bool UnsortedSegmentSumTiling(const std::string& op_type,
                              ...
                              OpRunInfo& run_info)
{
    ...
    if (op_paras.inputs.empty()) {
        ge::OpsOneInputShapeErrReport("UnsortedSegmentSum", "op_paras.inputs",
            "op_paras.inputs is empty.");
        GE_LOGE("op[%s] op_paras.inputs is empty.", op_type.c_str());
        return false;
    }
    if (op_paras.inputs.size() < 2) {
        ge::OpsOneInputShapeErrReport("UnsortedSegmentSum", "op_paras.inputs",
            "op_paras.inputs.size() < 2.");
        GE_LOGE("op[%s] op_paras.inputs.size() < 2.", op_type.c_str());
        return false;
    }
    if (op_paras.inputs[0].tensor.empty()) {
        ge::OpsOneInputShapeErrReport("UnsortedSegmentSum", "input_data",
            "input_data tensor is empty.");
        GE_LOGE("op[%s] input_data tensor is empty.", op_type.c_str());
        return false;
    }
    if (op_paras.inputs[1].tensor.empty()) {
        ge::OpsOneInputShapeErrReport("UnsortedSegmentSum", "ids",
            "ids tensor is empty.");
        GE_LOGE("op[%s] ids tensor is empty.", op_type.c_str());
        return false;
    }
    ...
    if (input_shape.size() < ids_shape.size()) {
        ...
        GE_LOGE("op[%s] dim of input must be greater than or equal with dim of ids", op_type.c_str());
        return false;
    }
    for (unsigned i = 0; i < ids_shape.size(); i++) {
        if (input_shape[i] != ids_shape[i]) {
            ...
            GE_LOGE("op[%s] front shape of input must be equal with ids shape", op_type.c_str());
            return false;
        }
    }
    ...
    flag = GetEleDtype(input_dtype,
                       input_ele_byte);
    if (!flag) {
        ge::OpsOneInputShapeErrReport("UnsortedSegmentSum", "input_data",
            "get input_ele_byte failed.");
        GE_LOGE("op[%s] get input_ele_byte failed.", op_type.c_str());
        return false;
    }
    ...
    flag = GetEleDtype(ids_dtype,
                       ids_ele_byte);
    if (!flag) {
        ge::OpsOneInputShapeErrReport("UnsortedSegmentSum", "ids",
            "get ids_ele_byte failed.");
        GE_LOGE("op[%s] get ids_ele_byte failed.", op_type.c_str());
        return false;
    }
    ...
    int32_t ub_tensor_num = 0;
    flag = GetUssCompileParams(op_type,
    ...
    if (input_dtype == DTYPE_FP32) {
        ...
        // select key
        flag = GetTilingMode(input_shape,
                             e_size,
                             input_dtype,
                             ub_tensor_ele_num,
                             params.select_key_input_scalar);
        if (!flag) {
            ge::OpsOneInputShapeErrReport("UnsortedSegmentSum", "tiling_mode",
                "GetTilingMode failed.");
            GE_LOGE("op[%s] GetTilingMode failed.", op_type.c_str());
            return false;
        }
        ...
        // is using all core
        flag = IsUsingAllCore(ids_size,
                              core_num,
                              params.need_core_num_input_scalar);
        ComputeEleNumOneCore(ids_min_ele_num,
                             ids_size,
                             params.need_core_num_input_scalar,
                             e_size,
        ...
        ComputeIdsParamsMovGm2ub(params.ids_ele_num_front_core_input_scalar,
                                 ub_tensor_size,
                                 ids_ele_byte,
        ...
        ComputeIdsParamsMovGm2ub(params.ids_ele_num_last_core_input_scalar,
                                 ub_tensor_size,
                                 ids_ele_byte,
        ...
        if (params.select_key_input_scalar == SELECT_KEY_MODE_FP32_INPUT_LAST_AXIS_ALIGN_SMALL_E) {
            ...
            // input data params
            // front part front core
            ComputeInputParamsMovGm2ub(ub_tensor_size,
                                       input_ele_byte,
                                       params.input_ele_num_front_core_input_scalar,
                                       e_size,
            ...
            // input data params
            // front part front core
            params.input_front_burst_len_front_part_front_core_input_scalar = params.ids_front_burst_len_front_core_input_scalar;
            params.input_front_ele_num_ub_front_part_front_core_input_scalar = params.ids_ele_num_ub_front_part_front_core_input_scalar;
            params.input_front_rows_front_part_front_core_input_scalar = params.input_front_ele_num_ub_front_part_front_core_input_scalar;
            // last part front core
            params.input_front_burst_len_last_part_front_core_input_scalar = params.ids_last_burst_len_front_core_input_scalar;
            params.input_front_ele_num_ub_last_part_front_core_input_scalar = params.ids_ele_num_ub_last_part_front_core_input_scalar;
            params.input_front_rows_last_part_front_core_input_scalar = params.input_front_ele_num_ub_last_part_front_core_input_scalar;
            // front part last core
            params.input_front_burst_len_front_part_last_core_input_scalar = params.ids_front_burst_len_last_core_input_scalar;
            params.input_front_ele_num_ub_front_part_last_core_input_scalar = params.ids_ele_num_ub_front_part_last_core_input_scalar;
            params.input_front_rows_front_part_last_core_input_scalar = params.input_front_ele_num_ub_front_part_last_core_input_scalar;
            // last part last core
            params.input_front_burst_len_last_part_last_core_input_scalar = params.ids_last_burst_len_last_core_input_scalar;
            params.input_front_ele_num_ub_last_part_last_core_input_scalar = params.ids_ele_num_ub_last_part_last_core_input_scalar;
            params.input_front_rows_last_part_last_core_input_scalar= params.input_front_ele_num_ub_last_part_last_core_input_scalar;

            // output init params
            // front part front core
            ComputeInitOutputUbParams(params.ids_ele_num_ub_front_part_front_core_input_scalar,
            ...
            // last part front core
            ComputeInitOutputUbParams(params.ids_ele_num_ub_last_part_front_core_input_scalar,
                                      output_ub_ele_num_one_row,
            ...
            // front part last core
            ComputeInitOutputUbParams(params.ids_ele_num_ub_front_part_last_core_input_scalar,
                                      output_ub_ele_num_one_row,
            ...
            // last part last core
            ComputeInitOutputUbParams(params.ids_ele_num_ub_last_part_last_core_input_scalar,
                                      output_ub_ele_num_one_row,
            ...
            ComputeInitOutputUbParams(params.ids_ele_num_ub_last_part_last_core_input_scalar,
                                      ...
                                      params.output_ub_init_last_repeat_time_last_part_last_core_input_scalar,
                                      params.output_ub_init_times_last_part_last_core_input_scalar);
            params.input_last_axis_align_front_part_ele_num_input_scalar = e_size / FP32_ELE_NUM_ALIGN_32B * FP32_ELE_NUM_ALIGN_32B;
            ...
            params.e_num_last_part_input_scalar = ComputeDivRemainders(e_size, params.e_num_front_part_input_scalar, params.e_mov_times_gm2ub_input_scalar - 1);
            params.e_ub2gm_last_burst_len_input_scalar = params.e_num_last_part_input_scalar * input_ele_byte / BYTE_BLOCK;

            // input data params
            // front part front core
            params.input_mov_times_gm2ub_front_part_front_core_input_scalar = params.ids_ele_num_ub_front_part_front_core_input_scalar;
            ...
            // last part front core
            params.input_mov_times_gm2ub_last_part_front_core_input_scalar = params.ids_ele_num_ub_last_part_front_core_input_scalar;
            ...
            // front part last core
            params.input_mov_times_gm2ub_front_part_last_core_input_scalar = params.ids_ele_num_ub_front_part_last_core_input_scalar;
            ...
            // last part last core
            params.input_mov_times_gm2ub_last_part_last_core_input_scalar = params.ids_ele_num_ub_last_part_last_core_input_scalar;
            ...
            // output init params
            params.input_last_axis_align_front_part_ele_num_input_scalar = params.e_num_last_part_input_scalar / FP32_ELE_NUM_ALIGN_32B * FP32_ELE_NUM_ALIGN_32B;
            params.input_last_axis_align_floor_ele_num_input_scalar = UssCeil(params.e_num_last_part_input_scalar, FP32_ELE_NUM_ALIGN_32B);
            params.last_part_vadd_mask_input_scalar = params.e_num_last_part_input_scalar - params.input_last_axis_align_front_part_ele_num_input_scalar;
            ...
            // output init params
            params.output_ub_init_times_front_part_front_core_input_scalar = UssCeilDiv(params.ids_ele_num_ub_front_part_front_core_input_scalar, MASK_FP32);
            params.output_ub_init_times_last_part_front_core_input_scalar = UssCeilDiv(params.ids_ele_num_ub_last_part_front_core_input_scalar, MASK_FP32);
            params.output_ub_init_times_front_part_last_core_input_scalar = UssCeilDiv(params.ids_ele_num_ub_front_part_last_core_input_scalar, MASK_FP32);
            params.output_ub_init_times_last_part_last_core_input_scalar = UssCeilDiv(params.ids_ele_num_ub_last_part_last_core_input_scalar, MASK_FP32);
            params.last_part_vadd_mask_input_scalar = params.ids_ele_num_ub_last_part_last_core_input_scalar - (params.output_ub_init_times_last_part_last_core_input_scalar - 1) * MASK_FP32;
        }
        ...
        if (op_paras.const_inputs.find(key_num_segments) == op_paras.const_inputs.end()) {
            ge::OpsOneInputShapeErrReport("UnsortedSegmentSum", "num_segments",
                "num_segments not exists.");
            GE_LOGE("op[%s] num_segments not exists.", op_type.c_str());
            return false;
        }
        const int32_t* num_segments_ptr = reinterpret_cast<const int32_t*>(std::get<0>(op_paras.const_inputs.at(key_num_segments)));
        ...
        IsUsingAllCoreByNumSegments(num_segments,
                                    core_num,
                                    params.need_core_num_input_scalar);
        ComputeNumSegmentsParams(params.need_core_num_input_scalar,
                                 num_segments,
                                 params.num_segments_front_core_input_scalar,
                                 params.num_segments_last_core_input_scalar);
        // ids params
        params.ids_size_input_scalar = ids_size;
        ComputeIdsParamsMovGm2ub(ids_size,
                                 ub_tensor_size,
                                 ids_ele_byte,
        ...
        // e num params
        params.e_num_input_scalar = e_size;
        ComputeENumParams(params.e_num_input_scalar,
                          input_ele_byte,
                          ub_tensor_size,
                          params.e_mov_times_gm2ub_input_scalar,
                          params.e_ub2gm_front_burst_len_input_scalar,
                          params.e_num_front_part_input_scalar,
