...
namespace optiling {
bool IsInAxisReduceStdV2Update(std::vector<int32_t>& input, int32_t value) {
...
bool ReduceStdV2UpdateTiling(const std::string& op_type, const TeOpParas& op_paras, const nlohmann::json& op_info,
                             OpRunInfo& run_info) {
  ...
  if (op_info.count("reduce_mean_cof_dtype") > 0) {
    ...
    for (uint32_t i = 0; i < input_shape.size(); i++) {
      if (IsInAxisReduceStdV2Update(reduce_axis, i)) {
...
REGISTER_OP_TILING_FUNC_BUFFERED(ReduceStdV2Update, ReduceStdV2UpdateTiling);
}  // namespace optiling
