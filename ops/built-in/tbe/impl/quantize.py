...
class Quantize(object):
    ...
    def __init__(self, input_x, scales, zero_points, axis, dtype, kernel_name):
        ...
        self.dtype_bytes_size_x = tp.cce_intrin.get_bit_len(self.input_x_dtype) // 8
        self.data_each_block_x = 32 // self.dtype_bytes_size_x
        self.dtype_bytes_size_scales = tp.cce_intrin.get_bit_len(self.scales_dtype) // 8
        self.data_each_block_scales = 32 // self.dtype_bytes_size_scales
        self.dtype_bytes_size_zero_points = tp.cce_intrin.get_bit_len(self.zero_points_dtype) // 8
        self.data_each_block_zero_points = 32 // self.dtype_bytes_size_zero_points
        self.determine_upper_and_lower_bounds(dtype)
        self.dtype_bytes_size_y = tp.cce_intrin.get_bit_len(self.output_y_dtype) // 8
        ...
        if(self.per_tensor):
            ...
            self.ub_size = tp.cce_conf.get_soc_spec(tp.cce_conf.UB_SIZE) - ext_size * 2
            if(self.x_need_cast):
        ...
        if (self.in_num <= self.ub_tensor_size):
            ...
            self.ai_core_num = tp.cce_conf.get_soc_spec(tp.cce_conf.CORE_NUM)
    ...
    def quantize_compute_each_loop(self, move_offset, move_num):
        ...
        if(self.per_tensor):
            self.tik_instance.data_move(self.input_scales_ub,
                                        self.input_scales_gm,
                                        0, 1, 1, 0, 0)
            self.tik_instance.data_move(self.input_zero_points_ub,
                                        self.input_zero_points_gm,
                                        0, 1, 1, 0, 0)
            if(self.zero_points_dtype != "int32"):
                # zp from int8/uint8 to int32
                self.tik_instance.vec_conv(self.data_each_block_zero_points, '',
                                           self.input_zero_points_ub_fp16,
                                           self.input_zero_points_ub,
                                           1, 8, 8)
                self.tik_instance.vec_conv(self.data_each_block_zero_points, '',
                                           self.input_zero_points_ub_fp32,
                                           self.input_zero_points_ub_fp16,
                                           1, 8, 8)
            else:
                ...
                self.tik_instance.vec_conv(self.data_each_block_zero_points, '',
                                           ...
                                           1, 8, 8)
            # scale to 1/scale
            self.tik_instance.vec_rec_high_preci(self.data_each_block_scales,
                                                 self.input_scales_ub_fp32_t,
                                                 self.input_scales_ub,
                                                 self.wk_tensor,
                                                 1, 8, 8)
            ...
            self.tik_instance.data_move(self.input_scales_ub,
                                        self.input_scales_gm[move_offset],
                                        0, 1, burse_len_scale, 0, 0)
            self.tik_instance.data_move(self.input_zero_points_ub,
                                        self.input_zero_points_gm[move_offset],
                                        0, 1, burse_len_zp, 0, 0)
            # zp from int32 to fp32
    ...
    def per_channel_x_zero(self, num):
        ...
        for loop_id in range(loop):
            ...
            if(self.x_need_cast):
                self.tik_instance.vec_cmpv_eq(self.is_x_zero,
                                              self.input_x_ub_fp32[compute_offset],
                                              self.zero_compare_tensor,
                                              1, 8, 0)
            else:
                self.tik_instance.vec_cmpv_eq(self.is_x_zero,
                                              ...
                                              self.zero_compare_tensor,
                                              1, 8, 0)
            self.tik_instance.vec_cmpv_eq(self.is_scale_zero,
                                          self.input_scales_ub[compute_offset],
                                          self.zero_compare_tensor,
                                          1, 8, 0)
            self.tik_instance.vec_and(self.max_mask_x_dtype,
                                      self.is_zero,
                                      self.is_x_zero,
                                      self.is_scale_zero,
                                      1, 8, 8, 8)
            self.tik_instance.vec_sel(self.max_mask_x_dtype, 0,
                                      self.tmp_x_ub[compute_offset],
                                      self.is_zero,
                                      self.min_vale_tensor,
                                      self.tmp_x_ub[compute_offset],
                                      1, 8, 8)
        ...
        if(last_num > 0):
            ...
            self.tik_instance.vec_cmpv_eq(self.is_scale_zero,
                                          ...
                                          self.zero_compare_tensor,
                                          1, 8, 0)
            self.tik_instance.vec_and(last_num,
                                      self.is_zero,
                                      self.is_x_zero,
                                      self.is_scale_zero,
                                      1, 8, 8, 8)
    ...
    def quantize_per_channel(self, num):
        ...
        if (self.ub_tensor_size >= self.max_mask_x_dtype * 255):
            if(loop > 0):
                for index in range(loop):
                    ...
                    if(self.x_need_cast):
                        self.tik_instance.vec_mul(self.max_mask_x_dtype,
                                                  self.tmp_x_ub[compute_offset],
                                                  self.input_x_ub_fp32[compute_offset],
                        ...
                        self.tik_instance.vec_mul(self.max_mask_x_dtype,
                                                  ...
                                                  self.input_scales_ub_fp32_t[compute_offset],
                                                  255, 8, 8, 8)
                    self.tik_instance.vec_add(self.max_mask_x_dtype,
                                              self.tmp_x_ub[compute_offset],
                                              self.tmp_x_ub[compute_offset],
        ...
        if(last_num > 0):
            if(self.x_need_cast):
                self.tik_instance.vec_mul(last_num,
                                          self.tmp_x_ub[compute_offset],
                                          self.input_x_ub_fp32[compute_offset],
                                          self.input_scales_ub_fp32_t[compute_offset],
                                          1, 8, 8, 8)
    ...
    def scale_t(self, num):
        ...
        last_num = num % self.max_mask_x_dtype
        if(last_num > 0):
            self.tik_instance.vec_rec_high_preci(last_num,
                                                 self.input_scales_ub_fp32_t[compute_offset:],
                                                 self.input_scales_ub[compute_offset:],
                                                 self.wk_tensor,
                                                 1, 8, 8)
            ...
            if(self.ub_tensor_size >= self.max_mask_x_dtype * 255):
                if(loop > 0):
                    for index in range(loop):
                        ...
                        self.tik_instance.vec_conv(self.max_mask_x_dtype, '',
                                                   self.input_zero_points_ub_fp32[compute_offset],
                                                   self.input_zero_points_ub[compute_offset],
                                                   255, 8, 8)
            compute_offset = loop * self.max_mask_x_dtype * 255
            repeat_time = num % (self.max_mask_x_dtype * 255) // self.max_mask_x_dtype
            if(repeat_time > 0):
        ...
        if(self.ub_tensor_size >= mask_num * 255):
            if(loop > 0):
                for index in range(loop):
                    ...
                    self.tik_instance.vec_conv(mask_num, '',
                                               self.input_zero_points_ub_fp16[compute_offset],
                                               self.input_zero_points_ub[compute_offset],
                                               255, 8, 4)
        ...
        if (self.ub_tensor_size >= self.max_mask_x_dtype * 255):
            if(loop > 0):
                for index in range(loop):
                    compute_offset = index * self.max_mask_x_dtype * 255
                    self.tik_instance.vec_conv(self.max_mask_x_dtype, '',
                                               self.output_y_fp16[compute_offset],
                                               self.int32_tensor[compute_offset],
                                               255, 4, 8, 1.0)
        compute_offset = loop * self.max_mask_x_dtype * 255
        repeat_time = num % (self.max_mask_x_dtype * 255) // self.max_mask_x_dtype
        if(repeat_time > 0):
        ...
        if(self.ub_tensor_size >= max_mask * 255):
            if(loop > 0):
                for index in range(loop):
                    ...
                    self.tik_instance.vec_conv(max_mask, '',
                                               self.output_y_int[compute_offset],
                                               self.output_y_fp16[compute_offset],
                                               255, 4, 8)
    ...
    def fp32x_to_int32(self, num):
        ...
        if (self.ub_tensor_size >= self.max_mask_x_dtype * 255):
            if(loop > 0):
                for index in range(loop):
                    compute_offset = index * self.max_mask_x_dtype * 255
                    self.tik_instance.vec_conv(self.max_mask_x_dtype, 'round',
                                               self.int32_tensor[compute_offset],
                                               self.tmp_x_ub[compute_offset],
                                               255, 8, 8)
        compute_offset = loop * self.max_mask_x_dtype * 255
    ...
    def min_max_tensor(self, num):
        ...
        if (self.ub_tensor_size >= self.max_mask_x_dtype * 255):
            if(loop > 0):
                for index in range(loop):
                    compute_offset = index * self.max_mask_x_dtype * 255
                    self.tik_instance.vec_min(self.max_mask_x_dtype,
                                              self.int32_tensor[compute_offset],
                                              self.int32_tensor[compute_offset],
                                              self.compare_int32_max_tensor,
                                              255, 8, 8, 0)
                    self.tik_instance.vec_max(self.max_mask_x_dtype,
                                              self.int32_tensor[compute_offset],
                                              self.int32_tensor[compute_offset],
                                              self.compare_int32_min_tensor,
                                              255, 8, 8, 0)
        compute_offset = loop * self.max_mask_x_dtype * 255
        repeat_time = num % (self.max_mask_x_dtype * 255) // self.max_mask_x_dtype
        if(repeat_time > 0):
        ...
        last_num = num % self.max_mask_x_dtype
        if(last_num > 0):
            self.tik_instance.vec_min(last_num,
                                      self.int32_tensor[compute_offset],
                                      self.int32_tensor[compute_offset],
                                      self.compare_int32_max_tensor,
                                      1, 8, 8, 0)
            self.tik_instance.vec_max(last_num,
                                      self.int32_tensor[compute_offset],
                                      self.int32_tensor[compute_offset],
                                      self.compare_int32_min_tensor,
                                      1, 8, 8, 0)
    ...
    def quantize_per_tensor(self, num):
        ...
        if (self.ub_tensor_size >= self.max_mask_x_dtype * 255):
            if(loop > 0):
                for index in range(loop):
                    compute_offset = index * self.max_mask_x_dtype * 255
                    if(self.x_need_cast):
                        self.tik_instance.vec_muls(self.max_mask_x_dtype,
                                                   self.tmp_x_ub[compute_offset],
                                                   self.input_x_ub_fp32[compute_offset],
                                                   self.scale_scalar_fp32,
                                                   255, 8, 8)
                    else:
                        self.tik_instance.vec_muls(self.max_mask_x_dtype,
                                                   self.tmp_x_ub[compute_offset],
                                                   ...
                                                   self.scale_scalar_fp32,
                                                   255, 8, 8)
                    self.tik_instance.vec_adds(self.max_mask_x_dtype,
                                               self.tmp_x_ub[compute_offset],
                                               self.tmp_x_ub[compute_offset],
                                               self.zp_scalar_fp32, 255, 8, 8)
        compute_offset = loop * self.max_mask_x_dtype * 255
        repeat_time = num % (self.max_mask_x_dtype * 255) // self.max_mask_x_dtype
        if(repeat_time > 0):
            if(self.x_need_cast):
        ...
        if(last_num > 0):
            if(self.x_need_cast):
                self.tik_instance.vec_muls(last_num,
                                           self.tmp_x_ub[compute_offset],
                                           self.input_x_ub_fp32[compute_offset],
                                           self.scale_scalar_fp32,
                                           1, 8, 8)
                ...
                self.tik_instance.vec_muls(last_num,
                                           ...
                                           self.scale_scalar_fp32,
                                           1, 8, 8)
            self.tik_instance.vec_adds(last_num, self.tmp_x_ub[compute_offset],
                                       self.tmp_x_ub[compute_offset],
                                       self.zp_scalar_fp32,
                                       1, 8, 8)
    ...
    def init_ub_tensor_and_scalar(self):
        ...
        self.tmp_x_ub = self.tik_instance.Tensor("float32",
                                                 (self.ub_tensor_size,),
                                                 name="tmp_x_ub",
                                                 scope=tik.scope_ubuf)
        if(self.per_tensor):
            ...
            self.input_scales_ub = self.tik_instance.Tensor(self.scales_dtype,
                                                            (self.data_each_block_scales,),
            ...
            self.input_zero_points_ub = self.tik_instance.Tensor(self.zero_points_dtype,
                                                                 ...
                                                                 scope=tik.scope_ubuf)
            self.wk_tensor = self.tik_instance.Tensor("float32",
                                                      (self.max_mask_x_dtype * 2,),
                                                      name="wk_tensor",
                                                      scope=tik.scope_ubuf)
            self.input_scales_ub_fp32_t = self.tik_instance.Tensor("float32",
                                                                   (self.data_each_block_scales,),
                                                                   name="input_scales_ub_fp32_t",
                                                                   scope=tik.scope_ubuf)
            self.input_zero_points_ub_fp32 = self.tik_instance.Tensor("float32",
                                                                      (self.data_each_block_zero_points,),
            ...
            if(self.zero_points_dtype != "int32"):
                self.input_zero_points_ub_fp16 = self.tik_instance.Tensor("float16",
                                                                          ...
                                                                          scope=tik.scope_ubuf)
            if(self.x_need_cast):
                self.input_x_ub_fp32 = self.tik_instance.Tensor("float32",
                                                                (self.ub_tensor_size,),
            ...
            if(self.input_x_dtype == "float16"):
                self.input_x_ub_fp32 = self.tik_instance.Tensor("float32",
                                                                ...
                                                                name="input_x_ub_fp32",
                                                                scope=tik.scope_ubuf)
        self.zero_compare_tensor = self.tik_instance.Tensor("float32",
                                                            (self.max_mask_x_dtype,),
                                                            name="zero_compare_tensor",
                                                            scope=tik.scope_ubuf)
        self.scale_compare_tensor = self.tik_instance.Tensor("float32",
                                                             (self.max_mask_x_dtype,),
        ...
        self.min_vale_tensor = self.tik_instance.Tensor("float32",
                                                        (self.max_mask_x_dtype,),
        ...
        self.is_x_zero = self.tik_instance.Tensor('uint16',
                                                  (self.max_mask_x_dtype,),
                                                  name="is_x_zero",
                                                  scope=tik.scope_ubuf)
        self.is_scale_zero = self.tik_instance.Tensor('uint16',
                                                      (self.max_mask_x_dtype,),
                                                      name="is_scale_zero",
                                                      scope=tik.scope_ubuf)
        self.is_zero = self.tik_instance.Tensor('uint16',
                                                (self.max_mask_x_dtype,),
                                                name="is_zero",
                                                scope=tik.scope_ubuf)
        self.int32_tensor = self.tik_instance.Tensor("int32",
                                                     (self.ub_tensor_size,),
                                                     name="int32_tensor",
                                                     scope=tik.scope_ubuf)
        self.compare_int32_min_tensor = self.tik_instance.Tensor("int32",
                                                                 (self.max_mask_x_dtype,),
                                                                 name="compare_int32_min_tensor",
                                                                 scope=tik.scope_ubuf)
        self.compare_int32_max_tensor = self.tik_instance.Tensor("int32",
                                                                 (self.max_mask_x_dtype,),
        ...
        if (self.output_y_dtype != "int32"):
            self.output_y_fp16 = self.tik_instance.Tensor("float16",
                                                          (self.ub_tensor_size,),
                                                          name="output_y_fp16",
                                                          scope=tik.scope_ubuf)
            if(self.output_y_dtype == "int8"):
                self.output_y_int = self.tik_instance.Tensor("int8",
                                                             (self.ub_tensor_size,),
                                                             name="output_y_int",
                                                             scope=tik.scope_ubuf)
            elif(self.output_y_dtype == "uint8"):
                self.output_y_int = self.tik_instance.Tensor("uint8",
                                                             (self.ub_tensor_size,),
                                                             name="output_y_int",
                                                             scope=tik.scope_ubuf)
...
def quantize(x, scales, zero_points, y, dtype, axis=1, kernel_name="quantize_per_channel"):
    ...
    if (dtype_scale not in check_scale_tuple):
        raise RuntimeError("Scales only support %s while dtype is %s" %
                           (",".join(check_scale_tuple), dtype_scale))
    if (dtype_zp not in check_zp_tuple):
        raise RuntimeError("Zero_points only support %s while dtype is %s" %
                           (",".join(check_zp_tuple), dtype_zp))
    if (dtype not in check_dtype):
        raise RuntimeError("Dtype only support %s while dtype is %s" %
                           (",".join(check_dtype), dtype))
