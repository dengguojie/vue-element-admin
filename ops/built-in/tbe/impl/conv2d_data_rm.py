...
def conv2d_data_rm_compute(input_tensor, res_tensor=None):
    ...
    if type(input_tensor) != tvm.tensor.Tensor:
        err_man_cube.raise_err_three_paras("E67018", "Input", "a tensor", type(input_tensor))
    if input_tensor.dtype not in ("int8", "float16"):
        err_man_cube.raise_err_three_paras("E60032", "Wrong input_tensor", "int8 and float16", input_tensor.dtype)
