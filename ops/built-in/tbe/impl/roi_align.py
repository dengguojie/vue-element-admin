...
class RoiAlign:
    ...
    def __init__(self, feature_map_dict,
        ...
        self.flowtable_scale_fp32 = [0.0625, 0.0625, 0.0625, 0.0625, 0.0625, 0.0625]
    ...
    def roialign_compute_each_core(self, block_num, block_offset, block_left):
        ...
        rois_block_ub = self.tik_inst.Tensor("float16", (128, 16), \
                                             scope=tbe_platform.scope_ubuf, name="rois_block_ub")
        feature_map_ub = self.tik_inst.Tensor("float16", (self.max_fmap_ub_size * 16, ), \
                                              scope=tbe_platform.scope_ubuf, name="feature_map")
        output_ub = self.tik_inst.Tensor("float16", \
                                         (self.repeat * self.pooled_w * 8 * C0SIZE, ), \
                                         scope=tbe_platform.scope_ubuf, name="output_ub")
        val_ub = self.tik_inst.Tensor("float16", (self.repeat * ONEVECTOR * 4, ), \
                                      scope=tbe_platform.scope_ubuf, name="val_ub")

        batch_index_int32 = self.tik_inst.Tensor("int32", (128, ), \
                                                 scope=tbe_platform.scope_ubuf, name="batch_index_int32")

        y_low_int = self.tik_inst.Tensor("int32", (128, ), \
                                         scope=tbe_platform.scope_ubuf, name="y_low_int")
        y_high_int = self.tik_inst.Tensor("int32", (128, ), \
                                          scope=tbe_platform.scope_ubuf, name="y_high_int")
        x_low_int = self.tik_inst.Tensor("int32", (128, ), \
                                         scope=tbe_platform.scope_ubuf, name="x_low_int")
        x_high_int = self.tik_inst.Tensor("int32", (128, ), \
                                          scope=tbe_platform.scope_ubuf, name="x_high_int")

        ly = self.tik_inst.Tensor("float16", (128, ), \
                                  scope=tbe_platform.scope_ubuf, name="ly")
        lx = self.tik_inst.Tensor("float16", (128, ), \
                                  scope=tbe_platform.scope_ubuf, name="lx")
        hy = self.tik_inst.Tensor("float16", (128, ), \
                                  scope=tbe_platform.scope_ubuf, name="hy")
        ...
        if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS", "SD3403"):
            ...
            roi_gridw_fp32 = self.tik_inst.Tensor("float32", (128, ), \
                                                  scope=tbe_platform.scope_ubuf, name="roi_gridw_fp32")
            x_start_fp32_ub = self.tik_inst.Tensor("float32", (ROINUM,), \
                                                   name="x_start_fp32_ub", scope=tbe_platform.scope_ubuf)
            y_start_fp32_ub = self.tik_inst.Tensor("float32", (ROINUM,), \
                                                   name="y_start_fp32_ub", scope=tbe_platform.scope_ubuf)
        else:
        ...
        bin_scale = self.tik_inst.Tensor("float16", (128, ), \
                                         scope=tbe_platform.scope_ubuf, name="bin_scale")

        x_start_ub = self.tik_inst.Tensor("float16", (ROINUM,), \
                                          name="x_start_ub", scope=tbe_platform.scope_ubuf)
        ...
        with self.tik_inst.new_stmt_scope():
            ...
            if cce_product in ("Hi3796CV300CS", "SD3403"):
                ...
                self.tik_inst.vec_conv(64, '', index_arr, index_arr_int32, 2, 4, 8, 1.0)
                ...
                with self.tik_inst.if_scope(
                                        ...
                                        if self.repeat == 1:
                                            ...
                                            if left_mask != 0:
                                                self.tik_inst.vmuls(left_mask, val_ub[(loop_repeat - 1) * ONEVECTOR], \
                                                                    feature_map_ub[(loop_repeat - 1) * s * ONEVECTOR +
                                                                                   id1], \
                                                                    hx0, 1, 1, s, 8, 8)
                                                self.tik_inst.vmuls(left_mask, val_ub[(loop_repeat - 1) * ONEVECTOR +
                                                                                      len_c1], \
                    ...
                    with self.tik_inst.else_scope():
                        ...
                        with self.tik_inst.for_range(0, self.input_c1) as loop_j:
                            # mov ub to out
                            self.tik_inst.data_move(self.output_gm[block_offset + loop_b * ROINUM + loop_n, loop_j,
                                                                   0, 0, 0], \
    ...
    def roi_align_perf_scale(self, rois, xstart, roisnum, sample_ratio,
        ...
        with self.tik_inst.new_stmt_scope():
            cmp_buf = self.tik_inst.Tensor("float16", (ROINUM,), \
                                           name="cmp_buf_ub", scope=tbe_platform.scope_ubuf)
            cmp_res = self.tik_inst.Tensor("uint16", (ROINUM,), \
                                           name="cmp_res_ub", scope=tbe_platform.scope_ubuf)
            temp_ub = self.tik_inst.Tensor("uint16", (ROINUM,), \
                                           name="temp_ub", scope=tbe_platform.scope_ubuf)
            cmp16_ub = self.tik_inst.Tensor("uint16", (ROINUM,), \
                                            name="cmp16_ub", scope=tbe_platform.scope_ubuf)
    ...
    def roialign_perf_gen_grid(self,
        ...
        if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS", "SD3403"):
            ...
            xhigh_fp32 = tik_instance.Tensor("float32", (ROINUM, ), \
                                             name="xhigh_fp32", scope=tbe_platform.scope_ubuf)
            yhigh_fp32 = tik_instance.Tensor("float32", (ROINUM, ), \
                                             name="yhigh_fp32", scope=tbe_platform.scope_ubuf)

            lx_fp32 = tik_instance.Tensor("float32", (ROINUM, ), \
                                          name="lx_fp32", scope=tbe_platform.scope_ubuf)
            ly_fp32 = tik_instance.Tensor("float32", (ROINUM, ), \
                                          name="ly_fp32", scope=tbe_platform.scope_ubuf)
            hx_fp32 = tik_instance.Tensor("float32", (ROINUM, ), \
                                          name="hx_fp32", scope=tbe_platform.scope_ubuf)
            hy_fp32 = tik_instance.Tensor("float32", (ROINUM, ), \
                                          name="hy_fp32", scope=tbe_platform.scope_ubuf)
        else:
            dtype_num = 2
        ...
        xpos = tik_instance.Tensor("float16", (ROINUM, ), \
                                   name="xpos", scope=tbe_platform.scope_ubuf)
        ypos = tik_instance.Tensor("float16", (ROINUM, ), \
                                   name="ypos", scope=tbe_platform.scope_ubuf)

        xlow = tik_instance.Tensor("float16", (ROINUM, ), \
                                   name="xlow", scope=tbe_platform.scope_ubuf)
        ylow = tik_instance.Tensor("float16", (ROINUM, ), \
                                   name="ylow", scope=tbe_platform.scope_ubuf)
        xhigh = tik_instance.Tensor("float16", (ROINUM, ), \
                                    name="xhigh", scope=tbe_platform.scope_ubuf)
        yhigh = tik_instance.Tensor("float16", (ROINUM, ), \
                                    name="yhigh", scope=tbe_platform.scope_ubuf)

        cmp_buf = tik_instance.Tensor("float16", (ROINUM, ), \
                                      name="cmp_buf", scope=tbe_platform.scope_ubuf)
        cmp_xy = tik_instance.Tensor("float16", (ROINUM, ), \
                                     name="cmp_xy", scope=tbe_platform.scope_ubuf)
        cmp_const = tik_instance.Tensor("float16", (ROINUM, ), \
                                        name="cmp_const", scope=tbe_platform.scope_ubuf)

        lx_pos_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                         name="lx_pos_cmp", scope=tbe_platform.scope_ubuf)
        ...
        hy_pos_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                         name="hy_pos_cmp", scope=tbe_platform.scope_ubuf)
        lx_neg_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                         name="lx_neg_cmp", scope=tbe_platform.scope_ubuf)
        ly_neg_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                         name="ly_neg_cmp", scope=tbe_platform.scope_ubuf)
        hx_neg_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                         name="hx_neg_cmp", scope=tbe_platform.scope_ubuf)
        hy_neg_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                         name="hy_neg_cmp", scope=tbe_platform.scope_ubuf)
        xlow_pos_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                           name="xlow_pos_cmp", scope=tbe_platform.scope_ubuf)
        ylow_pos_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                           name="ylow_pos_cmp", scope=tbe_platform.scope_ubuf)
        xhigh_pos_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                            name="xhigh_pos_cmp", scope=tbe_platform.scope_ubuf)
        yhigh_pos_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                            name="yhigh_pos_cmp", scope=tbe_platform.scope_ubuf)
        xlow_neg_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                           name="xlow_neg_cmp", scope=tbe_platform.scope_ubuf)
        ylow_neg_cmp = tik_instance.Tensor("float16", (ROINUM, ), \
                                           name="ylow_neg_cmp", scope=tbe_platform.scope_ubuf)
...
def roi_align_cce(feature_map_dict, rois_dict, roisn_dict, output_dict, \
    ...
    if cce_product == tbe_platform.HI3796CV300ES:
...
def _get_grid_weight_per_roi(tik_instance, roi_bin_h_fp32_value,
    ...
    roi_bin_ph_gh_hy_int32 = tik_instance.Tensor(
        "int32", [1], name="roi_bin_ph_gh_hy_int32", scope=tbe_platform.scope_ubuf)
    roi_bin_ph_gh_ly_int32 = tik_instance.Tensor(
        "int32", [1], name="roi_bin_ph_gh_ly_int32", scope=tbe_platform.scope_ubuf)
    roi_bin_ph_gh_pos_fp32 = tik_instance.Tensor(
        dtype, [1], name="roi_bin_ph_gh_pos_fp32", scope=tbe_platform.scope_ubuf)
    roi_bin_ph_gh_ly_weight = tik_instance.Tensor(
        dtype, [1], name="roi_bin_ph_gh_ly_weight", scope=tbe_platform.scope_ubuf)
    roi_bin_ph_gh_hy_weight = tik_instance.Tensor(
        dtype, [1], name="roi_bin_ph_gh_hy_weight", scope=tbe_platform.scope_ubuf)
    tmp_float16 = tik_instance.Tensor(
        "float16", [1], name="tmp_float16", scope=tbe_platform.scope_ubuf)

    const_one = tik_instance.Tensor(
        "int32", [1], name="const_one", scope=tbe_platform.scope_ubuf)
    ...
    const_value_fp32 = tik_instance.Tensor(
        dtype, [1], name="const_value_fp32", scope=tbe_platform.scope_ubuf)
    ...
    if support_div is False:
        _newton(tik_instance, 1, tmp_bin_h_fp32, tmp_bin_h_fp32,
    ...
    roi_y_floor = tik_instance.Tensor(
        "int32", [1], name="roi_y_floor", scope=tbe_platform.scope_ubuf)
...
def _compute_w1234(tik_instance, h_y, l_y, h_x, l_x, w1_lt, w2_rt, w3_lb, w4_rb,
    ...
    if dtype == "float32" and vconvf_suppot is True:
        ...
        ly_tensor = tik_instance.Tensor( \
            dtype, [1], name="ly_tensor", scope=tbe_platform.scope_ubuf)
        ly_tensor[0].set_as(l_y)
        hx_tensor = tik_instance.Tensor( \
            dtype, [1], name="hx_tensor", scope=tbe_platform.scope_ubuf)
        hx_tensor[0].set_as(h_x)
        lx_tensor = tik_instance.Tensor( \
            dtype, [1], name="lx_tensor", scope=tbe_platform.scope_ubuf)
        lx_tensor[0].set_as(l_x)

        w1_tensor = tik_instance.Tensor( \
            dtype, [1], name="w1_tensor", scope=tbe_platform.scope_ubuf)
        w2_tensor = tik_instance.Tensor( \
            dtype, [1], name="w2_tensor", scope=tbe_platform.scope_ubuf)
        w3_tensor = tik_instance.Tensor( \
            dtype, [1], name="w3_tensor", scope=tbe_platform.scope_ubuf)
        w4_tensor = tik_instance.Tensor( \
            dtype, [1], name="w4_tensor", scope=tbe_platform.scope_ubuf)
...
def _get_average(tik_instance, grid_curr_h_f32, grid_curr_w_f32, val, c_valid,
    ...
    if vconvs32_suppot is False or dtype == "float16":
        grid_h_num_f_tensor = tik_instance.Tensor( \
            dtype, [1], name="grid_h_num_f_tensor", scope=tbe_platform.scope_ubuf)
        grid_h_num_f_tensor[0].set_as(grid_curr_h_f32)
        grid_w_num_f_tensor = tik_instance.Tensor( \
            dtype, [1], name="grid_w_num_f_tensor", scope=tbe_platform.scope_ubuf)
        grid_w_num_f_tensor[0].set_as(grid_curr_w_f32)
        h_w_tensor = tik_instance.Tensor( \
            dtype, [1], name="h_w_tensor", scope=tbe_platform.scope_ubuf)
    ...
    with tik_instance.for_range(0, pool_h) as p_h:
        with tik_instance.for_range(0, pool_w, thread_num=thread_num) as p_w:
            ...
            val = tik_instance.Tensor(
                dtype, [c_block, 16], name="val", scope=tbe_platform.scope_ubuf)
                ...
                with tik_instance.if_scope(c_valid != 0):
                    ...
                    with tik_instance.for_range(0, grid_curr_h) as g_h:
                        ...
                        with tik_instance.if_scope(verify == 0):
                            ...
                            if w_number_ub >= 2:
                                _load_a_w_to_l1(tik_instance, cache_table, cache_ub, feature_map,
                    ...
                    _get_average(tik_instance, grid_curr_h_f32,
...
def _prepare_vbi_xm(tik_instance, h_y, l_y, h_x, l_x, c1_block_num):
    ...
    """
    hy_tensor = tik_instance.Tensor( \
        "float16", [1], name="hy_tensor", scope=tbe_platform.scope_ubuf)
    hy_tensor[0].set_as(h_y)
    ly_tensor = tik_instance.Tensor( \
        "float16", [1], name="ly_tensor", scope=tbe_platform.scope_ubuf)
    ly_tensor[0].set_as(l_y)
    hx_tensor = tik_instance.Tensor( \
        "float16", [1], name="hx_tensor", scope=tbe_platform.scope_ubuf)
    hx_tensor[0].set_as(h_x)
    lx_tensor = tik_instance.Tensor( \
...
def _bilinear_interpolate(tik_instance, x_lo_w, x_hi_w, y_lo_w, y_hi_w, x_lo,
    ...
    val = tik_instance.Tensor(
        dtype, [fm_c1, pw_int, 16], name="val", scope=tbe_platform.scope_ubuf)

    tik_instance.vec_dup(16, val, 0.0, fm_c1 * pw_s, n_bust)

    roi_y_floor = tik_instance.Tensor(
        "int32", [128], name="roi_y_floor", scope=tbe_platform.scope_ubuf)
    ...
    with tik_instance.for_range(0, ph_int * sample_num_h) as grid_num_h:
        ...
        with tik_instance.if_scope(verify == 0):
            ...
            if w_number_ub >= 2:
                ...
                _load_a_w_to_l1(tik_instance, cache_table, cache_l1, feature_map,
                                index, y_low, n_bust, 0)
                _load_a_w_to_l1(tik_instance, cache_table, cache_l1, feature_map,
                                index, y_high, n_bust, 1)
            ...
            with tik_instance.for_range(0, pw_int * sample_num_w) as grid_num_w:
                ...
                with tik_instance.if_scope(verify == 0):
                    w1_lt = tik_instance.Tensor(
                        dtype, [fm_c1, 16], name="w1_lt", scope=tbe_platform.scope_ubuf)
                    w2_rt = tik_instance.Tensor(
                        dtype, [fm_c1, 16], name="w2_rt", scope=tbe_platform.scope_ubuf)
                    w3_lb = tik_instance.Tensor(
                        dtype, [fm_c1, 16], name="w3_lb", scope=tbe_platform.scope_ubuf)
                    w4_rb = tik_instance.Tensor(
                        dtype, [fm_c1, 16], name="w4_rb", scope=tbe_platform.scope_ubuf)
                    x_low = tik_instance.Scalar(
        ...
        with tik_instance.if_scope((grid_num_h + 1) % sample_num_h == 0):
            _get_average(tik_instance, grid_h_num_f, grid_w_num_f, val, fm_c1,
                         pw_s, n_bust)
...
def _get_grid_weight(tik_instance, grid_w, grid_h, rois_start_w, rois_start_h,
                     height, width, dtype):
    """
    get grid size and coordinate in feature
    """
    x_lo_w = tik_instance.Tensor(
        dtype, [128], name="x_lo_w", scope=tbe_platform.scope_ubuf)
    x_hi_w = tik_instance.Tensor(
        dtype, [128], name="x_hi_w", scope=tbe_platform.scope_ubuf)
    y_lo_w = tik_instance.Tensor(
        dtype, [128], name="y_lo_w", scope=tbe_platform.scope_ubuf)
    y_hi_w = tik_instance.Tensor(
        dtype, [128], name="_lo_w", scope=tbe_platform.scope_ubuf)
    ...
    raw_x = tik_instance.Tensor(
        dtype, [128], name="raw_x", scope=tbe_platform.scope_ubuf)
    raw_y = tik_instance.Tensor(
        dtype, [128], name="raw_y", scope=tbe_platform.scope_ubuf)
    x_output = tik_instance.Tensor(
        dtype, [128], name="x_output", scope=tbe_platform.scope_ubuf)
    y_output = tik_instance.Tensor(
        dtype, [128], name="y_output", scope=tbe_platform.scope_ubuf)
    tmp_fp16 = tik_instance.Tensor(
        "float16", [128], name="tmp_fp16", scope=tbe_platform.scope_ubuf)

    const_value_0_127 = tik_instance.Tensor(
        dtype, (128,), name="const_value_0_127", scope=tbe_platform.scope_ubuf)
    if dtype == "float32":
        dtype_num = 1
    else:
        dtype_num = 2
    ...
    grid_w_vector = tik_instance.Tensor(
        dtype, [128], name="grid_w_vector", scope=tbe_platform.scope_ubuf)
    grid_h_vector = tik_instance.Tensor(
        dtype, [128], name="grid_h_vector", scope=tbe_platform.scope_ubuf)
    ...
    if vconv_f322s32f_suppot is False or dtype == "float16":
        ...
        point_05_tensor = tik_instance.Tensor(
            dtype, [1], name="point_05_tensor", scope=tbe_platform.scope_ubuf)
    ...
    const_zero = tik_instance.Tensor(
        dtype, [64 * dtype_num], name="const_zero", scope=tbe_platform.scope_ubuf)
    ...
    const_value_fp32 = tik_instance.Tensor(
        dtype, [64 * dtype_num], name="const_value_fp32", scope=tbe_platform.scope_ubuf)
...
def roi_align_compute(tik_instance, feature_map, ret, proposals_ub_x0,
    ...
    if vdiv_suppot is False:
        _newton(tik_instance, 64 * dtype_num, grid_h, roi_bin_h_fp32_value,
                grid_h_fp32,
                2 // dtype_num, dtype)
        _newton(tik_instance, 64 * dtype_num, grid_w, roi_bin_w_fp32_value,
                grid_w_fp32,
                2 // dtype_num, dtype)
    else:
            ...
            with tik_instance.for_range(
                ...
                if (cce_product in (tbe_platform.ASCEND_610, tbe_platform.ASCEND_710)) and (dtype == "float16") \
                    ...
                    roi_fm_index = tik_instance.Tensor(
                        dtype, [128], name="roi_fm_index", scope=tbe_platform.scope_ubuf)
...
def roi_align(feature_map_dict,
    ...
    dtype = feature_map_dict.get("dtype")
    cce_product = tbe_platform.get_soc_spec(tbe_platform.SOC_VERSION)
    if roi_end_mode > 1 and cce_product in (tbe_platform.ASCEND_910, tbe_platform.ASCEND_920A):
        if dtype != "float32":
