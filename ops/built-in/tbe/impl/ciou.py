...
class CIoU():
    ...
    def __init__(self, bboxes, gtboxes, trans, is_cross, mode, atan_sub_flag):
        ...
        self.ciou_shape = [1, self.gtboxes_shape[1]]
        ...
        with self.tik_instance.for_range(0, self.core_num, block_num=self.core_num) as _core_id:
            ...
            if self.core_tail_num != 0:
                with self.tik_instance.if_scope(_core_id == (self.core_num - 1)):
                    ...
                    if self.core_num == 1:
                        dst_gm_offset = 0
                    self.ciou_process_one_loop(bb_loop, dst_gm_offset, repeat_time_max, bb_tail, bb_tail_offset)
    ...
    def ciou_process_one_loop(self, bb_loop, dst_gm_offset, repeat_time_max, bb_tail, bb_tail_offset):
        with self.tik_instance.for_range(0, bb_loop) as bb_loop_index:
            gm_point_offset = bb_loop_index * self.bb_ub_segment + dst_gm_offset
            self._run_segment(self.max_eliments, repeat_time_max, gm_point_offset)
        if bb_tail != 0:
            gm_point_offset = bb_tail_offset + dst_gm_offset
            if bb_tail // self.max_eliments > 0:
                self._run_segment(self.max_eliments, bb_tail // self.max_eliments, gm_point_offset)
