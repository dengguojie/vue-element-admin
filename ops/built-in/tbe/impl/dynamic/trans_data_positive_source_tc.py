        ...
        with tik_inst.for_range(0, vnc_row_cnt) as vnc_idx:
            with tik_inst.if_scope(vnc_idx != vnc_row_cnt - 1):
                tik_inst.data_move(src_ub[in_ub_offset + vnc_idx * vnc_col_size],
                                   src_in_gm[in_gm_offset + vnc_idx * loop_offset], 0, 1,
                                   tdc.ceil_div(data_size, ele_per_block), 0, 0)
            with tik_inst.else_scope():  # data maybe not enough for last line
                tik_inst.data_move(src_ub[in_ub_offset + vnc_idx * vnc_col_size],
                                   src_in_gm[in_gm_offset + vnc_idx * loop_offset], 0, 1,
                                   tdc.ceil_div(last_data_size, ele_per_block), 0, 0)
...
def _copy_data_in_1011(args):
    ...
    (tik_inst, src_in_gm, src_ub, in_gm_offset, in_ub_offset, vnc_line_cnt, dst_r2nd_step_in, pln_cl_size, pl_c_size,
     vnc_col_size, ele_per_block) = args

    with tik_inst.new_stmt_scope(disable_sync=True):
        with tik_inst.for_range(0, vnc_line_cnt) as vnc_idx:
            tik_inst.data_move(src_ub[in_ub_offset + vnc_idx * vnc_col_size],
                               src_in_gm[in_gm_offset + vnc_idx * dst_r2nd_step_in], 0, 1,
                               tdc.ceil_div(pln_cl_size * pl_c_size, ele_per_block), 0, 0)
