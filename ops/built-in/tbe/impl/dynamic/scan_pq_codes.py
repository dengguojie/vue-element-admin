...
class ScanPQCodes():
    ...
    def _init_gm_tensor(self):
        ...
        self.pq_distance_gm = self.tik_instance.Tensor(self.adc_tables_dtype, (_ceil_fill(self.total_limit, SLICE_SIZE),),
        ...
        with self.tik_instance.if_scope(bucket_idx // MAX_BUCKET_LEN > 0):
            with self.tik_instance.for_range(0, bucket_idx // MAX_BUCKET_LEN) as loop_idx:
                ...
                with self.tik_instance.for_range(0, MAX_BUCKET_LEN) as idx:
                    ...
                    output_count.set_as(output_count + _ceil_fill(bucket_counts, SLICE_SIZE))
        with self.tik_instance.if_scope(bucket_idx % MAX_BUCKET_LEN > 0):
            self.tik_instance.data_move(self.bucket_limits_ub_int32,
    ...
    def _handle_pq_distance(self, args):
        (bucket_offset_output, bucket_offset_max, slice_size) = args
        # extrim
    ...
    def _run_one_core_loop(self, args):
        ...
        thread_tail.set_as(bucket_limit % SLICE_SIZE)
        with self.tik_instance.for_range(0, thread_loop, thread_num=2) as thread_idx:
            ...
            self.block_extrim_ub_fp16 = self.tik_instance.Tensor(self.adc_tables_dtype, (BLOCK_FLOAT16 * 2, ),
        ...
        with self.tik_instance.if_scope(thread_tail > 0):
            ...
            args = (bucket_offset_input, thread_tail * IVF_UNIT_LEN, INNER_LOOP_TIME, thread_loop)
            self._handle_input_data(args)
            self.tik_instance.vadds(MASK_FLOAT16, self.pq_distance_ub_fp16,
                                    self.pq_distance_ub_fp16, bucket_base_dis,
                                    SLICE_SIZE // MASK_FLOAT16, 1, 1, 8, 8)
            ...
            self.tik_instance.data_move(self.pq_index_gm[bucket_offset_output + SLICE_SIZE * thread_loop],
                                        pq_index_ub_int32, 0, 1,
                                        SLICE_SIZE // BLOCK_INT32, 0, 0)
            self.tik_instance.data_move(self.grouped_extrim_distance_gm[bucket_offset_max + (SLICE_SIZE // self.group_size) * thread_loop],
                                        self.grouped_extrim_distance_ub_fp16, 0, 1, 1, 0, 0)
            self.tik_instance.data_move(self.pq_distance_gm[bucket_offset_output + SLICE_SIZE * thread_loop],
                                        self.pq_distance_ub_fp16, 0, 1,
                                        SLICE_SIZE // BLOCK_FLOAT16, 0, 0)
