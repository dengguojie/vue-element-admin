...
class MaxpoolGradCustom(MaxpoolGardObject):
    ...
    def tik_instance_cut_nc1_cut_one_h(self, block_id):
        ...
        # vector_repeat_time
        v_rep_time = col2img_dyw * self.channel * dtype_size // ONE_REPEAT
        ...
        v_rep_time_col = (2 * self.col2img_w * self.channel * self.col2img_h * dtype_size + ONE_REPEAT - 1) // ONE_REPEAT
        ...
        with self.tik_instance.if_scope(self.dyh * self.dyw * self.channel > (L1_SIZE // 2)):
            true_val.set_as(0)
        with self.tik_instance.else_scope():
            true_val.set_as(1)
            ...
            with self.tik_instance.for_range(0, self.dyh) as looph:  # 5
                with self.tik_instance.if_scope(true_val == 1):
                    self.tik_instance.data_move(data_max_ub, data_input_l1[looph * self.dyw * self.channel],
                                                ...
                                                self.dyw * self.channel * dtype_size // BLOCK_SIZE,
                                                constant.STRIDE_ZERO, constant.STRIDE_ZERO)
                with self.tik_instance.else_scope():
                    self.tik_instance.data_move(
                ...
                with self.tik_instance.if_scope(true_val == 1):
                    self._data_move_from_out_to_l1buf(
                        data_mask_l1, self.data_mask, block_num * mask_one_window * self.kernel_w * self.kernel_h +
                        looph * self.dyw, mask_one_window * self.kernel_w * self.kernel_h, dtype_size)
                ...
                with self.tik_instance.for_range(0, self.kernel_h * self.kernel_w) as mask_id:  # 169
                    ...
                    with self.tik_instance.else_scope():
                        self.tik_instance.data_move(data_mask_ub,
                                                    self.data_mask[block_num * mask_one_window * self.kernel_w * self.kernel_h +
                ...
                with self.tik_instance.else_scope():
                    ...
                    with self.tik_instance.if_scope(looph * self.stride_h < self.pad_top):
                        ...
                        src_address.set_as(src_address + (self.pad_top - looph * self.stride_h) * self.col2img_w * self.channel)
                    ...
                    self.tik_instance.data_move(data_vmul_ub_col2img_fp32[0],
                                                data_vmul_ub_col2img_fp32[self.stride_h * self.channel * self.col2img_w],
    ...
    def tik_instance_cut_nc1h_cut_one_h(self, block_id):
        ...
        # vector_repeat_time
        v_rep_time = wo_max * self.channel * dtype_size // ONE_REPEAT
        v_rep_cycle_fp32 = 2 * v_rep_time // V_MAX_REPEAT
        # v_rep_last
        v_rep_last_fp32 = 2 * v_rep_time % V_MAX_REPEAT
        ...
        with self.tik_instance.for_range(0, real_cycle) as cycle_id:
            ...
            with self.tik_instance.if_scope(self.hoverlap is 0):
                with self.tik_instance.if_scope(block_h != 0):
                    dxh_address_offset.set_as(dxh_address_offset + (block_h * self.ho_every * self.kernel_h - self.pad_top) *
                                              self.dxw * self.channel)
            with self.tik_instance.else_scope():
                with self.tik_instance.if_scope(block_h != 0):
                    dxh_address_offset.set_as(dxh_address_offset + ((block_h * (self.ho_every - 1) + 1) *
            ...
            with self.tik_instance.for_range(0, h_cycle) as looph:  # 3
                ...
                with self.tik_instance.else_scope():
                    in_src_address.set_as(in_src_address + (block_h * (self.ho_every - 1) + looph) * self.dyw * self.channel)
                ...
                self.tik_instance.data_move(data_max_ub, self.data_input[in_src_address], constant.SID,
                                            constant.DEFAULT_NBURST, self.dyw * self.channel * dtype_size // BLOCK_SIZE,
                                            constant.STRIDE_ZERO, constant.STRIDE_ZERO)

                self.clean_fp16_multi_repeat(data_mask_ub, dtype_size)

                # mask copy gm to ub
                self.tik_instance.data_move(data_mask_ub, self.data_mask[mask_address], constant.SID, self.kernel_h * self.kernel_w,
                ...
                with self.tik_instance.for_range(0, self.kernel_h * self.kernel_w) as mask_id:
                    ...
                    self.tik_instance.col2img(data_vmul_ub_col2img_fp32[0], data_vsel_ub_fp32[0], (0, 0, 0, 0),
                                              ...
                                              0, self.stride_w, self.stride_h, self.kernel_w, self.kernel_h, 1, 1, wo_max // 16)
