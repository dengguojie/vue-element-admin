...
class RoiAlignGrad():
    ...
    def _mov_data_ddr(self, x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index, sum_in_ub_flag):
        """mov_data_ddr
        """
        tik_instance = self.tik_instance
        h_num = self.x_height
        ...
        with tik_instance.if_scope(h_index < (h_num - 1)):
            with tik_instance.if_scope(w_index < (w_num - 1)):
                self._mov_data_ddr_all(x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index,
                                       sum_in_ub_flag)
            with tik_instance.else_scope():
                self._mov_data_ddr_onerow(x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index,
                                          sum_in_ub_flag)
        with tik_instance.else_scope():
            with tik_instance.if_scope(w_index < (w_num - 1)):
                self._mov_data_ddr_oneline(x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index,
                                           sum_in_ub_flag)
            with tik_instance.else_scope():
                self._mov_data_ddr_onepoint(x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index,
                                            sum_in_ub_flag)

    def _mov_data_ddr_onepoint(self, x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index,
                               sum_in_ub_flag):
        ...
        if sum_in_ub_flag:
            ...
            with tik_instance.if_scope(c1_gap <= 65535):
                ...
                tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[0, 0, 0], '', c1_num, 2, c1_gap, 6)
            with tik_instance.else_scope():
                with tik_instance.if_scope(w_gap <= 65535):
                    with tik_instance.for_range(0, c1_num) as i:
                        x_offset = self._get_x_diff_offset(image_index, start_c1 + i, h_index, w_index)
                        tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[i, 0, 0], '', 2, 4, w_gap, 0)
                with tik_instance.else_scope():
                    with tik_instance.for_range(0, c1_num) as i:
                        x_offset = self._get_x_diff_offset(image_index, start_c1 + i, h_index, w_index)
                        tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[i, 0, 0], '', 1, 2, 0, 0)

    def _mov_data_ddr_oneline(self, x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index, sum_in_ub_flag):
        """move xdiff to gm
        """
        tik_instance = self.tik_instance
        h_num = self.x_height
        ...
        if sum_in_ub_flag:
            ...
            with tik_instance.if_scope(repeat_str <= 255):
                x_offset = self._get_temp_ub_offset(image_index, start_c1, h_index, w_index)
                tik_instance.vadd(32, x_diff[x_offset], x_diff[x_offset], x_diff_ub[0, 0, 0], c1_num, 1, 1, 1,
                                  repeat_str, repeat_str, 8)
            with tik_instance.else_scope():
                with tik_instance.for_range(0, c1_num) as i:
                    x_offset = self._get_temp_ub_offset(image_index, start_c1 + i, h_index, w_index)
                    tik_instance.vadd(32, x_diff[x_offset], x_diff[x_offset], x_diff_ub[i, 0, 0], 1, 1, 1, 1, 8, 8, 8)
        else:
            ...
            with tik_instance.if_scope(c1_gap <= 65535):
                x_offset = self._get_x_diff_offset(image_index, start_c1, h_index, w_index)
                tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[0, 0, 0], '', c1_num, 4, c1_gap, 4)
            with tik_instance.else_scope():
                with tik_instance.if_scope(w_gap <= 65535):
                    with tik_instance.for_range(0, c1_num) as i:
                        x_offset = self._get_x_diff_offset(image_index, start_c1 + i, h_index, w_index)
                ...
                with tik_instance.else_scope():
                    with tik_instance.for_range(0, c1_num) as i:
                        x_offset = self._get_x_diff_offset(image_index, start_c1 + i, h_index, w_index)
                        tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[i, 0, 0], '', 1, 4, 0, 0)

    def _mov_data_ddr_onerow(self, x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index, sum_in_ub_flag):
        """move xdiff to gm
        """
        tik_instance = self.tik_instance
        h_num = self.x_height
        ...
        if sum_in_ub_flag:
            ...
            with tik_instance.if_scope(repeat_str <= 255):
                ...
                tik_instance.vadd(16, x_diff[x_offset], x_diff[x_offset], x_diff_ub[0, 2, 0], c1_num, 1, 1, 1,
                                  repeat_str, repeat_str, 8)
            with tik_instance.else_scope():
                with tik_instance.for_range(0, c1_num) as i:
                    x_offset = self._get_temp_ub_offset(image_index, start_c1 + i, h_index, w_index)
                    ...
                    tik_instance.vadd(16, x_diff[x_offset], x_diff[x_offset], x_diff_ub[i, 2, 0], 1, 1, 1, 1, 8, 8, 8)
        else:
            c1_gap = (((h_num * w_num) - 1) * 16 * 4) // 32
            w_gap = ((w_num - 2) * 16 * 4) // 32
            # `c1_gap, w_gap = self._modify_value(c1_gap, w_gap)`
            with tik_instance.if_scope(c1_gap <= 65535):
                ...
                tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[0, 2, 0], '', c1_num, 2, c1_gap, 6)
            ...
            with tik_instance.else_scope():
                ...
                with tik_instance.else_scope():
                    with tik_instance.for_range(0, c1_num) as i:
                        ...
                        tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[i, 2, 0], '', 1, 2, 0, 0)

    def _mov_data_ddr_all(self, x_diff, x_diff_ub, image_index, start_c1, c1_num, h_index, w_index, sum_in_ub_flag):
        """mov_data_ddr_all
        """
        tik_instance = self.tik_instance
        h_num = self.x_height
        ...
        if sum_in_ub_flag:
            ...
            with tik_instance.if_scope(repeat_str <= 255):
                ...
                tik_instance.vadd(32, x_diff[x_offset], x_diff[x_offset], x_diff_ub[0, 0, 0], c1_num, 1, 1, 1,
                                  repeat_str, repeat_str, 8)
                x_offset = self._get_temp_ub_offset(image_index, start_c1, h_index + 1, w_index)
                tik_instance.vadd(32, x_diff[x_offset], x_diff[x_offset], x_diff_ub[0, 2, 0], c1_num, 1, 1, 1,
                                  repeat_str, repeat_str, 8)
            with tik_instance.else_scope():
                with tik_instance.for_range(0, c1_num) as i:
                    x_offset = self._get_temp_ub_offset(image_index, start_c1 + i, h_index, w_index)
                    tik_instance.vadd(32, x_diff[x_offset], x_diff[x_offset], x_diff_ub[i, 0, 0], 1, 1, 1, 1, 8, 8, 8)
                    x_offset = self._get_temp_ub_offset(image_index, start_c1 + i, h_index + 1, w_index)
                    tik_instance.vadd(32, x_diff[x_offset], x_diff[x_offset], x_diff_ub[i, 2, 0], 1, 1, 1, 1, 8, 8, 8)
        else:
            c1_gap = (((h_num * w_num) - 2) * 16 * 4) // 32
            w_gap = ((w_num - 2) * 16 * 4) // 32
            # `c1_gap, w_gap = self._modify_value(c1_gap, w_gap)`
            with tik_instance.if_scope(c1_gap <= 65535):
                ...
                tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[0, 0, 0], '', c1_num, 4, c1_gap, 4)
                x_offset = self._get_x_diff_offset(image_index, start_c1, h_index + 1, w_index)
                tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[0, 2, 0], '', c1_num, 4, c1_gap, 4)
            with tik_instance.else_scope():
                with tik_instance.if_scope(w_gap <= 65535):
                    with tik_instance.for_range(0, c1_num) as i:
                        x_offset = self._get_x_diff_offset(image_index, start_c1 + i, h_index, w_index)
                ...
                with tik_instance.else_scope():
                    with tik_instance.for_range(0, c1_num) as i:
                        ...
                        tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[i, 0, 0], '', 1, 4, 0, 0)
                        x_offset = self._get_x_diff_offset(image_index, start_c1 + i, h_index + 1, w_index)
                        tik_instance.tensor_mov(x_diff[x_offset], x_diff_ub[i, 2, 0], '', 1, 4, 0, 0)

    def _calc_w_vec(self, x_lo_w, x_hi_w, y_lo_w, y_hi_w):
        tik_instance = self.tik_instance
        w1_vec = tik_instance.Tensor("float32", (128,), name="w1", scope=tbe_platform.scope_ubuf)
        w2_vec = tik_instance.Tensor("float32", (128,), name="w2", scope=tbe_platform.scope_ubuf)
        w3_vec = tik_instance.Tensor("float32", (128,), name="w3", scope=tbe_platform.scope_ubuf)
        w4_vec = tik_instance.Tensor("float32", (128,), name="w4", scope=tbe_platform.scope_ubuf)
    ...
    def _calc_x_diff_ub(self, w1_vec, w2_vec, w3_vec, w4_vec, row_num_i, available_c1_num, x_ind, x_lo, x_hi, y_hi,
                        y_diff_ub, c1_num, calc_c1_num):
        tik_instance = self.tik_instance
        ...
        x_diff_ub = tik_instance.Tensor("float32", [available_c1_num, 4, 16],
                                        name="x_diff_ub",
                                        scope=tbe_platform.scope_ubuf)
        tmp_result = tik_instance.Tensor("float32", [3 * available_c1_num * 16],
                                         name="tmp_result",
                                         scope=tbe_platform.scope_ubuf)
        ...
        tik_instance.vmuls(16, x_diff_ub[0, 0, 0], y_diff_ub[0, 0, grad_index_reg, 0], w1_reg, calc_c1_num, 1, 1, 8,
                           pool_w * 2)
        tik_instance.vmuls(16, tmp_result[0], y_diff_ub[0, 0, grad_index_reg, 0], w2_reg, calc_c1_num, 1, 1, 2,
                           pool_w * 2)
        tik_instance.vmuls(16, tmp_result[c1_num * 16], y_diff_ub[0, 0, grad_index_reg, 0], w3_reg, calc_c1_num, 1, 1,
                           2, pool_w * 2)
        tik_instance.vmuls(16, tmp_result[c1_num * 2 * 16], y_diff_ub[0, 0, grad_index_reg, 0], w4_reg, calc_c1_num, 1,
                           1, 2, pool_w * 2)

        tik_instance.vadd(16, x_diff_ub[0, w2_loc_reg, 0], x_diff_ub[0, w2_loc_reg, 0], tmp_result[0], calc_c1_num, 1,
                          1, 1, 8, 8, 2)
        tik_instance.vadd(16, x_diff_ub[0, w3_loc_reg, 0], x_diff_ub[0, w3_loc_reg, 0], tmp_result[c1_num * 16],
                          calc_c1_num, 1, 1, 1, 8, 8, 2)
        tik_instance.vadd(16, x_diff_ub[0, w4_loc_reg, 0], x_diff_ub[0, w4_loc_reg, 0], tmp_result[c1_num * 2 * 16],
                          calc_c1_num, 1, 1, 1, 8, 8, 2)

        return x_diff_ub, x_lo_reg

    def _roi_align_calc_grad_line_align_mode1(self, x_lo_w, x_hi_w, y_lo_w, y_hi_w, x_lo, x_hi, y_hi, y_lo, row_num,
                                              y_diff_ub, x_ind, image_index, start_c1, calc_c1_num):
        ...
        with tik_instance.new_stmt_scope():
            ...
            with tik_instance.for_range(0, row_num, thread_num=thread_num) as i:
                x_diff_ub, x_lo_reg = self._calc_x_diff_ub(w1_vec, w2_vec, w3_vec, w4_vec, i, available_c1_num, x_ind,
                                                           x_lo, x_hi, y_hi, y_diff_ub, c1_num, calc_c1_num)

                self._mov_data_ddr(self.x_diff, x_diff_ub, image_index, start_c1, calc_c1_num, y_lo, x_lo_reg, False)

    def _roi_align_calc_grad_line_align_mode2(self, x_lo_w, x_hi_w, y_lo_w, y_hi_w, x_lo, x_hi, y_hi, y_lo, row_num,
                                              y_diff_ub, x_ind, image_index, start_c1, calc_c1_num):
        """calc one line gradient
        """
        tik_instance = self.tik_instance
        ...
        with tik_instance.new_stmt_scope():
            ...
            with tik_instance.for_range(0, row_num, thread_num=thread_num) as i:
                x_diff_ub, x_lo_reg = self._calc_x_diff_ub(w1_vec, w2_vec, w3_vec, w4_vec, i, available_c1_num, x_ind,
                                                           x_lo, x_hi, y_hi, y_diff_ub, c1_num, calc_c1_num)
                self._mov_data_ddr(tmp_ub, x_diff_ub, 0, start_c1, calc_c1_num, 0, x_lo_reg, True)

            with tik_instance.if_scope(y_lo < (h_num - 1)):
                x_diff_offset = self._get_x_diff_offset(image_index, 0, y_lo, 0)
                tik_instance.data_move(self.x_diff[x_diff_offset], tmp_ub, 0, c1_num, w_num * 2 * 16 // 8, 0,
                                       h_num * w_num * 16 // 8 - w_num * 2 * 16 // 8)
            with tik_instance.else_scope():
                x_diff_offset = self._get_x_diff_offset(image_index, 0, y_lo, 0)
                tik_instance.data_move(self.x_diff[x_diff_offset], tmp_ub, 0, c1_num, w_num * 16 // 8, 0,
                                       h_num * w_num * 16 // 8 - w_num * 16 // 8)
    ...
    def _get_ydiff_line(self, y_diff_ub, n_index, start_c1, calc_c1_num, line_index):
        ...
        if c1_gap <= 65535:
            ...
            tik_instance.tensor_mov(y_diff_ub[0], self.y_diff[y_diff_offset], '', calc_c1_num, w_num * 2, 0, c1_gap)
        else:
            with tik_instance.for_range(0, calc_c1_num) as i:
                y_diff_offset = self._get_y_diff_offset(n_index, start_c1 + i, line_index, 0)
                tik_instance.tensor_mov(y_diff_ub[0], self.y_diff[y_diff_offset], '', 1, w_num * 2, 0, 0)
    ...
    def _malloc_res(self):
        ...
        c1_size = 16
        y_diff_ub = tik_instance.Tensor("float32", [c1_size, 1, pool_w, 16],
                                        name="y_diff_ub",
                                        scope=tbe_platform.scope_ubuf)

        return y_diff_ub

    def _roi_align_calc_grad_block_align(self, line_num, row_num, x_lo_w, x_hi_w, y_lo_w, y_hi_w, x_lo, x_hi, y_lo,
                                         y_hi, n_index, x_ind, y_ind, image_index, sum_in_ub_flag):
        ...
        with tik_instance.for_range(0, line_num) as i:
            ...
            if not sum_in_ub_flag:
                self._roi_align_calc_grad_line_align_mode1(x_lo_w, x_hi_w, y_lo_w_s, y_hi_w_s, x_lo, x_hi, y_hi_s,
                                                           y_lo_s, row_num, y_diff_ub, x_ind, image_index, 0,
                                                           calc_c1_num)
            else:
                self._roi_align_calc_grad_line_align_mode2(x_lo_w, x_hi_w, y_lo_w_s, y_hi_w_s, x_lo, x_hi, y_hi_s,
                                                           y_lo_s, row_num, y_diff_ub, x_ind, image_index, 0,
                                                           calc_c1_num)

    def _roi_align_calc_grid(self, h_ind, w_ind, const_value_0_127, grid_w, grid_h, rois_start_w, rois_start_h):
        """calc one block gradient
        """
        tik_instance = self.tik_instance
        x_lo_w = tik_instance.Tensor("float32", [128], name="x_lo_w", scope=tbe_platform.scope_ubuf)
        x_hi_w = tik_instance.Tensor("float32", [128], name="x_hi_w", scope=tbe_platform.scope_ubuf)
        y_lo_w = tik_instance.Tensor("float32", [128], name="y_lo_w", scope=tbe_platform.scope_ubuf)
        y_hi_w = tik_instance.Tensor("float32", [128], name="y_hi_w", scope=tbe_platform.scope_ubuf)
        ...
        raw_x = tik_instance.Tensor("float32", [128], name="x", scope=tbe_platform.scope_ubuf)
        raw_y = tik_instance.Tensor("float32", [128], name="y", scope=tbe_platform.scope_ubuf)
        x_vec = tik_instance.Tensor("float32", [128], name="x", scope=tbe_platform.scope_ubuf)
        y_vec = tik_instance.Tensor("float32", [128], name="y", scope=tbe_platform.scope_ubuf)
        x_ind = tik_instance.Tensor("int32", [128], name="x_ind", scope=tbe_platform.scope_ubuf)
        y_ind = tik_instance.Tensor("int32", [128], name="y_ind", scope=tbe_platform.scope_ubuf)
        ...
        grid_w_vector = tik_instance.Tensor("float32", [128], name="grid_w_vector", scope=tbe_platform.scope_ubuf)
        grid_h_vector = tik_instance.Tensor("float32", [128], name="grid_h_vector", scope=tbe_platform.scope_ubuf)
        tik_instance.vmuls(64, grid_w_vector, const_value_0_127, grid_w, 2, 1, 1, 8, 8)
        tik_instance.vmuls(64, grid_h_vector, const_value_0_127, grid_h, 2, 1, 1, 8, 8)
        ...
        const_one = tik_instance.Tensor("int32", [8], name="const_one", scope=tbe_platform.scope_ubuf)
        ...
        const_value_fp32 = tik_instance.Tensor("float32", [16], name="const_value", scope=tbe_platform.scope_ubuf)
        const_value_int32 = tik_instance.Tensor("int32", [16], name="const_value", scope=tbe_platform.scope_ubuf)
        ...
        tmp_fp32 = tik_instance.Tensor("float32", [128], name="tmp_fp32", scope=tbe_platform.scope_ubuf)
        ...
        cmp_mask = tik_instance.Tensor("uint16", (32,), name="cmp_mask", scope=tbe_platform.scope_ubuf)

        tik_instance.vcmpv_lt(cmp_mask, raw_x, const_value_fp32, 1, 1, 0, 8, 0)
        tik_instance.vcmpv_gt(cmp_mask[16], raw_x, const_value_fp32[8], 1, 1, 0, 8, 0)
        tik_instance.vor(4, cmp_mask, cmp_mask, cmp_mask[16], 1, 1, 1, 1, 8, 8, 8)

        dst_cmp_mask = tik_instance.mov_tensor_to_cmpmask(cmp_mask)
        tik_instance.vsel(64, 0, x_lo_w, dst_cmp_mask, const_zero, x_lo_w, 1, 1, 0, 1, 8, 0, 8)
        tik_instance.vsel(64, 0, x_hi_w, dst_cmp_mask, const_zero, x_hi_w, 1, 1, 0, 1, 8, 0, 8)

        tik_instance.vcmpv_lt(cmp_mask, raw_x[64], const_value_fp32, 1, 1, 0, 8, 0)
        tik_instance.vcmpv_gt(cmp_mask[16], raw_x[64], const_value_fp32[8], 1, 1, 0, 8, 0)
        tik_instance.vor(4, cmp_mask, cmp_mask, cmp_mask[16], 1, 1, 1, 1, 8, 8, 8)

        dst_cmp_mask = tik_instance.mov_tensor_to_cmpmask(cmp_mask)
        tik_instance.vsel(64, 0, x_lo_w[64], dst_cmp_mask, const_zero, x_lo_w[64], 1, 1, 0, 1, 8, 0, 8)
        tik_instance.vsel(64, 0, x_hi_w[64], dst_cmp_mask, const_zero, x_hi_w[64], 1, 1, 0, 1, 8, 0, 8)
        ...
        tik_instance.vcmpv_gt(cmp_mask[16], raw_y, const_value_fp32[8], 1, 1, 0, 8, 0)
        tik_instance.vor(4, cmp_mask, cmp_mask, cmp_mask[16], 1, 1, 1, 1, 8, 8, 8)

        dst_cmp_mask = tik_instance.mov_tensor_to_cmpmask(cmp_mask)
        tik_instance.vsel(64, 0, y_lo_w, dst_cmp_mask, const_zero, y_lo_w, 1, 1, 0, 1, 8, 0, 8)
        tik_instance.vsel(64, 0, y_hi_w, dst_cmp_mask, const_zero, y_hi_w, 1, 1, 0, 1, 8, 0, 8)

        tik_instance.vcmpv_lt(cmp_mask, raw_y[64], const_value_fp32, 1, 1, 0, 8, 0)
        tik_instance.vcmpv_gt(cmp_mask[16], raw_y[64], const_value_fp32[8], 1, 1, 0, 8, 0)
        tik_instance.vor(4, cmp_mask, cmp_mask, cmp_mask[16], 1, 1, 1, 1, 8, 8, 8)

        dst_cmp_mask = tik_instance.mov_tensor_to_cmpmask(cmp_mask)
        tik_instance.vsel(64, 0, y_lo_w[64], dst_cmp_mask, const_zero, y_lo_w[64], 1, 1, 0, 1, 8, 0, 8)
        tik_instance.vsel(64, 0, y_hi_w[64], dst_cmp_mask, const_zero, y_hi_w[64], 1, 1, 0, 1, 8, 0, 8)
    ...
    def _roi_align_calc_scale_batch(self, rois_data_ub, sample_num=None):
        ...
        """
        tik_instance = self.tik_instance
        roi_h_fp32 = tik_instance.Tensor("float32", [Constant.BATCH_SIZE],
                                         name="roi_h_fp32",
                                         scope=tbe_platform.scope_ubuf)
        roi_w_fp32 = tik_instance.Tensor("float32", [Constant.BATCH_SIZE],
                                         name="roi_w_fp32",
                                         scope=tbe_platform.scope_ubuf)
        ...
        tik_instance.vsub(64, roi_w_fp32, rois_end_w, rois_start_w, Constant.BATCH_SIZE * 2 // 128, 1, 1, 1, 8, 8, 8)
        tik_instance.vsub(64, roi_h_fp32, rois_end_h, rois_start_h, Constant.BATCH_SIZE * 2 // 128, 1, 1, 1, 8, 8, 8)

        const_zero = tik_instance.Tensor("float32", [16], name="const_zero", scope=tbe_platform.scope_ubuf)
        tik_instance.vector_dup(16, const_zero, 0, 1, 0, 0)

        # compare roi_width adn roi_height to 1
        tik_instance.vmax(64, roi_w_fp32, roi_w_fp32, const_zero, Constant.BATCH_SIZE * 2 // 128, 1, 1, 0, 8, 8, 0)
        tik_instance.vmax(64, roi_h_fp32, roi_h_fp32, const_zero, Constant.BATCH_SIZE * 2 // 128, 1, 1, 0, 8, 8, 0)

        # Declare roi_bin_size tik_instance.Tensor
        rois_bin_w = tik_instance.Tensor("float32", [Constant.BATCH_SIZE],
                                         name="roi_bin_w",
                                         scope=tbe_platform.scope_ubuf)
        rois_bin_h = tik_instance.Tensor("float32", [Constant.BATCH_SIZE],
                                         name="roi_bin_h",
                                         scope=tbe_platform.scope_ubuf)
        # bin size
        tik_instance.vmuls(64, rois_bin_w[:], roi_w_fp32[:], 1.0 / self.pooled_width, Constant.BATCH_SIZE * 2 // 128, 1,
                           1, 8, 8)
        tik_instance.vmuls(64, rois_bin_h[:], roi_h_fp32[:], 1.0 / self.pooled_height, Constant.BATCH_SIZE * 2 // 128,
                           1, 1, 8, 8)

        sample_num_w = tik_instance.Tensor("int32", [Constant.BATCH_SIZE],
                                           name="sample_num_w",
                                           scope=tbe_platform.scope_ubuf)
        sample_num_h = tik_instance.Tensor("int32", [Constant.BATCH_SIZE],
                                           name="sample_num_h",
                                           scope=tbe_platform.scope_ubuf)
        ...
        if sample_num is not None:
            if sample_num > 0:
                ...
                tik_instance.vconv(64, 'ceil', sample_num_w, rois_bin_w, Constant.BATCH_SIZE * 2 // 128, 1, 1, 8, 8)
                tik_instance.vconv(64, 'ceil', sample_num_h, rois_bin_h, Constant.BATCH_SIZE * 2 // 128, 1, 1, 8, 8)

        rois_start_w = tik_instance.Tensor("float32", [Constant.BATCH_SIZE],
                                           name="roi_h_fp32",
                                           scope=tbe_platform.scope_ubuf)
        rois_start_h = tik_instance.Tensor("float32", [Constant.BATCH_SIZE],
                                           name="roi_w_fp32",
                                           scope=tbe_platform.scope_ubuf)
        rois_index = tik_instance.Tensor("int32", [Constant.BATCH_SIZE],
                                         name="roi_index",
                                         scope=tbe_platform.scope_ubuf)
    ...
    def _convert_rois_data_to5n(self, rois_data_index, rois_num):
        ...
        """
        tik_instance = self.tik_instance
        rois_data_ub = tik_instance.Tensor("float32", (5, Constant.BATCH_SIZE),
                                           name="rois_data_ub",
                                           scope=tbe_platform.scope_ubuf)

        with tik_instance.if_scope(self.rois_row_length == 5):
            rois_data_tmp = tik_instance.Tensor("float32", (Constant.BATCH_SIZE, 5),
                                                name="rois_data_tmp",
                                                scope=tbe_platform.scope_ubuf)
            tik_instance.tensor_mov(rois_data_tmp, self.rois[rois_data_index * 5], '', 1,
                                    ceil_value(rois_num * 5, "float32"), 0, 0)
        ...
        with tik_instance.else_scope():
            rois_data_tmp = tik_instance.Tensor("float32", (128, 8),
                                                name="rois_data_tmp",
                                                scope=tbe_platform.scope_ubuf)
            roi_pos = tik_instance.Tensor("float16", [Constant.BATCH_SIZE, 8],
                                          name="roi_pos",
                                          scope=tbe_platform.scope_ubuf)
            roi_pos_new = tik_instance.Tensor("float16", [5, Constant.BATCH_SIZE],
                                              name="roi_pos_new",
                                              scope=tbe_platform.scope_ubuf)

            tik_instance.tensor_mov(rois_data_tmp, self.rois[rois_data_index * 8], '', 1, (4 * rois_num * 8) // 32, 0,
                                    0)
    ...
    def _compute_mode_1(self, core_bias, core_rois_n, sum_in_ub_flag):
        ...
        const_value_0_127 = tik_instance.Tensor("float32", (Constant.BATCH_SIZE,),
                                                name="const_value_0_127",
                                                scope=tbe_platform.scope_ubuf)
        with tik_instance.for_range(0, Constant.BATCH_SIZE) as i:
        ...
        with tik_instance.for_range(0, rois_batch_num) as i:
            ...
            rois_data_ub = self._convert_rois_data_to5n(core_bias + Constant.BATCH_SIZE * i, core_rois_n)

            # calc spatial_scale
            rois_bin_w, rois_bin_h, _, _, rois_start_w, rois_start_h, rois_index = \
            ...
            with tik_instance.for_range(0, core_rois_n) as j:
                ...
                with tik_instance.for_range(0, calc_h_num) as k:
                    ...
                    with tik_instance.for_range(0, calc_w_num) as w_index:
                        ...
                        self._roi_align_calc_grad_block_align(line_num, row_num, x_lo_w, x_hi_w, y_lo_w, y_hi_w, x_lo,
                                                              x_hi, y_lo, y_hi, core_bias + (i * 128) + j, x_ind, y_ind,
                                                              image_index, sum_in_ub_flag)

    @staticmethod
    def _compute_mode_2(core_bias):
        """
        compute mode 2
        """
        ...
        with tik_instance.for_range(0, self.core_num, block_num=self.core_num) as block_id:
            ...
            with self.tik_instance.if_scope(block_id < self.real_core_num):
                ...
                core_tail = self.tik_instance.Scalar(init_value=self.rois_n % self.real_core_num,
                                                     dtype="int32",
                                                     name="core_tail")
                core_bias = self.tik_instance.Scalar(init_value=core_rois_n * block_id, dtype="int32", name="core_bias")
                ...
                with tik_instance.if_scope(self.tiling_mode == Constant.TILING_MODE_3):
                    with tik_instance.new_stmt_scope():
    ...
    def roi_align_grad_compute(self):
        ...
        """
        tik_instance = self.tik_instance
        self.y_diff = tik_instance.Tensor("float32", (Constant.PARAMS_SIZE,),
                                          name="y_diff",
                                          scope=tbe_platform.scope_gm)
        self.rois = tik_instance.Tensor("float32", (Constant.PARAMS_SIZE,),
                                        name="rois_data",
                                        scope=tbe_platform.scope_gm)
        self.x_diff = tik_instance.Tensor("float32", (Constant.PARAMS_SIZE,),
                                          name="x_diff",
                                          scope=tbe_platform.scope_gm,
                                          is_atomic_add=True)
        ...
        if self.exist_rois_n:
            self.rois_n_gm = tik_instance.Tensor("int32", (Constant.PARAMS_SIZE,),
                                                 name="rois_n_gm",
                                                 scope=tbe_platform.scope_gm)
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT,
                            para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_INT,
                            para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_FLOAT, para_check.OPTION_ATTR_INT,
