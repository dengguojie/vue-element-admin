...
def act_ulq_clamp_min_grad(input_x, input_y, input_z, output, kernel_name='act_ulq_clamp_min_grad'):
    ...
    if output_shape != ():
        for i in output_shape:
            if i != 1:
                error_info = {'errCode': 'E80001', 'op_name': kernel_name, 'param_name': 'clamp_min_grad'}
                raise RuntimeError(error_info, "In op[{}], the mandatory parameter[{}] is missed.".format(
                    kernel_name, 'clamp_min_mask'))
    ...
    if input_x_type != output_type:
        error_manager_vector.raise_err_inputs_dtype_not_equal(
            kernel_name, 'y_grad', 'clamp_min_grad', input_x_type, output_type)
