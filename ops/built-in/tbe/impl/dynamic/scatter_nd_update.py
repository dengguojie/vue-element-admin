...
class ScatterNdUpdate():
    ...
    def calc_indices(self, indices_ub_index):
        ...
        with self.tik_instance.for_range(0, self.indices_last_dim) as i:
            tiling_offset = self.tik_instance.Scalar("int64", name="tiling_offset")
            updates_sum_index = self.tik_instance.Scalar("int32", name="updates_sum_index")
            ...
            self.var_read_index.set_as(self.var_read_index + updates_sum_index * tiling_offset)
    ...
    def init_ub_tensor(self):
        ...
        self.updates_burst_len = self.tik_instance.Scalar("int64", name="updates_burst_len")
        ...
        with self.tik_instance.if_scope(self.updates_loop_num > 0):
            with self.tik_instance.for_range(0, self.updates_loop_num) as updates_loop_index:
                self.update_var((indices_in_index // self.indices_last_dim + indices_ub_index) * self.update_data_num +
                                ...
                                updates_loop_index * self.updates_ub_num + self.var_read_index, mode)

        with self.tik_instance.if_scope(self.updates_last_num > 0):
            self.update_var((indices_in_index // self.indices_last_dim + indices_ub_index) * self.update_data_num +
                            self.updates_loop_num * self.updates_ub_num, self.updates_last_num,
                            self.updates_loop_num * self.updates_ub_num + self.var_read_index, mode)
        ...
        with self.tik_instance.for_range(0, indice_num // self.indices_last_dim) as indices_ub_index:
            ...
            with self.tik_instance.for_range(0, self.update_data_num) as updates_ub_index:
                self.update_value.set_as(
                    self.updates_ub[(indices_ub_index + indices_in_index // self.indices_last_dim) *
    ...
    def traversing_updates_single_core_and_ub_not_enough(self, indices_in_index, indice_num):
        ...
        with self.tik_instance.for_range(0, indice_num // self.indices_last_dim) as indices_ub_index:
            ...
            self.tik_instance.data_move(
                ...
                self.updates_gm[(indices_in_index // self.indices_last_dim + indices_ub_index) * self.update_data_num],
                0, 1, 1, 0, 0)
            if self.var_dtype in ("int8", "uint8"):
                self.tik_instance.vec_conv(32, "", self.var_vconv_ub, self.var_tile_ub, 1, 8, 4)
                self.tik_instance.vec_conv(32, "", self.updates_vconv_ub, self.updates_tile_ub, 1, 8, 4)
    ...
    def traversing_indices(self):
        ...
        with self.tik_instance.if_scope(self.indices_loop_num > 0):
            with self.tik_instance.for_range(0, self.indices_loop_num) as indices_loop_index:
                self.move_indices(indices_loop_index * max_ub_indices_num, max_ub_indices_num)
        with self.tik_instance.if_scope(self.indices_last_num > 0):
            self.move_indices(self.indices_loop_num * max_ub_indices_num, self.indices_last_num)
...
def scatter_nd_update(var, indices, updates, var_out, use_locking=False, kernel_name="scatter_nd_update"):
    """
