...
def _cal_max_boxes_num():
    ...
    temp_size_const = RPN_PROPOSAL_NUM * fp16_size + RPN_PROPOSAL_NUM * fp16_size + uint16_size
    ...
    tiling_size = TILING_PARAMS_NUM * int32_size
    ...
    if support_vreduce and support_v4dtrans:
        ...
        boxes_num_max_align16 = boxes_num_max // RPN_PROPOSAL_NUM * RPN_PROPOSAL_NUM
    else:
...
def _nms_with_mask_compute(tik_instance, tiling_gm, input_num_scalar, thresh, proposal_num_align16, kernel_name_var):
    ...
    tik_instance: TIK API
    """
    proposals = tik_instance.Tensor("float16", (proposal_num_align16, ELEMENT_NUM),
                                    name="in_proposals",
                                    scope=tik.scope_gm)
    support_vreduce = tbe_platform.api_check_support("tik.vreduce", "float16")
    support_v4dtrans = tbe_platform.api_check_support("tik.v4dtrans", "float16")
    # output shape is [N,5]
    ret = tik_instance.Tensor("float16", (proposal_num_align16, VALID_COLUMN_NUM),
                              name="out_proposals",
                              scope=tik.scope_gm)
    ...
    selected_reduced_proposals_ub = tik_instance.Tensor("float16", (proposal_num_align16, ELEMENT_NUM),
    ...
    with tik_instance.for_range(0, _ceil_div(input_num_scalar, BURST_PROPOSAL_NUM), thread_num=1) as burst_index:
        # update counter
