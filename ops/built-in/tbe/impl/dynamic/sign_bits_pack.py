...
class SignBitsPack():
    ...
    def __init__(self, input_x, y, size, kernel_name):
        ...
        self.dst_ub = None
        self.pack_rate = 8
        self.u8_size = intrin.get_bit_len("uint8") // 8
        ...
        self.dtype_size = intrin.get_bit_len(self.dtype) // 8
        ...
        self.ub_split = 6
        self.max_ele = int((self.ub_ele - 1) // self.ub_split)
...
def _check_param(input_x, size):
    """
    check paramters, if one is invalid, then raise error
    -----------------------
    :param size: output needs to be splited by size
