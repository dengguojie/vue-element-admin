...
def maximum_compute(x1, x2, y, kernel_name="maximum"):
    ...
    shape_x = dynamic.shape_to_list(x1.shape)
    shape_y = dynamic.shape_to_list(x2.shape)
    shape1, shape2, shape_max = shape_util.broadcast_shapes(shape_x, shape_y,
                                                 param_name_input1="x1",
                                                 param_name_input2="x2")

    data1 = dynamic.broadcast(x1, shape_max)
    data2 = dynamic.broadcast(x2, shape_max)

    res = dynamic.vmax(data1, data2)

    return res


@tbe_platform.register_operator("Maximum")
...
def maximum(x1, x2, y, kernel_name="maximum"):
    ...
    ins = tbe_platform.classify([x1, x2], tbe_platform.Mode.ELEWISE_WITH_BROADCAST)
    schedules, tensors = [], []
    for (x1, x2) in ins:
