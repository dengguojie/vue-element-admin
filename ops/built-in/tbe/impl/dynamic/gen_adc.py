...
class GenADC(object):
    ...
    def __init__(self, query, code_book, centroids, bucket_list, adc_tables, distance_type, kernel_name):
        ...
        para_check.check_dtype(self.query_dtype, ("float16", "float32"), param_name="query")
        para_check.check_dtype(self.code_book_dtype, ("float16"), param_name="code_book")
        ...
        self.remaining_row = self.tik_inst.Scalar("int64", name="remaining_row")
        self.bucket_list_burst_len = self.tik_inst.Scalar("int64", name="bucket_list_burst_len")
        self.remain_bucket_list_burst_len = self.tik_inst.Scalar("int64", name="remain_bucket_list_burst_len")
        self.core_used_num = self.tik_inst.Scalar("int64", name="core_used_num")
        self.dim_ns = self.tik_inst.Scalar("int64", name="dim_ns")
    ...
    def _init_ub_data(self):
        ...
        self.query_ub = self.tik_inst.Tensor(self.op_query_data_type, self.query_shape, name="query_ub",
                                             scope=tik.scope_ubuf)

        if self.op_data_type != self.op_query_data_type:
            self.query_fp16_ub = self.tik_inst.Tensor(self.op_data_type, self.query_shape, name="query_fp16_ub",
                                                      scope=tik.scope_ubuf)
    ...
    def _compute_distance(self, block_i, core_row_offset, process_row):
        ...
        with self.tik_inst.for_range(0, process_row, name="i0_inner") as i:
            with self.tik_inst.for_range(0, self.dim_m // self.dim_m_stride, name="dim_m_i") as dim_m_i:
                with self.tik_inst.for_range(0, self.dim_ksub // self.dim_ksub_stride, name="dim_ksub_i") as dim_ksub_i:
                    self.tik_inst.data_move(self.code_book_local_ub,
                                            ...
                                            0, 1,
                                            self.dim_m_stride * self.dim_ksub_stride * self.dim_dsub * 2 // 32, 0, 0)
                    ...
                    def _compute_reduce_for_two_eles():
                        ...
                        with self.tik_inst.for_range(0, vcpadd_cycle_times, name="vcpadd_i") as vcpadd_i:
                            ...
                            with self.tik_inst.if_scope(vcpadd_i == (vcpadd_repeat_times - 1)):
                                ...
                                self.tik_inst.vcpadd(128, self.reduce_0_local_ub[vcpadd_i * 128 * 128 // 2],
                                                     self.code_book_local_ub[vcpadd_i * 128 * 128],
                                                     vcpadd_tile_repeat_times, 1, 1, 8)
