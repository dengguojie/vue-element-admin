...
def _greater_equal_compare(data, shape, dtype, data_min):
    ...
    the compare result
    """
    data_zero = te.lang.dynamic.broadcast(tvm.const(SCALAR_ZERO, dtype), shape,
                                          dtype)
    if dtype == "int32":
        data_one = te.lang.dynamic.broadcast(tvm.const(SCALAR_ONE, "float16"),
                                             shape, "float16")
    else:
        data_one = te.lang.dynamic.broadcast(tvm.const(SCALAR_ONE, dtype),
                                             shape, dtype)

    res_sub = te.lang.dynamic.vsub(data[1], data[0])
    res_min = te.lang.dynamic.vmin(res_sub, data_min)
    res_max = te.lang.dynamic.vmax(res_min, data_zero)

    if dtype == "float32":
        # max num of float32 is 2**126
        # but cce can only support 2**62, so use 62/62/2 to adaptor 126
        res_mul1 = te.lang.dynamic.vmuls(res_max, tvm.const(SCALAR_MUL_FP32,
                                                            dtype=dtype))
        res_mul2 = te.lang.dynamic.vmuls(res_mul1, tvm.const(SCALAR_MUL_FP32,
                                                             dtype=dtype))
        res_mul = te.lang.dynamic.vmuls(res_mul2, tvm.const(SCALAR_MUL1_FP32,
                                                            dtype=dtype))
    elif dtype == "float16":
        # max num of float16 is 2**24
        # but cce can only support 2**12, so use 12/12 to adaptor 24
        res_mul1 = te.lang.dynamic.vmuls(res_max, tvm.const(SCALAR_MUL_FP16,
                                                            dtype=dtype))
        res_mul = te.lang.dynamic.vmuls(res_mul1, tvm.const(SCALAR_MUL_FP16,
                                                            dtype=dtype))
    else:
...
def greater_equal_compute(input_x, input_y, output_z,
    ...
    if dtype_x == "float32":
        ...
        data_min = te.lang.dynamic.broadcast(
            tvm.const(SCALAR_MIN_FP32, dtype=dtype_x), shape_max, dtype_x)
    elif dtype_x == "float16":
        # minimun num of float16 2**(-24)
        data_min = te.lang.dynamic.broadcast(
            tvm.const(SCALAR_MIN_FP16, dtype=dtype_x), shape_max, dtype_x)
    else:
        # minimun num of int32 1
        data_min = te.lang.dynamic.broadcast(
            tvm.const(SCALAR_ONE, dtype=dtype_x), shape_max, dtype_x)
...
def greater_equal(input_x, input_y, output_z, kernel_name="greater_equal"):
    ...
    for (input_x, input_y) in ins:
        with te.op.compute():
            ...
            res = greater_equal_compute(tensor_x, tensor_y, output_z,
                                        kernel_name)
