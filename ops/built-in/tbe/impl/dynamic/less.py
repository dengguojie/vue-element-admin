...
def _less_compare(data, shape, dtype, data_min):
    ...
    if dtype == "float32":
        ...
        res_mul2 = te.lang.dynamic.vmuls(res_mul1, tvm.const(SCALAR_MUL_FP32,
                                                             dtype=dtype))
        res = te.lang.dynamic.vmuls(res_mul2,
                                    tvm.const(SCALAR_MUL1_FP32, dtype=dtype))
    elif dtype == "float16":
        ...
        res_mul1 = te.lang.dynamic.vmuls(res_max, tvm.const(SCALAR_MUL_FP16,
                                                            dtype=dtype))
        res = te.lang.dynamic.vmuls(res_mul1,
                                    tvm.const(SCALAR_MUL_FP16, dtype=dtype))
    else:
...
def less(input_x, input_y, output_z, kernel_name="less"):
    ...
    for (input_x, input_y) in ins:
        with te.op.compute():
            ...
            res = less_compute(tensor_x, tensor_y, output_z,
                               kernel_name="less")
