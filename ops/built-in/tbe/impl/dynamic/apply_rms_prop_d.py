...
def apply_rms_prop_d(var,
    ...
    for (_var, _ms, _mom, _grad) in ins:
        with tbe.compute():
            shape_var, _, _, _ = shape_util.variable_shape([_var, _ms, _mom, _grad])
            shape_refine = (functools.reduce(operator.mul, shape_var),)
            data_var = tvm.placeholder(shape_refine, name="data_var", dtype=compute_dtype)
            data_ms = tvm.placeholder(shape_refine, name="data_ms", dtype=compute_dtype)
            data_mom = tvm.placeholder(shape_refine, name="data_mom", dtype=compute_dtype)
            data_grad = tvm.placeholder(shape_refine, name="data_grad", dtype=compute_dtype)
