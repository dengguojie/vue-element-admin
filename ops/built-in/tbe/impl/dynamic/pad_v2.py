...
def op_select_format(x, paddings, y, kernel_name="pad_v2"):
    """
    op_select_format for pad_v2
    ...
    str_constant_values_type = ",".join(x_dtype)
    str_constant_values_format = ",".join(padding_format)
    ...
    input2 = util_select_op_base.gen_param(classify="input2",
                                           ...
                                           datatype=str_constant_values_type,
                                           format=str_constant_values_format,
                                           unknownshape_format=str_padding_format)
...
class PadInit(OpBase):
    """
    Function: class that execute pad_v2
    """

    def __init__(self, kernel_name, max_shape_len=8):
        OpBase.__init__(self)
        self.const_values_dtype = "float16"
        ...
        self.ub_number = (self.ub_size_bytes - Constant.RESERVED_UB_SIZE) // self.inner_bytes_size
    ...
    def init_src_dst_gm(self,
                        input_dict_list,
                        output_dict_list,
                        pad_input_idx=0,
                        pad_const_values_idx=2,
                        pad_outnput_idx=0):
        """
        init gm tensor set tiling, input, paddings output tensor(gm)
        """
        self.const_values_dtype = input_dict_list[pad_const_values_idx].get("dtype").lower()
        self.const_values_scalar = self.tik_instance.Scalar(dtype=self.const_values_dtype,
                                                            name="const_values_scalar",
                                                            init_value=0)
        tiling_dict = {"dtype": self.tiling_dtype, "shape": self.tiling_shape}
        self.input_bytes_size = tbe_platform.get_bit_len(input_dict_list[pad_input_idx]["dtype"]) // Constant.EIGHT_BIT
        ...
        self.sync_workspace = self.tik_instance.Tensor("int64", (self.core_nums * 32 // 8,),
                                                       name="barrier_workspace",
                                                       scope=tik.scope_gm,
                                                       is_workspace=True,
                                                       is_atomic_add=True)
        self.output_gm = self.output_gm_list[pad_outnput_idx].reinterpret_cast_to(self.inner_dtype)
        self.const_values_gm = self.input_gm_list[pad_const_values_idx]
    ...
    def do_pad_with_move_cut_outer_default(self, is_last_align=False):
        ...
        self.fill_vnchw_ub(self.block_num + 1, ping_data_ub_one_block)
        self.fill_vnchw_ub(self.block_num + 1, pang_data_ub_one_block)
    ...
    def do_pad_with_vnchw_for_last_two_dim(self):
        ...
        def run_outer_by_outer(second_dim_start, do_inner_num, do_outer_num, align_tail, disable_sync_mte3=False):
            ...
            origin_output_tail_data_ub_ping = self.tik_instance.Tensor(self.inner_dtype, (16 * 16,),
                                                                       ...
                                                                       scope=tik.scope_ubuf)
            self.fill_vnchw_ub(max_output_size, vnchw_output_data_ub_ping)
            ...
            origin_output_tail_data_ub_pang = self.tik_instance.Tensor(self.inner_dtype, (16 * 16,),
                                                                       name="origin_output_tail_data_ub_ping",
                                                                       scope=tik.scope_ubuf)
            self.fill_vnchw_ub(max_output_size, vnchw_output_data_ub_pang)
            ping_ub_list = [
    ...
    def calc_core_args(self, core_index, output_shape):
        ...
        """
        core_len = self.tik_instance.Scalar(dtype="int64", name="core_len", init_value=0)
        core_offset = self.tik_instance.Scalar(dtype="int64", name="core_offset", init_value=0)
        core_len.set_as((output_shape + self.core_nums - 1) // self.core_nums)
        core_len.set_as((core_len + self.block_num - 1) // self.block_num)
        core_len.set_as(core_len * self.block_num)
        core_offset.set_as(core_index * core_len)
        with self.tik_instance.if_scope((core_offset + core_len) > output_shape):
            core_len.set_as(0)
            with self.tik_instance.if_scope(core_offset < output_shape):
                core_len.set_as(output_shape - core_offset)
                core_len.set_as((core_len + self.block_num - 1) // self.block_num)
                core_len.set_as(core_len * self.block_num)
                core_offset.set_as(output_shape - core_len)
        return core_offset, core_len

    def data_move_fill_gm(self, core_offset, core_len, fill_gm_ub):
        """
        move data to output_gm by core_offset, data_len
        """
        with self.tik_instance.new_stmt_scope():
            core_len_block_num = self.tik_instance.Scalar(dtype="int64", name="core_len_block_num", init_value=0)
            ...
            core_len_block_num.set_as((core_len + self.block_num - 1) // self.block_num)
            data_move_loop_num.set_as(core_len_block_num // Constant.FILL_NUM)
            data_move_loop_left.set_as(core_len_block_num % Constant.FILL_NUM)
            with self.tik_instance.new_stmt_scope(disable_sync=True):
    ...
    def pre_compute(self, core_index):
        """
        fill gm by const before compute
        """
        with self.tik_instance.new_stmt_scope():
            const_bytes_size = tbe_platform.get_bit_len(self.const_values_dtype) // Constant.EIGHT_BIT
            fill_gm_ub_shape = Constant.FILL_NUM * Constant.BLOCK_BYTES // const_bytes_size
            ...
            one_block_const_values_ub = self.tik_instance.Tensor(self.const_values_dtype, (16,),
                                                                name="one_block_const_value_ub",
                                                                scope=tik.scope_ubuf)
            fill_gm_ub = self.tik_instance.Tensor(self.const_values_dtype, (fill_gm_ub_shape,),
                                                name="fill_gm_ub",
                                                scope=tik.scope_ubuf)

            self.tik_instance.data_move(one_block_const_values_ub, self.const_values_gm[0], 0, 1, 1, 0, 0)
            self.const_values_scalar.set_as(one_block_const_values_ub[0])
            if tbe_platform.get_bit_len(self.const_values_dtype) in [64]:
                for i in range(4):
                    one_block_const_values_ub[i].set_as(self.const_values_scalar)
            ...
            self.data_move_fill_gm(core_offset, core_len, fill_gm_ub)
        if self.core_nums > 1:
            self.tik_instance.block_barrier(self.sync_workspace)

    def fill_vnchw_ub(self, block_num, used_ub):
        """
        fill ub by const value
        """
        with self.tik_instance.new_stmt_scope():
            const_bytes_size = tbe_platform.get_bit_len(self.const_values_dtype) // Constant.EIGHT_BIT
            ub_vnchw_before = self.tik_instance.Tensor(self.const_values_dtype, (Constant.FILL_NUM,),
                                                    name="ub_vnchw_before",
                                                    scope=tik.scope_ubuf)
            ub_vnchw_after = self.tik_instance.Tensor(self.inner_dtype, (Constant.FILL_NUM,),
                                                    name="ub_vnchw_after",
                                                    scope=tik.scope_ubuf)
            loop_num = self.tik_instance.Scalar(dtype="int64", name="loop_num", init_value=0)
            loop_left = self.tik_instance.Scalar(dtype="int64", name="loop_left", init_value=0)

            src_list = []
            if tbe_platform.get_bit_len(self.const_values_dtype) in [64]:
                for i in range(4):
                    ub_vnchw_before[i].set_as(self.const_values_scalar)
                ub_vnchw_before = ub_vnchw_before.reinterpret_cast_to(self.inner_dtype)
                src_list = [ub_vnchw_before] * Constant.TRANS_MIN_BLKS
            else:
                self.tik_instance.vector_dup(Constant.FILL_NUM // const_bytes_size, ub_vnchw_before,
                                            self.const_values_scalar, 2, 1, 8)
                ub_vnchw_before = ub_vnchw_before.reinterpret_cast_to(self.inner_dtype)
                src_list = [ub_vnchw_before[i * Constant.TRANS_MIN_BLKS] for i in range(Constant.TRANS_MIN_BLKS)]
            dst_list = [ub_vnchw_after[i * Constant.TRANS_MIN_BLKS] for i in range(Constant.TRANS_MIN_BLKS)]
            self.tik_instance.vnchwconv(False, False, dst_list, src_list, 1, 0, 0)
            loop_num.set_as(block_num // Constant.TRANS_MIN_BLKS)
    ...
    def pad_compute(self, outer_compile_info=None):
        ...
        Constant.MODE0: the last dim of output > 128*core_num, and cut by last dim
                and do pad_v2 with data move
        Constant.MODE1: the last dim of output => 960, and cut by outer dim(0-4)
                and do pad_v2 with data move
        Constant.MODE2: the last dim of output < 960, and cut by outer dim(0-3)
                and do pad_v2 with vnchw
        """
        # op step 0. init gm memory
        # do in function init_src_dst_gm

        # op step 1. regist the tiling funtion base on tiling_key
        self.regist_compute(Constant.MODE0, self.do_pad_with_move_cut_inner)
        self.regist_compute(Constant.MODE1, self.do_pad_with_move_cut_outer)
        self.regist_compute(Constant.MODE2, self.do_pad_with_vnchw_for_last_two_dim)
        self.regist_compute(Constant.MODE3, self.do_pad_with_vnchw_for_last_three_dim)
...
@register_operator("PadV2")
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT,
                            para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def pad_v2(x, paddings, constant_values, y, kernel_name="pad_v2"):
    """ calculating pad_v2 tensor by paddings parameters
    ...
    kernel_name : str
        cce kernel name, default value is "pad_v2"
    ...
    constant_values_dtype = constant_values.get("dtype").lower()
    if constant_values_dtype != src_dtype:
        error_detail = "dtype of x and constant_values should be same"
        error_manager_vector.raise_err_two_input_dtype_invalid(kernel_name, "x", "constant_values", error_detail)
    ...
    obj.init_src_dst_gm((x, paddings, constant_values), (y,),
                        pad_input_idx=0,
                        pad_const_values_idx=2,
                        pad_outnput_idx=0)
