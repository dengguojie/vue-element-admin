...
class LayerNormalizeBase(object):
    ...
    def __init__(self, tiling_ub_list, epsilon, kernel_name, cont, data_type, elementwise, output_mean_var, input_output_tensor_dict, core_index, sync_workspace, n_max_data_num, input_mode, atomic_clean_diff_shape):
        """
    ...
    def _get_mask(self, data_num, format_num_floor, format_num_ceil):
        ...
        if self.input_mode == DYNAMIC and not isinstance(data_num, int):
            ...
            with self.tik_inst.if_scope(index_start < min(index_end, mask_split)):
                ...
                with self.tik_inst.for_range(index_start, min(index_end, mask_split)) as index_l:
                    ...
                    with self.tik_inst.for_range(0, index_l) as l_repeat:
            ...
            with self.tik_inst.if_scope(max(mask_split, index_start) < index_end):
                ...
                with self.tik_inst.for_range(max(mask_split, index_start), index_end) as index_h:
                    ...
                    with self.tik_inst.for_range(0, (index_h - mask_split)) as h_repeat:
    ...
    def _mean_var_move_out_base(self):
        ...
        if self.gm_type != self.ub_type:
            ...
            elif self.input_mode != DYNAMIC and self.act_core_use > 1:
                self.tik_inst.block_barrier(self.sync_workspace)
            with self.tik_inst.if_scope(self.core_index < self.act_core_use - 1):
    ...
    def _y_workspace_move_out(self):
        batch_index = self.each_core_batch_num * self.core_index
        if self.gm_type != self.ub_type and self.input_mode == DYNAMIC and self.atomic_clean_diff_shape:
        ...
        elif self.gm_type != self.ub_type and self.input_mode != DYNAMIC and self.tiling_mode == 0 and self.atomic_clean_diff_shape:
    ...
    def _y_move_out_fp16(self, batch_index_start):
        ...
        buf_y_all = {
            "y_ub_type_ub": TensorOperatorParam(y_ub_type_ub, data_num_align,
                                                0, num_per_cmd=self.ub_repeat_data_num),
            "y_gm_type_ub": TensorOperatorParam(y_gm_type_ub, data_num_align,
                                                0, num_per_cmd=self.ub_repeat_data_num)}
        cmd_vconv = [VecCmd(cmd_name="vconv", dst_name="y_gm_type_ub",
                            src0_name="y_ub_type_ub", round_mode=""),
                     ]
        self.data_move(y_ub_type_ub, self.y_ub_type[batch_index_start, 0],
                       0, 1, block_num_move_in, 0, 0)
        VecExecutor.exec_vec_cmd(buf_y_all, cmd_vconv, "y_ub_type_ub")
        self.data_move(self.input_y[batch_index_start, 0], y_gm_type_ub,
                       0, 1, block_num_move_out, 0, 0)
...
class LayerNormalizeSplitD(LayerNormalizeBase):
    ...
    def _count_mean_each_batch_loop(self, batch_index, start_index, data_num, mean_ub_all):
        ...
        input_data_ub = self.tik_inst.Tensor(self.ub_type, (data_num_align_shape,),
        ...
        if self.gm_type != self.ub_type:
            gm_data_tensor_ub = self.tik_inst.Tensor(
                self.gm_type, (data_num_align_shape,), self.tik.scope_ubuf, "gm_data_tensor_ub")
    ...
    def _count_stand_each_batch_loop(self, batch_index, start_index, data_num, scalar_all):
        ...
        gm_data_tensor_ub = None
        if self.elementwise:
            gamma_beta_tensor_ub = self.tik_inst.Tensor(self.ub_type, (data_num_align_shape,),
            ...
            buf_stand_ub["gamma_beta_tensor_ub"] = TensorOperatorParam(gamma_beta_tensor_ub, data_num_align, 0,
                                                                       num_per_cmd=self.ub_repeat_data_num)
        if self.gm_type != self.ub_type:
            gm_data_tensor_ub = self.tik_inst.Tensor(self.gm_type, (data_num_align_shape,),
    ...
    def _mode_data_move_out(self, data_tensor_ub, gm_data_tensor_ub, buf_stand_ub,
        ...
        if self.input_mode == DYNAMIC and not isinstance(data_num, int):
            with self.tik_inst.if_scope(data_num % self.gm_block_data_num != 0):
                last_block_ub_index, last_block_gm_index = self._get_last_block_info(start_index, data_num)
                self.data_move(self.input_y[batch_index, last_block_gm_index],
                               tensor_ub[last_block_ub_index], 0, 1, 1, 0, 0)
        else:
            if data_num % self.gm_block_data_num != 0:
                last_block_ub_index, last_block_gm_index = self._get_last_block_info(start_index, data_num)
                self.data_move(self.input_y[batch_index, last_block_gm_index],
...
class LayerNormalizeSplitN(LayerNormalizeBase):
    ...
    def _init_data_tensor(self, max_ub_num, batch_num, data_num):
        ...
        gm_data_tensor_ub = None
        if self.input_mode == DYNAMIC:
            input_data_ub = self.tik_inst.Tensor(self.ub_type, (max_ub_num, ),
                                                 self.tik.scope_ubuf, "input_data_ub")
            input_data_ub = input_data_ub.reshape((max_ub_num // data_num, data_num))
            input_data_square_ub = self.tik_inst.Tensor(self.ub_type, (max_ub_num, ),
                                                        self.tik.scope_ubuf, "input_data_square_ub")
            input_data_square_ub = input_data_square_ub.reshape((max_ub_num // data_num, data_num))
            if self.gm_type != self.ub_type:
                gm_data_tensor_ub = self.tik_inst.Tensor(self.gm_type, (max_ub_num, ),
                                                         self.tik.scope_ubuf, "gm_data_tensor_ub")
    ...
    def _data_move_in(self, data_ub_all, tensor_gm, buf_data_all, cmd_gm_vconv_ub,
        ...
        if self.input_mode == DYNAMIC:
            with self.tik_inst.if_scope(self.data_num == data_num_align):
                self.data_move(tensor_ub, tensor_gm[batch_index_start, 0], 0,
                               1, batch_num * each_batch_gm_block_num, 0, 0)
    ...
    def _data_move_in_not_align(self, tensor_ub, tensor_gm, batch_index_start, batch_num, mode):
        if mode == 0:
            ...
            if self.input_mode != DYNAMIC and block_num_floor == 0:
                with self.tik_inst.for_range(0, batch_num) as batch_index:
    ...
    def _count_mean_var(self, buf_mean_all):
        ...
        cmd_count_mean_var = [
            VecCmd(cmd_name="vmuls", dst_name="batch_mean_ub",
                   src0_name="batch_mean_ub", scalar=self.mean_cof),  # E(x)
            VecCmd(cmd_name="vmuls", dst_name="batch_mean_square_ub",
                   src0_name="batch_mean_square_ub", scalar=self.mean_cof),  # E(x^2)
            VecCmd(cmd_name="vmul", dst_name="batch_variance_ub",
                   src0_name="batch_mean_ub", src1_name="batch_mean_ub"),  # E(x)^2
            VecCmd(cmd_name="vsub", dst_name="batch_variance_ub",
                   src0_name="batch_mean_square_ub", src1_name="batch_variance_ub"),  # E(x^2) - E(x)^2
    ...
    def _mode1_data_move_out(self, data_ub_all, buf_data_all, batch_index_start, batch_num, data_num_align):
        input_data_ub, _, gm_data_tensor_ub = data_ub_all
        if self.input_mode == DYNAMIC and self.atomic_clean_diff_shape:
            ...
            if self.ub_type == self.gm_type:
                ...
                with self.tik_inst.if_scope(self.data_num < self.gm_block_data_num):
                    move_mode.set_as(1)
                    self.tik_inst.set_atomic_add(1)
                    self._mode1_data_move_out_exe(tensor_ub, batch_index_start, batch_num,
                                                  data_num_align, input_y, block_data_num, move_mode)
                    self.tik_inst.set_atomic_add(0)
                with self.tik_inst.else_scope():
                    move_mode.set_as(0)
                    self._mode1_data_move_out_exe(tensor_ub, batch_index_start, batch_num,
                ...
                with self.tik_inst.else_scope():
                    ...
                    self._mode1_data_move_out_exe(tensor_ub, batch_index_start, batch_num,
                                                  data_num_align, input_y, block_data_num, move_mode)
                    self.tik_inst.set_atomic_add(0)
        elif self.input_mode == DYNAMIC and not self.atomic_clean_diff_shape:
            block_data_num = self.tik_inst.Scalar("int32", "block_data_num")
            move_mode = self.tik_inst.Scalar("int32", "move_mode")
            if self.ub_type == self.gm_type:
                tensor_ub = input_data_ub
            ...
            if self.ub_type == self.gm_type:
                ...
                if self.data_num < self.gm_block_data_num and self.atomic_clean_diff_shape:
                    ...
                    self._mode1_data_move_out_exe(tensor_ub, batch_index_start, batch_num,
                                                  data_num_align, input_y, block_data_num, move_mode)
            elif (self.data_num >= self.gm_block_data_num and self.ub_type != self.gm_type) or not self.atomic_clean_diff_shape:
                cmd_ub_vconv_gm = [VecCmd(cmd_name="vconv", dst_name="gm_data_tensor_vconv_ub",
                                          src0_name="input_data_ub", round_mode="")]
                VecExecutor.exec_vec_cmd(buf_data_all, cmd_ub_vconv_gm, "input_data_ub")
                tensor_ub = gm_data_tensor_ub
    ...
    def _mode1_data_move_out_exe(self, tensor_ub, batch_index_start, batch_num,
                                 data_num_align, input_y, block_data_num, move_mode):
        ...
        if self.input_mode == DYNAMIC:
            with self.tik_inst.if_scope(self.data_num == data_num_align):
                self.data_move(input_y[batch_index_start, 0], tensor_ub,
                               0, 1, batch_num * each_batch_gm_block_num, 0, 0)
            with self.tik_inst.elif_scope(self.data_num % self.gm_block_data_num == 0):
            ...
            with self.tik_inst.else_scope():
                ...
                with self.tik_inst.for_range(0, batch_num) as batch_index:
                    self.data_move(input_y[batch_index_start + batch_index, 0], tensor_ub[batch_index, 0],
...
def tiling_init_dynamic(cont):
    byte_size = cont.const_dtype_byte.get(DTYPE_INT32)
    block_size = cont.const_block_byte
    tiling_gm = cont.tinst.Tensor(DTYPE_INT32, (TILING_ARG_NUM,), name="tiling_gm", scope=cont.tik.scope_gm)
    tiling_ub = cont.tinst.Tensor(DTYPE_INT32, (TILING_ARG_NUM,), name="tiling_ub", scope=cont.tik.scope_ubuf)
    # move tiling params from gm to ub

    cont.tinst.data_move(tiling_ub, tiling_gm, 0, 1, TILING_ARG_NUM * byte_size // block_size, 0, 0)
    ...
    tiling_ub_list = [tiling_gm, tiling_mode, batch_num, data_num,
                      act_core_num, each_core_batch_num, last_core_batch_num, mean_cof]
...
def _gen_tiling_case_const(input_x, gamma, beta, input_y):
    ...
    run_info = op_tiling.do_op_tiling(operation.get_context().get_op_type(),
                                      operation.get_compile_info(), inputs, outputs)
    tiling_format = {
        "tiling_mode": "int",
        "batch_num": "int",
        "data_num": "int",
    ...
    tiling_data["mean_cof"] = 1.0 / tiling_data["data_num"]
    tiling_ub_list = [None]
    for key in tiling_format:
        tiling_ub_list.append(tiling_data[key])
    return tiling_ub_list
...
def layer_normalize(input_x, gamma, beta, input_y, mean, variance, begin_norm_axis, begin_params_axis, epsilon,
                    kernel_name="LayerNormalize", atomic_clean_diff_shape=True):
    """
    op layer_norm
    """
    ...
    if input_mode == DYNAMIC:
        ...
        input_shape = (MAX_INT32_SIZE,)
        input_x = cont.tinst.Tensor(data_type, input_shape, cont.tik.scope_gm, "x")
        input_x = input_x.reshape((MAX_INT32_SIZE // tiling_ub_list[3], tiling_ub_list[3]))
        input_y = cont.tinst.Tensor(data_type, input_shape, cont.tik.scope_gm, "y", is_atomic_add=y_atomic_add)
        input_y = input_y.reshape((MAX_INT32_SIZE // tiling_ub_list[3], tiling_ub_list[3]))
        data_shape = (1, MAX_INT32_SIZE)
        batch_shape = (MAX_INT32_SIZE,)
    ...
    if output_mean_var:
        ...
        if data_type != ub_type:
            ...
            if atomic_clean_diff_shape:
                y_ub_type = cont.tinst.Tensor(ub_type, input_shape, cont.tik.scope_gm,
                                              "y_ub_type", is_workspace=True, is_atomic_add=True)
            if input_mode == DYNAMIC and atomic_clean_diff_shape:
                y_ub_type = y_ub_type.reshape((MAX_INT32_SIZE // tiling_ub_list[3], tiling_ub_list[3]))
            mean_ub_type = cont.tinst.Tensor(ub_type, batch_shape_align, cont.tik.scope_gm,
                                             "mean_ub_type", is_workspace=True, is_atomic_add=True)
            variance_ub_type = cont.tinst.Tensor(
                ub_type, batch_shape_align, cont.tik.scope_gm, "variance_ub_type", is_workspace=True, is_atomic_add=True)
    ...
    if data_type != ub_type:
        ...
        sync_workspace = cont.tinst.Tensor(sync_type, (sync_data_num,), name="gm_barrier",
                                           scope=cont.tik.scope_gm, is_workspace=True, is_atomic_add=True)
    ...
    with cont.tinst.for_range(0, ai_core_num, block_num=ai_core_num) as core_index:
        if input_mode == DYNAMIC:
            with cont.tinst.if_scope(tiling_mode == SELECT_KEY_MODE_SPLIT_N and core_index < act_core_use):
                obj_layer_normalize = LayerNormalizeSplitN(tiling_ub_list, epsilon, kernel_name, cont, data_type,
                                                           elementwise, output_mean_var, input_output_tensor_dict, core_index, sync_workspace, n_max_data_num, input_mode, atomic_clean_diff_shape)
                obj_layer_normalize.mode_compute()
            with cont.tinst.elif_scope(tiling_mode == SELECT_KEY_MODE_SPLIT_D and core_index < act_core_use):
                obj_layer_normalize = LayerNormalizeSplitD(tiling_ub_list, epsilon, kernel_name, cont, data_type,
                                                           elementwise, output_mean_var, input_output_tensor_dict, core_index, sync_workspace, n_max_data_num, input_mode,
                                                           atomic_clean_diff_shape)
                obj_layer_normalize.mode_compute()
        else:
            if tiling_mode == SELECT_KEY_MODE_SPLIT_N:
                obj_layer_normalize = LayerNormalizeSplitN(tiling_ub_list, epsilon, kernel_name, cont, data_type,
                                                           elementwise, output_mean_var, input_output_tensor_dict, core_index, sync_workspace, n_max_data_num, input_mode, atomic_clean_diff_shape)
            else:
                obj_layer_normalize = LayerNormalizeSplitD(tiling_ub_list, epsilon, kernel_name, cont, data_type,
