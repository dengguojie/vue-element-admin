...
class LayerNormalizeBase(object):
    ...
    def __init__(self, tiling_ub_list, epsilon, kernel_name, cont, data_type, elementwise, output_mean_var, input_output_tensor_dict, core_index, sync_workspace, n_max_data_num, input_mode):
        """
    ...
    def _y_workspace_move_out(self):
        batch_index = self.each_core_batch_num * self.core_index
        if self.gm_type != self.ub_type and self.input_mode == DYNAMIC:
        ...
        elif self.gm_type != self.ub_type and self.input_mode != DYNAMIC and self.tiling_mode == 0:
        ...
        if self.input_mode == DYNAMIC:
            ...
            if self.ub_type == self.gm_type:
                ...
                if self.data_num < self.gm_block_data_num:
                    ...
                    self._mode1_data_move_out_exe(tensor_ub, batch_index_start, batch_num,
                                                  data_num_align, input_y, block_data_num, move_mode)
            elif self.data_num >= self.gm_block_data_num and self.ub_type != self.gm_type:
                cmd_ub_vconv_gm = [VecCmd(cmd_name="vconv", dst_name="gm_data_tensor_vconv_ub",
                                          src0_name="input_data_ub", round_mode="")]
                VecExecutor.exec_vec_cmd(buf_data_all, cmd_ub_vconv_gm, "input_data_ub")
                tensor_ub = gm_data_tensor_ub
...
def layer_normalize(input_x, gamma, beta, input_y, mean, variance, begin_norm_axis, begin_params_axis, epsilon,
    ...
    op layer_norm
    """
    tik_container = AContainer("dynamic")
    ...
    is_atomic_add = (ub_type == data_type)
    if input_mode == DYNAMIC:
        ...
        input_y = cont.tinst.Tensor(data_type, input_shape, cont.tik.scope_gm, "y", is_atomic_add=is_atomic_add)
    ...
    if output_mean_var:
        mean = cont.tinst.Tensor(data_type, batch_shape, cont.tik.scope_gm, "mean", is_atomic_add=is_atomic_add)
        variance = cont.tinst.Tensor(data_type, batch_shape, cont.tik.scope_gm, "variance", is_atomic_add=is_atomic_add)
