...
def _output_m_compute(m, beta1, grad):
    ...
    """
    input_dtype = m.dtype
    shape_m_grad = shape_util.shape_to_list(m.shape)
...
def _inner_lr_compute(lr, beta2_power, beta1_power, compute_shape):
    ...
    lr_broad = tbe.broadcast(lr, compute_shape)
...
def _output_var_t_compute_use_nesterov(var, lr_t, m_t, beta1, grad, epsilon, v_t):
    """
...
def apply_adam_d_compute(var, m, v, beta1_power, beta2_power, lr, beta1, beta2, epsilon, grad, var_out, m_out, v_out,
    ...
    None
    """
    m_t = _output_m_compute(m, beta1, grad)
    ...
    if use_nesterov is True:
        var_t = _output_var_t_compute_use_nesterov(var, lr_r, m_t, beta1, grad, epsilon, v_t)
    else:
...
def apply_adam_d(var, m, v, beta1_power, beta2_power, lr, beta1, beta2, epsilon, grad, var_out, m_out, v_out,
                 use_locking=False, use_nesterov=False, kernel_name="apply_adam_d"):
    """
    the opreator's compute
    lr_t = learning_rate*(sqrt(1-beta2_power)) / (1-beta1_power)
    m_t = m + (1-beta1)*(grad-m)
