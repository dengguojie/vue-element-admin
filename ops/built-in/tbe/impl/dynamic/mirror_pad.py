...
class MirrorPadInit():
    ...
    def __init__(self, mode, kernel_name):
        ...
        self.input_dim_1 = None
        self.input_dim_2 = None
        self.input_dim_3 = None
        self.input_dim_4 = None
        self.pading_00 = None
        self.pading_01 = None
        self.pading_10 = None
        self.pading_11 = None
        ...
        self.pading_40 = None
        self.pading_41 = None
        self.output_dim_0 = None
        self.output_dim_1 = None
        self.output_dim_2 = None
        self.output_dim_3 = None
        self.output_dim_4 = None
        ...
        self.pading_value = None
        self.outer_move_out_core_offset = None
        self.outer_move_out_index = None
        self.in_and_out_indexes = None
        self.dividends = None
        self.two_mode_diff = None
        self.conv1_stride_0 = None
        self.conv1_stride_1 = None
        self.conv2_stride_0 = None
        self.conv2_stride_1 = None
        self.inner_outsize = None
        self.inner_insize = None
        self.input_dim_4_align_blocks = None
        self.output_dim_4_align_blocks = None
        self.inner_loop_times = None
        self.inner_process_nums_per_loop = None
        self.inner_process_nums_tail_loop = None
        self.ub_first = None
        self.ub_second = None
        self.ub_addr_rollback = None
        self.one_loop_process_lines = None
        self.move_out_start_line = None
        self.padding40_align_blocks = None
        self.padding41_align_blocks = None
        self.input_dim_4_align_blocks = None
        self.tail_loop_true_nums = None
        self.outer_move_in_index = None
        self.address_rollback_scalar = None
        self.start_blocks = None
        self.move_in_start_blocks = None
    ...
    def tiling_args(self):
        ...
        #0-3:out_dim 0-3  4-7: in_dim 0-3
        self.in_and_out_indexes = self.tik_instance.ScalarArray(self.tiling_dtype,
                                                                8,
                                                                "in_and_out_indexes",
                                                                init_value=0)
        self.dividends = self.tik_instance.ScalarArray(self.tiling_dtype, 5, "dividends", init_value=1)
    ...
    def init_src_dst_gm(self, input_dict, padding_dict, output_dict):
        """
        ...
        self.tiling_gm = self.tik_instance.Tensor(self.tiling_dtype,
                                                  ...
                                                  name="tiling_gm",
                                                  scope=tik.scope_gm)
        gm_in = self.tik_instance.Tensor(input_dict.get("dtype"),
    ...
    def get_outer_move_in_index(self, axis_nums):
        ...
        self.in_and_out_indexes[1].set_as(
            (self.outer_move_out_index - self.dividends[0] * self.in_and_out_indexes[0]) / self.dividends[1])
        self.in_and_out_indexes[2].set_as((self.outer_move_out_index - self.dividends[0] * self.in_and_out_indexes[0] -
                                           self.dividends[1] * self.in_and_out_indexes[1]) / self.dividends[2])
        if axis_nums == AXIS_DIMS_4:
            self.in_and_out_indexes[3].set_as(self.outer_move_out_index -
                                              self.dividends[0] * self.in_and_out_indexes[0] -
                                              self.dividends[1] * self.in_and_out_indexes[1] -
                                              self.dividends[2] * self.in_and_out_indexes[2])
        with self.tik_instance.if_scope(self.in_and_out_indexes[0] < self.pading_00):
            self.in_and_out_indexes[4].set_as(self.pading_00 - self.in_and_out_indexes[0] - self.two_mode_diff)
        with self.tik_instance.if_scope(self.in_and_out_indexes[0] >= self.pading_00 + self.tiling_input_shape[0]):
            self.in_and_out_indexes[4].set_as(2 * self.tiling_input_shape[0] - self.in_and_out_indexes[0] +
                                              self.pading_00 - 2 + self.two_mode_diff)
        with self.tik_instance.if_scope((self.pading_00 <= self.in_and_out_indexes[0]) &
                                        (self.in_and_out_indexes[0] < self.pading_00 + self.tiling_input_shape[0])):
        ...
        with self.tik_instance.if_scope(self.in_and_out_indexes[1] < self.pading_10):
            self.in_and_out_indexes[5].set_as(self.pading_10 - self.in_and_out_indexes[1] - self.two_mode_diff)
        with self.tik_instance.if_scope(self.in_and_out_indexes[1] >= self.pading_10 + self.tiling_input_shape[1]):
            self.in_and_out_indexes[5].set_as(2 * self.tiling_input_shape[1] - self.in_and_out_indexes[1] +
                                              self.pading_10 - 2 + self.two_mode_diff)
        with self.tik_instance.if_scope((self.pading_10 <= self.in_and_out_indexes[1]) &
                                        (self.in_and_out_indexes[1] < self.pading_10 + self.tiling_input_shape[1])):
        ...
        with self.tik_instance.if_scope(self.in_and_out_indexes[2] < self.pading_20):
            self.in_and_out_indexes[6].set_as(self.pading_20 - self.in_and_out_indexes[2] - self.two_mode_diff)
        with self.tik_instance.if_scope(self.in_and_out_indexes[2] >= self.pading_20 + self.tiling_input_shape[2]):
            self.in_and_out_indexes[6].set_as(2 * self.tiling_input_shape[2] - self.in_and_out_indexes[2] +
                                              self.pading_20 - 2 + self.two_mode_diff)
        with self.tik_instance.if_scope((self.pading_20 <= self.in_and_out_indexes[2]) &
                                        (self.in_and_out_indexes[2] < self.pading_20 + self.tiling_input_shape[2])):
        ...
        if axis_nums == AXIS_DIMS_4:
            with self.tik_instance.if_scope(self.in_and_out_indexes[3] < self.pading_30):
                self.in_and_out_indexes[7].set_as(self.pading_30 - self.in_and_out_indexes[3] - self.two_mode_diff)
            with self.tik_instance.if_scope(self.in_and_out_indexes[3] >= self.pading_30 + self.tiling_input_shape[3]):
                self.in_and_out_indexes[7].set_as(2 * self.tiling_input_shape[3] - self.in_and_out_indexes[3] +
                                                  self.pading_30 - 2 + self.two_mode_diff)
            with self.tik_instance.if_scope((self.pading_30 <= self.in_and_out_indexes[3]) &
                                            (self.in_and_out_indexes[3] < self.pading_30 + self.tiling_input_shape[3])):
                self.in_and_out_indexes[7].set_as(self.in_and_out_indexes[3] - self.pading_30)
        # AXIS_DIMS_3
        if axis_nums == AXIS_DIMS_3:
            self.outer_move_in_index.set_as(self.in_and_out_indexes[4] * self.input_dim_1 * self.input_dim_2 +
                                            self.in_and_out_indexes[5] * self.input_dim_2 + self.in_and_out_indexes[6])
        elif axis_nums == AXIS_DIMS_4:
            self.outer_move_in_index.set_as(self.in_and_out_indexes[4] * self.input_dim_1 * self.input_dim_2 *
                                            self.input_dim_3 +
                                            self.in_and_out_indexes[5] * self.input_dim_2 * self.input_dim_3 +
    ...
    def do_tiling_key_mode_1(self, core_id):
        ...
        self.address_rollback_scalar = self.tik_instance.Scalar(self.tiling_dtype,
                                                                "address_rollback_scalar",
                                                                init_value=0)
        with self.tik_instance.if_scope(core_id < self.core_used_num - 1):
            self.outer_process_per_core(self.num_per_core, True)
        with self.tik_instance.if_scope(core_id == self.core_used_num - 1):
            self.outer_process_per_core(self.num_tail_core, False)
    ...
    def do_tiling_key_mode_0(self, core_id):
        ...
        self.address_rollback_scalar = self.tik_instance.Scalar(self.tiling_dtype,
                                                                ...
                                                                init_value=0)
        with self.tik_instance.if_scope(core_id < self.core_used_num - 1):
            self.outer_process_per_core_mode_0(self.num_per_core, True)
        with self.tik_instance.if_scope(core_id == self.core_used_num - 1):
            self.outer_process_per_core_mode_0(self.num_tail_core, False)

    def outer_process_per_core_mode_0(self, outer_num, need_addr_roolback):
        """
        mode_0 outer prcocess
        """
        with self.tik_instance.for_range(0, outer_num) as i:
            self.outer_move_out_index.set_as(self.outer_move_out_core_offset + i)
            self.get_outer_move_in_index(AXIS_DIMS_4)
            if need_addr_roolback:
                with self.tik_instance.if_scope((i == outer_num - 1) & (self.pading_41 != TRANS_MIN_BLKS)):
                    self.address_rollback_scalar.set_as(2)
            self.last_dim_inner_process()

    def outer_process_per_core(self, outer_num, need_addr_roolback):
        """
        ...
        with self.tik_instance.for_range(0, outer_num) as i:
            ...
            if need_addr_roolback:
                with self.tik_instance.if_scope((i == outer_num - 1) & (self.pading_31 == 0) &
                                                (self.output_dim_4 % TRANS_MIN_BLKS != 0)):
                    self.address_rollback_scalar.set_as(1)
                with self.tik_instance.if_scope((i == outer_num - 1) & (self.pading_31 != 0) &
                                                (self.output_dim_4 % TRANS_MIN_BLKS != 0)):
                    self.address_rollback_scalar.set_as(2)
            self.inner_process()
    ...
    def last_dim_inner_process(self):
        ...
        self.ub_second = self.tik_instance.Tensor(self.inner_dtype, [self.output_dim4_max_cnt_block_align * 16 * 16],
                                                  name="ub_second",
                                                  scope=tik.scope_ubuf)
        ...
        self.move_in_start_blocks = self.tik_instance.Scalar(self.tiling_dtype, "move_in_start_blocks", init_value=0)
        # padding40
        ...
        with self.tik_instance.if_scope(self.pading_41 != 0):
            one_loop_process_blocks = self.output_dim4_max_cnt_block_align
            with self.tik_instance.if_scope((self.pading_41 < 16) & (self.address_rollback_scalar == 2)):
                self.tik_instance.data_move(
            ...
            with self.tik_instance.else_scope():
                ...
                self.tik_instance.vnchwconv(False, False, dst_list_1, src_list_1, self.inner_process_nums_tail_loop,
                                            self.conv2_stride_0, self.conv2_stride_1)
                with self.tik_instance.if_scope((self.address_rollback_scalar == 2) & (self.inner_loop_times == 1)):
                    for i in range(16):
    ...
    def inner_process(self):
        ...
        self.output_dim_4_align_blocks = self.tik_instance.Scalar(self.tiling_dtype,
                                                                  ...
                                                                  init_value=0)
        self.inner_loop_times = self.tik_instance.Scalar(self.tiling_dtype, "inner_loop_times", init_value=0)
        self.inner_process_nums_per_loop = self.tik_instance.Scalar(self.tiling_dtype,
                                                                    "inner_process_nums_per_loop",
                                                                    init_value=0)
        self.inner_process_nums_tail_loop = self.tik_instance.Scalar(self.tiling_dtype,
                                                                     "inner_process_nums_tail_loop",
                                                                     init_value=0)
        ...
        max_blocks_per_line = self.output_dim4_max_cnt_block_align
        # in this tiling :input_dim_4_align_blocks <= max_blocks_per_line
        # process padding30
        self.one_loop_process_lines.set_as(max_blocks_per_line / self.output_dim_4_align_blocks)
        with self.tik_instance.if_scope(self.pading_30 != 0):
            self.inner_loop_times.set_as(
                (self.pading_30 + self.one_loop_process_lines - 1) / self.one_loop_process_lines)
            self.inner_process_nums_per_loop.set_as(self.one_loop_process_lines)
            with self.tik_instance.for_range(0, self.inner_loop_times - 1) as inner_loop_index:
                self.start_line.set_as(self.pading_30 + 1 - self.two_mode_diff -
                ...
                self.inner_pading30_process(self.one_loop_process_lines)
            self.inner_process_nums_tail_loop.set_as(self.pading_30 -
                                                     (self.inner_loop_times - 1) * self.inner_process_nums_per_loop)
            self.start_line.set_as(1 - self.two_mode_diff)
            self.move_out_start_line.set_as((self.inner_loop_times - 1) * self.one_loop_process_lines)
            self.inner_pading30_process(self.inner_process_nums_tail_loop)
        # process input_dim3
        ...
        with self.tik_instance.for_range(0, self.inner_loop_times - 1) as inner_loop_index:
            self.move_out_start_line.set_as(inner_loop_index * self.one_loop_process_lines)
            self.inner_input_dim3_process(self.one_loop_process_lines, False)
        with self.tik_instance.if_scope(self.address_rollback_scalar == 1):
            with self.tik_instance.if_scope(self.inner_process_nums_tail_loop * self.output_dim_4 < 16):
                ...
                self.inner_input_dim3_process(self.inner_process_nums_tail_loop, True)
            with self.tik_instance.if_scope(self.inner_process_nums_tail_loop * self.output_dim_4 > 16):
                self.inner_process_nums_tail_loop.set_as(self.input_dim_3 -
                                                         (self.inner_loop_times - 1) * self.inner_process_nums_per_loop)
                self.move_out_start_line.set_as((self.inner_loop_times - 1) * self.one_loop_process_lines)
            ...
            with self.tik_instance.if_scope(self.inner_process_nums_tail_loop * self.output_dim_4 == 16):
                self.inner_process_nums_tail_loop.set_as(self.input_dim_3 -
                                                         (self.inner_loop_times - 1) * self.inner_process_nums_per_loop)
                self.move_out_start_line.set_as((self.inner_loop_times - 1) * self.one_loop_process_lines)
                self.inner_input_dim3_process(self.inner_process_nums_tail_loop, False)
        with self.tik_instance.else_scope():
            self.inner_process_nums_tail_loop.set_as(self.input_dim_3 -
                                                     (self.inner_loop_times - 1) * self.inner_process_nums_per_loop)
            self.move_out_start_line.set_as((self.inner_loop_times - 1) * self.one_loop_process_lines)
        ...
        with self.tik_instance.if_scope(self.pading_31 != 0):
            ...
            self.inner_process_nums_tail_loop.set_as(self.pading_31 -
                                                     (self.inner_loop_times - 1) * self.inner_process_nums_per_loop)
            with self.tik_instance.for_range(0, self.inner_loop_times - 1) as inner_loop_index:
                self.start_line.set_as(self.input_dim_3 + self.two_mode_diff - 1 -
                                       (inner_loop_index + 1) * self.one_loop_process_lines)
                self.move_out_start_line.set_as(inner_loop_index * self.one_loop_process_lines)
                self.inner_pading31_process(self.one_loop_process_lines, False)
            with self.tik_instance.if_scope(self.address_rollback_scalar == 2):
                with self.tik_instance.if_scope(self.inner_process_nums_tail_loop * self.output_dim_4 < 16):
                    self.inner_process_nums_tail_loop.set_as(16 + self.output_dim_4 - 1 / self.output_dim_4)
                    self.start_line.set_as(self.input_dim_3 - self.pading_31 + self.two_mode_diff - 1)
                    self.move_out_start_line.set_as(self.pading_31 - self.inner_process_nums_tail_loop)
                    self.inner_pading31_process(self.inner_process_nums_tail_loop, True)
                with self.tik_instance.if_scope(self.inner_process_nums_tail_loop * self.output_dim_4 > 16):
                    self.start_line.set_as(self.input_dim_3 - self.pading_31 + self.two_mode_diff - 1)
                    self.move_out_start_line.set_as((self.inner_loop_times - 1) * self.one_loop_process_lines)
                    self.inner_process_nums_tail_loop.set_as(self.pading_31 - (self.inner_loop_times - 1) *
                                                             self.inner_process_nums_per_loop)
                    self.inner_pading31_process(self.inner_process_nums_tail_loop, True)
                with self.tik_instance.if_scope(self.inner_process_nums_tail_loop * self.output_dim_4 == 16):
                    self.start_line.set_as(self.input_dim_3 - self.pading_31 + self.two_mode_diff - 1)
                    self.move_out_start_line.set_as((self.inner_loop_times - 1) * self.one_loop_process_lines)
                    self.inner_process_nums_tail_loop.set_as(self.pading_31 - (self.inner_loop_times - 1) *
                                                             self.inner_process_nums_per_loop)
                    self.inner_pading31_process(self.inner_process_nums_tail_loop, False)
            with self.tik_instance.else_scope():
                self.start_line.set_as(self.input_dim_3 - self.pading_31 + self.two_mode_diff - 1)
                self.move_out_start_line.set_as((self.inner_loop_times - 1) * self.one_loop_process_lines)
                self.inner_process_nums_tail_loop.set_as(self.pading_31 -
    ...
    def inner_pading30_process(self, process_lines):
        """
        pading dim3 left padding by data_move
        """
        self.get_vnchw_stride(process_lines, False)
        self.tik_instance.data_move(
            self.ub_first, self.input_gm[self.outer_move_in_index * self.input_dim_3 * self.input_dim_4 +
                                         self.start_line * self.input_dim_4], 0, 1,
            (process_lines * self.input_dim_4 + 15) / 16, 0, 0)
        dst_list_0 = [self.ub_second[TRANS_MIN_BLKS * i] for i in range(TRANS_MIN_BLKS)]
        src_list_0 = [self.ub_first] * TRANS_MIN_BLKS
        self.tik_instance.vnchwconv(False, False, dst_list_0, src_list_0, (process_lines * self.input_dim_4 + 15) / 16,
                                    self.conv1_stride_0, self.conv1_stride_1)
        with self.tik_instance.for_range(0, process_lines) as line_index:
            self.tik_instance.data_move(
                self.ub_first[line_index * self.input_dim_4 * TRANS_MIN_BLKS],
                self.ub_second[(process_lines - line_index - 1) * self.input_dim_4 * TRANS_MIN_BLKS], 0, 1,
                self.input_dim_4, 0, 0)
        self.do_data_move_for_pading_dim_4(self.ub_first, self.ub_second, process_lines)
        ...
        self.tik_instance.vnchwconv(False, False, dst_list_1, src_list_1, (process_lines * self.output_dim_4 + 15) / 16,
                                    self.conv2_stride_0, self.conv2_stride_1)
        self.tik_instance.data_move(
            self.output_gm[self.outer_move_out_index * self.output_dim_3 * self.output_dim_4 +
                           self.move_out_start_line * self.output_dim_4], self.ub_first, 0, 1,
            (process_lines * self.output_dim_4 + 15) / 16, 0, 0)

    def inner_input_dim3_process(self, process_lines, need_addr_roolback):
        """
        ...
        self.tik_instance.data_move(
            self.ub_first, self.input_gm[self.outer_move_in_index * self.input_dim_3 * self.input_dim_4 +
                                         self.move_out_start_line * self.input_dim_4], 0, 1,
            (process_lines * self.input_dim_4 + 15) / 16, 0, 0)
        dst_list_0 = [self.ub_second[TRANS_MIN_BLKS * i] for i in range(TRANS_MIN_BLKS)]
        src_list_0 = [self.ub_first] * TRANS_MIN_BLKS
        self.tik_instance.vnchwconv(False, False, dst_list_0, src_list_0, (process_lines * self.input_dim_4 + 15) / 16,
                                    self.conv1_stride_0, self.conv1_stride_1)
        self.do_data_move_for_pading_dim_4(self.ub_second, self.ub_first, process_lines)
        dst_list_1 = [self.ub_second[TRANS_MIN_BLKS * i] for i in range(TRANS_MIN_BLKS)]
        src_list_1 = [self.ub_first[TRANS_MIN_BLKS * i] for i in range(TRANS_MIN_BLKS)]
        self.tik_instance.vnchwconv(False, False, dst_list_1, src_list_1, (process_lines * self.output_dim_4 + 15) / 16,
                                    self.conv2_stride_0, self.conv2_stride_1)
        if need_addr_roolback:
            self.tik_instance.data_move(
                ...
                (process_lines * self.output_dim_4 + 15) / 16 - 1, 0, 0)
            for i in range(16):
                self.ub_addr_rollback[15 - i].set_as(self.ub_second[process_lines * self.output_dim_4 - i - 1])
            self.tik_instance.data_move(
                self.output_gm[(self.outer_move_out_index + 1) * self.output_dim_3 * self.output_dim_4 - 16],
                self.ub_addr_rollback, 0, 1, 1, 0, 0)
        else:
            self.tik_instance.data_move(
                self.output_gm[self.outer_move_out_index * self.output_dim_3 * self.output_dim_4 +
    ...
    def inner_pading31_process(self, process_lines, need_addr_roolback):
        """
        ...
        if need_addr_roolback:
            self.tik_instance.data_move(
                ...
                self.ub_first, 0, 1, (process_lines * self.output_dim_4 + 15) / 16 - 1, 0, 0)
            for i in range(16):
                self.ub_addr_rollback[15 - i].set_as(self.ub_first[process_lines * self.output_dim_4 - i - 1])
            self.tik_instance.data_move(
                self.output_gm[(self.outer_move_out_index + 1) * self.output_dim_3 * self.output_dim_4 - 16],
                self.ub_addr_rollback, 0, 1, 1, 0, 0)
        else:
            self.tik_instance.data_move(
                self.output_gm[self.outer_move_out_index * self.output_dim_3 * self.output_dim_4 +
                               (self.pading_30 + self.input_dim_3 + self.move_out_start_line) * self.output_dim_4],
                self.ub_first, 0, 1, (process_lines * self.output_dim_4 + 15) // 16, 0, 0)
...
@register_operator("MirrorPad")
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT,
                            para_check.REQUIRED_ATTR_STR, para_check.KERNEL_NAME)
def mirror_pad(x, paddings, y, mode, kernel_name="mirror_pad"):
    ...
    kernel_name : str
        cce kernel name, default value is "mirror_pad"
    ...
    obj = MirrorPadInit(mode, kernel_name)
    obj.init_src_dst_gm(x, paddings, y)

    return obj.mirror_pad_compute()
