...
MODE_6 = 6
# only support fp16, e.g [2028,85], 85->[2,2,1,80]
MODE_7 = 7
# sub mode of mode 3
SUB_MODE_1 = 1
SUB_MODE_2 = 2
...
class SplitV():
    ...
    def compute_mode_2(self, core_id):
        ...
        size_splits_ub = tik_instance.Tensor(self.size_splits_dtype, (num_split_align,), name="size_splits_ub",
                                             scope=tik.scope_ubuf)
        tik_instance.data_move(size_splits_ub, self.size_splits_gm, 0, 1, num_split_align // data_block, 0, 0)
        ...
        for out_i in range(0, self.num_split):
            ...
            with tik_instance.else_scope():
                each_core_elems_temp = ceil_value(out_i_elems, self.core_num)
    ...
    def compute_mode_3(self, core_id):
        ...
        for out_i in range(0, self.num_split):
            ...
            outer = self.shape_before // self.core_num
            tail = self.shape_before % self.core_num
            core_num_i.set_as(tail)
            self.row_num1.set_as(1)
            self.row_num2.set_as(1)
            with tik_instance.if_scope(outer > 0):
                core_num_i.set_as(self.core_num)
    ...
    def compute_mode_7_internal(self, rows, gm_rows_offset, size_splits_ub):
        ...
        """
        with self.tik_instance.new_stmt_scope():
            data_ub = self.tik_instance.Tensor(self.input_dtype, (rows * 128,), name="data_ub",
                                               scope=tik.scope_ubuf)
    ...
    def mode_8_tiling(self):
        ...
        """
        self.outer_loop = self.num_split // self.core_num
        self.outer_tail = self.num_split % self.core_num
        self.block_num = self.outer_tail
        # number of outputs processed by each core
        self.num1 = 1
        self.num2 = 1
        if self.outer_loop > 0:
            self.block_num = self.core_num
            if self.outer_tail > 0:
                self.num1 = self.outer_loop + 1
                self.num2 = self.outer_loop
            else:
                self.num1 = self.outer_loop
                self.num2 = self.outer_loop
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT,
                            para_check.DYNAMIC_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
