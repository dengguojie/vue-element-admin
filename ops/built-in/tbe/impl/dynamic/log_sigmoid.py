...
@register_operator_compute("LogSigmoid", op_mode="dynamic", support_fusion=True)
def log_sigmoid_compute(x, y, kernel_name='log_sigmoid'):
    ...
    tempt_adds_pos = tbe.vadds(tempt_exp_pos, tvm.const(1, "float32"))  # 1+e^(-x)
    tempt_log_pos = tbe.vlog(tempt_adds_pos)  # ln(1+e^(-x))
    ...
    tempt_adds_neg = tbe.vadds(tempt_exp_neg, tvm.const(1, "float32"))  # 1+e^(x)
    tempt_log_neg = tbe.vlog(tempt_adds_neg)  # ln(1+e^(x))
