...
class SignBitsUnpack():
    ...
    def __init__(self, x, y, size, dtype, kernel_name):
        ...
        self.per_mask_u8 = 16
        self.mask_f16 = 128
        if self.dtype == Constant.DT_FLOAT:
            self.mask = 64
            self.pack_rate = 32
        ...
        self.dtype_size = tbe_platform.get_bit_len(self.tensor_dtype) // 8
    ...
    def init_gm_tensor(self):
        ...
        self.tiling_gm = self.tik_inst.Tensor("int64", (Constant.TILING_ARG_NUM,), name="tiling_gm",
                                                  scope=tik.scope_gm)
        self.x_gm = self.tik_inst.Tensor("uint8",
                                         (Constant.MAX_INT32,),
                                         name="x_gm", scope=tik.scope_gm)
        self.y_gm = self.tik_inst.Tensor(self.tensor_dtype,
                                         (Constant.MAX_INT32,),
    ...
    def unpack_bit_compute(self, repeat_time, repeat_left, offset):
        ...
        :return: None
        """
        if self.dtype == Constant.DT_FLOAT16:
            dst_ub = self.y_ub
        else:
            dst_ub = self.f16_ub
...
def _check_param(x, y, size, dtype=Constant.DT_FLOAT):
    """
    ...
    if len(x_shape) != 1:
        raise RuntimeError("input x shape must be 1 dim.")

    if x_dtype != "uint8":
        raise RuntimeError("input x dtype must be uint8.")

    if dtype != Constant.DT_FLOAT16 and dtype != Constant.DT_FLOAT:
        raise RuntimeError("param dtype must be float16 or float32.")

    if size <= 0:
        raise RuntimeError("dim size must be larger than zero.")
