...
class MaxPoolWithargmaxPytorch:
    ...
    def reduce_max_repeat_width(self, dst, src0, src1, size, src0_blk=1, src1_blk=1):
        ...
        if self.strides_w * self.burst_c0 <= 31:
            with self.tik_instance.if_scope(size > 0):
                size_loop = size // self.mask
                size_left = size % self.mask
                repeat_loop = size_loop // Constant.REPEAT_LIMIT
    ...
    def reduce_max_repeat_height_ksize_more_width(self, ub_x, ub_y, repeat_ph, size_ow):
        ...
        if self.strides_w == self.ksize_w:
            ...
            self.reduce_max_repeat_height(ub_y, ub_x, ub_x[self.c_zero:], repeat_ph, size_ow,
                                          self.strides_w, self.strides_w,
                                          self.output_w, self.pad_w, self.pad_w)
            with self.tik_instance.for_range(0, self.ksize_w - 2) as idx:
                offset_w = (idx + 2) * self.c_zero
    ...
    def tiling_h_dim_core_nc_process(self, core_idx, core_ele, loop_idx, ele):
        ...
        def _inner(ub_x, ub_y, ub_z, ele_idx):
            ...
            self.caculate_mask(ub_x, ub_y, ub_z, ele, self.output_w * self.c_zero,
                               self.align_output_w, self.pad_w * self.c_zero, self.ksize_h, self.ksize_w)
    ...
    def tiling_w_dim_core_nc_process(self, core_idx, core_ele, h_idx, loop_idx, ele, align_ele):
        ...
        def _inner(ub_x, ub_y, ub_z, ele_idx):
            ...
            # reduce max width
            if self.ksize_w == 1:
                self.reduce_max_repeat_width_ksize_one_width(ub_x, ub_y, self.ksize_h, ele * self.c_zero,
                                                             self.len_w * self.c_zero)
            else:
                self.reduce_max_repeat_width_ksize_more_width(ub_x, ub_y, self.ksize_h, ele * self.c_zero,
                                                              self.len_w * self.c_zero)
            # reduce max height
            if self.ksize_h == 1:
            ...
            self.caculate_mask(ub_x, ub_y, ub_z, 1, ele * self.c_zero,
                               align_ele, self.len_w * self.c_zero, self.ksize_h, self.ksize_w)
            ...
            # move out the mask and gather the mask in gm at the same time
            base_dst = (core_idx * self.one_core_ele + ele_idx) * self.ksize_w * self.ksize_h * \
