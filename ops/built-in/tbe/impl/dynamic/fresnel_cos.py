...
def generic_fresnel_asymp(x, y):
    ...
    res_1 = tbe.vmul(data_f, data_s)
    res_2 = tbe.vmul(data_g, data_c)
    res_sub = tbe.vsub(res_1, res_2)
    res_3 = tbe.vmul(res_sub, data_y)
    res = tbe.vadd(const_half, res_3)
...
def _generic_fresnel_cos_interval(x):
    ...
    data_cn_polevl = polevl(tmp_x4, Constant.SCALAR_CN, Constant.CN_COUNT)
    data_cd_polevl = polevl(tmp_x4, Constant.SCALAR_CD, Constant.CD_COUNT)
    res = tbe.vmul(x, data_cn_polevl)
    res = tbe.vdiv(res, data_cd_polevl)

    return res


@register_operator_compute("FresnelCos", op_mode="dynamic", support_fusion=True)
def fresnel_cos_compute(x, y, kernel_name="fresnel_cos"):
    """
    do element-wise fresnel_cos compute
    |x| > b, y = 0.5
    x^2 >= a, rec = 1 / (π * x^2), 
              f = 1 - rec^2 * polevel(rec^2, FN, 9) / plevel(rec^2, FD, 10),
              g = rec * polevel(rec^2, GN, 10) / plevel(rec^2, GD, 11)
              y = 0.5 + 1 / (π * x) * (f * sin(π * x^2 / 2) - g * cos(π * x^2 / 2))
    x^2 < a, y = x * polevel(x^4, CN, 5) / plevel(x^4, CD, 6)
    Parameters:
    ----------
    ...
    kernel_name : cce kernel name, default value is "fresnel_cos"
    ...
    res_lt_a = _generic_fresnel_cos_interval(x_d_0)
...
@register_operator("FresnelCos")
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT,
                            para_check.KERNEL_NAME)
def fresnel_cos(x, y, kernel_name="fresnel_cos"):
    """
    Parameters:
    ----------
    x : the placeholder of data input
    ...
    for (_x,) in ins:
        with tbe.compute():
            ...
            data_input = tvm.placeholder(x_shape[0], dtype=dtype_input,
                                         name="data_input")
            res = fresnel_cos_compute(data_input, y, kernel_name)
