...
class SparseApplyFtrl(SparseApplyDynamic):
    ...
    def _check_param(self):
        ...
        para_check.check_dtype(self.linear_dtype, ("float32", ), param_name="linear")
    ...
    def _calculate(self, repeat_times, mask, offset):
        ...
        if isinstance(offset, int) and offset > tmp_ub.size:
            return
        if isinstance(offset, int) and offset > tmp2_ub.size:
            return
        len_ele = mask * repeat_times
        if isinstance(len_ele, int) and len_ele > self.block_len:
            return
...
def sparse_apply_ftrl_v2_d(var,
    ...
    None
    """
    apply_ftrl = SparseApplyFtrl(var, accum, linear, grad, indices, lr, lr_power, l1, l2, l2_shrinkage, kernel_name)

    apply_ftrl.add_input("var_in_gm", "float32")
    apply_ftrl.add_input("accum_in_gm", "float32")
    apply_ftrl.add_input("linear_in_gm", "float32")
    apply_ftrl.add_output("var_out_gm", "float32")
    apply_ftrl.add_output("accum_out_gm", "float32")
    apply_ftrl.add_output("linear_out_gm", "float32")
