...
class SparseApplyAdadelta(SparseApplyDynamic):
    ...
    def _calculate(self, repeat_times, mask, offset):
        ...
        if isinstance(offset, int) and offset > tmp1_ub.size:
            return
...
def sparse_apply_adadelta_d(var,
    ...
    sparse_apply_adadelta.add_input("var_in_gm", var_dtype)
    sparse_apply_adadelta.add_input("accum_in_gm", var_dtype)
    sparse_apply_adadelta.add_input("accum_update_in_gm", var_dtype)
    sparse_apply_adadelta.allocate_scalar_gm("lr_gm", var_dtype)
    sparse_apply_adadelta.allocate_scalar_gm("rho_gm", var_dtype)

    sparse_apply_adadelta.add_output("var_out_gm", var_dtype)
    sparse_apply_adadelta.add_output("accum_out_gm", var_dtype)
