...
def dynamic_rnn_v2(input_x, weight_input, weight_hidden, bias, seq_length, init_h, init_c, wci, wcf,
    ...
    if is_dynamic:
        dynamic_config = dynamic_build_config_dict
        dynamic_config["dump_cce_code"] = False
        dynamic_config["sync_mode"] = 2
...
def dynamic_rnn_core(input_x, weight_i, weight_h, bias, s_init_h_gm, s_init_c_gm,
    ...
    for tensor in elewise_tensors:
        if tensor != barrier_tensor:
            s[tensor].set_storage_bound(SHAPE_I)

    s[s_state_h_ub].set_storage_bound(SHAPE_H)
    s[s_state_c_ub].set_storage_bound(SHAPE_I)

    s[a_l1_1].set_storage_bound(SHAPE_A_Z_BIGZ)
    s[b_l1_1].set_storage_bound(SHAPE_B_1)
    s[a_l0a_1].set_storage_bound(SHAPE_A_Z_BIGZ)
    s[b_l0b_1].set_storage_bound(SHAPE_B_1)
    s[c_l0c_1].set_storage_bound(SHAPE_C_1)
    s[c_ub_1].set_storage_bound(SHAPE_C_1)

    if bias is not None:
        s[bias_ub].set_storage_bound(SHAPE_BIAS)
        if fp16_input_output:
            s[bias_ub_fp32].set_storage_bound(SHAPE_BIAS)
        s[bias_bc_ub].set_storage_bound(SHAPE_C_1)

    s[a_l1_2].set_storage_bound(SHAPE_H_Z_BIGZ)
    s[b_l1_2].set_storage_bound(SHAPE_B_2)
    s[a_l0a_2].set_storage_bound(SHAPE_H_Z_BIGZ)
    s[b_l0b_2].set_storage_bound(SHAPE_B_2)
    s[c_l0c_2].set_storage_bound(SHAPE_C_2)
    s[c_ub_2].set_storage_bound(SHAPE_C_2)
    s[c_ub_bias].set_storage_bound(SHAPE_C_2)

    s[i_t].set_storage_bound(SHAPE_I)
    s[j_t].set_storage_bound(SHAPE_I)
    s[f_t].set_storage_bound(SHAPE_I)
    s[o_t].set_storage_bound(SHAPE_I)

    s[f_t_sigmoid_ub].set_storage_bound(SHAPE_I)
    s[i_t_sigmoid_ub].set_storage_bound(SHAPE_I)
    s[o_t_sigmoid_ub].set_storage_bound(SHAPE_I)
    s[j_t_tanh_ub].set_storage_bound(SHAPE_I)

    if is_gate_output:
        if fp16_input_output:
            s[f_t_sigmoid_fp16].set_storage_bound(SHAPE_I)
            s[i_t_sigmoid_fp16].set_storage_bound(SHAPE_I)
            s[o_t_sigmoid_fp16].set_storage_bound(SHAPE_I)
            s[j_t_tanh_fp16].set_storage_bound(SHAPE_I)
        if fp16_input_output:
            s[s_state_c_back_fp32].set_storage_bound(SHAPE_I)

    if bias_dtype == 'float16':
        s[update_c_fp16].set_storage_bound(SHAPE_I)

    if bias_dtype == 'float16':
        s[update_c_fp16_back].set_storage_bound(SHAPE_I)
        s[update_c_fp16_back_fp32].set_storage_bound(SHAPE_I)
    else:
        s[update_c_fp32_back].set_storage_bound(SHAPE_I)

    if is_gate_output:
        if fp16_input_output:
            s[c_t_tanh_fp16].set_storage_bound(SHAPE_I)

    if fp16_input_output:
        s[update_h_fp16].set_storage_bound(SHAPE_I)
        s[update_h_gm_as_y_back].set_storage_bound(SHAPE_I)
        s[last_update_h_back].set_storage_bound(SHAPE_I)
    else:
        s[update_h_gm_as_y_back].set_storage_bound(SHAPE_I)
        s[update_h_fp16_cast].set_storage_bound(SHAPE_I)
        s[last_update_h_back].set_storage_bound(SHAPE_I)
    ...
    if bias is not None:
        ...
        s[bias_bc_ub].emit_insn(bias_bc_ub.op.axis[0], 'vector_broadcast')
