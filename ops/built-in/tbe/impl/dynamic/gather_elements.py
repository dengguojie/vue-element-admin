...
def check_supported(x_dict, indices_dict, y_dict, dim=0, kernel_name="GatherElements"):
    ...
    for i in indices_shape:
        indicesTotal *= i
    paramsDSize = Constant.TYPE_LEN_DICT.get(x_dtype)
    paramsBlockNum = Constant.BLOCK_SIZE // paramsDSize
    paramsTotalCeil = (paramsTotal + paramsBlockNum - 1) // paramsBlockNum * paramsBlockNum
    if paramsTotalCeil >= Constant.CACHE_UB_SIZE // paramsDSize:
        if indicesTotal >= Constant.THRESHOULD:
            reason = "params num is large than cache_ub_size and indices num is large than 2k"
            return False, reason
    return True, ""
...
class GatherElements(object):
    ...
    def output_matrix(self, indices_index):
        ...
        loop_pre = indices_index // (self.params_row * self.indices_axis)
    ...
    def __init__(self, params_dict, indices_dict, y_dict, axis, kernel_name):
        ...
        self.param_large_than_indices = None
    ...
    def get_tiling_args(self, tiling_ub):
        ...
        self.param_large_than_indices = self.tik_instance.Scalar(dtype=self.tiling_dtype, 
                                                                 name="param_large_than_indices")
        self.param_large_than_indices.set_as(tiling_ub[14])
        self.indices_axis = self.tik_instance.Scalar(dtype=self.tiling_dtype, name="indices_axis")
        self.indices_axis.set_as(tiling_ub[15])
    ...
    def gather_elements_compute_tiling(self):
        ...
        half_ub_size = (self.ub_size - Constant.RESERVED_UB_SIZE) // 2
        remain_half_ub_size = (self.ub_size - Constant.CACHE_UB_SIZE - Constant.RESERVED_UB_SIZE) // 2

        tiling_ub = tik_instance.Tensor(self.tiling_dtype, (16,), name="tiling_ub", scope=tik.scope_ubuf)
        tiling_size = ceil_value(Constant.TILING_ARG_NUM * Constant.TYPE_LEN_DICT.get(self.tiling_dtype), 
                                 Constant.BLOCK_SIZE)
        tik_instance.data_move(tiling_ub, self.tiling_gm, 0, 1, tiling_size, 0, 0)
    ...
    def compute_mode_1(self, half_ub_size, block_id):
        ...
        indices less than the number contained in one block of each core
        ...
        with tik_instance.if_scope(block_id < 1):
            ...
            with tik_instance.if_scope(self.indices_row_num_last > 0):
                tik_instance.data_move(indices_ub, self.indices, 0, 1, 
                                       ceil_value(self.indices_row_num_last * self.indices_dsize, Constant.BLOCK_SIZE),
                                       0, 0)
            
            self.compute_x_larger_cache(indices_ub, res_ub, self.x, self.y, 0, self.indices_row_num_last)
    ...
    def compute_mode_2(self, half_ub_size, block_id):
        ...
        tik_instance.data_move(x_ub, self.x, 0, 1, 
                               ceil_value(self.params_total, Constant.BLOCK_SIZE // self.params_dsize), 0, 0)
        ...
        with tik_instance.if_scope(block_id < 1):
            ...
            self.compute_x_less_cache(indices_ub, res_ub, x_ub, self.y, 0, self.indices_row_num_last)
    ...
    def compute_mode_3(self, half_ub_size, block_id):
        ...
        indices_num_each_block = Constant.BLOCK_SIZE // self.indices_dsize
        params_num_each_block = Constant.BLOCK_SIZE // self.params_dsize
        ...
        with tik_instance.for_range(0, self.indices_loop_num) as indices_loop_i:
            ...
            tik_instance.data_move(indices_ub, self.indices[indices_num_offset], 0, 1,
                                   ...
                                   0, 0)
            self.compute_x_larger_cache(indices_ub, res_ub, self.x, 
                                        self.y[indices_num_offset], indices_num_offset, self.indices_row_num_once)
        ...
        with tik_instance.if_scope(self.indices_row_num_last > 0):
            ...
            tik_instance.data_move(indices_ub, self.indices[indices_num_offset], 0, 1,
                                   ...
                                   0, 0)
            self.compute_x_larger_cache(indices_ub, res_ub, self.x, 
                                        self.y[indices_num_offset], indices_num_offset, self.indices_row_num_last)
        ...
        with tik_instance.if_scope(tik.all(self.indices_num_remaining > 0, block_id < self.remaining_block_num)):
            indices_num_offset = self.indices_num_each_core * self.need_core_num + \
                                    block_id * indices_num_each_block * self.param_large_than_indices
            tik_instance.data_move(indices_ub, self.indices[indices_num_offset], 0, 1, 
                                   self.param_large_than_indices, 0, 0)
            self.process_remaining_num_block_x_block(indices_num_each_block * self.param_large_than_indices, 
                                                     indices_num_offset, indices_ub, res_ub, self.x)
            block_indices_in_params = indices_num_each_block * self.param_large_than_indices // params_num_each_block
            tik_instance.data_move(self.y[indices_num_offset], res_ub, 0, 1, block_indices_in_params, 0, 0)
        ...
        with tik_instance.if_scope(tik.all(self.indices_num_remaining > 0, 
            ...
            indices_num_offset = self.indices_num_each_core * self.need_core_num + \
                                    block_id * indices_num_each_block * self.param_large_than_indices
            tik_instance.data_move(indices_ub, self.indices[indices_num_offset], 0, 1, 
                                   ceil_value(self.remaining_block_remain, indices_num_each_block), 0, 0)
            self.process_remaining_num_block_x_block(self.remaining_block_remain, 
                                                     indices_num_offset, indices_ub, res_ub, self.x)
            block_indices_in_params = ceil_value(self.remaining_block_remain, params_num_each_block)
            tik_instance.data_move(self.y[indices_num_offset], res_ub, 0, 1, block_indices_in_params, 0, 0)
    ...
    def compute_mode_4(self, half_ub_size, block_id):
        ...
        half_ub_indices = half_ub_size // self.indices_dsize
        indices_loop_num = self.indices_num_each_core // half_ub_indices
        indices_row_num_once = half_ub_indices
        indices_row_num_last = self.indices_num_each_core % half_ub_indices
        ...
        with tik_instance.for_range(0, indices_loop_num) as indices_loop_i:
            ...
            tik_instance.data_move(indices_ub, self.indices[indices_num_offset], 0, 1,
                                   ceil_value(indices_row_num_once * self.indices_dsize, Constant.BLOCK_SIZE), 0, 0)
            self.compute_x_less_cache(indices_ub, res_ub, x_ub, 
                                      self.y[indices_num_offset], indices_num_offset, indices_row_num_once)
        ...
        with tik_instance.if_scope(indices_row_num_last > 0):
            ...
            tik_instance.data_move(indices_ub, self.indices[indices_num_offset], 0, 1,
                                   ceil_value(indices_row_num_last * self.indices_dsize, Constant.BLOCK_SIZE), 0, 0)
            self.compute_x_less_cache(indices_ub, res_ub, x_ub, 
                                      self.y[indices_num_offset], indices_num_offset, indices_row_num_last)
    
        with tik_instance.if_scope(tik.all(self.indices_num_remaining > 0, block_id < self.remaining_block_num)):
            indices_num_offset = self.indices_num_each_core * self.need_core_num + \
                                 block_id * indices_num_each_block * self.param_large_than_indices
            tik_instance.data_move(indices_ub, self.indices[indices_num_offset], 
                                   0, 1, self.param_large_than_indices, 0, 0)
            self.process_remaining_num_block(indices_num_each_block * self.param_large_than_indices, 
                                             indices_num_offset, indices_ub, res_ub, x_ub)
            block_indices_in_params = indices_num_each_block * self.param_large_than_indices // params_num_each_block
            tik_instance.data_move(self.y[indices_num_offset], res_ub, 0, 1, block_indices_in_params, 0, 0)
        ...
        with tik_instance.if_scope(tik.all(self.indices_num_remaining > 0, 
            ...
            tik_instance.data_move(indices_ub, self.indices[indices_num_offset], 0, 1, 
                                   ceil_value(self.remaining_block_remain, indices_num_each_block), 0, 0)
            self.process_remaining_num_block(self.remaining_block_remain, indices_num_offset, indices_ub, res_ub, x_ub)
            block_indices_in_params = ceil_value(self.remaining_block_remain, params_num_each_block)
            tik_instance.data_move(self.y[indices_num_offset], res_ub, 0, 1, block_indices_in_params, 0, 0)
    
    def compute_x_larger_cache(self, indices_ub, res_ub, x_src, y_dst, offset, processNum):
        """
        ...
        tik_instance = self.tik_instance
        with tik_instance.for_range(0, processNum) as index_i:
            tail_row, loop_pre = self.output_matrix(index_i + offset)
            with tik_instance.new_stmt_scope():
                ...
                block_ub = tik_instance.Tensor(self.params_dtype, (Constant.BLOCK_SIZE // self.params_dsize, ),
                                               name="block_ub", scope=tik.scope_ubuf)
                with tik_instance.if_scope(gm_offset + Constant.BLOCK_SIZE // self.params_dsize > self.params_total):
                    tik_instance.data_move(block_ub, x_src[1 + gm_offset - Constant.BLOCK_SIZE // self.params_dsize],
                                           0, 1, 1, 0, 0)
                    res_ub[index_i].set_as(block_ub[Constant.BLOCK_SIZE // self.params_dsize - 1])
                with tik_instance.else_scope():
                    tik_instance.data_move(block_ub, x_src[gm_offset], 0, 1, 1, 0, 0)
                    res_ub[index_i].set_as(block_ub)
        
        tik_instance.data_move(y_dst, res_ub, 0, 1, 
                               ceil_value(processNum * self.params_dsize, Constant.BLOCK_SIZE), 0, 0)
    
    def compute_x_less_cache(self, indices_ub, res_ub, x_src, y_dst, offset, processNum):
        """
    ...
    def process_remaining_num_block(self, indices_num_each_block, indices_num_offset, indices_ub, res_ub, x_src):
        ...
        Parameters
        ----------
        indices_num_each_block: indices num of each block
    ...
    def process_remaining_num_block_x_block(self, indices_num_each_block, 
                                            indices_num_offset, indices_ub, res_ub, x_src):
        """
        ...
        with tik_instance.new_stmt_scope():
            ...
            with tik_instance.for_range(0, indices_num_each_block) as indices_num_i:
                ...
                with tik_instance.if_scope(gm_offset + Constant.BLOCK_SIZE // self.params_dsize > self.params_total):
                    tik_instance.data_move(block_ub, x_src[1 + gm_offset - Constant.BLOCK_SIZE // self.params_dsize], 
                                           0, 1, 1, 0, 0)
                    res_ub[indices_num_i].set_as(block_ub[Constant.BLOCK_SIZE // self.params_dsize - 1])
                with tik_instance.else_scope():
                    tik_instance.data_move(block_ub, x_src[gm_offset], 0, 1, 1, 0, 0)
