...
def wts_arq_compute(w, w_min, w_max, num_bits, offset_flag):
    ...
    shape_w_min = shape_util.shape_to_list(w_min.shape)
    shape_w_max = shape_util.shape_to_list(w_max.shape)

    _, _, shape_broadcast = shape_util.broadcast_shapes(shape_w, shape_w_min)
    _, _, shape_broadcast = shape_util.broadcast_shapes(shape_w, shape_w_max)
...
def wts_arq(w, w_min, w_max, y, num_bits=8, offset_flag=False, kernel_name='wts_arq'):
    ...
    if len(w_shape) != len(w_min_shape):
        error_detail = "The shape size of w_min&w_max must be the same as w!"
