...
class DynSeqOuter(object):
    ...
    def dyn_seq_outer_compute(self):
        ...
        with self.tik_instance.for_range(0, self.used_aicore_num, block_num=self.used_aicore_num) as i:
            with self.tik_instance.for_range(0, batch_num_per_aicore) as j:
                self.dyn_seq_outer_compute_core(i + j * self.used_aicore_num)
            with self.tik_instance.if_scope(i < batch_tail):
                self.dyn_seq_outer_compute_core(batch_num_per_aicore * self.used_aicore_num + i)
    ...
    def dyn_seq_outer_compute_core(self, task_idx):
        ...
        with self.tik_instance.for_range(0, task_num) as idx:
            ...
            if self.dtype == "float16" and self.version == "aic":
                self.tik_instance.set_atomic_add(2)
            elif self.dtype == "float32" and self.version != "mini":
