...
class SpaceToBatchND:
    ...
    def run_block_h(self, ub_a, ub_b, core_idx, idx_bh, ele_idx):
        ...
        offset_gm_in = (core_idx * self.one_core_ele + ele_idx) * self.input_h * self.input_w * self.channel_zero + \
    ...
    def run_width(self, ub_a, ub_b, core_idx, idx_oh, idx_bh, ele_idx):
        """run width function.
        """
        # vector dup and move in
                ...
                with self.tik_instance.for_range(0, self.block_h) as idx_bh:
                    with self.tik_instance.for_range(0, core_ele, thread_num=2) as ele_idx:
                        ...
                        ub_b = self.tik_instance.Tensor(self.dtype, (self.ub_ele // 4,),
                                                        name="ub_b",
                                                        scope=tik.scope_ubuf)
                        self.run_width(ub_a, ub_b, core_idx, idx_oh, idx_bh, ele_idx)
    ...
    def run_last(self, ub_a, core_idx, idx_oh, idx_bh, idx_ow, idx_bw, ele_idx):
        """run last function.
        """
        # vector dup and move in
                        ...
                        with self.tik_instance.for_range(0, self.block_w) as idx_bw:
                            with self.tik_instance.for_range(0, core_ele) as ele_idx:
                                ub_a = self.tik_instance.Tensor(self.dtype, (self.ub_ele,),
                                                                name="ub_a",
                                                                scope=tik.scope_ubuf)
                                self.run_last(ub_a, core_idx, idx_oh, idx_bh, idx_ow, idx_bw, ele_idx)
