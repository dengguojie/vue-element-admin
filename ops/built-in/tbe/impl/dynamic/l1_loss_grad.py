...
def l1_loss_grad_compute(grads, predict, label, y, reduction="mean", kernel_name="l1_loss_grad"):
    ...
    zero_tensor = tbe.vmuls(predict, tvm.const(0, dtype=data_type))
    sign = tbe.vcmpsel(predict, label, "gt", 1.0, -1.0)
    sign = tbe.cast_to(sign, data_type)
    # rectify sign to 0 when predict equal to label
    sign = tbe.vcmpsel(predict, label, "eq", zero_tensor, sign)
    sign = tbe.cast_to(sign, data_type)
    ...
    if predict.dtype.lower() == "float32" and cce_product == "Ascend310":
