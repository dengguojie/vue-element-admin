...
class Iou:
    ...
    def __init__(self, bboxes, gtboxes, mode):
        ...
        self.product = tbe_platform.api_check_support(
            "tik.vdiv", "float32")
    ...
    def iou_process(self, core_id):
        ...
        with self.tik_instance.if_scope(core_id < self.core_num):
            ...
            with  self.tik_instance.if_scope(self.core_tail_num != 0):
                with self.tik_instance.if_scope(core_id == (
                        self.core_num - 1)):
                    ...
                    with self.tik_instance.for_range(
                            0, self.bb_loop, thread_num=2) as bb_loop_index:
        ...
        with self.tik_instance.for_range(0, for_range) as conv_index:
            ...
            for i in range(2):
                self.area_x0[conv_index * 2 + i] \
                    .set_as(index_reg[i * 4 + 0])
                self.area_y0[conv_index * 2 + i] \
                    .set_as(index_reg[i * 4 + 1])
                self.area_x1[conv_index * 2 + i] \
                    .set_as(index_reg[i * 4 + 2])
                self.area_y1[conv_index * 2 + i] \
                    .set_as(index_reg[i * 4 + 3])
    ...
    def calcu_area(self, run_point, area_ub, inter_mode=False):
        ...
        if inter_mode is False:
            self.tik_instance.vadds(self.max_eliments, area_ub,
                                    area_ub, 0.01, repeat_time, 1, 1, 8,
                                    8)
            self.tik_instance.vadds(self.max_eliments, self.area_y1_y0,
                                    self.area_y1_y0, 0.01, repeat_time, 1, 1, 8,
                                    8)
...
def iou(bboxes, gtboxes, overlap, mode="iou", kernel_name="iou"):
    ...
    res = Iou(bboxes, gtboxes, mode).run_tik(kernel_name)
