...
def _padding_short_c(tik_inst, dst_ub, src_ub, c_len, dhw_len, reg_list):
    ...
    c_align_len = _ceil_div(c_len, Constant.C0_LEN) * Constant.C0_LEN
    ...
    def _inner_conv_process():
        ...
        with tik_inst.for_range(0, dhw_len) as dhw_idx:
            ...
            if c_left:
                for idx in Constant.REG_IDX_LIST[:c_left]:
...
def _padding_long_c(tik_inst, dst_ub, src_ub, c_len, reg_list):
    ...
    c0_cnt_in_c = c_len // Constant.C0_LEN
    c_left = c_len % Constant.C0_LEN
    reg_cnt = len(reg_list)
    ...
    if c_left:
        ...
        if reg_loop:
            with tik_inst.for_range(0, reg_loop) as reg_lp_idx:
                for idx in Constant.REG_IDX_LIST:
                    src_offset = reg_lp_idx * reg_cnt + idx + c0_cnt_in_c * Constant.C0_LEN
                    reg_list[idx].set_as(src_ub[src_offset])

                for idx in Constant.REG_IDX_LIST:
                    dst_offset = reg_lp_idx * reg_cnt + idx + c0_cnt_in_c * Constant.C0_LEN
                    dst_ub[dst_offset].set_as(reg_list[idx])

        if left_cnt:
            for idx in Constant.REG_IDX_LIST[:c_left]:
                src_offset = reg_loop * reg_cnt + idx + c0_cnt_in_c * Constant.C0_LEN
                reg_list[idx].set_as(src_ub[src_offset])

            for idx in Constant.REG_IDX_LIST[:c_left]:
                dst_offset = reg_loop * reg_cnt + idx + c0_cnt_in_c * Constant.C0_LEN
                dst_ub[dst_offset].set_as(reg_list[idx])
...
def _multi_core_on_n(tik_inst, data_in, data_out, shape_in):
    ...
    dhwc_align_size = func_reduce(lambda x, y: x * y, (axis_d, hw_size, axis_c1, Constant.C0_LEN))
    # deduct 32 Bytes to avoid the repeat times of MTE bigger than 4095
    half_ub_size = (Constant.UB_SIZE - 4 * Constant.BLOCK_BYTE_SIZE) // 2 // dtype_factor
    ...
    if axis_c % Constant.C0_LEN:
        ...
        ub_size = Constant.UB_SIZE // dtype_factor
    ...
    with tik_inst.for_range(0, core_num, block_num=core_num) as block_idx:
            ...
            with tik_inst.for_range(0, n_len) as n_idx:
                ...
                def _transfer_4_dhwc_less_half_ub():
                    ...
                    tik_inst.data_move(in_ub,
                                       ...
                                       0, 1, _ceil_div(dhwc_size, Constant.BLOCK_BYTE_SIZE // dtype_factor),
                                       0, 0)
                    ...
                    with tik_inst.for_range(0, axis_d) as d_idx:
                        with tik_inst.for_range(0, axis_c1) as c1_idx:
                            ...
                            mid_offset = (d_idx * hw_size * axis_c1 + c1_idx) * Constant.C0_LEN
                        ...
                        with tik_inst.for_range(0, hw_size) as hw_idx_1:
                            ...
                            def _inner_c_transfer(sub_c, in_offset, out_offset):
                                ...
                                tik_inst.data_move(
                                    in_ub,
                                    data_in[in_offset],
                                    0, 1, _ceil_div(sub_c, Constant.BLOCK_BYTE_SIZE // dtype_factor), 0, 0)

                                c0_cnt_in_sub_c = _ceil_div(sub_c, Constant.C0_LEN)
                                repeat_stride = hw_size * ni_no_size * dtype_factor // 2
                                hwninoc0_size = hw_size * ni_no_size * Constant.C0_LEN
                                if sub_c % Constant.C0_LEN:
                                    with tik_inst.if_scope(n_idx == 0):
                                        _clean_ubuf(tik_inst, out_ub,
                                                    0, c0_cnt_in_sub_c * Constant.C0_LEN)
                                ...
                                if c0_cnt_in_sub_c <= 4095 and repeat_stride <= 65536:
                                    ...
                                    with tik_inst.for_range(0, c0_cnt_in_sub_c) as c0_idx:
                                        tik_inst.data_move(
                                            ...
                                            temp_ub[c0_idx * Constant.C0_LEN],
                            ...
                            if axis_c < ub_size:
                                ...
                                with tik_inst.for_range(0, c_lp_cnt) as c_lp_idx:
                                    ...
                                    output_offset_1 = (block_idx * per_core_n_cnt + n_idx +
                                                       (d_idx_1 * axis_c1 * hw_size + hw_idx_1) *
                                                       ni_no_size + c_lp_idx * (ub_size // Constant.C0_LEN)
                                                       * hw_size * ni_no_size) * Constant.C0_LEN
                                ...
                                if c_left:
                                    ...
                                    output_offset_1 = (block_idx * per_core_n_cnt + n_idx +
                                                       (d_idx_1 * axis_c1 * hw_size + hw_idx_1) *
                                                       ni_no_size + c_lp_cnt * (ub_size // Constant.C0_LEN)
                                                       * hw_size * ni_no_size) * Constant.C0_LEN
        ...
        if axis_n % Constant.C0_LEN:
            ...
            def _padding_ni_no_process(z_lp_cnt, z_lp_index):
                ...
                """
                hwc0_size = hw_size * Constant.C0_LEN
                if hwc0_size > ub_size:
                    _clean_ubuf(tik_inst, in_ub, 0, ub_size)
                    hw_lp_cnt = hwc0_size // ub_size
                    repeat_time = ub_size // Constant.C0_LEN
                    hw_left_size = hwc0_size % ub_size // Constant.C0_LEN
                else:
                    _clean_ubuf(tik_inst, in_ub, 0, hw_size * Constant.C0_LEN)
                    hw_lp_cnt = 1
                    repeat_time = hw_size
                    hw_left_size = 0
                ...
                def _padding_ni_no_cube(out_offset):
                    ...
                    with tik_inst.for_range(0, hw_lp_cnt) as hw_lp_idx:
                        tik_inst.data_move(data_out[out_offset + hw_lp_idx * ni_no_size * Constant.C0_LEN],
                                           in_ub,
                                           0, repeat_time, dtype_factor // 2,
                                           0, (ni_no_size - 1) * dtype_factor // 2)
                    if hw_left_size:
                        tik_inst.data_move(data_out[out_offset + hw_lp_cnt * ni_no_size * Constant.C0_LEN],
                                           in_ub,
                    ...
                    with tik_inst.for_range(0, axis_c1) as c1_idx:
                        output_offset = \
                            (axis_n + block_idx * z_lp_cnt + z_lp_index +
                             (d_idx * axis_c1 + c1_idx) * hw_size * ni_no_size) * Constant.C0_LEN
...
def ndhwc_2_fractal_z_3d(src, dst, src_format, dst_format,
    ...
    dst_shape = (in_shape[1], _ceil_div(in_shape[4], Constant.C0_LEN), in_shape[2],
                 in_shape[3], _ceil_div(in_shape[0], Constant.C0_LEN), Constant.C0_LEN, Constant.C0_LEN)
    ...
    white_list = [[9973, 1, 253, 2, 3], [7, 1, 1, 2, 4243]]
    if list(in_shape) in white_list:
        trans_data_positive_source_tc.trans_data_positive_source_tc(src, dst, src_format,
