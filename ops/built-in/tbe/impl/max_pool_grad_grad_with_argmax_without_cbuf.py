...
class MaxPoolGradGradWithArgmaxWithoutCbuf:
    def __init__(self, x, grad, argmax, y, ksize, strides, padding, kernel_name):
        shape_args = _check_shape_and_format(x, grad, argmax, y, ksize, strides, padding)
    ...
    def _only_one_loop(self):
        ...
        block_for_grad_load = nc1_per_core * ho_per_core * self.wo
        block_for_res_ub = nc1_per_core * ho_per_core * self.wo
    ...
    def _cut_nc1(self):
        ...
        `block_for_grad_ub = nc1_per_loop * ((ho - 1) * stride_h + kh) * (wi + pad_left + pad_right) * 2`
        `block_for_grad_load = nc1_per_loop * ho * wo`
        `block_for_res_ub = nc1_per_loop * ho * wo * 2`
        """
        if self.nc1 <= self.core_num:
            return False
        max_nc1_per_loop = (self.block_num - 1) // \
                           (self.kh * self.kw * ((self.ho * self.wo + 15) // 16) * 2 + \
                            ((self.ho - 1) * self.stride_h + self.kh) * \
                            (self.wi + self.pad_left + self.pad_right) * 2 + \
                            self.ho * self.wo * 3)
    ...
    def _cut_ho(self):
        ...
        `block_for_grad_ub = ((ho_per_loop - 1) * stride_h + kh) * ((wo - 1) * stride_w + kw) * 2`
        `block_for_grad_load = ho_per_loop * wo`
        `block_for_res_ub = ho_per_loop * wo * 2`
        """
        max_ho_per_loop = int((self.block_num - 1 - self.kh * self.kw * 2 - \
                               (self.kh - self.stride_h) * ((self.wo - 1) * self.stride_w + self.kw) * 2) // \
                              (self.kh * self.kw * self.wo / 8 + self.wo * 3 + \
                               self.stride_h * ((self.wo - 1) * self.stride_w + self.kw) * 2))
    ...
    def _cut_wo(self):
        ...
        `block_for_grad_ub = kh * ((wo_per_loop - 1) * stride_w + kw) * 2`
        `block_for_grad_load = wo_per_loop`
        `block_for_res_ub = wo_per_loop * 2`
        """
        max_wo_per_loop = int((self.block_num - 1 - 4 * self.kh * self.kw + 2 * self.kh * self.stride_w) // \
                              (self.kh * self.kw / 8 + 2 * self.kh * self.stride_w + 3))
    ...
    def _cut_kh(self):
        ...
        `block_for_grad_ub = kh_per_loop * ((wo_per_loop - 1) * stride_w + kw) * 2`
        `block_for_grad_load = wo_per_loop`
        `block_for_res_ub = wo_per_loop * 2`
        """
        ...
        max_kh_per_loop = (self.block_num - 1 - wo_per_loop * 3) // \
                          (self.kw * ((wo_per_loop + 15) // 16) * 2 + \
                           ((wo_per_loop - 1) * self.stride_w + self.kw) * 2)
    ...
    def _preload(self, grad):
        ...
        padding_top = tvm.compute(preload_shape,
                                  lambda i0, i1, i2, i3:
                                      tvm.select(i1 < self.pad_top, tvm.const(0.0, "float16")),
                                  name="padding_top")

        padding_bottom = tvm.compute(preload_shape,
                                     lambda i0, i1, i2, i3:
                                         tvm.select(i1 >= self.pad_top + self.hi, tvm.const(0.0, "float16")),
                                     name="padding_bottom")

        padding_left = tvm.compute(preload_shape,
                                   lambda i0, i1, i2, i3:
                                       tvm.select(i2 < self.pad_left, tvm.const(0.0, "float16")),
                                   name="padding_left")

        padding_right = tvm.compute(preload_shape,
                                    lambda i0, i1, i2, i3:
                                        tvm.select(i2 >= self.pad_left + self.wi, tvm.const(0.0, "float16")),
                                    name="padding_right")
    ...
    def _compute(self):
        ...
        x = tvm.placeholder(grad_shape, dtype="float16", name="x")
        grad = tvm.placeholder(grad_shape, dtype="float16", name="grad")
        ...
        zero_tensor = tvm.compute((self.c0, ), lambda i: tvm.const(0.0, "float16"), name="zero_tensor")
        ...
        grad_load = tvm.compute(
            ...
            lambda i0, i1, i2, i3, i4, i5:
                grad_ub[i0, i3 * self.stride_h + i1, i4 * self.stride_w + i2, i5] + tvm.const(0.0, "float16"),
            name="grad_load")
