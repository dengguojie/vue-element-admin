...
class Iou3D:
    ...
    def sum_area_of_triangles(self, ori_idx_fp16_ub, x_tensor_ub, y_tensor_ub, slope_tensor_ub,
        ...
        """
        val_fp16_ub = self.tik_instance.Tensor("float16", [Constant.BATCH], name="val_fp16_ub",
        ...
        with self.tik_instance.for_range(corners_num, Constant.BATCH) as idx:
            val_fp16_ub[idx].set_as(min_val)
    ...
    def init_ub_local_variable(self):
        ...
        y4_of_boxes_ub = self.tik_instance.Tensor(self.dtype, [self.k_align], name="y4_of_boxes_ub",
                                                  scope=tik.scope_ubuf)
        x_tensor_ub = self.tik_instance.Tensor(self.dtype, [Constant.BATCH], name="x_tensor_ub",
                                               scope=tik.scope_ubuf)
        y_tensor_ub = self.tik_instance.Tensor(self.dtype, [Constant.BATCH], name="y_tensor_ub",
                                               scope=tik.scope_ubuf)
        slope_tensor_ub = self.tik_instance.Tensor(self.dtype, [Constant.BATCH], name="slope_tensor_ub",
                                                   scope=tik.scope_ubuf)
