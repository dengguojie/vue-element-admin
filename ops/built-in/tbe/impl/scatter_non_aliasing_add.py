...
class ScatterNonAliasingAdd(object):
    ...
    def __init__(self, inputs, indices, updates, outputs, kernel_name):
        ...
        self.inputs_ele_num = functools.reduce(lambda x, y: x * y, self.inputs_shape)
        self.indices_num = functools.reduce(lambda x, y: x * y, self.indices_shape)
        self.kernel_name = kernel_name
        self._check_param(outputs)
        if self.indices_shape[-1] == len(self.inputs_shape):
            self.update_data_num = 1
        else:
            self.update_data_num = functools.reduce(lambda x, y: x * y, self.inputs_shape[self.indices_shape[-1]:])
        self.max_indice = functools.reduce(lambda x, y: x * y, self.inputs_shape[0:self.indices_shape[-1]])
        self.block_number = tbe_platform.VECTOR_INST_BLOCK_NUM
        self.ub_size_bytes = (tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) - self.unused_ub_sizes)
        self.inputs_dtype_bytes_size = tbe_platform.get_bit_len(self.inputs_dtype) // 8
        self.indices_dtype_bytes_size = tbe_platform.get_bit_len(self.indices_dtype) // 8
        self.inputs_data_each_block = BLOCK_LENGTH // self.inputs_dtype_bytes_size
        self.indices_data_each_block = BLOCK_LENGTH // self.indices_dtype_bytes_size
        self.indices_ub_number = 0
        self.updates_ub_number = 0
        self.index_loop_num = 0
        self.updates_read_index = None
        self.inputs_read_index = None
        self.indices_loop_index = None
        self.indices_tmp = None
        self.inputs_ub = None
        self.updates_ub = None
        self.indices_ub = None
        self.inputs_tile_ub = None
        self.updates_tile_ub = None

        self.max_num_one_repeat = tbe_platform.ELEMENTS_VECTOR_OP_FP16
        if self.inputs_dtype in ("float32", "int32"):
            self.max_num_one_repeat = tbe_platform.ELEMENTS_VECTOR_OP_FP16 // 2
    ...
    def _gen_block_num(self):
        if self.update_data_num < self.inputs_data_each_block or len(self.inputs_shape) == 1:
            self.block_num = 1
        else:
    ...
    def _copy_input_to_output(self, indices_loop_index):
        """
        ...
        None
        """
        ub_size_bytes = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) - self.unused_ub_sizes
        input_to_ub_number = ub_size_bytes // self.inputs_dtype_bytes_size
        input_to_ub_number = math.ceil(input_to_ub_number / self.inputs_data_each_block) * self.inputs_data_each_block
        with self.tik_instance.new_stmt_scope():
            inputs_ub = self.tik_instance.Tensor(self.inputs_dtype, (input_to_ub_number, ),
                                                 name="inputs_ub_copy",
                                                 scope=tik.scope_ubuf)

            def _do_copy_input_to_output(inputs_ub, indices_in_index, indice_num):
                indices_burst_len = math.floor(indice_num / self.inputs_data_each_block)
                if indices_burst_len == 0:
                    indices_burst_len = 1
                self.tik_instance.data_move(inputs_ub, self.inputs_gm[indices_in_index], 0, 1, indices_burst_len, 0, 0)
                self.tik_instance.data_move(self.outputs_gm[indices_in_index], inputs_ub, 0, 1, indices_burst_len, 0, 0)
                ...
                align_offset = 0
                if tile_ele_num != 0:
                    align_ele_num = (indice_num // self.inputs_data_each_block * self.inputs_data_each_block)
                    align_offset = (indices_in_index + align_ele_num - (self.inputs_data_each_block - tile_ele_num))
                    self.tik_instance.data_move(inputs_ub, self.inputs_gm[align_offset], 0, 1, 1, 0, 0)
                    self.tik_instance.data_move(self.outputs_gm[align_offset], inputs_ub, 0, 1, 1, 0, 0)

            input_loop_num = self.inputs_ele_num // self.block_num // input_to_ub_number

            if input_loop_num > 0:
                with self.tik_instance.for_range(0, input_loop_num) as input_loop_index:
                    _do_copy_input_to_output(
                        inputs_ub, indices_loop_index * self.inputs_ele_num // self.block_num +
                        input_loop_index * input_to_ub_number, input_to_ub_number)

            input_last_num = self.inputs_ele_num // self.block_num % input_to_ub_number
            if input_last_num > 0:
                _do_copy_input_to_output(
                    inputs_ub,
                    indices_loop_index * self.inputs_ele_num // self.block_num + input_loop_num * input_to_ub_number,
                    input_last_num)
    ...
    def _init_ub_tensor(self):
        ...
        None
        """
        self.inputs_read_index = self.tik_instance.Scalar("int32", name="inputs_read_index")
        ...
        if updates_size_bytes * 2 < self.ub_size_bytes * UB_SIZE_RATIO:
            self.updates_ub_number = math.ceil(
                self.update_data_num / self.inputs_data_each_block) * self.inputs_data_each_block
            ...
            self.indices_ub_number = math.ceil(
                self.indices_ub_number / self.indices_data_each_block) * self.indices_data_each_block
            ...
            self.updates_ub_number = (self.ub_size_bytes - indices_size_bytes) // 2 // self.inputs_dtype_bytes_size

            self.updates_ub_number = math.ceil(
                self.updates_ub_number / self.inputs_data_each_block) * self.inputs_data_each_block
        else:
            self.indices_ub_number = (self.ub_size_bytes // self.indices_dtype_bytes_size // 2 // BLOCK_LENGTH *
                                      BLOCK_LENGTH)
            self.updates_ub_number = self.indices_ub_number // 2
        self.inputs_ub = self.tik_instance.Tensor(self.inputs_dtype, (self.updates_ub_number, ),
                                                  name="inputs_ub",
                                                  scope=tik.scope_ubuf)
        self.updates_ub = self.tik_instance.Tensor(self.updates_dtype, (self.updates_ub_number, ),
                                                   name="updates_ub",
                                                   scope=tik.scope_ubuf)
        self.indices_ub = self.tik_instance.Tensor(self.indices_dtype, (self.indices_ub_number, ),
                                                   name="indices_ub",
                                                   scope=tik.scope_ubuf)

        self.inputs_tile_ub = self.tik_instance.Tensor(self.inputs_dtype, (self.inputs_data_each_block, ),
                                                       name="inputs_tile_ub",
                                                       scope=tik.scope_ubuf)
        self.updates_tile_ub = self.tik_instance.Tensor(self.updates_dtype, (self.inputs_data_each_block, ),
                                                        name="updates_tile_ub",
                                                        scope=tik.scope_ubuf)

    def _get_inputs_read_index(self, indices_ub_index):
        """
        ...
        for i in range(0, self.indices_shape[-1] - 1):
            ...
            self.inputs_read_index.set_as(self.inputs_read_index + self.indices_tmp *
                                          functools.reduce(lambda x, y: x * y, self.inputs_shape[i + 1:]))
        self.indices_tmp.set_as(self.indices_ub[indices_ub_index + self.indices_shape[-1] - 1])
        if self.indices_shape[-1] == len(self.inputs_shape):
            self.inputs_read_index.set_as(self.inputs_read_index + self.indices_tmp)
        else:
            self.inputs_read_index.set_as(
                self.inputs_read_index +
                self.indices_tmp * functools.reduce(lambda x, y: x * y, self.inputs_shape[self.indices_shape[-1]:]))

    def _get_updates_read_index(self, indices_ub_index):
        """
    ...
    def _updates_the_inputs(self, indices_in_index, indice_num):
        """
        ...
        with self.tik_instance.for_range(0, indice_loop_num) as indices_ub_index:
            self._get_inputs_read_index(indices_ub_index)
            if self.block_num > 1:
                ...
                self._get_updates_read_index(indices_ub_index)
                self.inputs_read_index.set_as(self.inputs_read_index)
                self._calc_updates()

    def _calc_updates(self):
        """
        ...
        if updates_loop > 0:
            with self.tik_instance.for_range(0, updates_loop) as loop_index:
                self._calc_updates_small(loop_index * self.updates_ub_number, self.updates_ub_number)

        last_num = self.update_data_num % self.updates_ub_number
        if last_num > 0:
            self._calc_updates_small(updates_loop * self.updates_ub_number, last_num)

    def _calc_updates_small(self, read_index_offset, element_num):
        """
        ...
        if updates_burst_len == 0:
            updates_burst_len = 1
        self.tik_instance.data_move(self.inputs_ub, self.outputs_gm[self.inputs_read_index + read_index_offset], 0, 1,
                                    updates_burst_len, 0, 0)

        self.tik_instance.data_move(self.updates_ub, self.updates_gm[self.updates_read_index + read_index_offset], 0, 1,
                                    updates_burst_len, 0, 0)
        ...
        align_offset = 0
        if tile_ele_num != 0:
            align_ele_num = (element_num // self.inputs_data_each_block * self.inputs_data_each_block)
            align_offset = (read_index_offset + align_ele_num - (self.inputs_data_each_block - tile_ele_num))
            self.tik_instance.data_move(self.inputs_tile_ub, self.outputs_gm[self.inputs_read_index + align_offset], 0,
                                        1, 1, 0, 0)

            self.tik_instance.data_move(self.updates_tile_ub, self.updates_gm[self.updates_read_index + align_offset],
                                        0, 1, 1, 0, 0)
        max_repeat_times = tbe_platform.VECTOR_INST_MAX_REPEAT_TIMES
        ...
        if compute_loop > 0:
            with self.tik_instance.for_range(0, compute_loop) as index:
                ...
                self._calc_process(self.max_num_one_repeat, index_offset, index_offset, index_offset, max_repeat_times,
                                   False)
        last_loop = element_num % (self.max_num_one_repeat * max_repeat_times) // self.max_num_one_repeat

        if last_loop > 0:
            index_offset = compute_loop * self.max_num_one_repeat * max_repeat_times
            self._calc_process(self.max_num_one_repeat, index_offset, index_offset, index_offset, last_loop, False)

        compute_mask = element_num % self.max_num_one_repeat
        if compute_mask > 0:
            index_offset = (element_num // self.max_num_one_repeat * self.max_num_one_repeat)
            if (tile_ele_num == 0 or self.update_data_num < self.inputs_data_each_block):
                self._calc_process(compute_mask, index_offset, index_offset, index_offset, 1, False)

                self.tik_instance.data_move(self.outputs_gm[self.inputs_read_index + read_index_offset], self.inputs_ub,
                                            0, 1, updates_burst_len, 0, 0)
            else:
                self._calc_process(compute_mask, index_offset, index_offset, index_offset, 1, False)
                self._calc_process(self.inputs_data_each_block, 0, 0, 0, 1, True)
                self.tik_instance.data_move(self.outputs_gm[self.inputs_read_index + read_index_offset], self.inputs_ub,
                                            0, 1, updates_burst_len, 0, 0)
                self.tik_instance.data_move(self.outputs_gm[self.inputs_read_index + align_offset], self.inputs_tile_ub,
                                            0, 1, 1, 0, 0)
        else:
...
# 'pylint: disable=too-many-arguments
    def _calc_process(self, mask, dest_addr, src_addr1, src_addr2, repeat_times, is_tile):
        ...
        if is_tile:
            compute_repeat_strid = (self.max_num_one_repeat // self.inputs_data_each_block)
    ...
    def _traversing_indices(self):
        ...
        if indices_loop_num > 0:
            with self.tik_instance.for_range(0, indices_loop_num) as indices_loop_index:
                self._updates_the_inputs(indices_loop_index * self.indices_ub_number, self.indices_ub_number)

        indices_last_num = self.indices_num % self.indices_ub_number
        if indices_last_num > 0:
            self._updates_the_inputs(indices_loop_num * self.indices_ub_number, indices_last_num)

    def _check_param(self, outputs):
        """
        Check parameter
        ...
        para_check.check_shape(self.inputs_shape, param_name="inputs")
        para_check.check_shape(self.indices_shape, param_name="indices")
        para_check.check_shape(self.updates_shape, param_name="updates")
        para_check.check_shape(outputs_shape, param_name="outputs")

        check_list_inputs = ("float16", "float32", "int32")
        check_list_indices = ("int32", "int64")
        para_check.check_dtype(self.inputs_dtype, check_list_inputs, param_name="inputs")
        para_check.check_dtype(self.indices_dtype, check_list_indices, param_name="indices")
        para_check.check_dtype(self.updates_dtype, check_list_inputs, param_name="updates")
        para_check.check_dtype(outputs_dtype, check_list_inputs, param_name="outputs")
    ...
    def scatter_operator(self):
        ...
        if self.block_num > 1:
            with self.tik_instance.for_range(0, self.block_num, block_num=self.block_num) as indices_loop_index:
                self._copy_input_to_output(indices_loop_index)
                self._init_ub_tensor()
                self.indices_loop_index.set_as(indices_loop_index)

                self._traversing_indices()
        else:
            self._copy_input_to_output(0)
            self._init_ub_tensor()
            self._traversing_indices()

        self.tik_instance.BuildCCE(kernel_name=self.kernel_name,
                                   inputs=(self.inputs_gm, self.indices_gm, self.updates_gm),
...
def scatter_non_aliasing_add(inputs, indices, updates, outpus, kernel_name="ScatterNonAliasingAdd"):
    ...
    tik instance
    """
    instance = ScatterNonAliasingAdd(inputs, indices, updates, outpus, kernel_name)

    tik_instance = instance.scatter_operator()
