from .ascend import TensorOperatorParam
from .ascend import VecCmd
from .ascend import VecExecutor
...
class MergeSort(object):
    ...
    def get_top_proposal(self, result_proposal, batch_index, boxes_num, sort_num, get_proposal_ub, *args):
        ...
        ----------
        result_proposal:
            A gm Tensor. shape: (n, proposal, 8)
        batch_index :
            int/Scalar(int)
        boxes_num:
            int
        sort_num:
            int
        get_proposal_ub:
            fun to get ub_proposal_1, ub_proposal_2
        *args:
            param of get_proposal_ub
        Returns
        -------
        """
    ...
    def _merge_sort_element(self, ub_proposal_1, ub_proposal_2, proposal_num, element_count=1):
        ...
        if valid_not_merge > 1:
            element_count_list = [element_count for _ in range(valid_not_merge)]
    ...
    def _merge_proposal_element_list(self, ub_proposal_1, ub_proposal_2, element_list):
        ...
        src_list = [ub_proposal_1[index_temp, 0] for index_temp in
                    index_start_all]
    ...
    def _merge_sort_channel_all(self, ub_proposal_1, ub_proposal_2,
        ...
        while repeat_times > 0 and element_count < self._element_count_max_num:
            # sort align proposal
            src_list = [ub_proposal_1[index_i * element_count, 0] for index_i in range(self._merge_channel_num)]
            element_count_list = [element_count for _ in range(self._merge_channel_num)]
            self._tik_inst.vmrgsort4(ub_proposal_2, src_list, element_count_list,
            ...
            if valid_not_merge == 1:
                ...
                src_list = [ub_proposal_1[0] for _ in range(self._merge_channel_num)]
    ...
    def get_top_proposal_large(self, dst_gm, src_gm, batch_index, boxes_num,
        ...
        ----------
        dst_gm:
            A gm Tensor. shape: (n, proposal + tail_num, 8) tail_num >= 2.
        src_gm:
            A gm Tensor. shape: (n, proposal + tail_num, 8) tail_num >= 2
        batch_index :
            int/Scalar(int)
        ...
        *args:
        ...
        src_gm_sorted_num = [each_loop_sort_num for _ in range(loop_times)]
    ...
    def _merge_sort_large_num(self, dst_gm, src_gm, batch_index, sort_num_align, loop_info_all):
        for loop_info in loop_info_all:
            c_num, c_num_next, sorted_num, next_sorted_num, src_gm_rem_list, last_src_gm_rem_list = loop_info
            dst_gm, src_gm = src_gm, dst_gm
    ...
    def _large_src_to_dst(self, dst_gm, src_gm, batch_index, result_index,
                          start_index, pro_num):
        """
        if channel_num = 1, src_gm -> dst_gm
        """
    ...
    def _merge_sort_gm_channels(self, dst_gm, src_gm, batch_index,
        ...
        (valid_bit_, list_index_, block_num_, selected_num_sum_, selected_num_,
         rem_num_all_, result_index_) = \
            self._large_init_scalar(src_gm_rem_list, sort_num_align)

        (src_gm_idx_list_, src_gm_rem_list_, src_ub_list_, src_ub_num_list_,
         slot_map_list_, dst_ub_num_list_) = \
            self._large_init_scalar_list(
                src_ub, ub_data_num, src_gm_rem_list, start_index_, sorted_num)
            ...
            with self._tik_inst.if_scope(selected_num_sum_ < sort_num_align):
                valid_bit_, slot_map_list_, src_ub_list_ = \
                    self._large_gm_to_ub(
                        src_gm, batch_index,
                        (list_num, ub_data_num, valid_bit_,
                         list_index_, block_num_),
                        (src_gm_idx_list_, src_gm_rem_list_, src_ub_list_,
                         src_ub_num_list_, slot_map_list_))
                with self._tik_inst.if_scope(valid_bit_ > 0):
                    with self._tik_inst.if_scope(valid_bit_ > 1):
                        ...
                        self._large_update_scalar(
                            ...
                            (src_gm_idx_list_, src_gm_rem_list_,
                             dst_ub_num_list_, slot_map_list_))
                        self._large_ub_to_gm(
                            dst_gm, dst_ub, batch_index,
                            (result_index_start_, result_index_,
                             selected_num_, selected_num_sum_,
                             block_num_, rem_num_all_))
                    with self._tik_inst.else_scope():
                        self._large_update_scalar(
                            (list_num, selected_num_, valid_bit_),
                            (src_gm_idx_list_, src_gm_rem_list_,
