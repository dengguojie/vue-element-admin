...
class UpSampleBicubic2dBackward:
    ...
    def __init__(self, grads, original_size, scales, coordinate_transformation_mode,
        ...
        ub_size_bytes = tp.get_soc_spec("UB_SIZE")

        dtype_bytes_size = tp.cce_intrin.get_bit_len(
            self.dtype_grads) // 8
...
class ResizeLinearBackward:
    ...
    def __init__(self,
        ...
        block_bite_size = 32
        dtype_bytes_size = tp.cce_intrin.get_bit_len(self.x_dtype) // 8
        self.data_each_block = block_bite_size // dtype_bytes_size
    ...
    def init_output_gm_as_zero(self):
        ...
        loop_time = np.ceil(self.output_num / self.data_each_block).astype(np.int)
