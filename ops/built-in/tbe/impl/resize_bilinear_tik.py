...
class ResizeBilinear:
    ...
    def check_supported_tik(self, method="bilinear"):
        ...
        if input_type in (
                "float32",
                "float16",
        ) and method in ("bilinear", ) and len(input_shape) == 4:
...
def fill_index_in_ub(tik_instance, idx_ub, idx_num):
    ...
    # when the type is not int32, will fill in int32 ub and cast to idx_ub dtype
        ...
        with tik_instance.new_stmt_scope():
            idx_ub_int32 = tik_instance.Tensor("int32", (idx_num, ), name="idx_ub_int32", scope=tik.scope_ubuf)
...
def do_resize_bilinear_compute_one_core(box_num_sigment, obj, box_num_offset):
    ...
    tik_instance.vector_dup(obj.boxes_vector_num, height_offset_ub, obj.image_c0 * obj.image_width, 1, 1, 8)
    tik_instance.vector_dup(obj.boxes_vector_num, width_offset_ub, obj.image_c0, 1, 1, 8)
    ...
    if obj.crop_height > 1:
        ...
        tik_instance.vmuls([obj.height_mask_list[0], obj.height_mask_list[1]], boxes_ub_scale, boxes_ub_scale,
                           (obj.image_height - 1) / (obj.crop_height - 1), repeat_time, 1, 1, 8, 8)
    if obj.crop_width > 1:
        # to get scale_w:  scale * (image_width - 1) / (crop_width - 1)
        tik_instance.vmuls(obj.width_mask_list, boxes_ub_scale, boxes_ub_scale,
                           (obj.image_width - 1) / (obj.crop_width - 1), repeat_time, 1, 1, 8, 8)
    # to get h_small: h_small * (image_height - 1)
    if obj.crop_height > 1:
        # to get h_small: h_small * (image_height - 1)
        tik_instance.vmuls(obj.height_mask_list, boxes_ub_small, boxes_ub_small, obj.image_height - 1, repeat_time, 1,
                           1, 8, 8)
    else:
        # to get h_small: (h_small + h_big) * (image_height - 1) * 0.5
        tik_instance.vmuls(obj.height_mask_list, boxes_ub_small, boxes_ub_big, 0.5, repeat_time, 1, 1, 8, 8)
        tik_instance.vmuls(obj.height_mask_list, boxes_ub_small, boxes_ub_small, obj.image_height - 1, repeat_time, 1,
                           1, 8, 8)

    if obj.crop_width > 1:
        # to get w_small: w_small * (image_width - 1)
        tik_instance.vmuls(obj.width_mask_list, boxes_ub_small, boxes_ub_small, obj.image_width - 1, repeat_time, 1, 1,
                           8, 8)
    else:
        # to get w_small: (w_small + w_big) * (image_width - 1) * 0.5
        tik_instance.vmuls(obj.width_mask_list, boxes_ub_small, boxes_ub_big, 0.5, repeat_time, 1, 1, 8, 8)
        tik_instance.vmuls(obj.width_mask_list, boxes_ub_small, boxes_ub_small, obj.image_width - 1, repeat_time, 1, 1,
                           8, 8)
    ...
    util_tik_comm_func.tik_func_vcomple(tik_instance,
                                        "vmul",
                                        box_index_ub,
                                        box_index_ub,
                                        batch_offset_ub,
    ...
    with tik_instance.for_range(0, box_num_sigment) as _box_idx:
        ...
        tik_instance.vmuls(obj.crop_height, input_boxes_in_h, index_ub, scaler_h_scale, 1, 1, 1, 8, 8)
        ...
        tik_instance.vmuls(obj.crop_width, input_boxes_in_w, index_ub, scaler_w_scale, 1, 1, 1, 8, 8)
        tik_instance.vadds(obj.crop_height, input_boxes_in_h, input_boxes_in_h, scaler_h_small, 1, 1, 1, 8, 8)
        tik_instance.vadds(obj.crop_width, input_boxes_in_w, input_boxes_in_w, scaler_w_small, 1, 1, 1, 8, 8)
        ...
        with tik_instance.new_stmt_scope():
            tmp_float_ub_0 = obj.apply_mem((get_ceil_int(obj.crop_height, obj.boxes_block_num) * obj.boxes_block_num, ),
                                           "tmp_float_ub_0", tik.scope_ubuf, obj.boxes_type)
            if not cast_flag:
                util_tik_comm_func.tik_func_vconv(tik_instance,
                                                  h_top_index,
                                                  input_boxes_in_h,
                                                  obj.crop_height,
                                                  mode="floor",
                                                  mini_mid_ub=tmp_float_ub_0)
            else:
                util_tik_comm_func.tik_func_vconv(tik_instance,
                                                  h_top_index,
                                                  input_boxes_in_h,
            ...
            util_tik_comm_func.tik_func_vcomple(tik_instance,
                                                ...
                                                obj.crop_height,
                                                src1_rep=0)
            util_tik_comm_func.tik_func_vcomple(tik_instance, "vsub", input_boxes_in_h, input_boxes_in_h,
                                                tmp_float_ub_0, obj.crop_height)
            util_tik_comm_func.tik_func_vconv(tik_instance,
                                              h_index_post,
                                              input_boxes_in_h,
                                              obj.crop_height,
                                              mode="ceil")
            tmp_float_ub_1 = obj.apply_mem((get_ceil_int(obj.crop_width, obj.boxes_block_num) * obj.boxes_block_num, ),
            ...
            util_tik_comm_func.tik_func_vcomple(tik_instance,
                                                ...
                                                obj.crop_width,
                                                src1_rep=0)
            util_tik_comm_func.tik_func_vcomple(tik_instance, "vsub", input_boxes_in_w, input_boxes_in_w,
                                                tmp_float_ub_1, obj.crop_width)
            util_tik_comm_func.tik_func_vconv(tik_instance, w_index_post, input_boxes_in_w, obj.crop_width, mode="ceil")
        ...
        with tik_instance.for_range(0, obj.crop_height) as _crop_height_idx:
            ...
            with tik_instance.for_range(0, obj.crop_width, thread_num=thread_num) as _crop_width_idx:
                ...
                with tik_instance.new_stmt_scope():
                    ...
                    h0_w_ub = obj.apply_mem((obj.image_c1 * obj.image_c0 * 2, ), "h0_w_ub", tik.scope_ubuf, "float32")
                    h1_w_ub = obj.apply_mem((obj.image_c1 * obj.image_c0 * 2, ), "h1_w_ub", tik.scope_ubuf, "float32")
                    if obj.image_block_num != obj.block_num:
                        h0_w_ub_fp16 = obj.apply_mem((obj.image_c1 * obj.image_c0 * 2, ), "h0_w_ub_fp16",
                                                     tik.scope_ubuf)
                        h1_w_ub_fp16 = obj.apply_mem((obj.image_c1 * obj.image_c0 * 2, ), "h1_w_ub_fp16",
                                                     tik.scope_ubuf)
                        ...
                        if obj.image_block_num == obj.block_num:
                            ...
                            if obj.image_width > 1:
                                tik_instance.data_move(
                                    h0_w_ub, image_gm[input_batch_offset + input_h_offset + input_w_offset], 0,
                                    obj.image_c1, c0_block_num * 2,
                                    obj.image_height * obj.image_width * c0_block_num - c0_block_num * 2, 0)
                                if obj.image_height > 1:
                                    tik_instance.data_move(
                                        h1_w_ub, image_gm[input_batch_offset + input_h_offset + input_w_offset +
                                                          obj.image_width * obj.image_c0], 0, obj.image_c1,
                                        c0_block_num * 2,
                                        obj.image_height * obj.image_width * c0_block_num - c0_block_num * 2, 0)
                                else:
                                ...
                                tik_instance.data_move(h0_w_ub,
                                                       ...
                                                       obj.image_height * obj.image_width * c0_block_num - c0_block_num,
                                                       c0_block_num)
                                tik_instance.data_move(h0_w_ub[obj.image_c0],
                                                       image_gm[input_batch_offset + input_h_offset + input_w_offset],
                                                       0, obj.image_c1, c0_block_num,
                                                       obj.image_height * obj.image_width * c0_block_num - c0_block_num,
                                                       c0_block_num)
                                if obj.image_height > 1:
                                    tik_instance.data_move(
                                        h1_w_ub, image_gm[input_batch_offset + input_h_offset + input_w_offset +
                                                          obj.image_width * obj.image_c0], 0, obj.image_c1,
                                        c0_block_num, obj.image_height * obj.image_width * c0_block_num - c0_block_num,
                                        c0_block_num)
                                    tik_instance.data_move(
                                        h1_w_ub[obj.image_c0],
                                        image_gm[input_batch_offset + input_h_offset + input_w_offset +
                                                 obj.image_width * obj.image_c0], 0, obj.image_c1, c0_block_num,
                            ...
                            with tik_instance.new_stmt_scope():
                                ...
                                if obj.image_width > 1:
                                    tik_instance.data_move(
                                        h0_w_ub_fp16, image_gm[input_batch_offset + input_h_offset + input_w_offset], 0,
                                        obj.image_c1, c0_block_fp16 * 2,
                                    ...
                                    if obj.image_height > 1:
                                        tik_instance.data_move(
                                            h1_w_ub_fp16, image_gm[input_batch_offset + input_h_offset +
                                                                   input_w_offset + obj.image_width * obj.image_c0], 0,
                                            obj.image_c1, c0_block_fp16 * 2,
                                            obj.image_height * obj.image_width * c0_block_fp16 - c0_block_fp16 * 2, 0)
                                    ...
                                    if obj.image_height > 1:
                                        ...
                                        tik_instance.data_move(
                                            ...
                                            image_gm[input_batch_offset + input_h_offset + input_w_offset +
                                                     obj.image_width * obj.image_c0], 0, obj.image_c1, c0_block_fp16,
                                            obj.image_height * obj.image_width * c0_block_fp16 - c0_block_fp16,
                                            c0_block_fp16)
                                        util_tik_comm_func.tik_func_vconv(tik_instance, h1_w_ub, h1_w_ub_fp16,
                                                                          obj.image_c1 * obj.image_c0 * 2)
                        ...
                        util_tik_comm_func.tik_func_vcomple(tik_instance, "vsub", h1_w_ub, h1_w_ub, h0_w_ub,
                                                            obj.image_c1 * obj.image_c0 * 2)

                        util_tik_comm_func.tik_func_vmuls(tik_instance, h1_w_ub, h1_w_ub, h_lerp,
                                                          obj.image_c1 * obj.image_c0 * 2)

                        util_tik_comm_func.tik_func_vcomple(tik_instance, "vadd", h0_w_ub, h1_w_ub, h0_w_ub,
                                                            obj.image_c1 * obj.image_c0 * 2)

                        tik_fun = tik_instance.vsub
                        tik_fun(obj.image_c0, h1_w_ub, h0_w_ub[16], h0_w_ub, obj.image_c1, 1, 1, 1, 2, 4, 4)
                        util_tik_comm_func.tik_func_vmuls(tik_instance, h1_w_ub, h1_w_ub, w_lerp,
                                                          obj.image_c1 * obj.image_c0)

                        tik_fun = tik_instance.vadd
                        tik_fun(obj.image_c0, h1_w_ub[obj.image_c1 * obj.image_c0:], h1_w_ub, h0_w_ub, obj.image_c1, 1,
                                1, 1, 2, 2, 4)
                        output_offset = \
                        ...
                        tik_instance.data_move(output_gm[output_offset], h1_w_ub[obj.image_c1 * obj.image_c0:], 0,
                                               obj.image_c1, c0_block_num, 0,
...
def resize_bilinear(x, y, crop_size, kernel_name="resize_bilinear_v2"):
    ...
    with tik_instance.for_range(0, core_used, block_num=core_used) as _core_idx:
        ...
        resize_bilinear_obj.index_ub = resize_bilinear_obj.apply_mem([max_idx], "tmp_index_ub", tik.scope_ubuf,
                                                                     resize_bilinear_obj.boxes_type)
