...
def check_supported(input_data, output_data, ksize, strides, padding,
    ...
    if window_h * window_w > SCALAR_255 and (window_h > SCALAR_20 or window_w > SCALAR_20) and is_global is False:
...
def get_fusion_params(input_data, output_data, is_fused_compute=True):
    ...
    l1_addr_flag = \
        input_data.op.attrs["L1_addr_flag"].value if "L1_addr_flag" in input_data.op.attrs else -1
    l1_addr_offset = \
        input_data.op.attrs["L1_addr_offset"] if "L1_addr_offset" in input_data.op.attrs else -1
    l1_valid_size = \
        input_data.op.attrs["L1_valid_size"] if "L1_valid_size" in input_data.op.attrs else -1
