...
def check_supported(x, weight, bias, y, shifts=(), kernel_name="swin_attention_ffn"):
    ...
    seq_num = x_shape[0]//batch_num
    seq_len = x_shape[1]
    head_len = x_shape[2]
    if weight_shape != (head_len, head_len):
        return False, "weight shape not support"
    if bias_shape != (head_len, ):
        return False, "bias shape not support"
    if output_shape != (batch_num, seq_num * seq_len, head_len):
        return False, "output shape not support"
    support_shape_all = ((9216, 128), (2304, 256), (576, 512))
    if (seq_num * seq_len, head_len) not in support_shape_all:
        return False, "input shape not support"
