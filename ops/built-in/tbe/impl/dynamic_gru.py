...
def _check_dtype(input_x, weight, bias, cw, cb, init_h, y, output_h, rg, ig, ng):
    ...
    def _check_equal_bias_dtype(tensor, name):
        if tensor["dtype"] != bias_dtype:
            error_manager_vector.raise_err_inputs_dtype_not_equal("DynamicGRU", "b",
                                                                  name, bias_dtype, tensor["dtype"])
...
def dynamic_gru(input_x, weight, bias, cw, cb,
    ...
    with tik_instance.for_range(0, loop_t) as i:
        with tik_instance.for_range(0, loop_m) as j:
...
def _dynamic_gru_inner(input_list, custom_list):
    ...
    b_l1_1 = tvm.compute(shape_b_1,
                         ...
                         name="b_l1_1",
                         tag="out_to_l1")
    a_l0a_1 = tvm.compute(shape_a_z_bigz, lambda *indices: a_l1_1(*indices), name="a_l0a_1", tag="l1_to_l0")
    ...
    r_t_h_fp16 = tvm.compute(shape_i,
    ...
    b_l1_2 = tvm.compute(shape_b_2,
                         ...
                         name="b_l1_2",
                         tag="out_to_l1")
    a_l0a_2 = tvm.compute(shape_a_z_bigz, lambda *indices: a_l1_2(*indices), name="a_l0a_2", tag="l1_to_l0")
    ...
    # for RL tuning
    output_list, sch_rl = rl_bank.tik_dsl_bank_proc(output_list)
    ...
    sch[r_t_h_fp16].set_scope(tbe_platform.scope_ubuf)
    ...
    if is_gate_output:
        ...
        if fp16_input_output:
            ...
            sch[h_t_tanh_fp16].set_scope(tbe_platform.scope_ubuf)
    ...
    l1_k_outer_1, l1_k_inner_1 = sch[c_l0c_1].split(c_l0c_1.op.reduce_axis[0], factor=factor_l1_k)
    ...
    l0_k_outer_1, l0_k_inner_1 = sch[c_l0c_1].split(l1_k_inner_1, factor=factor_l0_k)
    ...
    c_ub_bias_1_outer, c_ub_bias_1_inner = sch[c_ub_bias_1].split(c_ub_bias_1.op.axis[2], factor=factor_l1_n)
    sch[c_ub_bias_1].reorder(c_ub_bias_1.op.axis[0],
    ...
    sch[c_l0c_1].compute_at(sch[c_ub_bias_1], c_ub_bias_1_outer)
    sch[c_ub_1].compute_at(sch[c_ub_bias_1], c_ub_bias_1_outer)
    sch[bias_ub_1].compute_at(sch[c_ub_bias_1], c_ub_bias_1_outer)
    sch[bias_bc_ub_1].compute_at(sch[c_ub_bias_1], c_ub_bias_1_outer)
    if fp16_input_output:
        sch[bias_ub_1_fp32].compute_at(sch[c_ub_bias_1], c_ub_bias_1_outer)
    sch[c_ub_bias_1].emit_insn(c_ub_bias_1.op.axis[1], "vector_add")

    r_t_h_fp16_outer, r_t_h_fp16_inner = sch[r_t_h_fp16].split(r_t_h_fp16.op.axis[1], factor=factor_l1_n)
    sch[r_t_h_fp16].reorder(r_t_h_fp16.op.axis[0],
                            r_t_h_fp16.op.axis[2],
                            r_t_h_fp16_outer,
                            r_t_h_fp16_inner,
                            r_t_h_fp16.op.axis[3],
                            r_t_h_fp16.op.axis[4])
    for tensor in elewise_tensors_r_t_h_fp16:
        sch[tensor].set_scope(tbe_platform.scope_ubuf)
        if tensor == c_ub_bias_1:
            continue
        sch[tensor].compute_at(sch[r_t_h_fp16], r_t_h_fp16_outer)
        insn = _get_emit_insn_map(tensor)
        sch[tensor].emit_insn(tensor.op.axis[0], insn)
    if is_gate_output:
        sch[r_t_gm].compute_at(sch[r_t_h_fp16], r_t_h_fp16_outer)
        sch[r_t_gm_back].compute_at(sch[r_t_h_fp16], r_t_h_fp16_outer)
        if fp16_input_output:
            sch[r_t_sigmoid_fp16].compute_at(sch[r_t_h_fp16], r_t_h_fp16_outer)
            sch[r_t_gm_back_fp32].compute_at(sch[r_t_h_fp16], r_t_h_fp16_outer)
    sch[r_t_h_fp16].emit_insn(r_t_h_fp16_inner, "vector_conv")
    ...
    l1_k_outer_2, l1_k_inner_2 = sch[c_l0c_2].split(c_l0c_2.op.reduce_axis[0], factor=factor_l1_k)
    ...
    l0_k_outer_2, l0_k_inner_2 = sch[c_l0c_2].split(l1_k_inner_2, factor=factor_l0_k)
    ...
    sch[update_h_gm].reorder(update_h_gm.op.axis[0],
                             update_h_gm.op.axis[2],
                             update_h_gm_outer,
                             update_h_gm_inner,
    ...
    sch[c_ub_bias_2].compute_at(sch[update_h_gm], update_h_gm_outer)
    ...
    if is_gate_output:
        ...
        if fp16_input_output:
            ...
            sch[h_t_tanh_fp16].compute_at(sch[update_h_gm], update_h_gm_outer)
    ...
    sch[c_ub_bias_1].compute_at(sch[update_h_gm], update_h_gm.op.axis[2])
    sch[r_t_h_fp16].compute_at(sch[update_h_gm], update_h_gm.op.axis[2])
    sch[s_state_h].compute_at(sch[update_h_gm], update_h_gm.op.axis[2])
    sch[s_state_h_fp16].compute_at(sch[update_h_gm], update_h_gm.op.axis[2])

    sch[update_h_gm].bind(update_h_gm.op.axis[2], tvm.thread_axis("blockIdx.x"))
    ...
    if is_gate_output:
        ...
        if fp16_input_output:
            ...
            sch[h_t_tanh_fp16].emit_insn(h_t_tanh_fp16.op.axis[0], "vector_conv")
