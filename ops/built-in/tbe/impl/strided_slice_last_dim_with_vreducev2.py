...
class StridedSliceLastDimWithVreducev2:
    ...
    def __init__(self, input_shape, dtype, begin, end, stride, kernel_name):
        ...
        if self.vreducev2_flag == 1:
            ...
            self.rows_each_repeat_tensor_each_core = pattern_tensor_dict_each_core["rows_each_repeat_tensor"]
            self.repeat_rows_mask_num_each_core = pattern_tensor_dict_each_core["repeat_rows_mask_num"]
            self.thread_num_each_core = pattern_tensor_dict_each_core["thread_num"]
            self.rows_each_repeat_each_core = pattern_tensor_dict_each_core["rows_each_repeat"]
            self.repeat_times_each_core = pattern_tensor_dict_each_core["repeat_times"]
            self.roll_back_rows_each_core = pattern_tensor_dict_each_core["roll_back_rows"]
            ...
            if self.tail_rows != 0:
                ...
                self.rows_each_repeat_tensor_tail_row = pattern_tensor_dict_tail_rows["rows_each_repeat_tensor"]
                self.repeat_rows_mask_num_tail_row = pattern_tensor_dict_tail_rows["repeat_rows_mask_num"]
                self.thread_num_tail_row = pattern_tensor_dict_tail_rows["thread_num"]
                self.rows_each_repeat_tail_row = pattern_tensor_dict_tail_rows["rows_each_repeat"]
                self.repeat_times_tail_row = pattern_tensor_dict_tail_rows["repeat_times"]
                self.roll_back_rows_tail_row = pattern_tensor_dict_tail_rows["roll_back_rows"]
                self.rows_repeat_tail_gm = self.tik_instance.Tensor(self.mask_dtype,
    ...
    def get_pattern_tensor(self, to_do_rows):
        ...
        pattern_tensor_dict = {
            ...
            "thread_num": repeat_params_dict["thread_num"],
            "rows_each_repeat": repeat_params_dict["rows_each_repeat"],
            "repeat_times": repeat_params_dict["repeat_times"],
            "roll_back_rows": repeat_params_dict["roll_back_rows"]
        }
    ...
    def vreducev2_process(self, blk_idx, repeat_rows_mask_num, thread_num, rows_each_repeat, repeat_times,
        ...
        with self.tik_instance.if_scope(gm_flag == 0):
            gm2ub(self.tik_instance, pattern_ub_repeat, self.rows_repeat_core_gm,
                  len(self.rows_each_repeat_tensor_each_core))
        with self.tik_instance.else_scope():
            gm2ub(self.tik_instance, pattern_ub_repeat, self.rows_repeat_tail_gm,
                  len(self.rows_each_repeat_tensor_tail_row))
            ...
            with self.tik_instance.for_range(0, repeat_times) as repeat_idx:
                ...
                with self.tik_instance.for_range(0, one_line_loop_times) as loop_time_idx:
                    ...
                    with self.tik_instance.if_scope(loop_time_idx == one_line_loop_times - 1):
                        gm2ub(self.tik_instance, input_ub,
                              self.input_gm[repeat_input_addr + loop_time_idx * num_each_loop - curr_roll_back_num],
                              loop_tail_count_32b_align)
                        ub2gm(self.tik_instance,
                              self.output_gm[repeat_output_addr + loop_time_idx * num_each_loop - curr_roll_back_num],
