...
@tbe_platform.fusion_manager.fusion_manager.register("ascend_anti_quant")
def ascend_anti_quant_compute(x, y, scale, offset, sqrt_mode=False, kernel_name="ascend_anti_quant"):
    ...
    out_shape = (in_shape[0], in_shape[1] * 2, in_shape[2], in_shape[3], in_shape[4] // 2)
    ...
    offset_ub = tvm.compute(in_shape, lambda *indice: cast_f16_ub(*indice) + offset_value, name="offset_ub")

    scale_value = tvm.const(scale, "float16")
    if sqrt_mode:
        scale_sqrt_ub = tvm.compute(in_shape, lambda *indice: offset_ub(*indice) * scale_value, name="scale_sqrt_ub")
