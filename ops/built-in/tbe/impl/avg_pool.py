...
def _check_window_rule(ksize, strides, data_format):
    ...
    if len(ksize) != 4:
        err_man_cube.raise_err_four_paras("E80012", "avg_pool", "ksize", "4", "4", len(ksize))

    if len(strides) != 4:
        err_man_cube.raise_err_four_paras("E80012", "avg_pool", "strides", "4", "4", len(strides))

    if ksize[0] != 1 or (ksize_c != 1):
        err_man_cube.raise_err_three_paras("E80000", "avg_pool", ",".join(("ksize[1]", "ksize[3]")),
                                           "1", ",".join((str(ksize[1]), str(ksize[3]))))


    if strides[0] != 1 or strides_c != 1:
        err_man_cube.raise_err_three_paras("E80000", "avg_pool", ",".join(("strides[1]", "strides[3]")),
                                           "1", ",".join((str(strides[1]), str(strides[3]))))

    if data_format not in("NCHW", "NHWC", "NC1HWC0"):
        err_man_cube.raise_err_three_paras("E80015", "avg_pool", "x", ",".join(("NC1HWC0", "NCHW", "NHWC")),
                                           data_format)
