...
class DecodeBbox:
    ...
    def __init__(self, tik_instance, input_data):
        ...
        self.output_region_proposal_ub = \
            tik_instance.Tensor(input_dtype, (input_n*8*16, 8),
        ...
        tik_instance.vector_dup(self.mask, self.output_region_proposal_ub, 0,
                                input_n*8*self.ratio, 1, 8)

        self.input_score_ub = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                                  name="input_score_ub",
                                                  scope=tik.scope_ubuf)
        self.input_bbox_ub = tik_instance.Tensor(input_dtype, (4, input_n*8*16),
                                                 name="input_bbox_ub",
                                                 scope=tik.scope_ubuf)
        self.input_region_proposal_ub = \
            tik_instance.Tensor(input_dtype, (4, input_n*8*16),
        ...
        self.pred_box_left_up_x = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                                      name="pred_box_left_up_x",
                                                      scope=tik.scope_ubuf)
        self.pred_box_left_up_y = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                                      name="pred_box_left_up_y",
                                                      scope=tik.scope_ubuf)
        self.pred_box_right_down_x = \
            tik_instance.Tensor(input_dtype, (1, input_n*8*16),
    ...
    def updata_bbox_position(self, a_buffer_ub, b_buffer_ub, c_buffer_ub):
        ...
        with self.tik_instance.if_scope(True):
            widths = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                         name="widths", scope=tik.scope_ubuf)
            heights = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                          name="heights", scope=tik.scope_ubuf)
            ctr_x = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                        name="ctr_x", scope=tik.scope_ubuf)
            ctr_y = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                        name="ctr_y", scope=tik.scope_ubuf)

            pred_center_x = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                                name="pred_center_x",
                                                scope=tik.scope_ubuf)
            pred_center_y = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
                                                name="pred_center_y",
                                                scope=tik.scope_ubuf)
            pred_w = tik_instance.Tensor(input_dtype, (1, input_n*8*16),
    ...
    def decode_bbox_sel(self, input_n, result_tensor, matrix):
        """
        :param input_n:
        :param result_tensor:
        :param matrix:
        ...
        with tik_instance.if_scope(True):
            zeros_tensor = tik_instance.Tensor(self.input_dtype, (1, input_n*8*16),
                                               name="zeros_tensor",
                                               scope=tik.scope_ubuf)
            ones_tensor = tik_instance.Tensor(self.input_dtype, (1, input_n*8*16),
                                              name="ones_tensor",
                                              scope=tik.scope_ubuf)

            tik_instance.vector_dup(self.mask, zeros_tensor, 0,
                                    input_n*self.ratio, 1, 8)
            tik_instance.vector_dup(self.mask, ones_tensor, 1,
                                    input_n*self.ratio, 1, 8)

            if self.input_dtype == "float16":
                with tik_instance.for_range(0, input_n) as i:
                    dst_cmp_mask = \
    ...
    def get_decode_bbox(self):
        ...
        max_length = (input_n*8*16)//16

        a_buffer_ub = tik_instance.Tensor(self.input_dtype, (1, input_n*8*16),
                                          name="a_buffer_ub",
                                          scope=tik.scope_ubuf)
        b_buffer_ub = tik_instance.Tensor(self.input_dtype, (1, input_n*8*16),
                                          name="b_buffer_ub",
                                          scope=tik.scope_ubuf)
        c_buffer_ub = tik_instance.Tensor("uint16", [max_length],
                                          name="c_buffer_ub",
                                          scope=tik.scope_ubuf)
        d_buffer_ub = tik_instance.Tensor(self.input_dtype, (1, input_n*8*16),
                                          name="d_buffer_ub",
                                          scope=tik.scope_ubuf)

        self.updata_bbox_position(a_buffer_ub, b_buffer_ub, d_buffer_ub)

        result_x_tensor = tik_instance.Tensor(self.input_dtype, (1, input_n*8*16),
                                              name="result_x_tensor",
                                              scope=tik.scope_ubuf)
        result_y_tensor = tik_instance.Tensor(self.input_dtype, (1, input_n*8*16),
                                              name="result_y_tensor",
                                              scope=tik.scope_ubuf)
        score_bitmap = tik_instance.Tensor(self.input_dtype, (1, input_n*8*16),
                                           name="score_bitmap",
                                           scope=tik.scope_ubuf)
        ...
        tik_instance.vcmpv_ge(c_buffer_ub, a_buffer_ub, b_buffer_ub,
                              input_n*self.ratio, 1, 1, 8, 8)
        self.decode_bbox_sel(input_n, result_x_tensor, c_buffer_ub)
        ...
        tik_instance.vcmpv_ge(c_buffer_ub, a_buffer_ub, b_buffer_ub,
                              input_n*self.ratio, 1, 1, 8, 8)
        self.decode_bbox_sel(input_n, result_y_tensor, c_buffer_ub)
        ...
        tik_instance.vconcat(self.output_region_proposal_ub,
                             self.pred_box_left_up_x, input_n*8, 0)
        tik_instance.vconcat(self.output_region_proposal_ub,
                             self.pred_box_left_up_y, input_n*8, 1)
        tik_instance.vconcat(self.output_region_proposal_ub,
                             self.pred_box_right_down_x, input_n*8, 2)
        tik_instance.vconcat(self.output_region_proposal_ub,
                             self.pred_box_right_down_y, input_n*8, 3)
        tik_instance.vconcat(self.output_region_proposal_ub,
                             score_bitmap, input_n*8, 4)
...
class OneCoreProcess:
    ...
    def get_offset(self, batch_id, i, num):
        ...
        input_n = (self.num_anchor*self.H*self.W + 127) // 128
        burst = ((input_n*128 - self.num_anchor*self.H*self.W)*8*self.size + 31)//32
        tail = burst*32//(8*self.size) - (input_n*128 - self.num_anchor*self.H*self.W)
        output_bbox_offset = batch_id*(input_n*128+tail)*8 \
