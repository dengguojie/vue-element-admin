...
class TilingParams:
    ...
    def _calc_ub_factor(self, ub_size, ori_dtype, reduce_axis_size, act_format):
        inner_bytes_size = tbe_platform.get_bit_len(ori_dtype) // Constant.EIGHT_BIT
        ...
        inner_bytes_size = tbe_platform.get_bit_len(Constant.FP_32) // Constant.EIGHT_BIT
...
class LayerNormGradV2(util_tik_comm_func.OpBase):
    ...
    def _mad_compute(self, dst_ub, l0a_ub, is_pd_gamma):
        with self.tik_instance.new_stmt_scope():
            ...
            tensor_l1a = self._apply_tensor(Constant.FP_16, mad_dims, "tensor_l1a", tik.scope_cbuf)
            tensor_l0a = self._apply_tensor(Constant.FP_16, mad_dims, "tensor_l0a", tik.scope_ca)
            tensor_l0c = self._apply_tensor(Constant.FP_32, mad_dims, "tensor_l0c", tik.scope_cc)
    ...
    def _is_support_mmad(self):
        return self._is_nz() and self.tiling_params.mid_factor == Constant.BLOCK_NUM_FP16

    def _apply_tensor(self, dtype, dims, name, scope=tik.scope_ubuf):
    ...
    def _calc_var_etl_2(self, var_elt_ub, broadcast_var_elt_ub, mean_input_offset):
        with self.tik_instance.new_stmt_scope():
            ...
            epsilon = 1e-5 if var_elt_ub.dtype == Constant.FP_16 else 1e-12
    ...
    def _calc_drop_out(self, cast_dy_ub, x_input_offset, mask_input_offset):
        with self.tik_instance.new_stmt_scope():
            mask_ub = self._apply_tensor("uint8", self.tiling_params.x_ub_dims, "mask_ub")
...
def op_select_format(input_dy, input_x, input_variance, input_mean,
    ...
    supported_datatype = "float16,float16,float16,float32,float32,float32"
    supported_datatype_mask = "uint8,uint8,uint8,uint8,uint8,uint8"
    supported_datatype_beta = "float,float,float,float,float,float"
    supported_format = "NCHW,NHWC,ND,NCHW,NHWC,ND"
    supported_format_with_reduce = "NCHW,NHWC,ND,NCHW,NHWC,ND"
...
def ln_dropout_grad(input_dy, input_x, input_variance, input_mean,
                    input_gamma, input_mask, output_pd_x, output_pd_x_dropout, output_pd_gamma,
                    output_pd_beta, keep_prob, kernel_name="ln_dropout_grad", impl_mode="high_performance"):
    """
