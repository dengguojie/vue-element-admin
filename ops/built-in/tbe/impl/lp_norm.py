...
_CCE_PLAT = cceconf.get_soc_spec('SOC_VERSION')
...
def lp_norm0_compute(abs_x, x_type, y, axes, keepdim, kernel_name):
    ...
    one_tensor = tbe.vadds(zero_tensor, tvm.const(1, dtype=x_type))
...
def lp_norm_compute(abs_x, y, p, axes, keepdim, kernel_name):
    ...
    zero_tensor = tbe.vmuls(log_sum_x, tvm.const(0, dtype="float32"))
    p_tensor = tbe.vadds(zero_tensor, tvm.const(p, dtype="float32"))
...
def lp_norm(x, y, p=2, axes=None, keepdim=False, epsilon=1e-12, kernel_name="lp_norm"):
    ...
    elif _CCE_PLAT == 'Ascend310' and p == 2:
        res = lp_norm2_compute(abs_data, y, axes, keepdim, kernel_name)
    else:
        if x_type == "float16":
            abs_data = tbe.cast_to(abs_data, "float32", False)
