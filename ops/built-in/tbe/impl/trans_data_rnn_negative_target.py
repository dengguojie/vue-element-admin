...
def _once_vnchwconv_invert(args):
    ...
    with tik_inst.new_stmt_scope():
        src_stride = tik_inst.Scalar(dtype="int32")
        dst_stride = tik_inst.Scalar(dtype="int32")
        hidden_ceil = tik_inst.Scalar(dtype="int32")
        hidden_left = tik_inst.Scalar(dtype="int32")
        hidden_head_len = tik_inst.Scalar(dtype="int32")
        hidden_mid_start = tik_inst.Scalar(dtype="int32")
        hidden_mid_cnt = tik_inst.Scalar(dtype="int32")
        hidden_tail_start = tik_inst.Scalar(dtype="int32")
        hidden_tail_len = tik_inst.Scalar(dtype="int32")
...
def _reorder_data(args, tiling_mode):
    ...
    (tik_inst, src_ub, ub_offset, in_dtype, cl_plp_size, dst_c_size, c_plp_size,
     cr_lp_cnt, cr_lp_idx, dst_cr_lp_unit, cr_pln_size, is_mc_cr, c0_len, ele_per_block, c0_pad_size, hidden_cnt,
     hidden_output, hidden_size, core_step_in) = args
    ...
    with tik_inst.if_scope(tiling_mode == 2001):
        vnc_args = (tik_inst, src_ub, ub_offset, in_dtype, cl_plp_size, c_plp_size,
                    cr_lp_idx, dst_cr_lp_unit, cr_pln_size, c0_len, ele_per_block, hidden_output,
                    hidden_size, core_step_in)
    ...
    with tik_inst.else_scope():
        ...
        vnc_args = (tik_inst, src_ub, ub_offset, in_dtype, cl_plp_size, dst_c_size, c_plp_size,
                    cr_lp_cnt, hidden_output, cr_pln_size, is_mc_cr, c0_len, ele_per_block, tiling_mode, c0_pad_size)
...
def _update_output_offset(args):
    ...
    out_offset = (cl_lp_idx * dst_cl_lp_step_out - cl_backend * dst_cl_step_out +
                  hidden_dst_cr_lp_step_out - cr_backend * dst_cr_step_out +
                  c_lp_idx * src_c_lp_step_out - c_backend * src_c_step_out + hidden_core_step_out)
                    ...
                    with tik_inst.new_stmt_scope(disable_sync=True):
                        with tik_inst.for_range(0, cl_plp_size) as cl_idx:
                            with tik_inst.for_range(0, sub_c_size) as c_idx:
                                ...
                                tik_inst.data_move(dst_out_gm[tmp_gm_offset], src_ub[tmp_ub_offset],
                                                   0, 1, hidden_output // ele_per_block, 0, 0)
                    with tik_inst.if_scope(hidden_output % ele_per_block > 0):
                        with tik_inst.for_range(0, cl_plp_size) as cl_idx:
                            with tik_inst.for_range(0, sub_c_size) as c_idx:
                                ...
                                for i in tdc.REG_IDX_LIST[:ele_per_block]:
                                    src_ub[i:].set_as(tmp_reg[i])
                                tik_inst.data_move(dst_out_gm[tmp_gm_offset + hidden_output - ele_per_block], src_ub,
                                                   0, 1, 1, 0, 0)


def calc_cr_offset(tik_inst, core_step_in, block_idx, hidden_size, c0_size, cr_offset):
    hidden_head_len = 0
    hidden_mid_start = 0
    hidden_mid_cnt = 0
    hidden_tail_start = 0
    ...
    with tik_inst.if_scope(core_step_in > hidden_ceil - hidden_left):
        with tik_inst.if_scope(hidden_left < hidden_size):
            with tik_inst.if_scope(hidden_left == 0):
                hidden_head_len = (0)
                hidden_mid_start = (0)
            with tik_inst.else_scope():
                hidden_head_len = (hidden_size - hidden_left)
                hidden_mid_start = (hidden_ceil - hidden_left)
        with tik_inst.else_scope():
            hidden_head_len = (0)
            hidden_mid_start = (hidden_ceil - hidden_left)
        with tik_inst.if_scope(core_step_in > hidden_mid_start):
    ...
    with tik_inst.else_scope():
        with tik_inst.if_scope(hidden_left < hidden_size):
            ...
            with tik_inst.if_scope(core_step_in > hidden_size - hidden_left):
...
def _func_transform_200(tensor_args, tp_args):
    ...
    def _inner_func(tiling_args):
        ...
        with tik_inst.if_scope(tik.all(mc_pos == 2, tiling_mode == 2001)):
            calc_cr_offset(tik_inst, core_step_in, block_idx, hidden_size, c0_len, cr_core_offset)
            ...
            with tik_inst.for_range(0, c_lp_cnt) as c_lp_idx:  # axis C
                ...
                with tik_inst.for_range(0, cl_lp_cnt) as cl_lp_idx:  # axis C-LEFT
                    ...
                    with tik_inst.if_scope(mc_pos == 2):
                        calc_cr_offset(tik_inst, dst_cr_lp_step_out, cr_lp_idx, hidden_len, c0_len, cr_lp_offset)
                        hidden_dst_cr_lp_step_out.set_as(cr_lp_idx * dst_cr_lp_step_out - cr_lp_offset)
                        hidden_core_step_out.set_as(block_idx * core_step_out - cr_core_offset)
                    with tik_inst.else_scope():
                    ...
                    reorder_args = (tik_inst, src_ub, ub_offset, in_dtype, cl_plp_size, dst_c_size, c_plp_size,
                                    cr_lp_cnt, cr_lp_idx, dst_cr_lp_unit, cr_pln_size, is_mc_cr, c0_len, ele_per_block,
                                    c0_pad_size, hidden_cnt, hidden_output, hidden_len, block_idx * core_step_in)
                    _reorder_data(reorder_args, tiling_mode)
                    out_gm_args = (cl_lp_idx, dst_cl_lp_step_out, cl_backend, dst_cl_step_out, c_lp_idx,
                                   src_c_lp_step_out, cr_lp_idx, hidden_dst_cr_lp_step_out, cr_backend, dst_cr_step_out,
                                   hidden_core_step_out, c_backend * c0_len, src_c_step_out, block_idx)
...
def trans_data_rnn_negative_target(src, dst, input_size, hidden_size, kernel_name="trans_data_rnn_negative_target"):
    ...
    src_ub = tik_inst.Tensor(in_dtype, (ub_size,), tik.scope_ubuf, "src_ub")
