...
def get_input_gm_list(tik_instance, need_valid_num, valid_num_shape, need_clip_window,
    ...
    for input_scalar in input_scalar_list:
        scalar_dtype = input_scalar.get("dtype")
        scalar_tensor = tik_instance.Tensor(scalar_dtype, [1], name="input_scalar", scope=tik.scope_gm)
...
def check_par(max_size_per_class, max_total_size, classes, boxes_type):
    ...
    para_check.check_dtype(boxes_type, ("float16",), param_name="boxes")
...
class CNMS:
    ...
    def __init__(self,
        ...
        if self.classes == self.boxes_classes and self.boxes_classes == 1:
            if self.max_size_per_class > self.max_total_size:
                self.max_size_per_class = self.max_total_size
        check_par(self.max_size_per_class, self.max_total_size, self.classes, self.box_type)
        # whether down the boxes to avoid fp16 overflow
        self.down_flag = False
    ...
    def set_default_to_scores(self, scores, data_size, whole_size=Constant.PER_LOOP_UNIT):
        ...
        with tik_instance.if_scope(tail != 0):
            offset.set_as((data_size // vector_mask + 1) * vector_mask)
    ...
    def sort_second_round_data(self, real_batch_idx, class_idx, dst):
        ...
        left_size = self.boxes_num - Constant.PER_LOOP_UNIT
        ...
        with tik_instance.new_stmt_scope():
            ...
            if loop_num > 0:
                ...
                with tik_instance.if_scope(tail > 0):
                    ...
                    burst_lens = (tail * Constant.UNIT_ELE) // Constant.BLOCK_ELE
                    tail_left = (tail * Constant.UNIT_ELE) % Constant.BLOCK_ELE
                    if burst_lens > 0:
                ...
                self.init_tensor(scores_idx_ub, score_idx_lens * 2)
                burst_lens = left_size * Constant.UNIT_ELE // Constant.BLOCK_ELE
                tail_left = left_size * Constant.UNIT_ELE % Constant.BLOCK_ELE
                if burst_lens > 0:
    ...
    def get_batch_scores_idx(self, src, real_batch_idx):
        ...
        if self.classes * self.max_size_per_class < Constant.PER_LOOP_UNIT:
            with tik_instance.new_stmt_scope():
                ...
                self.init_tensor(score_tmp, size, Constant.FP16_MINS)
                self.init_tensor(score_idx_sort, size * Constant.UNIT_ELE, Constant.FP16_MINS)
    ...
    def move_data_out(self, x1, x2, y1, y2, scores_ub, classes_out, valid_detection, batch_idx):
        ...
        with tik_instance.for_range(0, Constant.BLOCK_ELE) as idx5:
