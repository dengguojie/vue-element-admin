...
class Argmax(ArgmaxBase):
    ...
    def __init__(self, shape_x, dtype_x, axis, kernel_name):
        ...
        self.data_gm = self.tik_instance.Tensor(self.dtype_x,
    ...
    def argmax_compute(self):
        ...
        """
        # if not need split
        self.result_gm = self.tik_instance.Tensor("int32", (self.gm_result_size,), name="result_gm", scope=tik.scope_gm)
        ...
        self.tik_instance.BuildCCE(kernel_name=self.kernel_name, inputs=(self.data_gm,), outputs=(self.result_gm,))
        return self.tik_instance
        ...
        if self.first_dim_size >= core_one:
            ...
            with self.tik_instance.for_range(0, core_number, block_num=core_number) as num_core_i:
                with self.tik_instance.for_range(0, core_loop) as num_core_j:
                    ...
                    self.compute_argmax_not_last_axis_cut_by_first_dim(first_i, segment_loop, offset, segment_tail,
                                                                       segment_tail_data)
                if core_over != 0:
            ...
            with self.tik_instance.for_range(0, self.first_dim_size) as num_i:
                if core_tail == 0:
                    with self.tik_instance.for_range(0, core_one, block_num=core_one) as core_id:
                        offset_in = \
                            num_i * self.axis_size * self.last_dim_size + \
                            core_id * core_one_seg
                        offset_out = num_i * self.last_dim_size + \
                                     core_id * core_one_seg
                        self.compute_argmax_not_last_axis_cut_by_last_dim(core_one_seg, offset_in, offset_out)
                else:
                    with self.tik_instance.for_range(0, core_one, block_num=core_one) as core_id:
                        offset_in = \
                            num_i * self.axis_size * self.last_dim_size + \
                        ...
                        offset_out = num_i * self.last_dim_size + \
                                     core_id * core_one_seg
                        with self.tik_instance.if_scope(core_id < core_one - 1):
                            self.compute_argmax_not_last_axis_cut_by_last_dim(core_one_seg, offset_in, offset_out)
                        with self.tik_instance.else_scope():
                            tail_data = \
                                self.last_dim_size - \
                                core_one_seg * (core_one - 1)
                            self.compute_argmax_not_last_axis_cut_by_last_dim(tail_data, offset_in, offset_out)

    def compute_argmax_not_last_axis_cut_by_last_dim(self, data_segment, in_offset, out_offset):
        """
    ...
    def do_not_last(self, segment, gm_in_offset, gm_out_offset):
        ...
        None
        """
        ub_a = self.tik_instance.Tensor(self.dtype_x, (self.segment,), name="ub_a", scope=tik.scope_ubuf)
        ub_c = self.tik_instance.Tensor("int32", (self.segment,), name="ub_c", scope=tik.scope_ubuf)
        ...
        self.tik_instance.vector_dup(OUT_MASK, ub_c, 0, _get_ceil_int(data_segment, OUT_MASK), 1, 8)
        thread_num = 2 if self.axis_size > 2 else 1
        with self.tik_instance.for_range(1, self.axis_size, thread_num=thread_num) as axis_i:
            ub_mask = self.tik_instance.Tensor("uint64", (self.segment // OUT_MASK,),
                                               name="ub_mask",
                                               scope=tik.scope_ubuf)
            ub_b = self.tik_instance.Tensor(self.dtype_x, (self.segment,), name="ub_b", scope=tik.scope_ubuf)
            ...
            self.tik_instance.vmax(self.full_mask, ub_a, ub_a, ub_b, repeat, 1, 1, 1, 8, 8, 8)
    ...
    def do_not_last_fp16_default(self, segment, gm_in_offset, gm_out_offset):
        ...
        ub_out_fp16 = self.tik_instance.Tensor("float16", (self.segment,), name="ub_out_fp16", scope=tik.scope_ubuf)
        ...
        self.tik_instance.vector_dup(self.data_each_vector, ub_out_fp16, 0,
                                     _get_ceil_int(data_segment, self.data_each_vector), 1, 8)
        # Init out
        ...
        with self.tik_instance.for_range(1, self.axis_size, thread_num=thread_num) as axis_i:
            ...
            self.tik_instance.vmax(self.full_mask, ub_a, ub_a, ub_b, repeat, 1, 1, 1, 8, 8, 8)
        vec_conv_repeat = _get_ceil_int(data_segment, OUT_MASK)
        self.tik_instance.vec_conv(64, "round", ub_c, ub_out_fp16, vec_conv_repeat, 8, 4)
    ...
    def do_not_last_fp16_align(self, segment, gm_in_offset, gm_out_offset):
        ...
        axis_ub_offset = \
            ...
            self.data_each_vector
        repeat = _get_ceil_int(data_segment, self.data_each_vector)
        ub_max = self.tik_instance.Tensor(self.dtype_x, (axis_ub_offset,), name="ub_max", scope=tik.scope_ubuf)
        ...
        def _run_one_sigment(axis_idx, axis_len):
            ub_a = self.tik_instance.Tensor(self.dtype_x, (self.segment,), name="ub_a", scope=tik.scope_ubuf)
            # copy all in
            self.tik_instance.data_move(ub_a, self.data_gm[gm_in_offset + axis_idx * data_segment], 0, axis_len,
                                        nbust_len, 0, des_copy_stride)
            with self.tik_instance.for_range(0, axis_len) as axis_i:
                ...
                self.tik_instance.vmax(self.full_mask, ub_max, ub_max, ub_a[_axis_offset], repeat, 1, 1, 1, 8, 8, 8)
        ...
        if axis_loop > 1:
            ...
            thread_num = 1
        if axis_loop != 0:
            with self.tik_instance.for_range(0, axis_loop, thread_num=thread_num) as _axis_loop:
    ...
    def argmax_last_axis(self):
        ...
        with self.tik_instance.for_range(0, core_num_used, block_num=core_num_used) as n_i:
    ...
    def do_argmax_last_axis_fp16_a100(self, ub_buf_size, loop, n_i):
        ...
        ub_result = self.tik_instance.Tensor(
            self.dtype_x, (_get_ceil_int(_get_ceil_int(ub_buf_size, self.data_each_vector) * 2, self.data_each_vector) *
                           self.data_each_vector,),
            name="ub_result",
            scope=tik.scope_ubuf)
        ub_data = self.tik_instance.Tensor(self.dtype_x,
                                           (_get_ceil_int(ub_buf_size, self.data_each_vector) * self.data_each_vector,),
                                           name="ub_data",
                                           scope=tik.scope_ubuf)
        offset = loop * self.segment + n_i * self.axis_size
        self.tik_instance.data_move(ub_data, self.data_gm[offset], 0, 1,
                                    _get_ceil_int(ub_buf_size, self.data_each_block), 0, 0)
        ...
        if tail != 0:
            ...
            self.tik_instance.vector_dup([mask_h, mask_l], ub_data[_offset * self.data_each_vector], SCALAR_MIN_FP16, 1,
                                         1, 8)

        max_value = self.tik_instance.Scalar(self.dtype_x)
        max_cnt = self.tik_instance.Scalar("int32")
        max_index = self.tik_instance.Scalar("int32")
        repeat_times = _get_ceil_int(ub_buf_size, self.data_each_vector)
        self.tik_instance.vcmax(self.data_each_vector,
                                ub_result,
                                ub_data,
                                repeat_times,
                                1,
                                1,
                                8,
                                max_cnt_index=[max_value, max_index, max_cnt])
        if self.loop_times == 0:
            self.result_int32.set_as(max_index)
            self.result_out_scalar.set_as(max_value)
        else:
            ub_result_cmp = self.tik_instance.Tensor(self.dtype_x, (self.data_each_vector,),
                                                     name="ub_result_cmp",
                                                     scope=tik.scope_ubuf)
            ub_result_idx = self.tik_instance.Tensor("int32", (16,), name="ub_result_idx", scope=tik.scope_ubuf)
            ub_result_cmp[0].set_as(self.result_out_scalar)
            ub_result_cmp[1].set_as(max_value)
            ub_result_idx[0].set_as(self.result_int32)
            ub_result_idx[1].set_as(max_index + loop * self.segment)
            ...
            self.result_int32.set_as(ub_result_idx[max_index1])
    ...
    def do_argmax_last_axis_fp16_default(self, ub_buf_size, loop, n_i):
        ...
        if repeat_times > 64:
            ...
            ub_second_result = self.tik_instance.Tensor(
                self.dtype_x, (_get_ceil_int(repeat_times, self.data_each_vector) * self.data_each_vector,),
            ...
            ub_third_result = self.tik_instance.Tensor(
                self.dtype_x, (_get_ceil_int(_repeat_times, self.data_each_vector) * self.data_each_vector,),
                name="ub_third_result",
                scope=tik.scope_ubuf)

            _mask = _calu_mask_by_one_zero(repeat_times % 64)
            self.tik_instance.vcmax(_mask, ub_third_result, ub_second_result, 1, 1, 1, 8)
            ...
            ub_second_result = self.tik_instance.Tensor(
                ...
                name="ub_second_result",
                scope=tik.scope_ubuf)
            self.tik_instance.vcmax(_mask, ub_second_result, ub_result, 1, 1, 1, 8)
            second_max_index = self.tik_instance.Scalar("uint16")
    ...
    def compute_argmax_last_axis_fp16_more_dims(self, n_i, core_segment, segment_core):
        """
        compute arg when do last axis
        ...
        if self.c_align_ubsize >= self.data_each_vector:
            vector_size_repeat = _get_ceil_int(self.c_align_ubsize, self.data_each_vector)
            ...
            vector_size = _get_ceil_int(self.c_align_ubsize, self.data_each_block)
        ...
        if _loop_segment != 0:
            if _loop_segment >= 2 and align_num == 1:
                ...
                thread_num = 1
            with self.tik_instance.for_range(0, _loop_segment, thread_num=thread_num) as _loop:
    ...
    def do_argmax_last_axis_fp16_less_vector(self, offset, segment_len):
        """do arg in one segment fo float16
        """
        align_loop = get_align_num(self.c_align_ubsize, self.data_each_block)
        ub_data = self.tik_instance.Tensor(self.dtype_x, (self.segment,), name="ub_data", scope=tik.scope_ubuf)
        if align_loop != 1:
            ub_data_1 = self.tik_instance.Tensor(self.dtype_x, (self.segment,), name="ub_data_1", scope=tik.scope_ubuf)
        ...
        for align_idx, _ in enumerate(range(align_loop)):
            ...
            nburst_len = _get_ceil_int(self.c_align_ubsize, self.data_each_block)
            ...
            self.tik_instance.data_move(copy_ub, self.data_gm[input_gm_offset], 0, nburst, nburst_len,
                                        src_nburst_stride, des_nburst_stride)
            # get one axis vcmax
            self.tik_instance.vcmax(self.c_align_ubsize, copy_ub, copy_ub, repeat_vcmax, 1, 1, repeat_block)
            with self.tik_instance.for_range(0, repeat_vcmax) as _vcmax_idx:
                ...
                self.ub_result_int32[_vcmax_idx * align_loop + align_idx].set_as(result_int32)
    ...
    def do_argmax_last_axis_fp16_more_vector(self, offset, segment_len):
        ...
        ub_data = self.tik_instance.Tensor(self.dtype_x, (self.segment,), name="ub_data", scope=tik.scope_ubuf)
        ub_second_result = self.tik_instance.Tensor(
            self.dtype_x, (_get_ceil_int(segment_len * 2, self.data_each_vector) * self.data_each_vector,),
            name="ub_second_result",
            scope=tik.scope_ubuf)
        ...
        for align_idx, _ in enumerate(range(align_loop)):
            ...
            if tail != 0:
                ...
                self.tik_instance.vector_dup([mask_h, mask_l], copy_ub[_offset * self.data_each_vector],
                                             SCALAR_MIN_FP16, nburst, 1, vector_size_repeat * 8)
            repeat_times = _get_ceil_int(self.c_align_ubsize, self.data_each_vector)
            self.tik_instance.vcmax(self.data_each_vector, copy_ub, copy_ub, vector_size_repeat * nburst, 8 * 8, 1, 8)
            _mask = _calu_mask_by_repeat_times(repeat_times)
            self.tik_instance.vcmax(_mask, second_ub, copy_ub, nburst, 1, 8, repeat_times * 8)
            with self.tik_instance.for_range(0, nburst) as out_idx:
                second_max_index = self.tik_instance.Scalar("uint16")
                second_max_index.set_as(second_ub[out_idx * 2 + 1])
                last_max_index = self.tik_instance.Scalar("uint16")
                last_max_index.set_as(copy_ub[vector_size * out_idx + second_max_index * 8 + 1])
                result_int32.set_as(second_max_index * 8 + last_max_index)
                self.ub_result_int32[out_idx * align_loop + align_idx].set_as(result_int32)

    def compute_argmax_last_axis_copy_one_time(self, n_i, core_segment, segment_core):
        """
        compute arg when do last axis
    ...
    def do_argmax_last_axis_fp16_copy_one_time(self, ub_buf_size, loop, n_i):
        ...
        None
        """
        ub_result = self.tik_instance.Tensor(self.dtype_x, (_get_ceil_int(ub_buf_size, self.data_each_vector) * 2,),
                                             name="ub_result",
                                             scope=tik.scope_ubuf)
    ...
    def do_argmax_last_axis_fp32(self, ub_buf_size, loop, n_i):
        ...
        self.tik_instance.data_move(ub_data, self.data_gm[offset], 0, 1, nbust, 0, 0)
        tail = ub_buf_size % self.data_each_vector
        if tail != 0:
            mask_h = 0
            mask = 2**tail - 1
            mask_l = MAX_MASK_INT64 - mask
            _offset = ub_buf_size // self.data_each_vector
            self.tik_instance.vector_dup([mask_h, mask_l], ub_data[_offset * self.data_each_vector], SCALAR_MIN_FP32, 1,
                                         1, 8)
        ...
        with self.tik_instance.for_range(0, repeat) as i:
            ...
            with self.tik_instance.if_scope(mask_l != 0):
                self.tik_instance.vector_dup([mask_l, mask_l], ub_index_int32, index * self.data_each_vector, 1, 1, 8)
