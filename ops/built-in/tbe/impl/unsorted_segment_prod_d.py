...
def check_supported(x, segment_ids, y, num_segments, kernel_name="unsorted_segment_prod_d"):
    """
...
# pylint: disable=unused-argument,invalid-name
@tbe_platform.fusion_manager.fusion_manager.register("unsorted_segment_prod_d")
def unsorted_segment_prod_d_compute(x, segment_ids, y, num_segments, kernel_name="unsorted_segment_prod_d"):
    """
    compute for unsorted_segment_prod_d_compute
    """
    res = tbe.unsorted_segment_prod(x, segment_ids, num_segments)
    return res
...
def unsorted_segment_prod_d(x, segment_ids, y, num_segments, kernel_name="unsorted_segment_prod_d"):
    """
    ...
    check_list_ids = ("int32", )
    para_check.check_dtype(segment_ids_dtype, check_list_ids, param_name="segment_ids")
    prod_support = tbe_platform.api_check_support("te.lang.cce.unsorted_segment_prod", "float32")
    if dtype == "float32" and not prod_support:
        raise RuntimeError("Input dtype only support float16 while input dtype is float32")
    if num_segments <= 0:
        raise RuntimeError(
            "unsorted_segment_prod_d only supports num_segments greater than 0, while num_segments is %d" %
            (num_segments))

    first_shape = int(shape[0])
    ids_length = int(segment_ids_shape[0])
    if first_shape != ids_length:
        raise RuntimeError("unsorted_segment_prod_d only support inputs[0] equal to segment_ids_shape[0], while"
                           " inputs[0] is %d, segment_ids_shape[0] is %d" % (first_shape, ids_length))
    total_ub_size = (num_segments + first_shape) * BLOCK_LENGTH + (
        (BLOCK_LENGTH // 2 - first_shape % (BLOCK_LENGTH // 4)) + first_shape) * (BLOCK_LENGTH // 8)
    if total_ub_size > UB_SIZE_MAX // 2:
        raise RuntimeError("unsorted_segment_prod_d num_segments=%d, shape[0]=%d, greater than UB_SIZE_MAX" %
