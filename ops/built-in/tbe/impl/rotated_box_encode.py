...
class RotatedBoxEncode():
    ...
    def init_tik_inst(self):
        """init_tik_inst
        """
        self.tik_inst = tik.Tik(tik.Dprofile("v100", "cloud"))
...
def _check_param(anchor_box, gt_box, y, weight):
    ...
    if anchor_dtype == "float16" and anchor_shape[2] < 16:
        raise RuntimeError("when the data type is float16, the data shape[2] must be greater than 16.")

    if anchor_dtype == "float32" and anchor_shape[2] < 8:
        raise RuntimeError("when the data type is float32, the data shape[2] must be greater than 8.")
