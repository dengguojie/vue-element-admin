...
def relu6_grad_compute(input_grad, input_x, output_y, kernel_name="relu6_grad"):
    ...
    result = input_grad * (input_x > 0.0 * input_x < 6.0)
    """
    ...
    if tbe_platform.get_soc_spec(tbe_platform.SOC_VERSION) in ("Ascend310", "Ascend910") or input_dtype == "float32":
        ...
        final_res = tbe.vmul(y_y_esp_min, input_grad)
    else:
        is_gt_0 = tbe.vcmpsel(input_x, 0.0, 'gt', 1.0, 0.0)
        condition = tbe.vcmpsel(input_x, 6.0, 'lt', is_gt_0, 0.0)
        final_res = tbe.vmul(input_grad, condition)
...
def relu6_grad(input_grad, input_x, output_y, kernel_name="relu6_grad"):
    ...
    final_res = relu6_grad_compute(input_grad, input_data_orginal, output_y, kernel_name="relu6_grad")
    with tvm.target.cce():
        auto_sch = tbe.auto_schedule(final_res)

    config = {"name": kernel_name, "tensor_list": (input_grad, input_data_orginal, final_res)}
