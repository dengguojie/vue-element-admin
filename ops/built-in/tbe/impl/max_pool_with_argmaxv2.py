...
UB_SIZE = tbe_platform.cce_conf.CceProductParams().getParams("Unified_Buffer")
...
class MaxPoolWithargmaxPytorch():
    ...
    def __init__(self, x, ksize, strides, padding, dtype, dilation, ceil_mode,
        ...
        mask_tmp = self.fmap_h_num * SCALAR_C0 - self.fmap_h
    ...
    def tik_instance_function(self, kernel_name):
        ...
        if need_cut_h or need_cut:
            ...
            if need_cut_h_w:
                raise RuntimeError("cut h_w is not yet support now\n")
            ...
            with self.tik_instance.for_range(
                    0, block_dim, block_num=block_dim) as block_index:
                with self.tik_instance.if_scope(block_index != block_dim - 1):
                    with self.tik_instance.for_range(
                            0, nc1_cuth_size) as nc1_cuth_index:
    ...
    def _check_if_need_cut_h_w(self):
        ...
        need_cut = False
        ub_size_used_max = \
            self.output_wh * self.c0_size * self.kernel_h * self.kernel_w * 2
        ub_size_cut_h_max = \
            self.output_w * self.c0_size * self.kernel_h * self.kernel_w * 2
    ...
    def _fun_no_cut(self, block_index, nc1_index, nc1_size):
        ...
        fmap_shape_ub = (self.fmap_h_num * SCALAR_C0, self.kernel_h,
        ...
        data_x_max = self.tik_instance.Tensor("float16",
                                              (self.fmap_h_num, SCALAR_C0, SCALAR_C0),
                                              name="data_x_max",
                                              scope=tik.scope_ubuf)

        # copy input fmap from gm to l1
        gm_l1_burst_len = int(self.input_wh * self.c0_size // SCALAR_C0)
        ...
        with self.tik_instance.for_range(0, self.fmap_h_num) as h_index:
            source_h = (((h_index * 256 * self.fmap_w) //
                         (SCALAR_C0 * self.fmap_w)) // self.output_w) * \
            ...
            if self.mask_zero != 0 and self.fmap_w != 1:
                self.tik_instance.vector_dup(
                    ...
                    mask_ub[w_index * self.fmap_h_num * self.c0_size +
                            self.fmap_h_num * SCALAR_C0 - SCALAR_C0], 0, 1, 1, 8)
    ...
    def _calc_only_cut_h(self, cur_h_idx, cut_h_size, cut_stride, cut_h_num,
        ...
        none
        """
        fmap_cut_h_num = _ceil_div(fmap_cut_h, SCALAR_C0)
        mask_ub = self.tik_instance.Tensor("uint16", mask_shape_ub,
                                           name="mask_ub",
                                           scope=tik.scope_ubuf)
        data_x_max = self.tik_instance.Tensor(
            "float16", (fmap_cut_h_num, SCALAR_C0, SCALAR_C0),
        ...
        with self.tik_instance.if_scope(cur_h_idx != 0):
            with self.tik_instance.if_scope(cur_h_idx != (cut_h_num - 1)):
                ...
                with self.tik_instance.if_scope(cur_h_idx == (cut_h_num - 2)):
                    ...
                    self.tik_instance.data_move(
                        input_fmap_l1,
                        self.input_fmap_gm[nc1_num * self.input_h *
                                           self.input_w * self.c0_size +
                                           (cur_h_idx * cut_stride -
                                            self.pad[2]) * self.input_w *
                                           self.c0_size],
                ...
                with self.tik_instance.else_scope():
                    with self.tik_instance.if_scope(cur_h_idx != 1):
                        self.tik_instance.data_move(
                            input_fmap_l1,
                            self.input_fmap_gm[nc1_num * self.input_wh *
                                               self.c0_size +
                                               (cur_h_idx * cut_stride -
                                                self.pad[2]) *
                                               self.input_w * self.c0_size],
                        ...
                        with self.tik_instance.for_range(
                                0, fmap_cut_h_num) as h_index:
                            source_h = (((h_index * 256 * self.fmap_w) //
                                         (SCALAR_C0 * self.fmap_w)) //
                            ...
                            self.tik_instance.load3dv1(
                                ...
                                self.dilation_h, 1, 0, self.fmap_w, 0,
                                self.pad_value)
                    with self.tik_instance.else_scope():
                        # copy input fmap from gm to l1
                        len_tmp1 = \
                        ...
                        with self.tik_instance.for_range(0, fmap_cut_h_num) \
                            ...
                            self.tik_instance.load3dv1(
                                fmap_ub[h_index * 256 * self.fmap_w],
                                input_fmap_l1[0],
                                (self.pad[0], self.pad[1], pad_max1, 0),
                                (cut_h_size - pad_max1),
                                self.input_w, 0, 0, 0,
                                self.scalar_source_w,
                                self.scalar_source_h,
                ...
                if self.fmap_w != 1:
                    self._calc_max_and_mask(
                        fmap_cut_h_num, fmap_ub, data_x_max, mask_ub)

                    # move max output to gm
                    gm_max_burst_len = int(fmap_cut_h)
                    self.tik_instance.data_move(
                        self.output_max_gm[
                            nc1_num * self.output_wh * self.c0_size +
                            cur_h_idx * fmap_cut_h * self.c0_size],
            ...
            with self.tik_instance.else_scope():
                ...
                fmap_h_tail_num = _ceil_div(fmap_h_tail, SCALAR_C0)
                # copy input fmap from gm to l1
                gm_l1_burst_len = \
                    int(cut_h_tail * self.input_w * self.c0_size // SCALAR_C0)
                ...
                if self.fmap_w != 1:
                    ...
                    self._dup_mask_fun(mask_ub, mask_shape_ub)
                mask_cut = fmap_h_tail_num * SCALAR_C0 - fmap_h_tail
                ...
                with self.tik_instance.for_range(
                    ...
                    if mask_zero_cut != 0 and self.fmap_w != 1:
                        self.tik_instance.vector_dup([
                            ...
                            fmap_h_tail_num * SCALAR_C0 - SCALAR_C0], 0, 1, 1, 8)

                    self.tik_instance.data_move(
                        self.output_mask_gm[
                            offset_output_mask +
                            w_index * (self.fmap_h_num + 1) * SCALAR_C0],
                        mask_ub[w_index * fmap_cut_h_num *
                                self.c0_size],
    ...
    def _fun_only_cut_h(self, block_index, nc1_cuth_index, cut_h_size,
        ...
        fmap_cut_h = self.output_w * out_size_cut_h
        fmap_cut_h_num = _ceil_div(fmap_cut_h, SCALAR_C0)
        fmap_shape_ub = (fmap_cut_h_num * SCALAR_C0, self.kernel_h,
                         self.kernel_w, self.c0_size)
    ...
    def _calc_mask_fun(self, data_input_max, data_input_ub, index_w, index_h,
        ...
        mask_ub: mask in ub
        """
        self.tik_instance.vcmpv_eq(mask_ub[index_w * fmap_h_num * SCALAR_C0 +
                                           index_h * SCALAR_C0],
    ...
    def _calc_max_and_mask(self, fmap_h_num, fmap_ub, data_x_max,
        ...
        if scalar_repeat_times > 255:
            ...
            self.tik_instance.vector_dup(MASK, data_x_max, MIN_VALUE_FP16,
                                         ...
                                         SRC0STRIDEM1)
        # cal max, iter ker_h x ker_w x 3:ceil_div(outh x outw, SCALAR_C0)
    ...
    def _remove_repeated_fun(self, mask_ub, fmap_cut_h=0,
        ...
        if fmap_cut_h != 0:
            if fmap_cut_w != 0:
                fmap_h_num = _ceil_div(fmap_cut_w, SCALAR_C0)
            else:
                fmap_h_num = _ceil_div(fmap_cut_h, SCALAR_C0)
        else:
            fmap_h_num = _ceil_div(self.fmap_h, SCALAR_C0)

        mask_or_shape_ub = (fmap_h_num, SCALAR_C0)
            ...
            with self.tik_instance.if_scope(index_w > 0):
                if fmap_tail_w == 0:
                    if fmap_tail_h == 0:
                        ...
                        fmap_tail_num = _ceil_div(fmap_tail_h, SCALAR_C0)
                        self.tik_instance.vor(
                            SCALAR_C0, mask_or[0],
                            mask_ub[index_w * fmap_tail_num * SCALAR_C0],
                            mask_or[0],
                            fmap_h_num, DSTSTRIDEM0, SRC0STRIDEM0,
                            SRC1STRIDEM0, DSTSTRIDEM0, SRC0STRIDEM0,
                            SRC1STRIDEM0)
                        self.tik_instance.vand(
                            SCALAR_C0, mask_ub[index_w * fmap_tail_num * SCALAR_C0],
                            mask_not[0],
                            mask_ub[index_w * fmap_tail_num * SCALAR_C0],
                            fmap_h_num, DSTSTRIDEM0, SRC0STRIDEM0,
                            SRC1STRIDEM0, DSTSTRIDEM0, SRC0STRIDEM0,
                    ...
                    fmap_tail_num = _ceil_div(fmap_tail_w, SCALAR_C0)
                    self.tik_instance.vor(
                        SCALAR_C0, mask_or[0],
                        mask_ub[index_w * fmap_tail_num * SCALAR_C0],
                        mask_or[0],
                    ...
                    self.tik_instance.vand(
                        ...
                        fmap_h_num, DSTSTRIDEM0, SRC0STRIDEM0,
                        SRC1STRIDEM0, DSTSTRIDEM0, SRC0STRIDEM0, SRC1STRIDEM0)
                self.tik_instance.vnot(SCALAR_C0, mask_not[0], mask_or[0],
                                       fmap_h_num, SRC0STRIDEM0,
                                       SRC1STRIDEM0, SRC0STRIDEM0,
                                       SRC1STRIDEM0)
            with self.tik_instance.else_scope():
                self.tik_instance.vnot(SCALAR_C0, mask_not[0], mask_ub[0],
                                       fmap_h_num, SRC0STRIDEM0,
                                       SRC1STRIDEM0, SRC0STRIDEM0,
                                       SRC1STRIDEM0)
        ...
        if scalar_repeat_times > SCALAR_MAX:
            with self.tik_instance.for_range(0, repeat_times) as repeat_index:
                with self.tik_instance.if_scope(repeat_index !=
                                                (repeat_times - 1)):
                    self.tik_instance.vector_dup(
                        MASK, mask_ub[repeat_index * SCALAR_MAX * SCALAR_C0],
                        65535, 30, DSTSTRIDEM0, SRC0STRIDEM1)
                with self.tik_instance.else_scope():
                    self.tik_instance.vector_dup(
                        SCALAR_C0, mask_ub[repeat_index * SCALAR_MAX * SCALAR_C0],
                        65535, (scalar_repeat_times - repeat_index * SCALAR_MAX),
                        DSTSTRIDEM0, DSTSTRIDEM0)
        else:
            self.tik_instance.vector_dup(SCALAR_C0, mask_ub, 65535,
                                         scalar_repeat_times, DSTSTRIDEM0,
                                         DSTSTRIDEM0)
...
def max_pool_with_argmax(x, y, argmax, ksize, strides, pads, dtype=DT_INT32,
    ...
    """
    _check_param(
        x, ksize, strides, pads, dtype, dilation, ceil_mode, kernel_name)
