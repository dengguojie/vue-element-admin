...
def _get_load3d_tiling(fmap_shape,
    ...
    def _get_l0ub_tiling():
        ...
        if max_howokhkw_l0ub < kernel_h * kernel_w:
            ...
            tiling["shape"]["howo"] = min(howo_block, max_howokhkw_l0ub // (kernel_h * kernel_w))
...
def _max_pool_grad_grad_ir_builder(ins, outs, ksize, strides, padding="SAME", kernel_name="cce_max_pool_grad_grad"):
    ...
    output_h, pad_t, pad_b = tbe.te_compute.common.tf_get_windowed_output_size_verbose(
    ...
    def _img_to_cubf(ho_o, wo_o, actual_pad_top_value, actual_pad_left_value):
        ...
        pad_mode_call = tvm.call_pure_intrin("int32", "tvm_cce_string_print", 'PAD_NONE')
    ...
    if block_tiling["type"] == BlockTilingType.DIVISIBLE:
        ...
        with tvm_ir.for_range(0, block_tiling["fuse_factor"], name="fuse_factor", dtype=SCALAR_DTYPE) as fuse_factor:
