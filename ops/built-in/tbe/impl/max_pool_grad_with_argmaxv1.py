...
class MaxpoolGard(MaxpoolGradCustom):
    ...
    def tik_instance_shape_large_shape(self, kernel_name):
        ...
        data_mask = self.tik_instance.Tensor("uint16", (
            batch * channel1 * windowh * windoww * mask_one_window,), name="data_mask", scope=tik.scope_gm)
                ...
                with self.tik_instance.for_range(0, nc1,
                    ...
                    with self.tik_instance.for_range(0, dyh,
                        ...
                        with self.tik_instance.for_range(0, dyw) as loopw:
                            ...
                            # dy copy gm to ub
                            self.tik_instance.data_move(data_max_ub, data_input[block_num * nc1 * dyh * dyw * channel +
                                                                                loopc1 * dyh * dyw * channel + new_looph
                                                                                * dyw * channel + channel * new_loopw],
                                                        constant.SID, 1, constant.DEFAULT_BURST_LEN + w_ratio, 0, 0)
                            if h_ratio != 0:
                                self.tik_instance.data_move(data_max_ub, data_input[block_num * nc1 * dyh * dyw *
                                                                                    channel + loopc1 * dyh * dyw *
                                                                                    channel + (new_looph + 1) * dyw *
                            ...
                            self.tik_instance.data_move(data_mask_ub, data_mask[block_num * nc1 * mask_one_window *
                                                        ...
                                                        constant.SID, windowh * windoww, constant.DEFAULT_BURST_LEN,
                                                        mask_stride, constant.STRIDE_ZERO)
                            if hoverlap != 0:
                                self.tik_instance.data_move(data_mask_ub[col2img_dyw * windowh * windoww],
                                                            data_mask[block_num * nc1 * mask_one_window * windoww *
                                                                      windowh + loopc1 * mask_one_window * windoww *
                                                                      windowh + (new_looph + 1) * dyw + loopw],
                                                            constant.SID, windowh * windoww, constant.DEFAULT_BURST_LEN,
                                                            mask_stride, constant.STRIDE_ZERO)
                            ...
                            with self.tik_instance.for_range(0,
                                ...
                                self.tik_instance.vsel(constant.MASK128, 0, data_vsel_ub[0], cmpmask, data_max_ub[0],
...
def _pooling_output_shape(input_size, kernel_size, pad, stride, dilation, ceil_mode):
    return _pooling_output_shape_pad_lr(input_size, kernel_size, pad, pad, stride, dilation, ceil_mode)


def _pool2d_shape_check(kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w, output_h,
                        output_w):
...
def check_param(x, grad, argmax, y, ksize, strides, padding, dtype, dilation,
    ...
    para_check.check_shape_size(input_gard_shape)
    para_check.check_shape_size(argmax_shape)
    para_check.check_shape_size(y_shape)
