...
class NCHW2FRACTAL4Compute:
    ...
    def __init__(self, src_shape, dtype, kernel_name):
        ...
        self.dst_shape = [(Constant.C0 * self.src_shape[2] * self.src_shape[3] +
    ...
    def data_rearrange_case_zero(self, tik_instance, ub_ori, ub_trans,
        ...
        zero padding is not required for processed N axis
        """
        chw = Constant.C0 * self.src_shape[2] * self.src_shape[3]
        # zero padding when C != 4
        if self.src_shape[1] != Constant.C0:
        ...
        with tik_instance.for_range(0, loop_number) as num_loop:
            with tik_instance.for_range(0, chw // Constant.CUBE_SIZE) as num_row:
                with tik_instance.for_range(0, Constant.CUBE_SIZE // 4) as row_index:
                    trans_offset = loop_number * self.dst_shape[3] * \
                                   num_row + num_loop * Constant.CUBE_SIZE + \
                                   row_index * 4
                    ori_offset = num_loop * self.src_shape[1] * \
                                 self.src_shape[2] * self.src_shape[3] + \
                                 num_row * Constant.CUBE_SIZE // 4 + row_index
                    with tik_instance.for_range(0, self.src_shape[1]) as num_c:
                        ub_trans[trans_offset + num_c].set_as(
                            ub_ori[ori_offset + num_c * self.src_shape[2] *
                                   self.src_shape[3]])
            if chw % Constant.CUBE_SIZE != 0:
                with tik_instance.for_range(0, (chw % Constant.CUBE_SIZE) // 4) \
                        as row_index:
                    trans_offset = (self.dst_shape[0] - 1) * \
                                   self.dst_shape[3] * loop_number + \
                                   num_loop * Constant.CUBE_SIZE + row_index * 4
                    ori_offset = num_loop * self.src_shape[1] * \
                                 self.src_shape[2] * self.src_shape[3] + \
                                 (self.dst_shape[0] - 1) * Constant.CUBE_SIZE // 4 + \
                                 row_index
                    with tik_instance.for_range(0, self.src_shape[1]) as num_c:
                        ub_trans[trans_offset + num_c].set_as(
                            ub_ori[ori_offset + num_c * self.src_shape[2] *
    ...
    def data_rearrange_case_two(self, tik_instance, ub_ori, ub_trans,
        ...
        zero padding is not required for processed N axis
        """
        row_number = hw_number * Constant.C0 // Constant.CUBE_SIZE
        # zero padding when C != 4
        if self.src_shape[1] != Constant.C0:
        ...
        with tik_instance.for_range(0, row_number) as num_row:
            with tik_instance.for_range(0, Constant.CUBE_SIZE // 4) as row_index:
                trans_offset = self.dst_shape[3] * num_row + \
                               row_index * 4
                ori_offset = num_row * Constant.CUBE_SIZE // 4 + row_index
                with tik_instance.for_range(0, self.src_shape[1]) as num_c:
                    ub_trans[trans_offset + num_c].set_as(
    ...
    def data_rearrange_case_three(self, tik_instance, ub_ori, ub_trans,
        ...
        and zero padding is not required for processed N axis
        """
        chw = hw_number * Constant.C0
        row_number = chw // Constant.CUBE_SIZE
        hw_align = (hw_number + Constant.CUBE_SIZE - 1) // Constant.CUBE_SIZE * Constant.CUBE_SIZE
        # zero padding when C != 4
        if self.src_shape[1] != Constant.C0:
            dup_number = (chw + Constant.CUBE_SIZE - 1) // Constant.CUBE_SIZE * self.dst_shape[3]
            offset = 0
            self.vector_dup_zero_python(tik_instance, ub_trans,
                                        dup_number, offset)
        # zero padding when C0 * H * W is not divisible by 16.
        elif (hw_number * Constant.C0) % Constant.CUBE_SIZE != 0:
        ...
        if chw % Constant.CUBE_SIZE != 0:
            with tik_instance.for_range(0, (chw % Constant.CUBE_SIZE) // 4) \
                ...
                trans_offset = self.dst_shape[3] * row_number + \
                               row_index * 4
                ori_offset = row_number * Constant.CUBE_SIZE // 4 + row_index
                with tik_instance.for_range(0, self.src_shape[1]) as num_c:
                    ub_trans[trans_offset + num_c].set_as(ub_ori
                                                          [ori_offset +
                                                           num_c *
    ...
    def data_rearrange_case_four(self, tik_instance, ub_trans, hw_number):
        ...
        """
        # dst is all 0
        dup_number = (Constant.C0 * hw_number + Constant.CUBE_SIZE - 1) // Constant.CUBE_SIZE * Constant.CUBE_SIZE
    ...
    def data_move_case_three(self, tik_instance, ub_ori, ub_trans, num_n,
        ...
        with tik_instance.for_range(0, ori_hw // loop_hw) as num_loop_hw:
            if ori_hw % Constant.CUBE_SIZE != 0 or \
                    (ori_hw - loop_hw) // self.num_data > Constant.MAX_STRIDE_BLK:
                with tik_instance.for_range(0, self.src_shape[1]) as num_c:
            ...
            if (self.dst_shape[1] * self.dst_shape[2] - 1) * \
                    self.dst_shape[3] // self.num_data > Constant.MAX_STRIDE_BLK:
                with tik_instance.for_range(0, Constant.C0 * loop_hw // Constant.CUBE_SIZE) \
                        as num_x1:
                    dst_gm_index = (Constant.C0 * loop_hw // Constant.CUBE_SIZE * num_loop_hw +
                ...
                tik_instance.data_move(self.dst_gm[dst_gm_index],
                                       ub_trans[0],
                                       0, Constant.C0 * loop_hw // Constant.CUBE_SIZE, 1, 0,
                                       (self.dst_shape[1] *
                                        self.dst_shape[2] - 1) *
                                       self.dst_shape[3] // self.num_data)
        if loop_remainder != 0:
            ...
            if (self.dst_shape[1] * self.dst_shape[2] - 1) * \
                    self.dst_shape[3] // self.num_data > Constant.MAX_STRIDE_BLK:
                with tik_instance.for_range(0, (Constant.C0 * loop_remainder +
                                                Constant.CUBE_SIZE - 1) // Constant.CUBE_SIZE) \
                        as num_x1:
                    dst_gm_index = (Constant.C0 * loop_hw // Constant.CUBE_SIZE *
                                    (ori_hw // loop_hw) +
                                    num_x1) * self.dst_shape[1] * \
                                   self.dst_shape[2] * self.dst_shape[3] + \
                                   num_n * self.dst_shape[3]
                ...
                tik_instance.data_move(self.dst_gm[dst_gm_index],
                                       ub_trans[0],
                                       0,
                                       (Constant.C0 * loop_remainder + Constant.CUBE_SIZE - 1) //
                                       Constant.CUBE_SIZE, 1, 0,
                                       (self.dst_shape[1] *
                                        self.dst_shape[2] - 1) *
                                       self.dst_shape[3] // self.num_data)
    ...
    def data_move_case_four(self, tik_instance, ub_trans, num_n,
        ...
        with tik_instance.for_range(0, ori_hw // loop_hw) as num_loop_hw:
            ...
            if (self.dst_shape[1] * self.dst_shape[2] - 1) * \
                ...
                tik_instance.data_move(self.dst_gm[dst_gm_index],
                                       ub_trans[0],
                                       0, Constant.C0 * loop_hw // Constant.CUBE_SIZE,
                                       1, 0,
                                       (self.dst_shape[1] * self.dst_shape[2] -
                                        1) * self.dst_shape[3] //
                                       self.num_data)
    ...
    def format_transfer_case_zero(self, tik_instance):
        ...
        with tik_instance.for_range(0, core_number, block_num=core_number) \
            ...
            if self.src_shape[0] % Constant.CUBE_SIZE != 0:
    ...
    def format_transfer_case_one(self, tik_instance):
        ...
        the transfer process when UB can not put in C0 * H * W data
        """
        ub_ori_data = self.ub_memory - self.ub_memory % (Constant.CUBE_SIZE * Constant.C0)
        ub_trans_data = ub_ori_data
        # loop_hw is a multiple of 16
        loop_hw, loop_remainder = _cal_core_loop_python(
            Constant.C0, self.src_shape[2] * self.src_shape[3], ub_ori_data)
        # divide the core according to N1 * N0
        total_core_loop_num = self.dst_shape[1] * self.dst_shape[2]
        core_number = _set_core_num(total_core_loop_num)
