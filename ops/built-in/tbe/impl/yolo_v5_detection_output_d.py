...
from impl.yolo_v5_cls_prob import check_param
from impl.yolo_v5_cls_prob import ClsProbComputer
from impl.yolo_v5_cls_prob import get_loop_param
...
class DetectionOutput(ClsProbComputer):
    ...
    def __init__(self, input_dict):
        ...
        self.bbox = self.instance.Tensor(self.dtype,
                                         ...
                                         name="box_out", scope=cce_params.scope_gm)
    ...
    def multi_class(self, batch, image_ub, param):
        ...
        if cce_conf.get_soc_spec("SOC_VERSION") not in (
            ...
            mask = self.instance.Tensor(dtype, (self.max_ub_num,),
                                        name="mask", scope=cce_params.scope_ubuf)
    ...
    def filter_obj(self, mask, xyhw_ub, param):
        ...
        obj_loop_times, ub_size, obj_last_ub_size = \
            get_loop_param(self.obj_num, self.max_ub_num)
        ...
        mask_loop, mask_size, mask_last_ub = \
            get_loop_param(mask_total, self.max_ub_num)
    ...
    def get_xyhw(self, obj_ub, mask, xyhw_ub, param):
        ...
        if self.obj_data.size // (8 * self.dsize) > self.max_ub_num:
            ...
            with self.instance.if_scope(
                    _all(mod == 0, param["mask_offset"] != 0)):
        ...
        if self.obj_data.size // (8 * self.dsize) > self.max_ub_num:
            with self.instance.if_scope(
                    _all(param["obj_loop_times"] - 1 == param["cycle"],
                         param["mask_offset"] != 0)):
                offset = param["mask_cycle"] * self.max_ub_num
        ...
        if cce_conf.get_soc_spec("SOC_VERSION") in (
            ...
            if self.obj_data.size // (8 * self.dsize) > self.max_ub_num:
                with self.instance.if_scope(param["use_gm_mask"] == 1):
                    with self.instance.if_scope(
                            _any(param["mask_offset"] % self.max_ub_num == 0,
                                 param["inner_cycle"] == 0)):
                        nburst = self.instance.Scalar("int32")
                        nburst.set_as(
                            self.max_ub_num * self.dsize // constant.BLOCK_SIZE)
                        with self.instance.if_scope(
                                param["mask_loop"] - 1 == param["mask_cycle"]):
                            nburst.set_as(
                                get_datamove_nburst(self.instance, param[
    ...
    def nms_filter(self, proposals_selected, selected_tmp, selected_count,
        ...
        supvec_ub = self.instance.Tensor("uint16", (iou_num,), name="supVec_ub",
                                         scope=cce_params.scope_ubuf)
        repeat_supvec = get_vector_repeat_times(self.instance,
                                                selected_class * 2)
