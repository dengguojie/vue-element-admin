...
def tensor_scatter_update(x,
    ...
    y: dict
        data of output.
    kernel_name: str
        kernel name, default value is "tensor_scatter_update"

    Returns:
    None
    """
    x_dtype = x.get("dtype").lower()
    check_list = ("float16", "float32", "bool", "int8", "uint8")
    check_dtype(x_dtype, check_list)

    tensor_scatter = TensorScatter(x, indices, updates, y, kernel_name, "update")
