...
def _once_vnchwconv_invert(args):
    ...
    with tik_inst.new_stmt_scope():
        ...
        hidden_offset = tik_inst.Scalar(dtype="int32")
        hidden_ceil = tik_inst.Scalar(dtype="int32")
        hidden_head_len = tik_inst.Scalar(dtype="int32")
        hidden_head_output = tik_inst.Scalar(dtype="int32")
        hidden_middle_output = tik_inst.Scalar(dtype="int32")
        hidden_tail_output = tik_inst.Scalar(dtype="int32")
        ...
        hidden_tail_end = tik_inst.Scalar(dtype="int32")
        hidden_cr_output = tik_inst.Scalar(dtype="int32")
        hidden_left.set_as(0)
        with tik_inst.if_scope(plp_cr_size == cr_lp_unit):
            hidden_left.set_as((core_step_in + plp_cr_size * cr_lp_idx) % hidden_size)
        ...
        with tik_inst.else_scope():
            with tik_inst.if_scope((hidden_size - hidden_left) < plp_cr_size):
                ...
                with tik_inst.if_scope(hidden_tail_end > 0):
                    hidden_tail_output.set_as(hidden_tail_end + hidden_offset)
            ...
            with tik_inst.for_range(0, c_lp_cnt) as c_lp_idx:
                ...
                with tik_inst.for_range(0, cr_lp_cnt) as cr_lp_idx:
                    ...
                    with tik_inst.for_range(0, nout_lp_cnt) as nout_lp_idx:
                        ...
                        with tik_inst.else_scope():
                            hidden_dst_cr_lp_step_out.set_as(cr_lp_idx * cr_lp_step_out)
