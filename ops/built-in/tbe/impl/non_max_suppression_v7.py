...
class NonMaxSuppression(object):
    ...
    def init_tik_output_mem(self):
        ...
        with self.tik_instance.new_stmt_scope():
            ...
            self.tik_instance.data_move(self.gm_nms_result,
                                        ub_nms_result, 0, 1, loop_burst_len, 0, 0)
        # workspace
...
def tik_func_trans_to_proposals_center_box(tik_instance, proposals_ub, boxes_ub, score_ub, proposal_num):
    ...
    x1_ub = tik_instance.Tensor("float16", [lens],
                                name="x1_ub", scope=tik.scope_ubuf)
    y1_ub = tik_instance.Tensor("float16", [lens],
                                name="y1_ub", scope=tik.scope_ubuf)
    x2_ub = tik_instance.Tensor("float16", [lens],
                                name="x2_ub", scope=tik.scope_ubuf)
    y2_ub = tik_instance.Tensor("float16", [lens],
    ...
    half_x2_ub = tik_instance.Tensor("float16", [lens],
                                     name="half_x2_ub", scope=tik.scope_ubuf)
    half_y2_ub = tik_instance.Tensor("float16", [lens],
                                     name="half_y2_ub", scope=tik.scope_ubuf)
    half_ub = tik_instance.Tensor("float16", [lens],
                                  name="half_ub", scope=tik.scope_ubuf)

    tik_func_vector(tik_instance, half_ub, 0.5, lens)

    tik_func_add_sub_mul(tik_instance, "vmul", half_x2_ub, x2_ub, half_ub,
                         lens, 1, 1, 1, 8, 8, 8)
    tik_func_add_sub_mul(tik_instance, "vmul", half_y2_ub, y2_ub, half_ub,
                         lens, 1, 1, 1, 8, 8, 8)
    tik_func_add_sub_mul(tik_instance, "vsub", x2_ub, x1_ub, half_x2_ub,
                         lens, 1, 1, 1, 8, 8, 8)
    tik_func_add_sub_mul(tik_instance, "vsub", y2_ub, y1_ub, half_y2_ub,
                         lens, 1, 1, 1, 8, 8, 8)
    # concat x1 to proposals_ub
    ...
    tik_func_add_sub_mul(tik_instance, "vadd", x2_ub, x1_ub, half_x2_ub,
                         lens, 1, 1, 1, 8, 8, 8)
    tik_func_add_sub_mul(tik_instance, "vadd", y2_ub, y1_ub, half_y2_ub,
                         lens, 1, 1, 1, 8, 8, 8)
...
def tik_func_trans_index_proposals(tik_instance, proposals_ub, index_ub_list, score_ub, proposal_num):
    ...
    """
    # for index id
    batch_id_ub, class_id_ub, index_id_ub = index_ub_list
    trans_repeat = ceil_div(proposal_num, 16)
...
def clip_window_compute(tik_instance, input_gm_list, input_ub_list, gm_offset,
    ...
    real_box_offset = gm_offset[0] * input_boxes_gm.shape[1] * input_boxes_gm.shape[2] + gm_offset[3]
    tik_instance.data_move(tmp_batch_box_ub,
                           input_boxes_gm[real_box_offset],
                           0, nburst,
                           box_len, 0, 0)
    tik_instance.data_move(tmp_index_ub,
                           input_index_gm[gm_offset[0], gm_offset[1], gm_offset[3], 0],
                           0, nburst,
                           index_len, 0, 0)
    # for index id
    with tik_instance.for_range(0, 3) as i:
...
def get_sorted_proposal_compute(tik_instance, output_ub, output_index_ub, input_gm_list, gm_offset, copy_num,
    ...
    with tik_instance.new_stmt_scope():
        ...
        # apply ub for score copy_gm_to_ub
        ub_tmp_index = tik_instance.Tensor("float16", [3, sorted_num],
                                           name="ub_tmp_index", scope=tik.scope_ubuf)
...
def do_nms_compute(tik_instance, nms_var_dict, thresh):
    ...
    with tik_instance.for_range(0, ceil_div(total_input_proposal_num, BURST_PROPOSAL_NUM)) as burst_index:
        ...
        tik_instance.data_move(temp_proposals_ub, ub_max_topk[start_index],
                               0, 1, 1024 // 16, 0, 0)
        tik_instance.data_move(temp_index_proposals_ub, ub_index_topk[start_index],
                               0, 1, 1024 // 16, 0, 0)
...
def nms_for_single_class(batch_idx, class_idx, nms, core_idx):
    ...
    if nms.single_loop:
        get_sorted_proposal_compute(tik_instance, topk_out_ub, index_out_ub, nms.input_gm_list, gm_offset,
                                    nms.boxes_num,
                                    ceil_div(nms.boxes_num, 16) * 16, nms.center_point_box)
    else:
        get_sorted_proposal_compute(tik_instance, topk_out_ub, index_out_ub, nms.input_gm_list, gm_offset,
    ...
    with tik_instance.new_stmt_scope():
        ...
        tik_instance.data_move(nmsed_result_ub, l1_buffer,
                               0, 1, loop_burst_len, 0, 0)
        tik_instance.data_move(nmsed_result_index_ub, l1_buffer,
                               0, 1, loop_burst_len, 0, 0)
        ...
        loop_burst_len = (nms.max_selected_nms_num_in_ub * 8) // 16
        tik_instance.data_move(l1_buffer[l1_offset], nmsed_result_index_ub,
                               0, 1, loop_burst_len, 0, 0)
        burst_lens_gm = (nms.selected_proposals_cnt * 8) // 16
        tik_instance.data_move(nms.gm_nms_result, nmsed_result_index_ub,
                ...
                with tik_instance.new_stmt_scope():
                    ...
                    with tik_instance.if_scope(global_cnt % 2 == 1):
                        ...
                        tik_instance.data_move(ub_temp, nms.gm_temp[8],
                                               0, 1, 1, 0, 0)
                        tik_instance.data_move(nms.gm_nms_result[global_cnt * 8], ub_temp,
                                               0, 1, 1, 0, 0)
                        tik_instance.data_move(nms.gm_nms_result[(global_cnt + 1) * 8],
                                               nmsed_result_index_ub[(global_cnt + 1) * 8],
                                               0, 1, burst_lens_gm, 0, 0)
                    with tik_instance.else_scope():
                        # if even
                        tik_instance.data_move(nms.gm_nms_result[global_cnt * 8], nmsed_result_index_ub[global_cnt * 8],
                                               0, 1, burst_lens_gm, 0, 0)
...
def batch_multi_class_nms_copy_out(tik_instance, nms, ub_result_boxes,
    ...
    loop_burst_len = ceil_div(proposal_num, 16)
    apply_men_len = ceil_div(proposal_num * 3, 16)
    less_flag = False
    ...
    score_thresh = nms.get_score_threshold()
    ...
    # ub index temp
    ub_index_temp = tik_instance.Tensor("float16", [proposal_num, 3],
                                        name="ub_index_temp", scope=tik.scope_ubuf)
    ub_index_temp_int32 = tik_instance.Tensor("int32", [proposal_num, 3],
    ...
    with tik_instance.new_stmt_scope():
        # process scores
        ub_out_scores = tik_instance.Tensor("float16", [proposal_num],
                                            name="ub_out_scores", scope=tik.scope_ubuf)
        with tik_instance.for_range(0, proposal_num) as i:
            ub_out_scores[i] = ub_result_boxes[i * 8 + 4]

        filter_score_compute(tik_instance, ub_index_temp_int32, ub_out_scores, proposal_num, score_thresh)

    tik_instance.data_move(nms.output_gm_list[0][output_batch_offset],
                           ub_index_temp_int32, 0, 1, apply_men_len * 2, 0, 0)
...
def batch_multi_class_nms_output(tik_instance, core_idx, _batch_idx, nms):
    ...
    output_batch_offset = _batch_idx * nms.max_output_class * 3 * nms.classes
