...
class MapIndexProcess:
    ...
    def __init__(self, input_data):
        ...
        if self.have_level_index:
            self.level_index = self.tik_instance.Tensor("int32",
                                                        ...
                                                        name="level_index",
                                                        scope=tik.scope_gm)
            self.level_index_ub = self.tik_instance.Tensor("int32", [((level_index_shape[0] + 7) // 8) * 8],
                                                           name="level_index",
                                                           scope=tik.scope_ubuf)
            self.tik_instance.data_move(self.level_index_ub, self.level_index, 0, 1, (level_index_shape[0] + 7) // 8, 0,
                                        0, 0)
    ...
    def cce_map_index(self, kernel_name="map_index"):
        ...
        res = tik_instance.Tensor("uint16", [count], name="res", scope=tik.scope_ubuf)
        ...
        with tik_instance.for_range(0, self.x_length) as i:
            ...
            sub_value = tik_instance.Tensor("int32", [max], name="sub_value", scope=tik.scope_ubuf)
            map_index_vec_dup(tik_instance, 64, sub_value, 1, max)
            map_index_vec_sub(tik_instance, 64, sub_value, self.data_seq_ub[i * length:], tmp, length)
            ...
            if tik_name in ["Hi3796CV300CS", "SD3403"]:
                ...
                tik_instance.vconv(64, "", sub_fp16, sub_value, max // 64, 1, 1, 4, 8, 1.0)

            # sub_FP16 compv 0
            zero_ub = tik_instance.Tensor("float16", [max], name="tmp_fp16", scope=tik.scope_ubuf)
            map_index_vec_dup(tik_instance, 128, zero_ub, 0, max)

            cmp_res = tik_instance.Tensor("uint16", [count], name="cmp_res", scope=tik.scope_ubuf)
            map_index_vec_dup(tik_instance, 128, cmp_res, 0, count)
            with tik_instance.if_scope(i == 0):
                tik_instance.vec_cmpv_eq(res, sub_fp16, zero_ub, max // 128, 8, 8)
...
def map_index(x_dic, data_seq_dic, level_index_dic, y_dic, kernel_name="map_index"):
    """
    :param x_dic:
    :param data_seq_dic:
    ...
    para_check.check_dtype(data_seq_dtype.lower(), check_list, param_name="data_seq")
    ...
    if level_index_dic:
        ...
        map_index_result = MapIndexProcess((tik_instance, x_dic, data_seq_dic, y_dic, level_index_dic))
    else:
        map_index_result = MapIndexProcess((tik_instance, x_dic, data_seq_dic, y_dic))
