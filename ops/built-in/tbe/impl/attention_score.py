...
class MatMulSoftmax:
        ...
        with self.tik_instance.for_range(0, m_range) as m:
            with self.tik_instance.for_range(0, n_range) as n:
                self.mid_data_to_ub(tensor_c, tensor_c_ub, om_size, cur_m_idx, cur_m_size, m, single_m_size,
                                    ub_mask, block_idx, cur_b_idx, cur_om_idx)
                tensor_c_ub_back = self.softmax_compute_last_part(tensor_c_ub, om_size, cur_m_size, cur_k_size,
                                                                  single_m_size, single_k_size,
                                                                  input_keep_prob, None, block_idx, cur_b_idx,
                                                                  cur_om_idx, cur_m_idx, m, ub_cast, reduce_ub, ub_mask)
        ...
        with self.tik_instance.for_range(0, m_range) as m:
            with self.tik_instance.for_range(0, n_range) as n:
                ...
                self.tik_instance.mmad(tensor_c_l0c, tensor_a_l0a, tensor_b_l0b,
                                       ...
                                       0)
                # do the next time
                tensor_c_ub_back_2 = self.softmax_compute_last_part(tensor_c_ub2, om_size, cur_m_size, cur_k_size,
                                                                    single_m_size, single_k_size,
                                                                    input_keep_prob, None, block_idx, cur_b_idx,
                                                                    cur_om_idx, cur_m_idx + 1, m,
    ...
    def softmax_compute_last_part(self, tensor_c_ub, om_size, cur_m_size, cur_n_size,
                                  cur_m, cur_n, input_keep_prob, ub_mask_fp16,
    ...
    def compute_one_core(self, block_idx):
        """
        """
        batch_range_value, outer_m_range_value, inner_m_range_value = self.tiling_batch_m_axis(self.batch_dim,
                                                                                               self.first_m_dim)
    ...
    def compute_process(self):
        with self.tik_instance.for_range(0, self.cur_op_core_num, block_num=self.cur_op_core_num) as block_idx:
            with self.tik_instance.if_scope(block_idx < self.cur_op_core_num - 1):
                self.compute_one_core(block_idx)
