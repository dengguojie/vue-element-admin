...
def check_supported(x, y, argmax, output_size, kernel_name='adaptive_max_pool2d'):
    ...
    dtype_bytes = tbe_platform.get_bit_len(input_dtype) // BITS_OF_BYTE
...
class AdaptiveMaxPool2d(object):
    ...
    def __init__(self, x, y, argmax, output_size, kernel_name='adaptive_max_pool2d'):
        ...
        self.ub_availble = tbe_platform.cce_conf.get_soc_spec(
            tbe_platform.cce_conf.UB_SIZE) - UB_RESERVED_SIZE
        # params run on cpu
        ...
        self.k_info_size = (self.output_h + self.output_w) * (
                tbe_platform.get_bit_len('int32') // BITS_OF_BYTE)
        ...
        self.dtype_bytes = tbe_platform.get_bit_len(self.x_dtype) // BITS_OF_BYTE
        self.ub_max_num = self.ub_availble // self.dtype_bytes
        self.ub_sec_eles = self.ub_max_num // UB_DIVIDE_NUM
        self.elem_per_block = BLOCK_BYTES // self.dtype_bytes
        ...
        self.ub_sec_cells = self.ub_max_cells // UB_DIVIDE_NUM
    ...
    def cal_tiling_info(self):
        ...
        if self.output_h == self.input_h and self.output_w == self.input_w:
            ...
            # self.cal_core_num(self.total_cells, 1)
    ...
    def tiling_w_dim_core_nc_process(self, core_idx, loop_idx, unit_num):
        ...
        """
        # dup all
        size = unit_num * self.k_max_h * self.k_max_w * C_ZERO
        repeat_times = size // MASK
        repeat_left = size % MASK
        if repeat_times > 0:
            self.tik_instance.vector_dup(MASK, self.ub_a, MIN_FP16, repeat_times, 1, 8)
        if repeat_left > 0:
            offset = repeat_times * MASK
            self.tik_instance.vector_dup(repeat_left, self.ub_a[offset], MIN_FP16, 1, 1, 8)
        ...
        with self.tik_instance.for_range(0, unit_num) as unit_idx:
            ...
            in_offset = (plane_id * self.input_h * self.input_w + k_start_h * self.input_w + k_start_w) * C_ZERO
            ub_offset = unit_idx * self.k_max_w * C_ZERO
            self.move_gm_to_ub_one_window(self.ub_a, in_offset, ub_offset, h_idx, w_idx, unit_num)

        # reduce max width
        if self.k_max_w == 1:
            self.reduce_max_repeat_width_ksize_one_width(self.ub_a, self.ub_b, self.k_max_h, unit_num * C_ZERO)
        else:
            self.reduce_max_repeat_width_ksize_more_width(self.ub_a, self.ub_b, self.k_max_h, unit_num * C_ZERO)
        # reduce max height
        if self.k_max_h == 1:
            self.reduce_max_repeat_width_ksize_one_height(self.ub_b, self.ub_a, 1, unit_num * C_ZERO)
        else:
            self.reduce_max_repeat_width_ksize_more_height(self.ub_b, self.ub_a, 1, unit_num * C_ZERO)
        # move out
        offset_out = (core_idx * self.one_core_unit + loop_idx * self.max_unit) * C_ZERO
    ...
    def move_gm_to_ub_one_window(self, ub_dst, base_offset, ub_offset, h_idx, w_idx, win_num):
        ...
        with self.tik_instance.if_scope(dst_gap <= TIK_MOVE_STRIDE_MAX and src_gap <= TIK_MOVE_STRIDE_MAX):
        ...
        with self.tik_instance.else_scope():
            with self.tik_instance.for_range(0, k_size_h) as h_loop:
                src_offset = base_offset + h_loop * self.input_w * C_ZERO
                dst_offset = ub_offset + h_loop * (win_num * self.k_max_w) * C_ZERO
    ...
    def tiling_unit_output_w_process(self, ub_x, ub_y, core_idx, loop_idx, unit_num):
        ...
        in_offset = loop_plane_start_idx * self.input_h * self.input_w * C_ZERO + (
                loop_k_start_h * self.input_w) * C_ZERO
        ...
        # dup all
        size = unit_num * self.k_max_h * self.output_w * self.k_max_w * C_ZERO
        repeat_times = size // MASK
        repeat_left = size % MASK
        if repeat_times > 0:
            self.tik_instance.vector_dup(MASK, ub_x, MIN_FP16, repeat_times, 1, 8)
        if repeat_left > 0:
            offset = repeat_times * MASK
            self.tik_instance.vector_dup(repeat_left, ub_x[offset], MIN_FP16, 1, 1, 8)
        ...
        with self.tik_instance.for_range(0, unit_num) as unit_idx:
            ...
            dst_offset_h = (unit_idx * self.output_w * self.k_max_h * self.k_max_w) * C_ZERO
        ...
        # reduce max height
        if self.k_max_h == 1:
            self.reduce_max_repeat_width_ksize_one_height(ub_y, ub_x, unit_num, self.output_w * C_ZERO)
        else:
            self.reduce_max_repeat_width_ksize_more_height(ub_y, ub_x, unit_num, self.output_w * C_ZERO)
        # # move out
        offset_out = (core_idx * self.one_core_unit + loop_idx * self.max_unit) * self.output_w * C_ZERO
    ...
    def move_ub_to_ub_one_window_before_pool(self, ub_dst, ub_src, src_base_offset, dst_base_offset, h_idx, w_idx):
        ...
        src_offset = src_base_offset + k_start_w * C_ZERO
        src_gap = self.input_w - k_size_w
        dst_offset = dst_base_offset + w_idx * self.k_max_w * C_ZERO
        dst_gap = self.output_w * self.k_max_w - k_size_w

        with self.tik_instance.if_scope(dst_gap <= TIK_MOVE_STRIDE_MAX):
            self.tik_instance.data_move(ub_dst[dst_offset], ub_src[src_offset], 0,
                                        k_size_h, k_size_w,
                                        src_gap, dst_gap)
        with self.tik_instance.else_scope():
            with self.tik_instance.for_range(0, k_size_h) as h_loop:
                loop_src_offset = src_offset + h_loop * self.input_w * C_ZERO
                loop_dst_offset = dst_offset + h_loop * (self.output_w * k_size_w) * C_ZERO
    ...
    def reduce_max_repeat_width_ksize_more_width(self, ub_x, ub_y, repeat_ph, size_ow):
        ...
        """
        self.reduce_max_repeat_width(ub_y, ub_x, ub_x[C_ZERO:], repeat_ph * size_ow, self.k_max_w, self.k_max_w)
    ...
    def reduce_max_repeat_width(self, dst, src0, src1, size, src0_blk=1, src1_blk=1):
        ...
        if max(src0_blk, src1_blk) * 8 <= STRIDE_LIMIT:
            if size > 0:
                size_loop = size // MASK
                size_left = size % MASK
    ...
    def copy_only_process(self, core_idx, loop_idx, block_num):
        """Only execute move in and move out
        """
        offset = (core_idx * self.one_core_unit + loop_idx * self.max_unit) * C_ZERO
    ...
    def check_param(self):
        ...
        if list(self.y_shape) != list(self.x_shape[:2]) + list(self.output_size) + list([self.x_shape[4]]):
