...
class MaxpoolGradBase():
    ...
    def tik_instance_cut_nc1_cut_h(self, kernel_name):
        ...
        data_mask = self.tik_instance.Tensor(
            "uint16",
            (batch * channel1 * kernelh * kernelw * mask_one_window,),
            name="data_mask", scope=tik.scope_gm)
        data_output = self.tik_instance.Tensor(
            dtype, self.y_shape, name="data_output", scope=tik.scope_gm)

        data_input_origin = self.tik_instance.Tensor(
            dtype, self.y_shape, name="data_input_origin", scope=tik.scope_gm)

        real_block, block_cycle, block_index = self.get_block_param(block)

        with self.tik_instance.for_range(0, real_block, block_num=real_block) \
                as block_id:  # 1
            ...
            with self.tik_instance.for_range(0, real_cycle) as cycle_id:  # 1
                ...
                data_vsel_ub_zero = self.tik_instance.Tensor(
                    dtype, (128,), name="data_vsel_ub_zero",
                    scope=tik.scope_ubuf)

                # data move only 16 fp16 every time
                self.tik_instance.data_move(
                    data_vsel_ub_zero[0], data_input_origin[0],
                ...
                with self.tik_instance.for_range(0, nc1) as loopc1:  # 1
                    ...
                    data_max_ub = self.tik_instance.Tensor(
                        dtype, (ho_max * wo_max * channel,),
                        name="data_max_ub", scope=tik.scope_ubuf)
                    # 33x13x16+64
                    data_vmul_ub_col2img_fp32 = \
                        self.tik_instance.Tensor(
                            "float32", (col2img_w * channel * col2img_h + 64,),
                            name="data_vmul_ub_col2img_fp32",
                            scope=tik.scope_ubuf)
                    # 33x13x16+128
                    data_vmul_ub_col2img_fp16 = \
                        self.tik_instance.Tensor(
                            dtype, (col2img_w * channel * col2img_h + 128,),
                            name="data_vmul_ub_col2img_fp16",
                            scope=tik.scope_ubuf)
                    # mask define 6x16
                    data_mask_ub = self.tik_instance.Tensor(
                        "uint16", (ho_max * wo_max,), name="data_mask_ub",
                        scope=tik.scope_ubuf)
                    ...
                    with self.tik_instance.for_range(0, h_cycle) as looph:  # 1
                        ...
                        # 一次搬入
                        self.tik_instance.data_move(
                            data_max_ub, data_input[
                                (block_num * nc1 * dyh + loopc1 * dyh +
                        ...
                        with self.tik_instance.for_range(0,
                            ...
                            data_vsel_ub_fp32 = self.tik_instance.Tensor(
                                "float32", (ho_max * wo_max * channel,),
                                name="data_vsel_ub_fp32", scope=tik.scope_ubuf)
                            if v_rep_time > 0:  # 12
                                with self.tik_instance.for_range(
                                        0, v_rep_time, thread_num=1) as cycle:
                                    # 一次抽取8个mask，用来比较128个数
                                    cmpmask = self.tik_instance. \
                            ...
                            if v_rep_cycle_fp32 > 0:  # 0
                                with self.tik_instance.for_range(
                                        0, v_rep_cycle_fp32, thread_num=1) \
                                        as cycle:
                            ...
                            self.tik_instance.col2img(
                                data_vmul_ub_col2img_fp32[0],  # 33x13x16 + 64
                                data_vsel_ub_fp32[0],  # 16x6x16
                                (0, 0, 0, 0), col2img_h, col2img_w,
                                fetch_filter_w, fetch_filter_h,
                                left_top_w, left_top_h, stridew, strideh,
                                kernelw, kernelh, self.dilation[1],
                                self.dilation[2], ho_max * wo_max // 16)

                        # convert fp32 to fp16
                        if v_rep_cycle_col > 0:  # 0
                            with self.tik_instance.for_range(
                                    0, v_rep_cycle_col) as cycle:
                                self.tik_instance.vconv(
                                    constant.MASK64, "",
                                    data_vmul_ub_col2img_fp16[
                                        cycle * V_MAX_REPEAT * FP32_MAX],
                        ...
                        if hoverlap == 0:
                            ...
                            with self.tik_instance.if_scope(looph == 0):
                                src_address.set_as(src_address + pad_top *
                                                   col2img_w * channel)
                                output_cuthline.set_as(
                                    output_cuthline - pad_top)
                                with self.tik_instance.if_scope(
                                        looph == h_cycle - 1):
                            ...
                            with self.tik_instance.else_scope():
                                with self.tik_instance.if_scope(
                                        looph == h_cycle - 1):
                                    output_cuthline.set_as(dxh - col2img_h *
                                                           (h_cycle - 1) +
                                                           pad_top)
                        else:
                            with self.tik_instance.if_scope(looph == 0):
                                output_cuthline.set_as(
                                    strideh * ho_max - pad_top)
                                src_address.set_as(
                                    src_address + pad_top *
                                    col2img_w * channel)
                                with self.tik_instance.if_scope(
                        ...
                        idx = block_num * nc1 * dxh * dxw * channel + \
                              loopc1 * dxh * dxw * channel + dxh_address_offset
    ...
    def tik_instance_cut_nc1h_cut_h(self, kernel_name):
        ...
        if hoverlap == 0:
            ...
            h_cycle_every = (ho_every - 1 - ho_max_every) // (
                    ho_max_every - 1) + 2
            h_cycle_last = (ho_last - 1 - ho_max_last) // (ho_max_last - 1) + 2

        mask_one_window = ((dyh * dyw + 15) // 16 + 1) * 16
        # vector_repeat_time
        v_rep_time_last = ho_max_last * wo_max * channel * dtype_size // \
                          ONE_REPEAT
        ...
        v_rep_time_every = ho_max_every * wo_max * channel * dtype_size // \
                           ONE_REPEAT
        ...
        real_block, block_cycle, block_index = \
            self.get_block_param(all_blocknum)
        with self.tik_instance.for_range(
                0, real_block, block_num=real_block) as block_id:  # 32
            real_cycle = self.tik_instance.Scalar("int32")
            block_base = self.tik_instance.Scalar("int32")
            block_num = self.tik_instance.Scalar("int32")
            with self.tik_instance.if_scope(block_id < block_index):
            ...
            with self.tik_instance.for_range(0, real_cycle) as cycle_id:  # 1
                ...
                with self.tik_instance.if_scope(block_h == ho_count - 1):
                    data_max_ub = self.tik_instance.Tensor(
                        dtype, (ho_max_last * wo_max * channel,),
                        name="data_max_ub", scope=tik.scope_ubuf)
                    data_vmul_ub_col2img_fp32 = \
                        self.tik_instance.Tensor(
                            "float32",
                    ...
                    with self.tik_instance.for_range(
                            0, h_cycle_last) as looph:  # 4
                        in_src_address.set_as(
                            block_batch * dyh * dyw * channel)
                        mask_address.set_as(  # 56 x 56 x 3 x 3
                            block_batch * mask_one_window * kernelw * kernelh)
                        if hoverlap == 0:
                            new_looph.set_as(looph * ho_max_last)
                            with self.tik_instance.if_scope(
                                    looph == h_cycle_last - 1):
                                in_nburst.set_as(ho_last - looph * ho_max_last)
                            in_src_address.set_as(
                                in_src_address + (block_h * ho_every +
                            ...
                            with self.tik_instance.if_scope(
                                    looph == h_cycle_last - 1):
                                in_nburst.set_as(
                                    ho_last - looph * (ho_max_last - 1))
                            in_src_address.set_as(
                                in_src_address + (block_h * (ho_every - 1) +
                                                  new_looph) * dyw * channel)
                            mask_address.set_as(
                                mask_address + (block_h * (ho_every - 1) +
                                                new_looph) * dyw)

                        self.clean_fp32_multi_repeat(data_vmul_ub_col2img_fp32,
                                                     dtype_size * 2)
                        self.clean_max_ub(data_max_ub, dtype)
                        ...
                        with self.tik_instance.for_range(
                                0, kernelh * kernelw) as mask_id:  # 3x3
                            with self.tik_instance.for_range(
                                    0, in_nburst) as cycle:  # 7
                                # mask copy gm to ub
                                self.tik_instance.data_move(
                                    data_mask_ub[cycle * wo_max],
                            ...
                            if v_rep_last_fp32_last != 0:  # 112
                                self.tik_instance.vconv(
                                    constant.MASK64, "",
                                    data_vsel_ub_fp32[
                                        v_rep_cycle_fp32_last * V_MAX_REPEAT *
                                        FP32_MAX],
                                    data_vsel_ub[
                                        v_rep_cycle_fp32_last * V_MAX_REPEAT *
                                        FP32_MAX],
                        ...
                        if v_rep_cycle_col_last > 0:  # 0
                            with self.tik_instance.for_range(
                                    0, v_rep_cycle_col_last) as cycle:
                                self.tik_instance.vconv(
                                    constant.MASK64, "",
                                    data_vmul_ub_col2img_fp16[
                                        cycle * V_MAX_REPEAT * FP32_MAX],
                        ...
                        if hoverlap == 0:
                            ...
                            dst_address.set_as(
                                dst_address + (ho_count - 1) * ho_every *
                                strideh * dxw * channel + looph *
                                ho_max_last * strideh * dxw * channel -
                                pad_top * dxw * channel)
                            with self.tik_instance.if_scope(
                                    looph == h_cycle_last - 1):
                                output_cuthline.set_as(
                                    dxh - ho_every * (ho_count - 1) * strideh -
                                    looph * ho_max_last * strideh + pad_top)
                        else:  # 1
                            src_address.set_as(
                                pad_left * channel + strideh *
                                col2img_w * channel)
                            output_cuthline.set_as((ho_max_last - 1) * strideh)
                            with self.tik_instance.if_scope(
                                    looph == h_cycle_last - 1):
                                output_cuthline.set_as(
                                    (ho_last - looph * (ho_max_last - 1) - 1) *
                                    strideh + kernelh - strideh -
                                    pad_bottom + 1)
                            dst_address.set_as(
                                dst_address +
                                ((block_h * (ho_every - 1) +
                        ...
                        self.tik_instance.data_move(data_output[dst_address],
                                                    ...
                                                    constant.SID,
                                                    output_cuthline,
                                                    self.offset_w,  # 12 x 112
                                                    col2img_w - self.offset_w,
                                                    # 17
                                                    dxw - self.offset_w)  # 0
                with self.tik_instance.else_scope():
                    data_max_ub = self.tik_instance.Tensor(
                        dtype, (ho_max_every * wo_max * channel,),
                        name="data_max_ub", scope=tik.scope_ubuf)
                    data_vmul_ub_col2img_fp32 = \
                        self.tik_instance.Tensor(
                            "float32",
                    ...
                    with self.tik_instance.for_range(
                        ...
                        if hoverlap == 0:
                            in_src_address.set_as(
                                ...
                                dyw * channel)
                            mask_address.set_as(
                                mask_address + (block_h * ho_every +
                                                looph * ho_max_every) * dyw)
                            with self.tik_instance.if_scope(
                                    looph == h_cycle_every - 1):
                                in_nburst.set_as(
                                    ho_every - looph * ho_max_every)
                        else:
                            in_src_address.set_as(
                                in_src_address +
                                (block_h * (ho_every - 1) +
                        ...
                        with self.tik_instance.for_range(
                                0, kernelh * kernelw) as mask_id:  # 3 x 3
                            with self.tik_instance.for_range(
                                    0, in_nburst) as cycle:  # 2
                                # mask copy gm to ub
                                self.tik_instance.data_move(
                                    data_mask_ub[cycle * wo_max],
                                    data_mask[mask_address + cycle * dyw +
                            ...
                            if v_rep_time_every > 0:  # 16
                                with self.tik_instance.for_range(
                                        0, v_rep_time_every, thread_num=1) \
                                        as cycle:
                                    cmpmask = \
                                        self.tik_instance. \
                                            mov_tensor_to_cmpmask(
                        ...
                        if v_rep_cycle_col_every > 0:  # 0
                            with self.tik_instance.for_range(
                                    0, v_rep_cycle_col_every) as cycle:
                                self.tik_instance.vconv(
                                    constant.MASK64, "",
                                    data_vmul_ub_col2img_fp16[
                                        cycle * V_MAX_REPEAT * FP32_MAX],
                        ...
                        if hoverlap == 0:
                            ...
                            with self.tik_instance.if_scope(block_h == 0):
                                with self.tik_instance.if_scope(looph == 0):
                                    ...
                                    src_address.set_as(pad_left * channel +
                                                       ...
                                                       channel)
                                    dst_address.set_as(
                                        block_batch * dxh * dxw * channel)
                            with self.tik_instance.if_scope(
                                    looph == h_cycle_every - 1):
                                output_cuthline.set_as(
                                    ho_every * strideh - col2img_h_every *
                                    (h_cycle_every - 1))
                        else:
                            src_address.set_as(
                                pad_left * channel + strideh * col2img_w *
                                channel)
                            output_cuthline.set_as(
                                (ho_max_every - 1) * strideh)
                            with self.tik_instance.if_scope(
                                    looph == h_cycle_every - 1):
                                output_cuthline.set_as(
                                    (ho_every - looph *
                            ...
                            dst_address.set_as(
                                dst_address + ((block_h * (ho_every - 1) +
                                                looph * (ho_max_every - 1) -
                                                pad_top + 1) * strideh + 1) *
                                dxw * channel)
    ...
    def clean_gm(self, dtype, num, start_index, data_output):
        ...
        if clear_cycle != 0:
            data_ub_zero_256 = self.tik_instance.Tensor(
                dtype, (256,), name="data_ub_zero_256", scope=tik.scope_ubuf)
            self.clean_fp16_one_repeat(data_ub_zero_256, dtype)
            with self.tik_instance.for_range(0, clear_cycle) as cycle:
                self.tik_instance.data_move(
                    data_output[start_index + 256 * cycle],
                    data_ub_zero_256[0],
                    constant.SID, constant.DEFAULT_NBURST,
                    16,
                    constant.STRIDE_ZERO,
    ...
    def clean_max_ub(self, data_max_ub, dtype):
        ...
        if repeat_time_num > 255:
            with self.tik_instance.for_range(0,
                                             repeat_time_num) as repeat_index:
    ...
    def clean_fp32_multi_repeat(self, data_vmul_ub_col2img_fp32, dtype_size):
        ...
        The fun just for clean ub
        """
        v_rep_clear_time = data_vmul_ub_col2img_fp32.shape[
                               0] * dtype_size // ONE_REPEAT
        ...
        if v_rep_clear_cycle > 0:
            with self.tik_instance.for_range(0, v_rep_clear_cycle,
                                             thread_num=1) as cycle:
