...
@para_check.check_op_params(para_check.OPTION_INPUT, para_check.REQUIRED_INPUT,
                            ...
                            para_check.OPTION_ATTR_LIST_FLOAT, para_check.OPTION_ATTR_LIST_BOOL,
...
def stn_pre(theta, w_index, h_index, pos_coef, pos_offset,
    ...
    para_check.check_shape(shape, param_name="theta")
...
class SpatialTransformer:
    ...
    def __init__(self, theta, w_index, h_index, pos_coef,
        ...
        self.theta_type_bytes_size = tbe_platform.get_bit_len(theta.get('dtype')) // 8
        self.h_w_index_type_byte_size = tbe_platform.get_bit_len(w_index.get('dtype')) // 8
        ...
        self.theta_gm = self.tik_instance.Tensor(
            ...
            name='theta_gm', scope=tbe_platform.scope_gm
        )
        # input w_index gm
        self.w_index_gm = self.tik_instance.Tensor(
            w_index.get('dtype'), (size[2] * size[3] + 512,), name='w_index_gm', scope=tbe_platform.scope_gm
        )
        # input w_index gm
        self.h_index_gm = self.tik_instance.Tensor(
            h_index.get('dtype'), (size[2] * size[3] + 512,), name='h_index_gm', scope=tbe_platform.scope_gm
        )
        ...
        self.output_pos_coef_gm = self.tik_instance.Tensor(
            ...
            name='output_pos_coef_gm', scope=tbe_platform.scope_gm
        )
        # output pos_offset
        self.output_pos_offset_gm = self.tik_instance.Tensor(
            ...
            name='output_pos_offset_gm', scope=tbe_platform.scope_gm
        )
    ...
    def process_on_each_core(self, batch_id, each_batch_loop_count):
        ...
        with self.tik_instance.for_range(0, each_batch_loop_count, thread_num=thread_num) as hw_index_handle_count:
            ...
            w_index_ub = self.tik_instance.Tensor(
                self.w_index.get('dtype'), (self.w_index_ub_size,), name='w_index_ub', scope=tbe_platform.scope_ubuf
            )
            ...
            h_index_ub = self.tik_instance.Tensor(
                self.h_index.get('dtype'), (self.w_index_ub_size,), name='h_index_ub', scope=tbe_platform.scope_ubuf
            )
            self.tik_instance.data_move(
                dst=h_index_ub,
            ...
            if self.calc_by_fp16 or self.w_index.get('dtype') == 'float32':
                ...
                w_index_ub_fp32 = self.tik_instance.Tensor(
                    'float32', (self.w_index_ub_size,), name='w_index_ub_fp32', scope=tbe_platform.scope_ubuf
                )
            ...
            with self.tik_instance.new_stmt_scope():
                ceof_res_ub, offset_res_ub = self.merge_res(ch_cw_res_ceof_ub, ch_cw_res_offset_ub,
                                                            ch_fw_res_ceof_ub, ch_fw_res_offset_ub,
                                                            fh_cw_res_ceof_ub, fh_cw_res_offset_ub,
                ...
                if self.if_skip_read_ceof:
                    ...
                    if burst_len > 1 and tail_num > 0:
                        ...
                        block_ub = self.tik_instance.Tensor(
                            self.pos_coef.get('dtype'), (block_num_size,), name='block_ub',
                    ...
                    if burst_len > 1 and tail_num > 0:
                        block_int32_ub = self.tik_instance.Tensor(
                            'int32', (8,), name='block_int32_ub', scope=tbe_platform.scope_ubuf
                        )
    ...
    def merge_res(self, ch_cw_res_ceof_ub, ch_cw_res_offset_ub, ch_fw_res_ceof_ub, ch_fw_res_offset_ub,
        ...
        ceof_res_ub = self.tik_instance.Tensor(
            self.pos_coef.get('dtype'), (128 * 4,), name='ceof_res_ub', scope=tbe_platform.scope_ubuf
        )
        if self.pos_coef.get('dtype') == 'float16':
            ...
            fh_fw_res_ceof_ub32 = self.tik_instance.Tensor(
                'float32', (128,), name='fh_fw_res_ceof_ub32', scope=tbe_platform.scope_ubuf
            )
            ...
            ch_fw_res_ceof_ub32 = self.tik_instance.Tensor(
                'float32', (128,), name='ch_fw_res_ceof_ub32', scope=tbe_platform.scope_ubuf
            )
    ...
    def calc_ceof_and_offset(self, h_index_ub, theta1_1, theta1_2, theta1_3, theta2_1,
        ...
        if not self.calc_by_fp16:
            origin_w_or_res_ub16 = self.tik_instance.Tensor(
                "float16", (self.w_index_ub_size,), name='origin_w_or_res_ub16', scope=tbe_platform.scope_ubuf
            )
            self.tik_instance.vec_conv(64, '', origin_w_or_res_ub16, origin_w_or_res_ub, 2, 4, 8)
            origin_h_res_ub16 = self.tik_instance.Tensor(
                "float16", (self.w_index_ub_size,), name='origin_h_res_ub16', scope=tbe_platform.scope_ubuf
            )
    ...
    def mian_calc_ceof_offset(self, origin_h_or_res_ub, origin_w_or_res_ub, origin_h_res_ub16, origin_w_res_ub16):
        ...
        fh_fw_res_ceof_ub = self.tik_instance.Tensor(
            'float16', (128,), name='tmp_res_ceof_ub', scope=tbe_platform.scope_ubuf
        )
        if self.calc_by_fp16:
            self.calc_ceof(floor_w_ub, origin_h_res_ub16, origin_w_res_ub16, floor_h_fp16_ub, fh_fw_res_ceof_ub)
        else:
            self.calc_ceof_by_fp32(floor_w_ub, origin_h_or_res_ub, origin_w_or_res_ub, floor_h_fp16_ub,
                                   fh_fw_res_ceof_ub)
        fh_fw_res_offset_ub = self.tik_instance.Tensor(
            'int32', (128,), name='fh_fw_res_offset_ub', scope=tbe_platform.scope_ubuf
        )
        int32_16_ub = self.tik_instance.Tensor(
            'int32', (128,), name='int32_16_ub', scope=tbe_platform.scope_ubuf
        )
        with self.tik_instance.new_stmt_scope():
            tmp_fh_fw_res_offset_ub = self.tik_instance.Tensor(
                'int32', (128,), name='tmp_fh_fw_res_offset_ub', scope=tbe_platform.scope_ubuf
            )
            ...
            self.tik_instance.vec_add(64, fh_fw_res_offset_ub, fh_fw_res_offset_ub, tmp_fh_fw_res_offset_ub,
                                      2, 8, 8, 8)
        # filter ceof and offset
        float_filter_ub = self.tik_instance.Tensor(
            "float16", (128,), name='float_filter_ub', scope=tbe_platform.scope_ubuf
        )
        self.tik_instance.vec_dup(128, float_filter_ub, 0.0, 1, 1)
        filter_index = self.tik_instance.Tensor(
            'uint16', (8,), name='filter_index', scope=tbe_platform.scope_ubuf
        )
        ...
        fh_cw_res_ceof_ub = self.tik_instance.Tensor(
            'float16', (128,), name='fh_cw_res_ceof_ub', scope=tbe_platform.scope_ubuf
        )
        if self.calc_by_fp16:
            self.calc_ceof(ceil_w_ub, origin_h_res_ub16, origin_w_res_ub16, floor_h_fp16_ub, fh_cw_res_ceof_ub)
        else:
            self.calc_ceof_by_fp32(ceil_w_ub, origin_h_or_res_ub, origin_w_or_res_ub, floor_h_fp16_ub,
                                   fh_cw_res_ceof_ub)
        fh_cw_res_offset_ub = self.tik_instance.Tensor(
            'int32', (128,), name='fh_cw_res_offset_ub', scope=tbe_platform.scope_ubuf
        )
        with self.tik_instance.new_stmt_scope():
            tmp_fh_cw_res_offset_ub = self.tik_instance.Tensor(
                'int32', (128,), name='tmp_fh_cw_res_offset_ub', scope=tbe_platform.scope_ubuf
            )
            ...
            self.tik_instance.vec_add(64, fh_cw_res_offset_ub, fh_cw_res_offset_ub, tmp_fh_cw_res_offset_ub,
                                      2, 8, 8, 8)
        self.tik_instance.vec_dup(128, float_filter_ub, 0.0, 1, 1)
        ...
        ch_fw_res_ceof_ub = self.tik_instance.Tensor(
            'float16', (128,), name='ch_fw_res_ceof_ub', scope=tbe_platform.scope_ubuf
        )
        if self.calc_by_fp16:
        ...
        ch_fw_res_offset_ub = self.tik_instance.Tensor(
            'int32', (128,), name='ch_fw_res_offset_ub', scope=tbe_platform.scope_ubuf
        )
        with self.tik_instance.new_stmt_scope():
            tmp_ch_fw_res_offset_ub = self.tik_instance.Tensor(
                'int32', (128,), name='tmp_ch_fw_res_offset_ub', scope=tbe_platform.scope_ubuf
            )
            ...
            self.tik_instance.vec_add(64, ch_fw_res_offset_ub, ch_fw_res_offset_ub, tmp_ch_fw_res_offset_ub,
                                      2, 8, 8, 8)
        ...
        ch_cw_res_ceof_ub = self.tik_instance.Tensor(
            'float16', (128,), name='ch_cw_res_ceof_ub', scope=tbe_platform.scope_ubuf
        )
        if self.calc_by_fp16:
        ...
        ch_cw_res_offset_ub = self.tik_instance.Tensor(
            'int32', (128,), name='ch_cw_res_offset_ub', scope=tbe_platform.scope_ubuf
        )
        with self.tik_instance.new_stmt_scope():
            tmp_ch_cw_res_offset_ub = self.tik_instance.Tensor(
                'int32', (128,), name='tmp_ch_cw_res_offset_ub', scope=tbe_platform.scope_ubuf
            )
            ...
            self.tik_instance.vec_add(64, ch_cw_res_offset_ub, ch_cw_res_offset_ub, tmp_ch_cw_res_offset_ub,
                                      2, 8, 8, 8)
        ...
        ch_fw_ceof_ub = self.tik_instance.Tensor(
            'float16', (128,), name='ch_fw_ceof_ub', scope=tbe_platform.scope_ubuf
        )
        ...
        ch_cw_ceof_1_ub = self.tik_instance.Tensor(
            'float16', (128,), name='ch_cw_ceof_1_ub', scope=tbe_platform.scope_ubuf
        )
        ...
        fh_cw_ceof_ub = self.tik_instance.Tensor(
            'float16', (128,), name='fh_cw_ceof_ub', scope=tbe_platform.scope_ubuf
        )
    ...
    def calc_ceil_w(self, origin_w_or_res_ub):
        ...
        if self.soc_version in ("Hi3796CV300CS", "SD3403"):
            ...
            self.tik_instance.vec_conv(128, '', ceil_w_ub, ceil_w_ub_int16, 1, 8, 8)
        else:
    ...
    def calc_ceil_h(self, origin_h_or_res_ub):
        ...
        if self.soc_version in ("Hi3796CV300CS", "SD3403"):
            ...
            self.tik_instance.vec_conv(128, '', ceil_h_ub, ceil_h_ub_int16, 1, 8, 8)
        else:
    ...
    def calc_floor_w(self, origin_w_or_res_ub):
        ...
        if self.soc_version in ("Hi3796CV300CS", "SD3403"):
            ...
            self.tik_instance.vec_conv(128, '', floor_w_ub, floor_w_ub_int16, 1, 8, 8)
        else:
    ...
    def calc_floor_h(self, origin_h_or_res_ub):
        ...
        if self.soc_version in ("Hi3796CV300CS", "SD3403"):
            ...
            self.tik_instance.vec_conv(128, '', floor_h_fp16_ub, floor_h_ub_int16, 1, 8, 8)
        else:
    ...
    def calc_origin_w(self, h_index_ub, origin_w_ub, theta2_1, theta2_2, theta2_3, w_index_ub):
        ...
        if self.calc_by_fp16:
            ...
            origin_w_or_res_ub = self.tik_instance.Tensor(
                "float32", (self.w_index_ub_size,), name='origin_w_or_res_ub', scope=tbe_platform.scope_ubuf
            )
            repeat = 2
            mask = 64
    ...
    def calc_origin_h(self, h_index_ub, theta1_1, theta1_2, theta1_3, w_index_ub):
        ...
        if self.calc_by_fp16:
            ...
            origin_w_ub = self.tik_instance.Tensor(
                'float16', (self.w_index_ub_size,), name='origin_w_ub', scope=tbe_platform.scope_ubuf
            )
            repeat = 1
            mask = 128
        else:
            origin_h_or_res_ub = self.tik_instance.Tensor(
                "float32", (self.w_index_ub_size,), name='origin_h_or_res_ub', scope=tbe_platform.scope_ubuf
            )
            origin_w_ub = self.tik_instance.Tensor(
                "float32", (self.w_index_ub_size,), name='origin_w_ub', scope=tbe_platform.scope_ubuf
            )
            repeat = 2
            mask = 64
    ...
    def calc_theta(self, batch_id, d_type):
        ...
        if self.use_default_theta.count(True) != 6:
            ...
            if not self.calc_by_fp16 and self.theta.get('dtype') == 'float16':
                ...
                # get theta
                theta1_1, theta1_2, theta1_3, theta2_1, theta2_2, theta2_3 = \
                    self.get_theta_by_mix(d_type, input_theta_ub_fp32)
            else:
                theta1_1, theta1_2, theta1_3, theta2_1, theta2_2, theta2_3 = \
                    self.get_theta_by_mix(d_type, input_theta_ub)
        else:
    ...
    def filter_coef(self, tmp_res_ceof_ub, float_filter_ub, floor_h_ub, floor_w_ub, filter_index, res_offset):
        ...
        with self.tik_instance.new_stmt_scope():
            ...
            tmp_res_ceof_1_ub = self.tik_instance.Tensor(
                'float16', (128,), name='tmp_res_ceof_1_ub', scope=tbe_platform.scope_ubuf
            )
            ...
            tmp_1_ub = self.tik_instance.Tensor(
                'float16', (128,), name='tmp_1_ub', scope=tbe_platform.scope_ubuf
            )
            offset_mul = self.tik_instance.Tensor(
                'float16', (128,), name='offset_mul', scope=tbe_platform.scope_ubuf
            )
            offset_mul_int32 = self.tik_instance.Tensor(
                'int32', (128,), name='offset_mul_int32', scope=tbe_platform.scope_ubuf
            )
    ...
    def calc_ceof_by_fp32(self, w_ub, origin_h_or_res_ub, origin_w_or_res_ub, h_ub, tmp_res_ceof_ub):
        ...
        with self.tik_instance.new_stmt_scope():
            res = self.tik_instance.Tensor(
                'float32', (128,), name='res', scope=tbe_platform.scope_ubuf
            )
            w_ub_fp32 = self.tik_instance.Tensor(
                'float32', (128,), name='w_ub_fp32', scope=tbe_platform.scope_ubuf
            )
            h_ub_fp32 = self.tik_instance.Tensor(
                'float32', (128,), name='h_ub_fp32', scope=tbe_platform.scope_ubuf
            )
            ...
            tmp_w_ceof_ub = self.tik_instance.Tensor(
                'float32', (128,), name='tmp_w_ceof_ub', scope=tbe_platform.scope_ubuf
            )
    ...
    def calc_ceof(self, w_ub, origin_h_or_res_ub, origin_w_or_res_ub, h_ub, tmp_res_ceof_ub):
        ...
        tmp_w_ceof_ub = self.tik_instance.Tensor(
            'float16', (128,), name='tmp_w_ceof_ub', scope=tbe_platform.scope_ubuf
        )
