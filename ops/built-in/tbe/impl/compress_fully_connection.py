...
def fully_connection_check_rule(x, w, compress_index, b, offset_w, y,
    ...
    if dtype_x == 'float16' and (shape_w[2] != 16 or shape_w[3] != 16):
        err_man_cube.raise_err_three_paras("E61300", 
                                           "compress_fully_connection",
                                           "w last two dims", "16", 
                                           "for no quant")
    ...
    if km_shape != kn_shape:
        err_man_cube.raise_err_four_paras("E80017", "fully_connection", "km_shape",
                                          "kn_shape", km_shape, kn_shape)
    ...
    if b is not None:
        ...
        if b_size != n_shape:
            err_man_cube.raise_err_two_paras("compress_fully_connection", "the C1*C0", "aligned_Cout")
...
def compress_fully_connection_compute(
    ...
    if offset_w is not None:
        err_man_cube.raise_err_specific("compress_fully_connection", "tensor offset_w must be None!")
...
def compress_fully_connection(x, w, compress_index, b, offset_w, y,
    ...
    if offset_w is None:
        ...
        err_man_cube.raise_err_specific("compress_fully_connection", "offset_w must be None!")
