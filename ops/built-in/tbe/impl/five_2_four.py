...
def _get_param_more_dim(tvm_ib, src_shape, dtype, max_dim, shape_all, is_zn2nchw=0):
    ...
    ub_bytes = ((Constant.UB_SIZE_B - 32) // 2)
    if shape_all > 380000000:
        device_core_num = 1
        ub_bytes = ((Constant.UB_SIZE_B // 2 - 32) // 2)
...
def _get_param_split_dim(tvm_ib, src_shape, dtype, max_dim, shape_all, is_zn2nchw=0):
    ...
    float_size = cce.cce_intrin.get_bit_len(dtype) // 8
    cp_align_len = cce_params.BLOCK_REDUCE_INT8 // float_size
    ub_bytes = Constant.UB_SIZE_B - 64 - cp_align_len * 32 * 2
    device_core_num = Constant.AICORE_NUM
    if shape_all > 380000000:
        device_core_num = 1
        ub_bytes = Constant.UB_SIZE_B // 2 - 64 - (cp_align_len * 32 * 2)
    elif device_core_num == 32 and (max_dim > 800000 or shape_all > 320000000):
        device_core_num = 16
    ub_ele = ub_bytes // float_size
...
def _func_split_dim(args):
    ...
    num_row_before_core = \
        num_g * param.get("num_row_one_group") + param.get("block_index") * param.get("num_row_one_core")
...
def _get_param_more_dim_fp16(tvm_ib, src_shape, dst_shape, dtype,
    ...
    if shape_all > 380000000 and \
        ...
        device_core_num = 1
        ub_bytes = Constant.UB_SIZE_B // 2 - (cp_align_len * 32)
    elif device_core_num == 32 and (max_dim > 800000 or shape_all > 320000000):
        device_core_num = 16
    ub_half = ub_bytes // 2
    c_0 = 16
...
def _get_param_more_row_nhwc(tvm_ib, dst_shape, dtype, max_dim, shape_all):
    ...
    cp_align_len = cce_params.BLOCK_REDUCE_INT8 // float_size
    device_core_num = Constant.AICORE_NUM
    ub_bytes = (Constant.UB_SIZE_B - 64) // 10 * 4
    if shape_all > 380000000:
        device_core_num = 1
        ub_bytes = (((Constant.UB_SIZE_B // 2) - 64)) // 10 * 4
    elif device_core_num == 32 and (max_dim > 800000 or shape_all > 320000000):
        device_core_num = 16
...
def _more_ub_ir_nhwc(output_data, input_data, base_shape, core_num):
    ...
    tvm_ib = tvm.ir_builder.create()
    num_bit = 4
    ub_maximum_size = Constant.UB_SIZE_B // 2
...
def _check_ci_align_nhwc_fencore(src_shape, dst_shape, dst_format, dtype):
    ...
    if col_len_shape > col_len_ub and n_i < Constant.AICORE_NUM:
        return True
...
def _check_16_16_64_64(dst_shape, dst_format, dtype):
    ...
    if dst_shape not in ([16, 16, 64, 64], [16, 32, 64, 64]):
        return False
    ...
    if dim_ele > cp_align_len \
            and new_space_split_col_fp32 <= ub_ele < space_ele:
        return True
    ...
    if one_unit_ele <= ub_ele:
        ...
        if hw_i <= hw_ub:
            ...
            if num_dim_group <= n_i and num_dim_ub > 1:
                return "little"
            return "small"
        if hw_i > hw_ub and n_i < device_core_num:
...
def five_2_four(src, dst, src_format, dst_format, kernel_name='five_2_four'):
    ...
    elif dst_format.lower() == "nchw" and dtype == "float32" and dst_shape in [[200, 1, 10, 26]] and \
            cce.cce_conf.intrinsic_check_support("Intrinsic_vnchwconv", "float32"):
        trans_data_negative_target_ntc.trans_data_negative_target_ntc(src, dst, src_format, dst_format, kernel_name)
    # Notes: vbi only supported by 1951
            ...
            src_shape in [[1, 4, 65, 65, 16], [1, 8, 33, 33, 16], [1, 15, 17, 17, 16], [1, 1, 65, 65, 16],
                          ...
                          [8, 4, 65, 65, 16], [8, 8, 33, 33, 16], [8, 15, 17, 17, 16], [8, 1, 65, 65, 16],
                          [1, 3, 53, 90, 16]] and \
            dst_shape in [[1, 58, 65, 65], [1, 116, 33, 33], [1, 232, 17, 17], [1, 2, 65, 65],
                          ...
                          [8, 58, 65, 65], [8, 116, 33, 33], [8, 232, 17, 17], [8, 2, 65, 65], [1, 39, 53, 90]]:
