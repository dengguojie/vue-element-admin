...
class InitShape:
    ...
    def __init__(self, n_x):
        self.n_x = n_x
        # number of 16*16
        self.n_maxtrix = int_ceil_div(n_x * Constant.EIGHT, Constant.MATRIX)
        # shape of calculate
...
def box_coordinate_offset_integer_block(tik_instance, shape, mid_tensor, i, j):
    ...
    with tik_instance.if_scope(tik.any((Constant.TEN * i + j == Constant.ONE),
                                       (Constant.TEN * i + j == Constant.THIRTY))):
        # 01 replace by 2*y1 / 30 replace by 2*x1
        tik_instance.vmuls(Constant.SIXTEEN * shape.rep_stride,
                           mid_tensor.data_anchor_x0y0[Constant.BLOCK_DATA * i +
                                                       Constant.SIXTEEN * j],
    ...
    with tik_instance.else_scope():
        with tik_instance.if_scope(tik.any((Constant.TEN * i + j == Constant.TEN),
                                           (Constant.TEN * i + j == Constant.TWENTYONE))):
...
def box_coordinate_offset_remainder_block(tik_instance, shape, mid_tensor, i, j):
    ...
    with tik_instance.if_scope(tik.any((Constant.TEN * i + j == Constant.ONE),
        ...
        tik_instance.vmuls(Constant.MASK,
                           mid_tensor.data_anchor_x0y0[Constant.BLOCK_DATA * i +
                                                       Constant.SIXTEEN * j],
                           mid_tensor.data_y_ub_trs1[Constant.SIXTEEN * j],
                           dump_twice,
        ...
        tik_instance.vmuls(Constant.SIXTEEN * shape.rep_stride,
                           mid_tensor.data_anchor_x0y0[Constant.BLOCK_DATA * i +
                                                       Constant.SIXTEEN * j +
                                                       shape.repeat_whxy *
                                                       Constant.FOUR * Constant.MATRIX],
                           mid_tensor.data_y_ub_trs1[Constant.SIXTEEN * j +
                                                     shape.repeat_whxy *
                                                     Constant.MATRIX_DATA],
                           dump_twice,
                           Constant.ONE,
                           Constant.EIGHT,
                           Constant.FOUR,
...
def calculate_process(tik_instance, gm_tensor, shape,
    ...
    with tik_instance.for_range(0, shape.n_maxtrix) as i:
        tik_instance.vtranspose(mid_tensor.data_x_ub_trs[Constant.MATRIX*i],
                                mid_tensor.data_x_ub[Constant.MATRIX*i])
        tik_instance.vtranspose(mid_tensor.data_y_ub_trs[Constant.MATRIX*i],
                                mid_tensor.data_y_ub[Constant.MATRIX*i])
        ...
        with tik_instance.for_range(0, Constant.FOUR) as i:
            with tik_instance.for_range(0, Constant.TWO) as j:
                tik_instance.vsub(Constant.SIXTEEN * shape.rep_stride,
                                  mid_tensor.data_anchor_wh[Constant.BLOCK_DATA * i +
                                                            Constant.SIXTEEN * j],
                                  mid_tensor.data_y_ub_trs1[Constant.SIXTEEN * j +
                                                            Constant.BLOCK_DATA],
                                  mid_tensor.data_y_ub_trs1[Constant.SIXTEEN * j],
    ...
    # transpose output
    with tik_instance.for_range(0, shape.n_maxtrix) as i:
        tik_instance.vtranspose(mid_tensor.data_z_ub[Constant.MATRIX*i],
                                mid_tensor.data_z_ub1[Constant.MATRIX*i])
    # copy ub to gm
