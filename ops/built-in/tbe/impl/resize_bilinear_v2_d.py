...
def is_dynamic_white_list(ori_format, ori_input_shape, out_size):
    ...
    nchw_input_shape = [
        dict_zip_shape["N"], ((dict_zip_shape["C"] + shape_c0 - 1) // shape_c0) * shape_c0, dict_zip_shape["H"],
        dict_zip_shape["W"]
    ]
...
def check_supported(images, y, size, align_corners=False, half_pixel_centers=False, kernel_name="resize_bilinear_v2"):
    ...
    if len(image_shape) != 4:
        ...
        reason = "the size of image_shape must be 4, len(image_shape)=%s" % len(image_shape)
        return False, reason
...
def _resize_bilinear_ir(inputs, outputs, align_corners, half_pixel_centers):
    ...
    if tbe_platform.cce_conf.intrinsic_check_support("Intrinsic_vconv", "f322s32f"):
        devices = "cloud"
    else:
        devices = "mini"
    ...
    pad_mode = tvm.call_pure_intrin("int32", "tvm_cce_string_print", "PAD_NONE")
    ...
    if h_in == h_out and w_in == w_out:
        ...
        trans_station = apply_store_buffer(ib, dtype, [ub_block * c0], name="trans_station")
        if dtype.lower() == "float16":
            trans_station_fp32 = apply_store_buffer(ib, "float32", [ub_block * c0], name="trans_station_fp32")
        ...
        def _inner_run(core_lp_idx):
            with ib.new_scope():
                ...
                ib.emit(
                    tvm.call_extern(
                        dtype, "copy_gm_to_ubuf", trans_station.access_ptr('w'),
                        inputs.access_ptr('r',
                                          offset=block.var * ub_block * c0 + core_lp_idx * core_counts * ub_block * c0),
                        0, 1, ub_block, 0, 0))
                ...
                with ib.new_scope():
                    ...
                    ib.emit(
                        tvm.call_extern(
                            ...
                            outputs.access_ptr(
                                'w', offset=block.var * ub_block * c0 + core_lp_idx * core_counts * ub_block * c0),
        ...
        if tail_counts > 0:
            with ib.if_scope(block.var < 1):
                ...
                with ib.new_scope():
                    ib.scope_attr(param.CCE_AXIS, "coproc_scope", 5)
                    ib.emit(
                        tvm.call_extern(dtype, "copy_gm_to_ubuf", trans_station.access_ptr('w'),
                                        inputs.access_ptr('r', offset=tail_start * c0), 0, 1, tail_counts, 0, 0))

                if dtype.lower() == "float32":
                    with ib.new_scope():
                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 6)
                        ib.emit(
                            tvm.call_extern(dtype, "copy_ubuf_to_gm", outputs.access_ptr('w', offset=tail_start * c0),
                    ...
                    with ib.new_scope():
                        ...
                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 6)
                        ib.emit(
                            tvm.call_extern("float32", "copy_ubuf_to_gm", outputs.access_ptr('w',
    ...
    # No.2 situation : input H/W = (1,1)
        ...
        if dtype == "float16":
            ...
            f16_in = apply_store_buffer(ib, "float16", [actual_loop * f16_c0], name="f16_in")
            f16_out = apply_store_buffer(ib, "float16", [free_space * f16_c0], name="f16_out")
            f32_out = apply_store_buffer(ib, "float32", [free_space_fp32], name="f32_out")

            if expand_size > HW_SIZE_512:
                f16_8 = apply_store_buffer(ib, "float16", [8 * f16_c0], name="f16_8")
            elif expand_size > 32:
                f16_64 = apply_store_buffer(ib, "float16", [64 * f16_c0], name="f16_64")
            # Note:
            ...
            if expand_size > 32:
                ib.emit(
                    tvm.call_extern("uint64", "set_vector_mask", tvm.const((1 << 64) - 1, dtype="uint64"),
            ...
            if expand_loop > HW_SIZE_512:
                            ...
                            if expand_size > HW_SIZE_512:
                                ...
                                def _inner_run_level3(core_lp3_idx):
                                    ...
                                    with ib.new_scope():
                                        ...
                                        ib.emit(
                                            tvm.call_extern("float16", "vadds", f16_out.access_ptr('w'),
                                                            f16_8.access_ptr('r'), tvm.const(0.0, dtype="float16"),
                                                            repeat_64, 1, 1, 8, 0))
                                    with ib.new_scope():
                                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 2)
                                        ib.emit(
                                            tvm.call_extern("float16", "copy_ubuf_to_ubuf",
                                                            f16_out.access_ptr('w', offset=HW_SIZE_512 * f16_c0),
                                                            f16_out.access_ptr('r'), 0, 1, HW_SIZE_512, 0, 0))
                                    with ib.new_scope():
                                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 2)
                                        ib.emit(
                                            tvm.call_extern("float16", "copy_ubuf_to_ubuf",
                                                            f16_out.access_ptr('w', offset=HW_SIZE_1024 * f16_c0),
                                        ...
                                        with ib.for_range(0, loop_level4) as i5:
                                            with ib.new_scope():
                                                ...
                                                with ib.for_range(0, _inner_loop) as inner_idx:
                                                    ...
                                                    kernel_api.kernel_cast_to_fuc(ib, ip_addr, [free_space_fp32, 8 * 8],
                                                                                  "vconv_f162f32")
                                                    ib.scope_attr(param.CCE_AXIS, "coproc_scope", 6)
                                                    ib.emit(
                                                        tvm.call_extern(
                                                            ...
                                                            outputs.access_ptr(
                                                                'w',
                                                                offset=((i1 * 32 + i2) * HW_SIZE_512 + block.var) *
                                    ...
                                    if tail_level4 > 0:
                                        with ib.new_scope():
                                            ...
                                            if _inner_tail > 1:
                                                ...
                                                kernel_api.kernel_cast_to_fuc(ib, ip_addr, [_inner_tail, 8 * 8],
                                                                              "vconv_f162f32")
                                                ib.scope_attr(param.CCE_AXIS, "coproc_scope", 6)
                                                ib.emit(
                                                    tvm.call_extern(
                                                        ...
                                                        outputs.access_ptr(
                                                            'w',
                                                            offset=(
                                                                (i1 * 32 + i2) * HW_SIZE_512 + block.var) * offset_4 +
                                                            loop_level4 * HW_SIZE_2048 * f16_c0 +
                                                            _inner_loop * free_space_fp32 +
                                                            core_lp3_idx * core_counts * offset_4),
                                                        f32_out.access_ptr('r'), 0, 1, (_inner_tail + 7) // 8, 0, 0))

                                with ib.for_range(0, core_lp3, name="core_lp3") as core_lp3_idx:
                                    _inner_run_level3(core_lp3_idx)
                                if core_lp3_tail > 0:
                                    with ib.if_scope(block.var < core_lp3_tail):
                                ...
                                with ib.for_range(0, loop_level3) as i4:
                                    ...
                                    with ib.for_range(0, loop_level4) as i5:
                                        with ib.new_scope():
                                            ib.scope_attr(param.CCE_AXIS, "coproc_scope", 2)
                                            ib.emit(
                                                tvm.call_extern("float16", "vadds",
                                                                f16_out.access_ptr('w', offset=i5 * offset_l4),
                                                                f16_64.access_ptr('r', offset=i5 * 8 * f16_c0),
                ...
                if tail_level1 > 0:
                    ...
                    if tail_level1 > HW_SIZE_512:
                        ...
                        with ib.new_scope():
                            ib.scope_attr(param.CCE_AXIS, "coproc_scope", 5)
                            ib.emit(
                                tvm.call_extern("float16", "copy_gm_to_ubuf", f16_in.access_ptr('w'),
                                                inputs.access_ptr('r', offset=loop_level1 * l1_half * f16_c0), 0, 1,
                                                tail_level1, 0, 0))
                    # level_1 loop
                    ...
                    if loop_l1 > 0:
                        with ib.for_range(0, loop_l1) as i1:
                            ...
                            if expand_size > HW_SIZE_512:
                                ...
                                with ib.for_range(0, core_lp2, name="core_lp2") as core_lp2_idx:
                                    _inner_run_lp2(core_lp2_idx)
                                if core_lp2_tail > 0:
                                    with ib.if_scope(block.var < core_lp2_tail):
                                        ...
                                        with ib.new_scope():
                                            ...
                                            ib.emit(
                                                tvm.call_extern("float16", "vadds",
                                                                ...
                                                                f16_64.access_ptr('r', offset=i4 * 8 * f16_c0),
                                                                tvm.const(0.0, dtype="float16"), repeat_l3, 1, 1, 8, 0))
                                    offset_4 = expand_size * f16_c0
                                    expand_4 = 8 * expand_size
                                    with ib.new_scope():
                                        _inner_loop = \
                                ...
                                with ib.for_range(0, loop_le1) as i2:
                                    ...
                                    with ib.for_range(0, loop_le2) as i3:
                                        with ib.new_scope():
                                            ...
                                            ib.emit(
                                                tvm.call_extern("float16", "vadds",
                                                                f16_out.access_ptr('w', offset=i3 * offset_le2),
                                                                f16_64.access_ptr('r', offset=i3 * 8 * f16_c0),
                                                                tvm.const(0.0, dtype="float16"), repeat_le2, 1, 1, 8,
                                                                0))
                                    offset_4 = expand_size * f16_c0
                                    expand_4 = 8 * expand_size
                                    with ib.new_scope():
                            ...
                            if tail_le1 > 0:
                                ...
                                with ib.for_range(0, loop_le2) as i4:
                                    with ib.new_scope():
                                        ...
                                        ib.emit(
                                            tvm.call_extern("float16", "vadds",
                                                            f16_out.access_ptr('w', offset=i4 * offset_le2),
                                                            f16_64.access_ptr('r', offset=i4 * 8 * f16_c0),
                ...
                with ib.new_scope():
                    ib.scope_attr(param.CCE_AXIS, "coproc_scope", 5)
                    ib.emit(
                        tvm.call_extern("float16", "copy_gm_to_ubuf", f16_in.access_ptr('w'), inputs.access_ptr('r'), 0,
                                        1, expand_loop, 0, 0))
                # Note:
                #   1. H * W >= HW_SIZE_512 is common
                #   2. HW_SIZE_512 > H * W > 32
                ...
                if expand_size > HW_SIZE_512:
                    ...
                    def _inner_run_lev1(core_lp1_idx):
                        ...
                        with ib.new_scope():
                            ib.scope_attr(param.CCE_AXIS, "coproc_scope", 2)
                            ib.emit(
                                tvm.call_extern("float16", "vadds", f16_out.access_ptr('w'), f16_8.access_ptr('r'),
                    ...
                    # level_1 loop
                    with ib.for_range(0, core_lp1, name="core_lp1") as core_lp1_idx:
                        ...
                        with ib.for_range(0, loop_lev1) as i2:
                            ...
                            with ib.for_range(0, loop_lev2) as i3:
                                with ib.new_scope():
                                    ...
                                    ib.emit(
                                        tvm.call_extern("float16", "vadds",
                                                        f16_out.access_ptr('w', offset=i3 * offset_lev2),
                                                        f16_64.access_ptr('r', offset=i3 * 8 * f16_c0),
                                ...
                                if _inner_tail > 0:
                                    ...
                                    ib.emit(
                                        tvm.call_extern(
                                            "float32", "copy_ubuf_to_gm",
                                            outputs.access_ptr(
                                                'w', offset=i2 * 8 * offset_4 + _inner_loop * free_space_fp32),
                                            f32_out.access_ptr('r'), 0, 1, (_inner_tail + 7) // 8, 0, 0))
                    if tail_lev1 > 0:
                        with ib.new_scope():
                            ib.scope_attr(param.CCE_AXIS, "coproc_scope", 4)
                            ...
                            with ib.for_range(0, _inner_loop) as inner_idx:
                                ...
                                ib.emit(
                                    tvm.call_extern(
                                        "float32", "copy_ubuf_to_gm",
                                        outputs.access_ptr(
                                            'w', offset=loop_lev1 * 8 * offset_4 + inner_idx * free_space_fp32),
                                        f32_out.access_ptr('r'), 0, 1, free_space_fp32 // 8, 0, 0))
                            if _inner_tail > 0:
                                ip_addr = [[f32_out, 0], [f16_out, _inner_loop * free_space_fp32]]
                                kernel_api.kernel_cast_to_fuc(ib, ip_addr, [_inner_tail, 8 * 8], "vconv_f162f32")
                                ...
                                ib.emit(
                                    tvm.call_extern(
                                        "float32", "copy_ubuf_to_gm",
                                        outputs.access_ptr(
                                            'w', offset=loop_lev1 * 8 * offset_4 + _inner_loop * free_space_fp32),
                                        f32_out.access_ptr('r'), 0, 1, (_inner_tail + 7) // 8, 0, 0))
                                ...
                                with ib.for_range(0, _inner_loop) as inner_idx:
                                    ...
                                    ib.emit(
                                        tvm.call_extern(
                                            ...
                                            outputs.access_ptr(
                                                'w',
                                                offset=i2 * 12 * HW_SIZE_512 * f16_c0 + inner_idx * free_space_fp32),
                                            f32_out.access_ptr('r'), 0, 1, free_space_fp32 // 8, 0, 0))
                    if tail_lev2 > 0:
                        with ib.new_scope():
                            ib.scope_attr(param.CCE_AXIS, "coproc_scope", 4)
                            ...
                            with ib.for_range(0, _inner_loop) as inner_idx:
                                ...
                                ib.emit(
                                    tvm.call_extern(
                                        ...
                                        outputs.access_ptr(
                                            'w',
                                            offset=loop_lev2 * 12 * HW_SIZE_512 * f16_c0 + inner_idx * free_space_fp32),
                            ...
                            if _inner_tail > 0:
                                ...
                                ib.emit(
                                    tvm.call_extern(
                                        "float32", "copy_ubuf_to_gm",
                                        outputs.access_ptr('w',
                                                           offset=loop_lev2 * 12 * HW_SIZE_512 * f16_c0 +
                                                           _inner_loop * free_space_fp32), f32_out.access_ptr('r'), 0,
                                        1, (_inner_tail + 7) // 8, 0, 0))
            ...
            f32_out = apply_store_buffer(ib, "float32", [free_space * f32_c0], name="f32_out")

            if expand_size > HW_SIZE_512:
                f32_8 = apply_store_buffer(ib, "float32", [8 * f32_c0], name="f32_8")
            elif expand_size > 16:
                f32_32 = apply_store_buffer(ib, "float32", [32 * f32_c0], name="f32_32")
            # Note:
            #   1. input data larger than HW_SIZE_512 should use L1 optimize
            #   2. input small data do not need L1
            ...
            if expand_loop > HW_SIZE_512:
                            ...
                            if expand_size > HW_SIZE_512:
                                ...
                                def _inner_run_lev3(core_lp3_idx):
                                    ...
                                    with ib.new_scope():
                                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 2)
                                        ib.emit(
                                            tvm.call_extern("float32", "vadds", f32_out.access_ptr('w'),
                                                            f32_8.access_ptr('r'), tvm.const(0.0, dtype="float32"),
                                                            repeat_128, 1, 1, 8, 0))
                                    with ib.new_scope():
                                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 2)
                                        ib.emit(
                                            tvm.call_extern("float32", "copy_ubuf_to_ubuf",
                                                            f32_out.access_ptr('w', offset=HW_SIZE_512 * f16_c0),
                                                            f32_out.access_ptr('r'), 0, 1, HW_SIZE_1024, 0, 0))
                                    with ib.new_scope():
                                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 2)
                                        ib.emit(
                                            tvm.call_extern("float32", "copy_ubuf_to_ubuf",
                                                            f32_out.access_ptr('w', offset=HW_SIZE_1024 * f16_c0),
                                        ...
                                        with ib.new_scope():
                                            ...
                                            ib.emit(
                                                tvm.call_extern("float32", "vadds",
                                                                ...
                                                                f32_32.access_ptr('r', offset=i5 * 4 * f16_c0),
                                                                tvm.const(0.0, dtype="float32"), repeat_l4, 1, 1, 8, 0))
                                    offset_4 = expand_size * f16_c0
                ...
                if tail_level1 > 0:
                    if tail_level1 > HW_SIZE_512:
                        ...
                        with ib.new_scope():
                            ib.scope_attr(param.CCE_AXIS, "coproc_scope", 5)
                            ib.emit(
                                tvm.call_extern("float32", "copy_gm_to_ubuf", f32_in.access_ptr('w'),
                                                inputs.access_ptr('r', offset=loop_level1 * f32_half * f16_c0), 0, 1,
                                                tail_level1 * 2, 0, 0))
                    # level_1 loop
                    loop_l1 = tail_level1 // HW_SIZE_512
                    tail_l1 = tail_level1 % HW_SIZE_512
                                ...
                                with ib.for_range(0, loop_l2) as i3:
                                    ...
                                    with ib.for_range(0, loop_l3) as i4:
                                        with ib.new_scope():
                                            ...
                                            ib.emit(
                                                tvm.call_extern("float32", "vadds",
                                                                f32_out.access_ptr('w', offset=i4 * offset_l3),
                                                                f32_32.access_ptr('r', offset=i4 * 4 * f16_c0),
                                                                tvm.const(0.0, dtype="float32"), repeat_l3, 1, 1, 8, 0))
                                    offset_4 = expand_size * f16_c0
                                    expand_4 = 4 * expand_size * 2
                                    with ib.new_scope():
                                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 6)
                                ...
                                with ib.for_range(0, loop_le1) as i2:
                                    ...
                                    with ib.for_range(0, loop_le2) as i3:
                                        with ib.new_scope():
                                            ...
                                            ib.emit(
                                                tvm.call_extern("float32", "vadds",
                                                                f32_out.access_ptr('w', offset=i3 * offset_le2),
                                                                f32_32.access_ptr('r', offset=i3 * 4 * f16_c0),
                                                                tvm.const(0.0, dtype="float32"), repeat_le2, 1, 1, 8,
                                                                0))
                                    offset_4 = expand_size * f16_c0
                                    expand_4 = 4 * expand_size * 2
                                    with ib.new_scope():
                            ...
                            if tail_le1 > 0:
                                ...
                                with ib.for_range(0, loop_le2) as i4:
                                    with ib.new_scope():
                                        ...
                                        ib.emit(
                                            tvm.call_extern("float32", "vadds",
                                                            f32_out.access_ptr('w', offset=i4 * offset_le2),
                                                            f32_32.access_ptr('r', offset=i4 * 4 * f16_c0),
                ...
                with ib.new_scope():
                    ib.scope_attr(param.CCE_AXIS, "coproc_scope", 5)
                    ib.emit(
                        tvm.call_extern("float32", "copy_gm_to_ubuf", f32_in.access_ptr('w'), inputs.access_ptr('r'), 0,
                                        1, expand_loop * 2, 0, 0))
                # Note:
                #   1. H * W >= HW_SIZE_512 is common
                #   2. HW_SIZE_512 > H * W > 16
                ...
                if expand_size > HW_SIZE_512:
                    ...
                    def _inner_run_lev1(core_lp1_idx):
                        ...
                        with ib.new_scope():
                            ib.scope_attr(param.CCE_AXIS, "coproc_scope", 2)
                            ib.emit(
                                tvm.call_extern("float32", "vadds", f32_out.access_ptr('w'), f32_8.access_ptr('r'),
                                    ...
                                    ib.emit(
                                        tvm.call_extern(
                                            ...
                                            outputs.access_ptr('w',
                                                               ...
                                                               i3 * HW_SIZE_2048 * f16_c0 +
                                                               core_lp1_idx * core_counts * offset_2),
                                            f32_out.access_ptr('r'), 0, 1, HW_SIZE_2048 * 2, 0, 0))
                        if tail_lev2 > 0:
                            with ib.new_scope():
                                ib.scope_attr(param.CCE_AXIS, "coproc_scope", 6)
                                ib.emit(
                        ...
                        with ib.for_range(0, loop_lev1) as i2:
                            ...
                            with ib.for_range(0, loop_lev2) as i3:
                                with ib.new_scope():
                                    ...
                                    ib.emit(
                                        tvm.call_extern("float32", "vadds",
                                                        f32_out.access_ptr('w', offset=i3 * offset_lev2),
                                                        f32_32.access_ptr('r', offset=i3 * 4 * f16_c0),
                            ...
                            with ib.new_scope():
                                ...
                                ib.emit(
                                    tvm.call_extern(dtype, "copy_ubuf_to_gm",
                                                    outputs.access_ptr('w', offset=i2 * 4 * offset_4),
                    ...
                    if tail_lev1 > 0:
                        ...
                        with ib.new_scope():
                            ...
                            ib.emit(
                                tvm.call_extern(dtype, "copy_ubuf_to_gm",
                                                outputs.access_ptr('w', offset=loop_lev1 * 4 * offset_4),
                                                f32_out.access_ptr('r'), 0, 1, expand_4, 0, 0))
                            ...
                            with ib.new_scope():
                                ...
                                ib.emit(
                                    tvm.call_extern("float32", "copy_ubuf_to_gm",
                                                    outputs.access_ptr('w', offset=i2 * 6 * HW_SIZE_512 * f16_c0),
                                                    f32_out.access_ptr('r'), 0, 1, HW_SIZE_512 * 12, 0, 0))
                    if tail_lev2 > 0:
                        with ib.new_scope():
                            ib.scope_attr(param.CCE_AXIS, "coproc_scope", 4)
                            ib.emit(
            ...
            if expand_loop > f16_size:
                    ...
                    with ib.for_range(0, loop_level1) as i1:
                        ...
                        with ib.for_range(0, loop_level2) as i2:
                            ...
                            with ib.new_scope():
                                ...
                                kernel_api.kernel_cast_to_fuc(ib, ip_addr, [f16_size * 16, 8 * 8], "vconv_f162f32")
                                ib.scope_attr(param.CCE_AXIS, "coproc_scope", 6)
                                ib.emit(tvm.call_extern(
                                    "float32", "copy_ubuf_to_gm",
                                    outputs.access_ptr(
                ...
                if f16_stride > gap_limit:
                    ...
                    with ib.new_scope():
                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 5)
                        ib.emit(
                            tvm.call_extern("float16", "copy_gm_to_ubuf", f16_out.access_ptr('w'),
                                            inputs.access_ptr('r', offset=in_offset), 0, expand_loop, 1, f16_stride, 0))
                with ib.new_scope():
                    ip_addr = [[out_ub_f32, 0], [f16_out, 0]]
            ...
            if expand_loop > f32_size:
                ...
                if f32_stride > gap_limit:
                    ...
                    with ib.new_scope():
                        ib.scope_attr(param.CCE_AXIS, "coproc_scope", 5)
                        ib.emit(
                            tvm.call_extern("float32", "copy_gm_to_ubuf", f32_out.access_ptr('w'),
                                            inputs.access_ptr('r', offset=in_offset), 0, expand_loop, 2, f32_stride, 0))
                with ib.new_scope():
                    ib.scope_attr(param.CCE_AXIS, "coproc_scope", 6)
                    ib.emit(
                        tvm.call_extern("float32", "copy_ubuf_to_gm", outputs.access_ptr('w'), f32_out.access_ptr('r'),
                                        0, 1, expand_loop * 2, 0, 0))

    # No.4 situation: for mini_dc
    elif tbe_platform.cce_conf.intrinsic_check_support("Intrinsic_vbi", "float16") and dtype == "float16":
        # use multi-core resource
        ub_size = tbe_platform.cce_conf.get_soc_spec(tbe_platform.cce_conf.UB_SIZE)
        l1_size = tbe_platform.cce_conf.get_soc_spec(tbe_platform.cce_conf.L1_SIZE)
        h_per_core = h_out // core_counts + (1 if h_out % core_counts > 0 else 0)
        ...
        def _apply_ub_dc():
            ...
            if not is_bigger_ub_size:
                ...
                ub_info["float32_temp_wo_ub"] = apply_store_buffer(ib,
                                                                   "float32", [vbi_ub_size * 4],
                                                                   name="float32_temp_wo_ub")
                # apply ub for w_out position, size is ceil(w_out // 8) * 8 * 4
                ub_info["float32_w_pos_ub"] = apply_store_buffer(ib,
                                                                 "float32", [vbi_ub_size * 4],
                                                                 name="float32_w_pos_ub")
                # apply ub for w_out position, size is ceil(w_out // 8) * 8 * 4
                ub_info["int32_w_pos_ub"] = apply_store_buffer(ib, "int32", [vbi_ub_size * 4], name="int32_w_pos_ub")
                # apply ub for w scalar, size is ceil(w_out // 8) * 8 * 4
                ub_info["float16_w_scalar_ub"] = apply_store_buffer(ib,
                                                                    "float16", [vbi_ub_size * 4],
                                                                    name="float16_w_scalar_ub")
                # apply ub for output line, size is w_out * c0
                ub_info["out_ub"] = apply_store_buffer(ib, "float16", [w_out * f16_c0], name="out_ub")
                # apply ub for output line, size is w_out * c0
                ub_info["float32_out_ub"] = apply_store_buffer(ib, "float32", [w_out * f16_c0], name="float32_out_ub")
            else:
                # apply ub for w_out position, size is wo_threshold * 4
                ub_info["float32_temp_wo_ub"] = apply_store_buffer(ib,
                                                                   "float32", [wo_threshold * 4],
                                                                   name="float32_temp_wo_ub")
                # apply ub for w_out position, size is vbi_ub_size * 4
                l1_info["float32_temp_wo_l1"] = apply_store_buffer(ib,
                                                                   "float32", [vbi_ub_size * 4],
                                                                   name="float32_temp_wo_l1",
                ...
                ub_info["float32_w_pos_ub"] = apply_store_buffer(ib,
                                                                 ...
                                                                 name="float32_w_pos_ub")
                # apply ub for w_out position, size is wo_threshold * 4
                ub_info["int32_w_pos_ub"] = apply_store_buffer(ib, "int32", [wo_threshold * 4], name="int32_w_pos_ub")
                # apply ub for w_out position, size is vbi_ub_size * 4
                l1_info["int32_w_pos_l1"] = apply_store_buffer(ib,
                                                               "int32", [vbi_ub_size * 4],
                                                               name="int32_w_pos_l1",
                                                               scope=param.scope_cbuf)
                # apply ub for w scalar, size is wo_threshold * 4
                ub_info["float16_w_scalar_ub"] = apply_store_buffer(ib,
                                                                    "float16", [wo_threshold * 4],
                                                                    name="float16_w_scalar_ub")
                # apply ub for output line, size is wo_threshold * c0
                ub_info["out_ub"] = apply_store_buffer(ib, "float16", [wo_threshold * f16_c0], name="out_ub")
                # apply ub for output line, size is wo_threshold * c0
                ub_info["float32_out_ub"] = apply_store_buffer(ib,
                                                               "float32", [wo_threshold * f16_c0],
                                                               name="float32_out_ub")

            # apply ub for const 0, size is 16
            ub_info["float32_const_0"] = apply_store_buffer(ib, "float32", [VBI_BLOCK_COUNT], name="float32_const_0")

            # apply ub for h_out position, size is h_out
            ub_info["int32_h_pos_ub"] = apply_store_buffer(ib, "int32", [vbi_ub_size_h], name="int32_h_pos_ub")

            # apply ub for w_out position, size is h_out * 2
            ub_info["float32_h_scalar_ub"] = apply_store_buffer(ib,
                                                                "float32", [vbi_ub_size_h * 2],
                                                                name="float32_h_scalar_ub")

            # apply ub for top_left_right line, size is (w_in + 1) * f16_c0 * 2
            ub_info["w0_w1_in_ub"] = apply_store_buffer(ib, "float16", [(w_in + 1) * f16_c0 * 2], name="w0_w1_in_ub")

            # apply ub for half_pixel_centers, size is 64
            if half_pixel_centers:
                ub_info["float32_const_1"] = apply_store_buffer(ib, "float32", [f32_c0 * 8], name="float32_const_1")

            # apply l1 for hw scalar, size is vbi_ub_size *4 * h_out
            if is_smaller_l1_size:
                l1_info["float16_hw_scalar_l1"] = apply_store_buffer(ib,
                                                                     "float16", [hw_scalar_l1_size],
                                                                     name="float16_hw_scalar_l1",
                                                                     scope=param.scope_cbuf)

        _apply_ub_dc()
        # apply reg for using to get pos
        pos_reg = ib.allocate("int32", (8, ), name="pos_reg", scope=param.scope_reg)
        pos_reg_float32 = ib.allocate("float32", (2, ), name="pos_reg_float32", scope=param.scope_reg)
        ...
        def _vector_dup_fuc(ub_var, data_len, dtype, value):
            ...
            if loop_cnt > 0:
                ib.emit(
                    tvm.call_extern(dtype, "vector_dup", ub_var.access_ptr('w'), tvm.const(value, dtype=dtype),
                                    loop_cnt, 1, 0, 8, 0))
            if left_cnt > 0:
                ib.emit(
                    tvm.call_extern("uint64", "set_vector_mask", tvm.const(0, dtype="uint64"),
                                    tvm.const((1 << left_cnt) - 1, dtype="uint64")))
                ib.emit(
                    tvm.call_extern(dtype, "vector_dup", ub_var.access_ptr('w', offset=loop_cnt * repeat_len),
                                    tvm.const(1.0, dtype=dtype), 1, 1, 0, 8, 0))
                ib.emit(
        ...
        def _gen_w_seq_data():
            ...
            ib.emit(
                tvm.call_extern("uint64", "set_vector_mask", tvm.const(-1, dtype="uint64"), tvm.const(-1,
                                                                                                      dtype="uint64")))
        ...
        def _calc_h_out_pos_scalar():
            ...
            def _inner_h_copy_data(vbi_cnt, data_len, add_value, offset):
                ...
                ib.emit(
                    tvm.call_extern("uint64", "set_vector_mask", tvm.const(0, dtype="uint64"),
                                    tvm.const((1 << data_len) - 1, dtype="uint64")))
            ...
            if h_out_vbi_count < VBI_BLOCK_COUNT * 2:
                ...
                if loop_left > 0:
                    _inner_h_copy_data(loop_left + 1, f32_c0, f32_c0, loop_cnt * f32_c0 * 8 - f32_c0)
        ...
        def _calc_w_out_pos(wo_len, prev_wo_cnt):
            ...
            if w_out_vbi_count < 4:
                ...
                if loop_left > 0:
                    _inner_copy_data(2, f32_c0 * 4, f32_c0, loop_cnt * f32_c0 * 8 - f32_c0 * 4)
            ...
            if loop_cnt_64 > 0:
                ...
                ib.emit(
                    tvm.call_extern(
                        "uint64", "set_vector_mask", tvm.const(0, dtype="uint64"),
                ...
                ib.emit(
                    tvm.call_extern(
                        "uint64", "set_vector_mask", tvm.const(0, dtype="uint64"),
                        tvm.const(((1 << 64) - 1) - ((1 << 56) - 1) + ((1 << 48) - 1) - ((1 << 40) - 1) +
        ...
        def _calc_w_out_scalar(temp_ub, h_scalar, neg_h_scalar, wo_len):
            ...
            if loop_cnt_64 > 0:
                ...
                ib.emit(
                    tvm.call_extern(
                        "uint64", "set_vector_mask", tvm.const(0, dtype="uint64"),
                        tvm.const(((1 << 56) - 1) - ((1 << 48) - 1) + ((1 << 40) - 1) - ((1 << 32) - 1) +
                                  ((1 << 24) - 1) - ((1 << 16) - 1) + ((1 << 8) - 1),
                                  dtype="uint64")))
                ib.emit(
        ...
        with ib.for_range(0, nc1) as nc1_idx:
            ...
            with ib.for_range(0, h_per_core) as h_loop_idx:
                with ib.if_scope(core_idx * h_per_core + h_loop_idx < h_out):
                    # get h_out pos
                    ib.emit(
                    ...
                    def _inner_vbi_process(data_len, elem_offset):
                        ...
                        if repeat_vbi_left > 0:
                            if repeat_vbi_left <= 64:
                                ib.emit(
                                    tvm.call_extern("uint64", "set_vector_mask", tvm.const(0, dtype="uint64"),
                                                    tvm.const((1 << repeat_vbi_left) - 1, dtype="uint64")))
                            else:
                    ...
                    if wo_loop_count > 0:
                        ...
                        with ib.for_range(0, wo_loop_count) as wo_lp_idx:
                            _copy_l1_to_ub(wo_threshold // f32_c0, wo_lp_idx * wo_threshold)
                            _inner_vbi_process(wo_threshold, wo_lp_idx * wo_threshold)

                        if wo_loop_left > 0:
                            left_block = (wo_loop_left + f32_c0 - 1) // f32_c0
                            _copy_l1_to_ub(left_block, wo_loop_count * wo_threshold)
                            _inner_vbi_process(wo_loop_left, wo_loop_count * wo_threshold)
                    else:
    ...
    # No.5 normal input/output H/W
        ...
        def apply_ub():
            ...
            ub_info["x0_512_x1_512_ub"] = apply_store_buffer(ib,
                                                             "float32", [HW_SIZE_512 * 2 * f32_c0],
                                                             name="x_512_x1_512")
            # apply ub for x(h+1,w) HW_SIZE_512  x1(h+1,w+1) HW_SIZE_512
            ub_info["x2_512_x3_512_ub"] = apply_store_buffer(ib,
                                                             "float32", [HW_SIZE_512 * 2 * f32_c0],
                                                             name="x2_512_x3_512")
            ub_info["int32_512_ub"] = apply_store_buffer(ib, "int32", [HW_SIZE_256 * f32_c0], name="int32_512")
            ub_info["int32_512_ub_y"] = apply_store_buffer(ib, "int32", [8 * f32_c0], name="int32_512_ub_y")
            ub_info["const_0"] = apply_store_buffer(ib, "float32", [8 * f32_c0], name="const_0")

            ub_info["x0_scale_x1_scale_512_ub"] = apply_store_buffer(ib,
                                                                     "float32", [HW_SIZE_512 * f32_c0],
                                                                     name="x0_scale_x1_scale_512_ub")
            ub_info["y0_scale_ub"] = apply_store_buffer(ib, "float32", [2 * f32_c0], name="y0_scale_ub")

            ub_info["out_f32"] = apply_store_buffer(ib, "float32", [HW_SIZE_512 * f32_c0], name="out_f32")
            l1_info["l1_ypos"] = apply_store_buffer(ib,
                                                    "int32", [HW_SIZE_512 * 3 * f32_c0],
                                                    name="l1_ypos",
                                                    scope=param.scope_cbuf)
            ...
            l1_info["l1_xscale"] = apply_store_buffer(
                ...
                name="l1_xscale",
                scope=param.scope_cbuf)
            l1_info["l1_yscale"] = apply_store_buffer(ib,
                                                      "float32", [2 * HW_SIZE_512 * 3 * f32_c0],
                                                      name="l1_yscale",
                                                      scope=param.scope_cbuf)
            l1_info["l1_input"] = apply_store_buffer(ib,
                                                     "float32", [2 * (w_in + 1) * 16],
                                                     name="l1_input",
                                                     scope=param.scope_cbuf)
            ub_info["ub_input"] = apply_store_buffer(ib, "float32", [2 * (w_in + 1) * 16], name="ub_input")
            ub_info["f16_512"] = apply_store_buffer(ib, "float16", [HW_SIZE_512 * 8], name="f16_512")
            if devices == "cloud":
                ub_info["f16_512_1"] = apply_store_buffer(ib, "float16", [HW_SIZE_256 * 8], name="f16_512_1")
            else:
                ub_info["f16_512_1"] = apply_store_buffer(ib, "float16", [HW_SIZE_512 * 8], name="f16_512_1")
            # apply ub for x(h,w) HW_SIZE_512  x1(h,w+1) HW_SIZE_512
            ub_info["x0_512_x1_512_ub_1"] = apply_store_buffer(ib,
                                                               "float32", [HW_SIZE_512 * 2 * f32_c0],
                                                               name="x_512_x1_512_1")
            # apply ub for x(h+1,w) HW_SIZE_512  x1(h+1,w+1) HW_SIZE_512
            ub_info["x2_512_x3_512_ub_1"] = apply_store_buffer(ib,
                                                               "float32", [HW_SIZE_512 * 2 * f32_c0],
                                                               name="x2_512_x3_512_1")
            # for unfold
            ub_info["x0_512_x1_512_ub_unfold"] = apply_store_buffer(ib,
                                                                    "float32", [HW_SIZE_512 * 2 * f32_c0],
                                                                    name="x0_512_x1_512_unfold")
            ub_info["x2_512_x3_512_ub_unfold"] = apply_store_buffer(ib,
                                                                    "float32", [HW_SIZE_512 * 2 * f32_c0],
        ...
        range_512 = apply_store_buffer(ib, "float32", [HW_SIZE_512 * f32_c0], name="range_512")
        ...
        ib.emit(
            tvm.call_extern("float32", "vector_dup", range_512.access_ptr('w', offset=8 * 0),
        ...
        with ib.for_range(0, loop_level1 - 1) as i:
            ...
            ib.emit(
                tvm.call_extern("float32", "vadds", range_512.access_ptr('w', offset=8 * (i + 1)),
                                range_512.access_ptr('r', offset=8 * i), tvm.const(1.0, dtype="float32"), 1, 1, 1, 1,
                                1))
        ib.emit(
            tvm.call_extern("uint64", "set_vector_mask", tvm.const(-1, dtype="uint64"), tvm.const(-1, dtype="uint64")))
        loop_level1 = HW_SIZE_256 // 8
        with ib.for_range(0, loop_level1 - 1) as i:
            ib.emit(tvm.call_extern('int32', 'pipe_barrier', args_str_v))
            ib.emit(
                tvm.call_extern("float32", "vadds", range_512.access_ptr('w', offset=8 * 8 * (i + 1)),
                                range_512.access_ptr('r', offset=8 * 8 * i), tvm.const(8.0, dtype="float32"), 1, 1, 1,
                                8, 8))
        ...
        def calu_x_y_out_pos(_ib, _loop_info, _scale_info, _ub_info, _l1_info):
            ...
            def _cast_f32_to_int32(_data_info, f32_ub_info, int32_ub_info, dst_f32_ub_info, fix_flag=False):
                ...
                if devices == "cloud":
                    _addr_info = [_ub_int32_info, _ub_fp32_info]
                    kernel_api.kernel_cast_to_fuc(_ib, _addr_info, _data_info, "vconv_f322s32f")
                    _addr_info = [_ub_des_fp32, _ub_int32_info]
                    kernel_api.kernel_cast_to_fuc(_ib, _addr_info, _data_info, "vconv_s322f32")
                else:
                    _ub_fp16_512 = _ub_info["f16_512_1"]
                    _addr_info = [[_ub_fp16_512, 0], _ub_fp32_info]
                    kernel_api.kernel_cast_to_fuc(_ib, _addr_info, _data_info, "vconv_f322f16")
                    _addr_info = [_ub_int32_info, [_ub_fp16_512, 0]]
                    kernel_api.kernel_cast_to_fuc(_ib, _addr_info, _data_info, "vconv_f162s32f")
                    ib.emit(tvm.call_extern("float16", "set_deqscale", tvm.const(1.0, dtype="float16")))
                    _addr_info = [[_ub_fp16_512, 0], _ub_int32_info]
                    kernel_api.kernel_cast_to_fuc(_ib, _addr_info, _data_info, "vconv_deq")
                    _addr_info = [_ub_des_fp32, [_ub_fp16_512, 0]]
                    kernel_api.kernel_cast_to_fuc(_ib, _addr_info, _data_info, "vconv_f162f32")
                    ...
                    if fix_flag:
                        ...
                        _addr_info = [[fix_tmp_fp32_ub, 0], _ub_des_fp32, _ub_fp32_info]
                        kernel_api.kernel_two_to_one_common_fuc(_ib, _addr_info, _data_info, "vsub")
                        kernel_api.kernel_vector_dup_fuc(_ib, [tmp_one_block_ub, 0], 0, [8, 64])
                        kernel_api.kernel_vector_dup_fuc(_ib, [tmp_one_block_ub, 8], SCALAR_MIN_FP32, [8, 64])
                        _ib.emit(
                            tvm.call_extern(fix_tmp_fp32_ub.dtype, "vmax", fix_tmp_fp32_ub.access_ptr('w', offset=0),
                                            tmp_one_block_ub.access_ptr('r', offset=0),
                                            fix_tmp_fp32_ub.access_ptr('r', offset=0), 32, 1, 0, 1, 8, 0, 8))
                        _ib.emit(
                            tvm.call_extern(fix_tmp_fp32_ub.dtype, "vmin", fix_tmp_fp32_ub.access_ptr('w', offset=0),
                                            tmp_one_block_ub.access_ptr('r', offset=8),
                                            fix_tmp_fp32_ub.access_ptr('r', offset=0), 32, 1, 0, 1, 8, 0, 8))

                        kernel_api.kernel_scalar_to_one_fuc(_ib, [[fix_tmp_fp32_ub, 0], [fix_tmp_fp32_ub, 0]],
                                                            _data_info, ["vmuls", SCALAR_MUL_FP32])
                        kernel_api.kernel_scalar_to_one_fuc(_ib, [[fix_tmp_fp32_ub, 0], [fix_tmp_fp32_ub, 0]],
                                                            _data_info, ["vmuls", SCALAR_MUL_FP32])
                        kernel_api.kernel_scalar_to_one_fuc(_ib, [[fix_tmp_fp32_ub, 0], [fix_tmp_fp32_ub, 0]],
                                                            _data_info, ["vmuls", SCALAR_MUL2_FP32])
                        _addr_info = [_ub_des_fp32, _ub_des_fp32, [fix_tmp_fp32_ub, 0]]
                        kernel_api.kernel_two_to_one_common_fuc(_ib, _addr_info, _data_info, "vsub")
                        _addr_info = [[_ub_fp16_512, 0], _ub_des_fp32]
                        kernel_api.kernel_cast_to_fuc(_ib, _addr_info, _data_info, "vconv_f322f16")
                        _addr_info = [_ub_int32_info, [_ub_fp16_512, 0]]
                        kernel_api.kernel_cast_to_fuc(_ib, _addr_info, _data_info, "vconv_f162s32r")

            def _process_pos_to_l1(src_ub_info, mid_ub_info, des_l1_info, para_list, loop_index):
                ...
                if _is_h:
                    if devices == "cloud":
                        _ib.emit(
                            tvm.call_extern(_input_fp32_ub.dtype, "vadds", _input_fp32_ub.access_ptr('w'),
                                            _input_fp32_512_num.access_ptr('r'),
                                            _core_id * tvm.const(_h_per_core, dtype="float32"), 32, 1, 1, 8, 8))
                        _input_fp32_512_num = _input_fp32_ub
                    else:
                        scalar_dtype = _get_scalar_dtype()
                        int_reg = _ib.allocate(scalar_dtype, (1, ), name="int_data", scope=param.scope_reg)
                        int_reg[0] = _core_id
                        int32_ub_8 = apply_store_buffer(ib, "int32", [8], name="int32_ub_8")
                        float32_ub_8 = apply_store_buffer(ib, "float32", [64], name="float32_ub_8")
                        kernel_api.kernel_vector_dup_fuc(_ib, [int32_ub_8, 0], int_reg[0], [1, 64])
                        ib.emit(tvm.call_extern("float16", "set_deqscale", tvm.const(1.0, dtype="float16")))
                        float16_ub_8 = apply_store_buffer(ib, "float16", [128], name="float16_ub_8")
                        _addr_info = [[float16_ub_8, 0], [int32_ub_8, 0]]
                        kernel_api.kernel_cast_to_fuc(_ib, _addr_info, [1, 64], "vconv_deq")
                        _addr_info = [[float32_ub_8, 0], [float16_ub_8, 0]]
                        kernel_api.kernel_cast_to_fuc(_ib, _addr_info, [1, 64], "vconv_f162f32")
                        kernel_api.kernel_scalar_to_one_fuc(_ib, [[float32_ub_8, 0], [float32_ub_8, 0]], [1, 64],
                                                            ["vmuls", _h_per_core])
                        core_reg = _ib.allocate("float32", (1, ), name="core_reg", scope=param.scope_reg)
                        _ib.emit(tvm.call_extern('int32', 'pipe_barrier', args_str_v_all))
                ...
                if half_pixel_centers:
                    _ib.emit(
                        tvm.call_extern(_input_fp32_512_num.dtype, "vadds", _input_fp32_512_num.access_ptr('w'),
                                        _input_fp32_512_num.access_ptr('r'), tvm.const(0.5, dtype="float32"), 32, 1, 1,
                                        8, 8))
                _ib.emit(
                    tvm.call_extern(_input_fp32_ub.dtype, "vmuls", _input_fp32_ub.access_ptr('w'),
                                    _input_fp32_512_num.access_ptr('r'), tvm.const(_scale, dtype="float32"), 32, 1, 1,
                                    8, 8))
                if half_pixel_centers:
                    _ib.emit(
                        tvm.call_extern(_input_fp32_ub.dtype, "vadds", _input_fp32_ub.access_ptr('w'),
                    ...
                    if _input_fp32_512_num == range_512:
                        _ib.emit(
                            tvm.call_extern(_input_fp32_512_num.dtype, "vadds", _input_fp32_512_num.access_ptr('w'),
                                            _input_fp32_512_num.access_ptr('r'), tvm.const(-0.5, dtype="float32"), 32,
                                            1, 1, 8, 8))
                ...
                if not _is_h and is_get_error_w:
                    whether_fix = True
                # process x pos
                _cast_f32_to_int32(data_info, [_input_fp32_ub, 0], [_mid_ub_int32, 0],
                                   [_input_fp32_ub, HW_SIZE_256 * 8], whether_fix)
                _addr_info = [[_out_l1_int, loop_index * HW_SIZE_256 * 8], [_mid_ub_int32, 0]]
                kernel_api.kernel_cp_fuc(_ib, _addr_info, [HW_SIZE_256 * 8, 8], "copy_ubuf_to_cbuf")
                # x_float - x_int
                _addr_info = [[_input_fp32_ub, HW_SIZE_256 * 8], [_input_fp32_ub, 0], [_input_fp32_ub, HW_SIZE_256 * 8]]
                kernel_api.kernel_two_to_one_common_fuc(_ib, _addr_info, data_info, "vsub")

                _ib.emit(
                    tvm.call_extern(_input_fp32_ub.dtype, "vsub", _input_fp32_ub.access_ptr('w', offset=0),
                                    _mid_one_ub_fp32.access_ptr('r'),
                                    _input_fp32_ub.access_ptr('r', offset=HW_SIZE_256 * 8), 32, 1, 1, 1, 8, 0, 8))
                # copy ub tp l1
                if _is_h:
                    _ib.emit(
                        tvm.call_extern(_out_l1_fp32.dtype, "copy_ubuf_to_cbuf",
                                        _out_l1_fp32.access_ptr("rw", offset=loop_index * HW_SIZE_256 * 8),
                                        _input_fp32_ub.access_ptr("r", offset=0), 0, 2, HW_SIZE_256, 0,
                                        loop_levely * HW_SIZE_256 - HW_SIZE_256))
                else:
                    _addr_info = [[_out_l1_fp32, loop_index * HW_SIZE_256 * 2 * 8], [_input_fp32_ub, 0]]
                    kernel_api.kernel_cp_fuc(_ib, _addr_info, [HW_SIZE_256 * 2 * 8, 8], "copy_ubuf_to_cbuf")

            with _ib.for_range(0, min_loop) as _loop:
                _process_pos_to_l1([x_index_ub, range_512], [int32_ub, one_ub],
                                   [_l1_info["l1_xpos"], _l1_info["l1_xscale"]], [_scale_w, idx, False, h_per_core],
                                   _loop)
                _process_pos_to_l1([y_index_ub, range_512], [int32_ub, one_ub],
                                   [_l1_info["l1_ypos"], _l1_info["l1_yscale"]], [_scale_h, idx, True, h_per_core],
                                   _loop)
                kernel_api.kernel_scalar_to_one_fuc(_ib, [[range_512, 0], [range_512, 0]], [HW_SIZE_256 * 8, 64],
                                                    ["vadds", 256.0])
            if _x_loop != _y_loop:
                ...
                with _ib.for_range(0, tail_loop) as _loop:
                    _process_pos_to_l1([index_ub, range_512], [int32_ub, one_ub], [index_l1_int, index_l1_fp32],
                                       [tail_scale, idx, is_h, h_per_core], _loop + min_loop)
                    kernel_api.kernel_scalar_to_one_fuc(_ib, [[range_512, 0], [range_512, 0]], [HW_SIZE_256 * 8, 64],
                                                        ["vadds", 256.0])

        calu_x_y_out_pos(ib, [loop_levelx, loop_levely], [scale_w, scale_h], ub_info, l1_info)
        ...
        def _copy_input_to_l1(src_gm_ub, src_offset, dst_l1_ub, dat_len, mid_ub, y_index):
            ...
            if _dtype == "float32":
                ...
                with ib.for_range(0, vconv_loop) as vconv_loop_i:
                    ...
                    kernel_api.kernel_cast_to_fuc(ib, _addr, [HW_SIZE_256 * 8, 8 * 8], "vconv_f162f32")
                    if is_input_in_ub is not True:
                        _addr = [[dst, vconv_loop_i * HW_SIZE_256 * 8], [fp32_mid, 0]]
                        kernel_api.kernel_cp_fuc(ib, _addr, [HW_SIZE_256 * 8, 8], "copy_ubuf_to_cbuf")
                    else:
                        _addr = [[fp32_dst, vconv_loop_i * HW_SIZE_256 * 8], [fp32_mid, 0]]
                        kernel_api.kernel_cp_fuc(ib, _addr, [HW_SIZE_256 * 8, 8], "copy_ubuf_to_ubuf")
                if vconv_tail != 0:
                    _addr = [[fp16_mid, 0], [src, src_offset + vconv_loop * HW_SIZE_256 * 8]]
                    kernel_api.kernel_cp_fuc(ib, _addr, [vconv_tail, 16], "copy_gm_to_ubuf")
                    _addr = [[fp32_mid, 0], [fp16_mid, 0]]
                    kernel_api.kernel_cast_to_fuc(ib, _addr, [vconv_tail, 8 * 8], "vconv_f162f32")
                    if is_input_in_ub is not True:
                        _addr = [[dst, vconv_loop * HW_SIZE_256 * 8], [fp32_mid, 0]]
                        kernel_api.kernel_cp_fuc(ib, _addr, [vconv_tail, 8], "copy_ubuf_to_cbuf")
                        _addr = [[dst, cp_segment], [fp32_mid, vconv_tail - 16]]
                        kernel_api.kernel_cp_fuc(ib, _addr, [16, 8], "copy_ubuf_to_cbuf")
                    else:
                        _addr = [[fp32_dst, vconv_loop * HW_SIZE_256 * 8], [fp32_mid, 0]]
                        kernel_api.kernel_cp_fuc(ib, _addr, [vconv_tail, 8], "copy_ubuf_to_ubuf")
                        _addr = [[fp32_dst, cp_segment], [fp32_mid, vconv_tail - 16]]
                ...
                with ib.if_scope(y_index != size_in[2] - 1):
                    with ib.for_range(0, vconv_loop) as vconv_loop_i:
                        _addr = [[fp16_mid_1, 0], [src, src_offset + vconv_loop_i * HW_SIZE_256 * 8]]
                        kernel_api.kernel_cp_fuc(ib, _addr, [HW_SIZE_256 * 8, 16], "copy_gm_to_ubuf")
                        _addr = [[fp32_mid_1, 0], [fp16_mid_1, 0]]
                        kernel_api.kernel_cast_to_fuc(ib, _addr, [HW_SIZE_256 * 8, 8 * 8], "vconv_f162f32")
                        if is_input_in_ub is not True:
                            _addr = [[dst, des_offset + vconv_loop_i * HW_SIZE_256 * 8], [fp32_mid_1, 0]]
                            kernel_api.kernel_cp_fuc(ib, _addr, [HW_SIZE_256 * 8, 8], "copy_ubuf_to_cbuf")
                        else:
                            _addr = [[fp32_dst, des_offset + vconv_loop_i * HW_SIZE_256 * 8], [fp32_mid_1, 0]]
                            kernel_api.kernel_cp_fuc(ib, _addr, [HW_SIZE_256 * 8, 8], "copy_ubuf_to_ubuf")
                    if vconv_tail != 0:
                        _addr = [[fp16_mid_1, 0], [src, src_offset + vconv_loop * HW_SIZE_256 * 8]]
                        kernel_api.kernel_cp_fuc(ib, _addr, [vconv_tail, 16], "copy_gm_to_ubuf")
                        _addr = [[fp32_mid_1, 0], [fp16_mid_1, 0]]
                        kernel_api.kernel_cast_to_fuc(ib, _addr, [vconv_tail, 8 * 8], "vconv_f162f32")
                        if is_input_in_ub is not True:
                            _addr = [[dst, des_offset + vconv_loop * HW_SIZE_256 * 8], [fp32_mid_1, 0]]
                            kernel_api.kernel_cp_fuc(ib, _addr, [vconv_tail, 8], "copy_ubuf_to_cbuf")
                            _addr = [[dst, 2 * cp_segment + 16], [fp32_mid_1, vconv_tail - 16]]
                            kernel_api.kernel_cp_fuc(ib, _addr, [16, 8], "copy_ubuf_to_cbuf")
                        else:
                            _addr = [[fp32_dst, des_offset + vconv_loop * HW_SIZE_256 * 8], [fp32_mid_1, 0]]
                            kernel_api.kernel_cp_fuc(ib, _addr, [vconv_tail, 8], "copy_ubuf_to_ubuf")
                            _addr = [[fp32_dst, 2 * cp_segment + 16], [fp32_mid_1, vconv_tail - 16]]
                            kernel_api.kernel_cp_fuc(ib, _addr, [16, 8], "copy_ubuf_to_ubuf")
                    else:
                        if is_input_in_ub is not True:
                            _addr = [[dst, 2 * cp_segment + 16], [fp32_mid_1, HW_SIZE_256 * 8 - 16]]
                            kernel_api.kernel_cp_fuc(ib, _addr, [16, 8], "copy_ubuf_to_cbuf")
                        else:
            ...
            if index_align_loop > 0:
                with ib.for_range(0, index_align_loop) as _index:
                    ...
                    with ib.for_range(0, 8, name="reg_index") as reg_index:
                        ib.emit(
                            tvm.call_extern(x0x1_ub.dtype, copy_cmd,
                                            x0x1_ub.access_ptr('w', offset=(reg_index + _index * reg_count) * 16),
                                            src_addr.access_ptr('r', offset=pos_reg[reg_index + 2] * c0), 0, 2, 2, 0,
                                            510))
                    with ib.for_range(0, 8, name="reg_index") as reg_index:
                        ib.emit(
                            tvm.call_extern(x2x3_ub.dtype, copy_cmd,
                                            x2x3_ub.access_ptr('w', offset=(reg_index + _index * reg_count) * 16),
                                            src_addr.access_ptr('r', offset=(pos_reg[reg_index + 2] + w_in + 1) * c0),
                                            0, 2, 2, 0, 510))
                ...
                for reg_index in reg_list_left:
                    ib.emit(
                        tvm.call_extern(
                            ...
                            0))
                    # copy HW_SIZE_512 to block segment
                with ib.for_range(0, index_align_left, name="reg_index") as reg_index:
                    ib.emit(
                        tvm.call_extern(
        ...
        def _process(part_index, index_block, y_index, x_index, output_offset):
            ...
            if index_block <= HW_SIZE_256:
                ...
                ib.emit(
                    tvm.call_extern(x0x1_ub.dtype, "vmul", x0x1_ub.access_ptr('w'),
                                    y_scale_2_ub.access_ptr('r', offset=0), x0x1_ub_unfold.access_ptr('r'), _repeat * 2,
                                    1, 0, 1, 8, 0, 8))

                # x2*Yv  x3*Yv
                ib.emit(
                    tvm.call_extern(x2x3_ub.dtype, "vmul", x2x3_ub.access_ptr('w'),
                                    y_scale_2_ub.access_ptr('r', offset=8), x2x3_ub_unfold.access_ptr('r'), _repeat * 2,
                                    1, 0, 1, 8, 0, 8))
                # x0*(1-Yv)  x1*(1-Yv)
                ib.emit(
                    tvm.call_extern(x0x1_ub.dtype, "vmul", x0x1_ub.access_ptr('w', offset=HW_SIZE_512 * 8),
                                    y_scale_2_ub.access_ptr('r', offset=0),
                                    x0x1_ub_unfold.access_ptr('r', offset=HW_SIZE_512 * 8), _repeat * 2, 1, 0, 1, 8, 0,
                                    8))
                # x2*Yv  x3*Yv
                ib.emit(
                    tvm.call_extern(x2x3_ub.dtype, "vmul", x2x3_ub.access_ptr('w', offset=HW_SIZE_512 * 8),
                ...
                ib.emit(
                    tvm.call_extern(x0x1_ub.dtype, "vmul", x0x1_ub.access_ptr('w', offset=8),
                                    ...
                                    x0x1_ub.access_ptr('r', offset=8), _repeat, 2, 1, 2, 16, 8, 16))

                ib.emit(
                    tvm.call_extern(x2x3_ub.dtype, "vmul", x2x3_ub.access_ptr('w', offset=8),
                                    x_scale_512_ub.access_ptr('r', offset=scale_offset * 8),
                                    x2x3_ub.access_ptr('r', offset=8), _repeat, 2, 1, 2, 16, 8, 16))

                ib.emit(
                    tvm.call_extern(x0x1_ub.dtype, "vmul", x0x1_ub.access_ptr('w', offset=HW_SIZE_512 * 8),
                                    x_scale_512_ub.access_ptr('r', offset=HW_SIZE_256 * 8 + scale_offset * 8),
                                    x0x1_ub.access_ptr('r', offset=HW_SIZE_512 * 8), _repeat, 2, 1, 2, 16, 8, 16))

                ib.emit(
                    tvm.call_extern(x2x3_ub.dtype, "vmul", x2x3_ub.access_ptr('w', offset=HW_SIZE_512 * 8),
                                    x_scale_512_ub.access_ptr('r', offset=HW_SIZE_256 * 8 + scale_offset * 8),
                                    x2x3_ub.access_ptr('r', offset=HW_SIZE_512 * 8), _repeat, 2, 1, 2, 16, 8, 16))

                ib.emit(
                    tvm.call_extern(x0x1_ub.dtype, "vmul", x0x1_ub.access_ptr('w', offset=HW_SIZE_512 * 8 + 8),
                                    x_scale_512_ub.access_ptr('r', offset=HW_SIZE_256 * 8 + scale_offset * 8),
                                    x0x1_ub.access_ptr('r', offset=HW_SIZE_512 * 8 + 8), _repeat, 2, 1, 2, 16, 8, 16))

                ib.emit(
                    tvm.call_extern(x2x3_ub.dtype, "vmul", x2x3_ub.access_ptr('w', offset=HW_SIZE_512 * 8 + 8),
                                    x_scale_512_ub.access_ptr('r', offset=HW_SIZE_256 * 8 + scale_offset * 8),
                                    x2x3_ub.access_ptr('r', offset=HW_SIZE_512 * 8 + 8), _repeat, 2, 1, 2, 16, 8, 16))
                # vadd x0+x1
            ...
            if dtype == "float161":  # do not change "float161", avoid to do fp32 to fp16
                ...
                ib.emit(
                    tvm.call_extern("float16", "copy_ubuf_to_gm", outputs.access_ptr('w', offset=out_offset),
                                    out_ub.access_ptr('r'), 0, 1, index_align_index_num, 0, 0))
            else:
                ib.emit(
                    tvm.call_extern(outputs.dtype, "copy_ubuf_to_gm", outputs.access_ptr('w', offset=out_offset),
                                    out_ub.access_ptr('r'), 0, 1, index_align_index_num * 2, 0, 0))
                if dtype == "float16":
            ...
            if x_segment_tail != 0:
                ...
                with ib.for_range(0, h_per_core) as y_loop_tail:
                    with ib.if_scope(idx * h_per_core + y_loop_tail < h_out):
                        ...
                        # caculate data
                        _process(0, x_segment_tail, y_loop_tail, x_segment_loop, src_nc1_output_offset)
...
def resize_bilinear_v2_d_compute(images,
    ...
    res = tvm.extern(tuple(shape_out), [images],
                     lambda ins, outs: _resize_bilinear_ir(ins[0], outs[0], align_corners, half_pixel_centers),
                     name="res",
                     dtype="float32")
    return res
...
def resize_bilinear_v2_d(images,
    ...
    res = resize_bilinear_v2_d_compute(image_data, y, size, align_corners, half_pixel_centers, kernel_name)
    # build & output
