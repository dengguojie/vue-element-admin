...
@util.check_input_type(dict, dict, dict, dict, (list, tuple), (list, tuple), (list, tuple), int,
                       (list, tuple), bool, str)
def max_pool_grad_with_argmax(x, grad, argmax, y, ksize, strides, pads, dtype=DT_INT32,
                              dilation=(1, 1, 1, 1), ceil_mode=False, kernel_name="max_pool_grad_with_argmaxv2"):
    """
    ...
    :param ceil_mode:
    """
    check_param(x, grad, argmax, y, ksize, strides, pads, dtype, dilation, ceil_mode,
                kernel_name)
    maxpoolgard = MaxpoolGard(grad, argmax, x, ksize, strides, pads, dilation, ceil_mode)
        ...
        with self.tik_instance.for_range(0, real_block, block_num=real_block) as block_id:
            ...
            with self.tik_instance.for_range(0, real_cycle) as cycle_id:
                ...
                with self.tik_instance.for_range(0, nc1, thread_num=1) as loopc1:
                    ...
                    data_max_ub = self.tik_instance.Tensor(dtype, (h_input * col2img_dyw * channel,),
                    ...
                    # mask define
                    data_mask_ub = self.tik_instance.Tensor("uint16", (h_input * col2img_dyw * windowh * windoww,),
                                                            name="data_mask_ub", scope=tik.scope_ubuf)
                    data_vsel_ub = self.tik_instance.Tensor(dtype, (h_input * col2img_dyw * channel,),
                                                            name="data_vsel_ub", scope=tik.scope_ubuf)
                    data_vsel_ub_fp32 = self.tik_instance.Tensor("float32", (h_input * col2img_dyw * channel,),
                        ...
                        with self.tik_instance.for_range(0, dyw) as loopw:
                            ...
                            self.tik_instance.data_move(data_max_ub,
                                                        data_input[block_num * nc1 * dyh * dyw *
                                                                   ...
                                                                   new_looph * dyw *
                                                                   channel + channel * new_loopw], constant.SID, 1,
                                                        constant.DEFAULT_BURST_LEN + w_ratio, 0, 0)
                            if h_ratio != 0:
                                self.tik_instance.data_move(data_max_ub,
                                                            data_input[block_num * nc1 * dyh * dyw *
                                                                       channel + loopc1 * dyh * dyw * channel +
                                                                       ...
                                                                       channel * new_loopw],
                                                            constant.SID, 1, constant.DEFAULT_BURST_LEN + w_ratio,
                                                            0, 0)
                            # mask copy gm to ub
                            self.tik_instance.data_move(data_mask_ub, data_mask[block_num * nc1 * mask_one_window *
                            ...
                            if hoverlap != 0:
                                self.tik_instance.data_move(data_mask_ub[col2img_dyw * windowh * windoww],
                                                            data_mask[block_num * nc1 * mask_one_window *
                                                                      windoww * windowh + loopc1 * mask_one_window *
                                                                      windoww * windowh + (new_looph + 1) * dyw +
                                                                      loopw],
                                                            constant.SID, windowh * windoww,
                                                            constant.DEFAULT_BURST_LEN,
                                                            mask_stride, constant.STRIDE_ZERO)
                            ...
                            with self.tik_instance.for_range(0, windowh * windoww) as mask_id:
                                cmpmask = self.tik_instance.mov_tensor_to_cmpmask(data_mask_ub[col2img_dyw * mask_id])
...
def _pool2d_shape_check(kernel_h, kernel_w, stride_h, stride_w, \
                        pad_h, pad_w, dilation_h, dilation_w, output_h, output_w):
...
def check_output_dim_with_ksize_stride(padding, input_gard_shape, y_shape, ksize, strides, dilation, ceil_mode):
    """
    ...
    dyh = _pooling_output_shape(input_height, windowh, pad_h, stride_h, dilation_h, ceil_mode)
    dyw = _pooling_output_shape(input_width, windoww, pad_w, stride_w, dilation_w, ceil_mode)
...
def check_param(x, grad, argmax, y, ksize, strides, padding, dtype, dilation, ceil_mode,
                kernel_name):
    """
