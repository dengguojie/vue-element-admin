...
def _check_shape_dtype_format(input_shape, input_dtype, input_format, stride_h, stride_w):
    ...
    if len(input_shape) != DIM_5HD:
        error_manager_vector.raise_err_input_value_invalid("upsample", "x", "5", str(len(input_shape)))
    ...
    if input_dtype not in check_list:
        error_manager_vector.raise_err_input_dtype_not_supported("upsample", "x", ", ".join(check_list), input_dtype)
    shape_c0 = C0
    ...
    if input_format != "NC1HWC0":
        error_manager_vector.raise_err_input_format_invalid("upsample", "x", "NC1HWC0", str(input_format))
