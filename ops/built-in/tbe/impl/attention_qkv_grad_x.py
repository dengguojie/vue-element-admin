...
class AttentionQKVGradX:
    ...
    def __init__(self, params):
        ...
        self.n1_shape = self.kernel_shape[1]
    ...
    def _tiling_args_compute(self):
        ...
        self.block_n = self.core_num // self.block_m
    ...
    def _init_gm_tensor(self, tik_instance):
        ...
        self.kernel_value_gm = tik_instance.Tensor(self.dtype, self.kernel_shape, name="kernel_value_gm",
            scope=tik.scope_gm)
        self.inputs = [self.ln_dx_gm, self.query_dx_gm, self.key_dw_gm, self.value_dw_gm,
    ...
    def _single_core_process(self, tik_instance, matmul_m_idx, matmul_n_idx):
        '''
        single core process
        '''
        cl0_shape = (self.matmul_n_l0, self.matmul_m_l0, Constant.M0, Constant.N0)
        l0c = tik_instance.Tensor(Constant.FP32_DTYPE, cl0_shape, name="l0c", scope=tik.scope_cbuf_out)
        ub_shape = (self.matmul_n_l0, self.matmul_m_l0, Constant.M0, Constant.N0)
        ...
        for i in range(Constant.KERNEL_NUM):
            if i == 0:
                self._add_ub_process(tik_instance, [matmul_m_idx, matmul_n_idx], [add_ub, add_ub_cast, l0c])
            ...
            with tik_instance.for_range(0, self.k1_shape // self.matmul_k_al1 // \
                ...
                for ping_pong in range(Constant.DOUBLE_BUFFER):
                    self._matmul_l0c_compute(tik_instance, gm_tensor_list, idx_list, ping_pong=ping_pong)
    ...
    def _add_ub_process(self, tik_instance, idx_list, tensor_list):
        '''
        add_ub process
        '''
        ...
        tik_instance.data_move(add_ub, self.ln_dx_gm[ln_src_offset:], 0, self.matmul_n_l0,
            self.matmul_m_l0 * Constant.M0, (self.m1_shape - self.matmul_m_l0) * Constant.M0, 0)
        vconv_repeat_size = self.matmul_n_l0 * self.matmul_m_l0 * Constant.FRAC_SIZE // Constant.FP32_REPEAT_SIZE
        vconv(tik_instance, add_ub, add_ub_cast, vconv_repeat_size, True)
        tik_instance.data_move(l0c, add_ub_cast, 0, 1, self.matmul_n_l0 * self.matmul_m_l0, 0, 0)
    ...
    def _matmul_l0c_compute(self, tik_instance, tensor_list, idx_list, ping_pong):
        ...
        matmul_l0c_compute
        '''
        al1_shape = (self.matmul_k_al1, self.matmul_m_l1, Constant.M0, Constant.C0)
        ...
        tik_instance.data_move(al1, x_gm[a_src_offset:], 0, self.matmul_k_al1, self.matmul_m_l1 * Constant.M0,
            (self.m1_shape - self.matmul_m_l1) * Constant.M0, 0)
        # bl1_process
        ...
        tik_instance.data_move(bl1, kernel_gm[b_src_offset:], 0, self.matmul_k_bl1,
            self.matmul_n_l1 * Constant.N0, (self.n1_shape - self.matmul_n_l1) * Constant.N0, 0)
        with tik_instance.for_range(0, self.matmul_m_l1 // self.matmul_m_l0):
            ...
            with tik_instance.for_range(0, self.matmul_m_l0) as mal0_idx:
                ...
                tik_instance.load2dv1(al0[al0_offset:], al1[mal0_idx * Constant.FRAC_SIZE:], 0, self.matmul_k_l0,
                    self.matmul_m_l0, 0, False)
            # bl0 process
            ...
            with tik_instance.for_range(0, self.matmul_n_l1 // self.matmul_n_l0):
                tik_instance.load2dv1(bl0, bl1, 0, self.matmul_n_l0 * self.matmul_k_l0, 1, 0, False)
