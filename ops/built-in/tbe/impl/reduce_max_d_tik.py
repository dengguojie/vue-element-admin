...
class Argmax(ArgmaxBase):
    ...
    def max_compute(self):
        ...
        """
        # if not need split
        self.result_gm_value = self.tik_instance.Tensor(self.dtype_x, (self.gm_result_size, ),
        ...
        with self.tik_instance.for_range(0, core_num_used, block_num=core_num_used) as n_i:
            ...
            if core_segment_tail != 0:
                ...
                with self.tik_instance.else_scope():
                    self.compute_max_last_axis(n_i, core_segment_tail, core_segment)
    ...
    def compute_max_last_axis(self, n_i, core_segment, segment_core):
        ...
        def _run(segment_len, segment_index):
            with self.tik_instance.for_range(0, segment_len) as core_i:
                ...
                self.tik_instance.vector_dup(self.data_each_vector, self.ub_result_64, self.minest_value, 1, 1, 8)
                if loop_times != 0:
                    thread_num = 1
                    if loop_times > 2:
                        thread_num = 2
    ...
    def get_half_max(self, tik_instance, vmax_ub, vector_num):
        ...
        tik_instance.vmax(64, vmax_ub, vmax_ub, vmax_ub[ub2_offset], output_num, 1, 1, 1, 8, 8, 8)
    ...
    def do_max_last_axis_fp32(self, ub_buf_size, loop, n_i):
        ...
        total_block = repeat
        while total_block > 1:
            total_block = self.get_half_max(self.tik_instance, ub_data, total_block)

        self.tik_instance.vmax(self.data_each_vector, self.ub_result_64, ub_data, self.ub_result_64, 1, 1, 1, 1, 0, 8,
                               0)
    ...
    def get_one_from_64(self, mode=None):
        ...
        self.tik_instance.vmax(32, ub_block_0, ub_block_0, ub_block_4, 1, 1, 1, 1, 8, 8, 8)
        self.tik_instance.vmax(16, ub_block_4, ub_block_0, ub_block_2, 1, 1, 1, 1, 8, 8, 8)
        self.tik_instance.vmax(8, ub_block_0, ub_block_4, ub_block_5, 1, 1, 1, 1, 8, 8, 8)
        index_reg = [self.tik_instance.Scalar(dtype=self.dtype_x) for _ in range(8)]
        for i in range(7):
        ...
        self.tik_instance.vmax(1, ub_block_0, ub_block_0, ub_block_1, 1, 1, 1, 1, 8, 8, 8)
        self.tik_instance.vmax(1, ub_block_2, ub_block_2, ub_block_3, 1, 1, 1, 1, 8, 8, 8)
        self.tik_instance.vmax(1, ub_block_4, ub_block_4, ub_block_5, 1, 1, 1, 1, 8, 8, 8)
        self.tik_instance.vmax(1, ub_block_6, ub_block_6, ub_block_7, 1, 1, 1, 1, 8, 8, 8)
        self.tik_instance.vmax(1, ub_block_0, ub_block_0, ub_block_2, 1, 1, 1, 1, 8, 8, 8)
        self.tik_instance.vmax(1, ub_block_4, ub_block_6, ub_block_4, 1, 1, 1, 1, 8, 8, 8)
        self.tik_instance.vmax(1, ub_block_0, ub_block_0, ub_block_4, 1, 1, 1, 1, 8, 8, 8)
