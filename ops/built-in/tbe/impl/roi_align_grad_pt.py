...
def _clear_ub(tik_instance, dst_ub):
    ...
    None
    """
    # 64: float32 mask, 255: repeat_times
    shape = dst_ub.shape
    data_len = 1
    for i in shape:
        data_len = data_len * i
...
def _get_roi_pos(tik_instance, rois_start_w, rois_start_h, rois_end_w, rois_end_h, scale, roi_end_mode):
    if roi_end_mode == 0:
        tik_instance.vmuls(64, rois_start_w, rois_start_w, scale, 2, 1, 1, 8, 8)
        tik_instance.vmuls(64, rois_start_h, rois_start_h, scale, 2, 1, 1, 8, 8)
        tik_instance.vmuls(64, rois_end_w, rois_end_w, scale, 2, 1, 1, 8, 8)
        tik_instance.vmuls(64, rois_end_h, rois_end_h, scale, 2, 1, 1, 8, 8)
    else:
        tik_instance.vmuls(64, rois_start_w, rois_start_w, scale, 2, 1, 1, 8, 8)
        tik_instance.vadds(64, rois_start_w, rois_start_w, -0.5, 2, 1, 1, 8, 8)
        tik_instance.vmuls(64, rois_start_h, rois_start_h, scale, 2, 1, 1, 8, 8)
        tik_instance.vadds(64, rois_start_h, rois_start_h, -0.5, 2, 1, 1, 8, 8)
        tik_instance.vmuls(64, rois_end_w, rois_end_w, scale, 2, 1, 1, 8, 8)
        tik_instance.vadds(64, rois_end_w, rois_end_w, -0.5, 2, 1, 1, 8, 8)
        tik_instance.vmuls(64, rois_end_h, rois_end_h, scale, 2, 1, 1, 8, 8)
        tik_instance.vadds(64, rois_end_h, rois_end_h, -0.5, 2, 1, 1, 8, 8)


def _roi_align_calc_scale_batch(tik_instance, rois_data_ub, scale, pool_w,
                                pool_h, roi_end_mode, sample_num=None):
    """calc one block gradient
    """
...
def roi_align_grad_compute(tik_instance, y_diff, rois_data, x_diff, rois_n,
                           ...
                           sample_num, core_bias, roi_end_mode):
    """calc one block gradient
    ...
    with tik_instance.for_range(0, rois_batch_num) as i:
        ...
        sample_num_h, rois_start_w, rois_start_h, rois_index \
            = _roi_align_calc_scale_batch(tik_instance, rois_data_ub,
                                          spatial_scale,
                                          pooled_width, pooled_height, roi_end_mode,
                                          sample_num)
...
def roi_align_grad_compute_multicore(
        ...
        spatial_scale, sample_num, output_grad_shape, roi_end_mode, kernel_name):
    """calc one block gradient
    ...
    if rois_n in (1280, 256) and sample_num == 2 and c1_num == 16:
        with tik_instance.for_range(0, core_num, block_num=core_num) as core_index:
            ...
            roi_align_grad_compute_align(tik_instance, input_grad, rois_data, output_grad,
                                         ...
                                         spatial_scale, sample_num, core_bias, roi_end_mode)
    else:
        with tik_instance.for_range(0, core_num, block_num=core_num) as core_index:
...
def roi_align_grad_pt(y_diff, rois, x_diff, pooled_width, pooled_height, spatial_scale, sample_num, roi_end_mode,
                      kernel_name="roi_align_grad_pt"):
    """
    algorithm: floor
