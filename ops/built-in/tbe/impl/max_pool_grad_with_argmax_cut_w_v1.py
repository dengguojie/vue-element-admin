...
from impl.max_pool_grad_with_argmax_cut_h_v1 import MaxpoolGradBase
...
class MaxpoolGardObject(MaxpoolGradBase):
    ...
    def tik_instance_cut_nc1_cut_w(self, kernel_name):
        ...
        while col2img_w * col2img_h * channel * dtype_size > self.ub_limit:
            ...
            col2img_w = wo_max * stridew if woverlap == 0 else (wo_max - 1) * stridew + windoww
        if wo_max == 0:
            raise RuntimeError("The shape or ksize or stride is too large and not supported, please check!")
        ...
        with self.tik_instance.for_range(0, real_block, block_num=real_block) as block_id:
            ...
            with self.tik_instance.for_range(0, real_cycle) as cycle_id:
                ...
                with self.tik_instance.for_range(0, nc1) as loopc1:
                    ...
                    # mask define
                    data_mask_ub = self.tik_instance.Tensor("uint16", (ho_max * wo_max,), name="data_mask_ub",
                                                            scope=tik.scope_ubuf)
                    ...
                    with self.tik_instance.for_range(0, h_cycle) as looph:
                        ...
                        with self.tik_instance.for_range(0, w_cycle) as loopw:
                            ...
                            self.tik_instance.data_move(
                                data_max_ub,
                                data_input[(block_num * nc1 * dyh + loopc1 * dyh + new_looph) * dyw *
                                           channel + new_loopw * channel],
                                constant.SID, ho_max, in_burstlen, dyw - in_burstlen, wo_max - in_burstlen)
                            ...
                            with self.tik_instance.for_range(0, windowh * windoww) as mask_id:
                                with self.tik_instance.for_range(0, ho_max) as cycle:
                                    ...
                                    self.tik_instance.data_move(
                                        ...
                                        data_mask[block_num * nc1 * mask_one_window * windoww * windowh +
                                                  loopc1 * mask_one_window * windoww * windowh +
                                                  (new_looph + cycle) * dyw + mask_one_window * mask_id + new_loopw],
                                        constant.SID, 1, (in_burstlen + 15) // 16, 0, 0)
                                data_vsel_ub = self.tik_instance.Tensor(
                                    dtype, (ho_max * wo_max * channel,), name="data_vsel_ub", scope=tik.scope_ubuf)
                                data_vsel_ub_fp32 = \
                                    self.tik_instance.Tensor("float32", (ho_max * wo_max * channel,),
                                ...
                                self.tik_instance.col2img(
                                    ...
                                    col2img_h, col2img_w, fetch_filter_w, fetch_filter_h, left_top_w, left_top_h,
                                    stridew, strideh, windoww, windowh, 1, 1, ho_max * wo_max // 16)
                            ...
                            if woverlap == 0:
                                ...
                                with self.tik_instance.else_scope():
                                    ...
                                    dst_address.set_as(dst_address + (loopw * col2img_w - pad_left) * channel)
                                    with self.tik_instance.if_scope(loopw == w_cycle - 1):
                                ...
                                with self.tik_instance.else_scope():
                                    ...
                                    dst_address.set_as(dst_address + ((loopw - 1) * stridew * (wo_max - 1) +
    ...
    def tik_instance_cut_nc1h_cut_w(self, kernel_name):
        ...
        v_rep_time_col_last = (2 * (col2img_w * channel * col2img_h_last + 64) * dtype_size) // ONE_REPEAT
        ...
        v_rep_time_col_every = (2 * (col2img_w * channel * col2img_h_every + 64) * dtype_size) // ONE_REPEAT
            ...
            with self.tik_instance.for_range(0, real_cycle) as cycle_id:
                ...
                with self.tik_instance.if_scope(block_h == ho_count - 1):
                    data_max_ub = self.tik_instance.Tensor(
                        dtype, (ho_max_last * wo_max * channel,), name="data_max_ub", scope=tik.scope_ubuf)
                    data_vmul_ub_col2img_fp32 = \
                        self.tik_instance.Tensor("float32", (col2img_w * channel * col2img_h_last + 64,),
                                                 name="data_vmul_ub_col2img_fp32", scope=tik.scope_ubuf)
                    data_vmul_ub_col2img_fp16 = \
                        self.tik_instance.Tensor(dtype, (col2img_w * channel * col2img_h_last + 128,),
                                                 name="data_vmul_ub_col2img_fp16", scope=tik.scope_ubuf)
                    # mask define
                    ...
                    with self.tik_instance.for_range(0, h_cycle_last) as looph:
                        ...
                        if hoverlap == 0:
                            ...
                            in_src_address.set_as(in_src_address + (block_h * ho_every + new_looph) * dyw * channel)
                            mask_address.set_as(mask_address + (block_h * ho_every + new_looph) * dyw)
                        else:
                            with self.tik_instance.if_scope(looph != 0):
                            ...
                            in_src_address.set_as(
                                in_src_address + (block_h * (ho_every - 1) + new_looph) * dyw * channel)
                            mask_address.set_as(mask_address + (block_h * (ho_every - 1) + new_looph) * dyw)
                        new_loopw = self.tik_instance.Scalar("int32")
                        new_loopw.set_as(0)
                        in_burstlen = self.tik_instance.Scalar("int32")
                        in_burstlen.set_as(wo_max)
                        with self.tik_instance.for_range(0, w_cycle) as loopw:
                            ...
                            self.tik_instance.data_move(data_max_ub,
                                                        data_input[in_src_address + new_loopw * channel],
                                                        constant.SID, in_nburst, in_burstlen, dyw - in_burstlen,
                                                        wo_max - in_burstlen)
                            ...
                            with self.tik_instance.for_range(0, windowh * windoww) as mask_id:
                                with self.tik_instance.for_range(0, in_nburst) as cycle:
                                    ...
                                    self.tik_instance.data_move(
                                        data_mask_ub[cycle * wo_max],
                                        data_mask[mask_address + cycle * dyw + mask_one_window * mask_id + new_loopw],
                                        constant.SID, 1, (in_burstlen + 15) // 16, 0, 0)
                                data_vsel_ub = self.tik_instance.Tensor(
                                ...
                                data_vsel_ub_fp32 = \
                                    self.tik_instance.Tensor(
                                        "float32", (ho_max_last * wo_max * channel,),
                                        name="data_vsel_ub_fp32", scope=tik.scope_ubuf)
                                ...
                                self.tik_instance.col2img(data_vmul_ub_col2img_fp32[0], data_vsel_ub_fp32[0],
                                                          (0, 0, 0, 0), col2img_h_last, col2img_w, fetch_filter_w,
                                                          fetch_filter_h, left_top_w, left_top_h,
                                                          stridew, strideh, windoww, windowh, dilate_w, dilate_h,
                            ...
                            if hoverlap == 0:
                                ...
                                dst_address.set_as(dst_address + (ho_count - 1) * ho_every * strideh *
                                                   dxw * channel + looph * ho_max_last *
                                                   strideh * dxw * channel -
                                                   pad_top * dxw * channel)
                                with self.tik_instance.if_scope(looph == h_cycle_last - 1):
                                    output_cuthline.set_as(dxh - ho_every * (ho_count - 1) * strideh -
                                ...
                                dst_address.set_as(dst_address +
                                                   ((block_h * (ho_every - 1) + looph * (ho_max_last - 1) + 1) *
                            ...
                            self.tik_instance.data_move(data_output[dst_address],
                                                        ...
                                                        dxw - burst_len)
                with self.tik_instance.else_scope():
                    data_max_ub = self.tik_instance.Tensor(dtype, (ho_max_every * wo_max * channel,),
                                                           name="data_max_ub", scope=tik.scope_ubuf)
                    data_vmul_ub_col2img_fp32 = \
                    ...
                    with self.tik_instance.for_range(0, h_cycle_every) as looph:
                        ...
                        mask_address.set_as(block_batch * mask_one_window * windoww * windowh)
                        if hoverlap == 0:
                            in_src_address.set_as(in_src_address + (block_h * ho_every + looph * ho_max_every) *
                                                  dyw * channel)
                            mask_address.set_as(mask_address + (block_h * ho_every + looph * ho_max_every) * dyw)
                        ...
                        with self.tik_instance.for_range(0, w_cycle) as loopw:
                            ...
                            self.tik_instance.data_move(
                                data_max_ub,
                                data_input[in_src_address + new_loopw * channel],
                                constant.SID, in_nburst, in_burstlen, dyw - in_burstlen, wo_max - in_burstlen)
                            ...
                            with self.tik_instance.for_range(0, windowh * windoww) as mask_id:
                                with self.tik_instance.for_range(0, in_nburst) as cycle:
                                    ...
                                    self.tik_instance.data_move(
                                        ...
                                        constant.SID, 1, (in_burstlen + 15) // 16, 0, 0)
                                data_vsel_ub = self.tik_instance.Tensor(
                                    dtype, (ho_max_every * wo_max * channel,), name="data_vsel_ub",
                                    scope=tik.scope_ubuf)
                                data_vsel_ub_fp32 = \
                                    self.tik_instance.Tensor(
                                        "float32", (ho_max_every * wo_max * channel,),
                                        name="data_vsel_ub_fp32", scope=tik.scope_ubuf)
                            ...
                            if hoverlap == 0:
                                ...
                                dst_address.set_as(dst_address + ((block_h * ho_every + looph * ho_max_every) *
                                                                  strideh - pad_top) * dxw * channel)
                                ...
                                with self.tik_instance.if_scope(looph == h_cycle_every - 1):
                                    output_cuthline.set_as((ho_every - looph * (ho_max_every - 1) - 1) * strideh)
                                dst_address.set_as(dst_address +
                                                   ((block_h * (ho_every - 1) + (looph - 1) * (ho_max_every - 1) +
                                                     ho_max_every) * strideh - pad_top) * dxw * channel)
                                with self.tik_instance.if_scope(block_h == 0):
                                    with self.tik_instance.if_scope(looph == 0):
                            ...
                            self.tik_instance.data_move(data_output[dst_address],
                                                        data_vmul_ub_col2img_fp16[src_address],
                                                        constant.SID, output_cuthline, burst_len,
                                                        col2img_w - burst_len, dxw - burst_len)
        self.tik_instance.BuildCCE(kernel_name=kernel_name,
                                   inputs=(data_input_origin, data_input, data_mask),
                                   outputs=data_output, enable_l2=False)
        return self.tik_instance
