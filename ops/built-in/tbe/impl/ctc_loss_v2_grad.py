...
def check_supported(grad_out, log_probs, targets, input_lengths, target_lengths, neg_log_likelihood, log_alpha, grad,
    ...
    if (shape_log_alpha[2] - 1) // 2 > Constant.LABEL_MAX:
...
class CTCLossV2Grad():
    ...
    Modify : 2021-5-26
    """
    def __init__(self, log_probs, targets, log_alpha, blank, kernel_name):
        self.tik_instance = tik.Tik(tik.Dprofile())
        self.kernel_name = kernel_name

        shape_targets = targets.get("shape")
        self.sum_targets = functools_reduce(lambda x, y: x * y, shape_targets)
        self.mode_align = True if self.sum_targets == shape_targets[0] else False

        params = self.paras_check(log_probs, log_alpha, kernel_name)
        self.blank = blank
        self.T = params[0]
        self.N = params[1]
        self.C = params[2]
    ...
    @staticmethod
    def paras_check(log_probs, log_alpha, kernel_name):
        """
        ...
        return [shape_log_probs[0], shape_log_probs[1], shape_log_probs[2], (shape_log_alpha[2] - 1) // 2]
    ...
    def ctc_loss_grad_compute(self):
        ...
        with self.tik_instance.if_scope(self.C % Constant.BLOCK != 0):
            self.move_out()
    ...
    def ctc_loss_grad_compute_core(self, task_idx):
        ...
        self.tik_instance.vector_dup(Constant.BLOCK, grad_ub[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds],
                                     0,
                                     (self.C_BLOCK % (Constant.BLOCK * Constant.REPEAT_OFFSET)) // Constant.BLOCK,
                                     1, 1)
        ...
        if self.mode_align:
            ...
            offset.set_as(target_length_ub[0])
        ...
        with self.tik_instance.for_range(0, self.rounds) as j:
            self.tik_instance.vector_dup(Constant.BLOCK, copy_ub_zero[(Constant.BLOCK * Constant.REPEAT_OFFSET) * j],
                                         0, Constant.REPEAT_OFFSET, 1, 1)
        self.tik_instance.vector_dup(Constant.BLOCK,
                                     copy_ub_zero[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds], 0,
                                     (self.C_BLOCK % (Constant.BLOCK * Constant.REPEAT_OFFSET)) // Constant.BLOCK,
                                     1, 1)
        ...
        with self.tik_instance.for_range(t_i, self.T) as t:
            with self.tik_instance.if_scope(self.C % Constant.BLOCK != 0):
                self.tik_instance.data_move(self.grad_[t * self.N * self.C_BLOCK + task_idx * self.C_BLOCK],
                                            copy_ub_zero[0], 0, 1, self.C_BLOCK // Constant.BLOCK, 1, 1)
        ...
        self.tik_instance.data_move(log_probs_ub[0], self.log_probs[self.C * task_idx + self.N * self.C * (t_i - 1)],
                                    0, 1, self.C_BLOCK // Constant.BLOCK, 0, 0)
        ...
        self.tik_instance.vec_exp(Constant.BLOCK, copy_ub_a[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds],
                                  log_probs_ub[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds],
        ...
        with self.tik_instance.for_range(0, self.rounds) as j:
            self.tik_instance.vec_muls(Constant.BLOCK, copy_ub_b[(Constant.BLOCK * Constant.REPEAT_OFFSET) * j],
                                       copy_ub_a[(Constant.BLOCK * Constant.REPEAT_OFFSET) * j], grad_out,
                                       Constant.REPEAT_OFFSET, 1, 1)
        self.tik_instance.vec_muls(Constant.BLOCK, copy_ub_b[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds],
                                   copy_ub_a[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds], grad_out,
                                   (self.C_BLOCK % (Constant.BLOCK * Constant.REPEAT_OFFSET)) // Constant.BLOCK, 1, 1)

        with self.tik_instance.if_scope(self.C % Constant.BLOCK != 0):
            self.tik_instance.data_move(self.grad_[t * self.N * self.C_BLOCK + task_idx * self.C_BLOCK],
                                        copy_ub_b[0], 0, 1, self.C_BLOCK // Constant.BLOCK, 1, 1)
        with self.tik_instance.else_scope():
            self.tik_instance.data_move(self.grad[t * self.N * self.C + task_idx * self.C],
                                        copy_ub_b[0], 0, 1, self.C_BLOCK // Constant.BLOCK, 1, 1)
        ...
        with self.tik_instance.for_range(1, t_i):
            ...
            with self.tik_instance.for_range(0, self.rounds) as j:
                self.tik_instance.vector_dup(Constant.BLOCK, grad_ub[(Constant.BLOCK * Constant.REPEAT_OFFSET) * j],
                                             0, Constant.REPEAT_OFFSET, 1, 1)
            self.tik_instance.vector_dup(Constant.BLOCK,
                                         grad_ub[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds], 0,
                                         (self.C_BLOCK % (Constant.BLOCK * Constant.REPEAT_OFFSET)) // Constant.BLOCK,
                                         1, 1)

            self.tik_instance.data_move(log_probs_ub[0], self.log_probs[self.C * task_idx + self.N * self.C * t], 0,
                                        1, self.C_BLOCK // Constant.BLOCK, 0, 0)
            ...
            with self.tik_instance.for_range(start, end_loop) as s:
                ...
                offset.set_as((s - start) * Constant.BLOCK)
            ...
            with self.tik_instance.for_range(0, self.rounds) as j:
                self.tik_instance.vec_exp(Constant.BLOCK, copy_ub_a[(Constant.BLOCK * Constant.REPEAT_OFFSET) * j],
                                          log_probs_ub[(Constant.BLOCK * Constant.REPEAT_OFFSET) * j],
                                          Constant.REPEAT_OFFSET, 1, 1)
            self.tik_instance.vec_exp(Constant.BLOCK,
                                      copy_ub_a[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds],
                                      log_probs_ub[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds],
                                      (self.C_BLOCK % (Constant.BLOCK * Constant.REPEAT_OFFSET)) // Constant.BLOCK,
                                      1, 1)
            ...
            with self.tik_instance.for_range(0, self.rounds) as j:
                self.tik_instance.vec_muls(Constant.BLOCK, copy_ub_b[(Constant.BLOCK * Constant.REPEAT_OFFSET) * j],
                                           copy_ub_a[(Constant.BLOCK * Constant.REPEAT_OFFSET) * j],
                                           grad_out, Constant.REPEAT_OFFSET, 1, 1)
            self.tik_instance.vec_muls(Constant.BLOCK,
                                       copy_ub_b[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds],
                                       copy_ub_a[(Constant.BLOCK * Constant.REPEAT_OFFSET) * self.rounds], grad_out,
                                       (self.C_BLOCK % (Constant.BLOCK * Constant.REPEAT_OFFSET)) // Constant.BLOCK,
                                       1, 1)
...
def ctc_loss_v2_grad(grad_out, log_probs, targets, input_lengths, target_lengths, neg_log_likelihood, log_alpha, grad,
    ...
    op_obj = CTCLossV2Grad(log_probs, targets, log_alpha, blank, kernel_name)
