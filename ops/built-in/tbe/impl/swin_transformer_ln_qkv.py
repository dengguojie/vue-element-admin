...
class TilingArgs:
    ...
    def __init__(self, batch_num, m_0, m_1, m_2, m_3, k_0, k_1, n_0, n_1, n_2, n_3, dtype):
...
class SwinTransformLnMatmul(TilingArgs):
    ...
    def __init__(self, batch_num, m_0, m_1, m_2, m_3, k_0, k_1, n_0, n_1, n_2, n_3, dtype,
                 m_split, epsilon, kernel_name):
        # init tiling param
        super().__init__(batch_num, m_0, m_1, m_2, m_3, k_0, k_1, n_0, n_1, n_2, n_3, dtype)
        # init attr
            ...
            if n_1_num_each_loop == self.n_2 * self.n_3:
                ...
                for i in range(self.m_2):
                    self.tik_inst.data_move(result_gm[batch_index, m_2_1 * self.m_2 + m_2_0 + i,
                                            ...
                                            n_1_num_each_loop, m_3_0 * self.m_3,
                                            0, self.m_3 * self.m_3 - m_3_0 * self.m_3)
        else:
...
def swin_transformer_ln_qkv(x, gamma, beta, weight, bias,
    ...
    n_2 = result_shape[1]
    n_3 = result_shape[2]
    m_3 = int(math.sqrt((result_shape[3] * m_0)))
    dtype = x.get("dtype")
    if shifts:
        m_split = shifts[1]
    else:
        m_split = 0
    obj = SwinTransformLnMatmul(batch_num, m_0, m_1, m_2, m_3, k_0, k_1, n_0, n_1, n_2, n_3, dtype,
                                m_split, epsilon, kernel_name)
    obj.start_compute()
