...
def apply_adam_d_compute(var, m, v, beta1_power, beta2_power, lr, beta1, beta2,
    ...
    """
    inp_dtype = var.dtype
    var = tbe.vadds(var, tvm.const(0.0, dtype=inp_dtype))
    ...
    vmul_grad = tbe.vmul(grad, grad)
    vsub_v = tbe.vsub(vmul_grad, v)
    ...
    if use_nesterov is True:
        ...
        vmul_g_beta1 = tvm.compute(grad.shape,
                                   ...
                                   tag="elewise_single_VS_mul")
        vadd_m_v = tbe.vadd(vmul_g_beta1, vmul_m_beta1)
        vmul_alpha = tvm.compute(vadd_m_v.shape,
                                 lambda *indices: vadd_m_v(*indices) * alpha[0],
                                 tag="elewise_single_VS_mul")
        vdiv_beta1_sqrt = tbe.vdiv(vmul_alpha, vadd_sqrt)
        var = tbe.vsub(var, vdiv_beta1_sqrt)
    else:
        vmul_sqrt = tvm.compute(m.shape,
                                lambda *indices: m(*indices) * alpha[0],
                                tag="elewise_single_VS_mul")
        vdiv_beta1_sqrt = tbe.vdiv(vmul_sqrt, vadd_sqrt)
        var = tbe.vsub(var, vdiv_beta1_sqrt)
    res1 = tbe.vadds(var, tvm.const(0.0, dtype=inp_dtype))
    res2 = tbe.vadds(m, tvm.const(0.0, dtype=inp_dtype))
...
def apply_adam_d(var, m, v, beta1_power, beta2_power, lr, beta1, beta2, epsilon,
    ...
    args = util_apply_op_schedule.ApplyOpConfig.TensorArgs(input_dict, apply_adam_d_compute,
                                                           [var_out, m_out, v_out], 15)
    name = util_apply_op_schedule.ApplyOpConfig.TensorName(all=('var', 'm', 'v', 'beta1_power',
