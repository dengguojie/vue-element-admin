...
# ============================================================================
"""
max_pool_grad_with_argmax_v1_resnet50
"""
from te import tik
...
class MaxpoolGradV2Resnet50(object):
    ...
    def tik_instance_cut_nc1_cut_h(self, kernel_name):
        ...
        each_core_num = batch_num // self.blocknum
        if each_core_num == 0:
            each_core_num = 1
        c0_dim = 16
        ...
        batch_id = self.tik_instance.Scalar("int32", name="batch_id")

        with self.tik_instance.for_range(0, self.blocknum, block_num=self.blocknum) as loop_n:
            with self.tik_instance.for_range(0, each_core_num) as loop_core:
                batch_id.set_as((loop_n * each_core_num + loop_core) % batch_num)
                ...
                with self.tik_instance.for_range(0, loop_c) as loopc:  # 1
                    with self.tik_instance.for_range(0, loop_h // 2) as looph:  # 7 for ping pong buffer
                        ...
                        self.tik_instance.data_move(data_max_pool_input_ub[0],
                                                    data_input[batch_id * input_size +
                                                               loopc * loop_h *
                                                               input_block_size +
                                                               looph * 2 *
                                                               input_block_size],
                        ...
                        data_output_idx = batch_id * output_size + loopc * loop_h * \
                        ...
                        self.tik_instance_vconv_fp32(data_grad_ub, ub_fm_fp32_buf, output_block_algn_len)
                        data_output_idx = \
                            batch_id * output_size + loopc * loop_h * output_block_size + \
                        ...
                        with self.tik_instance.for_range(0, block_h * 2 + 1) as loop_i:
                            self.tik_instance.data_move(data_output[data_output_idx + loop_i * output_w * c0_dim],
                                                        data_grad_ub[
                                                            grad_address_offset + loop_i * alg_output_w * c0_dim],
                                                        0, 1, output_w, 0, 0)
...
def is_valid_shape(resnet50shape, shape):
    """
    compare two input shapes is equal.
    """
