...
class RoiExtractor:
    def __init__(self, feats, rois, roi_feats,
                 ...
                 spatial_scale=(4, 8, 16, 32), pooled_h=7, pooled_w=7,
    ...
    def map_roi_levels(self, target_lvls):
        ...
        loop = self.rois_total_num // ALIGN_LEN
        tail = self.rois_total_num % ALIGN_LEN
        ...
        def inner_compute(offset):
            if self.rois_dtype == "float16":
                j_value = self.tik_instance.Scalar(dtype="float16")
                with self.tik_instance.for_range(0, 128) as j:
                    j_value.set_as(rois_ub[j, 4])
                    y2_fp16[j].set_as(j_value)
                self.tik_instance.vextract(x1_fp16[0], rois_ub[0], 8, 1)
                self.tik_instance.vextract(y1_fp16[0], rois_ub[0], 8, 2)
                self.tik_instance.vextract(x2_fp16[0], rois_ub[0], 8, 3)
                ...
                if support_vextract is False:
                    j_value = self.tik_instance.Scalar(dtype="float32")
                    with self.tik_instance.for_range(0, 128) as j:
                        j_value.set_as(rois_ub[j, 1])
                        x1_fp32[j].set_as(j_value)
                        j_value.set_as(rois_ub[j, 2])
                        y1_fp32[j].set_as(j_value)
                        j_value.set_as(rois_ub[j, 3])
                        x2_fp32[j].set_as(j_value)
                        j_value.set_as(rois_ub[j, 4])
                        y2_fp32[j].set_as(j_value)
                else:
                    self.tik_instance.vextract(x1_fp32[0], rois_ub[0], 8, 1)
                    self.tik_instance.vextract(y1_fp32[0], rois_ub[0], 8, 2)
                    self.tik_instance.vextract(x2_fp32[0], rois_ub[0], 8, 3)
        ...
        if loop > 0:
            with self.tik_instance.for_range(0, loop) as loop_i:
                self.tik_instance.data_move(rois_ub_n5[0, 0], self.data_rois[loop_i * ALIGN_LEN, 0],
                                            0, 1, 40 * n_burst, 0, 0)
...
def roi_extractor(feats, rois, roi_feats,
                  finest_scale=56, roi_scale_factor=0, spatial_scale=(1./4, 1./8, 1./16, 1./32),
