...
def _sigmoid_compute(input_x):
    ...
    if dtype == "float32" and not mul_support:
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME,
...
def _get_tiling(in_x, hidden_size):
    ...
    while hidden_size % n_cut != 0 and n_cut != 1: # SIMEN start
        n_cut -= 1  # SIMEN end
...
def _check_dtype(x, weight_input, weight_hidden, bias_input, bias_hidden,
    ...
    def _check_equal_bias_dtype(tensor_dict, name):
        if tensor_dict["dtype"] != bias_dtype:
            error_manager_vector.raise_err_inputs_dtype_not_equal(OPERATION_NAME, "y", name, bias_dtype,
                                                                  tensor_dict["dtype"])
...
def _check_param(x, weight_input, weight_hidden, bias_input, bias_hidden, seq_length,
    ...
    if x["shape"][0] != output_h["shape"][0]:
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "x.shape[0] == output_h.shape[0]",
                                                          "output_h.shape[0]", output_h["shape"][0])

    # batch_size
    if x["shape"][2] != output_h["shape"][2]:
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "x.shape[2] == output_h.shape[2]",
                                                          "output_h.shape[2]", output_h["shape"][2])

    if seq_length is not None: # SIMEN
    ...
    if weight_input["shape"][0] != x["shape"][1]:
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "weight_input.shape[0] == x.shape[1]",
                                                          "weight_input.shape[0]", weight_input["shape"][0])
    if weight_hidden["shape"][0] != output_h["shape"][1]:
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "weight_hidden.shape[0] == output_h.shape[1]",
    ...
    if not operator.eq(output_h["shape"], y["shape"]):
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "y.shape == output_h.shape",
                                                          "y.shape", str(y["shape"]))

    if update is not None and not operator.eq(output_h["shape"], update["shape"]):
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "update.shape == output_h.shape",
                                                          "update.shape", str(update["shape"]))

    if reset is not None and not operator.eq(output_h["shape"], reset["shape"]):
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "reset.shape == output_h.shape",
                                                          "reset.shape", str(reset["shape"]))

    if new is not None and not operator.eq(output_h["shape"], new["shape"]):
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "new.shape == output_h.shape",
                                                          "new.shape", str(new["shape"]))

    if hidden_new is not None and not operator.eq(output_h["shape"], hidden_new["shape"]):
        error_manager_vector.raise_err_check_params_rules(OPERATION_NAME, "hidden_new.shape == output_h.shape",
                                                          "hidden_new.shape", str(hidden_new["shape"]))


# 'pylint: disable=too-many-arguments,too-many-branches,too-many-locals
def check_augru_attr(op_name, direction, cell_depth, keep_prob,
...
def dynamic_augru(x, weight_input, weight_hidden, weight_att, bias_input, bias_hidden, seq_length, init_h,
                   y, output_h, update, updte_att, reset, new, hidden_new,
                   direction="UNIDIRECTIONAL", cell_depth=1, keep_prob=1.0,
                   cell_clip=-1.0, num_proj=0, time_major=True, activation="tanh",
                   gate_order="zrh", reset_after=True, is_training=True, kernel_name="dynamic_augru"):
    """
    interface of op
    :return:
    """
    ...
    _check_param(x, weight_input, weight_hidden, bias_input, bias_hidden, seq_length,
                 y, output_h, update, reset, new, hidden_new)
    check_augru_attr(OPERATION_NAME, direction, cell_depth, keep_prob,
                      cell_clip, num_proj, time_major, activation, gate_order, reset_after)
...
def _solution(x, bias_input, bias_hidden, seq_length, init_h, y, update, gate_order, kernel_name, is_sync, reuse_type):
    ...
    shape_att = (t_size, 1, m_size, 16, 1) # SIMEN
    shape_i = (1, hidden_size, m_size, 16, 16) # SIMEN
    ...
    weight_att = tik_instance.Tensor(shape=shape_att, dtype=input_dtype, scope=tik.scope_gm, name="weight_att")
    if has_bias_input:
    ...
    if is_gate_output:
        ...
        i_t_weight_gm = tik_instance.Tensor(shape=shape_h, dtype=bias_dtype, scope=tik.scope_gm, name="i_t_weight_gm")
    ...
    build_input_list = [input_x, weight1, weight2, weight_att]
    if has_bias_input:
    ...
    if is_gate_output:
        build_output_list.extend([i_t_gm, i_t_weight_gm, r_t_gm, n_t_gm, hn_t_gm])
    ...
    with tik_instance.for_range(0, loop_t) as i:
        ...
        weight_att_var = weight_att[i * sub_t: i * sub_t + sub_t, :, :, :, :]
        if is_global_init:
...
def _dynamic_augru_inner(input_list, custom_list):
    """
    ...
    weight_att = input_list[3]
    bias1 = input_list[4]
    bias2 = input_list[5]
    s_init_h_gm = input_list[6]
    s_state_h_gm_last = input_list[7]
    sync = input_list[8]
    ...
    shape_weight_att = weight_att.shape
    ...
    # SIMEN START
    weight_att_ub = tvm.compute(shape_weight_att, lambda *indices: weight_att(*indices),
                                name="weight_ub", tag="out_to_ub")
    weight_att_ub_fp32 = weight_att_ub
    if fp16_input_output:
        weight_att_ub_fp32 = tvm.compute(shape_weight_att,
                                         lambda *indices: weight_att_ub(*indices).astype("float32"),
                                         name="weight_att_ub_fp32_ign",
                                         tag="elewise_single_cast")
    weight_att_bc_ub = broadcast(weight_att_ub_fp32, shape_i)
    ...
    # SIMEN START
    i_t_weight_origin = vmul(i_t_mid, weight_att_bc_ub)
    i_t_weight = vsub(i_t_mid, i_t_weight_origin)
    i_t_weight_mid = i_t_weight
    if is_gate_output:
        if fp16_input_output:
            i_t_weight_fp16 = tvm.compute(shape_i,
                                          lambda *indices: i_t_weight(*indices).astype("float16"),
                                          name="i_t_weight_fp16_ign",
                                          tag="elewise_single_cast")
            i_t_weight_gm = tvm.compute(shape_i,
                                        lambda *indices: i_t_weight_fp16(*indices),
                                        name="i_t_weight_gm",
                                        tag="ub_to_out")
            i_t_weight_gm_back = tvm.compute(shape_i,
                                             lambda *indices: i_t_weight_gm(*indices),
                                             name='i_t_weight_gm_back',
                                             tag="out_to_ub")
            i_t_weight_gm_back_fp32 = tvm.compute(shape_i,
                                                  lambda *indices: i_t_weight_gm_back(*indices).astype("float32"),
                                                  name="i_t_weight_gm_back_fp32_ign",
                                                  tag="elewise_single_cast")
            i_t_weight_mid = i_t_weight_gm_back_fp32
        else:
            i_t_weight_gm = tvm.compute(shape_i,
                                        lambda *indices: i_t_weight(*indices),
                                        name="i_t_weight_gm",
                                        tag="ub_to_out")
            i_t_weight_gm_back = tvm.compute(shape_i,
                                             lambda *indices: i_t_weight_gm(*indices),
                                             name="i_t_weight_gm_back",
                                             tag="out_to_ub")
            i_t_weight_mid = i_t_weight_gm_back
    # SIMEN END
    ...
    c_t_tmp2 = vmul(c_t_tmp1, i_t_weight_mid)
    ...
    if is_gate_output:
        ...
        output_list.append(i_t_weight_gm)
    ...
    # SIMEN START
    sch[weight_att_ub].set_scope(tbe_platform.scope_ubuf)
    sch[weight_att_bc_ub].set_scope(tbe_platform.scope_ubuf)
    if fp16_input_output:
        sch[weight_att_ub_fp32].set_scope(tbe_platform.scope_ubuf)
    # SIMEN END
    if bias1 is not None:
    ...
    if is_gate_output:
        ...
        sch[i_t_weight_gm_back].set_scope(tbe_platform.scope_ubuf) # SIMEN
        ...
        if fp16_input_output:
            ...
            sch[i_t_weight_fp16].set_scope(tbe_platform.scope_ubuf)
            ...
            sch[i_t_weight_gm_back_fp32].set_scope(tbe_platform.scope_ubuf) # SIMEN
    ...
    # SIMEN START
    # weight multiply schedule
    sch[weight_att_ub].compute_at(sch[update_h_gm], update_h_gm_outer)
    sch[weight_att_bc_ub].compute_at(sch[update_h_gm], update_h_gm_outer)
    if fp16_input_output:
        sch[weight_att_ub_fp32].compute_at(sch[update_h_gm], update_h_gm_outer)
    # SIMEN END

    if is_gate_output:
        sch[r_t_gm].compute_at(sch[update_h_gm], update_h_gm_outer)
        sch[r_t_gm_back].compute_at(sch[update_h_gm], update_h_gm_outer)
        sch[i_t_weight_gm].compute_at(sch[update_h_gm], update_h_gm_outer) # SIMEN
        sch[i_t_weight_gm_back].compute_at(sch[update_h_gm], update_h_gm_outer) # SIMEN
        ...
        if fp16_input_output:
            ...
            sch[i_t_weight_fp16].compute_at(sch[update_h_gm], update_h_gm_outer) # SIMEN
            sch[i_t_weight_gm_back_fp32].compute_at(sch[update_h_gm], update_h_gm_outer) # SIMEN
    ...
    if bias1 is not None:
        ...
        sch[bias_bc_ub_1].emit_insn(bias_bc_ub_1.op.axis[0], "unified_broadcast")
        if fp16_input_output:
    ...
    # SIMEN START
    sch[weight_att_ub].emit_insn(weight_att_ub.op.axis[0], "dma_copy")
    sch[weight_att_bc_ub].emit_insn(weight_att_bc_ub.op.axis[0], "unified_broadcast")
    if fp16_input_output:
        sch[weight_att_ub_fp32].emit_insn(weight_att_ub_fp32.op.axis[0], "dma_copy")
    # SIMEN END
    if is_gate_output:
        sch[r_t_gm].emit_insn(r_t_gm.op.axis[0], "dma_copy")
        sch[i_t_gm].emit_insn(i_t_gm.op.axis[0], "dma_copy")
        sch[i_t_weight_gm].emit_insn(i_t_weight_gm.op.axis[0], "dma_copy") # SIMEN
        ...
        sch[i_t_weight_gm_back].emit_insn(i_t_weight_gm_back.op.axis[0], "phony_insn") # SIMEN
        ...
        if fp16_input_output:
            ...
            sch[i_t_weight_fp16].emit_insn(i_t_weight_fp16.op.axis[0], "vector_conv") # SIMEN
            ...
            sch[i_t_weight_gm_back_fp32].emit_insn(i_t_weight_gm_back_fp32.op.axis[0], "phony_insn") # SIMEN
            ...
            sch[i_t_weight_gm_back_fp32].reused_by(i_t_weight) # SIMEN
            ...
            sch[i_t_weight_gm_back].reused_by(i_t_weight_fp16) # SIMEN
            ...
            sch[i_t_weight_gm_back].reused_by(i_t_weight)
