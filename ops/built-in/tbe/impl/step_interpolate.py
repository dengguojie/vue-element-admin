...
class StepInterpolate:
    ...
    def __init__(self, i, q, y_i_first, y_i_second, y_q_first, y_q_second, kernel_name="step_interpolate"):
        ...
        self.dtype_bytes_size_i = cce.get_bit_len(self.dtype_i) // Constant.BLOCK
    ...
    def gm_for_data(self):
        ...
        shape_y_second = (8,)
        self.i = self.tik_instance.Tensor(self.dtype_i, self.i_shape, name="i", scope=tik.scope_gm)
        self.q = self.tik_instance.Tensor(self.dtype_q, self.q_shape, name="q", scope=tik.scope_gm)
        lis = [i for i in range(int(index))]
        self.indices = self.tik_instance.Tensor(self.dtype_i, shape_indices, name="indices", scope=tik.scope_gm,
                                                init_value=lis)
        self.y_i_first = self.tik_instance.Tensor(self.dtype_i, shape_y_first, name="y_i_first", scope=tik.scope_gm)
        self.y_i_second = self.tik_instance.Tensor(self.dtype_i, shape_y_second, name="y_i_second",
                                                   scope=tik.scope_gm, is_atomic_add=True)
        self.y_q_first = self.tik_instance.Tensor(self.dtype_i, shape_y_first, name="y_q_first", scope=tik.scope_gm)
    ...
    def ub_for_data(self):
        ...
        data_ub_fp32 = tik_inst.Tensor("float32", shape_data, scope=tik.scope_ubuf, name="data_ub_fp32")
        data_ub_fp16 = tik_inst.Tensor("float16", shape_data, scope=tik.scope_ubuf, name="data_ub_fp16")
        data_ub_fp16_conv = tik_inst.Tensor("float16", shape_data_conv, scope=tik.scope_ubuf,
                                            name="data_ub_fp16_conv")
        block_ub = tik_inst.Tensor("float32", (16,), scope=tik.scope_ubuf, name="block_ub")
    ...
    def cal_code_core(self, core_index):
        ...
        size_per_core = 640
        times = 3
        decimal_ub = tik_instance.Tensor("float32", (12, size_per_core), scope=tik.scope_ubuf, name="decimal_ub")
        with tik_instance.new_stmt_scope():
            diff_ub = tik_instance.Tensor("float32", (times, size_per_core), scope=tik.scope_ubuf, name="diff_ub")
            with tik_instance.new_stmt_scope():
                indices_ub = tik_instance.Tensor("float32", (times, size_per_core), scope=tik.scope_ubuf,
                                                 name="indices_ub")
                int32_ub = tik_instance.Tensor("int32", (times, size_per_core),
                                               scope=tik.scope_ubuf, name="int32_ub")
            ...
            double_x_ub = tik_instance.Tensor("float32", (times, size_per_core),
                                              scope=tik.scope_ubuf, name="double_x_ub")
            cubic_x_ub = tik_instance.Tensor("float32", (times, size_per_core),
                                             scope=tik.scope_ubuf, name="cubic_x_ub")
            coe_x_ub1 = tik_instance.Tensor("float32", (times, size_per_core),
                                            scope=tik.scope_ubuf, name="coe_x_ub1") # 1/6 x3
            coe_x_ub2 = tik_instance.Tensor("float32", (times, size_per_core),
                                            scope=tik.scope_ubuf, name="coe_x_ub2") # 1/6 x 1/2 x
            coe_x_ub3 = tik_instance.Tensor("float32", (times, size_per_core),
                                            scope=tik.scope_ubuf, name="coe_x_ub3") # 1/2 x2
            coe_x_ub4 = tik_instance.Tensor("float32", (times, size_per_core),
                                            scope=tik.scope_ubuf, name="coe_x_ub4") # 1/2 x3, 1/3 x
            coe_x_ub5 = tik_instance.Tensor("float32", (times, size_per_core),
                                            scope=tik.scope_ubuf, name="coe_x_ub5")
            res_ub = tik_instance.Tensor("float32", (times, size_per_core), scope=tik.scope_ubuf, name="res_ub")
