 ...
 * Description: api of eigen tensor
 */
...
public:
     ...
     */
    template <typename T> typename TTypes<T>::ConstMatrix matrix() const
    {
        return tensor<T, 2>();
    }
    ...
    {
        return typename TTypes<T, NDIMS>::Tensor(reinterpret_cast<T *>(tensor_data_),
                                                 AsEigenDSizes<NDIMS>());
    }
     ...
     */
    template <typename T, size_t NDIMS> typename TTypes<T, NDIMS>::ConstTensor tensor() const
    {
        return typename TTypes<T, NDIMS>::ConstTensor(reinterpret_cast<const T *>(tensor_data_),
                                                      AsEigenDSizes<NDIMS>());
    }
     ...
     */
    template <int NDIMS, typename IndexType> Eigen::DSizes<IndexType, NDIMS> AsEigenDSizesWithPadding() const
    {
     ...
     * @return Eigen::DSizes: pad the rest of the sizes with 1
     */
    template <int NDIMS, typename IndexType = Eigen::DenseIndex> Eigen::DSizes<IndexType, NDIMS> AsEigenDSizes() const
    {
