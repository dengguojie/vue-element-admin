...
namespace aicpu {
    ...
    uint32_t SparseTensor::CreateSparseTensor(Tensor *ix,
                                              Tensor *vals,
                                              ...
                                              std::vector<int64_t> order)
    {
        ...
        if (dims != vals_dim0) {
            KERNEL_LOG_ERROR("ix dim_size_0: %lld != vals dim_size_0: %lld", dims, vals_dim0);
            return KERNEL_STATUS_INNER_ERROR;
        }
    ...
    uint32_t SparseTensor::IndicesValid()
    {
        ...
        int64_t dim_size = (ix_->GetTensor()->GetTensorShape()->GetDims() == 0)
            ? 1 : ix_->GetTensor()->GetTensorShape()->GetDimSize(0);
    ...
    bool SparseTensor::ValidateToDense(Tensor* out)
    {
        ...
        if (shapeSize != out_shape->GetDims()) {
            KERNEL_LOG_ERROR("output dims must match shape dims, got output dims=%d, shape dims=%d.",
                             out_shape->GetDims(), shapeSize);
            return false;
        }
        for (size_t d = 0; d < shape_.size(); ++d) {
          if (shape_[d] > out_shape->GetDimSize(d)) {
              KERNEL_LOG_ERROR("valid output shape dims value falied, index=%z, shape value=%d, "
                               "greater than output shape value=%d.", d, shape_[d], out_shape->GetDimSize(d));
              return false;
          }
        }
    ...
    GroupIterable SparseTensor::group(const std::vector<int64_t> &group_ix) const
    {
        ...
        return GroupIterable(const_cast<Tensor*>(ix_->GetTensor()),
            const_cast<Tensor*>(vals_->GetTensor()), dims_, group_ix);
    }
}
