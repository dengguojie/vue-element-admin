...
void StatelessDropOutGenMaskCpuKernel::StatelessDropOutGenMaskKernel(
    const uint64_t count, const float prob, const uint8_t *offset,
...
uint32_t StatelessDropOutGenMaskCpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_LOG_INFO("DropOutGenMaskCpuKernel kernel in.");

  if (NormalCheck(ctx, kInputNum, kOutputNum) != KERNEL_STATUS_OK) {
    KERNEL_LOG_ERROR("NormalCheck ERROR");
    return KERNEL_STATUS_PARAM_INVALID;
  }

  Tensor *probTensor = ctx.Input(1);
  Tensor *seedTensor = ctx.Input(2);
  Tensor *seed1Tensor = ctx.Input(3);
  ...
  switch (probDataType) {
    case DT_FLOAT16:
      prob = static_cast<float>(
          *(static_cast<Eigen::half *>(probTensor->GetData())));
      break;
    case DT_FLOAT:
      prob = *(static_cast<float *>(probTensor->GetData()));
      break;
    default:
      KERNEL_LOG_ERROR(
          "Data type of input is not support, input data type is [%s].",
  ...
  switch (seedDataType) {
    case DT_INT32:
      seed = *(static_cast<int32_t *>(seedTensor->GetData()));
      break;
    case DT_INT64:
      seed = *(static_cast<int64_t *>(seedTensor->GetData()));
      break;
    default:
      KERNEL_LOG_ERROR(
          "Data type of input is not support, input data type is [%s].",
  ...
  switch (seed1DataType) {
    case DT_INT32:
      seed1 = *(static_cast<int32_t *>(seed1Tensor->GetData()));
      break;
    case DT_INT64:
      seed1 = *(static_cast<int64_t *>(seed1Tensor->GetData()));
      break;
    default:
      KERNEL_LOG_ERROR(
          "Data type of input is not support, input data type is [%s].",
  ...
  KERNEL_LOG_INFO("prob[%f] seed[%ld] seed1[%ld] outputSize[%lu].", prob, seed,
                  seed1, outputSize);
  uint32_t ret = DoCompute(ctx, prob, seed, seed1, out_buff, outputSize);
  return ret;
}

uint32_t StatelessDropOutGenMaskCpuKernel::DoCompute(CpuKernelContext &ctx,
                                                     float prob, int64_t seed,
                                                     int64_t seed1,
                                                     uint8_t *out_buff,
                                                     uint64_t outputSize) {
  if (prob <= FLT_EPSILON) {
    memset_s(out_buff, outputSize, 0x00, outputSize);
    return KERNEL_STATUS_OK;
  }
  // if prob is 1, set all bits to 1
  if (fabs(prob - 1.0f) <= FLT_EPSILON) {
    memset_s(out_buff, outputSize, 0xff, outputSize);
    return KERNEL_STATUS_OK;
  }

  uint64_t key[2] = {static_cast<uint64_t>(seed1), static_cast<uint64_t>(seed)};
  ...
  auto shards = [&](int64_t start, int64_t limit) {
    ...
    uint64_t count = (static_cast<uint64_t>(limit - start)) << 7;
    uint8_t *out_ptr = out_buff;
    // cacluate skip bytes
    out_ptr += ((static_cast<uint64_t>(start)) << 4);
    StatelessDropOutGenMaskCpuKernel::StatelessDropOutGenMaskKernel(
        count, prob, reinterpret_cast<const uint8_t *>(&offset),
        reinterpret_cast<const uint8_t *>(&key), out_ptr);
  };
  // shared unit size
  const int64_t total_unit = static_cast<int64_t>(outputSize >> 4);
  ...
  StatelessDropOutGenMaskKernel(outputSize, prob,
