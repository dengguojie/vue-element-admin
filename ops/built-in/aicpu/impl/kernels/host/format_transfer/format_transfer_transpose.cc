...
bool IsTransposeArgValid(const uint8_t *src,
  ...
  if (GetSizeByDataType(src_data_type) < 0) {
    KERNEL_LOG_ERROR("The data type [%s] is not support",
...
uint32_t Transpose(const uint8_t *src, const std::vector<int64_t> &src_shape,
                   DataType src_data_type, const std::vector<int64_t> &perm_arg,
                   TransResult &result) {
  ...
  if (dst == nullptr) {
    KERNEL_LOG_ERROR(
        ...
        "to [%s]",
        dst_size, VectorToString(src_shape).c_str(),
  ...
  while (dst_index < dst_ele_num) {
    ...
    auto protected_size =
        dst_size - dst_offset_bytes < static_cast<int64_t>(SECUREC_MEM_MAX_LEN)
            ? dst_size - dst_offset_bytes
            : static_cast<int64_t>(SECUREC_MEM_MAX_LEN);
    auto ret = memcpy_s(dst.get() + dst_offset_bytes,
                        static_cast<size_t>(protected_size), src + src_offset,
    ...
    if (ret != EOK) {
      KERNEL_LOG_ERROR(
          ...
          VectorToString(src_shape).c_str(), VectorToString(perm_arg).c_str(),
          VectorToString(dst_shape).c_str(), dst_offset_bytes,
          VectorToString(dst_indexes).c_str());
      return KERNEL_STATUS_PARAM_INVALID;
    }
...
uint32_t TransposeWithShapeCheck(const uint8_t *data,
                                 const std::vector<int64_t> &src_shape,
  ...
  if (dst_shape != expected_shape) {
    KERNEL_LOG_ERROR(
        ...
        VectorToString(expected_shape).c_str());
    return KERNEL_STATUS_PARAM_INVALID;
  }
...
uint32_t GetPermByForamt(Format src_format, Format dst_format,
...
uint32_t FormatTransferTranspose::TransFormat(const TransArgs &args,
  ...
  auto ret = TransShape(args.src_format, args.src_shape, args.src_data_type,
                        args.dst_format, expected_shape, args.groups);
  ...
  return Transpose(args.data, args.src_shape, args.src_data_type,
                   perm_args[args.src_format][args.dst_format], result);
}

uint32_t FormatTransferTranspose::TransShape(
