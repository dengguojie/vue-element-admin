...
bool IsDataTypeSupport(DataType data_type) {
...
bool CheckShape(Format format, const ShapeVector &shape) {
  switch (format) {
    ...
    default:
      string error = "Trans format between " +
                     FmtToStr(FormatToSerialString(format)) +
 ...
 * @return
 */
uint32_t TransShapeToFracNz(const ShapeVector &src_shape, DataType data_type,
...
uint32_t CheckShapeRelation(const TransArgs &args, ShapeVector &hw_shape) {
  ...
  auto ret = TransShapeToFracNz(args.dst_shape, args.src_data_type,
                                expect_src_shape, hw_shape);
...
uint32_t TransFormatFromNdToFracNz(const TransArgs &args, TransResult &result,
                                   const ShapeVector &hw_shape) {
  ...
  if (dst == nullptr) {
    KERNEL_LOG_ERROR(
        ...
        "for dst buf [%ld]",
        FormatToSerialString(args.src_format).c_str(),
        FormatToSerialString(args.dst_format).c_str(), dst_size);
  ...
  for (int64_t times_idx = 0; times_idx < times; times_idx++) {
    ...
    for (int64_t h1h0_idx = 0; h1h0_idx < h; h1h0_idx++) {
      ...
      for (int64_t w1_idx = 0; w1_idx < num_w1; w1_idx++) {
        ...
        auto ret = memcpy_s(
            dst.get() + dst_offset, static_cast<size_t>(protected_size),
            args.data + src_offset, static_cast<size_t>(size * w0));
        if (ret != EOK) {
          KERNEL_LOG_ERROR(
      ...
      for (int64_t w0_idx = 0; w1_head + w0_idx < w; w0_idx++) {
        ...
        auto protected_size =
            dst_size - dst_offset < static_cast<int64_t>(SECUREC_MEM_MAX_LEN)
                ? dst_size - dst_offset
                : static_cast<int64_t>(SECUREC_MEM_MAX_LEN);
        auto ret = memcpy_s(dst.get() + dst_offset,
...
uint32_t TransFormatFromFracNzToNd(const TransArgs &args, TransResult &result,
                                   const ShapeVector &dst_hw_shape) {
  int size = GetSizeByDataType(args.src_data_type);
  int64_t dst_size = GetItemNumByShape(args.dst_shape) * size;
  if (dst_size == 0) {
    result.length = static_cast<size_t>(dst_size);
      ...
      for (int64_t w1_idx = 0; w1_idx < num_w1; w1_idx++) {
        ...
        auto protected_size =
            dst_size - dst_offset < static_cast<int64_t>(SECUREC_MEM_MAX_LEN)
                ? dst_size - dst_offset
                : static_cast<int64_t>(SECUREC_MEM_MAX_LEN);
        ret = memcpy_s(dst.get() + dst_offset,
                       static_cast<size_t>(protected_size),
                       args.data + src_offset, static_cast<size_t>(size * w0));
        if (ret != EOK) {
          KERNEL_LOG_ERROR(
...
uint32_t FormatTransferFractalNz::TransFormat(const TransArgs &args,
  ...
  if (!CheckShape(args.src_format, args.src_shape) ||
  ...
  auto ret = TransShapeToFracNz(args.src_shape, args.src_data_type,
                                expect_shape, hw_shape);
  if (ret != KERNEL_STATUS_OK) {
    return ret;
  }
...
uint32_t FormatTransferFractalNz::TransShape(Format src_format,
  ...
  if (!IsDataTypeSupport(data_type)) {
    KERNEL_LOG_ERROR(
        ...
        FormatToSerialString(dst_format).c_str(),
...
uint32_t FormatTransferFractalNzND::TransFormat(const TransArgs &args,
  ...
  if (!IsShapeValid(args.src_shape) ||
...
uint32_t FormatTransferFractalNzND::TransShape(Format src_format,
                                               const ShapeVector &src_shape,
                                               DataType data_type,
                                               Format dst_format,
                                               ShapeVector &dst_shape) {
...
REGISTER_FORMAT_TRANSFER(FormatTransferFractalNz, FORMAT_NCHW,
                         FORMAT_FRACTAL_NZ)
REGISTER_FORMAT_TRANSFER(FormatTransferFractalNz, FORMAT_NHWC,
                         FORMAT_FRACTAL_NZ)
REGISTER_FORMAT_TRANSFER(FormatTransferFractalNzND, FORMAT_FRACTAL_NZ,
                         FORMAT_ND)
REGISTER_FORMAT_TRANSFER(FormatTransferFractalNzND, FORMAT_FRACTAL_NZ,
                         FORMAT_NCHW)
REGISTER_FORMAT_TRANSFER(FormatTransferFractalNzND, FORMAT_FRACTAL_NZ,
