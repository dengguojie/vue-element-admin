...
void TransSrcDataToDstData(const TransArgs &args,
                           const std::vector<int64_t> &shape_ndhwc,
                           std::shared_ptr<uint8_t> &dst,
                           int64_t c0, int32_t data_size) {
  ...
  for (int64_t ndhwc_idx = 0; ndhwc_idx < ndhwc; ++ndhwc_idx) {
    ...
    const int64_t dst_index = n_idx * dc1hwc0 + (dhw_idx / hw) * c1hwc0 +
                              (c_idx / c0) * hwc0 + (dhw_idx % hw) * c0 + c_idx % c0;
...
uint32_t TransDstDataToNdc1hwc0(const TransArgs &args, TransResult &result) {
  ...
  if (dst == nullptr) {
    KERNEL_LOG_ERROR(
        "Failed to allocate memory for dst buf [%ld] when trans format from [%s] to [%s]",
        dst_size, FormatToSerialString(args.src_format).c_str(),
        FormatToSerialString(args.dst_format).c_str());
    return KERNEL_STATUS_PARAM_INVALID;
  }
  errno_t ret = memset_s(dst.get(), static_cast<size_t>(dst_size), 0,
  ...
  if (iter == kFormatTable.end()) {
    KERNEL_LOG_ERROR("src_format is wrong, now format is [%d]", args.src_format);
    return KERNEL_STATUS_PARAM_INVALID;
  }
...
uint32_t TransShapeToNdc1hwc0(const std::vector<int64_t> &src_shape, const Format &src_format,
  ...
  if (iter == kFormatTable.end()) {
    KERNEL_LOG_ERROR("src_format is wrong, now format is [%d]", src_format);
    return KERNEL_STATUS_PARAM_INVALID;
  }
...
uint32_t FormatTransferNdc1hwc0::TransShape(
