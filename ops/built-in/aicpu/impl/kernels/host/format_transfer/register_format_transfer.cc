...
struct FormatTransferRegistry {
  ...
  map<Format, map<Format, FormatTransferBuilder>> src_dst_builder;
};
...
FormatTransferRegister::FormatTransferRegister(FormatTransferBuilder builder,
                                               Format src, Format dst) {
  GetFormatTransferRegistry().RegisterBuilder(src, dst, move(builder));
}

shared_ptr<FormatTransfer> BuildFormatTransfer(const TransArgs &args) {
