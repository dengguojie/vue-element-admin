...
uint32_t TransShapeToFz3DWithGroups(int64_t n, int64_t c, int64_t d, int64_t h,
                                    int64_t w, DataType data_type,
...
uint32_t TransShapeNcdhwToFzWithGroups(const std::vector<int64_t> &src_shape,
                                       DataType data_type,
                                       std::vector<int64_t> &dst_shape,
                                       int64_t groups) {
  if (!CheckShapeValid(src_shape, kNcdhwDimsNum)) {
    return KERNEL_STATUS_PARAM_INVALID;
  }
  ...
  return TransShapeToFz3DWithGroups(n, c, d, h, w, data_type, dst_shape,
                                    groups);
}

uint32_t TransShapeDhwcnToFzWithGroups(const std::vector<int64_t> &src_shape,
                                       DataType data_type,
                                       std::vector<int64_t> &dst_shape,
                                       int64_t groups) {
...
uint32_t TransShapeNdhwcToFzWithGroups(const std::vector<int64_t> &src_shape,
                                       DataType data_type,
                                       std::vector<int64_t> &dst_shape,
                                       int64_t groups) {
...
uint32_t TransFormatToFzWithGroups(const TransArgs &args, TransResult &result,
  ...
  if (args.src_format == FORMAT_NDHWC) {
    n_dim = args.src_shape.at(kNdhwcN);
    d_dim = args.src_shape.at(kNdhwcD);
    h_dim = args.src_shape.at(kNdhwcH);
    w_dim = args.src_shape.at(kNdhwcW);
    c_dim = args.src_shape.at(kNdhwcC);
  } else if (args.src_format == FORMAT_DHWCN) {
    d_dim = args.src_shape.at(kDhwcnD);
    h_dim = args.src_shape.at(kDhwcnH);
    w_dim = args.src_shape.at(kDhwcnW);
    c_dim = args.src_shape.at(kDhwcnC);
    n_dim = args.src_shape.at(kDhwcnN);
  } else if (args.src_format == FORMAT_NCDHW) {
    n_dim = args.src_shape.at(kNcdhwN);
    c_dim = args.src_shape.at(kNcdhwC);
    d_dim = args.src_shape.at(kNcdhwD);
    h_dim = args.src_shape.at(kNcdhwH);
    w_dim = args.src_shape.at(kNcdhwW);
  } else {
    KERNEL_LOG_WARN(
        "Supporting format conversion from NCDHW, DHWCN and NDHWC to Fz_3d, current input "
        "format is [%s]",
            ...
            for (int64_t n = 0; n < cout_ori; n++) {
              ...
              if (args.src_format == FORMAT_DHWCN) {
                ...
                srx_inx = src_co * c_dim * d_dim * h_dim * w_dim +
                          ...
                          h * w_dim + w;
              } else if (args.src_format == FORMAT_NDHWC) {
...
uint32_t FormatTransferFractalz3D::TransShape(
  ...
  if (src_format == FORMAT_NDHWC &&
    ...
    return TransShapeNdhwcToFzWithGroups(src_shape, data_type, dst_shape,
                                         groups);
  }
...
REGISTER_FORMAT_TRANSFER(FormatTransferFractalz3D, FORMAT_NCDHW,
                         FORMAT_FRACTAL_Z_3D)
REGISTER_FORMAT_TRANSFER(FormatTransferFractalz3D, FORMAT_DHWCN,
                         FORMAT_FRACTAL_Z_3D)
REGISTER_FORMAT_TRANSFER(FormatTransferFractalz3D, FORMAT_NDHWC,
                         FORMAT_FRACTAL_Z_3D)
}  // namespace formats
}  // namespace  aicpu