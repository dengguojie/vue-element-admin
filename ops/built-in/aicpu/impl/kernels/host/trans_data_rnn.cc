...
uint32_t TransDataRNNCpuKernel::GenDataFractalZnCase1(std::vector<int64_t> &dims, int32_t hiddenSize, int32_t inputSize,
  ...
  int32_t stateSizeAlign = (stateSize == 0) ? hiddenSizeAlign : (stateSize + (ALIGN_16 - 1)) / ALIGN_16 * ALIGN_16;
  int32_t dstLen = (hiddenSizeAlign * hiddenCnt) * (stateSizeAlign + inputSizeAlign);

  int32_t shape0Align = stateSizeAlign + inputSizeAlign;
...
uint32_t TransDataRNNCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  if ((ret = GetInputAttrs(ctx, inputSize, hiddenSize, stateSize, srcFormat, dstFormat)) != KERNEL_STATUS_OK) {
    return ret;
  }
  KERNEL_LOG_INFO("AICPU TransDataRNN is begin. hiddenSize = %d, inputSize = %d, stateSize = %d",
                  hiddenSize, inputSize, stateSize);
  ...
  if (dstFormat == "ND_RNN_BIAS") {
    ...
    ret = GenDataFractalZn(inputDims, hiddenSize, inputSize, stateSize, srcTensor, dstTensor);
  } else {
