...
uint32_t MulKernel::MulCompute(CpuKernelContext &ctx) {
  ...
  KERNEL_LOG_INFO("Mul %s, input0: addr=%p, size=%llu; input1: addr=%p, size=%llu; output: addr %p, size %llu.",
                  ctx.GetOpType().c_str(),
  ...
  switch (static_cast<int32_t>(calc_info.shape_out.size())) {
    ...
    default:
      KERNEL_LOG_ERROR("Mul kernel not support rank=%zu.", calc_info.shape_out.size());
      return KERNEL_STATUS_PARAM_INVALID;
  }
