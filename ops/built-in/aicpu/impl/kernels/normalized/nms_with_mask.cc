...
uint32_t NMSWithMaskCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_FALSE(
      (data_type == DT_FLOAT || data_type == DT_FLOAT16),
      KERNEL_STATUS_PARAM_INVALID, "Input[0] data type[%s] is unsupported",
  ...
  if (box_size_ != kColNum5 && box_size_ != kColNum8) {
    KERNEL_LOG_INFO(
        "NMSWithMask the col number of input[0] must be [%d] or [%d], but got [%d]!",
        kColNum5, kColNum8, box_size_);
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_INFO(
          "NMSWithMask input[0] only support type[DT_FLOAT16, DT_FLOAT], but got type[%s]",
          DTypeStr(data_type).c_str());
      return KERNEL_STATUS_PARAM_INVALID;
      break;
  }
...
uint32_t NMSWithMaskCpuKernel::DoCompute(CpuKernelContext &ctx) {
  ...
  for (int64_t i = 0; i < num_input_; i++) {
    areas[i] = (input[i * box_size_ + 2] - input[i * box_size_]) *
               (input[i * box_size_ + 3] - input[i * box_size_ + 1]);
  }
