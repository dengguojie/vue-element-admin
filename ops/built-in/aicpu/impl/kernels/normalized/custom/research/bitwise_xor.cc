...
const char *kBitwiseXor = "BitwiseXor";
...
#define BITWISE_XOR_COMPUTE_CASE(DTYPE, TYPE, CTX)           \
  case (DTYPE): {                                            \
    uint32_t result = BitwiseXorCompute<TYPE>(CTX);          \
    if (result != KERNEL_STATUS_OK) {                        \
      KERNEL_LOG_ERROR("BitwiseXor kernel compute failed."); \
      return result;                                         \
    }                                                        \
    break;                                                   \
  }
...
namespace aicpu {
uint32_t BitwiseXorCpuKernel::Compute(CpuKernelContext &ctx) {
  // check params
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "BitwiseXor check input and output number failed.");
  KERNEL_HANDLE_ERROR(BitwiseXorParamCheck(ctx),
                      "BitwiseXor check params failed.");
  auto data_type = ctx.Input(0)->GetDataType();
  switch (data_type) {
    BITWISE_XOR_COMPUTE_CASE(DT_INT8, int8_t, ctx)
    BITWISE_XOR_COMPUTE_CASE(DT_INT16, int16_t, ctx)
    BITWISE_XOR_COMPUTE_CASE(DT_INT32, int32_t, ctx)
    BITWISE_XOR_COMPUTE_CASE(DT_INT64, int64_t, ctx)
    BITWISE_XOR_COMPUTE_CASE(DT_UINT8, uint8_t, ctx)
    BITWISE_XOR_COMPUTE_CASE(DT_UINT16, uint16_t, ctx)
    BITWISE_XOR_COMPUTE_CASE(DT_UINT32, uint32_t, ctx)
    BITWISE_XOR_COMPUTE_CASE(DT_UINT64, uint64_t, ctx)
    default:
      KERNEL_LOG_ERROR("BitwiseXor kernel data type [%s] not support.",
...
uint32_t BitwiseXorCpuKernel::BitwiseXorParamCheck(CpuKernelContext &ctx) {
  ...
  KERNEL_LOG_DEBUG(
      "BitwiseXorCpuKernel[%s], input0: size[%llu];"
...
*/
template <typename T>
void BitwiseXorCpuKernel::SpecialCompute(BcastShapeType type, int64_t start,
...
template <typename T>
uint32_t BitwiseXorCpuKernel::NoBcastCompute(CpuKernelContext &ctx) {
  ...
  if (data_size >= kParallelDataSizeSameShape) {
    ...
    auto sharder_bitwise_xor = [&](int64_t start, int64_t end) {
...
template <typename T>
uint32_t BitwiseXorCpuKernel::BcastCompute(CpuKernelContext &ctx,
                                           Bcast &bcast) {
  auto input0_data = reinterpret_cast<T *>(ctx.Input(0)->GetData());
  auto input1_data = reinterpret_cast<T *>(ctx.Input(1)->GetData());
  auto output_data = reinterpret_cast<T *>(ctx.Output(0)->GetData());
...
template <typename T>
uint32_t BitwiseXorCpuKernel::BitwiseXorCompute(CpuKernelContext &ctx) {
...
REGISTER_CPU_KERNEL(kBitwiseXor, BitwiseXorCpuKernel);
}  // namespace aicpu