...
std::uint32_t SinCheck(CpuKernelContext &ctx, uint32_t inputs_num,
                       uint32_t outputs_num) {
  return NormalCheck(ctx, kSinInputNum, kSinOutputNum)
...
std::uint32_t SinCpuKernel::Compute(CpuKernelContext &ctx) {
  return detail::SinCheck(ctx, kSinInputNum, kSinOutputNum)
             ? KERNEL_STATUS_PARAM_INVALID
             : detail::SinCompute(ctx);
}
