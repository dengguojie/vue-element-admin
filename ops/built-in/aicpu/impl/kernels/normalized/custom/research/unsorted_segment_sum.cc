...
uint32_t UnsortedSegmentSumCpuKernel::UnsortedSegmentSumComputeTemplate(
...
uint32_t UnsortedSegmentSumCpuKernel::DoComputeWithNumSegmentsType(
    CpuKernelContext &ctx, DataType num_segments_type) {
  switch (num_segments_type) {
    case DT_INT32:
      return UnsortedSegmentSumComputeTemplate<input_t, segment_ids_t, int32_t>(
          ctx);
    case DT_INT64:
      return UnsortedSegmentSumComputeTemplate<input_t, segment_ids_t, int64_t>(
          ctx);

    default:
      KERNEL_LOG_ERROR("UnsortedSegmentSum invalid num_segments_type type [%s]",
...
uint32_t UnsortedSegmentSumCpuKernel::DoComputeWithSegmentIdsType(
  ...
  switch (segment_ids_type) {
    ...
    default:
      KERNEL_LOG_ERROR("UnsortedSegmentSum invalid segment_ids_type type [%s]",
...
uint32_t UnsortedSegmentSumCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (input_type) {
    ...
    case DT_COMPLEX64:
      return DoComputeWithSegmentIdsType<std::complex<float>>(ctx,
                                                              segment_ids_type);
    case DT_COMPLEX128:
      return DoComputeWithSegmentIdsType<std::complex<double>>(
          ctx, segment_ids_type);
    default:
      KERNEL_LOG_ERROR("UnsortedSegmentSum invalid input type [%s]",
