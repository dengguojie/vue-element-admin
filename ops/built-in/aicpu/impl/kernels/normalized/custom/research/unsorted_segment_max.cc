...
namespace {
const char *kUnsortedSegmentMax = "UnsortedSegmentMax";
...
uint32_t UnsortedSegmentMaxCpuKernel::UnsortedSegmentMaxComputeTemplate(
  ...
  for (int64_t k = 0; k < data_size; k++) {
    *(output_y + k) = std::numeric_limits<input_t>::lowest();
  }
...
uint32_t UnsortedSegmentMaxCpuKernel::DoComputeWithNumSegmentsType(
    CpuKernelContext &ctx, DataType num_segments_type) {
  switch (num_segments_type) {
    case DT_INT32:
      return UnsortedSegmentMaxComputeTemplate<input_t, segment_ids_t, int32_t>(
          ctx);
    case DT_INT64:
      return UnsortedSegmentMaxComputeTemplate<input_t, segment_ids_t, int64_t>(
          ctx);

    default:
      KERNEL_LOG_ERROR("UnsortedSegmentMax invalid num_segments_type type [%s]",
...
uint32_t UnsortedSegmentMaxCpuKernel::DoComputeWithSegmentIdsType(
  ...
  switch (segment_ids_type) {
    ...
    default:
      KERNEL_LOG_ERROR("UnsortedSegmentMax invalid segment_ids_type type [%s]",
                       DTypeStr(segment_ids_type).c_str());
      return KERNEL_STATUS_PARAM_INVALID;
  }
}

uint32_t UnsortedSegmentMaxCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (input_type) {
    ...
    default:
      KERNEL_LOG_ERROR("UnsortedSegmentMax invalid input type [%s]",
...
REGISTER_CPU_KERNEL(kUnsortedSegmentMax, UnsortedSegmentMaxCpuKernel);
}  // namespace aicpu