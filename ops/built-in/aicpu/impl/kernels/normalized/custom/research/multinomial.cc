...
uint32_t Generate(Tensor *&input_0, Tensor *&input_1, Tensor *&output,
  ...
  if (total_num < kParallelDataNums) {
    ...
    for (int i = 0; i < batch_size; i++) {
      ...
      for (int64_t j = 0; j < num_classes; ++j) {
        if (isfinite(*(row_start + j))) {
          max_logit =
              std::max(max_logit, static_cast<double>(*(row_start + j)));
        }
      }

      // calculate cdf (Cumulative Distribution Function)
      for (int64_t j = 0; j < num_classes; ++j) {
        cumulative_distribution_function[j] =
            std::exp(static_cast<double>(*(row_start + j)) - max_logit);
      }
      ...
      for (int j = 0; j < num_samples; j++) {
        ...
        auto found_iter =
            std::upper_bound(cumulative_distribution_function,
                             cumulative_distribution_function + num_classes,
                             rand * running_total);

        *cur_out = static_cast<T_out>(
            std::distance(cumulative_distribution_function, found_iter));
        cur_out = cur_out + 1;
      }
    ...
    auto shard = [&](size_t start_outer, size_t end_outer) {
      ...
      auto shard_inner = [&](int start_inner, int end_inner) {
        ...
        auto j_end = j_start + kNumPerThread > num_samples
                         ? num_samples
                         : j_start + kNumPerThread;
        ...
        for (int j = j_start; j < j_end; j++) {
          ...
          *(cur_output + j) = static_cast<T_out>(
              std::distance(cumulative_distribution_function, found_iter));
        }
      };
      CpuKernelUtils::ParallelFor(
          ctx, ceil((double)num_samples / kNumPerThread), 1, shard_inner);
      if (cumulative_distribution_function != nullptr) {
        delete[] cumulative_distribution_function;
      }
    };
...
uint32_t MultinomialCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_FALSE(
      (input0_datatype == DT_FLOAT16 || input0_datatype == DT_DOUBLE ||
       input0_datatype == DT_FLOAT),
  ...
  KERNEL_CHECK_FALSE((input1_datatype == DT_INT32), KERNEL_STATUS_PARAM_INVALID,
                     "Input[1] data type must DT_INT32, but got data type[%s].",
                     DTypeStr(input1_datatype).c_str());

  // check input dimension
  KERNEL_CHECK_FALSE((input_0->GetTensorShape()->GetDims() == 2),
                     KERNEL_STATUS_PARAM_INVALID,
                     "Input[0] should be a matrix, but got rank [%d].",
                     input_0->GetTensorShape()->GetDims());
  KERNEL_CHECK_FALSE((input_1->GetTensorShape()->GetDims() == 0),
                     KERNEL_STATUS_PARAM_INVALID,
                     "Input[1] should be a scalar, but got rank [%d].",
                     input_1->GetTensorShape()->GetDims());

  // check num_classes positive
  auto num_classes = input_0->GetTensorShape()->GetDimSize(1);  // int64_t
  KERNEL_CHECK_FALSE((num_classes > 0), KERNEL_STATUS_PARAM_INVALID,
                     "num_classes should be positive, but got [%d].",
                     num_classes);

  // check num_samples nonnegative
  auto *num_samples_ptr =
      reinterpret_cast<int32_t *>(input_1->GetData());  // int32_t
  KERNEL_CHECK_FALSE((*num_samples_ptr >= 0), KERNEL_STATUS_PARAM_INVALID,
                     "num_samples should be nonnegative, but got [%d].",
                     *num_samples_ptr);
  ...
  if (attr_dtype != nullptr) {
    ...
    KERNEL_CHECK_FALSE((data_type == DT_INT32 || data_type == DT_INT64),
                       KERNEL_STATUS_PARAM_INVALID,
