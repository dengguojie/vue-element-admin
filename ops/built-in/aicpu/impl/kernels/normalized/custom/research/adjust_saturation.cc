...
template <typename T>
inline Rgb<T> ScalarAdjustSaturation(Rgb<T> image,
                                     std::float_t saturation_factor) {
...
template <>
inline Rgb<Eigen::half> ScalarAdjustSaturation(Rgb<Eigen::half> image,
                                               std::float_t saturation_factor) {
...
template <typename T>
inline std::uint32_t ComputeAdjustSaturationKernel(
  ...
  auto ScalarAdjustSaturation1 = [&](Rgb<T> image) {
    return ScalarAdjustSaturation(image, saturation_factor[0]);
  };
