...
#ifndef AICPU_KERNELS_NORMALIZED_UNSORTED_SEGMENT_MAX_H
#define AICPU_KERNELS_NORMALIZED_UNSORTED_SEGMENT_MAX_H

#include "cpu_kernel.h"

namespace aicpu {

class UnsortedSegmentMaxCpuKernel : public CpuKernel {
public:
  ~UnsortedSegmentMaxCpuKernel() = default;
  uint32_t Compute(CpuKernelContext &ctx) override;

private:
template <typename input_t, typename segment_ids_t, typename num_segments_t>
uint32_t UnsortedSegmentMaxComputeTemplate(CpuKernelContext &ctx);
