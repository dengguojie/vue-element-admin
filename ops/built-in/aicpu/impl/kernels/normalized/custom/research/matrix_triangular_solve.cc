...
uint32_t MatrixTriangularSolveCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(MatrixTriangularSolveCheck(ctx),
                      "MatrixTriangularSolve check params failed.");
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR(
          "MatrixTriangularSolve kernel data type [%s] not support.",
...
uint32_t MatrixTriangularSolveCpuKernel::MatrixTriangularSolveCheck(
...
template <typename T>
uint32_t MatrixTriangularSolveCpuKernel::MatrixTriangularSolveCompute(
    CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_NULLPTR(input_matrix, KERNEL_STATUS_PARAM_INVALID,
                       "Get input data0 failed.")
  auto input_rhs = reinterpret_cast<T *>(rhs_tensor->GetData());
  KERNEL_CHECK_NULLPTR(input_rhs, KERNEL_STATUS_PARAM_INVALID,
                       "Get input data1 failed.")
  ...
  size_t matrix_size = last_matrix_dims * last_matrix_dims;  // size of a matix
  ...
  auto shard_matrix_triangular_solve = [&](size_t start, size_t end) {
    for (size_t k = start; k < end; ++k) {
      Eigen::Map<
          ...
          eigen_input(input_matrix + k * matrix_size, last_matrix_dims,
                      last_matrix_dims);
      Eigen::Map<
          Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>
          eigen_rhs(input_rhs + k * rhs_size, last_matrix_dims, last_rhs_dims);
      Eigen::Map<
          Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>
          eigen_output(output_y + k * rhs_size, last_matrix_dims,
                       last_rhs_dims);
      if (lower_data) {
