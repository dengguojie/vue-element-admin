...
uint32_t MatrixBandPartCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_FALSE((num_lower->GetTensorShape()->GetDimSizes().empty()),
                     KERNEL_STATUS_PARAM_INVALID, "num_lower must be scalar.");
  KERNEL_CHECK_FALSE((num_upper->GetTensorShape()->GetDimSizes().empty()),
                     KERNEL_STATUS_PARAM_INVALID, "num_upper must be scalar.");
  DataType lower_type = num_lower->GetDataType();
  KERNEL_CHECK_FALSE((lower_type == DT_INT32 || lower_type == DT_INT64),
  ...
  KERNEL_CHECK_FALSE((upper_type == DT_INT32 || upper_type == DT_INT64),
                     KERNEL_STATUS_PARAM_INVALID,
                     "Unsupported num_upper data_type[%s], "
                     "only support DT_INT32 and DT_INT64.",
                     DTypeStr(upper_type).c_str());
  int32_t *lower_data = reinterpret_cast<int32_t *>(num_lower->GetData());
  KERNEL_CHECK_NULLPTR(lower_data, KERNEL_STATUS_PARAM_INVALID,
                       "Get num_lower data failed.");
  int32_t *upper_data = reinterpret_cast<int32_t *>(num_upper->GetData());
  KERNEL_CHECK_NULLPTR(upper_data, KERNEL_STATUS_PARAM_INVALID,
                       "Get num_upper data failed.");
  ...
  KERNEL_CHECK_FALSE(
      (input_size == output_size), KERNEL_STATUS_PARAM_INVALID,
  ...
  switch (data_type) {
    ...
    BAND_COMPUTE_CASE(DT_COMPLEX64, std::complex<float>, x, lower, upper, y, m,
                      n, ctx)
    BAND_COMPUTE_CASE(DT_COMPLEX128, std::complex<double>, x, lower, upper, y,
                      m, n, ctx)
    default:
      KERNEL_LOG_ERROR("MatrixBandPart kernel data type [%s] not support.",
...
template <typename T>
uint32_t MatrixBandPartCpuKernel::BandCompute(Tensor *x, int64_t lower,
                                              int64_t upper, Tensor *y,
                                              int64_t rows, int64_t cols,
                                              CpuKernelContext &ctx) {
  T *x_addrs = reinterpret_cast<T *>(x->GetData());
  KERNEL_CHECK_NULLPTR(x_addrs, KERNEL_STATUS_PARAM_INVALID,
                       "Get input data failed.");
  T *y_addrs = reinterpret_cast<T *>(y->GetData());
  KERNEL_CHECK_NULLPTR(y_addrs, KERNEL_STATUS_PARAM_INVALID,
  ...
  if (data_num < kParallelDataNums) {
    ...
    for (int64_t i = 0; i < batch_end; i++) {
      ...
      for (int64_t m = row_begin; m < row_end; m++) {
        ...
        int64_t band_start =
            lower < 0
                ? 0
                : std::min(cols, std::max(int64_t{0}, int64_t{m - lower}));
        int64_t band_end =
            upper < 0 ? cols : std::min(cols, int64_t{m + upper + 1});
        if (same_addr) {
          if (band_start > 0) {
            std::fill((y_addrs + base_index),
                      (y_addrs + base_index + band_start), zero);
          }
          if (band_end < cols) {
            std::fill((y_addrs + base_index + band_end),
                      (y_addrs + base_index + cols), zero);
          }
        } else {
    ...
    KERNEL_HANDLE_ERROR(
        CpuKernelUtils::ParallelFor(ctx, total_rows, total_rows / max_core_num,
                                    shard_band),
