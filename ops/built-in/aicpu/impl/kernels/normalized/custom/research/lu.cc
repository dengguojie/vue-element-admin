...
#define LU_COMPUTE_CASE(IN_DTYPE, IN_TYPE, OUT_DTYPE, CTX)                 \
  case (IN_DTYPE): {                                                       \
    switch (OUT_DTYPE) {                                                   \
      ...
      default:                                                             \
        KERNEL_LOG_ERROR("Lu kernel output p data type [%s] not support.", \
...
uint32_t LuCpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kLu);
  ...
  switch (input_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Lu kernel input data type [%s] not support.",
...
uint32_t LuCpuKernel::LuCheck(CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_FALSE((shape_size > 1), KERNEL_STATUS_PARAM_INVALID,
                     "Input must be at least rank 2, got [%zu].",
                     shape_x.size())
  KERNEL_CHECK_FALSE((shape_x[shape_size - 2] == shape_x[shape_size - 1]),
                     KERNEL_STATUS_PARAM_INVALID,
                     "Dimensions must be equal, but are [%zu] and [%zu].",
                     shape_x[shape_size - 2], shape_x[shape_size - 1])
...
uint32_t LuCpuKernel::LuCompute(CpuKernelContext &ctx) {
  ...
  typedef Eigen::Matrix<Tidx, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
      Indices;
  ...
  if (size_nn > 0) {
    ...
    if (data_size <= kParallelDataNums) {
      ...
      for (size_t i = 0; i < martix_num; i++) {
        ...
        KERNEL_CHECK_FALSE((min_abs_pivot > RealScalar(0)),
                           KERNEL_STATUS_PARAM_INVALID,
      ...
      KERNEL_HANDLE_ERROR(
          CpuKernelUtils::ParallelFor(ctx, martix_num,
                                      martix_num / max_core_num, shard_lu),
          "Lu Compute failed.")
    }
  }
