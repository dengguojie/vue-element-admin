...
T GetBatchSizeCheckDims(CpuKernelContext &ctx) {
  ...
  for (int i = 1; i < ctx.Input(0)->NumElements(); i++) {
    KERNEL_CHECK_FALSE((output_shape[i] >= 0), KERNEL_STATUS_PARAM_INVALID,
...
template <typename T>
void Generate(
    int64_t size, T mean, T stddev, T minval, T maxval, T **output_ptr,
    RNG_Engine &rng) {
  ...
  const T cutoff = T(2) *
                   exp(T(0.5) + (norm_min * (norm_min - sqrt_factor)) / T(4)) /
                   (norm_min + sqrt_factor);
...
uint32_t BatchGenerate(CpuKernelContext &ctx) {
  ...
  for (int batch = 0; batch < batch_size; batch++) {
    ...
    Generate<T_val>(int64_t(sample_size), *params[0], *params[1], minval,
                    maxval, &output_data, rng);
...
uint32_t ParameterizedTruncatedNormalCpuKernel::ParameterizedTruncatedNormalCheck(
  ...
  for (uint32_t i = 0; i < kInputNum; i++) {
    ...
    switch (i) {
      case 0:
        KERNEL_CHECK_FALSE(
            (input_datatype == DT_INT32 || input_datatype == DT_INT64),
      ...
      case 1:
        KERNEL_CHECK_FALSE(
            (input_datatype == DT_FLOAT16 || input_datatype == DT_FLOAT ||
             input_datatype == DT_DOUBLE),
            KERNEL_STATUS_PARAM_INVALID,
      ...
      default:
        KERNEL_CHECK_FALSE(
            (input_datatype == val_datatype), KERNEL_STATUS_PARAM_INVALID,
            "The data type of input[%u] [%s] need be same with input[1] [%s].",
            i, DTypeStr(input_datatype).c_str(), DTypeStr(val_datatype).c_str())
    }
    ...
    switch (i) {
      case 0:
        KERNEL_CHECK_FALSE((input_dims == 1), KERNEL_STATUS_PARAM_INVALID,
                           "Input[0] should be rank 1, but got rank [%d].",
                           input_dims);
        break;

      default:
        KERNEL_CHECK_FALSE(
            (input_dims <= 1), KERNEL_STATUS_PARAM_INVALID,
            "Input[%u] should be at most rank 1, but got rank [%d].", i,
            input_dims);
        if (input_dims == 1) {
          auto num_of_elems = input->NumElements();

          KERNEL_CHECK_FALSE((num_of_elems == 1 || num_of_elems == batch_size),
                             KERNEL_STATUS_PARAM_INVALID,
...
uint32_t ParameterizedTruncatedNormalCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(ParameterizedTruncatedNormalCheck(ctx),
...
REGISTER_CPU_KERNEL(kParameterizedTruncatedNormal,
                    ParameterizedTruncatedNormalCpuKernel);
}  // namespace aicpu
