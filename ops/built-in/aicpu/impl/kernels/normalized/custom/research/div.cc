...
uint32_t DivCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kDiv);
  BCalcInfo calc_info;
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Div kernel data type [%s] not support.",
...
*/
template <typename T>
uint32_t DivCpuKernel::SpecialComputeInt(BcastShapeType type, int64_t start,
    ...
    case BcastShapeType::SAME_SHAPE:
      for (int64_t i = start; i < end; ++i) {
        if (*(input2 + i) == static_cast<T>(0)) {
          ...
          if (((*(input1 + i)) * (*(input2 + i)) < static_cast<T>(0)) &&
              (mod != 0))
            *(output + i) =
                (*(input1 + i)) / (*(input2 + i)) - static_cast<T>(1);
          else
...
template <typename T>
uint32_t DivCpuKernel::SpecialCompute(BcastShapeType type, int64_t start,
                                      int64_t end, const T *input1,
                                      const T *input2, T *output) {
  switch (type) {
    case BcastShapeType::SAME_SHAPE:
...
uint32_t DivCpuKernel::NoBcastComputeInt(CpuKernelContext &ctx) {
  ...
  if (data_num >= kParallelDataNumSameShape) {
    ...
    auto sharder_div = [&](int64_t start, int64_t end) {
      SpecialComputeInt<T>(type, start, end, in0, in1, out);
    };
    KERNEL_HANDLE_ERROR(
    ...
    auto sharder_divnonan = [&](int64_t start, int64_t end) {
      for (int64_t i = start; i < end; ++i) {
        if (*(in1 + bcast.GetBroadcastYIndex(i)) == static_cast<T>(0)) {
          ...
          T mod;
          mod = *(in0 + bcast.GetBroadcastXIndex(i)) %
                *(in1 + bcast.GetBroadcastYIndex(i));
          if (((*(in0 + bcast.GetBroadcastXIndex(i))) *
                   (*(in1 + bcast.GetBroadcastYIndex(i))) <
               static_cast<T>(0)) &&
              (mod != 0))
