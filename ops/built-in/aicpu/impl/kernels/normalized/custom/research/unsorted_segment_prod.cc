...
uint32_t UnsortedSegmentProdCpuKernel::UnsortedSegmentProdComputeTemplate(
  ...
  if (data_size <= kParallelDataNums) {
    ...
    KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(
                            ctx, reshapesize, reshapesize / max_core_num,
                            shard_unsorted_segment_prod),
...
uint32_t UnsortedSegmentProdCpuKernel::DoComputeWithNumSegmentsType(
    CpuKernelContext &ctx, DataType num_segments_type) {
  switch (num_segments_type) {
    case DT_INT32:
      return UnsortedSegmentProdComputeTemplate<input_t, segment_ids_t,
                                                int32_t>(ctx);
    case DT_INT64:
      return UnsortedSegmentProdComputeTemplate<input_t, segment_ids_t,
                                                int64_t>(ctx);
    ...
    default:
      KERNEL_LOG_ERROR(
          "UnsortedSegmentProd invalid num_segments_type type [%s]",
...
uint32_t UnsortedSegmentProdCpuKernel::DoComputeWithSegmentIdsType(
  ...
  switch (segment_ids_type) {
    ...
    default:
      KERNEL_LOG_ERROR("UnsortedSegmentProd invalid segment_ids_type type [%s]",
...
uint32_t UnsortedSegmentProdCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (input_type) {
    ...
    default:
      KERNEL_LOG_ERROR("UnsortedSegmentProd invalid input type [%s]",
