...
uint32_t DiagPartCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kDiagPart);
  KERNEL_HANDLE_ERROR(DiagPartCheck(ctx), "[%s] check params failed.",
                      kDiagPart);
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("DiagPart kernel data type [%s] not supports.",
...
uint32_t DiagPartCpuKernel::DiagPartCheck(CpuKernelContext &ctx) {
  ...
  for (size_t i = 0; i < shape_output.size(); i++) {
    KERNEL_CHECK_FALSE(
        (shape_input[i] == shape_input[i + shape_output.size()]),
        KERNEL_STATUS_PARAM_INVALID,
        "Invalid shape: the input dimension [%zu] size [%zu] does not match "
        "the input dimension [%zu] size [%zu].",
        i, shape_input[i], i + shape_output.size(),
        shape_input[i + shape_output.size()]);
  }
