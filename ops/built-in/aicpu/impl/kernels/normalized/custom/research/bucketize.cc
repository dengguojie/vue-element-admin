...
uint32_t BucketizeCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_NULLPTR(ctx.GetAttr("boundaries"), KERNEL_STATUS_PARAM_INVALID,
                       "Get boundaries failed")

  // check input datatype
  Tensor *input = ctx.Input(kFirstInputIndex);
  DataType dt_input = input->GetDataType();
  KERNEL_CHECK_FALSE(
      (dt_input == DT_FLOAT || dt_input == DT_INT32 || dt_input == DT_INT64 ||
       dt_input == DT_DOUBLE),
  ...
  KERNEL_CHECK_FALSE((dt_output == DT_INT32), KERNEL_STATUS_PARAM_INVALID,
                     "Output data type must DT_INT32, but got data type[%s].",
  ...
  KERNEL_CHECK_FALSE((input_sizes == output_sizes), KERNEL_STATUS_PARAM_INVALID,
                     ...
                     "output [%s].",
                     VectorToString(input_sizes).c_str(),
                     VectorToString(output_sizes).c_str());
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Bucketize kernel data type [%s] not support.",
    ...
    auto sharder_bucketize = [&](int64_t start, int64_t end) {
      for (int64_t i = start; i < end; i++) {
        auto first_bigger_it = std::upper_bound(
            boundaries_data.begin(), boundaries_data.end(), input_data[i]);
