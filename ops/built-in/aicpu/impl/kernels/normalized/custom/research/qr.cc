...
uint32_t QrCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kQr);
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Qr kernel data type [%s] not support.",
...
uint32_t QrCpuKernel::QrCheck(CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_NULLPTR(attr_full_matrices, KERNEL_STATUS_PARAM_INVALID,
                       "Get full_matrices attr failed.")
  ...
  KERNEL_CHECK_FALSE((shape_x[shape_size - 2] > 0), KERNEL_STATUS_PARAM_INVALID,
                     "Dimension [%zu] must be at least 1, but [%zu].",
                     shape_size - 2, shape_x[shape_size - 2])
  KERNEL_CHECK_FALSE((shape_x[shape_size - 1] > 0), KERNEL_STATUS_PARAM_INVALID,
                     "Dimension [%zu] must be at least 1, but [%zu].",
                     shape_size - 1, shape_x[shape_size - 1])
    ...
    if (data_size <= kParallelDataNums) {
      ...
      KERNEL_HANDLE_ERROR(
          CpuKernelUtils::ParallelFor(ctx, martix_num,
                                      martix_num / max_core_num, shard_qr),
          "Qr Compute failed.")
    }
  }
  return KERNEL_STATUS_OK;
