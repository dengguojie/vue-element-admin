...
uint32_t MinimumCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Minimum kernel data type [%s] not support.",
...
*/
template <typename T>
void MinimumCpuKernel::SpecialCompute(BcastShapeType type, int64_t start,
  ...
  switch (type) {
    case BcastShapeType::SAME_SHAPE:
      for (int64_t i = start; i < end; ++i) {
        *(output + i) =
            *(input1 + i) < *(input2 + i) ? *(input1 + i) : *(input2 + i);
      }
      break;
