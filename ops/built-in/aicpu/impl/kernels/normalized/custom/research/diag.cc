...
uint32_t DiagCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kDiag);
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Diag kernel data type [%s] not support.",
...
uint32_t DiagCpuKernel::DiagCheck(CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_NULLPTR(ctx.Output(0)->GetTensorShape(),
                       KERNEL_STATUS_PARAM_INVALID,
  ...
  KERNEL_CHECK_FALSE((shape_input.size() != 0), KERNEL_STATUS_PARAM_INVALID,
                     ...
                     shape_input.size())
  KERNEL_CHECK_FALSE(
      (shape_input.size() != shape_output.size() * 2),
      KERNEL_STATUS_PARAM_INVALID,
  ...
  for (size_t i = 0; i < shape_output.size(); ++i) {
    KERNEL_CHECK_FALSE(
        (shape_input[i % shape_input.size()] == shape_output[i]),
        KERNEL_STATUS_PARAM_INVALID,
        "Invalid shape: the input dimension [%zu] size [%zu] does not match "
        "the output dimension [%zu] size [%zu].",
        i % shape_input.size(), shape_input[i % shape_input.size()], i,
        shape_output[i])
  }
...
uint32_t DiagCpuKernel::DiagCompute(CpuKernelContext &ctx) {
  ...
  if (data_size <= kParallelDataNums) {
    ...
    KERNEL_HANDLE_ERROR(
        CpuKernelUtils::ParallelFor(ctx, size, size / max_core_num, shard_diag),
        "Diag Compute failed.")
  }
  return KERNEL_STATUS_OK;
}
