...
uint32_t UnpackCpuKernel::UnpackWithDimZero(T *input_data_ptr,
  ...
  for (int64_t i = 0; i < unpack_num; i++) {
    ...
    KERNEL_CHECK_FALSE(
        ...
        "Memcpy size[%zu] from input value to output[%d] failed.", copy_size,
        i);
...
uint32_t UnpackCpuKernel::UnpackCompute(T *input_data_ptr,
  ...
  auto shard_unpack = [&](size_t start, size_t end) {
    ...
    for (uint64_t i = start; i < end; i++) {
      ...
      for (int64_t j = 0; j < prefix; j++) {
        auto mem_ret =
            memcpy_s(output_data_ptr, copy_size, input_copy_ptr, copy_size);
        KERNEL_CHECK_FALSE(
            (mem_ret == EOK), KERNEL_STATUS_PARAM_INVALID,
  ...
  KERNEL_HANDLE_ERROR(
      CpuKernelUtils::ParallelFor(ctx, unpack_num, unpack_num / max_core_num,
                                  shard_unpack),
