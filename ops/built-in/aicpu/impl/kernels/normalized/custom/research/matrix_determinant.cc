...
uint32_t MatrixDeterminantCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(MatrixDeterminantCheck(ctx),
                      "MatrixDeterminant check params failed.");
  ...
  if (dims[dims.size() - 1] == 0 ||
      dims[dims.size() - 2] != dims[dims.size() - 1]) {
    KERNEL_LOG_ERROR("The input must be one or more squares.");
    return KERNEL_STATUS_PARAM_INVALID;
  }
  // Check element type
  ...
  switch (input_dtype) {
    MatrixDeterminant_COMPUTE_CASE(DT_FLOAT, float, ctx)
    MatrixDeterminant_COMPUTE_CASE(DT_DOUBLE, double, ctx)
    MatrixDeterminant_COMPUTE_CASE(DT_COMPLEX64, std::complex<float>,ctx)
    MatrixDeterminant_COMPUTE_CASE(DT_COMPLEX128, std::complex<double>, ctx) 
    default: KERNEL_LOG_ERROR("MatrixDeterminant kernel dims data_type [%s] "
...
uint32_t MatrixDeterminantCpuKernel::MatrixDeterminantCheck(
...
template <typename T>
uint32_t MatrixDeterminantCpuKernel::MatrixDeterminantCompute(
    CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(
      CpuKernelUtils::ParallelFor(ctx, n, 1, shard_matrix_determinant),
