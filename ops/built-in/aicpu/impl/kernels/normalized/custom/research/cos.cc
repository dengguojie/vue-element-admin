...
inline Eigen::half ScalarCos(Eigen::half x) {
  const Eigen::half val{
      static_cast<Eigen::half>(std::cos(static_cast<std::float_t>(x)))};
  return Eigen::half_impl::isnan(val) ? Eigen::half{0.0f} : val;
}

inline std::uint32_t ParallelForCos(
...
inline std::uint32_t ComputeCosKernel(const CpuKernelContext &ctx) {
  ...
  return ParallelForCos(ctx, total, per_unit_size,
                        [&](std::int64_t begin, std::int64_t end) {
                          std::transform(input0 + begin, input0 + end,
                                         output + begin, ScalarCos<T>);
                        });
}
...
inline std::uint32_t CheckCos(CpuKernelContext &ctx, std::uint32_t inputs_num,
                              std::uint32_t outputs_num) {
  return NormalCheck(ctx, kCosInputNum, kCosOutputNum)
...
std::uint32_t CosCpuKernel::Compute(CpuKernelContext &ctx) {
  return detail::CheckCos(ctx, kCosInputNum, kCosOutputNum)
             ? KERNEL_STATUS_PARAM_INVALID
             : detail::ComputeCos(ctx);
}
