...
uint32_t Expm1CpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kExpm1);
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Expm1 kernel data type [%s] not support.",
...
uint32_t Expm1CpuKernel::Expm1Compute3(CpuKernelContext &ctx) {
  ...
  size_t data_num = ctx.Input(0)->NumElements();
  int64_t data_size = data_num * sizeof(T);
  if ((data_type == DT_DOUBLE && data_size <= 64 * 1024) ||
      (data_type == DT_FLOAT && data_size <= 16 * 1024)) {
    for (size_t i = 0; i < data_num; i++) {
