...
namespace {
const char *kUnsortedSegmentMin = "UnsortedSegmentMin";
...
uint32_t UnsortedSegmentMinCpuKernel::UnsortedSegmentMinComputeTemplate(
    CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, input_num, output_num),
                      " node input size should be [%llu],  get [%llu]",
                      input_num, ctx.GetInputsSize(),
                      " node output size should be [%llu],  get [%llu]",
                      output_num, ctx.GetOutputsSize());
  ...
  KERNEL_CHECK_NULLPTR(segmentids, KERNEL_STATUS_PARAM_INVALID,
                       "Get segment_ids failed")
  auto numsegments =
      reinterpret_cast<num_segments_t *>(ctx.Input(2)->GetData());
  KERNEL_CHECK_NULLPTR(numsegments, KERNEL_STATUS_PARAM_INVALID,
                       "Get num_segments failed")
  if (id_size <= 0){
    KERNEL_LOG_ERROR("segment_ids num elements should great than 0");
    return KERNEL_STATUS_PARAM_INVALID;
  }
  int64_t reshapesize = data_size / id_size;
  // Initialized to the minimum value of the data type
  for (int64_t k = 0; k < data_size; k++) {
    *(output_y + k) = std::numeric_limits<input_t>::max();
  }
  if (data_size <= kParallelDataNums) {
    ...
    for (int64_t i = 0; i < id_size; i++) {
      for (int64_t j = 0; j < reshapesize; j++) {
        if (*(segmentids + i) < *numsegments) {
          if (*(output_y + *(segmentids + i) * reshapesize + j) >
              *(input_x + i * reshapesize + j)) {
            *(output_y + *(segmentids + i) * reshapesize + j) =
                *(input_x + i * reshapesize + j);
          }
        }
      }
    ...
    // calculation process
    auto shard_unsorted_segment_max = [&](int64_t start, int64_t end) {
      for (int64_t i = 0; i < id_size; i++) {
    ...
    KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, reshapesize,
                                                    reshapesize / max_core_num,
                                                    shard_unsorted_segment_max),
...
uint32_t UnsortedSegmentMinCpuKernel::DoComputeWithNumSegmentsType(
    CpuKernelContext &ctx, DataType num_segments_type) {
  switch (num_segments_type) {
    case DT_INT32:
      return UnsortedSegmentMinComputeTemplate<input_t, segment_ids_t, int32_t>(
          ctx);
    case DT_INT64:
      return UnsortedSegmentMinComputeTemplate<input_t, segment_ids_t, int64_t>(
          ctx);

    default:
      KERNEL_LOG_ERROR("UnsortedSegmentMin invalid num_segments_type type [%s]",
...
uint32_t UnsortedSegmentMinCpuKernel::DoComputeWithSegmentIdsType(
    CpuKernelContext &ctx, DataType segment_ids_type) {
  auto num_segments_type = ctx.Input(2)->GetDataType();
  switch (segment_ids_type) {
    case DT_INT32:
      return DoComputeWithNumSegmentsType<input_t, int32_t>(ctx,
                                                            num_segments_type);
    case DT_INT64:
      return DoComputeWithNumSegmentsType<input_t, int64_t>(ctx,
                                                            num_segments_type);

    default:
      KERNEL_LOG_ERROR("UnsortedSegmentMin invalid segment_ids_type type [%s]",
                       DTypeStr(segment_ids_type).c_str());
      return KERNEL_STATUS_PARAM_INVALID;
  }
}

uint32_t UnsortedSegmentMinCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (input_type) {
    ...
    default:
      KERNEL_LOG_ERROR("UnsortedSegmentMin invalid input type [%s]",
...
REGISTER_CPU_KERNEL(kUnsortedSegmentMin, UnsortedSegmentMinCpuKernel);
}  // namespace aicpu
