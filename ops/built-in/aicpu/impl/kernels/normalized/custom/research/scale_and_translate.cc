...
uint32_t ScaleAndTranslateGradCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(ScaleAndTranslateGradCheck(ctx),
                      "ScaleAndTranslateGrad check params failed.");
  auto data_type = ctx.Input(0)->GetDataType();
  switch (data_type) {
    SCALEANDTRANSLATEGRAD_COMPUTE_CASE(DT_FLOAT, float, ctx)
    default:
      KERNEL_LOG_ERROR(
          "ScaleAndTranslateGrad kernel data type [%s] not support.",
          DTypeStr(data_type).c_str());
      return KERNEL_STATUS_PARAM_INVALID;
  }
...
uint32_t ScaleAndTranslateGradCpuKernel::ScaleAndTranslateGradCheck(
    CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_FALSE((input1_shape->GetDims() == 4),
                     KERNEL_STATUS_PARAM_INVALID,
                     "The original_image dims=[%d] must be 4-dimensional",
  ...
  DataType input3_type = ctx.Input(3)->GetDataType();
  // dtypes check
  KERNEL_CHECK_FALSE((input1_type == DT_FLOAT), KERNEL_STATUS_PARAM_INVALID,
                     "The input1's dtype=[%d] must be DT_FLOAT",
                     DTypeStr(input1_type).c_str())
  KERNEL_CHECK_FALSE((input2_type == DT_FLOAT), KERNEL_STATUS_PARAM_INVALID,
                     "The input2's type=[%d] must be DT_FLOAT",
                     DTypeStr(input2_type).c_str())
  KERNEL_CHECK_FALSE((input3_type == DT_FLOAT), KERNEL_STATUS_PARAM_INVALID,
                     "The input3's dtype=[%d] must be DT_FLOAT",
                     DTypeStr(input3_type).c_str())
...
uint32_t ComputeSpansCore(CpuKernelContext &context, const Kernel &kernel,
  ...
  Eigen::TensorMap<Eigen::Tensor<int32_t, 1>> starts_vec(
      spans->starts->data(), spans->starts->dimensions());
  Eigen::TensorMap<Eigen::Tensor<float, 1>> weights_vec(
      spans->weights->data(), spans->weights->dimensions());
  weights_vec.setZero();
  ...
  auto shard_x = [&](int start, int end) {
    for (auto x = start; x < end; ++x) {
      ...
      if (this_span_size > spans->span_size) {
        KERNEL_LOG_ERROR("Span is too large: [%d] vs [%d].", this_span_size,
                         spans->span_size);
        return KERNEL_STATUS_PARAM_INVALID;
      }
...
uint32_t ComputeGradSpansCore(CpuKernelContext &context, const Spans *spans,
  ...
  grad_spans->weights =
      new Eigen::Tensor<float, 1>(grad_spans->span_size * forward_input_size);
  Eigen::TensorMap<Eigen::Tensor<int32_t, 1>> grad_starts_vec(
      grad_spans->starts->data(), grad_spans->starts->dimensions());
  Eigen::TensorMap<Eigen::Tensor<float, 1>> grad_weights_vec(
      grad_spans->weights->data(), grad_spans->weights->dimensions());
...
uint32_t ComputeGradSpans(CpuKernelContext &context,
                          const SamplingKernelType kernel_type,
                          const int64_t forward_output_size,
                          ...
                          Spans *grad_spans) {
  Spans spans;
  ComputeSpans(context, kernel_type, forward_output_size, forward_input_size,
               scale, translate, antialias, &spans);
  uint32_t Status = ComputeGradSpansCore(context, &spans, forward_output_size,
...
uint32_t ScaleAndTranslateCpuKernel::ScaleAndTranslateCompute(
  ...
  SamplingKernelType kernel_type_ =
      SamplingKernelTypeFromString(kernel_type_str);
  ...
  KERNEL_CHECK_FALSE(
      (output_height > 0 && output_width > 0), KERNEL_STATUS_PARAM_INVALID,
      "output_height = [%d] and output_width = [%d] must be positive",
      output_height, output_width)
  KERNEL_CHECK_FALSE((channels > 0), KERNEL_STATUS_PARAM_INVALID,
                     "image_channel = [%d] must have at least one", channels)
  KERNEL_CHECK_FALSE(
      (input_height > 0 && input_width > 0), KERNEL_STATUS_PARAM_INVALID,
  ...
  KERNEL_CHECK_FALSE(
      (row_scale > 0 && col_scale > 0), KERNEL_STATUS_PARAM_INVALID,
  ...
  Spans col_spans;
  ComputeSpans(ctx, kernel_type_, output_width, input_width, col_scale,
               col_translation, antialias_, &col_spans);

  Spans row_spans;
  ComputeSpans(ctx, kernel_type_, output_height, input_height, row_scale,
               row_translation, antialias_, &row_spans);

  Eigen::Tensor<float, 4> intermediate_tensor_middle(batch_size, output_height,
...
template <typename T>
uint32_t ScaleAndTranslateGradCpuKernel::ScaleAndTranslateGradCompute(
    CpuKernelContext &ctx) {
  auto input_scale = reinterpret_cast<float *>(ctx.Input(2)->GetData());
  auto input_translation = reinterpret_cast<float *>(ctx.Input(3)->GetData());
  std::string kernel_type_str = ctx.GetAttr("kernel_type")->GetString();
  ...
  KERNEL_CHECK_FALSE(
      (row_scale > 0 && col_scale > 0), KERNEL_STATUS_PARAM_INVALID,
      "row_scale = [%d] and col_scale = [%d] must be of non-zero size",
      row_scale, col_scale)
  ...
  Spans col_spans;
  ComputeGradSpans(ctx, kernel_type_, forward_output_width, forward_input_width,
                   col_scale, col_translation, antialias_, &col_spans);

  Spans row_spans;
  ComputeGradSpans(ctx, kernel_type_, forward_output_height,
                   forward_input_height, row_scale, row_translation, antialias_,
                   &row_spans);

  Eigen::Tensor<float, 4> intermediate_tensor_middle(
      batch_size, forward_input_height, forward_output_width, channels);
  Eigen::TensorMap<Eigen::Tensor<float, 4>> intermediate_data(
      intermediate_tensor_middle.data(),
      intermediate_tensor_middle.dimensions());
  ...
  GatherSpans<T>()(ctx, row_spans.span_size, row_starts, row_weights,
                   col_spans.span_size, col_starts, col_weights, input_grad,
