...
const char *kReduceSum = "ReduceSum";
...
uint32_t ReduceSumCpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kReduceSumInputNum, kReduceSumOutputNum),
  ...
  switch (input_data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("ReduceSum kernel data type [%s] not support.",
                       DTypeStr(input_data_type).c_str());
      return KERNEL_STATUS_PARAM_INVALID;
  }
  return KERNEL_STATUS_OK;
}
uint32_t ReduceSumCpuKernel::ReduceSumCheck(CpuKernelContext &ctx) {
  KERNEL_CHECK_NULLPTR(ctx.Input(0)->GetData(),
...
uint32_t ReduceSumCpuKernel::ReduceSumOneAxes2(T *input_data, int64_t input_num, std::vector<int64_t> input_shape,
  ...
  uint32_t result = ReduceSumOneAxes2<T, T2>(output_data_temp, output_num_temp, input_shape,
                                             output_data, output_num, axes, axes_idx);
  if (output_data_temp != nullptr) {
    delete[] output_data_temp;
  }
