...
namespace aicpu {
  ...
  void GetIntConstData(const Tensor& const_tensor, std::vector<std::vector<int64_t>> & pad_data) {
    ...
    size_t pading_size = const_tensor.GetDataSize() / sizeof(T);
    for (size_t i = 0; i < pading_size; ++i) {
      vector_data.push_back((T)((*(const_data_ptr + i))));
      KERNEL_LOG_DEBUG("idx:value = %d", i);
    }
    for (size_t i = 1; i < vector_data.size(); i += GetDatastep) {
  ...
  uint32_t DoCompute(CpuKernelContext &ctx) {
    ...
    if (pad.empty()) {
      ...
      if (ret != EOK) {
        KERNEL_LOG_ERROR("PadD Memcpy failed, result = [%d].", ret);
...
uint32_t PadDCpuKernel::SetMap_padd(CpuKernelContext &ctx) {
  calls_padd[DT_FLOAT] = pad::DoCompute<float>;
  calls_padd[DT_FLOAT16] = pad::DoCompute<Eigen::half>;
  calls_padd[DT_INT32] = pad::DoCompute<int32_t>;
  Tensor *x = ctx.Input(0);
  ...
  return calls_padd[x_type](ctx);
}

uint32_t PadDCpuKernel::SetMap_pad(CpuKernelContext &ctx) {
  calls_pad[DT_INT8] = pad::DoCompute<int8_t>;
  calls_pad[DT_UINT8] = pad::DoCompute<uint8_t>;
  calls_pad[DT_INT16] = pad::DoCompute<int16_t>;
  calls_pad[DT_UINT16] = pad::DoCompute<uint16_t>;
  calls_pad[DT_INT32] = pad::DoCompute<int32_t>;
  calls_pad[DT_UINT32] = pad::DoCompute<uint32_t>;
  calls_pad[DT_INT64] = pad::DoCompute<int64_t>;
  calls_pad[DT_UINT64] = pad::DoCompute<uint64_t>;
  calls_pad[DT_BOOL] = pad::DoCompute<bool>;
  calls_pad[DT_FLOAT16] = pad::DoCompute<Eigen::half>;
  calls_pad[DT_FLOAT] = pad::DoCompute<float>;
  calls_pad[DT_DOUBLE] = pad::DoCompute<double>;
  Tensor *x = ctx.Input(0);
  auto x_type = x->GetDataType();
