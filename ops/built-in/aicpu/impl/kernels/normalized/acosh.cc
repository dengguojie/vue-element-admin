...
inline std::uint32_t ComputeAcoshKernel(const CpuKernelContext &ctx) {
  ...
  return ParallelForAcosh(ctx, total, per_unit_size,
                          [&](std::int64_t begin, std::int64_t end) {
                            std::transform(input0 + begin, input0 + end,
                                           output + begin, ScalarAcosh<T>);
                          });
}
template <typename T>
...
std::uint32_t AcoshCpuKernel::Compute(CpuKernelContext &ctx) {
  return detail::CheckAcosh(ctx, kAcoshInputNum, kAcoshOutputNum)
             ? KERNEL_STATUS_PARAM_INVALID
             : detail::ComputeAcosh(ctx);
}
