 ...
 * Description: implement of cache_swap_hashmap_kernels.h
 */
...
{
    template <typename T>
    T Compress(HashmapEntry<T> *entry_p, const int64_t &length, T entry)
    {
    ...
    template <typename T>
    uint32_t CacheSwapHashmapTask(std::vector<Tensor *> &inputs_, std::vector<Tensor *> &outputs_, int64_t &batch_size_, int64_t &hashmap_length_)
    {
        auto start = std::chrono::high_resolution_clock::now();

        if (inputs_.size() == 0 || outputs_.size() == 0)
        {
            KERNEL_LOG_ERROR("CacheSwapHashmapKernel::CacheSwapHashmapTask: input or output is empty.");
            return KERNEL_STATUS_PARAM_INVALID;
        }

        HashmapEntry<T> *hashmap = reinterpret_cast<HashmapEntry<T> *>(inputs_[0]->GetData());
        T *miss_emb_idx = reinterpret_cast<T *>(inputs_[1]->GetData());
        T step_ = *reinterpret_cast<T *>(inputs_[2]->GetData());

        T *swap_cache_idx = reinterpret_cast<T *>(outputs_[0]->GetData()); //16000*39, if miss, cacheIdx=-1
        T *old_emb_idx = reinterpret_cast<T *>(outputs_[1]->GetData());    // 16000*39, default -1

        float total_tag_count = 0;
        ...
        for (int64_t i = 0; i < batch_size_; ++i)
        {
            ...
            else
            {
                ...
                T compress_count = Compress(hashmap, hashmap_length_, tmp_entry);
        ...
        KERNEL_LOG_INFO("CacheSwapHashmap insert count %f.", total_tag_count / count_size);
        KERNEL_LOG_INFO("CacheSwapHashmap delect count %f.", total_delete_count / count_size);
        KERNEL_LOG_INFO("CacheSwapHashmap execute %fms.", std::chrono::duration<double, std::milli>(end - start).count());
        return KERNEL_STATUS_OK;
    }

    uint32_t CacheSwapHashmapKernel::DoCompute()
    {
        std::map<int, std::function<uint32_t(std::vector<Tensor *> &, std::vector<Tensor *> &, int64_t &, int64_t &)>> calls;
        ...
        {
            KERNEL_LOG_ERROR("CacheSwapHashmapKernel op don't support input tensor types: %s", typeid(param_type_).name());
            return KERNEL_STATUS_PARAM_INVALID;
        }
    ...
    uint32_t CacheSwapHashmapKernel::GetInputAndCheck(CpuKernelContext &ctx)
    {
        ...
        for (int i = 0; i < num_input; ++i)
        {
            ...
            if (tensor == nullptr)
            {
                KERNEL_LOG_ERROR("CacheSwapHashmapKernel::GetInputAndCheck: get input tensor[%d] failed", i);
                return KERNEL_STATUS_PARAM_INVALID;
            }
        ...
        for (int i = 0; i < num_output; ++i)
        {
            ...
            if (tensor == nullptr)
            {
                KERNEL_LOG_ERROR("CacheSwapHashmapKernel::GetInputAndCheck: get output tensor[%d] failed", i);
                return KERNEL_STATUS_PARAM_INVALID;
            }
        ...
        {
            KERNEL_LOG_ERROR("CacheSwapHashmapKernel::GetInputAndCheck: only support hashmap rank 2, but got %d", hashmap_shape->GetDims());
            return KERNEL_STATUS_PARAM_INVALID;
        }
        ...
        {
            KERNEL_LOG_ERROR("CacheSwapHashmapKernel::GetInputAndCheck: only support hashmap shape (n, 4), but got (%d, %d)", hashmap_length_, hashmap_shape->GetDimSize(1));
            return KERNEL_STATUS_PARAM_INVALID;
        }
