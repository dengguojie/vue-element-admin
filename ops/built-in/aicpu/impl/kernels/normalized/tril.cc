...
uint32_t TrilCpuKernel::ValidParam(CpuKernelContext &ctx) {
  ...
  KERNEL_CHECK_FALSE(input_dims >= minDims,
  ...
  KERNEL_CHECK_FALSE(input_data_type == output_data_type,
                     KERNEL_STATUS_PARAM_INVALID,
                     "The data type of input [%s] need be same with output [%s].",
                     DTypeStr(input_data_type).c_str(),
                     DTypeStr(output_data_type).c_str())

  KERNEL_CHECK_FALSE(input_shape->GetDimSizes() == output_shape->GetDimSizes(),
                     KERNEL_STATUS_PARAM_INVALID,
...
uint32_t TrilCpuKernel::ComputeTril(CpuKernelContext &ctx, size_t k) {
  ...
  MatrixMap input(reinterpret_cast<T *>(ctx.Input(0)->GetData()) + k * matrix_size_,
                    matrix_width_, matrix_height_);
  MatrixMap output(reinterpret_cast<T *>(ctx.Output(0)->GetData()) + k * matrix_size_,
                matrix_width_, matrix_height_);
...
uint32_t TrilCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Tril kernel data type [%s] not support.",
