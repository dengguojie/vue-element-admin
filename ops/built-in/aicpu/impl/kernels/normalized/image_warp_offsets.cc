          ...
          for (int64_t i_w = 0; i_w < output_shape[3]; ++i_w) {
            if (input1(i_n, i_i, i_h, i_w) >= static_cast<TIndex>(in_hxwxc)) {
              ...
              string err_msg = ConcatString(
                  kImageWarpOffsets, " op input[1] value[", i_n, "][", i_i,
                  "][", i_h, "][", i_w, "]=[", input1(i_n, i_i, i_h, i_w),
                  "] should < [", in_hxwxc, "]");
...
uint32_t ImageWarpOffsetsCpuKernel::CheckParam(CpuKernelContext &ctx,
  ...
  if (param_dim_sizes.size() != rank) {
    KERNEL_LOG_ERROR("%s shape rank should be [%d], but got shape[%zu].",
...
uint32_t ImageWarpOffsetsCpuKernel::CheckShapes(CpuKernelContext &ctx) {
  ...
  if (input1_shape[1] != kPointsNum) {
    KERNEL_LOG_ERROR(
        "%s op input[1] shape second dim should be [%d], but got "
        "shape[%s].",
