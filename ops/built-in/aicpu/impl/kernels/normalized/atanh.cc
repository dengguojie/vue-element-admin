...
inline std::uint32_t ComputeAtanhKernel(const CpuKernelContext &ctx) {
  ...
  return ParallelForAtanh(ctx, total, per_unit_size,
                          [&](std::int64_t begin, std::int64_t end) {
                            std::transform(input0 + begin, input0 + end,
                                           output + begin, ScalarAtanh<T>);
                          });
}
template <typename T>
...
std::uint32_t AtanhCpuKernel::Compute(CpuKernelContext &ctx) {
  return detail::CheckAtanh(ctx, kAtanhInputNum, kAtanhOutputNum)
             ? KERNEL_STATUS_PARAM_INVALID
             : detail::ComputeAtanh(ctx);
}
