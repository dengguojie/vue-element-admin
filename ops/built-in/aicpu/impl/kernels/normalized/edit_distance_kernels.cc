 ...
 * Description: implement of edit_distance_kernels.h
 */
...
namespace aicpu {
template <typename T>
static void DistanceTask(GroupIterable &hypothesis_grouper, GroupIterable &truth_grouper,
    GroupIterable::IteratorStep &hypothesis_iter, GroupIterable::IteratorStep &truth_iter,
    Eigen::TensorMap<Eigen::Tensor<float, 1, Eigen::RowMajor>> &output_t, std::vector<int64_t> &output_strides,
    bool &normalize_)
{
    ...
    while (hypothesis_iter != hypothesis_grouper.end() && truth_iter != truth_grouper.end()) {
        ...
        if (g_truth == g_hypothesis) {
            ...
            output_t(loc) =
                LevenshteinDistance<T>(std::vector<T>(truth_seq.data(), truth_seq.data() + truth_seq.size()),
                std::vector<T>(hypothesis_seq.data(), hypothesis_seq.data() + hypothesis_seq.size()), cmp);
            if (normalize_)
            ...
            auto loc =
                std::inner_product(g_hypothesis.begin(), g_hypothesis.end(), output_strides.begin(), int64_t { 0 });
...
template <typename T>
static void UpdateResult(GroupIterable &hypothesis_grouper, GroupIterable &truth_grouper,
    GroupIterable::IteratorStep &hypothesis_iter, GroupIterable::IteratorStep &truth_iter,
    Eigen::TensorMap<Eigen::Tensor<float, 1, Eigen::RowMajor>> &output_t, std::vector<int64_t> &output_strides,
    bool &normalize_)
{
...
template <typename T>
uint32_t EditDistanceTask(std::vector<Tensor *> &inputs_, std::vector<Tensor *> &outputs_, bool &normalize_)
{
    if (inputs_.size() == 0 || outputs_.size() == 0) {
        KERNEL_LOG_ERROR("EditDistanceKernel::EditDistanceTask: input or output is empty.");
    ...
    std::vector<int64_t> hypothesis_st_shape((int64_t *)inputs_[2]->GetData(),
        (int64_t *)inputs_[2]->GetData() + inputs_[2]->GetTensorShape()->GetDimSize(0));
    std::vector<int64_t> truth_st_shape((int64_t *)inputs_[5]->GetData(),
        (int64_t *)inputs_[5]->GetData() + inputs_[5]->GetTensorShape()->GetDimSize(0));
    ...
    SparseTensor truth;
    if (hypothesis.CreateSparseTensor(hypothesis_indices, hypothesis_values, hypothesis_st_shape, sorted_order) !=
        KERNEL_STATUS_OK || truth.CreateSparseTensor(truth_indices, truth_values, truth_st_shape, sorted_order) !=
        KERNEL_STATUS_OK) {
    ...
    if (outptr == NULL) {
        KERNEL_LOG_ERROR("EditDistanceKernel::EditDistanceTask: output[0]->GetData is null.");
        return KERNEL_STATUS_PARAM_INVALID;
    }
    std::vector<int64_t> output_shape = outputs_[0]->GetTensorShape()->GetDimSizes();
    Eigen::TensorMap<Eigen::Tensor<float, 1, Eigen::RowMajor>> output_t(outptr, outputs_[0]->NumElements());
    output_t.setZero();
    ...
    DistanceTask<T>(hypothesis_grouper, truth_grouper, hypothesis_iter, truth_iter, output_t, output_strides,
        normalize_);
    UpdateResult<T>(hypothesis_grouper, truth_grouper, hypothesis_iter, truth_iter, output_t, output_strides,
        normalize_);
...
uint32_t EditDistanceKernel::DoCompute()
{
    ...
    if (calls.find(param_type_) == calls.end()) {
        KERNEL_LOG_ERROR("EditDistanceKernel op don't support input tensor types: %s", typeid(param_type_).name());
        return KERNEL_STATUS_PARAM_INVALID;
    }
...
uint32_t EditDistanceKernel::GetInputAndCheck(CpuKernelContext &ctx)
{
    ...
    for (int i = 0; i < num_input; ++i) {
        ...
        if (tensor == nullptr) {
            KERNEL_LOG_ERROR("EditDistanceKernel::GetInputAndCheck: get input tensor[%d] failed", i);
            return KERNEL_STATUS_PARAM_INVALID;
        }
    ...
    for (int i = 0; i < num_output; ++i) {
        ...
        if (tensor == nullptr) {
            KERNEL_LOG_ERROR("EditDistanceKernel::GetInputAndCheck: get output tensor[%d] failed", i);
            return KERNEL_STATUS_PARAM_INVALID;
        }
