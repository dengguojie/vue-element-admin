...
uint32_t BetaincCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  if (a_shape->GetDims() > 0 && b_shape->GetDims() > 0) {
    KERNEL_CHECK_FALSE((a_shape->GetDimSizes() == b_shape->GetDimSizes()),
                       KERNEL_STATUS_PARAM_INVALID,
                       "Shapes of a and b are inconsistent")
  }
  if (a_shape->GetDims() > 0 && x_shape->GetDims() > 0) {
    KERNEL_CHECK_FALSE((a_shape->GetDimSizes() == x_shape->GetDimSizes()),
                       KERNEL_STATUS_PARAM_INVALID,
  ...
  KERNEL_CHECK_FALSE(
      (b_dtype == a_dtype), KERNEL_STATUS_PARAM_INVALID,
      "The data type of input[1] [%s] need be same with input[0] [%s].",
      DTypeStr(b_dtype).c_str(), DTypeStr(a_dtype).c_str());
  KERNEL_CHECK_FALSE(
      (x_dtype == a_dtype), KERNEL_STATUS_PARAM_INVALID,
      "The data type of input[2] [%s] need be same with input[0] [%s].",
      DTypeStr(x_dtype).c_str(), DTypeStr(a_dtype).c_str());

  switch (a_dtype) {
    BETAINC_COMPUTE_CASE(DT_FLOAT, float, ctx)
    BETAINC_COMPUTE_CASE(DT_DOUBLE, double, ctx)
    default:
      KERNEL_LOG_ERROR("Betainc kernel data type [%s] not support.",
...
uint32_t RunParallel(CpuKernelContext &ctx, std::vector<T *> data_pointers,
  ...
  auto shard_betainc = [&](size_t start, size_t end) {
    ...
    Eigen::TensorMap<Eigen::Tensor<T, 1>> input_2(data_pointers[2] + start,
                                                  end - start);
    Eigen::TensorMap<Eigen::Tensor<T, 1>> output(data_pointers[3] + start,
                                                 end - start);
