 ...
 * Description: implement of gather_kernels.h
 */
...
void CopyTask(size_t cur, std::vector<int64_t> &pos, T *input, I *index, int &dim, T *output,
    std::vector<int64_t> &output_shape, std::vector<int64_t> &out_cargo_size, std::vector<int64_t> &input_cargo_size)
{
    for (int64_t i = 0; i < output_shape[cur]; ++i) {
        ...
        if (cur == output_shape.size() - 1) {
            ...
            // CopyTask
            CopyTask(cur + 1, pos, input, index, dim, output, output_shape, out_cargo_size, input_cargo_size);
        }
    }
}

template <typename I, typename T> uint32_t GatherTask(std::vector<Tensor *> &inputs_, std::vector<Tensor *> &outputs_)
{
    ...
    KERNEL_CHECK_NULLPTR(input, KERNEL_STATUS_PARAM_INVALID, "GatherTask input null.")
    KERNEL_CHECK_NULLPTR(dim, KERNEL_STATUS_PARAM_INVALID, "GatherTask dim null.")
    KERNEL_CHECK_NULLPTR(index, KERNEL_STATUS_PARAM_INVALID, "GatherTask index null.")
    ...
    std::vector<int64_t> out_cargo_size = std::vector<int64_t>(output_shape.size(), 1);
    for (int i = out_cargo_size.size() - 2; i >= 0; --i) {
        out_cargo_size[i] = output_shape[i + 1] * out_cargo_size[i + 1];
    }
    // input_cargo_size
    std::vector<int64_t> input_shape = inputs_[0]->GetTensorShape()->GetDimSizes();
    std::vector<int64_t> input_cargo_size = std::vector<int64_t>(input_shape.size(), 1);
    ...
    int copy_dim = *dim;
    CopyTask<I, T>(0, pos, input, index, copy_dim, out, output_shape, out_cargo_size, input_cargo_size);
    return KERNEL_STATUS_OK;
}

uint32_t GatherKernel::DoCompute()
{
    std::map<int, std::map<int, std::function<uint32_t(std::vector<Tensor *> &, std::vector<Tensor *> &)>>> calls;
    ...
    if (calls.find(index_type_) == calls.end()) {
        KERNEL_LOG_ERROR("GatherKernel op don't support index tensor types: %s", typeid(index_type_).name());
        return KERNEL_STATUS_PARAM_INVALID;
    }
...
uint32_t GatherKernel::GetInputAndCheck(CpuKernelContext &ctx)
{
    ...
    for (int i = 0; i < num_input; ++i) {
        ...
        if (tensor == nullptr) {
            KERNEL_LOG_ERROR("GatherKernel::GetInputAndCheck: get input tensor[%d] failed", i);
            return KERNEL_STATUS_PARAM_INVALID;
        }
    ...
    for (int i = 0; i < num_output; ++i) {
        ...
        if (tensor == nullptr) {
            KERNEL_LOG_ERROR("GatherKernel::GetInputAndCheck: get output tensor[%d] failed", i);
            return KERNEL_STATUS_PARAM_INVALID;
        }
