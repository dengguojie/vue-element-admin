...
uint32_t SquaredDifferenceCpuKernel::SquaredDifferenceCheck(CpuKernelContext &ctx) {
  ...
  KERNEL_LOG_DEBUG(
      "SquaredDifferenceCpuKernel[%s], input0: size[%llu];"
...
uint32_t SquaredDifferenceCpuKernel::NoBcastCompute(CpuKernelContext &ctx) {
  ...
  if (data_num >= kParallelDataNumSameShape) {
    ...
    auto sharder_squareddifference = [&](int64_t start, int64_t end) {
...
template <typename T>
uint32_t SquaredDifferenceCpuKernel::SquaredDifferenceCompute(CpuKernelContext &ctx) {
