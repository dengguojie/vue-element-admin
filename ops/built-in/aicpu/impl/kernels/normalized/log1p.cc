...
uint32_t Log1pCpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kLog1p);
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("Log1p kernel data type [%s] not support.",
  ...
  if (data_size <= kParallelDataNums) {
    if (data_type == DT_COMPLEX64) {
      for (int64_t i = 0; i < data_num; i++) {
        ...
        KERNEL_CHECK_FALSE(array_x(0, i).real() >= static_cast<float>(-1),
      ...
      for (int64_t i = 0; i < data_num; i++) {
        array_x(0, i) = *(input_x + i);
        KERNEL_CHECK_FALSE(array_x(0, i).real() >= static_cast<double>(-1),
                           KERNEL_STATUS_PARAM_INVALID,
                           "[%llu] must be at least more than -1.", i);
        *(output_y + i) = Eigen::numext::log1p(*(input_x + i));
      }
