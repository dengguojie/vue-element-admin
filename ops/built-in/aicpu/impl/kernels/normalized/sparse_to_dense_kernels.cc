 ...
 * Description: implement of sparse to dense
 */
...
uint32_t SparseToDenseCpuKernel::SparseToDense(SparseTensor &st, Tensor *indices, Tensor *output)
{
    ...
    default:
        KERNEL_LOG_ERROR("sparse to dense can't support this data type = %d.", dt);
        return KERNEL_STATUS_PARAM_INVALID;
    }
...
uint32_t SparseToDenseCpuKernel::ValidParam(CpuKernelContext &ctx)
{
    ...
    bool validNull = ((output_tensor == nullptr) || default_value_tensor == nullptr ||
                      (sparse_values == nullptr) || (indices_tensor == nullptr) || (shape_tensor == nullptr));
    if (validNull) {
    ...
    bool validShapeNull = ((default_value_shape == nullptr) || values_shape == nullptr ||
    ...
    if ((values_dims_size != 0) && (values_dims_size != 1)) {
        KERNEL_LOG_ERROR("values_shape should be a scalar or a vector, got dim size=%d.",
...
{
    KERNEL_LOG_INFO("start to execute Compute.");
    if (ValidParam(ctx) != KERNEL_STATUS_OK) {
        KERNEL_LOG_ERROR("valid sparse to dense param error.");
        return KERNEL_STATUS_PARAM_INVALID;
    }
    Tensor *indices_tensor = ctx.Input(0);
    KERNEL_CHECK_NULLPTR(indices_tensor, KERNEL_STATUS_PARAM_INVALID, "indices_tensor is null")
    Tensor *shape_tensor = ctx.Input(1);
    KERNEL_CHECK_NULLPTR(shape_tensor, KERNEL_STATUS_PARAM_INVALID, "shape_tensor is null")
    Tensor *sparse_values = ctx.Input(2);
    KERNEL_CHECK_NULLPTR(sparse_values, KERNEL_STATUS_PARAM_INVALID, "sparse_values is null")
    Tensor *default_value_tensor = ctx.Input(3);
    KERNEL_CHECK_NULLPTR(default_value_tensor, KERNEL_STATUS_PARAM_INVALID, "default_value_tensor is null")
    Tensor *output_tensor = ctx.Output(0);
    KERNEL_CHECK_NULLPTR(output_tensor, KERNEL_STATUS_PARAM_INVALID, "output_tensor is null")
    ...
    SparseTensor st;
    if (st.CreateSparseTensor(indices_tensor, sparse_values, dense_shape, order) != KERNEL_STATUS_OK) {
    ...
    // set default value
    size_t type_size = GetSizeByDataType(static_cast<DataType>(output_tensor->GetDataType()));
    if (type_size < 1) {
    ...
    for (int index = 0; index < output_size; ++index) {
        int cpyRet = memcpy_s(output_addr + (index * type_size), type_size, default_value_addr, type_size);
        if (cpyRet < 0) {
            KERNEL_LOG_ERROR("memcpy_s default value failed index=%d, type_size=%d.", index, type_size);
    ...
    KERNEL_LOG_INFO("execute Compute end.");
    return KERNEL_STATUS_OK;
}
