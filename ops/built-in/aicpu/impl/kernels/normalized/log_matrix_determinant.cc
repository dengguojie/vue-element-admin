...
uint32_t LogMatrixDeterminantCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(LogMatrixDeterminantCheck(ctx),
                      "[%s] check params failed.", kLogMatrixDeterminant);
  ...
  switch (data_type) {
    ...
    LOG_MATRIX_DETERMINANT_COMPUTE_CASE(DT_COMPLEX128, std::complex<double>,
                                        ctx)
    default:
      KERNEL_LOG_ERROR(
          "LogMatrixDeterminant kernel data type [%s] not support.",
...
uint32_t LogMatrixDeterminantCpuKernel::LogMatrixDeterminantCheck(
  ...
  KERNEL_CHECK_NULLPTR(output_0->GetTensorShape(), KERNEL_STATUS_PARAM_INVALID,
                       "Get output sign tensor shape failed.")
  KERNEL_CHECK_NULLPTR(output_1->GetTensorShape(), KERNEL_STATUS_PARAM_INVALID,
    ...
    if (data_size <= kParallelDataNums) {
      for (int64_t i = 0; i < martix_num; i++) {
        ...
        if (!Eigen::numext::isfinite(log_abs_det)) {
          sign = 0;
          log_abs_det =
              log_abs_det > 0 ? -std::log(RealT(0)) : std::log(RealT(0));
        }
        *(output_sign + i) = sign;
      ...
      KERNEL_HANDLE_ERROR(
          CpuKernelUtils::ParallelFor(ctx, martix_num,
                                      martix_num / max_core_num, shard_work),
