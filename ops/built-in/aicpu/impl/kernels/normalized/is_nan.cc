...
uint32_t IsNanCpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", KIsNan)
  ...
  switch (data_type) {
    ...
    default:
      KERNEL_LOG_ERROR("IsNan kernel data type [%s] not supports.",
...
uint32_t IsNanCpuKernel::IsNanCheck(CpuKernelContext &ctx) {
...
template <typename T>
uint32_t IsNanCpuKernel::IsNanCompute(CpuKernelContext &ctx) {
