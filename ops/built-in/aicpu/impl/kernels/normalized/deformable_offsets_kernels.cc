...
uint32_t DeformableOffsetsCpuKernel::Compute(CpuKernelContext &ctx)
{
    ...
    DataType xDataType = xTensor->GetDataType();
    if (xDataType != DT_FLOAT16) {
        KERNEL_LOG_ERROR("The current x input type only supports DT_FLOAT16, x data type is %d.", xDataType);
        return KERNEL_STATUS_PARAM_INVALID;
    }
    DataType offsetsDataType = offsetsTensor->GetDataType();

    auto xData = reinterpret_cast<Eigen::half *>(xTensor->GetData());
    KERNEL_CHECK_NULLPTR(xData, KERNEL_STATUS_PARAM_INVALID, "input x get data failed.");
    auto offsetsDataPtr = offsetsTensor->GetData();
    KERNEL_CHECK_NULLPTR(offsetsDataPtr, KERNEL_STATUS_PARAM_INVALID, "input offset get data failed.");
    auto yData = reinterpret_cast<Eigen::half *>(yTensor->GetData());
    KERNEL_CHECK_NULLPTR(yData, KERNEL_STATUS_PARAM_INVALID, "input y get data failed.");

    switch (offsetsDataType) {
        case DT_FLOAT16: {
            auto offsetsData = reinterpret_cast<Eigen::half *>(offsetsDataPtr);
            return DoCompute<Eigen::half>(xData, offsetsData, yData);
        }
        case DT_FLOAT: {
            auto offsetsData = reinterpret_cast<float *>(offsetsDataPtr);
            return DoCompute<float>(xData, offsetsData, yData);
        }
        default: {
            KERNEL_LOG_ERROR("input offsets datatype error, offsets data type is %d.", offsetsDataType);
            return KERNEL_STATUS_PARAM_INVALID;
        }
    }
...
uint32_t DeformableOffsetsCpuKernel::GetInputParam(CpuKernelContext &ctx)
{
    ...
    KERNEL_CHECK_NULLPTR(strides, KERNEL_STATUS_PARAM_INVALID, "get attr:strides failed.");
    stride_list_ = strides->GetListInt();

    // get attr: pads
    AttrValue *pads = ctx.GetAttr("pads");
    KERNEL_CHECK_NULLPTR(pads, KERNEL_STATUS_PARAM_INVALID, "get attr:pads failed.");
    pads_list_ = pads->GetListInt();

    // get attr: ksize
    AttrValue *ksize = ctx.GetAttr("ksize");
    KERNEL_CHECK_NULLPTR(ksize, KERNEL_STATUS_PARAM_INVALID, "get attr:ksize failed.");
    ksize_list_ = ksize->GetListInt();

    // get attr: dilations
    AttrValue *dilations = ctx.GetAttr("dilations");
    KERNEL_CHECK_NULLPTR(dilations, KERNEL_STATUS_PARAM_INVALID, "get attr:dilations failed.");
    dilation_list_ = dilations->GetListInt();

    // get attr: data_format
    AttrValue *xDataFormat = ctx.GetAttr("data_format");
    KERNEL_CHECK_NULLPTR(xDataFormat, KERNEL_STATUS_PARAM_INVALID, "get attr:data_format failed.");
    data_format_ = xDataFormat->GetString();

    // get attr: deformable_groups
    AttrValue *deformGroups = ctx.GetAttr("deformable_groups");
    KERNEL_CHECK_NULLPTR(deformGroups, KERNEL_STATUS_PARAM_INVALID, "get attr:deformable_groups failed.");
    deformable_groups_ = deformGroups->GetInt();

    yTensor = ctx.Output(0);
    if (yTensor == nullptr) {
...
uint32_t DeformableOffsetsCpuKernel::ParseInputParam()
{
    ...
    if (stride_list_.size() == STRIDE_SIZE_LIMIT) {
        ...
        KERNEL_LOG_ERROR("input strideList's size error, strideList's size:%d.", stride_list_.size());
        return KERNEL_STATUS_PARAM_INVALID;
    }
    if (pads_list_.size() == PAD_SIZE_LIMIT) {
        ...
        KERNEL_LOG_ERROR("input padsList's size error, padsList's size:%d.", pads_list_.size());
        return KERNEL_STATUS_PARAM_INVALID;
    }
    if (ksize_list_.size() == KSIZE_SIZE_LIMIT) {
        ...
        KERNEL_LOG_ERROR("input ksizeList's size error, ksizeList's size:%d.", ksize_list_.size());
        return KERNEL_STATUS_PARAM_INVALID;
    }
    ...
    auto offsetsShape = offsetsTensor->GetTensorShape();
    ...
    if (yShape == nullptr) {
        KERNEL_LOG_ERROR("get yTensor failed.");
        return KERNEL_STATUS_PARAM_INVALID;
    }
...
uint32_t DeformableOffsetsCpuKernel::CheckInputParam()
{
    ...
    if ((0 == param_.ksizeX % 2) || (0 == param_.ksizeY % 2)) {
        KERNEL_LOG_ERROR("ksize does not support even numbers temporarily, ksizeX:%d, ksizeY:%d.", param_.ksizeX,
                          param_.ksizeY);
        return KERNEL_STATUS_PARAM_INVALID;
    }

    if (0 != x_.channels % deformable_groups_) {
        KERNEL_LOG_ERROR("input featuremap can not Divide deformableGroups.");
        return KERNEL_STATUS_PARAM_INVALID;
    }

    if (offset_.channels != deformable_groups_ * ksize_list_[0] * ksize_list_[1] * 3) {
        KERNEL_LOG_ERROR("offsets_channels should be deformable_group*k_x*k_y*3.");
        return KERNEL_STATUS_PARAM_INVALID;
    }

    if (x_.batch != offset_.batch || x_.height != offset_.height || x_.width != offset_.width) {
        KERNEL_LOG_ERROR("input x's or offset's shape error, x's shape is [%d %d %d %d], offset's shape is[%d %d %d %d].",
                        x_.batch, x_.channels, x_.height, x_.width, offset_.batch, offset_.channels, offset_.height,
                        offset_.width);
        return KERNEL_STATUS_PARAM_INVALID;
    }
...
template <typename T>
uint32_t DeformableOffsetsCpuKernel::DoCompute(Eigen::half *inputDataX, T *inputDataOffsets, Eigen::half *inputDataY)
{
    const int offsetIndexMax = offset_.batch * offset_.channels * offset_.height * offset_.width;
    const int offsetStrideIndex = 3 * param_.ksizeX * param_.ksizeY * offset_.height * offset_.width;
    for (int b = 0; b < y_.batch; b++) {
        ...
        for (int g = 0; g < deformable_groups_; g++) {
            int offsetIndex = b * offset_.channels * offset_.height * offset_.width + g * offsetStrideIndex;
            if (offsetIndex < 0 || offsetIndex >= offsetIndexMax) {
                KERNEL_LOG_ERROR("calculation offsets index over flow.");
                return KERNEL_STATUS_INNER_ERROR;
            }
            T *inputOffset = inputDataOffsets + offsetIndex;
            int64_t groupLen = x_.channels / deformable_groups_;
            for (int l = 0; l < groupLen; l++) {
                int currentAxis = g * groupLen + l;
                uint32_t res = ComputePosition<T>(inputX, inputOffset, inputY, currentAxis);
                if (res != KERNEL_STATUS_OK) {
                    return res;
                }
            }
...
template <typename T>
uint32_t DeformableOffsetsCpuKernel::ComputePosition(const Eigen::half *inputX, const T *inputOffset,
                                                     Eigen::half *inputY, int currentAxis)
{
    ...
    const int kxPoints = ((x_.width + param_.padLeft + param_.padRight - kSizeDilX) / param_.strideW) + 1;
    const int kyPoints = ((x_.height + param_.padUp + param_.padDown - kSizeDilY) / param_.strideH) + 1;

    KERNEL_LOG_DEBUG("kxPoints = %d, kyPoints = %d, kSizeDilX = %d, kSizeDilY = %d", kxPoints, kyPoints, kSizeDilX,
                      kSizeDilY);
    for (int yDst = 0; yDst < kyPoints; yDst++) {
        for (int xDst = 0; xDst < kxPoints; xDst++) {
            ...
            uint32_t res = ComputeResult<T>(inputX, inputOffset, inputY, xSrc, ySrc, currentAxis);
            if (res != KERNEL_STATUS_OK) {
                return res;
            }
        }
...
template <typename T>
uint32_t DeformableOffsetsCpuKernel::ComputeResult(const Eigen::half *inputX, const T *inputOffset,
                                                   Eigen::half *inputY, int xSrc, int ySrc, int currentAxis)
{
    const int offsetIndexMax = offset_.batch * offset_.channels *offset_.height * offset_.width;
    const int yIndexMax = y_.batch * y_.channels * y_.height * y_.width;
    const int offsetStrideX = 0;
    const int offsetStrideY = param_.ksizeX * param_.ksizeY * offset_.height * offset_.width;
    const int offsetStrideDelta = 2 * param_.ksizeX * param_.ksizeY * offset_.height * offset_.width;
    ...
    for (int kx = 0; kx < param_.ksizeX; kx++) {
        ...
        for (int ky = 0; ky < param_.ksizeY; ky++) {
            ...
            int mapStride = mapIndex * offset_.height * offset_.width + ySrc * offset_.height + xSrc;

            int offsetXIndex = mapStride + offsetStrideX;
            if (offsetXIndex < 0 || offsetXIndex >= offsetIndexMax) {
                KERNEL_LOG_ERROR("calculation offsets index x over flow.");
                return KERNEL_STATUS_INNER_ERROR;
            }
            int offsetYIndex = mapStride + offsetStrideY;
            if (offsetYIndex < 0 || offsetYIndex >= offsetIndexMax) {
                KERNEL_LOG_ERROR("calculation offsets index y over flow.");
                return KERNEL_STATUS_INNER_ERROR;
            }
            int offsetDeltaIndex = mapStride + offsetStrideDelta;
            if (offsetDeltaIndex < 0 || offsetDeltaIndex >= offsetIndexMax) {
                KERNEL_LOG_ERROR("calculation offsets index delta over flow.");
                return KERNEL_STATUS_INNER_ERROR;
            }

            T offsetsValX = inputOffset[offsetXIndex];
            T offsetsValY = inputOffset[offsetYIndex];
            T offsetsValDelta = inputOffset[offsetDeltaIndex];
            float offsetW = static_cast<float>(xSrc) + static_cast<float>(offsetsValX);
            float offsetH = static_cast<float>(ySrc) + static_cast<float>(offsetsValY);
            Eigen::half xVal;
            KERNEL_LOG_DEBUG("[currentAxis outH outW]= [%d %d %d]", currentAxis, outH, outW);
            uint32_t res = BilinearInterpolate(xVal, inputX, currentAxis, offsetH, offsetW);
            if (res != KERNEL_STATUS_OK) {
                return res;
            }
            int yIndex = outH * y_.width + outW;
            if (yIndex < 0 || yIndex >= yIndexMax) {
                KERNEL_LOG_ERROR("calculation y index over flow.");
                return KERNEL_STATUS_INNER_ERROR;
            }
            currentY[yIndex] = static_cast<Eigen::half>(static_cast<float>(xVal) * static_cast<float>(offsetsValDelta));
        }
    }
...
uint32_t DeformableOffsetsCpuKernel::BilinearInterpolate(Eigen::half &out, const Eigen::half *in, int c_axis,
                                                         float h, float w)
{
    ...
    // Calculate bilinear interpolation coordinate points according to the input format type
    if ("NCHW" == data_format_) {
