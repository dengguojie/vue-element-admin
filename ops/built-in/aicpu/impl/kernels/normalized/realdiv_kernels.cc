 ...
 * Description: implement of realdiv, z = x / y
 */
...
uint32_t RealDivKernel::Compute(CpuKernelContext &ctx)
{
    ...
    if ((ctx.GetInputsSize() != REALDIV_INPUT_NUM) || (ctx.GetOutputsSize() != REALDIV_OUTPUT_DESC_NUM)) {
        KERNEL_LOG_ERROR("Unexpected RealDiv node, node input size: %zu, node output size: %zu, node name: %s",
...
uint32_t RealDivKernel::ComputeDiffType(Tensor *x, Tensor *y, Tensor *z, DataType dataType)
{
    switch (dataType) {
        case DT_FLOAT16:
...
uint32_t RealDivKernel::ComputeRealdiv(Tensor *x, Tensor *y, Tensor *z)
{
    ...
    uint32_t ret = ComputeDiffShape<T>(dim, (T*)xAddr, (T*)yAddr, (T*)zAddr, xDimSize, yDimSize, zDimSize);
    if (ret != KERNEL_STATUS_OK) {
        return ret;
    }
...
template <typename T>
uint32_t RealDivKernel::ComputeDiffShape(int64_t dim, T *xAddr, T *yAddr, T *zAddr,
                                                    std::vector<int64_t> &xDimSize,
                                                    std::vector<int64_t> &yDimSize,
                                                    std::vector<int64_t> &zDimSize)
{
                            ...
                            std::vector<int64_t> &zDimSize)
{
    ...
    mapZ.reshape(zPad) = mapX.reshape(xPad).broadcast(xBcast) / mapY.reshape(yPad).broadcast(yBcast);
}
