...
template <typename T>
uint32_t MeshgridTask(std::vector<void *>& ioAddrs,
        std::string &indexing, size_t &ndim, std::vector<int> &bcast)
{
    ...
    for (int i = 0; i < static_cast<int>(ndim); ++i) { // 0~ndim
        ...
        Eigen::TensorMap<Eigen::Tensor<T, 2, Eigen::RowMajor>> input_map((T *)ioAddrs[i], bcast[i], 1);
        const auto &input = Eigen::Tensor<T, 2, Eigen::RowMajor>(input_map);
        Eigen::TensorMap<Eigen::Tensor<T, 2, Eigen::RowMajor>> output((T *)ioAddrs[ndim + i], row_, col_);
        ...
        for (size_t j = 0; j < row_ * col_ / bcast[i] / col_; ++j) {
            Eigen::array<size_t, 2> offsets_in = {0, col_ * j};
            Eigen::array<size_t, 2> offsets_out = {bcast[i] * j, 0};
            Eigen::array<size_t, 2> extents = {static_cast<size_t>(bcast[i]), col_};
...
uint32_t MeshgridCpuKernel::Compute(CpuKernelContext &ctx)
{
    ...
    std::map<int, std::function<uint32_t(std::vector<void *> &, std::string &, size_t &, std::vector<int> &)>> calls;
    ...
    if (calls.find(input_type_) == calls.end()) {
        KERNEL_LOG_ERROR("MeshgridCpuKernel op don't support input tensor types: %s", typeid(input_type_).name());
...
uint32_t MeshgridCpuKernel::GetInputAndCheck(CpuKernelContext &ctx)
{
    ...
    KERNEL_CHECK_NULLPTR(indexing, KERNEL_STATUS_PARAM_INVALID, "get attr:indexing failed.");
