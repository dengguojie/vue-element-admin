 ...
 * Description: implement of search_cache_idx_kernels.h
 */
...
{
    template <typename T>
    uint32_t SearchCacheIdxTask(std::vector<Tensor *> &inputs_, std::vector<Tensor *> &outputs_, int64_t &batch_size_, int64_t &hashmap_length_)
    {
        auto start = std::chrono::high_resolution_clock::now();

        if (inputs_.size() == 0 || outputs_.size() == 0)
        {
            KERNEL_LOG_ERROR("SearchCacheIdxKernel::SearchCacheIdxTask: input or output is empty.");
        ...
        T *output_cache_idx = reinterpret_cast<T *>(outputs_[0]->GetData());    //16000*39, if miss, cacheIdx=-1
        T *output_miss_idx = reinterpret_cast<T *>(outputs_[1]->GetData());     // 16000*39, default -1
        T *output_miss_emb_idx = reinterpret_cast<T *>(outputs_[2]->GetData()); // 16000*39, default -1
        ...
        KERNEL_LOG_INFO("SearchCacheIdx execute %fms.", std::chrono::duration<double, std::milli>(end - start).count());
    ...
    uint32_t SearchCacheIdxKernel::DoCompute()
    {
        ...
        {
            KERNEL_LOG_ERROR("SearchCacheIdxKernel op don't support input tensor types: %s", typeid(param_type_).name());
            return KERNEL_STATUS_PARAM_INVALID;
        }
    ...
    uint32_t SearchCacheIdxKernel::GetInputAndCheck(CpuKernelContext &ctx)
    {
        ...
        for (int i = 0; i < num_input; ++i)
        {
            ...
            if (tensor == nullptr)
            {
                KERNEL_LOG_ERROR("SearchCacheIdxKernel::GetInputAndCheck: get input tensor[%d] failed", i);
                return KERNEL_STATUS_PARAM_INVALID;
            }
        ...
        for (int i = 0; i < num_output; ++i)
        {
            ...
            if (tensor == nullptr)
            {
                KERNEL_LOG_ERROR("SearchCacheIdxKernel::GetInputAndCheck: get output tensor[%d] failed", i);
                return KERNEL_STATUS_PARAM_INVALID;
            }
        ...
        {
            KERNEL_LOG_ERROR("SearchCacheIdxKernel::GetInputAndCheck: only support hashmap rank 2, but got %d", hashmap_shape->GetDims());
            return KERNEL_STATUS_PARAM_INVALID;
        }
        ...
        {
            KERNEL_LOG_ERROR("SearchCacheIdxKernel::GetInputAndCheck: only support hashmap shape (n, 4), but got (%d, %d)", hashmap_length_, hashmap_shape->GetDimSize(1));
            return KERNEL_STATUS_PARAM_INVALID;
        }
