...
uint32_t InplaceTopKDistanceCpuKernel::GetInputAndCheck(const CpuKernelContext& ctx, Inputs& inputs) const {
  ...
  KERNEL_LOG_DEBUG("InplaceTopKDistance getInputAndCheck topk_pq_distance NumElements is[%d]",
                   inputs.topk_pq_distance->NumElements());
  KERNEL_LOG_DEBUG("InplaceTopKDistance getInputAndCheck topk_pq_distance shape is[%d]",
                   inputs.topk_pq_distance->GetTensorShape()->GetDims());
  KERNEL_LOG_DEBUG("InplaceTopKDistance getInputAndCheck pq_ivf shape is[%d]",
...
uint32_t InplaceTopKDistanceCpuKernel::ModifyInput(std::vector<Item<T>> items_vec, const Inputs& inputs) {
  ...
  KERNEL_LOG_DEBUG("InplaceTopKDistance attr order is [%s]", order.c_str());

  if (order == "asc") {
    KERNEL_LOG_DEBUG("InplaceTopKDistance modifyInput asc begin");
    uint64_t j = 0;
    ...
    KERNEL_LOG_DEBUG("InplaceTopKDistance modifyInput desc begin");
