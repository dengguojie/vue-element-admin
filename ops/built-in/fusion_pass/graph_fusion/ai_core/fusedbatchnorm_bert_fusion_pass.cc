...
Status FusedBatchNormBertFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_CHECK(batchNormNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(
                    FUSED_OP_TYPE.c_str(), "batchNormNode is null, fusion failed."), return PARAM_INVALID);
  ...
  if (batchNormDesc->GetOutputsSize() != OUTPUTS_SIZE_ASTRICT) {
  ...
  if (batchNormReduceDesc->GetOutputsSize() < OUTPUTS_SIZE_ASTRICT) {
  ...
  if (batchNormUpdateV2Desc->GetOutputsSize() < OUTPUTS_SIZE_ASTRICT) {
    OP_LOGW(FUSED_OP_TYPE.c_str(), "Index is beyond the size[%d] of output desc",
  ...
  FUSION_PASS_CHECK(PatternFusionUtil::UpdateInputAndOutputName(batchNormReduceDesc) != SUCCESS,
                    ...
                    FUSED_OP_TYPE.c_str(), "Update fusionNode:%s input and output name failed.",
  ...
  if (batchNormNode->GetOutDataAnchor(BATCH_NORM_SCALE_IDX)->GetPeerInDataAnchors().size() > 0) {
    ...
    for (InDataAnchorPtr inAnchorPtr : batchNormNode->GetOutDataAnchor(BATCH_NORM_SCALE_IDX)->GetPeerInDataAnchors()) {
      inAnchorPtr->UnlinkAll();
      FUSION_PASS_CHECK(
  ...
  if (batchNormNode->GetOutDataAnchor(OUT_ANCHOR_INDEX)->GetPeerInDataAnchors().size() > 0) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "The size of batchNormNode is [%d].",
            batchNormNode->GetOutDataAnchor(OUT_ANCHOR_INDEX)->GetPeerInDataAnchors().size());
    for (InDataAnchorPtr inAnchorPtr : batchNormNode->GetOutDataAnchor(OUT_ANCHOR_INDEX)->GetPeerInDataAnchors()) {
      inAnchorPtr->UnlinkAll();
      FUSION_PASS_CHECK(
  ...
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(batchNormNode),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "remove batchNormNode node[%s] failed", batchNormName.c_str()), return FAILED);
