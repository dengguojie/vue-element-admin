...
vector<FusionPattern *> FusedBatchNormInfGradFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("FusedBatchNormInfGradFusionPass");
  ...
  pattern->AddOpDesc(PATTERN_BATCHNORMGRAD, {BATCHNORMGRAD})
      ...
      .SetInputs(PATTERN_BATCHNORMGRAD,
                 {PATTERN_INPUTS1, PATTERN_INPUTS2, PATTERN_INPUTS3,
                  PATTERN_INPUTS4, PATTERN_INPUTS5})
...
Status FusedBatchNormInfGradFusionPass::Fusion(
    ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &fusionNodes)
{
  ...
  ge::NodePtr batchNormGradNode =
      GetNodeFromMapping(PATTERN_BATCHNORMGRAD, mapping);

  FUSION_PASS_CHECK(batchNormGradNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "batchNormGrad is null, fusion failed."),
           return PARAM_INVALID);
  bool isTraing = true;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetBool(batchNormGradNode->GetOpDesc(), IS_TRAING,
                                   isTraing),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Get is_traing attr failed."), return FAILED);

  FUSION_PASS_CHECK(isTraing, OP_LOGI(FUSED_OP_TYPE.c_str(), "is_traing is true, no need fusion."),
           return NOT_CHANGED);

  FUSION_PASS_CHECK(
            batchNormGradNode->GetOutDataAnchor(3)
                    ->GetPeerInDataAnchors()
                    .size() != 0 ||
            batchNormGradNode->GetOutDataAnchor(4)
                    ->GetPeerInDataAnchors()
                    .size() != 0,
  ...
  // copy Opdesc
    ...
    newOpdesc =
        std::make_shared<ge::OpDesc>(batchNormGradNode->GetName() + "_Infer", BNINFERGRAD);
  ...
  ge::GeTensorDesc input_tensor1 =
      batchNormGradNode->GetOpDesc()->GetInputDesc(0);
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor1) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input grads failed.",
                   newOpName.c_str()),
           return FAILED);

  ge::GeTensorDesc input_tensor3 =
      batchNormGradNode->GetOpDesc()->GetInputDesc(2);
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor3) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input scale failed.",
                   newOpName.c_str()),
           return FAILED);

  ge::GeTensorDesc input_tensor5 =
      batchNormGradNode->GetOpDesc()->GetInputDesc(4);
  ...
  // copy Opdesc
      ...
      newOpdesc2 =
          std::make_shared<ge::OpDesc>(batchNormGradNode->GetName() + "_Update", BNUPDATEGRAD);
  ...
  ge::GeTensorDesc update_input_tensor1 =
      batchNormGradNode->GetOpDesc()->GetInputDesc(0);
  FUSION_PASS_CHECK(newOpdesc2->AddInputDesc(update_input_tensor1) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input grads failed.", newOpName.c_str()), return FAILED);

  ge::GeTensorDesc update_input_tensor2 =
      batchNormGradNode->GetOpDesc()->GetInputDesc(1);
  FUSION_PASS_CHECK(newOpdesc2->AddInputDesc(update_input_tensor2) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input x failed.", newOpName.c_str()), return FAILED);

  ge::GeTensorDesc update_input_tensor3 =
      batchNormGradNode->GetOpDesc()->GetInputDesc(3);
  FUSION_PASS_CHECK(newOpdesc2->AddInputDesc(update_input_tensor3) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input batch_mean failed.", newOpName.c_str()), return FAILED);

  ge::GeTensorDesc update_input_tensor4 =
      batchNormGradNode->GetOpDesc()->GetInputDesc(4);
  FUSION_PASS_CHECK(newOpdesc2->AddInputDesc(update_input_tensor4) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input batch_variance failed.", newOpName.c_str()), return FAILED);

  // add output for node2
  ge::GeTensorDesc update_tensor1 =
      batchNormGradNode->GetOpDesc()->GetOutputDesc(1);
  FUSION_PASS_CHECK(newOpdesc2->AddOutputDesc(update_tensor1) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the output desc for the output diff_scale failed.", newOpName.c_str()), return FAILED);

  ge::GeTensorDesc update_tensor2 =
      batchNormGradNode->GetOpDesc()->GetOutputDesc(2);
  FUSION_PASS_CHECK(newOpdesc2->AddOutputDesc(update_tensor2) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the output desc for the output diff_offset failed.", newOpName.c_str()), return FAILED);
  ...
  // copy attr
    float epsilon;
    FUSION_PASS_CHECK(!ge::AttrUtils::GetFloat(batchNormGradNode->GetOpDesc(), EPSILON,
                                      epsilon),
  ...
  // copy output edge
    for (auto inDataAnchor :
      ...
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(batchNormGradNode->GetOutDataAnchor(0),
  ...
  if (batchNormGradNode->GetOutControlAnchor()) {
      for (auto inControlAnchor :
           batchNormGradNode->GetOutControlAnchor()->GetPeerInControlAnchors()) {
        FUSION_PASS_CHECK(
            ge::GraphUtils::RemoveEdge(batchNormGradNode->GetOutControlAnchor(),
  ...
  // copy output  for node2
    for (auto inDataAnchor :
         batchNormGradNode->GetOutDataAnchor(1)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(batchNormGradNode->GetOutDataAnchor(1),
                                          inDataAnchor) != SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out data edge failed."), return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(newNode2->GetOutDataAnchor(0),
                                       inDataAnchor) != SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out data edge failed."), return FAILED);
    }

    for (auto inDataAnchor :
         batchNormGradNode->GetOutDataAnchor(2)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(batchNormGradNode->GetOutDataAnchor(2),
                                          inDataAnchor) != SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out data edge failed."), return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(newNode2->GetOutDataAnchor(1),
                                       inDataAnchor) != SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out data edge failed."), return FAILED);
    }
      ...
      for (auto inControlAnchor :
        ...
        FUSION_PASS_CHECK(
            ge::GraphUtils::RemoveEdge(batchNormGradNode->GetOutControlAnchor(),
                                       inControlAnchor) != SUCCESS,
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out control edge failed."), return FAILED);
        FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(newNode2->GetOutControlAnchor(),
                                         inControlAnchor) != SUCCESS,
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out control edge failed."), return FAILED);
      }
    }
  ...
  // copy BatchNormGrad inputs to BNInferGrad nodes
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
             ...
             return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
                 batchNormGradNode->GetInDataAnchor(2)->GetPeerOutAnchor(),
                 newNode->GetInDataAnchor(1)) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
                     batchNormGradNode->GetInDataAnchor(2)
                         ->GetPeerOutAnchor()
                         ->GetOwnerNode()
                         ->GetName()
                         .c_str(),
             ...
             return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
                 batchNormGradNode->GetInDataAnchor(4)->GetPeerOutAnchor(),
                 newNode->GetInDataAnchor(2)) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
                     batchNormGradNode->GetInDataAnchor(4)
                         ->GetPeerOutAnchor()
                         ->GetOwnerNode()
                         ->GetName()
                         .c_str(),
  ...
  // copy BatchNormGrad inputs to BNUpdateGrad nodes
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
             ...
             return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
                 batchNormGradNode->GetInDataAnchor(1)->GetPeerOutAnchor(),
                 newNode2->GetInDataAnchor(1)) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
                     batchNormGradNode->GetInDataAnchor(1)
                         ->GetPeerOutAnchor()
                         ->GetOwnerNode()
                         ->GetName()
                         .c_str(),
             ...
             return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
                 batchNormGradNode->GetInDataAnchor(3)->GetPeerOutAnchor(),
                 newNode2->GetInDataAnchor(2)) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
                     batchNormGradNode->GetInDataAnchor(3)
                         ->GetPeerOutAnchor()
                         ->GetOwnerNode()
                         ->GetName()
                         .c_str(),
    ...
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(batchNormGradNode->GetOutControlAnchor(),
                                     newNode2->GetInControlAnchor()) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add control edge between node %s. and node %s failed.",
                     batchNormGradNode->GetName().c_str(),
                     newNode->GetName().c_str()),
             return FAILED);
...
REGISTER_PASS("BatchNormGradInfGradFusion", BUILT_IN_GRAPH_PASS,
              FusedBatchNormInfGradFusionPass);
}  // namespace fe