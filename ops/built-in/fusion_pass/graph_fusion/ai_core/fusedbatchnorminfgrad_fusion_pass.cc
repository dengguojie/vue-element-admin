...
namespace fe {
const int32_t BN_TRAINING_UPDATEV2_SCALE_IDX = 3;
const int32_t BATCH_NORM_SCALE_IDX = 2;
...
Status FusedBatchNormInfGradFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_CHECK(batchNormGradNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "batchNormGrad is null, fusion failed."),
  ...
  // copy Opdesc
  std::shared_ptr<ge::OpDesc> newOpdesc = std::make_shared<ge::OpDesc>(batchNormGradNode->GetName() + "_Infer",
                                                                       BNINFERGRAD);
  FUSION_PASS_CHECK(newOpdesc == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "newOpdesc is null, fusion failed."), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input grads failed.",
                                     newOpName.c_str()), return FAILED);

  ge::GeTensorDesc input_tensor3 = batchNormGradNode->GetOpDesc()->GetInputDesc(BATCH_NORM_SCALE_IDX);
  FUSION_PASS_CHECK(
  ...
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor5) != SUCCESS,
                    ...
                    "Op[%s]: add the input desc for the input batch_variance failed.",
                    newOpName.c_str()),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(newOpdesc->AddOutputDesc(tensor1) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                   "Op[%s]: add the output desc for the output x_backprop failed.",
                                                   newOpName.c_str()), return FAILED);
  ...
  ge::GeTensorDesc update_input_tensor3 = batchNormGradNode->GetOpDesc()->GetInputDesc(BN_TRAINING_UPDATEV2_SCALE_IDX);
  FUSION_PASS_CHECK(
      newOpdesc2->AddInputDesc(update_input_tensor3) != SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
      "Op[%s]: add the input desc for the input batch_mean failed.", newOpName.c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(newOpdesc2->AddOutputDesc(update_tensor1) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "Op[%s]: add the output desc for the output diff_scale failed.",
                    newOpName.c_str()),
                    return FAILED);

  ge::GeTensorDesc update_tensor2 = batchNormGradNode->GetOpDesc()->GetOutputDesc(BATCH_NORM_SCALE_IDX);
  FUSION_PASS_CHECK(newOpdesc2->AddOutputDesc(update_tensor2) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "Op[%s]: add the output desc for the output diff_offset failed.",
                    newOpName.c_str()),
                    return FAILED);
  ...
  for (auto inDataAnchor : batchNormGradNode->GetOutDataAnchor(BATCH_NORM_SCALE_IDX)->GetPeerInDataAnchors()) {
  ...
  FUSION_PASS_CHECK(
      ge::GraphUtils::AddEdge(batchNormGradNode->GetInDataAnchor(2)->GetPeerOutAnchor(),
