...
Status Conv2DbpInputBiasAddFusionPass::convert_dx_to_transpose(ge::ComputeGraph &graph, Mapping &mapping,
  ...
  FUSION_PASS_CHECK(
    conv_node == nullptr,
    OP_LOGE(FUSED_OP_TYPE.c_str(), "new conv_node not success."),
    return PARAM_INVALID);

  ge::NodePtr bias_node = GetNodeFromMapping(PATTERN_BIASADD, mapping);
  FUSION_PASS_CHECK(
    bias_node == nullptr,
    OP_LOGE(FUSED_OP_TYPE.c_str(), "new bias_node not success."),
    return PARAM_INVALID);

  ge::NodePtr bias_const_node = bias_node->GetInAllNodes().at(1);
  FUSION_PASS_CHECK(
    bias_const_node == nullptr,
    OP_LOGE(FUSED_OP_TYPE.c_str(), "new bias_const_node not success."),
    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(
    conv2d_transpose_d == nullptr,
    OP_LOGE(FUSED_OP_TYPE.c_str(), "new Conv2DTransposeD not success."),
    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(
    graph.RemoveNode(conv_node) == ge::GRAPH_FAILED,
    OP_LOGE("Removing conv_node is failed."),
    return FAILED);

  FUSION_PASS_CHECK(
    graph.RemoveNode(bias_node) == ge::GRAPH_FAILED,
    OP_LOGE("Removing bias_node is failed."),
    return FAILED);
...
void Conv2DbpInputBiasAddFusionPass::set_in_out_op_and_attr(ge::OpDescPtr &conv_op,
  ...
  */
  ...
  FUSION_PASS_CHECK(
    ...
    OP_LOGE("Adding output y for Conv2DTransposeD is failed."),
    return);
...
Status Conv2DbpInputBiasAddFusionPass::connect_edges(ge::NodePtr &conv_node,
  ...
  */
  FUSION_PASS_CHECK(
    ...
    OP_LOGE("Adding edge from fused node: %s's index[0] to fusion node: %s's index[1] is failed.",
  ...
  FUSION_PASS_CHECK(
    ...
    OP_LOGE("Adding edge from fused node: %s's index[1] to fusion node: %s's index[0] is failed.",
            conv_node->GetName().c_str(), conv2d_transpose_d->GetName().c_str()),
    return FAILED);

  FUSION_PASS_CHECK(
    ge::GraphUtils::AddEdge(bias_const_node->GetOutDataAnchor(0), conv2d_transpose_d->GetInDataAnchor(2)) != SUCCESS,
    OP_LOGE("Adding edge from fused node: %s's index[0] to fusion node: %s's index[2] is failed.",
  ...
  for (auto in_anchor : in_anchors) {
    ...
    FUSION_PASS_CHECK(
      ...
      OP_LOGE("Adding edge from fusion node: %s's index[0] to fused node: %s's indexes is failed.",
