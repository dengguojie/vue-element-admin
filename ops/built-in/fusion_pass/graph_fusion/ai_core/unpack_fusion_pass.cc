...
*/
Status UnpackFusionPass::AddUnpackOps(ge::OpDescPtr fused_desc,
  ...
  FUSION_PASS_CHECK(unpack_node == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "unpack_node:%s is null, fusion failed.",
                   unpack_node->GetName().c_str()),
           return PARAM_INVALID);
  FUSION_PASS_CHECK(SUCCESS !=
           ge::GraphUtils::AddEdge(splitvd_base_node->GetOutDataAnchor(i),
                                   unpack_node->GetInDataAnchor(0)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d]"
                    " to fusion node:%s's index[%d] failed.",
                   splitvd_base_node->GetName().c_str(), i,
                   unpack_node->GetName().c_str(), i),
           return FAILED);

  for (int64_t m = 0; m < mini_out; m++) {
    for (InDataAnchorPtr inAnchorPtr :fused_node->GetOutDataAnchor(
         MINI_OUT * i + m)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(SUCCESS !=ge::GraphUtils::RemoveEdge(
               fused_node->GetOutDataAnchor(MINI_OUT*i + m), inAnchorPtr),
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out data edge failed."),
               return FAILED);
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(
               unpack_node->GetOutDataAnchor(m), inAnchorPtr),
...
Status UnpackFusionPass::Fusion(ge::ComputeGraph &graph,
                                Mapping &mapping,
                                vector<ge::NodePtr> &newNodes) {
  ...
  if (num > MINI_OUT) {
    ...
    splitvd_base_desc->SetName(splitvd_base_desc->GetName() +
                               "/SplitVD" + "Base_node");
    ...
    ge::AttrUtils::SetListInt(splitvd_base_node->GetOpDesc(),
                              "size_splits", size_splits_new);
    ge::AttrUtils::SetInt(splitvd_base_node->GetOpDesc(),
                          "split_dim", axis);
    ge::AttrUtils::SetInt(splitvd_base_node->GetOpDesc(),
                          "num_split", nodes_num);
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(
             ...
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d]"
                     ...
                     splitvd_base_node->GetName().c_str(), 0),
             return FAILED);

    for (int64_t i = 0; i < nodes_num; i++) {
      if (i < nodes_num - 1) {
       AddUnpackOps(fused_desc, graph, newNodes,output_desc, fused_node,
                    splitvd_base_node, num, axis, i, i, MINI_OUT);
      } else if (i == nodes_num - 1 && last_node_num_unpack != 1) {
        AddUnpackOps(fused_desc, graph, newNodes, output_desc, fused_node,
                     splitvd_base_node, num, axis, i,
                     nodes_num - 1, last_node_num_unpack);
      } else {
        for (InDataAnchorPtr inAnchorPtr :
             fused_node->GetOutDataAnchor(MINI_OUT*i)->GetPeerInDataAnchors()) {
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(
                     fused_node->GetOutDataAnchor(MINI_OUT*i),inAnchorPtr),
                   OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out data edge failed."),
                   return FAILED);
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(
                     splitvd_base_node->GetOutDataAnchor(i), inAnchorPtr),
                   OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out data edge failed."),
                   return FAILED);
        }
      }
