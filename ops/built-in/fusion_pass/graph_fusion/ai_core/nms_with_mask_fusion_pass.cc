...
Status NMSWithMaskFusionPass::Fusion(ComputeGraph& graph, Mapping& mapping, vector<NodePtr>& fusion_nodes) {
  ...
  FUSION_PASS_CHECK(nms_node_ptr == nullptr, OP_LOGE(kFusedOpType.c_str(), "NMSWithMask Node is null, fusion failed."),
                    return PARAM_INVALID);
  OpDescPtr nms_desc_ptr = nms_node_ptr->GetOpDesc();
  FUSION_PASS_CHECK(nms_node_ptr == nullptr,
                    OP_LOGE(kFusedOpType.c_str(), "NMSWithMask desc is null, FE fusion failed."), return PARAM_INVALID);

  // clone pad node desc from nms
  OpDescPtr pad_desc_ptr = AttrUtils::CloneOpDesc(nms_desc_ptr);
  FUSION_PASS_CHECK(pad_desc_ptr == nullptr, OP_LOGE(kFusedOpType.c_str(), "Create PadD OpDesc failed, fusion failed."),
                    return PARAM_INVALID);
  ...
  pad_desc_ptr->AddOutputDesc("NMSWithMask", pad_output_tensor_desc);
  pad_desc_ptr->UpdateOutputDesc(0, pad_output_tensor_desc);
  nms_desc_ptr->UpdateInputDesc(0, pad_output_tensor_desc);

  // delete attr from nms
  FUSION_PASS_CHECK(pad_desc_ptr->DelAttr("iou_threshold") != SUCCESS,
                    OP_LOGE(kFusedOpType.c_str(), "Delete the attr of iou_threshold from nms."), return PARAM_INVALID);

  // set paddings attr for pad node
  FUSION_PASS_CHECK(
      !AttrUtils::SetListListInt(pad_desc_ptr, "paddings", std::vector<std::vector<int64_t>>{{0, 0}, {0, 3}}),
      OP_LOGE(kFusedOpType.c_str(), "Set paddings attr for pad node."), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(pad_node_ptr == nullptr,
                    OP_LOGE(kFusedOpType.c_str(), "The fusionNode: pad_node_ptr is null, fusion failed."),
                    return FAILED);
