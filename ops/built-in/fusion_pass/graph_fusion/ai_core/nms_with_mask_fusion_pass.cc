...
vector<FusionPattern*> NMSWithMaskFusionPass::DefinePatterns() {
  ...
  pattern->AddOpDesc(PATTERN_NMS_WITH_MASK, {NMS_WITH_MASK}).SetOutput(PATTERN_NMS_WITH_MASK);
...
Status NMSWithMaskFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ge::NodePtr nmsNodePtr = GetNodeFromMapping(PATTERN_NMS_WITH_MASK, mapping);
  FUSION_PASS_CHECK(nmsNodePtr == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "NMSWithMask Node is null, fusion failed."),
  ...
  FUSION_PASS_CHECK(padDescPtr == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Create PadD OpDesc failed, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (!(nmsInputDims.size() == 2 && nmsInputDims[1] == 5)) {
    OP_LOGE(FUSED_OP_TYPE.c_str(),
            "The input dim of %s is not 2 dims or the second dimension of \
                 input is not 5",
  ...
  FUSION_PASS_CHECK(SUCCESS != padDescPtr->DelAttr("iou_threshold"),
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "Delete the attr of iou_threshold from nms."), return PARAM_INVALID);

  // set paddings attr for pad node
  FUSION_PASS_CHECK(
      true != ge::AttrUtils::SetListListInt(padDescPtr, "paddings", std::vector<std::vector<int64_t>>{{0, 0}, {0, 3}}),
  ...
  FUSION_PASS_CHECK(padNodePtr == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode: padNodePtr is null, fusion failed."), return FAILED);

  // add the original edge of nms to pad
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(nmsNodePtr->GetInDataAnchor(0)->GetPeerOutAnchor(),
  ...
  FUSION_PASS_CHECK(
      SUCCESS != ge::GraphUtils::RemoveEdge(nmsNodePtr->GetInDataAnchor(0)->GetPeerOutAnchor(),
                                            nmsNodePtr->GetInDataAnchor(0)),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove input edge from fused node:%s.", nmsNodePtr->GetName().c_str()),
      return FAILED);

  // add the output of pad edge to nms
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(padNodePtr->GetOutDataAnchor(0), nmsNodePtr->GetInDataAnchor(0)),
