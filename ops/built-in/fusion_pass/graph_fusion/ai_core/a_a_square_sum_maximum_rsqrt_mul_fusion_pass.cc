...
vector<FusionPattern *> AASquareSumMaximumRsqrtMulFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
          new (std::nothrow) FusionPattern("AASquareSumMaximumRsqrtMulFusion");
...
Status AASquareSumMaximumRsqrtMulFusionPass::IsMatch(ge::NodePtr &squareNode,
                                                    ge::NodePtr &sumNode,
                                                    ge::NodePtr &maximumNode,
  ...
  FUSION_PASS_CHECK(mulNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mulNode is null, fusion failed."),
          return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerAllInDataAnchors(squareNode->GetOutDataAnchor(0), 1) != SUCCESS,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "%s contains more than one peer input",
                   squareNode->GetName().c_str()),
           return NOT_CHANGED);
  FUSION_PASS_CHECK(CheckPeerAllInDataAnchors(sumNode->GetOutDataAnchor(0), 1) != SUCCESS,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "%s contains more than one peer input",
                   sumNode->GetName().c_str()),
           return NOT_CHANGED);
  FUSION_PASS_CHECK(CheckPeerAllInDataAnchors(maximumNode->GetOutDataAnchor(0), 1) != SUCCESS,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "%s contains more than one peer input",
                   maximumNode->GetName().c_str()),
           return NOT_CHANGED);
  FUSION_PASS_CHECK(CheckPeerAllInDataAnchors(rsqrtNode->GetOutDataAnchor(0), 1) != SUCCESS,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "%s contains more than one peer input",
                   rsqrtNode->GetName().c_str()),
           return NOT_CHANGED);
  ...
  for (auto sum_in_anchor : sum_in_anchors) {
      ...
      if (sum_out_anchor == nullptr) continue;

      auto sum_in_node = sum_out_anchor->GetOwnerNode();
      if (sum_in_node->GetType() == "Const" || sum_in_node->GetType() == "Constant") {
  ...
  for (auto in_anchor : maximum_in_anchors) {
      ...
      if (maximum_in_node->GetType() == "Const" || maximum_in_node->GetType() == "Constant") {
  ...
  FUSION_PASS_CHECK(squareInputNode == nullptr, OP_LOGI(FUSED_OP_TYPE.c_str(), "squareInputNode is null, fusion failed."),
          return PARAM_INVALID);
...
Status AASquareSumMaximumRsqrtMulFusionPass::Fusion(ge::ComputeGraph &graph,
                                                   Mapping &mapping,
  ...
  for (auto in_anchor : sum_in_anchors) {
      ...
      if (in_node->GetType() == "Const" || in_node->GetType() == "Constant") {
  ...
  if (axis_w->GetTensorDesc().GetDataType() != ge::DT_INT32 &&
      axis_w->GetTensorDesc().GetDataType() != ge::DT_INT64) {
  ...
  if (axisType == ge::DT_INT32) {
      ...
      FUSION_PASS_CHECK(const_data_ptr == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "const_data_ptr is null, fusion failed."),
              return PARAM_INVALID);
  ...
  if (eps_w->GetTensorDesc().GetDataType() != ge::DT_FLOAT &&
      eps_w->GetTensorDesc().GetDataType() != ge::DT_FLOAT16) {
  ...
  if (rsqrtNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
      for (ge::InDataAnchorPtr inAnchorPtr : rsqrtNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(rsqrtNode->GetOutDataAnchor(0), inAnchorPtr),
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove edge from fused node:%s's index to fusion node:%s's 1st index failed.",
                  rsqrtNode->GetName().c_str(), mulNode->GetName().c_str()), return FAILED);
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(rsqrtNode) != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove rsqrt node failed."), return FAILED);
