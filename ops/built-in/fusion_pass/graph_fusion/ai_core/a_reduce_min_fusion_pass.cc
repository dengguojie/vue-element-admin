...
Status CheckMinFussionOrNot(vector<int64_t> tensor_info,
...
vector<FusionPattern *> AReduceMinFusionPass::DefinePatterns() {
  ...
  new (std::nothrow) FusionPattern("AReduceMinFusionPass");
...
Status AReduceMinFusionPass::Fusion(ge::ComputeGraph &graph,
  ...
  FUSION_PASS_CHECK(minNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "minNode is null, fusion failed."),
           return PARAM_INVALID);

  ge::GeTensorDesc tensor_input = \
  minNode->GetOpDesc()->GetInputDesc(0);
  ge::GeTensorDesc axis_input = \
  minNode->GetOpDesc()->GetInputDesc(1);
  ...
  for (auto inDataAnchor :
       minNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(minNode->GetOutDataAnchor(0),
                                        inDataAnchor) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove min and outnode edge failed."), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(minNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                                     inDataAnchor) != SUCCESS,
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(minNode) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove minNode failed."), return FAILED);
...
REGISTER_PASS("AReduceMinFusionPass", BUILT_IN_GRAPH_PASS,
              AReduceMinFusionPass);
}
