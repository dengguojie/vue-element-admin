...
Status ConstToAttrSpaceToBatchNdPass::Fusion(ComputeGraph& graph, Mapping& mapping, vector<NodePtr>& fusionNodes) {
  ...
  if ((input_format != FORMAT_NHWC) && (input_format != FORMAT_NCHW) && (input_format != FORMAT_NDHWC) &&
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "SpaceToBatchND has input which format is not NHWC or NCHW or NDHWC or NCDHW, graph not changed.");
    return NOT_CHANGED;
  }
  ...
  if (input_format == FORMAT_NHWC) {
    if ((input_dim_num != 4) || (block_vec.size() != 2) || (paddings_vec.size() != 4)) {
      if ((input_dim_num != 3) || (block_vec.size() != 1) || (paddings_vec.size() != 2)) {
        OP_LOGI(FUSED_OP_TYPE.c_str(),
                "SpaceToBatchND has input with format 'NHWC' which does not meet the rules, graph not changed.");
        return NOT_CHANGED;
      }
    }
  }
  ...
  if ((input_format == FORMAT_NCHW) && ((input_dim_num != 4) || (block_vec.size() != 3) || (paddings_vec.size() != 6) ||
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "SpaceToBatchND has input with format 'NCHW' which does not meet the rules, graph not changed.");
    return NOT_CHANGED;
  }
  ...
  if ((input_format == FORMAT_NDHWC) &&
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "SpaceToBatchND has input with format 'NDHWC' which does not meet the rules, graph not changed.");
    return NOT_CHANGED;
  }
  ...
  if ((input_format == FORMAT_NCDHW) &&
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "SpaceToBatchND has input with format 'NCDHW' which does not meet the rules, graph not changed.");
    return NOT_CHANGED;
  }
