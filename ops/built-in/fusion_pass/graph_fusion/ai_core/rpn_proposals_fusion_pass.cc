...
Status RpnProposalsFusionPass::Fusion(ge::ComputeGraph &graph,
                                       Mapping &mapping,
  ...
  FUSION_PASS_CHECK(scoreFilterPreSortDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node:%s's OpDesc is null, fusion failed.", fusedNode->GetName().c_str()),
           return PARAM_INVALID);
  ge::OpDescPtr rpnProposalPostProcessingDesc = AttrUtils::CloneOpDesc(fusedDesc);
  FUSION_PASS_CHECK(rpnProposalPostProcessingDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node:%s's OpDesc is null, fusion failed.", fusedNode->GetName().c_str()),
           return PARAM_INVALID);
  ...
  if (rpnProposalPostProcessingDesc->GetInputsSize() != 2) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Index is beyond the size[%d] of input desc", rpnProposalPostProcessingDesc->GetInputsSize());
    return FAILED;
  }

  if (scoreFilterPreSortDesc->GetOutputsSize() != 1) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Index is beyond the size[%d] of output desc", scoreFilterPreSortDesc->GetOutputsSize());
    return FAILED;
  }
  ...
  FUSION_PASS_CHECK(SUCCESS != scoreFilterPreSortDesc->DelAttr("min_size"),
  	...
  	return PARAM_INVALID);
  FUSION_PASS_CHECK(SUCCESS != scoreFilterPreSortDesc->DelAttr("nms_threshold"),
  	OP_LOGE(FUSED_OP_TYPE.c_str(), "Delete the attr of nms_threshold from scoreFilterPreSortDesc failed."),
  	return PARAM_INVALID);
  FUSION_PASS_CHECK(SUCCESS != scoreFilterPreSortDesc->DelAttr("post_nms_num"),
  ...
  FUSION_PASS_CHECK(scoreFilterPreSortNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", scoreFilterPreSortNode->GetName().c_str()), return PARAM_INVALID);
  FUSION_PASS_CHECK(rpnProposalPostProcessingNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", rpnProposalPostProcessingNode->GetName().c_str()), return PARAM_INVALID);
  for (unsigned int i = 0; i < fusedNode->GetAllInDataAnchors().size(); i++) {
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(i)->GetPeerOutAnchor(),
             ...
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.", fusedNode->GetName().c_str(), i, scoreFilterPreSortNode->GetName().c_str(), i),
             return FAILED);
    OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d].", fusedNode->GetName().c_str(), i, scoreFilterPreSortNode->GetName().c_str(), i);
  }
  for (unsigned int i = 0; i < fusedNode->GetInControlAnchor()->GetPeerOutControlAnchors().size(); i++) {
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInControlAnchor()->GetPeerOutControlAnchors().at(i),
             ...
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's control index[%d] to fusion node:%s's control index failed.", fusedNode->GetName().c_str(), i, scoreFilterPreSortNode->GetName().c_str()),
             return FAILED);
    OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's control index[%d] to fusion node:%s's control index.", fusedNode->GetName().c_str(), i, scoreFilterPreSortNode->GetName().c_str());
  }

  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(scoreFilterPreSortNode->GetOutDataAnchor(0), rpnProposalPostProcessingNode->GetInDataAnchor(0)),
  			...
  			return FAILED);
  OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's 1st index to fusion node:%s's 1st index.", scoreFilterPreSortNode->GetName().c_str(), rpnProposalPostProcessingNode->GetName().c_str());
  //OP_LOGE(FUSED_OP_TYPE.c_str(), "duiduan node name %s.", rpnProposalPostProcessingNode->GetInDataAnchor(1)->GetOwnerNode()->GetName()->c_str());

  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(scoreFilterPreSortNode->GetOutDataAnchor(1), rpnProposalPostProcessingNode->GetInDataAnchor(1)),
			  OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index to fusion node:%s's 2nd index failed. %d %d",
			  scoreFilterPreSortNode->GetName().c_str(), rpnProposalPostProcessingNode->GetName().c_str(), 
			  scoreFilterPreSortDesc->GetOutputsSize(), rpnProposalPostProcessingDesc->GetInputsSize()),
			  return FAILED);
  OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's 1st index to fusion node:%s's 2nd index.", scoreFilterPreSortNode->GetName().c_str(), rpnProposalPostProcessingNode->GetName().c_str());

  if (fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
	  OP_LOGI(FUSED_OP_TYPE.c_str(), "The size of layerNormXNode is [%d].", fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size());
	  for (InDataAnchorPtr inAnchorPtr : fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
		inAnchorPtr->UnlinkAll();
		FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(rpnProposalPostProcessingNode->GetOutDataAnchor(0), inAnchorPtr),
			   OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's 2nd index to fusion node:%s's 1st index failed.", fusedNode->GetName().c_str(), rpnProposalPostProcessingNode->GetName().c_str()),
			   return FAILED);
		OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s to fusion node:%s .", fusedNode->GetName().c_str(), rpnProposalPostProcessingNode->GetName().c_str());
	  }
	}
  ...
  // add control edge
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(scoreFilterPreSortNode->GetOutControlAnchor(),
  	rpnProposalPostProcessingNode->GetInControlAnchor()),
  	OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from node:%s's control to node:%s's control failed.", scoreFilterPreSortNode->GetName().c_str(), rpnProposalPostProcessingNode->GetName().c_str()),
             return FAILED);
  OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from node:%s's control to node:%s's control index.", scoreFilterPreSortNode->GetName().c_str(), rpnProposalPostProcessingNode->GetName().c_str());
  for (unsigned int i = 0; i < fusedNode->GetOutControlAnchor()->GetPeerInControlAnchors().size(); i++) {
  	FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(rpnProposalPostProcessingNode->GetOutControlAnchor(),
												fusedNode->GetOutControlAnchor()->GetPeerInControlAnchors().at(i)),
		   OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out control edge from fused node:%s's control index[%d] to fusion node:%s's control index failed.", fusedNode->GetName().c_str(), i, rpnProposalPostProcessingNode->GetName().c_str()),
		   return FAILED);
  OP_LOGD(FUSED_OP_TYPE.c_str(), "Add out control edge from fused node:%s's control index[%d] to fusion node:%s's control index.", fusedNode->GetName().c_str(), i, rpnProposalPostProcessingNode->GetName().c_str());
}
