...
Status MatrixSetDiagFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  int64_t dimsInput1 = matrixsetdiagInputShape.GetDimNum() - INT_NUM_TWO;
  ...
  FUSION_PASS_CHECK(dimNums1 == 0 || dimNums2 == 0,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "dims num should not be zero."),
                    return NOT_CHANGED);
  // GESHAPE->vector
  ...
  // check op support
  map<int, ge::GeTensorPtr> weights = {{2, assitPtr}};
  Status ret1 = ge::OpDescUtils::SetWeights(*matrixsetdiagVNode, weights);
  ...
  OP_LOGI(FUSED_OP_TYPE, "MatrixSetDiagFusionPass SUCCESS !!!!");
  return SUCCESS;
}
