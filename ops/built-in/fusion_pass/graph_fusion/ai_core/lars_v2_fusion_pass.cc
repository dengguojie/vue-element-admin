...
Status CreateSquareSumAllOp(ge::OpDescPtr src_op_desc, ge::OpDescPtr new_op_desc) {
...
Status LarsV2FusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Node:%s's OpDesc is null, fusion failed.", fused_node->GetName().c_str()),
      return PARAM_INVALID);
  ...
  ge::OpDescPtr square_sum_all_desc =
      std::make_shared<ge::OpDesc>(fused_desc->GetName() + "/SquareSumAll", "SquareSumAll");
  ...
  FUSION_PASS_CHECK(
      square_sum_all_node == nullptr,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", square_sum_all_node->GetName().c_str()),
      return PARAM_INVALID);
  FUSION_PASS_CHECK(
      lars_v2_update_node == nullptr,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", lars_v2_update_node->GetName().c_str()),
      return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(0)->GetPeerOutAnchor(),
                    ...
                    OP_LOGE(FUSED_OP_TYPE.c_str(),
                            "Add edge from fused node:%s's index 0 to fusion node:%s's "
                            "index 0 failed.",
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(1)->GetPeerOutAnchor(),
                                                       square_sum_all_node->GetInDataAnchor(1)),
                    OP_LOGE(FUSED_OP_TYPE.c_str(),
                            "Add edge from fused node:%s's index 1 to fusion node:%s's "
                            "index 1 failed.",
                            fused_node->GetName().c_str(), square_sum_all_node->GetName().c_str()),
                    return FAILED);
  ...
  for (unsigned int i = 0; i < fused_node->GetInControlAnchor()->GetPeerOutControlAnchors().size(); i++) {
    FUSION_PASS_CHECK(
        SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInControlAnchor()->GetPeerOutControlAnchors().at(i),
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(square_sum_all_node->GetOutDataAnchor(0),
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(square_sum_all_node->GetOutDataAnchor(1),
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(2)->GetPeerOutAnchor(),
                    ...
                    OP_LOGE(FUSED_OP_TYPE.c_str(),
                            "Add edge from fused node:%s's index 2 to fusion node:%s's "
                            "index 4 failed.",
                            fused_node->GetName().c_str(), lars_v2_update_node->GetName().c_str()),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(3)->GetPeerOutAnchor(),
                                                       lars_v2_update_node->GetInDataAnchor(5)),
                    OP_LOGE(FUSED_OP_TYPE.c_str(),
                            "Add edge from fused node:%s's index 3 to fusion node:%s's "
                            "index 5 failed.",
                            fused_node->GetName().c_str(), lars_v2_update_node->GetName().c_str()),
                    return FAILED);
  ...
  if (fused_node->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
    ...
    for (InDataAnchorPtr inAnchorPtr : fused_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
      ...
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(lars_v2_update_node->GetOutDataAnchor(0), inAnchorPtr),
                        OP_LOGE(FUSED_OP_TYPE.c_str(),
  ...
  for (auto lars_v2_out_control_node : fused_node->GetOutControlNodes()) {
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(lars_v2_update_node->GetOutControlAnchor(),
  ...
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(fused_node),
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "remove fusedNode node[%s] failed", fused_node->GetName().c_str()),
                    return FAILED);
