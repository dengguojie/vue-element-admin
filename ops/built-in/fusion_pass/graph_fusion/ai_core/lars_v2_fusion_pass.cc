...
vector<FusionPattern*> LarsV2FusionPass::DefinePatterns() {
  ...
  FUSION_PASS_CHECK(pattern == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "New a pattern object failed."),
                    return patterns);
...
Status LarsV2FusionPass::Fusion(ComputeGraph& graph, Mapping& mapping, vector<NodePtr>& fusion_nodes) {
  ...
  FUSION_PASS_CHECK(fused_node == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "The fusedNode is null, fusion failed."),
  ...
  FUSION_PASS_CHECK(fused_desc == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                                   "The fusedNode's OpDesc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                     "Node:%s's OpDesc is null, fusion failed.", fused_node->GetName().c_str()),
      return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(
      GraphUtils::AddEdge(fused_in_data_anchor1->GetPeerOutAnchor(), square_sum_all_node->GetInDataAnchor(1)) !=
          SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
              "Add edge from fused node:%s's index 1 to fusion node:%s's index 1 failed.",
              fused_node->GetName().c_str(), square_sum_all_node->GetName().c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
              "Add edge from fused node:%s's index 0 to fusion node:%s's index 2 failed.",
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
              "Add edge from fused node:%s's index 1 to fusion node:%s's index 3 failed.",
              square_sum_all_node->GetName().c_str(), lars_v2_update_node->GetName().c_str()),
      return FAILED);
  OP_LOGD(kFusedOpType.c_str(), "Add edge from fused node:%s's index 0 to fusion node:%s's index 3",
          square_sum_all_node->GetName().c_str(), lars_v2_update_node->GetName().c_str());
  ...
  FUSION_PASS_CHECK(
      GraphUtils::AddEdge(fused_in_data_anchor2->GetPeerOutAnchor(), lars_v2_update_node->GetInDataAnchor(4)) !=
          SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
              "Add edge from fused node:%s's index 2 to fusion node:%s's index 4 failed.",
              fused_node->GetName().c_str(), lars_v2_update_node->GetName().c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(
      GraphUtils::AddEdge(fused_in_data_anchor3->GetPeerOutAnchor(), lars_v2_update_node->GetInDataAnchor(5)) !=
          SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
              "Add edge from fused node:%s's index 3 to fusion node:%s's index 5 failed.",
              fused_node->GetName().c_str(), lars_v2_update_node->GetName().c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(fused_out_data_anchor0 == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                                   "The fused_out_data_anchor0 is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(GRAPH_SUCCESS != graph.RemoveNode(fused_node),
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                                   "Remove fusedNode node[%s] failed", fused_node->GetName().c_str()),
                    return FAILED);
