...
Status LarsV2FusionPass::Fusion(ComputeGraph& graph, Mapping& mapping, vector<NodePtr>& fusion_nodes) {
  ...
  FUSION_PASS_CHECK(fused_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The fusedNode is null, fusion failed."),
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "Node:%s's OpDesc is null, fusion failed.",
  ...
  FUSION_PASS_CHECK(GRAPH_SUCCESS != graph.RemoveNode(fused_node),
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "Remove fusedNode node[%s] failed",
                                                   fused_node->GetName().c_str()),
                    return FAILED);
