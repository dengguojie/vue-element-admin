...
Status AReduceMeanFusionPass::CheckMeanFussionOrNot(vector<int64_t> tensor_info,
...
vector<FusionPattern *> AReduceMeanFusionPass::DefinePatterns() {
  ...
  new (std::nothrow) FusionPattern("AReduceMeanFusionPass");
...
Status AReduceMeanFusionPass::Fusion(ge::ComputeGraph &graph,
  ...
  FUSION_PASS_CHECK(meanNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "meanNode is null, fusion failed."),
           return PARAM_INVALID);

  ge::GeTensorDesc tensor_input = \
  meanNode->GetOpDesc()->GetInputDesc(0);
  ge::GeTensorDesc axis_input = \
  meanNode->GetOpDesc()->GetInputDesc(1);
  ...
  if (!(CheckMeanFussionOrNot(tensor_info, const_data) == SUCCESS)) {
  ...
  for (auto inDataAnchor :
       meanNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(meanNode->GetOutDataAnchor(0),
                                        inDataAnchor) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove mean and outnode edge failed."), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(meanNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                                     inDataAnchor) != SUCCESS,
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(meanNode) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove meanNode failed."), return FAILED);
...
REGISTER_PASS("AReduceMeanFusionPass", BUILT_IN_GRAPH_PASS,
