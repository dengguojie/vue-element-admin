...
static Status SetInitHCTensorData(ge::GeTensorPtr initTensorPtr, ge::GeTensorDesc tensorDesc,
  ...
  FUSION_PASS_CHECK(srcData == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_NODE, "initTensorPtr->GetData().data() is NULL"),
                    return FAILED);
...
Status CommonLSTMFusionPass::ProcessLSTMBias(ge::NodePtr fusedNode, const InputIndexInfo &inputIndexInfo,
  ...
  if (hasBias) {
    ...
    FUSION_PASS_CHECK(biasT.empty(),
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "onnx LSTM biasT is null, fusion failed."),
                      return FAILED);
  ...
  if (num_directions == 1) {
    ...
    if (dataType == ge::DT_FLOAT16) {
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                     "Node:%s 's type is not in (float16, float32), fusion failed",
                                     fusedNode->GetName().c_str());
    }
    ...
    if (dataType == ge::DT_FLOAT16) {
      ...
      FUSION_PASS_CHECK(ret == FAILED,
                        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_NODE, "SetBiasTensorData failed"), return FAILED);
...
static Status SetWeightTensorData(ge::GeTensorPtr wTensorPtr, ge::GeTensorPtr rTensorPtr,
  ...
  FUSION_PASS_CHECK(wxhMergeData.get() == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_NODE, "wxhMergeData is NULL"),
                    return FAILED);
  T *wxData = (T *)wTensorPtr->GetData().data();
  T *whData = (T *)rTensorPtr->GetData().data();
  FUSION_PASS_CHECK(wxData == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_NODE, "wxData is NULL"), return FAILED);
  FUSION_PASS_CHECK(whData == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_NODE, "whData is NULL"), return FAILED);

  auto retMem = memset_s(wxhMergeData.get(), weightSize, 0, weightSize);
  FUSION_PASS_CHECK(retMem != EOK, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_NODE,
  ...
  FUSION_PASS_MAKE_SHARED((dstTensorPtr = std::make_shared<ge::GeTensor>(
                               tensorDesc,
                               reinterpret_cast<uint8_t *>(wxhMergeData.get()), weightSize * sizeof(T))),
                           return FAILED);
  tensorPtr.push_back(dstTensorPtr);
...
Status CommonLSTMFusionPass::ProcessLSTMWxh(ge::NodePtr fusedNode, const InputIndexInfo &inputIndexInfo,
  ...
  FUSION_PASS_CHECK(weightsW.empty(),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "LSTM weightsW is null, fusion failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(weightsR.empty(),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "LSTM weightsR is null, fusion failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(wCol == 0, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "wCol can not 0"),
                    return FAILED);
  FUSION_PASS_CHECK(rCol == 0, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "rCol can not 0"),
                    return FAILED);
  ...
  if (num_directions == 1) {
    ...
    if (dataType == ge::DT_FLOAT16) {
      ...
      FUSION_PASS_CHECK(ret == FAILED,
                        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_NODE, "SetWeightTensorData failed"), return FAILED);
      ret = SetWeightTensorData<uint16_t>(wTensorPtr, rTensorPtr, inputDims,
...
Status CommonLSTMFusionPass::AddExpandDimsConcatNode(ge::ComputeGraph &graph, ge::NodePtr fusedNode,
                                                     ge::NodePtr forwardRnnNode, ge::NodePtr backwardRnnNode,
                                                     ge::GeTensorDesc dynamicRnnOutputDesc, std::string nodeName,
                                                     vector<ge::NodePtr> &newNodes) {
  ge::OpDescPtr fusedDesc = fusedNode->GetOpDesc();
  std::string forwardExdName = fusedDesc->GetName() + "/ExpandDims" + "Forward";
  std::string backwardExdName = fusedDesc->GetName() + "/ExpandDims" + "Backward";
  auto forwardExdOp = ge::OperatorFactory::CreateOperator(forwardExdName.c_str(), "ExpandDims");
  auto backwardExdOp = ge::OperatorFactory::CreateOperator(backwardExdName.c_str(), "ExpandDims");
  ...
  FUSION_PASS_CHECK(backwardExdOp.IsEmpty(),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "create ExpandDims Backward Op error"),
                    return FAILED);
  auto backwardExdDesc = ge::OpDescUtils::GetOpDescFromOperator(backwardExdOp);
  backwardExdOp.BreakConnect();
  ...
  forwardExdDesc->UpdateInputDesc("x", dynamicRnnOutputDesc);
  forwardExdDesc->UpdateInputDesc("axis", tensorDesc);
  forwardExdDesc->UpdateOutputDesc("y", ExdTensorDesc);

  backwardExdDesc->UpdateInputDesc("x", dynamicRnnOutputDesc);
  backwardExdDesc->UpdateInputDesc("axis", tensorDesc);
  backwardExdDesc->UpdateOutputDesc("y", ExdTensorDesc);
  ...
  ge::NodePtr backwardExdNode = graph.AddNode(backwardExdDesc);
  ge::NodePtr AxisNode = graph.AddNode(AxisDesc);

  FUSION_PASS_CHECK(forwardExdNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "create ExpandDims forward node failed."),
                    return FAILED);
  FUSION_PASS_CHECK(backwardExdNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "create ExpandDims backward node failed."),
                    return FAILED);
  FUSION_PASS_CHECK(AxisNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Axis node is null, fusion failed."),
                    return FAILED);
  ...
  ret = ge::AttrUtils::SetListStr(backwardExdNode->GetOpDesc(), ATTR_NAME_OP_INFER_DEPENDS, original_names);
  if (!ret) {
    OP_LOGW(FUSED_OP_TYPE.c_str(), "ExpandDimNode set ATTR_NAME_OP_INFER_DEPENDS error.");
  }

  newNodes.push_back(forwardExdNode);
  newNodes.push_back(backwardExdNode);
  newNodes.push_back(AxisNode);

  // add forward rnn output y edge to expanddims node
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(forwardRnnNode->GetOutDataAnchor(0),
  ...
  // add backward rnn output y edge to expanddims node
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(backwardRnnNode->GetOutDataAnchor(0),
                                                       backwardExdNode->GetInDataAnchor(0)),
        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add DynamicRNN edge to fusion ExpandDim x failed."),
        return FAILED);
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(AxisNode->GetOutDataAnchor(0),
                                                       backwardExdNode->GetInDataAnchor(1)),
        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add axis edge to fusion ExpandDim axis failed."),
        return FAILED);

  // create concat node
  std::string concatOpName = fusedDesc->GetName() + "/ConcatD_" + nodeName;
  ...
  FUSION_PASS_CHECK(
      SUCCESS != ge::GraphUtils::AddEdge(forwardExdNode->GetOutDataAnchor(0),
                                         concatNode->GetInDataAnchor(0)),
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                     "add expanddims forward edge to fusion concat node failed."), return FAILED);
  FUSION_PASS_CHECK(
      SUCCESS != ge::GraphUtils::AddEdge(backwardExdNode->GetOutDataAnchor(0),
                                         concatNode->GetInDataAnchor(1)),
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                     "add expanddims backward edge to fusion concat node failed."), return FAILED);

  // add concat edge to output node
  for (InDataAnchorPtr oriTopPeerAnchorPtri : fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(
        SUCCESS != ge::GraphUtils::RemoveEdge(fusedNode->GetOutDataAnchor(0), oriTopPeerAnchorPtri),
        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "remove concat output edge to output node failed."),
        return FAILED);
    FUSION_PASS_CHECK(
        SUCCESS != ge::GraphUtils::AddEdge(concatNode->GetOutDataAnchor(0), oriTopPeerAnchorPtri),
        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add concat output edge to output node failed."),
        return FAILED);
  }
...
Status CommonLSTMFusionPass::AddRNNMaskNode(ge::NodePtr fusedNode, ge::NodePtr dynamicRnnNode, ge::ComputeGraph &graph,
                                            int32_t hiddenSize, vector<ge::NodePtr> &newNodes)
    {
        ...
        FUSION_PASS_CHECK(dimLength.size() != 1,
            VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Unexcepted seqlength input shape"),
                          return FAILED);
        ...
        FUSION_PASS_CHECK(maskNode == nullptr,
            VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                           "Create Mask node:%s failed", rnnMaskDesc->GetName().c_str()),
            return FAILED);
...
Status CommonLSTMFusionPass::AddSliceConcatNode(ge::ComputeGraph &graph, ge::NodePtr fusedNode,
  ...
  ge::AttrUtils::SetListInt(slice_desc_forward, "begin", {-1, 0, 0});
  ge::AttrUtils::SetListInt(slice_desc_forward, "end", {-2, slice_dims[1], slice_dims[2]});
  ...
  ge::AttrUtils::SetListInt(slice_desc_reverse, "begin", {-1, 0, 0});
  ge::AttrUtils::SetListInt(slice_desc_reverse, "end", {-2, slice_dims[1], slice_dims[2]});
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                     "add dynamicrnn edge to fusion slice_node failed."), return FAILED);
  FUSION_PASS_CHECK(
  ...
  FUSION_PASS_CHECK(concatOp.IsEmpty(),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                   "create concat %s operator error", nodeName.c_str()),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                     "add silce forward edge to fusion slice_node failed."), return FAILED);
  FUSION_PASS_CHECK(
      SUCCESS != ge::GraphUtils::AddEdge(slice_node_reverse->GetOutDataAnchor(0), concat_node->GetInDataAnchor(1)),
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                     "add slice reverse edge to fusion slice_node failed."), return FAILED);
  ...
  for (InDataAnchorPtr oriTopPeerAnchorPtri : fusedNode->GetOutDataAnchor(nodeIndex)->GetPeerInDataAnchors()) {
    ...
    FUSION_PASS_CHECK(
        ...
        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                       "add concat node to fusion node output failed."), return FAILED);
  }
  return SUCCESS;
}
...
Status CommonLSTMFusionPass::AddSliceNode(ge::ComputeGraph &graph, ge::NodePtr fusedNode,
  ...
  FUSION_PASS_CHECK(sliceOp.IsEmpty(),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "create slice_op Op error"),
                    return FAILED);
...
ge::OpDescPtr CommonLSTMFusionPass::CreateSplitDesc(ge::OpDescPtr splitDesc, ge::OpDescPtr fusedDesc,
  ...
  splitDesc->AddInputDesc("x", tensorDesc);
  ...
  splitDesc->AddOutputDesc("y0", tensorDesc);
  splitDesc->AddOutputDesc("y1", tensorDesc);
...
Status CommonLSTMFusionPass::SetOutputTensorDescAttr(uint16_t originOutputIndex, uint16_t fuseOutputIndex,
  ...
  FUSION_PASS_CHECK(fuseNode->GetOpDesc() == nullptr,
                    OP_LOGW(FUSED_OP_TYPE.c_str(), "fuseNode OpDesc is null"),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(originNode->GetOpDesc() == nullptr,
                    OP_LOGW(FUSED_OP_TYPE.c_str(), "originNode OpDesc is null"),
                    return FAILED);
  ge::AttrUtils::SetStr(fuseNode->GetOpDesc()->MutableOutputDesc(fuseOutputIndex),
  ...
  GraphPassUtil::SetDataDumpOriginFormat(
      originNode->GetOpDesc()->GetOutputDesc(originOutputIndex).GetOriginFormat(),
      fuseNode->GetOpDesc()->MutableOutputDesc(fuseOutputIndex));
  return SUCCESS;
}
...
Status CommonLSTMFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes)
{
  ...
  std::map<string, string> directionMapInfo {{"forward", "UNIDIRECTIONAL"},
                                            {"reverse", "REDIRECTIONAL"},
  ...
  if (direction == "bidirectional") {
    ...
    std::string dynamicRnnOpForwardName = fusedDesc->GetName() + "/DynamicRNN" + "Forward";
    auto dynamicRnnOpForward = ge::OperatorFactory::CreateOperator(
        dynamicRnnOpForwardName.c_str(), "DynamicRNN");
    ...
    std::string dynamicRnnOpReverseName = fusedDesc->GetName() + "/DynamicRNN" + "Reverse";
    auto dynamicRnnOpReverse = ge::OperatorFactory::CreateOperator(
        dynamicRnnOpReverseName.c_str(), "DynamicRNN");
    FUSION_PASS_CHECK(dynamicRnnOpReverse.IsEmpty(),
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "create DynamicRnn reverse operator error"),
    return FAILED);
    ...
    if (hasInitH) {
      ...
      if (nodeType != "Const" && nodeType != "Constant") {
        ...
        FUSION_PASS_CHECK(retInitH != SUCCESS,
                          VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Process init_h fail."),
                          return FAILED);
    ...
    if (hasInitC) {
      ...
      if (nodeType != "Const" && nodeType != "Constant") {
        ...
        FUSION_PASS_CHECK(retInitC != SUCCESS,
                          VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Process init_c fail."),
                          return FAILED);
    ...
    FUSION_PASS_CHECK(retW != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Process w fail."),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(retBias != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Process bias fail."),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(dynamicRnnForwardNode == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "dynamicRnn forward node is null, fusion failed."),
    return FAILED);
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(wReverse_node->GetOutDataAnchor(0),
                      ...
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "add reverse dynamicRnn edge to fusion node w failed."),
    return FAILED);
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(biasForward_node->GetOutDataAnchor(0),
                      ...
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "add forward dynamicRnn edge to fusion node bias failed."),
    return FAILED);
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(biasReverse_node->GetOutDataAnchor(0),
                      ...
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "add reverse dynamicRnn edge to fusion node bias failed."),
    return FAILED);
    ...
    if (hasInitH) {
      if (initHConst) {
        ...
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(inith_forward_node->GetOutDataAnchor(0),
                          ...
                          VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                         "add forward dynamicRnn edge to fusion node init_h failed."),
        return FAILED);
    ...
    if (hasInitC) {
      if (initCConst) {
        ...
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(initc_forward_node->GetOutDataAnchor(0),
                          ...
                          VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                         "add forward dynamicRnn edge to fusion node init_c failed."),
        return FAILED);
    ...
    FUSION_PASS_CHECK(AddExpandDimsConcatNode(graph, fusedNode, dynamicRnnForwardNode, dynamicRnnReverseNode,
                                              outputYDesc, "Y", newNodes) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "create ExpandDimsY Op operator error"),
                      return FAILED);
    FUSION_PASS_CHECK(AddSliceConcatNode(graph, fusedNode, dynamicRnnForwardNode, dynamicRnnReverseNode,
                      ...
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "create slice H Op operator error"), return FAILED);
    FUSION_PASS_CHECK(AddSliceConcatNode(graph, fusedNode, dynamicRnnForwardNode, dynamicRnnReverseNode,
                                         outputYDesc, newNodes, "C", 2) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "create slice C Op operator error"), return FAILED);
    ...
    std::string dynamicRnnOpName = fusedDesc->GetName() + "/DynamicRnn";
    auto dynamicRnnOp = ge::OperatorFactory::CreateOperator(dynamicRnnOpName.c_str(), "DynamicRNN");
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                      ...
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "add DynamicRNN edge to fusion node x failed."),
    return FAILED);
