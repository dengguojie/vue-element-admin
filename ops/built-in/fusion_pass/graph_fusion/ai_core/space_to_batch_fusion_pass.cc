...
vector<FusionPattern*> SpaceToBatchFusionPass::DefinePatterns() {
  ...
  pattern->AddOpDesc(PATTERN_SPACE, {"SpaceToBatch"})
      .SetOutput(PATTERN_SPACE);
...
Status SpaceToBatchFusionPass::Fusion(ge::ComputeGraph &graph,
                                      Mapping &mapping,
                                      vector<ge::NodePtr> &fusionNodes)
{
  ...
  std::vector<PassAttrInfo> attr_infos = {
      {1, "paddings", "SetListInt"}
  };
  const std::string fusion_op_type = "SpaceToBatchD";
  ge::OpDescPtr fusionDescPtr =
      PatternFusionUtil::GetFusionOpDesc(spaceNode, fusion_op_type, attr_infos);
  ...
  FUSION_PASS_CHECK(spaceDesc == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "spaceNode's OpDesc is null, fusion failed."),
           return PARAM_INVALID);
  ...
  // 第一个输入为非const的value，第二个输入为const的paddings
