...
namespace fe
{
    ...
    static const string PATTERN_PROPOSAL = "Proposal";
    ...
    void GenerateAnchors(const vector<float>& anchor_scale,
        const vector<float>& anchor_ratio, const float anchor_base_size, vector<float>& anchor_boxes) {
        ...
        {
            if (fabsf(anchor_ratio[i]) < 1e-6) { continue; }
            float ratio = size / anchor_ratio[i];
            float ws_ori = float(ROUND(sqrt(ratio)));
            float hs_ori = float(ROUND(ws_ori * anchor_ratio[i]));
    ...
    Status ProposalFusionPass::GenerateAnchorsFp16(uint16_t *output1, ge::NodePtr proposalVNode) {
        ...
        //获取anchor_base_size、feat_stride
        ...
        if ((int)anchor_scale.size() * (int)anchor_ratio.size() * 4 != channel) {
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Proposal input channel is invalid, scale.size*ratio.size*4 must equal channel,"
                "scale.size:%d, ratio.size:%d, channel:%d", anchor_scale.size(), anchor_ratio.size(), channel);
            return FAILED;
        }

        //生成单个锚点的原始anchor_scale.size() * anchor_ratio.size()个矩形框
    ...
    vector<FusionPattern*> ProposalFusionPass::DefinePatterns() {
        ...
        // define origin graph
        pattern->AddOpDesc(PATTERN_PROPOSAL, {PROPOSAL})
            .SetOutput(PATTERN_PROPOSAL);
...
Status ProposalFusionPass::Fusion(ge::ComputeGraph &graph,
                                  Mapping &mapping,
                                  vector<ge::NodePtr> &fusionNodes)
{
    ...
    FUSION_PASS_CHECK(proposalVNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "proposalVNode is null, fusion failed."), return PARAM_INVALID);

    // input of proposal
    ge::OpDescPtr proposalDesc = proposalVNode->GetOpDesc();
    FUSION_PASS_CHECK(proposalDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "proposalVNode's OpDesc is null, fusion failed."), return PARAM_INVALID);

    // 寻找proposal节点的输入的父节点
    //ge::InDataAnchorPtr proposalAnchorPtr0 = proposalVNode->GetInDataAnchor(0);
    //ge::OutDataAnchorPtr constAnchorPtr0 = proposalAnchorPtr0->GetPeerOutAnchor();
    //ge::NodePtr constNode0 = constAnchorPtr0->GetOwnerNode();

    // 获取进入proposal节点的输入描述：区分const与variable
    ge::GeTensorDesc proposalInputTensor = proposalVNode->GetOpDesc()->GetInputDesc(1);
    ...
    // GESHAPE->vector
    //vector<int64_t> dimInfo = proposalInputShape.GetDims();

    // Format
    //Format assitMatrixFormat = proposalInputTensor.GetFormat();
    ...
    FUSION_PASS_MAKE_SHARED((assitPtr = std::make_shared<ge::GeTensor>(tensorDesc, reinterpret_cast<uint8_t *>(inputAssit.get()),
                                                              dimNums * sizeof(uint16_t))), assitPtr = nullptr; return PARAM_INVALID);
