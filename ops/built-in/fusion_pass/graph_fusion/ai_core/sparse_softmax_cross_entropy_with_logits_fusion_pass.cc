...
vector<FusionPattern*> SparseSoftMaxFusionPass::DefinePatterns() {
    ...
    pattern->AddOpDesc(PATTERN_SPARSE_SOFTMAX, {SPARSE_SOFTMAX})
            .SetOutput(PATTERN_SPARSE_SOFTMAX);

    patterns.push_back(pattern);

    return patterns;
}
Status SparseSoftMaxFusionPass::Fusion(ge::ComputeGraph &graph,
                                       Mapping &mapping,
                                       vector<ge::NodePtr> &fusionNodes)
{
  ...
  FUSION_PASS_CHECK(OneHotNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "OneHotNode fusionNode:%s is null, fusion failed.",
                   OneHotNode->GetName().c_str()),
                   return PARAM_INVALID);
  ...
  *ptr1 = size_one;
  assitPtr2 = std::make_shared<ge::GeTensor>(tensorDesc1,
                                             reinterpret_cast<uint8_t *>(inputAssit1.get()),
  ...
  *ptr2 = size_zero;
  assitPtr3 = std::make_shared<ge::GeTensor>(tensorDesc2,
                                             reinterpret_cast<uint8_t *>(inputAssit2.get()),
  ...
  SoftMax->MutableInputDesc(0)->SetFormat(
      ge::FORMAT_ND);
  SoftMax->MutableOutputDesc(0)->SetFormat(
      ge::FORMAT_ND);
  SoftMax->MutableInputDesc(1)->SetOriginFormat(
      ge::FORMAT_ND);
  SoftMax->MutableInputDesc(1)->SetOriginShape(
      assitShape3);
  SoftMax->MutableInputDesc(1)->SetFormat(
      ge::FORMAT_ND);
  SoftMax->MutableInputDesc(1)->SetShape(
      assitShape3);
  SoftMax->MutableOutputDesc(1)->SetFormat(
      ge::FORMAT_ND);
  SoftMax->MutableInputDesc(1)->SetDataType(
      ge::DT_FLOAT16);

  FUSION_PASS_CHECK(SoftMax == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node:%s's OpDesc is null, fusion failed.",
  ...
  FUSION_PASS_CHECK(GatherV2Node == nullptr,
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(OneHotNode->GetOutDataAnchor(0),
                                              GatherV2Node->GetInDataAnchor(0)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                   OneHotNode->GetName().c_str(), 0,
                   GatherV2Node->GetName().c_str(), 0),
                   return FAILED);
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(1)->GetPeerOutAnchor(),
                                              GatherV2Node->GetInDataAnchor(1)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                   fusedNode->GetName().c_str(), 1,
                   GatherV2Node->GetName().c_str(), 1),
                   return FAILED);
  OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d].",
          fusedNode->GetName().c_str(), 1,
          GatherV2Node->GetName().c_str(), 1);
  ...
  if (fusedNode->GetInControlAnchor()->GetPeerOutControlAnchors().size()>1) {
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInControlAnchor()->GetPeerOutControlAnchors().at(1),
                                                GatherV2Node->GetInControlAnchor()),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's control index[%d] to fusion node:%s's control index failed.",
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                                              SoftMaxNode->GetInDataAnchor(0)),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                     fusedNode->GetName().c_str(), 0,
                     SoftMaxNode->GetName().c_str(), 0),
                     return FAILED);
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInControlAnchor()->GetPeerOutControlAnchors().at(0),
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(GatherV2Node->GetOutDataAnchor(0),
                                              SoftMaxNode->GetInDataAnchor(1)),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                     GatherV2Node->GetName().c_str(), 0,
                     SoftMaxNode->GetName().c_str(), 1),
                     return FAILED);
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d].",
          GatherV2Node->GetName().c_str(), 0,
          SoftMaxNode->GetName().c_str(), 1);
  ...
  if (fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
    ...
    for (InDataAnchorPtr inAnchorPtr : fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
      ...
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(SoftMaxNode->GetOutDataAnchor(0), inAnchorPtr),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's 2nd index to fusion node:%s's 1st index failed.",
                     fusedNode->GetName().c_str(),
                     SoftMaxNode->GetName().c_str()),
                     return FAILED);
  ...
  if (fusedNode->GetOutDataAnchor(1)->GetPeerInDataAnchors().size() > 0) {
    ...
    for (InDataAnchorPtr inAnchorPtr : fusedNode->GetOutDataAnchor(1)->GetPeerInDataAnchors()) {
      inAnchorPtr->UnlinkAll();
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(SoftMaxNode->GetOutDataAnchor(1), inAnchorPtr),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's 2nd index to fusion node:%s's 1st index failed.",
                     fusedNode->GetName().c_str(),
                     SoftMaxNode->GetName().c_str()),
                     return FAILED);
