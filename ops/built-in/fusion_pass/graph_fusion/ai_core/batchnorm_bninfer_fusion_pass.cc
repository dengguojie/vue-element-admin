...
Status BatchNormBnInferFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  if (is_not_const == true) {
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("x", input_tensor_x) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input x failed."), return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("scale", input_tensor_mean) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input scale failed."), return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("offset", input_tensor_variance) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input offset failed."), return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("mean", input_tensor_scale) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input mean failed."), return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("variance", input_tensor_offset) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input variance failed."), return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddOutputDesc("y", tensor1) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add output failed."), return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input mean failed.", newOpName.c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input variance failed.", newOpName.c_str()),
      return FAILED);
  ...
  if (newOpdesc->AddInputDesc("offset", input_tensor_offset) != SUCCESS){
    momentumPtr = nullptr;
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input offset failed.", newOpName.c_str());
    return FAILED;
  }
  // add output
  ge::GeTensorDesc tensor1 = batchNormNode->GetOpDesc()->GetOutputDesc(0);
