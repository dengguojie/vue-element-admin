...
vector<FusionPattern *> BatchNormBnInferFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("BatchNormBnInferFusionPass");
...
Status BatchNormBnInferFusionPass::Fusion(
    ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &fusionNodes)
{
  ...
  bool isTraing = true;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetBool(batchNormNode->GetOpDesc(), IS_TRAING,
                                   isTraing),
  ...
  FUSION_PASS_CHECK(
          batchNormNode->GetOutDataAnchor(1)
                  ->GetPeerInDataAnchors()
                  .size() != 0 ||
          batchNormNode->GetOutDataAnchor(2)
                  ->GetPeerInDataAnchors()
                  .size() != 0 ||
          batchNormNode->GetOutDataAnchor(3)
                  ->GetPeerInDataAnchors()
                  .size() != 0 ||
          batchNormNode->GetOutDataAnchor(4)
                  ->GetPeerInDataAnchors()
                  .size() != 0,
  ...
  for (int i=1; i<5; i++) {
    inputNode = batchNormNode->GetInDataAnchor(i)
                ->GetPeerOutAnchor()->GetOwnerNode();
    if(inputNode->GetType() != "Const" && inputNode->GetType() != "Constant") {
      isNotConst = true;
      break;
    }
  }
  ...
  newOpdesc =
      std::make_shared<ge::OpDesc>(batchNormNode->GetName(), BNINFER);
  ...
  if(isNotConst == true) {
    ...
    ge::GeTensorDesc input_tensor3 =
        batchNormNode->GetOpDesc()->GetInputDesc(INPUT_IDX_2);
    ...
    ge::GeTensorDesc input_tensor4 =
        batchNormNode->GetOpDesc()->GetInputDesc(INPUT_IDX_3);
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor4) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "BNInfer add input mean failed."), return FAILED);

    ge::GeTensorDesc input_tensor5 =
        batchNormNode->GetOpDesc()->GetInputDesc(INPUT_IDX_4);
    ...
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
             ...
             return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
                 batchNormNode->GetInDataAnchor(3)->GetPeerOutAnchor(),
                 newNode->GetInDataAnchor(3)) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
                     batchNormNode->GetInDataAnchor(3)
                         ->GetPeerOutAnchor()
                         ->GetOwnerNode()
                         ->GetName()
                         .c_str(),
             ...
             return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
                 batchNormNode->GetInDataAnchor(4)->GetPeerOutAnchor(),
                 newNode->GetInDataAnchor(4)) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
                     batchNormNode->GetInDataAnchor(4)
                         ->GetPeerOutAnchor()
                         ->GetOwnerNode()
                         ->GetName()
                         .c_str(),
  ...
  ge::GeTensorDesc input_tensor2 =
      batchNormNode->GetOpDesc()->GetInputDesc(INPUT_IDX_3);
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor2) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input mean failed.", newOpName.c_str()), return FAILED);

  ge::GeTensorDesc input_tensor3 =
      batchNormNode->GetOpDesc()->GetInputDesc(INPUT_IDX_4);
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor3) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input variance failed.", newOpName.c_str()), return FAILED);
  ...
  ge::Format assistFormat = input_tensor1.GetFormat();
  ...
  FUSION_PASS_CHECK(inputAssit.get() == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "inputAssit is NULL"),
           return PARAM_INVALID);
  Status ret = NnSet(1, FLOAT_NUM_ONE,
  ...
  FUSION_PASS_MAKE_SHARED((momentumPtr = std::make_shared<ge::GeTensor>(tensorDesc,
  ...
  auto newConstantOp =
      std::make_shared<ge::OpDesc>(newOpdesc->GetName() + "_momentum",
                                   "Constant");
  ...
  (void)newOpdesc->AddInputDesc(newConstantOp->GetOutputDesc(0));

  // continue to add other inputs tensor_desc
  ge::GeTensorDesc input_tensor4 =
      batchNormNode->GetOpDesc()->GetInputDesc(1);
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor4) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input scale failed.", newOpName.c_str()), return FAILED);

  ge::GeTensorDesc input_tensor5 =
      batchNormNode->GetOpDesc()->GetInputDesc(INPUT_IDX_2);
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor5) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input offset failed.", newOpName.c_str()), return FAILED);

  // add output
  ge::GeTensorDesc tensor1 = batchNormNode->GetOpDesc()->GetOutputDesc(0);
  FUSION_PASS_CHECK(newOpdesc->AddOutputDesc(tensor1) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the output desc for the output y failed.", newOpName.c_str()), return FAILED);
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::SetBool(newNode->GetOpDesc(), USE_GLOBAL_STATS, true),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Set use_global_stats attr failed"), return FAILED);

  FUSION_PASS_CHECK(!ge::AttrUtils::SetInt(newNode->GetOpDesc(), MODE, 1),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Set mode attr failed"), return FAILED);

  // add edge for momentum node and bninference node
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(momentumNode->GetOutDataAnchor(0),
                                   newNode->GetInDataAnchor(3)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add input data edge failed."), return FAILED);
