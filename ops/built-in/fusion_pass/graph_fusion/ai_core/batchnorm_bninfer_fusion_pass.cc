...
Status BatchNormBnInferFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_CHECK(batchNormNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "batchNorm is null, fusion failed."),
                    return PARAM_INVALID);
  bool is_traing = true;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetBool(batchNormNode->GetOpDesc(), IS_TRAING, is_traing),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Get is_traing attr failed."),
                    return FAILED);

  FUSION_PASS_CHECK(is_traing, OP_LOGI(FUSED_OP_TYPE.c_str(), "is_traing is true, no need fusion."),
                    return NOT_CHANGED);
  ...
  // copy Opdesc
  std::shared_ptr<ge::OpDesc> newOpdesc;
  FUSION_PASS_MAKE_SHARED(
      (newOpdesc = std::make_shared<ge::OpDesc>(batchNormNode->GetName(), BNINFER)),
      return FAILED);

  FUSION_PASS_CHECK(newOpdesc == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "newOpdesc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (is_not_const) {
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("x", input_tensor_x) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input x failed."),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("scale", input_tensor_mean) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input scale failed."),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("offset", input_tensor_variance) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input offset failed."),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("mean", input_tensor_scale) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input mean failed."),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddInputDesc("variance", input_tensor_offset) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add input variance failed."),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(newOpdesc->AddOutputDesc("y", tensor1) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "BNInfer add output failed."),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(!ge::AttrUtils::SetFloat(newNode->GetOpDesc(), EPSILON, epsilon),
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Set epsilon attr failed"),
                      return FAILED);
    ...
    if (batchNormNode->GetOutControlAnchor()) {
      for (auto inControlAnchor : batchNormNode->GetOutControlAnchor()->GetPeerInControlAnchors()) {
        FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(batchNormNode->GetOutControlAnchor(),
    ...
    FUSION_PASS_CHECK(
        ...
        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                       "Add control edge between node %s. and node %s failed.",
    ...
    FUSION_PASS_CHECK(graph.RemoveNode(batchNormNode) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove batchnorm node failed."),
                      return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input x failed.",
      newOpName.c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                     "Op[%s]: add the input desc for the input mean failed.",
      newOpName.c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                     "Op[%s]: add the input desc for the input variance failed.",
      newOpName.c_str()),
      return FAILED);
  ...
  std::shared_ptr<ge::OpDesc> newConstantOp;
  FUSION_PASS_MAKE_SHARED((newConstantOp = std::make_shared<ge::OpDesc>(newOpdesc->GetName() + "_momentum",
                          "Constant")),
  ...
  FUSION_PASS_CHECK(momentumNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "momentumNode is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (!ge::AttrUtils::SetBool(newNode->GetOpDesc(), USE_GLOBAL_STATS, true)){
    momentumPtr = nullptr;
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Set use_global_stats attr failed");
    return FAILED;
  }
  if (!ge::AttrUtils::SetInt(newNode->GetOpDesc(), MODE, 1)){
    momentumPtr = nullptr;
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Set mode attr failed");
    return FAILED;
  }
  ...
  if (ge::GraphUtils::AddEdge(momentumNode->GetOutDataAnchor(0), newNode->GetInDataAnchor(3)) != SUCCESS){
    momentumPtr = nullptr;
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Add input data edge failed.");
    return FAILED;
  }
  // copy output edge
  for (auto inDataAnchor : batchNormNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
  ...
  if (graph.RemoveNode(batchNormNode) != SUCCESS){
    momentumPtr = nullptr;
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove identity node failed.");
    return FAILED;
  }
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Define BatchNormBnInferFusionPass fusion end");
  return SUCCESS;
