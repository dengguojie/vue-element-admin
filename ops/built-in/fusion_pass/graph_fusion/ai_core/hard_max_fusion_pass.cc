...
vector<FusionPattern *> HardMaxPass::DefinePatterns()
{
    ...
    FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE("HardMaxPass", "new pattern error"), return patterns);
                                      ...
                                      int64_t &depth, int64_t &dim)
{
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "CreateArgMax GetAttr dimension fail"), return FAILED);
    if (dimension < 0) {
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "CreateArgmax AddInputDesc fail"), return FAILED);
    ret = new_desc->AddOutputDesc("y", output_desc);
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "CreateArgmax AddOutputDesc fail"), return FAILED);
                                      ...
                                      ge::NodePtr &new_node, int64_t depth, int64_t dim)
{
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "CreateOneHotDNode AddInputDesc one fail."),
        return FAILED);
    ret = new_desc->AddOutputDesc("y", output_desc);
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "CreateOneHotDNode AddInputDesc one fail."),
        return FAILED);
...
{
    if (output == nullptr) {
        OP_LOGE("Output", "output pointer is null!");
        return FAILED;
        }
	                             ...
	                             ge::GeTensorPtr &assit_on_value_ptr, int32_t on_value, ge::GeTensorDesc &on_value_tensor_desc) const
{
    ...
    FUSION_PASS_CHECK(on_value_assit.get() == nullptr, OP_LOGE("HardMaxPass", "on_value_assit is NULL"),
        return PARAM_INVALID);
		
    ret = NnSet(on_value_dim_num, UINT_NUM_ZERO, *reinterpret_cast<uint16_t *>(on_value_assit.get()));
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "NnSet failed."), return ret);
...
{   
    ...
    FUSION_PASS_CHECK(off_value_assit.get() == nullptr, OP_LOGE("HardMaxPass", "off_value_assit is NULL"),
        return PARAM_INVALID);
...
Status HardMaxPass::AddEdgeToOneHotDForOut(ge::NodePtr &fused_node, ge::NodePtr &one_hot_d_node) const
{
    ...
    for (auto in_data_anchor : fused_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
        ...
        FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "AddEdgeToOneHotDForOut removeEdge fail"),
        return FAILED);
        ret = ge::GraphUtils::AddEdge(one_hot_d_node->GetOutDataAnchor(0), in_data_anchor);
        FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "AddEdgeToOneHotDForOut addEdge fail"), return FAILED);
    }
    return SUCCESS;
}
...
Status HardMaxPass::RemoveFusedNode(ge::ComputeGraph &graph, ge::NodePtr &fused_node) const
{
    ...
    FUSION_PASS_CHECK(graph.RemoveNode(fused_node) != SUCCESS, OP_LOGE("HardMaxPass", "RemoveFusedNode error"),
        return FAILED);
    return SUCCESS;
}
...
Status HardMaxPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &fusion_nodes)
{
    ...
    FUSION_PASS_CHECK(fused_node == nullptr, OP_LOGE("HardMaxPass", "Fusion GetNode Error"), return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "Fusion fail"), return FAILED);
	
    ret = CreateOneHotDNode(graph, fused_node, argmax_d_node, one_hot_d_node, depth, dim);
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "Fusion CreateOneHotDNode fail"), return FAILED);
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "AddEdge to argmax_d_node fail"), return FAILED);
	
    ret = ge::GraphUtils::AddEdge(argmax_d_node->GetOutDataAnchor(0), one_hot_d_node->GetInDataAnchor(0));
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "AddEdge to one_hot_d_node fail"), return FAILED);
	
    ret = AddEdgeToOneHotDForOut(fused_node, one_hot_d_node);
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "AddEdgeToOneHotDForOut fail"), return FAILED);
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE("HardMaxPass", "RemoveFusedNode fail"), return FAILED);
