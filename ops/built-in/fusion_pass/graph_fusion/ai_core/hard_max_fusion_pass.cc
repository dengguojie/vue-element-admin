...
vector<FusionPattern *> HardMaxPass::DefinePatterns()
{
    ...
    FUSION_PASS_CHECK(pattern == nullptr, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "new pattern error"), return patterns);
    pattern->AddOpDesc(PATTERN_FUSEDNODE, { "HardMax" }).SetOutput(PATTERN_FUSEDNODE);
    patterns.push_back(pattern);
    return patterns;
}

Status HardMaxPass::CreateArgMaxDNode(ge::ComputeGraph &graph, ge::NodePtr &fused_node,
                                      ge::NodePtr &new_node, int64_t &depth, int64_t &dim)
{
    ge::OpDescPtr new_desc = nullptr;
    FUSION_PASS_MAKE_SHARED((new_desc = std::make_shared<ge::OpDesc>("ArgMaxD_For_HardMax",
                            "ArgMaxD")), return INTERNAL_ERROR);
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "CreateArgMax GetAttr dimension fail"), return FAILED);
    if (dimension < 0) {
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "CreateArgmax AddInputDesc fail"), return FAILED);
    ret = new_desc->AddOutputDesc("y", output_desc);
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "CreateArgmax AddOutputDesc fail"), return FAILED);
...
Status HardMaxPass::SetConstDesc(const vector<int64_t> &tensor_shape,
...
Status HardMaxPass::CreateOneHotDNode(ge::ComputeGraph &graph, ge::NodePtr &fused_node,
                                      ge::NodePtr &argmax_node, ge::NodePtr &new_node,
                                      int64_t depth, int64_t dim)
{
    ge::OpDescPtr new_desc = nullptr;
    FUSION_PASS_MAKE_SHARED((new_desc = std::make_shared<ge::OpDesc>("OneHotD_For_HardMax",
                            "OneHotD")), return INTERNAL_ERROR);
    ge::GeTensorDesc input_desc = argmax_node->GetOpDesc()->GetOutputDesc(0);
    ge::GeTensorDesc output_desc = fused_node->GetOpDesc()->GetInputDesc(0);
    auto ret = new_desc->AddInputDesc("x", input_desc);
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "CreateOneHotDNode AddInputDesc one fail."), return FAILED);
    ret = new_desc->AddOutputDesc("y", output_desc);
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "CreateOneHotDNode AddInputDesc one fail."), return FAILED);
    new_node = graph.AddNode(new_desc);
...
Status HardMaxPass::AddEdgeToOneHotDForOut(ge::NodePtr &fused_node,
    ge::NodePtr &one_hot_d_node) const
{
    for (auto in_data_anchor : fused_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
        auto ret = ge::GraphUtils::RemoveEdge(fused_node->GetOutDataAnchor(0), in_data_anchor);
        FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                          "AddEdgeToOneHotDForOut removeEdge fail"), return FAILED);
        auto res = ge::GraphUtils::AddEdge(one_hot_d_node->GetOutDataAnchor(0), in_data_anchor);
        FUSION_PASS_CHECK(res != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                          "AddEdgeToOneHotDForOut addEdge fail"), return FAILED);
    }
    return SUCCESS;
}
...
Status HardMaxPass::RemoveFusedNode(ge::ComputeGraph &graph, ge::NodePtr &fused_node) const
{
    ...
    FUSION_PASS_CHECK(graph.RemoveNode(fused_node) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "RemoveFusedNode error"), return FAILED);
    return SUCCESS;
}

Status HardMaxPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping,
    vector<ge::NodePtr> &fusion_nodes)
{
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "AddEdge to argmax_d_node fail"), return FAILED);
    ret = ge::GraphUtils::AddEdge(argmax_d_node->GetOutDataAnchor(0),
        one_hot_d_node->GetInDataAnchor(0));
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "AddEdge to one_hot_d_node fail"), return FAILED);
    ret = AddEdgeToOneHotDForOut(fused_node, one_hot_d_node);
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "AddEdgeToOneHotDForOut fail"), return FAILED);
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT("HardMaxPass",
                      "RemoveFusedNode fail"), return FAILED);
    return SUCCESS;
}
