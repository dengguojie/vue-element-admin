...
Status spaceToDepthAssistHelpFP16(const int32_t n, Dtype& output1,
    const vector<int64_t> spaceToDepthDInputDimInfo){
  ...
  for (int32_t windowIdx = 0; windowIdx < windowSize; windowIdx++) {
    for (int32_t channelIdx = 0; channelIdx < channelSize; channelIdx++) {
      ...
      OP_LOGI("spaceToDepthAssitHelpFP16",
             "=== Idx %d ===",
             fillAssistIdx);
    }
  }
...
vector<FusionPattern*> SpaceToDepthFusionPass::DefinePatterns() {
  ...
  FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new a pattern object failed."),
           return patterns);
  pattern->AddOpDesc(PATTERN_FUSEDNODE, {SPACE_TO_DEPTH_NODE})
      .SetOutput(PATTERN_FUSEDNODE);
  patterns.push_back(pattern);
  return patterns;
}

Status SpaceToDepthFusionPass::Fusion(ge::ComputeGraph &graph,
                                      Mapping &mapping,
  ...
  FUSION_PASS_CHECK(spaceToDepthNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(),
                   "spaceToDepthNode is null, fusion failed."),
           return PARAM_INVALID);
  ge::OpDescPtr spaceToDepthDesc = spaceToDepthNode->GetOpDesc();
  FUSION_PASS_CHECK(spaceToDepthDesc == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(),
                   "spaceToDepthDesc is null, fusion failed."),
           return PARAM_INVALID);
  std::string spaceToDepthName = spaceToDepthNode->GetName();
  // get attr block_size of fused node
  int32_t blockSize = -1;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetInt(spaceToDepthDesc, "block_size", blockSize),
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]: Cannot get attr:block_size from node.",
                   spaceToDepthName.c_str()),return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(spaceToDepthDesc->GetInputDesc(0).GetDataType() != DT_FLOAT16,
           OP_LOGE(FUSED_OP_TYPE.c_str(),
                   ...
                   "not fp16, cannot do fusion.", spaceToDepthName.c_str(),
                   spaceToDepthDesc->GetInputDesc(0).GetDataType()),
           return NOT_CHANGED);
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]: The OriginDataType of node's first input is %d,",
          spaceToDepthName.c_str(),
          spaceToDepthDesc->GetInputDesc(0).GetDataType());
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]: Get father node[Name:%s Type:%s]",
          spaceToDepthName.c_str(),
          sapceToDepthInNode->GetName().c_str(),
          sapceToDepthInNode->GetType().c_str());
  ...
  FUSION_PASS_CHECK(SUCCESS != PatternFusionUtil::ParseChannelIdx(spaceToDepthInput, inChannelIdx),
           OP_LOGE(FUSED_OP_TYPE.c_str(),
           ...
           return FAILED);
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]: The original format of node's input0 is %s.",
          spaceToDepthName.c_str(),
          ge::TypeUtils::FormatToSerialString(assitMatrixFormat).c_str());
  size_t inNumberIdx = -1;
  FUSION_PASS_CHECK(SUCCESS != ParseNumberIdx(spaceToDepthInput, inNumberIdx),
           OP_LOGE(FUSED_OP_TYPE.c_str(),
                   "Node[%s]: The original format of node's input0 is %s, which is unsupportable.",
                   spaceToDepthName.c_str(),
                   ge::TypeUtils::FormatToSerialString(assitMatrixFormat).c_str()),
  ...
  vector<int64_t> spaceToDepthDInputDimInfo = {spaceToDepthDChannel,
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]: Get first input size %d", spaceToDepthName.c_str(), spaceToDepthInputDimInfo[inNumberIdx]);
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]: Get first input size %d", spaceToDepthName.c_str(), spaceToDepthInputDimInfo[inChannelIdx]);
  ...
  FUSION_PASS_CHECK(inputAssit.get() == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]: inputAssit is NULL",
                                                         spaceToDepthName.c_str()), return PARAM_INVALID);

  Status ret = NnSet(destSize, UINT_NUM_ZERO, *reinterpret_cast<uint16_t*>(inputAssit.get()));
  FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]: NnSet failed.",
                                            spaceToDepthName.c_str()), return ret);

  ret = spaceToDepthAssistHelpFP16(destSize, *inputAssit.get(),
                                   spaceToDepthDInputDimInfo);
  FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(),  "Node[%s]: Generate assist matrix failed.",
                                            spaceToDepthName.c_str()), return ret);
