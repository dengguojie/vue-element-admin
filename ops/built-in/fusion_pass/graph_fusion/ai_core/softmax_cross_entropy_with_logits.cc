...
Status SoftmaxCrossEntropyWithLogitsPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_CHECK(softmax_cross_entropy_with_logits == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "softmax_cross_entropy_with_logits is null"), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(reshapeInDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshapeInDataAnchor is null"),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(reshape2OutDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshape2OutDataAnchor is null"),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(inputDataAnchor1 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "inputDataAnchor1 is null"),
                    return FAILED);

  auto inputDataAnchor2 = softmax_cross_entropy_with_logits->GetInDataAnchor(1);
  FUSION_PASS_CHECK(inputDataAnchor2 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "inputDataAnchor2 is null"),
                    return FAILED);

  auto outDataAnchor = softmax_cross_entropy_with_logits->GetOutDataAnchor(0);

  // delete reshape node
  FUSION_PASS_CHECK(RemoveNode(reshape, graph) == FAILED, OP_LOGE(FUSED_OP_TYPE.c_str(), "remove reshape node failed"),
                    return FAILED);
  FUSION_PASS_CHECK(RemoveNode(reshape_1, graph) == FAILED,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "remove reshape1 node failed"), return FAILED);
  FUSION_PASS_CHECK(RemoveNode(reshape_2, graph) == FAILED,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "remove reshape2 node failed"), return FAILED);
