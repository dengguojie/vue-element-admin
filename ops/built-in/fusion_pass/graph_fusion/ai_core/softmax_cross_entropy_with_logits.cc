...
*/
  Status SoftmaxCrossEntropyWithLogitsPass::RemoveNode(ge::NodePtr node, ge::ComputeGraph &graph) {
    ...
    for (size_t i = 0; i < node->GetAllInDataAnchors().size(); ++i) {
      ...
      FUSION_PASS_CHECK(preOutDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "preOutDataAnchor is null, remove node failed."), return FAILED);
...
Status SoftmaxCrossEntropyWithLogitsPass::Fusion(ge::ComputeGraph &graph,
                                                 Mapping &mapping,
                                                 vector<ge::NodePtr> &fusionNodes)
{
    ...
    ge::NodePtr softmax_cross_entropy_with_logits = GetNodeFromMapping(PATTERN_SOFTMAX_CROSS_ENTROPY_WITH_LOGITS, mapping);

    FUSION_PASS_CHECK(reshape == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshape is null"), return PARAM_INVALID);
    FUSION_PASS_CHECK(reshape_1 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshape_1 is null"), return PARAM_INVALID);
    FUSION_PASS_CHECK(reshape_2 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshape_2 is null"), return PARAM_INVALID);
    FUSION_PASS_CHECK(softmax_cross_entropy_with_logits == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "softmax_cross_entropy_with_logits is null"),
    ...
    if (first_input_tensor.GetShape().GetDims().size() != 4 or
        second_input_tensor.GetShape().GetDims().size() != 4) {
      OP_LOGI(FUSED_OP_TYPE.c_str(), "Input shape is not 4D, needn't to fusion, SoftmaxCrossEntropyWithLogitsPass fusion end");
      return SUCCESS;
    }
    ...
    if (first_output_tensor.GetShape().GetDims().size() != 4) {
      OP_LOGI(FUSED_OP_TYPE.c_str(), "Output shape is not 4D, cannot do fusion, SoftmaxCrossEntropyWithLogitsPass fussion end");
      return SUCCESS;
    }
    ...
    FUSION_PASS_CHECK(reshapeInDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshapeInDataAnchor is null"), return FAILED);
    ...
    FUSION_PASS_CHECK(reshape2OutDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshape2OutDataAnchor is null"), return FAILED);
    ...
    FUSION_PASS_CHECK(inputDataAnchor1 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "inputDataAnchor1 is null"), return FAILED);

    auto inputDataAnchor2 = softmax_cross_entropy_with_logits->GetInDataAnchor(1);
    FUSION_PASS_CHECK(inputDataAnchor2 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "inputDataAnchor2 is null"), return FAILED);

    auto outDataAnchor = softmax_cross_entropy_with_logits->GetOutDataAnchor(0);

    // delete reshape node
    FUSION_PASS_CHECK(RemoveNode(reshape, graph) == FAILED, OP_LOGE(FUSED_OP_TYPE.c_str(), "remove reshape node failed"), return FAILED);
    FUSION_PASS_CHECK(RemoveNode(reshape_1, graph) ==FAILED, OP_LOGE(FUSED_OP_TYPE.c_str(), "remove reshape1 node failed"), return FAILED);
    FUSION_PASS_CHECK(RemoveNode(reshape_2, graph) == FAILED, OP_LOGE(FUSED_OP_TYPE.c_str(), "remove reshape2 node failed"), return FAILED);

    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(reshapePeerOutDataAnchor, inputDataAnchor1) != ge::GRAPH_SUCCESS,
                             OP_LOGE(FUSED_OP_TYPE.c_str(), "add input edge 1 error"), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(reshape1PeerOutDataAnchor, inputDataAnchor2) != ge::GRAPH_SUCCESS,
                            OP_LOGE(FUSED_OP_TYPE.c_str(), "add input edge 2 error"), return FAILED);
    ...
    for (unsigned int i = 0; i < reshapePeerInDataAnchors.size(); ++i) {
      ...
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outDataAnchor, dstAnchor) != ge::GRAPH_SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "add output anchor Failed."),
               return FAILED);
    }
