...
vector<FusionPattern *> PaddDepthwiseConv2dFusionPass::DefinePatterns() {
  OP_LOGI(FUSED_OP_TYPE.c_str(),
          "Define PaddDepthwiseConv2dFusionPass pattern begin");
  vector<FusionPattern *> patterns;
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("PaddDepthwiseConv2dFusionPass");
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(),
          "Define PaddDepthwiseConv2dFusionPass pattern end");
  return patterns;
}

Status PaddDepthwiseConv2dFusionPass::Fusion(ge::ComputeGraph &graph,
                                             Mapping &mapping,
                                             vector<ge::NodePtr> &fusionNodes)
{
  OP_LOGI(FUSED_OP_TYPE.c_str(),
          "Define PaddDepthwiseConv2dFusionPass fusion begin");
  ge::NodePtr paddNode = GetNodeFromMapping(PATTERN_PADD, mapping);
  FUSION_PASS_CHECK(paddNode == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(),
                    "padD Node is null, fusion failed."),
                    return PARAM_INVALID);

  ge::NodePtr depthwiseConv2dNode = GetNodeFromMapping(PATTERN_DEPTHWISECONV2D,
                                                       mapping);
  FUSION_PASS_CHECK(depthwiseConv2dNode == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(),
                            "DepthwiseConv2D Node is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (paddingMode != "VALID") {
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "PaddDepthwiseConv2dFusion can only support VALID padding mode.");
    return NOT_CHANGED;
  }

  int64_t convCount = 0;
  for (auto peerInDataAnchor : paddNode->GetOutDataAnchor(
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGI(FUSED_OP_TYPE.c_str(), "Get paddNode paddings attr failed."),
      return NOT_CHANGED);
  ...
  if (paddings.size() < DIM_NUM4 || paddings[0].size() < DIRECTION_COUNT ||
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "The number of paddings not valid, can not fusion.");
    return NOT_CHANGED;
  }
  ...
  if (paddNode->GetOpDesc()->GetInputDesc(0).GetFormat() == ge::FORMAT_NCHW) {
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "Padd intput Format is not NCHW or NHWC, can not fusion.");
    return NOT_CHANGED;
  }
  ...
  if (paddingsT < 0 || paddingsT > 255 || paddingsB < 0 || paddingsB > 255 ||
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "Paddings value not in [0,255], can not fusion.");
    return NOT_CHANGED;
  }
    ge::NodePtr kernelNode =
            depthwiseConv2dNode->GetInDataAnchor(
                    0)->GetPeerOutAnchor()->GetOwnerNode();
  ...
  if (!paddNode->GetOutControlAnchor()->GetPeerInControlAnchors().empty()) {
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "PaddNode has control edge, can not fusion.");
    return NOT_CHANGED;
  }

  for (auto inDataAnchor :
          paddNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(inDataAnchor->GetOwnerNode()->GetOpDesc()->GetType() !=
                        DEPTHWISECONV2D,
                        OP_LOGI(FUSED_OP_TYPE.c_str(),
                        "Output node is not DepthwiseConv2D, can not fusion."),
                        return NOT_CHANGED);
  }
  ...
  for (ge::NodePtr nodePtr : nodeVector) {
    ...
    FUSION_PASS_CHECK(nodePtr->GetOpDesc()->UpdateInputDesc(
             ...
             OP_LOGE(FUSED_OP_TYPE.c_str(),
                     "Update %s input failed.", nodeName.c_str()),
             return FAILED);
    ...
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(
        ...
        nodePtr->GetInDataAnchor(0)) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(),
                     "Remove %s input0 edge error", nodeName.c_str()),
                     return FAILED);
    FUSION_PASS_CHECK(
            ge::GraphUtils::AddEdge(
                    paddNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                    nodePtr->GetInDataAnchor(0)) != SUCCESS,
                    OP_LOGE(FUSED_OP_TYPE.c_str(),
      ...
      return FAILED);
    FUSION_PASS_CHECK(!ge::AttrUtils::SetListInt(nodePtr->GetOpDesc(),
                                                 PADS, pads),
                      OP_LOGE(FUSED_OP_TYPE.c_str(),
                      "Set paddings to %s failed.", nodeName.c_str()),
                      return FAILED);
    FUSION_PASS_CHECK(!ge::AttrUtils::SetStr(nodePtr->GetOpDesc(),
                                             PADDING, "SAME"),
                      OP_LOGE(FUSED_OP_TYPE.c_str(),
                      "Set padding attr failed."),
                      return FAILED);
  }
  ...
  if (paddNode->GetOutControlAnchor()) {
    for (auto inControlAnchor :
      ...
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(
                            paddNode->GetOutControlAnchor(),
                            inControlAnchor) != SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(),
                       "Remove out control edge failed."),
                       return FAILED);
    }
  }
  FUSION_PASS_CHECK(graph.RemoveNode(paddNode) != SUCCESS,
                    OP_LOGE(FUSED_OP_TYPE.c_str(),
                    "Remove PadD node failed."), return FAILED);
  fusionNodes.push_back(depthwiseConv2dNode);

  OP_LOGI(FUSED_OP_TYPE.c_str(),
          "Define PaddDepthwiseConv2dFusionPass fusion end");
  return SUCCESS;
}
REGISTER_PASS("PaddDepthwiseConv2dFusionPass", BUILT_IN_GRAPH_PASS,
              PaddDepthwiseConv2dFusionPass);
}  // namespace fe
