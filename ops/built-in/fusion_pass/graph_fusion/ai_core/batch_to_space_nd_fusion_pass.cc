...
Status ConstToAttrBatchToSpaceNdPass::Fusion(ComputeGraph& graph, Mapping& mapping, vector<NodePtr>& fusionNodes) {
  ...
  // build a fusion node op desc, only for static
  OpDescPtr fusion_desc = PatternFusionUtil::GetFusionOpDesc(batch_node, fusion_optype, attr_infos);
  FUSION_PASS_CHECK(fusion_desc == nullptr, OP_LOGI(FUSED_OP_TYPE.c_str(), "op_type not changed to BatchToSpaceNDD."), return NOT_CHANGED);
  // check op support
  FUSION_PASS_CHECK(!CheckOpSupported(fusion_desc), OP_LOGI(FUSED_OP_TYPE.c_str(), "BatchToSpaceNDD not supported."),
                    return NOT_CHANGED);
  ...
  // check dynamic shape
  FUSION_PASS_CHECK(IsUnknownShape(input_shape.GetDims()), OP_LOGI(FUSED_OP_TYPE.c_str(), "BatchToSpaceND is dynamic."),
                    return NOT_CHANGED);
  ...
  if ((input_format != FORMAT_NHWC) && (input_format != FORMAT_NCHW) && (input_format != FORMAT_NDHWC) &&
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "BatchToSpaceND has input which format is not NHWC or NCHW or NDHWC or NCDHW, graph not changed, got "
            "format is %d.",
            input_format);
    return NOT_CHANGED;
  }
  ...
  if (input_format == FORMAT_NHWC) {
    if ((input_dim_num != 4) || (block_vec.size() != 2) || (crops_vec.size() != 4)) {
      if ((input_dim_num != 3) || (block_vec.size() != 1) || (crops_vec.size() != 2)) {
        OP_LOGI(FUSED_OP_TYPE.c_str(),
                "BatchToSpaceND has input with format 'NHWC' which does not meet the rules, graph not changed, got "
                "input dim num is %d, block size is %d, crops size is %d.",
                input_dim_num, block_vec.size(), crops_vec.size());
        return NOT_CHANGED;
      }
    }
  }
  ...
  if ((input_format == FORMAT_NCHW) && ((input_dim_num != 4) || (block_vec.size() != 3) || (crops_vec.size() != 6) ||
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "BatchToSpaceND has input with format 'NCHW' which does not meet the rules, graph not changed, got input "
            "dim num is %d, block size is %d, crops size is %d, block[0] is %d, crops[0] is %d, crops[1] is %d.",
            input_dim_num, block_vec.size(), crops_vec.size(), block_vec[0], crops_vec[0], crops_vec[1]);
    return NOT_CHANGED;
  }

  if ((input_format == FORMAT_NDHWC) && ((input_dim_num != 5) || (block_vec.size() != 3) || (crops_vec.size() != 6))) {
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "BatchToSpaceND has input with format 'NDHWC' which does not meet the rules, graph not changed, got "
            "input dim num is %d, block size is %d, crops size is %d.",
            input_dim_num, block_vec.size(), crops_vec.size());
    return NOT_CHANGED;
  }
  ...
  if ((input_format == FORMAT_NCDHW) && ((input_dim_num != 5) || (block_vec.size() != 4) || (crops_vec.size() != 8) ||
                                         (block_vec[0] != 1) || (crops_vec[0] != 0) || (crops_vec[1] != 0))) {
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "BatchToSpaceND has input with format 'NCDHW' which does not meet the rules, graph not changed, got input "
            "dim num is %d, block size is %d, crops size is %d, block[0] is %d, crops[0] is %d, crops[1] is %d.",
            input_dim_num, block_vec.size(), crops_vec.size(), block_vec[0], crops_vec[0], crops_vec[1]);
    return NOT_CHANGED;
  }
  ...
  if (ret != SUCCESS) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "BatchToSpaceND execute const to attr failed, graph not changed.");
    return NOT_CHANGED;
  }
