...
Status MatmulCastFusionPass::IsMatch(ge::NodePtr matmulNode, ge::NodePtr castNode) {
  ...
  if (ge::AttrUtils::HasAttr(matmulOp, MATMUL_DATATYPE_ATTR_KEY) &&
    ...
    FUSION_PASS_CHECK(!ge::AttrUtils::GetDataType(matmulOp, MATMUL_DATATYPE_ATTR_KEY,
                                         matmulDataType),
             OP_LOGD(FUSED_OP_TYPE.c_str(), "Fail to get attr T from matmul."),
             return FAILED);
    // get cast dest datatype
    ge::DataType castDesDataType = ge::DT_UNDEFINED;
    FUSION_PASS_CHECK(!ge::AttrUtils::GetDataType(castOp, CAST_DATATYPE_DES_ATTR_KEY,
                                         castDesDataType),
             OP_LOGD(FUSED_OP_TYPE.c_str(), "Fail to get attr DstT from cast."),
             return FAILED);
    if (matmulDataType != ge::DT_FLOAT16 ||
        castDesDataType != ge::DT_FLOAT) {
      OP_LOGD(FUSED_OP_TYPE.c_str(), "Matmul datatype is %u Cast data type is %u", matmulDataType,
              castDesDataType);
      return FAILED;
    }
  } else {
...
Status MatmulCastFusionPass::Fusion(ge::ComputeGraph &graph,
                                    Mapping &mapping,
  ...
  FUSION_PASS_CHECK(matmulNode == nullptr, OP_LOGW(FUSED_OP_TYPE.c_str(), "matmul node is null"),
           return FAILED);
  ...
  FUSION_PASS_CHECK(matmulNode->GetOpDesc()->UpdateOutputDesc(0, castOutputDesc)
               != ge::GRAPH_SUCCESS,
  ...
  OP_LOGD(FUSED_OP_TYPE.c_str(), "Node[%s] do Matmul + Cast fusion success!",
...
Status MatmulCastFusionPass::DoFusion(ge::NodePtr matmulNode) {
  ...
  FUSION_PASS_CHECK(tensorDesc == nullptr, OP_LOGD(FUSED_OP_TYPE.c_str(), "tensor desc is null"),
           return FAILED);
  return SUCCESS;
}
  ...
  if (outControlAnchor != nullptr) {
    for (ge::InControlAnchorPtr dstAnchor :
      ...
      if (ge::GraphUtils::RemoveEdge(outControlAnchor, dstAnchor) !=
                  ge::GRAPH_SUCCESS ||
              ge::GraphUtils::AddEdge(newNode->GetOutControlAnchor(),
                                      dstAnchor) != ge::GRAPH_SUCCESS) {
