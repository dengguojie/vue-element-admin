...
Status LayerNormTrainingFusionPass::RemoveSmalleNodes(ge::ComputeGraph& graph, const ge::NodePtr& mean_node1,
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(square_difference_node) != SUCCESS,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove node %s failed.", square_difference_node->GetName().c_str()),
                    return FAILED);
...
Status LayerNormTrainingFusionPass::AddTensorDescForLn(const ge::OpDescPtr& ln_opdesc, const ge::GeTensorDesc& x_tensor,
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Size of output of BatchNorm is %u.", ln_opdesc->GetOutputsSize());
  return SUCCESS;
}
...
Status LayerNormTrainingFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(square_difference_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "square_difference_node is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(square_difference_node->GetOpDesc()->GetInputsSize() < 2,
                    ...
                    return PARAM_INVALID);
  ge::OutDataAnchorPtr out_anchorof_x = square_difference_node->GetInDataAnchor(0)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(out_anchorof_x == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "out_anchorof_x must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr x_node = out_anchorof_x->GetOwnerNode();
  FUSION_PASS_CHECK(x_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "x_node must not be null"),
                    return PARAM_INVALID);
  ge::GeTensorDesc x_tensor = square_difference_node->GetOpDesc()->GetInputDesc(0);
  // output1
  ge::GeTensorDesc mean_out_tensor = square_difference_node->GetOpDesc()->GetInputDesc(1);
  ...
  FUSION_PASS_CHECK(input_node0 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "input_node0 must not be null"),
                    return PARAM_INVALID);
  /* output 2 */
  ge::GeTensorDesc varience_out_tensor = add_node1->GetOpDesc()->GetInputDesc(0);
  ...
  FUSION_PASS_CHECK(mul_node1->GetOpDesc()->GetInputsSize() < 2,
                    ...
                    return PARAM_INVALID);
  ge::OutDataAnchorPtr in_anchorof_mul = mul_node1->GetInDataAnchor(1)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(in_anchorof_mul == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "in_anchorof_mul must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr gamma_node = in_anchorof_mul->GetOwnerNode();
  FUSION_PASS_CHECK(gamma_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "gamma_node must not be null"),
                    return PARAM_INVALID);
  ge::GeTensorDesc gamma_tensor = mul_node1->GetOpDesc()->GetInputDesc(1);
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(out_anchorof_x, square_difference_node->GetInDataAnchor(0)) != SUCCESS,
