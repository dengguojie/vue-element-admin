...
static const std::string LAYER_NORM_OP_TYPE = "LayerNorm";
...
Status LayerNormTrainingFusionPass::AddTargetNode(FusionTurbo& turbo_instance,
  ...
  std::string layer_norm_name = mean_node_1->GetOpDesc()->GetName() + "/" + LAYER_NORM_OP_TYPE;
  layer_norm_node = turbo_instance.AddNodeOnly(layer_norm_name, LAYER_NORM_OP_TYPE);
  FUSION_PASS_CHECK(layer_norm_node == nullptr,
                    OP_LOGD(FUSED_OP_TYPE, "LayerNorm node is null, do nothing."), return NOT_CHANGED);

  std::string layer_norm_grad_name = rsqrt_grad_node->GetOpDesc()->GetName() + "/" + FUSED_OP_TYPE;
  layer_norm_grad_node = turbo_instance.AddNodeOnly(layer_norm_grad_name, FUSED_OP_TYPE);
