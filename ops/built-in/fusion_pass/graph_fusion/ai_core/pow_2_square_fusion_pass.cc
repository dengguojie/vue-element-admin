...
namespace fe{
  ...
  vector<FusionPattern*> Pow2SquareFusionPass::DefinePatterns(){
    ...
    pattern->AddOpDesc(PATTERN_POW, {POW})
    .SetOutput(PATTERN_POW);
    patterns.push_back(pattern);
    return patterns;
  }

  Status Pow2SquareFusionPass::Fusion(ge::ComputeGraph &graph,
                                      Mapping &mapping,
    ...
    FUSION_PASS_CHECK(pow_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(),"pow node is null, fusion failed."), return PARAM_INVALID);

    ge::OpDescPtr pow_desc = pow_node->GetOpDesc();
    FUSION_PASS_CHECK(pow_desc == nullptr,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "pow_node's Op_desc is null, fusion failed."),
    ...
    if(constData == 2.0 && constType2 == ge::DT_FLOAT && constSize2 == 4){
      ...
      FUSION_PASS_MAKE_SHARED((square_op = std::make_shared<ge::OpDesc>(pow_node->GetName() +
                     "/" + SQUARE, SQUARE)), return INTERNAL_ERROR);
      ...
      if (pow_node->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
        ...
        for (InDataAnchorPtr inAnchorPtr : pow_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
          ...
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(
                                square_node->GetOutDataAnchor(0),
                                inAnchorPtr),
                   OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's 2nd index to fusion "
                           "node:%s's 1st index failed.",
                           pow_node->GetName().c_str(),
                           square_node->GetName().c_str()),
                   return FAILED);
          OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's 2nd index to fusion node:%s's 1st "
                  "index.",
                  pow_node->GetName().c_str(),
                  square_node->GetName().c_str());
        }
      ...
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(pow_node->GetInDataAnchor(0)->GetPeerOutAnchor(),
               ...
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove pow node in data0 edge failed."), return FAILED);
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(pow_node->GetInDataAnchor(1)->GetPeerOutAnchor(),
               pow_node->GetInDataAnchor(1)),
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove pow node in data1 edge failed."), return FAILED);

      FUSION_PASS_CHECK(graph.RemoveNode(pow_node) != SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove pow node failed."), return FAILED);
      ...
      if(constNode2->GetAllOutDataAnchors().size() == 1){
        if(constNode2->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() == 0
          ...
          FUSION_PASS_CHECK(graph.RemoveNode(constNode2) != SUCCESS,
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove constant 2 node failed."), return FAILED);
        }
      }
      return SUCCESS;
    } else{
