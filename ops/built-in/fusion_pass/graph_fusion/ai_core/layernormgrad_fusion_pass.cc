...
Status LayerNormGradFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping, vector <ge::NodePtr> &fusionNodes) {
  ...
  for (size_t i = 0; i < layerNormXDesc->GetInputsSize(); i++) {
    vector <int64_t> dims = layerNormXDesc->GetInputDesc(i).GetShape().GetDims();
    if (dims.size() != 0) {
      for (size_t j = 0; j < dims.size(); j++) {
        auto dim = dims[j];
        if (PatternFusionUtil::IsUnknownShape(dim)) {
          is_unknown_shape = true;
          OP_LOGD(FUSED_OP_TYPE.c_str(), "LayerNormXBackpropV2 cannot be applied for unknown shape.");
          break;
        }
      }
    }
