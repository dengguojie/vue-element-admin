...
Status AddTransposeBeforeNode(const ge::NodePtr& fusedNode,
                                 const int64_t& inputIndex,
                                 const vector<int64_t>& permList,
  ...
  FUSION_PASS_CHECK(beforeTransposeDesc == nullptr, OP_LOGE(fuseNodeType.c_str(), "beforeTransposeDesc is null, fusion failed."),
           return FAILED);
...
Status AddTransposeAfterNode(const ge::NodePtr& fusedNode,
  ...
  FUSION_PASS_CHECK(afterTransposeDesc == nullptr, OP_LOGE(fuseNodeType.c_str(), "afterTransposeDesc is null, fusion failed."),
           return FAILED);
  ...
  for (auto inDataAnchor :
       fusedNode->GetOutDataAnchor(outputIndex)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(fusedNode->GetOutDataAnchor(outputIndex),
  ...
  // add input for transpose
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(fusedNode->GetOutDataAnchor(outputIndex),
...
template<typename T>
static std::vector<int64_t> GetConstIntValue(const uint8_t *const_data,
                                            size_t data_size) {
...
bool TbeFusionPassUtil::GetConstIntData(const ge::Tensor &data,
                                        ge::DataType data_type,
                                         std::vector<int64_t> &const_values) {
  using namespace std;
  using namespace std::placeholders;
  const std::map<DataType,
                 std::function<vector<int64_t>(const uint8_t *, size_t)>>
      type_call_map = {
...
bool TbeFusionPassUtil::GetConstIntData(const ge::Operator &op,
                                        const std::string &name,
                                        std::vector<int64_t> &values) {
  ge::Tensor tensor;
  if (GRAPH_SUCCESS != op.GetInputConstData(name, tensor)) {
    OP_LOGE("GetConstIntData", "GetInputConstData failed, op name is %s and input name is %s",
            op.GetName().c_str(),
            name.c_str());
    return false;
  }
