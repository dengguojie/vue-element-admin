...
Status RemapFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(img_input_shape.size() != 4, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "the input0 shape dim must be 4"),
  ...
  FUSION_PASS_CHECK(warpoffset_shape.size() != 4, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "the input1 shape dim must be 4"),
                    return PARAM_INVALID);
  // get output 1 shape
  ...
  FUSION_PASS_CHECK(output_desc_shape.size() != 4, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "the output shape dim must be 4"),
                    return PARAM_INVALID);
  // uint8 input
  int64_t ret = 0;
  if (img_input_desc.GetDataType() == DT_UINT8) {
    ...
    FUSION_PASS_CHECK(pre_cast1_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "pre_cast1_node is null."),
                      return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(pre_cast2_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "pre_cast2_node is null."),
                      return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(split_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "split_node is null."),
                    return PARAM_INVALID);

  // create floor_x node
  ge::NodePtr floor_x_node = nullptr;
  ret = CreateFloorxNode(floor_x_node, fused_node, graph, newNodes, temp_desc);
  FUSION_PASS_CHECK(floor_x_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "floor_x_node is null."),
                    return PARAM_INVALID);

  // create ceil_x node
  ge::NodePtr ceil_x_node = nullptr;
  ret = CreateCeilxNode(ceil_x_node, fused_node, graph, newNodes, temp_desc);
  FUSION_PASS_CHECK(ceil_x_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "ceil_x_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(floor_y_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "floor_y_node is null."),
                    return PARAM_INVALID);

  // create ceil_y node
  ge::NodePtr ceil_y_node = nullptr;
  ret = CreateCeilyNode(ceil_y_node, fused_node, graph, newNodes, temp_desc);
  FUSION_PASS_CHECK(ceil_y_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "ceil_y_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(cast1_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "cast1_node is null."),
                    return PARAM_INVALID);

  // create cast2 node
  ge::NodePtr cast2_node = nullptr;
  ret = CreateCastNode("cast2", cast2_node, fused_node, graph, newNodes, temp_desc);
  FUSION_PASS_CHECK(cast2_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "cast1_cast2_nodenode is null."),
                    return PARAM_INVALID);

  // create cast3 node
  ge::NodePtr cast3_node = nullptr;
  ret = CreateCastNode("cast3", cast3_node, fused_node, graph, newNodes, temp_desc);
  FUSION_PASS_CHECK(cast3_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "cast3_node is null."),
                    return PARAM_INVALID);

  // create cast4 node
  ge::NodePtr cast4_node = nullptr;
  ret = CreateCastNode("cast4", cast4_node, fused_node, graph, newNodes, temp_desc);
  FUSION_PASS_CHECK(cast4_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "cast4_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(mulsx1_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "mulsx1_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(mulsx2_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "mulsx2_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(mulsy1_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "mulsy1_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(mulsy2_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "mulsy2_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(add1_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add1_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(add2_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add2_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(add3_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add3_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(add4_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add4_node is null."),
                    return PARAM_INVALID);

  // create concat node
  ge::NodePtr concat_node = nullptr;
  ret = CreateConcatNode(concat_node, fused_node, graph, newNodes, temp_desc);
  FUSION_PASS_CHECK(concat_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "concat_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(remap_offsets_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "remap_offsets_node is null."),
                    return PARAM_INVALID);
  ...
  ret = CreateRemapResizeNode(remap_resize_node, fused_node, graph, newNodes, img_input_desc, warpoffset_input_desc,
                              output_desc);
  FUSION_PASS_CHECK(remap_resize_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "remap_resize_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "Add edge mulsy2_node to concat_node failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "Add edge img to remap_offsets_node failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "Add edge transpose to split node failed."),
                    return FAILED);
