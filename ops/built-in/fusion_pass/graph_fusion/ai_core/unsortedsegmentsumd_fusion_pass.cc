...
Status UnsortedSegmentSumdFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_CHECK(
      UnsortedSegmentSumdNode->GetInDataNodes().size() != 2,
      ...
      return NOT_CHANGED);
  FUSION_PASS_CHECK(
      UnsortedSegmentSumdNode->GetOutDataNodes().size() != 1,
  ...
  FUSION_PASS_CHECK(AicpuPad1to8fusedNode_desc == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "AicpuPad1to8fusedNode_desc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  UnsortedSegmentSumd8_desc = std::make_shared<ge::OpDesc>(
      UnsortedSegmentSumdNode->GetName() + "/" + "UnsortedSegmentSumd8", "UnsortedSegmentSumD");
  ...
  SlicefusedNode_desc = std::make_shared<ge::OpDesc>(UnsortedSegmentSumdNode->GetName() + "/" + "SliceLast", "SliceD");
  FUSION_PASS_CHECK(SlicefusedNode_desc == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "SlicefusedNode_desc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  // connect input edge
  FUSION_PASS_CHECK(
      ge::GraphUtils::AddEdge(UnsortedSegmentSumdNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                              AicpuPad1to8fused_node->GetInDataAnchor(0)) != SUCCESS,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
              UnsortedSegmentSumdNode->GetInDataAnchor(0)->GetPeerOutAnchor()->GetOwnerNode()->GetName().c_str(),
              AicpuPad1to8fused_node->GetName().c_str()),
      return FAILED);
  // connect input edge
  FUSION_PASS_CHECK(
      ge::GraphUtils::AddEdge(AicpuPad1to8fused_node->GetOutDataAnchor(0),
                              UnsortedSegmentSumd8_node->GetInDataAnchor(0)) != SUCCESS,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
              AicpuPad1to8fused_node->GetInDataAnchor(0)->GetPeerOutAnchor()->GetOwnerNode()->GetName().c_str(),
              UnsortedSegmentSumd8_node->GetName().c_str()),
      return FAILED);
  // connect input edge
  FUSION_PASS_CHECK(
      ge::GraphUtils::AddEdge(UnsortedSegmentSumdNode->GetInDataAnchor(1)->GetPeerOutAnchor(),
      ...
      return FAILED);
  FUSION_PASS_CHECK(
      ge::GraphUtils::AddEdge(UnsortedSegmentSumd8_node->GetOutDataAnchor(0),
                              SlicefusedNode_node->GetInDataAnchor(0)) != SUCCESS,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
              UnsortedSegmentSumd8_node->GetInDataAnchor(0)->GetPeerOutAnchor()->GetOwnerNode()->GetName().c_str(),
              SlicefusedNode_node->GetName().c_str()),
      return FAILED);

  // connect output edge
  for (auto inDataAnchor : UnsortedSegmentSumdNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(UnsortedSegmentSumdNode->GetOutDataAnchor(0), inDataAnchor) != SUCCESS,
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out data edge failed."), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(SlicefusedNode_node->GetOutDataAnchor(0), inDataAnchor) != SUCCESS,
