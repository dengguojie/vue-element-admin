...
vector<FusionPattern *> SoftmaxGradExtFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("SoftmaxGradExtFusionPass");
...
Status SoftmaxGradExtFusionPass::Fusion(
    ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes)
{
  ...
  newOpdesc =
      std::make_shared<ge::OpDesc>(mulGradNode->GetName() + "/" + SOFTMAXGRADEXT,
                                   SOFTMAXGRADEXT);
  ...
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor0) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input grad failed.", newOpName.c_str()), return FAILED);
  ...
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor1) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input x1 failed.", newOpName.c_str()), return FAILED);
  ...
  FUSION_PASS_CHECK(newOpdesc->AddInputDesc(input_tensor2) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input x2 failed.", newOpName.c_str()), return FAILED);
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
           ...
           return FAILED);
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
               mul1Node->GetInDataAnchor(0)->GetPeerOutAnchor(),
               newNode->GetInDataAnchor(2)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between node %s. and node %s failed.",
                   mul1Node->GetInDataAnchor(0)
                       ->GetPeerOutAnchor()
                       ->GetOwnerNode()
                       ->GetName()
                       .c_str(),
...
REGISTER_PASS("SoftmaxGradExtFusion", BUILT_IN_GRAPH_PASS,
