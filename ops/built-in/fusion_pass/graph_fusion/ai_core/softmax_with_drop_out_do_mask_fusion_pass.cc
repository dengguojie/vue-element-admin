...
Status SoftmaxWithDropOutDoMaskFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(softmaxNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "softmaxNode is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(dropoutNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "dropoutNode is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input mask failed.", newOpName.c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the output desc for the output y1 failed.", newOpName.c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the output desc for the output y2 failed.", newOpName.c_str()),
      return FAILED);
  ...
  for (auto &inDataAnchor : softmaxNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    ...
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(softmaxNode->GetOutDataAnchor(0), inDataAnchor) != SUCCESS,
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out data edge1 failed."), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(newNode->GetOutDataAnchor(0), inDataAnchor) != SUCCESS,
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out data edge1 failed."), return FAILED);
  }
  ...
  for (auto &inDataAnchor : dropoutNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(dropoutNode->GetOutDataAnchor(0), inDataAnchor) != SUCCESS,
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out data edge2 failed."), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(newNode->GetOutDataAnchor(1), inDataAnchor) != SUCCESS,
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out data edge2 failed."), return FAILED);
  }
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(softmaxNode) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove softmax node failed."),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(dropoutNode) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove dropout node failed."),
                    return FAILED);
