...
Status NormalizeFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(
    ...
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Node %s's Only Support 4 dim and 2 dim",
                                   fusedNode->GetName().c_str()),
    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.",
                                     normalizeSumDesc->GetName().c_str()),
      return PARAM_INVALID);
  FUSION_PASS_CHECK(
      normalizeScaleNode == nullptr,
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.",
                                     normalizeScaleDesc->GetName().c_str()),
      return PARAM_INVALID);
