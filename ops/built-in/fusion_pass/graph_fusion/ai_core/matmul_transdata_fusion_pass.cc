...
vector<FusionPattern*> MatMulTransdataFusionPass::DefinePatterns() {
  ...
  FusionPattern* pattern = new (std::nothrow) FusionPattern("MatmulTransdataFusionPass");
  FUSION_PASS_CHECK(
      pattern == nullptr,
      CUBE_CALL_ERR_REPORT(FUSED_OP_TYPE.c_str(), "new a pattern object failed."),
      return patterns);

  pattern->AddOpDesc(PATTERN_MATMUL, {OP_TYPE_MATMUL})
...
Status MatMulTransdataFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_CHECK(
    ...
    ge::CommonRuntimeErrLog(FUSED_OP_TYPE.c_str(), "right_mul_node is null, fusion failed."),
    return PARAM_INVALID
  );
  ge::NodePtr right_transdata_node = GetPeerOutNodeWithInDataAnchor(right_mul_node, 0);
  FUSION_PASS_CHECK(
    right_transdata_node == nullptr,
    ge::CommonRuntimeErrLog(FUSED_OP_TYPE.c_str(), "right_transdata_node is null, fusion failed."),
    return PARAM_INVALID
  );
  FUSION_PASS_CHECK(
    right_transdata_node->GetType() != OP_TYPE_TRANDATA,
    ge::CommonRuntimeErrLog(FUSED_OP_TYPE.c_str(), "input c is NZ, fusion failed"),
    return FAILED
  );
  ...
  FUSION_PASS_CHECK(
    ...
    ge::CommonRuntimeErrLog(FUSED_OP_TYPE.c_str(), "fail to add edge between left_transdata_node and left_mul_node"),
    return FAILED
  );
