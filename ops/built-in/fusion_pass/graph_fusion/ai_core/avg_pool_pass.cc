...
NodePtr AvgPoolFusionPass::AddMul(ge::ComputeGraph& graph, ge::NodePtr& avgPoolNode, ge::Format& inputOriginFormat) {
  ...
  if (dimMul.size() != 0) {
    if (inputOriginFormat == FORMAT_NHWC) {
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "inputOriginFormat only support NHWC and NCHW");
      return nullptr;
    }
  } else {
...
Status AvgPoolFusionPass::DoBiasOptimizeAvgpool(ge::ComputeGraph& graph, ge::NodePtr poolingNode,
  ...
  FUSION_PASS_CHECK(filter == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "filter is null ptr!"), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(PatternFusionUtil::SetWeightByIndex(poolingNode, biasPtr, 2, graph) != SUCCESS,
                    OP_LOGE(poolingNode->GetName().c_str(), "Fail to add bias const node for pooling node."),
                    return FAILED);
...
Status AvgPoolFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(avgPoolNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "avgPoolNode is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (preNode->GetOpDesc()->GetType() == "AscendQuant" || IsInt8) {
    ...
    if (HostcpuNode->GetType() == "RequantHostCpuOp") {
      ...
      FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "UpdateInputDesc0 failed."), return FAILED);
