...
vector<FusionPattern *> Conv2dbackpropFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("Conv2dbackpropFusionPass");
  FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new a pattern object failed."),
                 return patterns);
  pattern->AddOpDesc(PATTERN_CONV2DBACKPROPINPUTD, {CONV2DBACKPROPINPUTD})
          .SetOutput(PATTERN_CONV2DBACKPROPINPUTD);
  patterns.push_back(pattern);
  return patterns;
}

Status Conv2dbackpropFusionPass::IsMatch(ge::NodePtr &Conv2DBackpropInputNode,
                                         ge::NodePtr &Conv2DBackpropFilterNode,
                                         ge::NodePtr &Conv2DNode) {
  FUSION_PASS_CHECK(Conv2DBackpropInputNode->GetInAllNodes().size() != 2,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]'s output not equal to two, actually is [%zu], "
                   "match not success.",
                   Conv2DBackpropInputNode->GetName().c_str(),
                   Conv2DBackpropInputNode->GetInAllNodes().size()),
           return NOT_CHANGED);
  FUSION_PASS_CHECK(Conv2DBackpropInputNode->GetAllOutDataAnchors().size() != 1,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]'s output not equal to one, actually is [%zu], "
                   "match not success.",
                   Conv2DBackpropInputNode->GetName().c_str(),
                   Conv2DBackpropInputNode->GetAllOutDataAnchors().size()),
           return NOT_CHANGED);

  ge::NodePtr VariableNode = Conv2DBackpropInputNode->GetInAllNodes().at(0);
  FUSION_PASS_CHECK(VariableNode->GetType() != TRANSDATA &&
               VariableNode->GetType() != VARIABLE,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]'s type is not TransData or Variable, actually is "
                   "[%s], match not success.",
                   VariableNode->GetName().c_str(),
                   VariableNode->GetType().c_str()),
           return NOT_CHANGED);

  ge::NodePtr ConvBackFilterInNode =
      Conv2DBackpropInputNode->GetInAllNodes().at(1);
  for (auto BatchNormGradOutAnchor :
       ConvBackFilterInNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    if (BatchNormGradOutAnchor->GetOwnerNode()->GetType() ==
  ...
  FUSION_PASS_CHECK(Conv2DBackpropFilterNode->GetOpDesc() == nullptr,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Conv2DBackpropFilterDNode's OpDesc is null"),
           return NOT_CHANGED);
  FUSION_PASS_CHECK(Conv2DBackpropFilterNode->GetInAllNodes().size() != 2,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Node[%s]'s output not equal to two, actually is [%zu], "
                   "match not success.",
                   Conv2DBackpropFilterNode->GetName().c_str(),
                   Conv2DBackpropFilterNode->GetInAllNodes().size()),
           return NOT_CHANGED);
  FUSION_PASS_CHECK(Conv2DBackpropFilterNode->GetAllOutDataAnchors().size() != 1,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Node[%s]'s output not equal to one, actually is [%zu], "
                   "match not success.",
                   Conv2DBackpropFilterNode->GetName().c_str(),
                   Conv2DBackpropFilterNode->GetAllOutDataAnchors().size()),
           return NOT_CHANGED);
  ...
  for (auto ReluOutAnchor :
       ConvInNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
  ...
  FUSION_PASS_CHECK(Conv2DNode->GetOpDesc() == nullptr,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Conv2DNode's OpDesc is null"), return FAILED);
  FUSION_PASS_CHECK(Conv2DNode->GetInAllNodes().size() < 2,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Node[%s]'s output less than two, actually is [%zu], match "
                   "not success.",
                   Conv2DNode->GetName().c_str(),
                   Conv2DNode->GetInAllNodes().size()),
           return NOT_CHANGED);
  FUSION_PASS_CHECK(Conv2DNode->GetAllOutDataAnchors().size() != 1,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Node[%s]'s output not equal to one, actually is [%zu], "
                   "match not success.",
                   Conv2DNode->GetName().c_str(),
                   Conv2DNode->GetAllOutDataAnchors().size()),
           return NOT_CHANGED);
  return SUCCESS;
}
...
Status Conv2dbackpropFusionPass::CheckValidation(
    ge::OpDescPtr Conv2DDesc, ge::OpDescPtr Conv2DBackpropInputDesc,
  ...
  FUSION_PASS_CHECK(GetGroups(Conv2DDesc, Conv2DGroups) != SUCCESS,
           ...
           return FAILED);
  FUSION_PASS_CHECK(GetGroups(Conv2DBackpropInputDesc, Conv2DBackpropInputGroups) !=
               SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]'s group[%ld] is out of range, fusion failed.",
                   Conv2DBackpropInputDesc->GetName().c_str(),
                   Conv2DBackpropInputGroups),
           return FAILED);
  FUSION_PASS_CHECK(GetGroups(Conv2DBackpropFilterDesc, Conv2DBackpropFilterGroups) !=
               SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]'s group[%ld] is out of range, fusion failed.",
                   Conv2DBackpropFilterDesc->GetName().c_str(),
                   Conv2DBackpropFilterGroups),
           return FAILED);
  // group should be equal in conv2d, convbackpropInput and convbackpropFilter
  FUSION_PASS_CHECK(!(Conv2DGroups == Conv2DBackpropInputGroups &&
             Conv2DGroups == Conv2DBackpropFilterGroups),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Group is not equal in node[%s]:[%ld], node[%s]:[%ld] and "
...
Status Conv2dbackpropFusionPass::ParseConvNodeChannelIdx(ge::GeTensorDesc& ConvTensordesc,
...
Status Conv2dbackpropFusionPass::ParseConvNodeNumberIdx(ge::GeTensorDesc& ConvTensordesc,
                                     size_t &ConvNumberIdx) {
  // Get node's number dim idex, only support NCHW, HWCN and NHWC
  ge::Format Conv2DTensorFormat = ConvTensordesc.GetFormat();
  if (Conv2DTensorFormat == ge::FORMAT_NCHW ||
      Conv2DTensorFormat == ge::FORMAT_NHWC) {
    ConvNumberIdx = 0;
...
Status Conv2dbackpropFusionPass::ParseConvNodeChannel(ge::GeTensorDesc& ConvTensordesc,
                                   int64_t &ConvChannel) {
...
bool Conv2dbackpropFusionPass::GenerateSplitNode(ge::ComputeGraph &graph, ge::OpDescPtr srcDesc,
  ...
  splitOutDesc.Update(splitOutShape, inputDesc.GetFormat(),
...
bool Conv2dbackpropFusionPass::GenerateNewConvNodes(ge::ComputeGraph &graph, ge::OpDescPtr srcDesc,
                                 ...
                                 int64_t groups, size_t dimIdx,
                                 uint32_t anchorIdx) {
  // set new convNodes' 1st input from split
  ge::GeTensorDesc inDesc = splitOutDesc;
  inDesc.SetOriginDataType(
      srcDesc->GetInputDesc(anchorIdx).GetOriginDataType());
  ...
  FUSION_PASS_CHECK(newConvOutChn % groups != 0,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]'s dim size[%ld] divide groups[%ld] != 0",
                   srcDesc->GetName().c_str(), newConvOutShape.GetDim(dimIdx),
                   groups),
           return false);
  ...
  newConvOutDesc.SetOriginDataType(
      srcDesc->GetOutputDesc(0).GetOriginDataType());
  ...
  for (int64_t i = 0; i < groups; i++) {
    ...
    FUSION_PASS_CHECK(ge::GRAPH_SUCCESS !=
                 newConvDesc->UpdateOutputDesc(0, newConvOutDesc),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Update node:%s's 1st output failed.",
                     srcDesc->GetName().c_str()),
             return false);
...
bool Conv2dbackpropFusionPass::GenerateConcatNode(ge::ComputeGraph &graph, ge::OpDescPtr srcDesc,
  ...
  FUSION_PASS_MAKE_SHARED((concatDesc = std::make_shared<ge::OpDesc>(
                      srcDesc->GetName() + "_concat", "ConcatD")),
                 return false);
  for (int i = 0; i < groups; i++) {
...
bool Conv2dbackpropFusionPass::GenerateNewNodes(ge::ComputeGraph &graph, ge::OpDescPtr srcDesc,
  ...
  if (srcDesc->GetType() != "CONV2DBACKPROPFILTERD") {
    FUSION_PASS_CHECK(!GenerateSplitNode(graph, srcDesc, Conv2DGroups, splitNode,
                                splitOutDesc, dimChannelIdx, anchorIdx),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "generate split node before node[%s] failed.",
                     srcDesc->GetName().c_str()),
             return false);
  }
  splitOutDesc = splitNode->GetOpDesc()->GetOutputDesc(0);
  FUSION_PASS_CHECK(!GenerateNewConvNodes(graph, srcDesc, splitOutDesc, newConvNodes,
                                 newConvOutDesc, Conv2DGroups, dimChannelIdx,
                                 anchorIdx),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "generate new conv nodes for node[%s] failed.",
                   srcDesc->GetName().c_str()),
           return false);
  FUSION_PASS_CHECK(!GenerateConcatNode(graph, srcDesc, Conv2DGroups, newConvOutDesc,
                               concatNode, dimChannelIdx),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "generate concat node after node[%s] failed.",
                   srcDesc->GetName().c_str()),
           return false);
  return true;
}

bool Conv2dbackpropFusionPass::Relink(ge::NodePtr srcNode, ge::NodePtr splitNode,
  ...
  for (size_t i = 0; i < convOutNodes.size(); i++) {
    outAnchorIndexes.push_back(
        srcNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().at(i)->GetIdx());
  }
  ...
  for (int i = 0; i < Conv2DGroups; i++) {
    ...
    FUSION_PASS_CHECK(ge::GRAPH_SUCCESS !=
                 ge::GraphUtils::AddEdge(newConvNodes[i]->GetOutDataAnchor(0),
...
bool Conv2dbackpropFusionPass::RemoveOldNode(ge::ComputeGraph &graph, ge::NodePtr srcNode) {
  ...
  FUSION_PASS_CHECK(SUCCESS != graph.RemoveNode(srcNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "RemoveNode %s failed.", srcNode->GetName().c_str()),
           return false);
  return true;
}
// set conv and convBackprop nodes to depthwiseConv and depthwiseConvBackprop
Status Conv2dbackpropFusionPass::ProcessDepthwiseConv(
    ge::OpDescPtr &Conv2DDesc, ge::OpDescPtr &Conv2DBackpropInputDesc,
    ge::OpDescPtr &Conv2DBackpropFilterDesc, int64_t Conv2DGroups) {
...
Status Conv2dbackpropFusionPass::ProcessGroupConvFusion(
    ge::ComputeGraph &graph, ge::NodePtr Conv2DNode,
    ge::NodePtr Conv2DBackpropInputNode, ge::NodePtr Conv2DBackpropFilterNode,
  ...
  ge::OpDescPtr Conv2DBackpropFilterDesc =
      Conv2DBackpropFilterNode->GetOpDesc();
  ge::NodePtr splitNode;
  ...
  FUSION_PASS_CHECK(ParseConvNodeChannelIdx(convInputDesc0,
                                   dimChannelIdx) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Get node[%s]'s channel index failed.",
                   Conv2DNode->GetName().c_str()),
           return FAILED);
  FUSION_PASS_CHECK(ParseConvNodeNumberIdx(convInputDesc1, dimNumberIdx) !=
               SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Get node[%s]'s number index failed.",
                   Conv2DNode->GetName().c_str()),
           return FAILED);
  FUSION_PASS_CHECK(!GenerateNewNodes(graph, Conv2DDesc, splitNode, newConvNodes,
                             concatNode, Conv2DGroups, dimChannelIdx, 0),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Generate new nodes for node[%s] failed.",
                   Conv2DDesc->GetName().c_str()),
           return FAILED);
  FUSION_PASS_CHECK(!GenerateNewNodes(graph, Conv2DBackpropInputDesc, splitNode1,
                             newConvNodes1, concatNode1, Conv2DGroups,
                             dimChannelIdx, 1),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Generate new nodes for node[%s] failed.",
                   Conv2DBackpropInputDesc->GetName().c_str()),
           return FAILED);
  FUSION_PASS_CHECK(!GenerateNewNodes(graph, Conv2DBackpropFilterDesc, splitNode1,
                             newConvNodes2, concatNode2, Conv2DGroups,
                             dimChannelIdx, 0),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Generate new nodes for node[%s] failed.",
                   Conv2DBackpropFilterDesc->GetName().c_str()),
           return FAILED);
  FUSION_PASS_CHECK(
      !Relink(Conv2DNode, splitNode, newConvNodes, concatNode, Conv2DGroups, 0),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "relink fail"), return FAILED);
  FUSION_PASS_CHECK(!Relink(Conv2DBackpropInputNode, splitNode1, newConvNodes1,
                   concatNode1, Conv2DGroups, 1),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "relink fail"), return FAILED);
  FUSION_PASS_CHECK(!Relink(Conv2DBackpropFilterNode, splitNode1, newConvNodes2,
                   concatNode2, Conv2DGroups, 1),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "relink fail"), return FAILED);
  // link conv's input node to split node
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS !=
               ge::GraphUtils::AddEdge(
                   Conv2DNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                   splitNode->GetInDataAnchor(0)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add data to slice edge fail"), return false);
  FUSION_PASS_CHECK(
      ge::GRAPH_SUCCESS !=
          ge::GraphUtils::AddEdge(
              Conv2DBackpropInputNode->GetInDataAnchor(1)->GetPeerOutAnchor(),
              splitNode1->GetInDataAnchor(0)),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "add data to slice edge fail"), return false);
  for (int i = 0; i < Conv2DGroups; i++) {
    FUSION_PASS_CHECK(ge::GRAPH_SUCCESS !=
                 ge::GraphUtils::AddEdge(splitNode->GetOutDataAnchor(i),
                                         newConvNodes2[i]->GetInDataAnchor(0)),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "add slice to conv edge fail"), return false);
  }
  FUSION_PASS_CHECK(PatternFusionUtil::InsertSliceDNodes(graph, Conv2DNode, 1, newConvNodes, Conv2DGroups,
                        dimNumberIdx) != SUCCESS,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Insert SliceD node between node[%s] and weight node falied.",
              Conv2DNode->GetName().c_str()),
      return FAILED);
  if (Conv2DNode->GetAllInAnchors().size() > 2) {
    FUSION_PASS_CHECK(PatternFusionUtil::InsertSliceDNodes(graph, Conv2DNode, 2, newConvNodes, Conv2DGroups,
                          0) != SUCCESS,
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Insert SliceD node between node[%s] and bias node falied.",
                Conv2DNode->GetName().c_str()),
        return FAILED);
  }
  FUSION_PASS_CHECK(PatternFusionUtil::InsertSliceDNodes(graph, Conv2DBackpropInputNode, 0, newConvNodes1,
                        Conv2DGroups, dimNumberIdx) != SUCCESS,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Insert SliceD node between node[%s] and weight node falied.",
              Conv2DNode->GetName().c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(!RemoveOldNode(graph, Conv2DBackpropInputNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "RemoveNode %s failed.",
                   Conv2DBackpropInputNode->GetName().c_str()),
           return FAILED);
  FUSION_PASS_CHECK(!RemoveOldNode(graph, Conv2DBackpropFilterNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "RemoveNode %s failed.",
                   Conv2DBackpropFilterNode->GetName().c_str()),
           return FAILED);
  return SUCCESS;
...
Status Conv2dbackpropFusionPass::Fusion(ge::ComputeGraph &graph,
                                        Mapping &mapping,
                                        vector<ge::NodePtr> &newNodes) {
  ge::NodePtr Conv2DBackpropInputNode =
      GetNodeFromMapping(PATTERN_CONV2DBACKPROPINPUTD, mapping);
  ...
  if (IsMatch(Conv2DBackpropInputNode, Conv2DBackpropFilterNode, Conv2DNode) !=
      SUCCESS) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "Conv2D + Conv2DBackpropFilterD + Conv2DBackpropInputD fusion "
  ...
  ge::OpDescPtr Conv2DBackpropFilterDesc =
      Conv2DBackpropFilterNode->GetOpDesc();
  FUSION_PASS_CHECK(
      Conv2DBackpropFilterDesc == nullptr,
  ...
  FUSION_PASS_CHECK(CheckValidation(Conv2DDesc, Conv2DBackpropInputDesc,
                           Conv2DBackpropFilterDesc, Conv2DGroups) != SUCCESS,
  ...
  if (Conv2DGroups == 1) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "Group is one in node[%s], node[%s] and node[%s], no need to do "
            "fusion.",
            Conv2DDesc->GetName().c_str(),
            Conv2DBackpropInputDesc->GetName().c_str(),
            Conv2DBackpropFilterDesc->GetName().c_str());
  ...
  if (ParseConvNodeChannel(Conv2DInanchor0, Conv2DInputChannel) != SUCCESS ||
    ...
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parse node[%s]'s channel info not success.",
            Conv2DDesc->GetName().c_str());
    return FAILED;
  }
  // if conv's cin = cout, replace conv and convBackprop nodes to depthwise
  if (Conv2DGroups == Conv2DOutputChannel &&
      Conv2DGroups == Conv2DInputChannel) {
    OP_LOGI(FUSED_OP_TYPE.c_str(),
        "Node[%s]'s InputChannel[%ld], OutputChannel[%ld] and group is [%ld]",
        Conv2DDesc->GetName().c_str(), Conv2DInputChannel, Conv2DOutputChannel,
        Conv2DGroups);
    ProcessDepthwiseConv(Conv2DDesc, Conv2DBackpropInputDesc,
                         Conv2DBackpropFilterDesc, Conv2DGroups);
  } else if (Conv2DInputChannel % Conv2DGroups == 0 &&
             Conv2DOutputChannel % Conv2DGroups == 0) {
    OP_LOGI(FUSED_OP_TYPE.c_str(),
        "Node[%s]'s InputChannel[%ld], OutputChannel[%ld] and group is [%ld]",
        Conv2DDesc->GetName().c_str(), Conv2DInputChannel, Conv2DOutputChannel,
        Conv2DGroups);
    // split conv and convBackprop nodes into groups
    ProcessGroupConvFusion(graph, Conv2DNode, Conv2DBackpropInputNode,
                           Conv2DBackpropFilterNode, Conv2DGroups);
  } else {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "InputChannel[%ld] or OutputChannel[%ld] divide group[%ld] not "
            "equal to zero",
            Conv2DInputChannel, Conv2DOutputChannel, Conv2DGroups);
    return FAILED;
  }
  OP_LOGI(FUSED_OP_TYPE.c_str(),
      "Conv2D + Conv2DBackpropFilterD + Conv2DBackpropInputD fusion success!");
  return SUCCESS;
}
REGISTER_PASS("AConv2dbackpropFusionPass",
              BUILT_IN_GRAPH_PASS, Conv2dbackpropFusionPass);
} // namespace fe
