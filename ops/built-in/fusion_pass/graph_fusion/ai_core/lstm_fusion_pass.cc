...
ge::GeTensorDesc ALSTMFusionPass::ProcessStatic(ge::NodePtr fusedNode ,int32_t num_output,ge::NodePtr &innerproductNode,ge::NodePtr &dequantNode,ge::ComputeGraph &graph,vector<ge::NodePtr> &newNodes,bool &failStatus, int32_t xStaticIndex, int32_t wxStaticIndex)
{
    ...
    ge::OpDescPtr innerProductStaticDesc = std::make_shared<ge::OpDesc>(fusedDesc->GetName() + "/FullyConnection", "FullyConnection");
    ...
    fusedNode->GetInDataAnchor(wxStaticIndex)->GetPeerOutAnchor()->GetOwnerNode()->GetOpDesc()->UpdateOutputDesc(0, inputWTensorDesc);
    ...
    FUSION_PASS_CHECK(innerproductNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", innerProductStaticDesc->GetName().c_str()), failStatus=true);
    newNodes.push_back(innerproductNode);
    if(dataType == ge::DT_INT8){

           // create the OpDescPtr for AscendDequant
           ge::OpDescPtr dequantStaticDesc1 = std::make_shared<ge::OpDesc>(fusedDesc->GetName() + "/AscendDequant", "AscendDequant");
           ...
           FUSION_PASS_CHECK(dequantNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", dequantStaticDesc1->GetName().c_str()), failStatus=true);
...
ge::GeTensorPtr ALSTMFusionPass::ProcessWxh(ge::NodePtr fusedNode,bool &failStatus ,int32_t &wxIndex, int32_t &whIndex, int32_t c0Index)
{
    string opType=GetPeerNodeOptype(fusedNode);
    ...
    fusedNode->GetInDataAnchor(wxIndex)->GetPeerOutAnchor()->GetOwnerNode()->GetOpDesc()->UpdateOutputDesc(0, wxhTensorDesc);
    ...
    if(dataType == ge::DT_FLOAT16||dataType == ge::DT_FLOAT){
        ...
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Node:%s's dtype is not in (float16, int8), fusion failed.", fusedDesc->GetName().c_str());
            failStatus=true;
     }
...
vector<ge::NodePtr> ALSTMFusionPass::ProcessLstmCellV2(ge::NodePtr fusedNode,ge::ComputeGraph &graph,ge::GeTensorDesc &outInnerProductTensorDesc,const ge::GeTensorDesc &wxhTensorDesc,const ge::GeTensorDesc &hTensorDesc,vector<ge::NodePtr> &newNodes,bool &failStatus, int32_t biasIndex, bool has_static)
{
    ...
    fusedNode->GetInDataAnchor(biasIndex)->GetPeerOutAnchor()->GetOwnerNode()->GetOpDesc()->UpdateOutputDesc(0, biasTensorDesc);
    ...
    ge::GeTensorDesc xInputTensorDesc=ge::GeTensorDesc(shape_input_1, ge::FORMAT_FRACTAL_NZ, inputTensorDesc0.GetDataType());
    ...
    for (int64_t i=0 ;i<fusedDesc->GetInputDesc(0).GetShape().GetDim(0);i++) {
         ...
         ge::OpDescPtr basicLSTMDesc2 = std::make_shared<ge::OpDesc>(fusedDesc->GetName() + "/BasicLSTMCellV2"+std::to_string(i), "BasicLSTMCellV2");
         ...
         if(i==0&&expose_hidden){
             ge::GeTensorDesc hTensorInput=ge::GeTensorDesc(hTensorDesc.GetShape(), hTensorDesc.GetFormat(), hTensorDesc.GetDataType());
             hTensorInput.SetOriginShape(inputOriShape);
             ge::GeTensorDesc cTensorInput=ge::GeTensorDesc(cTensorDesc.GetShape(), cTensorDesc.GetFormat(), cTensorDesc.GetDataType());
         ...
         if(i == tSize){
             ...
             ge::GeTensorDesc hTensorCur=ge::GeTensorDesc(hTensorDesc.GetShape(), hTensorDesc.GetFormat(), hTensorDesc.GetDataType());
         ...
         FUSION_PASS_CHECK(basicLSTMNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", basicLSTMDesc2->GetName().c_str()), failStatus=true);
...
Status ALSTMFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes) {
      ...
      if(has_static){
           outInnerProductTensorDesc=ProcessStatic(fusedNode,num_output,innerproductNode,dequantNode,graph,newNodes,failStatus, xStaticIndex, wxStaticIndex);
           FUSION_PASS_CHECK(failStatus, OP_LOGE(FUSED_OP_TYPE.c_str(), "ProcessStatic:check failed, fusion failed."), return PARAM_INVALID);
       }
      ...
      ge::GeTensorDesc splitNode1Desc = ge::GeTensorDesc(inputTensorDesc0.GetShape(), inputTensorDesc0.GetFormat(), startType);
      ...
      FUSION_PASS_CHECK(splitXNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", spiltStaticDesc->GetName().c_str()), return PARAM_INVALID);
      ...
      ge::GeTensorDesc splitInputDesc=ge::GeTensorDesc(inputContTensorDesc.GetShape(), inputContTensorDesc.GetFormat(), ge::DT_FLOAT16);
      ...
      FUSION_PASS_CHECK(splitContNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", spiltContStaticDesc->GetName().c_str()), return PARAM_INVALID);
           newNodes.push_back(splitContNode);

      // process w_xh
      ge::GeTensorPtr wxTensorPtr= ProcessWxh(fusedNode,failStatus,wxIndex, whIndex, c0Index);
      FUSION_PASS_CHECK(failStatus, OP_LOGE(FUSED_OP_TYPE.c_str(), "ProcessWxh:check failed, fusion failed."), return FAILED);
      ...
      vector<ge::NodePtr> lstmCellV2Node=ProcessLstmCellV2(fusedNode,graph,outInnerProductTensorDesc,wxTensorPtr->GetTensorDesc(),hTensorDesc,newNodes,failStatus, biasIndex, has_static);
      ...
      FUSION_PASS_CHECK(concatNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", concatStaticDesc->GetName().c_str()), return PARAM_INVALID);
     ...
     if(has_static){
           // connect input0
           FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(xStaticIndex)->GetPeerOutAnchor(),
                                                            innerproductNode->GetInDataAnchor(0)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0, innerproductNode->GetName().c_str(), 0),
                        return FAILED);

           FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(wxStaticIndex)->GetPeerOutAnchor(),
                                                                       innerproductNode->GetInDataAnchor(1)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0, innerproductNode->GetName().c_str(), 0),
                                   return FAILED);
           OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 0, innerproductNode->GetName().c_str(), 0);


           if(opType=="AscendQuant"){
               FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(innerproductNode->GetOutDataAnchor(0),
                                                                  dequantNode->GetInDataAnchor(0)),
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", innerproductNode->GetName().c_str(), 0, dequantNode->GetName().c_str(), 0),
                              return FAILED);
               ...
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0, dequantNode->GetName().c_str(), 0),
                              return FAILED);
               OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 0, dequantNode->GetName().c_str(), 0);
           }
     }

     FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                                                              splitXNode->GetInDataAnchor(0)),
     OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0, splitXNode->GetName().c_str(), 0),
                          return FAILED);
     OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 0, splitXNode->GetName().c_str(), 0);

     FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(1)->GetPeerOutAnchor(),
                                                              splitContNode->GetInDataAnchor(0)),
     OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 1, splitContNode->GetName().c_str(), 0),
                          return FAILED);
     OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 1, splitContNode->GetName().c_str(), 0);

      // connect lstmcellv2
      for (int64_t i=0;i<numSplitX;i++) {

            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(splitXNode->GetOutDataAnchor(i),
                                                        lstmCellV2Node[i]->GetInDataAnchor(0)),
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", splitXNode->GetName().c_str(), 0,lstmCellV2Node[i]->GetName().c_str(), 0),
                    return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", splitXNode->GetName().c_str(), 0, lstmCellV2Node[i]->GetName().c_str(), 0);

            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(splitContNode->GetOutDataAnchor(i),
                                                          lstmCellV2Node[i]->GetInDataAnchor(1)),
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", splitContNode->GetName().c_str(), i,lstmCellV2Node[i]->GetName().c_str(), 0),
                      return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", splitContNode->GetName().c_str(), i, lstmCellV2Node[i]->GetName().c_str(), 0);
            ...
            if(has_static){
                if(opType=="AscendQuant"){
                    ...
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", dequantNode->GetName().c_str(), 0,lstmCellV2Node[i]->GetName().c_str(), 0),
                                             return FAILED);
                    OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", dequantNode->GetName().c_str(), 0, lstmCellV2Node[i]->GetName().c_str(), 0);
                }
                else{
                    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(innerproductNode->GetOutDataAnchor(0),
                                                                     lstmCellV2Node[i]->GetInDataAnchor(index)),
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", innerproductNode->GetName().c_str(), 0,lstmCellV2Node[i]->GetName().c_str(), 0),
                                 return FAILED);
                    OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", innerproductNode->GetName().c_str(), 0, lstmCellV2Node[i]->GetName().c_str(), 0);
                }
                index=index+1;
            }
            ...
            if(i==0&&expose_hidden){
                  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(h0Index)->GetPeerOutAnchor(),
                                                              lstmCellV2Node[i]->GetInDataAnchor(index)),
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0,lstmCellV2Node[i]->GetName().c_str(), 0),
                          return FAILED);
                  OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 0, lstmCellV2Node[i]->GetName().c_str(), 0);
                  index=index+1;
                ...
                FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(lstmCellV2Node[i-1]->GetOutDataAnchor(0),
                                                        lstmCellV2Node[i]->GetInDataAnchor(index)),
                OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", lstmCellV2Node[i-1]->GetName().c_str(), 0,lstmCellV2Node[i]->GetName().c_str(), 0),
                    return FAILED);
                OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", lstmCellV2Node[i-1]->GetName().c_str(), 0, lstmCellV2Node[i]->GetName().c_str(), 0);
                index=index+1;
           ...
           FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(wxIndex)->GetPeerOutAnchor(),
                                                   lstmCellV2Node[i]->GetInDataAnchor(index)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0, lstmCellV2Node[i]->GetName().c_str(), 1),
              return FAILED);
           OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 0, lstmCellV2Node[i]->GetName().c_str(), 1);
           index=index+1;

           // concat node
           FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(lstmCellV2Node[i]->GetOutDataAnchor(0),
                                                        concatNode->GetInDataAnchor(i)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", lstmCellV2Node[i]->GetName().c_str(), 0,concatNode->GetName().c_str(), 0),
                    return FAILED);
           OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", concatNode->GetName().c_str(), 0, concatNode->GetName().c_str(), 0);

           // bias
           FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(biasIndex)->GetPeerOutAnchor(),
                                                              lstmCellV2Node[i]->GetInDataAnchor(index)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", lstmCellV2Node[i]->GetName().c_str(), 0,fusedNode->GetName().c_str(), 0),
                          return FAILED);
           OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 0, fusedNode->GetName().c_str(), 0);
           index=index+1;
     ...
     if (fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
        OP_LOGI(FUSED_OP_TYPE.c_str(), "The fanout size of lstmNode is [%d].", fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size());
        for (InDataAnchorPtr inAnchorPtr : fusedNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
            ...
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(concatNode->GetOutDataAnchor(0), inAnchorPtr),
                OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0] failed.", concatNode->GetName().c_str(), fusedNode->GetName().c_str()),
                return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0].", concatNode->GetName().c_str(), fusedNode->GetName().c_str());
        }
     }

     if (fusedNode->GetOutDataAnchor(1)->GetPeerInDataAnchors().size() > 0) {
        OP_LOGI(FUSED_OP_TYPE.c_str(), "The fanout size of lstmNode is [%d].", fusedNode->GetOutDataAnchor(1)->GetPeerInDataAnchors().size());
        for (InDataAnchorPtr inAnchorPtr : fusedNode->GetOutDataAnchor(1)->GetPeerInDataAnchors()) {
            inAnchorPtr->UnlinkAll();
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(lstmCellV2Node[numSplitX-1]->GetOutDataAnchor(0), inAnchorPtr),
                OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0] failed.", lstmCellV2Node[numSplitX-1]->GetName().c_str(), fusedNode->GetName().c_str()),
                return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0].", lstmCellV2Node[numSplitX-1]->GetName().c_str(), fusedNode->GetName().c_str());
        }
       }

     if (fusedNode->GetOutDataAnchor(2)->GetPeerInDataAnchors().size() > 0) {
          OP_LOGI(FUSED_OP_TYPE.c_str(), "The fanout size of lstmNode is [%d].", fusedNode->GetOutDataAnchor(2)->GetPeerInDataAnchors().size());
          for (InDataAnchorPtr inAnchorPtr : fusedNode->GetOutDataAnchor(2)->GetPeerInDataAnchors()) {
                inAnchorPtr->UnlinkAll();
                FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(lstmCellV2Node[numSplitX-1]->GetOutDataAnchor(1), inAnchorPtr),
                       OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0] failed.", lstmCellV2Node[numSplitX-1]->GetName().c_str(), fusedNode->GetName().c_str()),
                       return FAILED);
                OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0].", lstmCellV2Node[numSplitX-1]->GetName().c_str(), fusedNode->GetName().c_str());
          }
