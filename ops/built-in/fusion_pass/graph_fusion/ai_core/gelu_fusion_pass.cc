...
vector<FusionPattern*> GeluFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("GeluFusionPass");
...
Status GeluFusionPass::Fusion(ge::ComputeGraph& graph,
                                Mapping& mapping,
  ...
  FUSION_PASS_CHECK(pow0_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "pow0_node is null, fusion failed."), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(tanh0_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "tanh0_node is null, fusion failed."), return PARAM_INVALID);
  FUSION_PASS_CHECK(add1_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "add1_node is null, fusion failed."), return PARAM_INVALID);
  FUSION_PASS_CHECK(mul2_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mul2_node is null, fusion failed."), return PARAM_INVALID);
  FUSION_PASS_CHECK(mul3_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mul3_node is null, fusion failed."), return PARAM_INVALID);
  ...
  gelu_desc =
      std::make_shared<ge::OpDesc>(pow0_node->GetName() + "/" + GELU, GELU);
  ...
  for (auto inDataAnchor :
       mul3_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(mul3_node->GetOutDataAnchor(0),
                                        inDataAnchor) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove out data edge failed."), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(gelu_node->GetOutDataAnchor(0),
                                     inDataAnchor) != SUCCESS,
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(tanh0_node) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove tanh0_node failed."), return FAILED);
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(mul2_node) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove mul2_node failed."), return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(mul3_node) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove mul3_node failed."), return FAILED);
