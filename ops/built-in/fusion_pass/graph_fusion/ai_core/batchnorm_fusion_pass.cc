...
int64_t BatchnormFusionPass::GetKernelNumOfOutputOfConv3D(const ge::NodePtr& conv) {
  ...
  if (curFormat == ge::FORMAT_NDHWC) {
    ...
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Node[%s]: don't support the format [%d].",
                                   conv->GetName().c_str(), curFormat);
    return 0;
  }
}
...
Status BatchnormFusionPass::RemoveSmalleNodes(ge::ComputeGraph& graph, const ge::NodePtr& addNode,
  ...
  FUSION_PASS_CHECK(
      graph.RemoveNode(mulNode1) != SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove node %s failed.", addNode->GetName().c_str()),
      return FAILED);

  FUSION_PASS_CHECK(
      graph.RemoveNode(mulNode2) != SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove node %s failed.", addNode->GetName().c_str()),
      return FAILED);

  FUSION_PASS_CHECK(
      graph.RemoveNode(mulNode3) != SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove node %s failed.", addNode->GetName().c_str()),
      return FAILED);

  FUSION_PASS_CHECK(
      graph.RemoveNode(subNode) != SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove node %s failed.", addNode->GetName().c_str()),
      return FAILED);
  return SUCCESS;
}
...
Status BatchnormFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(convNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "convNode is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(convNode->GetOutDataAnchor(0), 1) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "%s contains more than two peer input",
  ...
  FUSION_PASS_CHECK(mulNode1 == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "mulNode1 is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(mulNode2 == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "mulNode2 is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode2->GetOutDataAnchor(0), 1) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "%s contains more than one peer input",
                                                   mulNode2->GetName().c_str()),
                    return NOT_CHANGED);

  FUSION_PASS_CHECK(mulNode3 == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "mulNode3 is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(mulNode1->GetOpDesc()->GetInputsSize() < 2,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "mulNode1's %s input size is < 2",
                                                   mulNode1->GetName().c_str()),
                    return PARAM_INVALID);
  ge::OutDataAnchorPtr outAnchorOfScale = mulNode1->GetInDataAnchor(1)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(outAnchorOfScale == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "outAnchorOfScale must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr scaleNode = outAnchorOfScale->GetOwnerNode();
  FUSION_PASS_CHECK(scaleNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "scaleNode must not be null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(outAnchorOfOffset == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "outAnchorOfOffset must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr offsetNode = outAnchorOfOffset->GetOwnerNode();
  FUSION_PASS_CHECK(offsetNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "offsetNode must not be null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(outAnchorOfMean == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "outAnchorOfMean must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr meanNode = outAnchorOfMean->GetOwnerNode();
  FUSION_PASS_CHECK(meanNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "meanNode must not be null"),
                    return PARAM_INVALID);
  ...
  if (rsqrtNode == nullptr) {
    ...
    FUSION_PASS_CHECK(outAnchorOfVariance == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "outAnchorOfVariance must not be null"),
                      return PARAM_INVALID);
    varianceNode = outAnchorOfVariance->GetOwnerNode();
    FUSION_PASS_CHECK(varianceNode == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "varianceNode must not be null"),
                      return PARAM_INVALID);
    ...
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "addEpsNode is null and rsqrtNode is not null for %s",
                                   mulNode3->GetName().c_str());
    return FAILED;
  }
  ...
  FUSION_PASS_CHECK(outAnchorOfAdd == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "outAnchorOfAdd must not be null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(bnOpdesc == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "bnOpdesc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  for (auto& peerInAnchor : outAnchorOfAdd->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(peerInAnchor == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "peerInAnchor must not be null"),
                      return PARAM_INVALID);
  ...
  if (needDeleteAddAndSqrt) {
    ...
    FUSION_PASS_CHECK(
        graph.RemoveNode(rsqrtNode) != SUCCESS,
        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove node %s failed.", addNode->GetName().c_str()),
        return FAILED);
    FUSION_PASS_CHECK(
        graph.RemoveNode(addEpsNode) != SUCCESS,
        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove node %s failed.", addNode->GetName().c_str()),
        return FAILED);
  } else {
