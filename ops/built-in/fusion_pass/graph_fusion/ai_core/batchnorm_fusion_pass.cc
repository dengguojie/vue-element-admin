...
Status BatchnormFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(convNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "convNode is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(convNode->GetOutDataAnchor(0), 1) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "%s contains more than two peer input", convNode->GetName().c_str()),
                    return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(mulNode1 == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "mulNode1 is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode1->GetOutDataAnchor(0), 2) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "%s contains more than two peer input", mulNode1->GetName().c_str()),
                    return NOT_CHANGED);

  FUSION_PASS_CHECK(mulNode2 == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "mulNode2 is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode2->GetOutDataAnchor(0), 1) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "%s contains more than one peer input", mulNode2->GetName().c_str()),
                    return NOT_CHANGED);

  FUSION_PASS_CHECK(mulNode3 == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "mulNode3 is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode3->GetOutDataAnchor(0), 1) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "%s contains more than one peer input", mulNode3->GetName().c_str()),
                    return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(mulNode1->GetOpDesc()->GetInputsSize() < 2,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "mulNode1's %s input size is < 2", mulNode1->GetName().c_str()),
                    return PARAM_INVALID);
  ge::OutDataAnchorPtr outAnchorOfScale = mulNode1->GetInDataAnchor(1)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(outAnchorOfScale == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "outAnchorOfScale must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr scaleNode = outAnchorOfScale->GetOwnerNode();
  FUSION_PASS_CHECK(scaleNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "scaleNode must not be null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(outAnchorOfOffset == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "outAnchorOfOffset must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr offsetNode = outAnchorOfOffset->GetOwnerNode();
  FUSION_PASS_CHECK(offsetNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "offsetNode must not be null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(outAnchorOfMean == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "outAnchorOfMean must not be null"),
                    return PARAM_INVALID);
  ...
  if (rsqrtNode == nullptr) {
    ...
    FUSION_PASS_CHECK(varianceNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "varianceNode must not be null"),
                      return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(outAnchorOfVariance == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "outAnchorOfVariance must not be null"), return PARAM_INVALID);
    varianceNode = outAnchorOfVariance->GetOwnerNode();
    FUSION_PASS_CHECK(varianceNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "varianceNode must not be null"),
                      return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(outAnchorOfAdd == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "outAnchorOfAdd must not be null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(bnOpdesc == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "bnOpdesc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (needDeleteAddAndSqrt) {
    ...
    FUSION_PASS_CHECK(addEpsWeights.size() < 2,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "size of weight which is %lu of %s is less than 2",
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::SetFloat(bnOpdesc, ge::BATCHNORM_ATTR_EPSILON, eps),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "DestNode[%s]: set epsilon attr %s not success.",
