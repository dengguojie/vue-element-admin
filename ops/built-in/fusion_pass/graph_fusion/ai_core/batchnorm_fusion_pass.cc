...
vector<FusionPattern *> BatchnormFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern1 =
      new (std::nothrow) FusionPattern("BatchnormFusion1");
  FUSION_PASS_CHECK(pattern1 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new an object failed."),
           return patterns);
  pattern1
      ->AddOpDesc(ADD_PATTERN, {ADD})
  ...
  FusionPattern *pattern2 =
      new (std::nothrow) FusionPattern("BatchnormFusion2");
...
int64_t BatchnormFusionPass::GetKernelNumOfOutputOfConv3D(const ge::NodePtr& conv) {
  ...
  if (dims.size() < 5) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "dimNum of %s which is %u is less than 5",
  ...
  if (curFormat == ge::FORMAT_NDHWC) {
    ...
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]: don't support the format [%d].", conv->GetName().c_str(), curFormat);
    return 0;
  }
}

Status BatchnormFusionPass::CheckInputTypeValid(const ge::NodePtr& originalNode,
  ...
  FUSION_PASS_CHECK(inputNode->GetType() != expectOpType,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Match failed! Input of %s is not %s. It is %s, name %s",
                   ...
                   expectOpType.c_str(),
                   inputNode->GetType().c_str(),
                   inputNode->GetName().c_str()),
           return FAILED);
  return SUCCESS;
}

Status BatchnormFusionPass::CheckInputTensorValid(const ge::GeTensorDesc& tensorDesc,
                             const int64_t& kernelNum) {
  auto dims = tensorDesc.GetShape().GetDims();
  if (dims.size() != 1 || dims.at(0) != kernelNum) {
    OP_LOGW(FUSED_OP_TYPE.c_str(), "Match failed! Dims is %u and dims[0] is %lu and kernel num is% lu",
            dims.size(), dims.at(0), kernelNum);
    return FAILED;
  }
  return SUCCESS;
}
...
Status BatchnormFusionPass::RemoveSmalleNodes(ge::ComputeGraph &graph,
                         const ge::NodePtr& addNode,
                         const ge::NodePtr& mulNode1,
                         const ge::NodePtr& mulNode2,
                         const ge::NodePtr& mulNode3,
                         const ge::NodePtr& subNode) {
  FUSION_PASS_CHECK(graph.RemoveNode(addNode) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove node %s failed.", addNode->GetName().c_str()),
           return FAILED);
...
Status BatchnormFusionPass::AddTensorDescForBn(const ge::OpDescPtr& bnOpdesc,
  ...
  FUSION_PASS_CHECK(bnOpdesc->AddInputDesc(inputTensor) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input0 of bn failed."), return FAILED);
  FUSION_PASS_CHECK(bnOpdesc->AddInputDesc("scale", scaleTensor) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input1 of bn failed."), return FAILED);
  FUSION_PASS_CHECK(bnOpdesc->AddInputDesc("offset", offsetTensor) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input2 of bn failed."), return FAILED);
  FUSION_PASS_CHECK(bnOpdesc->AddInputDesc("mean", meanTensor) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input3 of bn failed."), return FAILED);
  FUSION_PASS_CHECK(bnOpdesc->AddInputDesc("variance", varianceTensor) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input4 of bn failed."), return FAILED);
...
Status BatchnormFusionPass::Fusion(
    ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes)
{
  ...
  ge::NodePtr addNode =
      GetNodeFromMapping(ADD_PATTERN, mapping);
  ge::NodePtr mulNode1 =
      GetNodeFromMapping(MUL_PATTERN_1, mapping);
  ge::NodePtr mulNode2 =
      GetNodeFromMapping(MUL_PATTERN_2, mapping);
  ge::NodePtr mulNode3 =
      GetNodeFromMapping(MUL_PATTERN_3, mapping);
  ge::NodePtr subNode =
      GetNodeFromMapping(SUB_PATTERN, mapping);
  ge::NodePtr rsqrtNode =
      GetNodeFromMapping(RSQRT_PATTERN, mapping);
  ge::NodePtr addEpsNode =
      GetNodeFromMapping(ADD_EPS_PATTERN, mapping);
  ge::NodePtr convNode =
      GetNodeFromMapping(CONV3D_PATTERN, mapping);

  OP_LOGD(FUSED_OP_TYPE.c_str(), "Check node null or not");
  FUSION_PASS_CHECK(convNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "convNode is null, fusion failed."),
           return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(convNode->GetOutDataAnchor(0), 1) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than two peer input",
                   convNode->GetName().c_str()),
           return NOT_CHANGED);

  FUSION_PASS_CHECK(addNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "addNode is null, fusion failed."),
           return PARAM_INVALID);

  FUSION_PASS_CHECK(mulNode1 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mulNode1 is null, fusion failed."),
           return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode1->GetOutDataAnchor(0), 2) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than two peer input",
                   mulNode1->GetName().c_str()),
           return NOT_CHANGED);

  FUSION_PASS_CHECK(mulNode2 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mulNode2 is null, fusion failed."),
           return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode2->GetOutDataAnchor(0), 1) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than one peer input",
                   mulNode2->GetName().c_str()),
           return NOT_CHANGED);

  FUSION_PASS_CHECK(mulNode3 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mulNode3 is null, fusion failed."),
           return PARAM_INVALID);
  ...
  ge::OutDataAnchorPtr outAnchorOfConv3D =
      mulNode3->GetInDataAnchor(0)->GetPeerOutAnchor();
  ...
  FUSION_PASS_CHECK(CheckInputTypeValid(mulNode3, inputNode, CONV3D) != SUCCESS,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Input Type invalid."),
           return NOT_CHANGED);
  uint32_t kernelNum = GetKernelNumOfOutputOfConv3D(inputNode);

  /* Get scale (gamma) */
  FUSION_PASS_CHECK(mulNode1->GetOpDesc()->GetInputsSize() < 2,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "mulNode1's %s input size is < 2",
                   mulNode1->GetName().c_str()),
           return PARAM_INVALID);
  ge::OutDataAnchorPtr outAnchorOfScale =
      mulNode1->GetInDataAnchor(1)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(outAnchorOfScale == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfScale must not be null"), return PARAM_INVALID);
  ge::NodePtr scaleNode = outAnchorOfScale->GetOwnerNode();
  FUSION_PASS_CHECK(scaleNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "scaleNode must not be null"), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(CheckInputTensorValid(scaleTensor, kernelNum) != SUCCESS,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "Input tensor invalid."),
           return NOT_CHANGED);

  /* Get offset (beta) */
  ge::OutDataAnchorPtr outAnchorOfOffset =
      subNode->GetInDataAnchor(0)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(outAnchorOfOffset == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfOffset must not be null"), return PARAM_INVALID);
  ge::NodePtr offsetNode = outAnchorOfOffset->GetOwnerNode();
  FUSION_PASS_CHECK(offsetNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "offsetNode must not be null"), return PARAM_INVALID);
  ...
  /* Get mean */
  ge::OutDataAnchorPtr outAnchorOfMean =
      mulNode2->GetInDataAnchor(0)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(outAnchorOfMean == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfMean must not be null"), return PARAM_INVALID);
  ge::NodePtr meanNode = outAnchorOfMean->GetOwnerNode();
  FUSION_PASS_CHECK(meanNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "meanNode must not be null"), return PARAM_INVALID);
  ...
  if (rsqrtNode == nullptr) {
    OP_LOGD(FUSED_OP_TYPE.c_str(), "Rsqrt Node is null, we don't need to add eps and rsqrt for %s",
            mulNode3->GetName().c_str());
    outAnchorOfVariance =
        mulNode1->GetInDataAnchor(0)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(outAnchorOfVariance == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfVariance must not be null"), return PARAM_INVALID);
    varianceNode = outAnchorOfVariance->GetOwnerNode();
  FUSION_PASS_CHECK(varianceNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "varianceNode must not be null"), return PARAM_INVALID);
    varianceTensor = mulNode1->GetOpDesc()->GetInputDesc(0);
  } else if (addEpsNode != nullptr && rsqrtNode != nullptr) {
    OP_LOGD(FUSED_OP_TYPE.c_str(), "we need to add eps and rsqrt for %s.",
            mulNode3->GetName().c_str());
    /* input 0 of addEpsNode is variance and input 1 is eps */
    outAnchorOfVariance =
        addEpsNode->GetInDataAnchor(0)->GetPeerOutAnchor();
    FUSION_PASS_CHECK(CheckPeerInDataAnchors(addEpsNode->GetOutDataAnchor(0), 1) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than two peer input",
                     addEpsNode->GetName().c_str()),
             return NOT_CHANGED);
    FUSION_PASS_CHECK(CheckPeerInDataAnchors(rsqrtNode->GetOutDataAnchor(0), 1) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than two peer input",
                     addEpsNode->GetName().c_str()),
             return NOT_CHANGED);
    FUSION_PASS_CHECK(outAnchorOfVariance == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfVariance must not be null"), return PARAM_INVALID);
    ...
    OP_LOGE(FUSED_OP_TYPE.c_str(), "addEpsNode is null and rsqrtNode is not null for %s",
            mulNode3->GetName().c_str());
    return FAILED;
  }
  ...
  FUSION_PASS_CHECK(outAnchorOfAdd == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfAdd must not be null"), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(bnOpdesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "bnOpdesc is null, fusion failed."),
           return PARAM_INVALID);

  /* 4. Add TensorDesc into new node. Bn must have five inputs and three
   * outputs. If not, we will return false. */
  (void)AddTensorDescForBn(bnOpdesc, inputTensor, scaleTensor, offsetTensor,
                           meanTensor, varianceTensor, bnOutTensor);
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outAnchorOfConv3D,
                                   bnNode->GetInDataAnchor(0)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out data edge failed."), return FAILED);
  FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outAnchorOfConv3D,
                                      mulNode3->GetInDataAnchor(0)) != SUCCESS,
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outAnchorOfScale,
                                   bnNode->GetInDataAnchor(1)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add scale(gamma) edge failed."), return FAILED);
  FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outAnchorOfScale,
                                      mulNode1->GetInDataAnchor(1)) != SUCCESS,
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outAnchorOfOffset,
                                   bnNode->GetInDataAnchor(2)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add offset(beta) edge failed."), return FAILED);
  FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outAnchorOfOffset,
                                      subNode->GetInDataAnchor(0)) != SUCCESS,
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outAnchorOfMean,
                                   bnNode->GetInDataAnchor(3)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add mean edge failed."), return FAILED);
  FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outAnchorOfMean,
                                      mulNode2->GetInDataAnchor(0)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove mean edge failed."), return FAILED);

  /* remove variance from mul node 2 and Add it to batchnorm*/
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outAnchorOfVariance,
                                   bnNode->GetInDataAnchor(4)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add variance edge failed."), return FAILED);
  ...
  for (auto& peerInAnchor : outAnchorOfAdd->GetPeerInDataAnchors()) {
	FUSION_PASS_CHECK(peerInAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "peerInAnchor must not be null"), return PARAM_INVALID);
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outAnchorOfAdd,
                                        peerInAnchor) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove edge %u failed.", index), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(bnNode->GetOutDataAnchor(0),
                                     peerInAnchor) != SUCCESS,
  ...
  Status ret =
      RemoveSmalleNodes(graph, addNode, mulNode1, mulNode2, mulNode3, subNode);
  if (ret != SUCCESS) {
    return ret;
  }
  ...
  if (needDeleteAddAndSqrt) {
    ...
    vector<ge::ConstGeTensorPtr> addEpsWeights =
        ge::OpDescUtils::GetWeights(addEpsNode);
    FUSION_PASS_CHECK(addEpsWeights.size() < 2,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "size of weight which is %u of %s is less than 2",
    ...
    (void)ge::AttrUtils::SetBool(bnOpdesc, "need_adding_eps_and_rsqrting",
                                 false);
  }

  FUSION_PASS_CHECK(
      !ge::AttrUtils::SetFloat(bnOpdesc, ge::BATCHNORM_ATTR_EPSILON, eps),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "DestNode[%s]: set epsilon attr %s not success.",
              bnOpdesc->GetName().c_str(), ge::BATCHNORM_ATTR_EPSILON.c_str()),
              return FAILED);
  FUSION_PASS_CHECK(!ge::AttrUtils::SetBool(bnOpdesc, "is_training", false),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Set is_traing attr failed."), return FAILED);

  OP_LOGI(FUSED_OP_TYPE.c_str(), "BatchNorm graph fusion success for node %s!",
          addNode->GetName().c_str());
  return SUCCESS;
}
