...
int64_t BatchnormFusionPass::GetKernelNumOfOutputOfConv3D(const ge::NodePtr& conv) {
  ...
  if (dims.size() < 5) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "dimNum of %s which is %u is less than 5", conv->GetName().c_str(), dims.size());
    return 0;
  }
...
Status BatchnormFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(convNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "convNode is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(convNode->GetOutDataAnchor(0), 1) != SUCCESS,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than two peer input", convNode->GetName().c_str()),
                    return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode1->GetOutDataAnchor(0), 2) != SUCCESS,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than two peer input", mulNode1->GetName().c_str()),
                    return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode2->GetOutDataAnchor(0), 1) != SUCCESS,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than one peer input", mulNode2->GetName().c_str()),
                    return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(CheckPeerInDataAnchors(mulNode3->GetOutDataAnchor(0), 1) != SUCCESS,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than one peer input", mulNode3->GetName().c_str()),
                    return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(mulNode1->GetOpDesc()->GetInputsSize() < 2,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "mulNode1's %s input size is < 2", mulNode1->GetName().c_str()),
                    return PARAM_INVALID);
  ge::OutDataAnchorPtr outAnchorOfScale = mulNode1->GetInDataAnchor(1)->GetPeerOutAnchor();
  FUSION_PASS_CHECK(outAnchorOfScale == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfScale must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr scaleNode = outAnchorOfScale->GetOwnerNode();
  FUSION_PASS_CHECK(scaleNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "scaleNode must not be null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(outAnchorOfOffset == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfOffset must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr offsetNode = outAnchorOfOffset->GetOwnerNode();
  FUSION_PASS_CHECK(offsetNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "offsetNode must not be null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(outAnchorOfMean == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfMean must not be null"),
                    return PARAM_INVALID);
  ge::NodePtr meanNode = outAnchorOfMean->GetOwnerNode();
  FUSION_PASS_CHECK(meanNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "meanNode must not be null"),
                    return PARAM_INVALID);
  ...
  if (rsqrtNode == nullptr) {
    ...
    FUSION_PASS_CHECK(outAnchorOfVariance == nullptr,
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfVariance must not be null"), return PARAM_INVALID);
    varianceNode = outAnchorOfVariance->GetOwnerNode();
    FUSION_PASS_CHECK(varianceNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "varianceNode must not be null"),
                      return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(
        ...
        OP_LOGE(FUSED_OP_TYPE.c_str(), "%s contains more than two peer input", addEpsNode->GetName().c_str()),
        return NOT_CHANGED);
    FUSION_PASS_CHECK(
    ...
    OP_LOGE(FUSED_OP_TYPE.c_str(), "addEpsNode is null and rsqrtNode is not null for %s", mulNode3->GetName().c_str());
    return FAILED;
  }
  ...
  FUSION_PASS_CHECK(outAnchorOfAdd == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "outAnchorOfAdd must not be null"),
                    return PARAM_INVALID);
