...
#include "dynamic_augru_seqlength_fusion_pass.h"
#include <iostream>
#include <vector>
#include <string>
#include <map>
...
using namespace ge;
namespace fe {
static const char* FUSED_NODE = "DynamicAUGRU";
...
vector<FusionPattern*> DynamicAUGRUSeqFusionPass::DefinePatterns() {
  ...
  FusionPattern* pattern = new (std::nothrow) FusionPattern("DynamicAUGRUSeqFusionPass");
  FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "DynamicAUGRU seqLength pattern object failed."),
                    return patterns);
...
Status DynamicAUGRUSeqFusionPass::AddRNNMaskNode(ge::NodePtr fusedNode, ge::ComputeGraph& graph,
  ...
  FUSION_PASS_CHECK(dimLength.size() != 1, OP_LOGE(FUSED_OP_TYPE.c_str(), "Unexcepted seqlength input shape"),
                    return FAILED);
  int64_t batchSize = dimLength[0];
  int64_t numStep = fusedNode->GetOpDesc()->GetInputDesc(0).GetShape().GetDim(0);
  int64_t hiddenSize = fusedNode->GetOpDesc()->GetInputDesc(WEIGHT_HIDDEN_INDEX).GetShape().GetDim(0);
  ...
  // Add Edge
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(seqLenIndex)->GetPeerOutAnchor(),
  ...
  if (PatternFusionUtil::IsUnknownShape(numStep) || PatternFusionUtil::IsUnknownShape(m_size)) {
    // Add Edge
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(xIndex)->GetPeerOutAnchor(),
...
Status DynamicAUGRUSeqFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  // get the NodePtr of dynamic_augru
  OP_LOGI(FUSED_OP_TYPE.c_str(), "dynamic_augru seqLength start fusion");

  ge::NodePtr fusedNode = GetNodeFromMapping(PATTERN_FUSEDNODE, mapping);
  FUSION_PASS_CHECK(fusedNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusedNode is null, fusion failed."),
                    return PARAM_INVALID);

  // get the OpDescPtr of dynamic_augru
  ge::OpDescPtr fusedDesc = fusedNode->GetOpDesc();
  FUSION_PASS_CHECK(fusedNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusedNode OpDesc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  ge::AttrUtils::SetInt(fusedDesc, "hidden_size", hiddenSize);
  ...
  if (hasSeqLength) {
    ...
    FUSION_PASS_CHECK(SUCCESS != AddRNNMaskNode(fusedNode, graph, newNodes),
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "AddRNNMaskNode return failed"), return FAILED);

    OP_LOGI(FUSED_OP_TYPE.c_str(), "dynamic_augru seqLength end fusion");
    return SUCCESS;
  } else {
    return NOT_CHANGED;
  }
...
REGISTER_PASS("DynamicAUGRUAddSeqPass", BUILT_IN_GRAPH_PASS, DynamicAUGRUSeqFusionPass);
}  // namespace fe
