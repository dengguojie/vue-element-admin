...
Status SPPPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(sppNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "sppNode is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (sppHyramidHeight == 1) {
    ...
    FUSION_PASS_MAKE_SHARED((singlePoolingOp = std::make_shared<ge::OpDesc>(sppNode->GetName() + "_spp_pooling",
                            "SppPooling")),
  ...
  for (int64_t i = 0; i < sppHyramidHeight; i++) {
    ...
    FUSION_PASS_CHECK(tmpNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "poolingNode[%ld] is null, fusion failed.",
                      i),
                      return PARAM_INVALID);
