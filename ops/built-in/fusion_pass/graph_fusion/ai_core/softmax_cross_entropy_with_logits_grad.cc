...
Status SoftmaxCrossEntropyWithLogitsGradPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_CHECK(reshape_grad_2 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshape_grad_2 is null"),
  ...
  FUSION_PASS_CHECK(reshape_grad == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshape_grad is null"),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(reshapeGrad2InDataAnchor == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "reshapeGrad2InDataAnchor is null"), return FAILED);
  auto reshapeGrad2PeerOutDataAnchor = reshapeGrad2InDataAnchor->GetPeerOutAnchor();

  auto mulInDataAnchor = mul->GetInDataAnchor(0);
  FUSION_PASS_CHECK(mulInDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mulInDataAnchor is null"),
                    return FAILED);

  auto mulPeerOutDataAnchor = mulInDataAnchor->GetPeerOutAnchor();
  FUSION_PASS_CHECK(mulPeerOutDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mulPeerOutDataAnchor is null"),
                    return FAILED);

  auto mulOutDataAnchor = mul->GetOutDataAnchor(0);
  FUSION_PASS_CHECK(mulOutDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mulOutDataAnchor is null"),
                    return FAILED);

  auto reshapeGradOutDataAnchor = reshape_grad->GetOutDataAnchor(0);
  FUSION_PASS_CHECK(reshapeGradOutDataAnchor == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "reshapeGradOutDataAnchor is null"), return FAILED);
  ...
  FUSION_PASS_CHECK(RemoveNode(reshape_grad_2, graph) == FAILED,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "remove reshape_grad_2 node failed"), return FAILED);
  FUSION_PASS_CHECK(RemoveNode(expandDim, graph) == FAILED,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "remove expandDims node failed"), return FAILED);
  FUSION_PASS_CHECK(RemoveNode(reshape_grad, graph) == FAILED,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "remove reshape_grad node failed"), return FAILED);
