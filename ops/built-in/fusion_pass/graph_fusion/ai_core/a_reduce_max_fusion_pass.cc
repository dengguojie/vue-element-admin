...
std::vector<FusionPattern*> AReduceMaxFusionPass::DefinePatterns() {
...
Status AReduceMaxFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, std::vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(maxNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                   "maxNode is null, fusion failed."),
  ...
  for (auto inDataAnchor : maxNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(maxNode->GetOutDataAnchor(0), inDataAnchor) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove max and outnode edge failed."),
                      return FAILED);
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(maxNode) != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                   "Remove maxNode failed."),
                    return FAILED);
