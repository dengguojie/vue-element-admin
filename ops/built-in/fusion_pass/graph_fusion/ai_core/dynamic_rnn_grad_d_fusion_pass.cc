...
static map<std::string, int> RNN_GRAD_NODE_INPUT_INDEX = {{"x", 0}, {"w", 1}, {"init_c", 5}, {"init_h", 4}, {"h", 6},
                                                          {"c", 7}, {"dy", 8}, {"dh", 9}, {"dc", 10}, {"i", 11},
                                                          {"j", 12}, {"f", 13}, {"o", 14}, {"tanhct", 15},
                                                          {"mask", 16}};
int64_t t_size = -1;
...
vector<ge::NodePtr> DynamicRNNGradDFusionPass::GetDynamicReshapeNode(std::string& reshapeNodeName,
  ...
  ge::GeShape shapeConstShape = ge::GeShape({1,});
  ...
  ge::GeShape lastConstShape = ge::GeShape({1,});
  ...
  ge::NodePtr tSplitNode = BuildTDgateSplit(shapeNode->GetOpDesc()->GetOutputDesc(0).Clone(),
                                                                    ...
                                                                    ge::ComputeGraph& graph, bool& failStatus)
{
  ...
  output_dims.push_back(4 * hidden_size);
                                            ...
                                            ge::ComputeGraph& graph, bool& failStatus, ge::GeShape dgateShape)
{
  ...
  x1_range.push_back(std::make_pair(dgateShape.GetDim(1), dgateShape.GetDim(1)));
  ...
  x2_range.push_back(std::make_pair(w_tensor_desc.GetShape().GetDim(0), w_tensor_desc.GetShape().GetDim(0)));
  x2_range.push_back(std::make_pair(w_tensor_desc.GetShape().GetDim(1), w_tensor_desc.GetShape().GetDim(1)));
  w_tensor_desc.SetShapeRange(x2_range);
  lstmBatchMatMulDesc->AddInputDesc("x2", w_tensor_desc);
  // add matmul output
  vector<int64_t> outputy_dims;
  outputy_dims.push_back(dgateShape.GetDim(0));
  outputy_dims.push_back(w_tensor_desc.GetShape().GetDim(0));
  ...
  y1_range.push_back(std::make_pair(1, 1000));
  y1_range.push_back(std::make_pair(w_tensor_desc.GetShape().GetDim(0), w_tensor_desc.GetShape().GetDim(0)));
...
vector<ge::OpDescPtr> DynamicRNNGradDFusionPass::GetDynamicSplitNode(std::string splitNodeName,
  ...
  FUSION_PASS_MAKE_SHARED(
      (lstmSplitDesc = std::make_shared<ge::OpDesc>(splitNodeName, "SplitV")),
      failStatus=true; return nullResult);
  ge::GeShape inputShape = GeShape({(matmulOutputDesc.GetOriginShape().GetDim(1) + 15) / 16, batch_nz_size, 16, 16});
  matmulOutputDesc.SetShape(inputShape);
  matmulOutputDesc.SetFormat(ge::FORMAT_FRACTAL_NZ);
  lstmSplitDesc->AddInputDesc("x", matmulOutputDesc);

  vector<int64_t> dx_dims;
  dx_dims.push_back((input_size + 15) / 16);
  ...
  vector<int64_t> dh_dims;
  dh_dims.push_back((hidden_size + 15) / 16);
  ...
  vector<int64_t> size_splits;
  size_splits.push_back((input_size + 15) / 16);
  size_splits.push_back((hidden_size + 15) / 16);
                                                              ...
                                                              ge::GeTensorDesc concatDgateOriDesc, bool& failStatus)
{
  ...
  ge::OpDescPtr cellNode = GetDynamicLSTMGradCellNode(cellNodeName, dynamicRNNGradNode,
                                    whileNode->GetOpDesc()->GetInputDesc(14).Clone(), graph, failStatus);
  ...
  ge::OpDescPtr matmulNode = GetDynamicMatMulNode(matmulNodeName, dynamicRNNGradNode,
                                               graph, failStatus, cellNode->GetOutputDesc(0).GetShape());
  ...
  vector<ge::OpDescPtr> result = GetDynamicBodySplitNode(splitNodeName, splitDimNodeName, splitSizeNodeName,
                              dynamicRNNGradNode, graph, failStatus, matmulNode->GetOutputDesc(0).Clone());
  ...
  vector<ge::OpDescPtr> bodyDxReshapeNodes = GetDynamicBodyDxReshapeNode(bodyDxReshapeNodeName, constDxReshapeInputName,
                                              dynamicRNNGradNode, result[0]->GetOutputDesc(0).Clone(),
  ...
  vector<ge::OpDescPtr> bodyDgateReshapeNodes = GetDynamicBodyReshapeNode(bodyReshapeNodeName, constReshapeInputName,
                                                                          dynamicRNNGradNode,
                                                                          cellNode->GetOutputDesc(0).Clone(),
                                                                          concatDgateDesc->GetInputDesc(0).Clone(),
                                                                          graph, failStatus);
  ...
  graph_builder.SetInput(1, {matmulNodeName}, {1});
                                                            ...
                                                            vector<ge::NodePtr>& newNodes, bool& failStatus)
{
  ...
  if (dynamicRNNGradNode->GetOpDesc()->GetInputDesc(RNN_GRAD_NODE_INPUT_INDEX["init_c"]).
    ...
    ge::GraphUtils::AddEdge(reshapeInitC->GetOutDataAnchor(0),
                            t0CellNode->GetInDataAnchor(0));
  } else {
  ...
  ge::GraphUtils::AddEdge(dynamicRNNGradNode->GetInDataAnchor(1)->GetPeerOutAnchor(), t0MatmulNode->GetInDataAnchor(1));
...
vector<ge::NodePtr> DynamicRNNGradDFusionPass::DynamicAddLSTMInputGradNode(ge::NodePtr dynamicRNNGradNode,
  ...
  ge::NodePtr shapeNode = BuildTShape(dynamicRNNGradDesc->GetInputDesc(0).Clone(),
                                      dynamicRNNGradNode, graph, failStatus);
  ge::NodePtr tSplitNode = BuildTSplit(shapeNode->GetOpDesc()->GetOutputDesc(0).Clone(),
                                       dynamicRNNGradNode, graph, failStatus);
  vector<ge::NodePtr> t0Nodes = BuildT0Graph(dynamicRNNGradNode, currTConst->GetOutputDesc(0).Clone(),
  ...
  ge::NodePtr dxReshapeNode = GetDynamicReshapeDxNode(dxReshapeNodeName, dynamicRNNGradNode,
                                                      splitT0Node->GetOpDesc()->GetOutputDesc(0).Clone(),
                                                      cellT0Node->GetOpDesc()->GetOutputDesc(0).Clone(),
                                                      graph, failStatus);
  ge::GeShape shapeShape = ge::GeShape({1,});
  auto shapeDesc = ge::GeTensorDesc(shapeShape, ge::FORMAT_ND, ge::DT_INT64);
  shapeDesc.SetOriginShape(shapeShape);
  shapeDesc.SetOriginFormat(ge::FORMAT_ND);
  ...
  vector<int64_t> dxConcatDims = {-1, batch_size, concatOriDesc.GetOriginShape().GetDim(1)};
  ...
  ge::GeShape shapeBodyShape = ge::GeShape({1,});
  ...
  vector<ge::NodePtr> whileNodes = BuildWhileNodes(dynamicRNNGradNode, graph, newNodes, failStatus, concatOriDesc,
                                                   concatDgateOriDesc, currTConst->GetOutputDesc(0).Clone(),
                                                   totalTConst->GetOutputDesc(0).Clone()
  ...
  vector<ge::NodePtr> dgateReshapeResult = GetDynamicReshapeNode(reshapeNodeName, dynamicRNNGradNode, cellT0Node,
                                                                 whileNodes[0]->GetOpDesc()->GetOutputDesc(13).Clone(),
  ...
  ge::GraphUtils::AddEdge(dxreshape_const_node->GetOutDataAnchor(0), dxReshapeConcatNode->GetInDataAnchor(0));
  ge::GraphUtils::AddEdge(tSplitNode->GetOutDataAnchor(1), dxReshapeConcatNode->GetInDataAnchor(1));
  ...
  ge::GraphUtils::AddEdge(dxreshape_body_const_node->GetOutDataAnchor(0), dxBodyReshapeConcatNode->GetInDataAnchor(0));
...
Status DynamicRNNGradDFusionPass::DynamicAddDbReduceSumNode(ge::NodePtr dynamicRNNGradNode, ge::NodePtr &while_node,
  ...
  reduce_output_origin_dims.push_back(1);
  reduce_output_origin_dims.push_back(4*hidden_size);
...
ge::NodePtr DynamicRNNGradDFusionPass::DynamicAddMatmulNode(ge::NodePtr dynamicRNNGradNode, ge::NodePtr concatNode,
  ...
  FUSION_PASS_MAKE_SHARED(
      ...
      failStatus=true; return nullptr);
  // input
  ge::GeTensorDesc inputTensorDescXh = ge::GeTensorDesc(concatNode->GetOpDesc()->GetOutputDesc(0).GetOriginShape(),
  ...
  vector<int64_t> x1NzDims = {inputTensorDescXhTotal.GetOriginShape().GetDim(0),
                              (inputTensorDescXhTotal.GetOriginShape().GetDim(2) + 15) / 16, batch_nz_size, 16, 16};
  inputTensorDescXhTotal.SetShape(GeShape(x1NzDims));
  inputTensorDescXhTotal.SetFormat(ge::FORMAT_FRACTAL_NZ);

  std::vector<std::pair<int64_t, int64_t>> x1_range;
  x1_range.push_back(std::make_pair(1, 1000));
  x1_range.push_back(std::make_pair((hidden_size + input_size) / 16, (hidden_size + input_size) / 16));
  ...
  vector<int64_t> x2NzDims = {while_node->GetOpDesc()->GetOutputDesc(13).GetOriginShape().GetDim(0),
                             (while_node->GetOpDesc()->GetOutputDesc(13).GetOriginShape().GetDim(2) + 15) / 16,
                             batch_nz_size, 16, 16};
  ge::GeTensorDesc inputTensorDescDgate = ge::GeTensorDesc(GeShape(x2NzDims), ge::FORMAT_FRACTAL_NZ, ge::DT_FLOAT16);
  inputTensorDescDgate.SetOriginShape(while_node->GetOpDesc()->GetOutputDesc(13).GetOriginShape());
  inputTensorDescDgate.SetOriginFormat(ge::FORMAT_ND);

  std::vector<std::pair<int64_t, int64_t>> x2_range;
  x2_range.push_back(std::make_pair(1, 1000));
  x2_range.push_back(std::make_pair((hidden_size * 4) / 16, (hidden_size * 4) / 16));
  ...
  outputDims.push_back(hidden_size * 4);

  vector<int64_t> outputNzDims;
  outputNzDims.push_back(t_size);
  outputNzDims.push_back((hidden_size * 4 + 15) / 16);
  outputNzDims.push_back((input_size + hidden_size + 15) / 16);
  outputNzDims.push_back(16);
  outputNzDims.push_back(16);
  ...
  y_range.push_back(std::make_pair(1, 1000));
  y_range.push_back(std::make_pair((hidden_size + input_size) / 16, (hidden_size + input_size) / 16));
  y_range.push_back(std::make_pair((hidden_size * 4) / 16, (hidden_size * 4) / 16));
...
ge::NodePtr DynamicRNNGradDFusionPass::DynamicAddConcatNode(ge::NodePtr dynamicRNNGradNode, ge::NodePtr hConcatNode,
  ...
  ge::GraphUtils::AddEdge(dynamicRNNGradNode->GetInDataAnchor(0)->GetPeerOutAnchor(), concatNode->GetInDataAnchor(0));
...
Status DynamicRNNGradDFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  if (batch_size == -1) {
    ...
    batch_nz_size = batch_size / 16;
    batch_start = batch_nz_size;
    batch_end = batch_nz_size;
  }
  input_nz_size = input_size / 16;
  hidden_nz_size = hidden_size / 16;
