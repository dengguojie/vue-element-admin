...
Status DreluFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping,
  ...
  FUSION_PASS_CHECK(reluGradCount > 1,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Relu have multiple output, can not fusion."),
           return NOT_CHANGED);
  ...
  if (relu2->GetOpDesc()->InferShapeAndType() != ge::GRAPH_SUCCESS) {
    FUSION_PASS_CHECK(graph.RemoveNode(relu2) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove ReluV2 failed."), return FAILED);
    OP_LOGI(FUSED_OP_TYPE.c_str(), "ReluV2 InferShapeAndType failed, can not fusion.");
    return NOT_CHANGED;
  }

  FUSION_PASS_CHECK(ReplaceNode(relu, relu2, graph) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "replace node failed"), return FAILED);

  if (IsUnknownShape(
          relu2->GetOpDesc()->GetOutputDesc(1).GetShape())) {
    OP_LOGW(FUSED_OP_TYPE.c_str(), "DreluFusionPass cannot be applied for unknown shape.");
    return SUCCESS;
  }
  std::vector<int64_t> relu2Dims =
      relu2->GetOpDesc()->GetOutputDesc(1).GetShape().GetDims();
  relu2->GetOpDesc()->MutableOutputDesc(1)->SetOriginDataType(
      relu2->GetOpDesc()->GetOutputDesc(1).GetDataType());

  reluGrad->GetOpDesc()->SetType(RELUGRAD_V2);
  reluGrad->GetOpDesc()->MutableInputDesc(1)->SetShape(ge::GeShape(relu2Dims));
  reluGrad->GetOpDesc()->MutableInputDesc(1)->SetOriginShape(
      ge::GeShape(relu2Dims));
  ...
  FUSION_PASS_CHECK(relu2OutDataAnchor == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "relu2OutDataAnchor is null, fusion failed."),
           return PARAM_INVALID);
  // delete edge
  FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(reluGradPeerOutDataAnchor,
                                      reluGradInDataAnchor) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "remove inputdata edge error"), return FAILED);
  // add edge
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(relu2OutDataAnchor, reluGradInDataAnchor) !=
               SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input edge error"), return FAILED);
  reluGrad->GetOpDesc()->MutableInputDesc(1)->SetOriginDataType(
      reluGrad->GetOpDesc()->GetInputDesc(1).GetDataType());
...
Status DreluFusionPass::RemoveNode(ge::NodePtr node, ge::ComputeGraph &graph) {
  ...
  for (size_t i = 0; i < node->GetAllInDataAnchors().size(); ++i) {
    ...
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(preOutDataAnchor, inDataAnchor) !=
                 SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "remove inputdata edge error"), return FAILED);
  }
  // delete node
...
ge::NodePtr DreluFusionPass::CreateNode(ge::ComputeGraph &graph, ge::NodePtr relu,
                              vector<ge::NodePtr> &fusionNodes) {
  ...
  tensorDesc.SetOriginFormat(
      relu->GetOpDesc()->GetOutputDesc(0).GetOriginFormat());
...
Status DreluFusionPass::ReplaceNode(ge::NodePtr oldNode, ge::NodePtr newNode,
  ...
  for (size_t i = 0; i < oldNode->GetAllInDataAnchors().size(); ++i) {
    ...
    FUSION_PASS_CHECK(inDataAnchorNew == nullptr,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "inDataAnchorNew is null, fusion failed."),
             return PARAM_INVALID);
    ...
    if (preOutDataAnchor != nullptr) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(preOutDataAnchor, inDataAnchor) !=
                   SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "remove inputdata edge error"), return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(preOutDataAnchor, inDataAnchorNew) !=
                   SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "add input edge error"), return FAILED);
    }
  }
  ...
  FUSION_PASS_CHECK(inCtrlAnchor == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "inCtrlAnchor is null, fusion failed."),
           return PARAM_INVALID);
  FUSION_PASS_CHECK(inCtrlAnchorNew == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "inCtrlAnchorNew is null, fusion failed."),
           return PARAM_INVALID);

  for (auto preOutCtrlAnchor : inCtrlAnchor->GetPeerOutControlAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(preOutCtrlAnchor, inCtrlAnchor) !=
                 SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "remove input control edge error"), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(preOutCtrlAnchor, inCtrlAnchorNew) !=
                 SUCCESS,
  ...
  for (auto outPeerDataAnchor : inCtrlAnchor->GetPeerOutDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outPeerDataAnchor, inCtrlAnchor) !=
                 SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "remove input control edge error"), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outPeerDataAnchor, inCtrlAnchorNew) !=
                 SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "add input control edge error"), return FAILED);
  }
  // output data edge
  for (size_t i = 0; i < oldNode->GetAllOutDataAnchors().size(); ++i) {
    ...
    for (auto nextInDataAnchor : outDataAnchor->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outDataAnchor, nextInDataAnchor) !=
                   SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "remove outData edge error"), return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outDataAnchorNew, nextInDataAnchor) !=
                   SUCCESS,
    ...
    for (auto nextInCtrlAnchor : outDataAnchor->GetPeerInControlAnchors()) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outDataAnchor, nextInCtrlAnchor) !=
                   SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "remove outData edge error"), return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outDataAnchorNew, nextInCtrlAnchor) !=
                   SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "add outData edge error"), return FAILED);
    }
  }
  ...
  FUSION_PASS_CHECK(outCtrlAnchor == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "outCtrlAnchor is null, fusion failed."),
           return PARAM_INVALID);
  auto outCtrlAnchorNew = newNode->GetOutControlAnchor();
  FUSION_PASS_CHECK(outCtrlAnchorNew == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "outCtrlAnchorNew is null, fusion failed."),
           return PARAM_INVALID);
  for (auto nextInCtrlAnchor : outCtrlAnchor->GetPeerInControlAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outCtrlAnchor, nextInCtrlAnchor) !=
                 SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "remove outControl edge error"), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outCtrlAnchorNew, nextInCtrlAnchor) !=
                 SUCCESS,
