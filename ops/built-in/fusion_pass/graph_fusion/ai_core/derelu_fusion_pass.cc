...
Status DreluFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(reluGradCount > 1,
                    ...
                    return NOT_CHANGED);
  FUSION_PASS_CHECK(relu == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "relu is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(reluGrad == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "reluGrad is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (relu2->GetOpDesc()->InferShapeAndType() != ge::GRAPH_SUCCESS) {
    FUSION_PASS_CHECK(graph.RemoveNode(relu2) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove ReluV2 failed."),
                      return FAILED);
  ...
  FUSION_PASS_CHECK(ReplaceNode(relu, relu2, graph) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "replace node failed"),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(reluGradInDataAnchor == nullptr,
                    ...
                    "reluGradInDataAnchor is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(reluGrad == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "reluGrad is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(relu2OutDataAnchor == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "relu2OutDataAnchor is null, fusion failed."),
                    return PARAM_INVALID);
  // delete edge
...
Status DreluFusionPass::RemoveNode(ge::NodePtr node, ge::ComputeGraph& graph) {
  ...
  // delete node
  FUSION_PASS_CHECK(graph.RemoveNode(node) != SUCCESS,
...
Status DreluFusionPass::ReplaceNode(ge::NodePtr oldNode, ge::NodePtr newNode, ge::ComputeGraph& graph) {
  ...
  for (size_t i = 0; i < oldNode->GetAllInDataAnchors().size(); ++i) {
    ...
    FUSION_PASS_CHECK(inDataAnchorNew == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "inDataAnchorNew is null, fusion failed."),
                      return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(inCtrlAnchor == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "inCtrlAnchor is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(inCtrlAnchorNew == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "inCtrlAnchorNew is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  for (auto preOutCtrlAnchor : inCtrlAnchor->GetPeerOutControlAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(preOutCtrlAnchor, inCtrlAnchor) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "remove input control edge error"),
                      return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(preOutCtrlAnchor, inCtrlAnchorNew) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add input control edge error"),
                      return FAILED);
  }
  ...
  for (size_t i = 0; i < oldNode->GetAllOutDataAnchors().size(); ++i) {
    ...
    for (auto nextInDataAnchor : outDataAnchor->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outDataAnchor, nextInDataAnchor) != SUCCESS,
                        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "remove outData edge error"),
                        return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outDataAnchorNew, nextInDataAnchor) != SUCCESS,
                        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add outData edge error"),
                        return FAILED);
    }
  ...
  FUSION_PASS_CHECK(outCtrlAnchor == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "outCtrlAnchor is null, fusion failed."),
                    return PARAM_INVALID);
  auto outCtrlAnchorNew = newNode->GetOutControlAnchor();
  FUSION_PASS_CHECK(outCtrlAnchorNew == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "outCtrlAnchorNew is null, fusion failed."),
                    return PARAM_INVALID);
  for (auto nextInCtrlAnchor : outCtrlAnchor->GetPeerInControlAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(outCtrlAnchor, nextInCtrlAnchor) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "remove outControl edge error"),
                      return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outCtrlAnchorNew, nextInCtrlAnchor) != SUCCESS,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "add outControl edge error"),
                      return FAILED);
  }
  // delete old node
