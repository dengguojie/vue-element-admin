...
vector<FusionPattern*> LinSpaceFusionPass::DefinePatterns() {
  ...
  pattern->AddOpDesc(PATTERN_LINSPACE, {LINSPACE})
    .SetOutput(PATTERN_LINSPACE);
  patterns.push_back(pattern);

  return patterns;
}

Status LinSpaceFusionPass::Fusion(ge::ComputeGraph &graph,
                                  Mapping &mapping,
                                  vector<ge::NodePtr> &fusionNodes)
{
  // 获取lin_space节点
  ge::NodePtr linspaceVNode = GetNodeFromMapping(PATTERN_LINSPACE, mapping);
  FUSION_PASS_CHECK(linspaceVNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "linspaceVNode is null, fusion failed."),
  ...
  ge::GeTensorDesc tensorDesc2 = linspaceVNode->GetOpDesc()->GetInputDesc(2);

  // 寻找lin_space的父节点
  ge::InDataAnchorPtr linspaceAnchorPtr2 = linspaceVNode->GetInDataAnchor(2);
  ...
  // 获取lin_space父节点的输出描述
  ge::GeTensorDesc linspaceInputTensor =
      constNode2->GetOpDesc()->GetOutputDesc(0);
  ...
  FUSION_PASS_CHECK(inputAssit.get() == nullptr,
           ...
           return PARAM_INVALID);
  Status ret = NnSet(num, FLOAT_NUM_ZERO,
  ...
  OpDescUtils::ClearInputDesc(linSpaceDDesc, 2);
  OpDescUtils::ClearInputDesc(linSpaceDDesc, 1);
  OpDescUtils::ClearInputDesc(linSpaceDDesc, 0);

  // new the assist node
  auto newConstantOp =
      std::make_shared<ge::OpDesc>(linSpaceDDesc->GetName() + "_assist",
                                   "Constant");
  ...
  linSpaceDDesc->AddInputDesc(2, linspaceDesc->GetInputDesc(1));
  ...
  fusionNodes.push_back(linSpaceDNode);
    FUSION_PASS_CHECK(linSpaceDNode == nullptr,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.",
                     linSpaceDNode->GetName().c_str()),
             return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(assistNode->GetOutDataAnchor(0),
                                              linSpaceDNode->GetInDataAnchor(0)),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from %s's index[%d] to %s's index[%d] failed.",
                   assistNode->GetName().c_str(), 0,
                   linSpaceDNode->GetName().c_str(), 0),
           return FAILED);

  for (unsigned int i = 0; i < linspaceVNode->GetAllInDataAnchors().size(); i++) {
    FUSION_PASS_CHECK(SUCCESS !=
               ge::GraphUtils::AddEdge(linspaceVNode->GetInDataAnchor(i)->GetPeerOutAnchor(),
                                       linSpaceDNode->GetInDataAnchor(i+1)),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from %s's index[%d] to %s's index[%d] failed.",
                     linspaceVNode->GetName().c_str(), i,
                     linSpaceDNode->GetName().c_str(), i+1),
             return FAILED);
  ...
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(linspaceVNode),
