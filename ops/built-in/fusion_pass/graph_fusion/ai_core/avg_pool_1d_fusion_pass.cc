...
Status AvgPool1DFusionPass::AvgValueTableGen(const vector<int64_t>& dim_info, const int64_t kernel_size,
  ...
  FUSION_PASS_CHECK(output == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The output pointer is null!"), return FAILED);
  FUSION_PASS_CHECK(dim_info.size() < 4,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "The dim_info at least has 4 elements!"),
                    return FAILED);
  FUSION_PASS_CHECK(padding.size() < 2,
  VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "The padding at least has 2 elements!"),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(stride == 0, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The stride should not be 0, fusion failed."),
                    return FAILED);
...
Status AvgPool1DFusionPass::Fusion(ComputeGraph& graph, Mapping& mapping, vector<NodePtr>& fusion_nodes) {
  ...
  FUSION_PASS_CHECK(avgpool1d_fussed_node == nullptr,
                    ...
                    "The avgpool1d_fussed_node is null, fusion failed."),
  ...
  FUSION_PASS_CHECK(avgpool1d_desc == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The avgpool1d_fussed_node's OpDesc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(avgpool1d_op_desc == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The opdesc of avgpool1d_fussed_node is nullptr."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(avgpool1d_dim_info.size() < 4,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The size of avgpool1d_dim_info should great than 3, fusion failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(strides == 0, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                   "The stride should not be 0, fusion failed."),
                    return PARAM_INVALID);
  for (size_t i = 1; i <= 3; i++) {
    ...
    FUSION_PASS_CHECK(PatternFusionUtil::IsUnknownShape(dim),
                      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                      "AvgPool1DFusionPass cannot be applied for unknown shape."),
                      return GRAPH_FAILED);
  }
  int64_t w_output = CalWoutput(avgpool1d_dim_info[3], pads[0], pads[1], k_size, strides, ceil_mode);
  FUSION_PASS_CHECK(w_output <= 0, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                   "Should keep w_output > 0!"), return GRAPH_FAILED);
  ...
  if (input_type == DT_FLOAT16) {
    ...
    FUSION_PASS_CHECK(input_assit.get() == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                      "The input_assit is NULL"),
                      return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "AssitHelp failed."),
                     return ret);
    ...
    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "The input_assit is NULL"),
                      return PARAM_INVALID);
    ret = AvgValueTableGen(orig_input_shape_v, k_size, strides, pads, ceil_mode, count_include_pad,
                           avgpool1d_assit_dim_info, input_assit.get());
    FUSION_PASS_CHECK(ret != SUCCESS,
    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "AssitHelp failed."), return ret);
    GeShape avppool1d_assit_shape(avgpool1d_assit_dim_info);
    // set tensor_desc
    GeTensorDesc tensor_desc(GeShape(), FORMAT_NC1HWC0, input_type);
    tensor_desc.SetFormat(FORMAT_NC1HWC0);
  ...
  FUSION_PASS_CHECK(avgpool1d_const_input_desc == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The avgpool1d_const_input_desc is null, fusion failed."),
                    return PARAM_INVALID);
