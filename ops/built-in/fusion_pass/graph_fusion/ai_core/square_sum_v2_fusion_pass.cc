...
static const char* PATTERN_SQUARE = "Square";
static const char* PATTERN_REDUCESUMD = "ReduceSumD";
static const char* AXES = "axes";
static const char* AXIS = "axis";
static const char* KEEPDIMS = "keep_dims";

static const char* SQUARESUMV2 = "SquareSumV2";
...
vector<FusionPattern*> SquareSumV2FusionPass::DefinePatterns() {
  OP_LOGI(FUSED_OP_TYPE, "Define SquareSumV2 pattern begin");
  ...
  OP_LOGI(FUSED_OP_TYPE, "Define SquareSumV2FusionPass pattern end");
  return patterns;
}

Status SquareSumV2FusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  OP_LOGI(FUSED_OP_TYPE, "Define SquareSumV2FusionPass fusion begin");
  ...
  FUSION_PASS_CHECK(sum_node == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "sum node is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(square_node == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "square_node node is null, fusion failed."),
                    return PARAM_INVALID);

  // dynamic shape not supported currently
  auto to_check_nodes = {square_node, sum_node};
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::GetListInt(sum_node->GetOpDesc(), AXES, axis),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "get axis attr failed"), return FAILED);
  bool keep_dims;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetBool(sum_node->GetOpDesc(), KEEPDIMS, keep_dims),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Get keep_dims attr failed."), return FAILED);

  vector<int64_t> dims = square_node->GetOpDesc()->GetInputDesc(0).GetShape().GetDims();
  FUSION_PASS_CHECK(dims.empty(), VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "input dims is empty, fusion failed."),
                    return PARAM_INVALID);
  ...
  OP_LOGD(FUSED_OP_TYPE, "reduce num %d.", (int)multi);
  if (((multi * size_of_dtype) % 32 != 0) || (dims[dims.size() - 1] * size_of_dtype < 32)) {
    OP_LOGI(FUSED_OP_TYPE, "reduce num does not align 32B, square_sum_v2 not support");
    return NOT_CHANGED;
  }

  FUSION_PASS_CHECK(
      square_node->GetOutAllNodes().size() != 2,
      OP_LOGI(FUSED_OP_TYPE, "square node[%s] don't have output1 out node.", square_node->GetName().c_str()),
      return NOT_CHANGED);
  FUSION_PASS_CHECK(
      square_node->GetOutDataAnchor(0)->GetFirstPeerAnchor()->GetOwnerNode()->GetName() == sum_node->GetName(),
      OP_LOGI(FUSED_OP_TYPE, "sum node is not second output node of square"), return NOT_CHANGED);

  // 2.get input_node and output_node
  ge::NodePtr input_node = square_node->GetInDataAnchor(0)->GetPeerOutAnchor()->GetOwnerNode();
  FUSION_PASS_CHECK(input_node == nullptr, OP_LOGI(FUSED_OP_TYPE, "input node is null."), return NOT_CHANGED);
  ...
  for (auto in_data_anchor : sum_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(sum_node->GetOutDataAnchor(0), in_data_anchor),
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Remove sum node out data edge failed."),
                      return FAILED);
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(square_sum_v2_node->GetOutDataAnchor(0), in_data_anchor),
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Add square_sum_v2 node out data edge failed."),
                      return FAILED);
  }
  ...
  for (auto in_data_anchor : square_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    if (in_data_anchor->GetOwnerNode()->GetName() != sum_node->GetName()) {
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(square_node->GetOutDataAnchor(0), in_data_anchor),
                        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Remove square node out data edge failed."),
                        return FAILED);
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::SetBool(square_sum_v2_desc, KEEPDIMS, keep_dims),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Set keep_dims attr failed"), return FAILED);
  FUSION_PASS_CHECK(!ge::AttrUtils::SetListInt(square_sum_v2_desc, AXIS, axis),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Set axis attr failed"), return FAILED);
  ...
  if (square_node->GetInControlAnchor()) {
    for (auto out_control_anchor : square_node->GetInControlAnchor()->GetPeerOutControlAnchors()) {
      FUSION_PASS_CHECK(
          ...
          VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Add square_sum_v2 node input control edge failed."),
          return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(out_control_anchor, square_node->GetInControlAnchor()) != SUCCESS,
                        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Remove square node input control edge failed."),
                        return FAILED);
    }
  }
  if (sum_node->GetOutControlAnchor()) {
    for (auto in_control_anchor : sum_node->GetOutControlAnchor()->GetPeerInControlAnchors()) {
      FUSION_PASS_CHECK(
          ...
          VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Add sum node out control edge failed."), return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(sum_node->GetOutControlAnchor(), in_control_anchor) != SUCCESS,
                        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Remove sum node out control edge failed."),
                        return FAILED);
    }
  }
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(square_node) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Remove square node failed."), return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(sum_node) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE, "Remove sum node failed."), return FAILED);

  OP_LOGI(FUSED_OP_TYPE, "Define SquareSumV2FusionPass fusion end");
