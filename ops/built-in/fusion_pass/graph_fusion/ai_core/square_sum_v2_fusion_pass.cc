...
Status SquareSumV2FusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(sum_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "sum node is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(square_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "square_node node is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(dims.empty(), OP_LOGE(FUSED_OP_TYPE.c_str(), "input dims is empty, fusion failed."),
                    return PARAM_INVALID);
  ...
  for (auto in_data_anchor : sum_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(square_sum_v2_node->GetOutDataAnchor(0), in_data_anchor),
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "Add square_sum_v2 node out data edge failed."), return FAILED);
  }
  ...
  for (auto in_data_anchor : square_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    if (in_data_anchor->GetOwnerNode()->GetName() != sum_node->GetName()) {
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(square_node->GetOutDataAnchor(0), in_data_anchor),
                        OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove square node out data edge failed."), return FAILED);
  ...
  if (square_node->GetInControlAnchor()) {
    for (auto out_control_anchor : square_node->GetInControlAnchor()->GetPeerOutControlAnchors()) {
      FUSION_PASS_CHECK(
          ...
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Add square_sum_v2 node input control edge failed."), return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(out_control_anchor, square_node->GetInControlAnchor()) != SUCCESS,
                        OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove square node input control edge failed."), return FAILED);
    }
  }
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(sum_node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove sum node failed."),
                    return FAILED);
