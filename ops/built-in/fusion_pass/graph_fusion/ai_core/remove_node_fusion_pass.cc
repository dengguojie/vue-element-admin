/**
 * @file a_remove_useless_node_fusion_pass.cpp
 *
...
namespace fe {
REGISTER_PASS("ARemoveNodeFusionPass", BUILT_IN_GRAPH_PASS,
              RemoveNodeFusionPass);
...
Status RemoveNodeFusionPass::Fusion(ge::ComputeGraph &graph,
                                    Mapping &mapping,
...
Status RemoveNodeFusionPass::Run(ge::ComputeGraph &graph) {
  ...
  for (NodePtr node : graph.GetDirectNode()) {
    ...
    Status status =
      FusionRemoveNodeRegistry::Instance()->GetRegisterByOpType(opType, reg);
    if (status != SUCCESS) {
      continue;
    }

    vector<LinkIndexPair> linkIndexPairVec;
    function<bool(NodePtr)> preCheckFunc;
    ...
    if (preCheckFunc != nullptr) {
      if (!preCheckFunc(node)) {
        OP_LOGD(opType.c_str(), "Remove node pre-check of node[%s, %s] is not passed.",
...
Status RemoveNodeFusionPass::RemoveNode(NodePtr node,
         vector<LinkIndexPair> linkIndexPairVec,
  ...
  GetLinkedDataAnchor(node, inDataAnchorMap, outDataAnchorMap,
                      inputOutDataAnchorMap, outputInDataAnchorMap);

  // if linked inAnchor or outAnchor is empty, do nothing
  FUSION_PASS_CHECK(inDataAnchorMap.empty() || outDataAnchorMap.empty(),
           OP_LOGD(nodeType.c_str(), "Node[%s] does not link to any input nodes or output nodes.",
                   nodeName.c_str()),
           return NOT_CHANGED);
  ...
  Status status = HandleUnlinkedInAnchor(inDataAnchorMap,
                                         linkIndexPairVec, graph);
  FUSION_PASS_CHECK(status == NOT_CHANGED,
           OP_LOGD(nodeType.c_str(), "This node[%s] can not be removed "
                   "for some in data anchors can not be removed.",
                   nodeName.c_str()),
           return status);
  FUSION_PASS_CHECK(status != NOT_CHANGED && status != SUCCESS,
           OP_LOGE(nodeType.c_str(), "Fail to handle the unlinked in data anchor of node[%s].",
                   nodeName.c_str()),
           return status);

  // relink control anchor
  status = ReLinkControlAnchor(node);
  FUSION_PASS_CHECK(status != SUCCESS,
           OP_LOGE(nodeType.c_str(), "Fail to re-link ctrl edge of node[%s] while removing it.",
                   nodeName.c_str()),
           return FAILED);

  // relink data anchor
  status = ReLinkDataAnchor(linkIndexPairVec, inDataAnchorMap,
                            outDataAnchorMap, inputOutDataAnchorMap,
                            outputInDataAnchorMap);
  FUSION_PASS_CHECK(status != SUCCESS,
           OP_LOGE(nodeType.c_str(), "Fail to re-link data edge of node[%s] while removing it.",
                   nodeName.c_str()),
           return FAILED);

  // remove node
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(node),
           OP_LOGE(nodeType.c_str(), "Fail to remove Node[%s].",
                   nodeName.c_str()),
           return FAILED);

  OP_LOGI(nodeType.c_str(), "The node[%s, %s] has been removed successfully.",
          nodeName.c_str(), node->GetType().c_str());
  return SUCCESS;
}

void RemoveNodeFusionPass::GetLinkedDataAnchor(NodePtr node,
         map<uint32_t, InDataAnchorPtr>& inDataAnchorMap,
         map<uint32_t, OutDataAnchorPtr>& outDataAnchorMap,
         map<uint32_t, OutDataAnchorPtr>& inputOutDataAnchorMap,
         map<uint32_t, vector<InDataAnchorPtr>>& outputInDataAnchorMap) {
...
bool RemoveNodeFusionPass::VerifyLinkPair(
                      map<uint32_t, InDataAnchorPtr>& inDataAnchorMap,
                      map<uint32_t, OutDataAnchorPtr>& outDataAnchorMap,
                      vector<LinkIndexPair>& linkIndexPairVec) {
  ...
  if (inDataAnchorMap.size() == PASS_UINT_ONE) {
    OP_LOGD("VerifyLinkPair", "This node has only one input node,"
  ...
  for (LinkIndexPair &linkPair : linkIndexPairVec) {
    ...
    if (inAnchorIter == inDataAnchorMap.end() ||
        outAnchorIter == outDataAnchorMap.end()) {
      continue;
    }
  ...
  FUSION_PASS_CHECK(newLinkIndexPairVec.empty(),
           OP_LOGD("VerifyLinkPair", "There is no valid link index pair according to node."),
           return false);
  // update link index pair
  linkIndexPairVec = newLinkIndexPairVec;

  // check every out data anchor has inanchor
  for (outAnchorIter = outDataAnchorMap.begin();
          outAnchorIter != outDataAnchorMap.end(); outAnchorIter++) {
...
Status RemoveNodeFusionPass::HandleUnlinkedInAnchor(
                    map<uint32_t, InDataAnchorPtr>& inDataAnchorMap,
                    vector<LinkIndexPair>& linkIndexPairVec,
                    ge::ComputeGraph &graph) {
  ...
  map<uint32_t, InDataAnchorPtr>::iterator inAnchorIter;
  for (inAnchorIter = inDataAnchorMap.begin();
              inAnchorIter != inDataAnchorMap.end(); inAnchorIter++) {
    bool isLink = false;
    for (LinkIndexPair &linkPair : linkIndexPairVec) {
  ...
  for (InAnchorRelatedInfo inAnchorRelatedInfo : relatedInfoVec) {
    for (NodePtr removeNode : inAnchorRelatedInfo.removeNode) {
      ...
      FUSION_PASS_CHECK(ReLinkControlAnchor(removeNode) != SUCCESS,
               OP_LOGE("HandleUnlinkedInAnchor", "Fail to re-link control edges before removing Node[%s]"
    ...
    for (RemoveEdgePair removeEdgePair : inAnchorRelatedInfo.removeEdgeVec) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(removeEdgePair.inAnchorPtr,
                       removeEdgePair.outAnchorPtr) != ge::GRAPH_SUCCESS,
    ...
    // then remove node
    for (NodePtr removeNode : inAnchorRelatedInfo.removeNode) {
      FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(removeNode),
...
bool RemoveNodeFusionPass::VerifyRemoveInAnchor(
  ...
  FUSION_PASS_CHECK(outAnchorLinkedCount > PASS_UINT_ONE,
           OP_LOGD(nodeOfOtherEnd->GetType().c_str(), "Node[%s] cannot be removed "
  ...
  OP_LOGD(nodeOfOtherEnd->GetType().c_str(), "The node[%s] can be removed "
...
Status RemoveNodeFusionPass::ReLinkControlAnchor(NodePtr node) {
  ...
  if (inControlAnchorPtr != nullptr) {
    if (!inControlAnchorPtr->GetPeerOutControlAnchors().empty() &&
        !node->GetOutDataNodes().empty()) {
      for (OutControlAnchorPtr outControlAnchorPtr :
              inControlAnchorPtr->GetPeerOutControlAnchors()) {
        for (NodePtr outNode : node->GetOutDataNodes()) {
          if (outNode->GetInControlAnchor() == nullptr) {
            continue;
          }
          FUSION_PASS_CHECK(AddCtrlEdge(outControlAnchorPtr,
                               outNode->GetInControlAnchor()) != SUCCESS,
                   OP_LOGE(outNode->GetType().c_str(), "Fail to re-link input control edge."),
                   return FAILED);
        }
      }
    }
  ...
  if (outControlAnchorPtr != nullptr) {
    if (!outControlAnchorPtr->GetPeerInControlAnchors().empty() &&
        !node->GetInDataNodes().empty()) {
      for (InControlAnchorPtr inControlAnchorPtr :
              outControlAnchorPtr->GetPeerInControlAnchors()) {
        ...
        for (NodePtr inNode : node->GetInDataNodes()) {
          ...
          FUSION_PASS_CHECK(AddCtrlEdge(inNode->GetOutControlAnchor(),
                               inControlAnchorPtr) != SUCCESS,
                   OP_LOGE(inNode->GetType().c_str(), "Fail to re-link output control edge."),
                   return FAILED);
        }
      }
    }
...
Status RemoveNodeFusionPass::AddCtrlEdge(OutControlAnchorPtr outCtrlAnchorPtr,
  ...
  for (InControlAnchorPtr inCtrlAnchorPtr :
           outCtrlAnchorPtr->GetPeerInControlAnchors()) {
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(outCtrlAnchorPtr,
                                   inCtrlAnchorPtr) != ge::GRAPH_SUCCESS,
           OP_LOGE("AddCtrlEdge", "Fail to add ctrl edge."),
           return FAILED);
  return SUCCESS;
}

Status RemoveNodeFusionPass::ReLinkDataAnchor(
            vector<LinkIndexPair>& linkIndexPairVec,
            map<uint32_t, InDataAnchorPtr>& inDataAnchorMap,
            map<uint32_t, OutDataAnchorPtr>& outDataAnchorMap,
            map<uint32_t, OutDataAnchorPtr>& inputOutDataAnchorMap,
  ...
  for (inIter = inDataAnchorMap.begin();
                inIter != inDataAnchorMap.end(); inIter++) {
    inIter->second->UnlinkAll();
  }
  for (outIter = outDataAnchorMap.begin();
                outIter != outDataAnchorMap.end(); outIter++) {
  ...
  for (LinkIndexPair linkPair : linkIndexPairVec) {
    OP_LOGD("ReLinkDataAnchor", "Re-link data anchor [%u, %u].",
    ...
    if (inputIter == inputOutDataAnchorMap.end() ||
        outputIter == outputInDataAnchorMap.end()) {
      continue;
    }
    ...
    for (InDataAnchorPtr inAnchor : outInputDataAnchorVec) {
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(inputOutDataAnchor, inAnchor)
                       != ge::GRAPH_SUCCESS,
...
bool GatherNdPreCheck(NodePtr node) {
  ...
  if (gather_nd_input_size != 2) {
    OP_LOGW(curOpType.c_str(), "gather_nd_input_size is %u which must be 2, pre check failed",
            gather_nd_input_size);
    return false;
  }
...
REGISTER_REMOVENODE("GatherNd")
        .SetPreCheckFunc(GatherNdPreCheck)
        .AddLinkPair(0, 0);
}