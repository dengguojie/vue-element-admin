...
Status IMGWarpFusionPass::CreateConstNode(vector<int64_t>& assit_tensor_shape, ge::NodePtr& fuse_node,
  ...
  FUSION_PASS_CHECK(const_assit.get() == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "const_assit is NULL."),
  ...
  FUSION_PASS_CHECK(const_opdesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Create const op desc failed."),
                    return NOT_CHANGED);
...
Status IMGWarpFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(const_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "const_node is null."), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(add_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "add_node is null."), return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(imgwarp_offsets_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "imgwarp_offsets_node is null."),
                    return PARAM_INVALID);
  ...
  ret = CreateIMGWarpResizeNode(imgwarp_resize_node, fused_node, graph, newNodes, img_input_desc, warpoffset_input_desc,
                                output_desc);
  FUSION_PASS_CHECK(imgwarp_resize_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "imgwarp_resize_node is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge offsets_node to add node failed."),
                    return FAILED);

  // const_node->add_node
  ret = ge::GraphUtils::AddEdge(const_node->GetOutAnchor(0), add_node->GetInDataAnchor(1));
  FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge const_node to add node failed."),
                    return FAILED);

  // add_node->imgwarp_offsets_node
  ret = ge::GraphUtils::AddEdge(add_node->GetOutAnchor(0), imgwarp_offsets_node->GetInDataAnchor(1));
  FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge add_node to imgwarp_offsets_node failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge img to imgwarp_offsets_node failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge add_node to imgwarp_resize_node failed."),
                    return FAILED);
