  ...
  if (num_split > 63) {
    ...
    FUSION_PASS_CHECK(splitd_base_node == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "splitd_base_node:%s is null, fusion failed.",
    ...
    for (size_t i = 0; i < nodes_num; i++) {
      if (i < nodes_num - 1) {
        ...
        FUSION_PASS_CHECK(
            splitd_node == nullptr,
            VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "splitd_node:%s is null, fusion failed.",
                                           splitd_node->GetName().c_str()),
            return PARAM_INVALID);
        ...
        if (last_node_num_split != 1) {
          ...
          FUSION_PASS_CHECK(last_splitd_node == nullptr,
                            VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                           "last_splitd_node:%s is null, fusion failed.",
