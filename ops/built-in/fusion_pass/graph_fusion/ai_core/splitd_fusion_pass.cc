...
static const char *FUSED_NODE = "SplitD";
static const std::string PATTERN_FUSEDNODE = "FusedNodeSplitD";
/*
          input                                                             input
            |                                                                 |
...
*/
vector<FusionPattern *>SplitDFusionPass::DefinePatterns() {
  vector <FusionPattern *> patterns;
  FusionPattern *pattern = new(std::nothrow) FusionPattern("SplitDFusionPass");
...
Status SplitDFusionPass::Fusion(ge::ComputeGraph &graph,
                              Mapping &mapping,
  ...
  if (num_split > 63) {
    ...
    fusionNodes.push_back(splitd_base_node);
    ge::AttrUtils::SetListInt(splitd_base_node->GetOpDesc(), "size_splits",
                              size_splits_new);
    ...
    FUSION_PASS_CHECK(splitd_base_node == nullptr,
            OP_LOGE(FUSED_OP_TYPE.c_str(), "splitd_base_node:%s is null, fusion failed.",
            splitd_base_node->GetName().c_str()), return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(0)->GetPeerOutAnchor(),
            ...
            fused_node->GetName().c_str(), (0),
            splitd_base_node->GetName().c_str(), 0), return FAILED);
    ...
    for (size_t i = 0; i < nodes_num; i++) {
      if (i<nodes_num-1) {
        ...
        fusionNodes.push_back(splitd_node);
        ...
        ge::AttrUtils::SetListInt(splitd_node->GetOpDesc(), "size_splits",
                                  size_splits_new2);
        ...
        FUSION_PASS_CHECK(splitd_node == nullptr,
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "splitd_node:%s is null, fusion failed.",
                  splitd_node->GetName().c_str()), return PARAM_INVALID);

        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(splitd_base_node->GetOutDataAnchor(i),
                splitd_node->GetInDataAnchor(0)),
                OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                splitd_base_node->GetName().c_str(), i,
                splitd_node->GetName().c_str(), i),  return FAILED);

        for (size_t m=0; m<63; m++) {
          for (InDataAnchorPtr inAnchorPtr :
              fused_node->GetOutDataAnchor(63*i+m)->GetPeerInDataAnchors()) {
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(fused_node->GetOutDataAnchor(63*i+m), inAnchorPtr),
        ...
        if (last_node_num_split != 1) {
          ...
          fusionNodes.push_back(last_splitd_node);
          ...
          ge::AttrUtils::SetInt(last_splitd_node->GetOpDesc(), "num_split",
                                last_node_num_split);
          ge::AttrUtils::SetListInt(last_splitd_node->GetOpDesc(), "size_splits",
                                    size_splits_new3);
          ...
          FUSION_PASS_CHECK(last_splitd_node == nullptr,
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "last_splitd_node:%s is null, fusion failed.",
                  last_splitd_node->GetName().c_str()), return PARAM_INVALID);

          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(splitd_base_node->GetOutDataAnchor(i),
                  last_splitd_node->GetInDataAnchor(0)),
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                  splitd_base_node->GetName().c_str(), i,
                  last_splitd_node->GetName().c_str(), i),  return FAILED);
          ...
          for (InDataAnchorPtr inAnchorPtr :
                fused_node->GetOutDataAnchor(63*i)->GetPeerInDataAnchors()) {
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "SplitD --> SplitD or SplitVD fusion SUCCESSS!!!!!");
  return SUCCESS;
}

REGISTER_PASS("ZSplitDFusionPass", BUILT_IN_GRAPH_PASS, SplitDFusionPass);
}
