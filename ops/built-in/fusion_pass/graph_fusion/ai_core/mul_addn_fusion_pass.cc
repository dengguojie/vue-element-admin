...
Status MulAddNPass::Fusion(ge::ComputeGraph &graph,
                                           Mapping &mapping,
  ...
  FUSION_PASS_CHECK(addn_node == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "addn_node is null, fusion failed."),
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGI(FUSED_OP_TYPE.c_str(),
          "Get the addn_node type is [%s], which is not AddN.",
  ...
  FUSION_PASS_CHECK(!find_addn,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "output node of Mul node is not AddN."),
           return NOT_CHANGED);
...
REGISTER_PASS("MulAddNPass", BUILT_IN_GRAPH_PASS,
              MulAddNPass);
}  // namespace fe
