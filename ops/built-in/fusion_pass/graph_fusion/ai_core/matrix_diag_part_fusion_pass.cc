...
namespace fe {
  ...
  vector<FusionPattern*> MatrixDiagPartFusionPass::DefinePatterns() {
    ...
    FusionPattern *pattern = new (std::nothrow)
                             FusionPattern("MatrixDiagPartFusionPass");
    FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new a pattern object failed."),
             return patterns);

    // define origin graph
    pattern->AddOpDesc(PATTERN_MATRIXDIAGPART, {MATRIXDIAGPART})
      .SetOutput(PATTERN_MATRIXDIAGPART);
