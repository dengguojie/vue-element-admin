...
Status LayerNormFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(squared0_node == nullptr,
                    ...
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(sub0_node == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "sub0_node is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(add0_node == nullptr,
                    ...
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(rsqrt0_node == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "rsqrt0_node is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (PatternFusionUtil::IsUnknownShape(input_dims[dims_size - 1]) || PatternFusionUtil::IsUnknownShape(mul0_dims[0])) {
    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "LayerNormFusionPass cannot be applied for unknown shape.");
    return NOT_CHANGED;
  }
  ...
  FUSION_PASS_MAKE_SHARED(
      (layer_desc = std::make_shared<ge::OpDesc>(add1_node->GetName() + "/" + LAYERNORM, LAYERNORM)), return FAILED);
  FUSION_PASS_CHECK(layer_desc == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "layer_desc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(sub0_node) != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Remove sub0_node failed."), return FAILED);
