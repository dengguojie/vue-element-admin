...
bool Concatv2dFusionPass::CheckConcatValid(const ge::NodePtr& node, const ge::Format format, const ge::GeShape shape,
  ...
  OP_LOGD(FUSED_OP_TYPE.c_str(), "%s 's concat_dim is %ld, target is %ld.", node->GetName().c_str(),
          concatDim, dimNum);
...
Status Concatv2dFusionPass::PatternParse(ge::NodePtr concatv2dNode, vector<ge::NodePtr>& fusedInputNodes,
  ...
  int32_t directOutNodeNum = concatv2dNode->GetOutAllNodes().size();
...
Status Concatv2dFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(concatv2dNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(
                    FUSED_OP_TYPE.c_str(), "new a pattern object failed"), return PARAM_INVALID);
  ...
  for (auto inputDesc : fusedConcatv2dOpDesc->GetAllInputsDesc()) {
    FUSION_PASS_CHECK(!ge::OpDescUtils::ClearInputDesc(fusedConcatv2dOpDesc, 0),
                      OP_LOGI(FUSED_OP_TYPE.c_str(), "Node:%s's clear %d th input failed.",
                              fusedConcatv2dOpDesc->GetName().c_str(), 0),
                      return PARAM_INVALID);
  }

  size_t input_idx = 0;
  for (auto inputNode : fusedInputNodes) {
    uint32_t GetIndex = inputNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().at(0)->GetIdx();
    ge::GeTensorDesc desc = inputNode->GetOutAllNodes().at(0)->GetOpDesc()->GetInputDesc(GetIndex);
  ...
  for (auto outAnchor : concatv2dNode->GetAllOutDataAnchors()) {
    for (InDataAnchorPtr inAnchorPtr : outAnchor->GetPeerInDataAnchors()) {
      ...
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedConcatv2dNode->GetOutDataAnchor(0), inAnchorPtr),
                        VECTOR_FUSION_INNER_ERR_REPORT(
                        FUSED_OP_TYPE.c_str(), "Add edge from %s to fusion node %s's %d th failed.",
                        concatv2dNode->GetName().c_str(), fusedConcatv2dNode->GetName().c_str(), 0),
                        return FAILED);
