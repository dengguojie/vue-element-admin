...
vector<FusionPattern *>Concatv2dFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new(std::nothrow) FusionPattern("Concatv2dFusionPass");
  FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new a pattern object failed."),
           return patterns);
  pattern->AddOpDesc(PATTERN_FUSEDNODE, {CONCATV2D})
...
bool Concatv2dFusionPass::CheckConcatValid(ge::NodePtr node, ge::Format format,
                             ge::GeShape shape, int32_t dimNum) {
  int32_t concatDim = 0;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetInt(node->GetOpDesc(),
                                  ATTR_CONCAT_DIM, concatDim),
           OP_LOGI(FUSED_OP_TYPE.c_str(), "There is no concat_dim attr."),
           return false);
  ...
  OP_LOGD(FUSED_OP_TYPE.c_str(), "%s 's concat_dim is %ld, target is %ld.",
          node->GetName().c_str(), concatDim, dimNum);
  ...
  OP_LOGD(FUSED_OP_TYPE.c_str(), "%s 's input format is %s, target is %s.",
          node->GetName().c_str(),
          ge::TypeUtils::FormatToSerialString(inputFormat).c_str(),
          ge::TypeUtils::FormatToSerialString(format).c_str());
  ...
  FUSION_PASS_CHECK(node->GetOutAllNodes().size() != 1,
          OP_LOGD(FUSED_OP_TYPE.c_str(), "%s 's output should be 1, can,t fuss.",
                  node->GetName().c_str()),
          return false);
  return true;
}

Status Concatv2dFusionPass::PatternParse(ge::NodePtr concatv2dNode,
  ...
  FUSION_PASS_CHECK(directOutNodeNum <= 0,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "There is no need to fusion, out node num %d.",
                   directOutNodeNum),
           return FAILED);
  int32_t concatDim = 0;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetInt(concatv2dNode->GetOpDesc(),
                                  ATTR_CONCAT_DIM, concatDim),
           OP_LOGI(FUSED_OP_TYPE.c_str(), "There is no need to fusion."),
           return FAILED);
  ...
  for (auto nodePtr : concatv2dNode->GetInAllNodes()) {
    if (nodePtr->GetType() == CONCATV2D) {
      ...
      for (auto preNode : nodePtr->GetInAllNodes()) {
        FUSION_PASS_CHECK(preNode->GetOutAllNodes().size() != 1,
                 OP_LOGD(FUSED_OP_TYPE.c_str(), "%s 's output should be 1, can,t fuss.",
                         preNode->GetName().c_str()),
                 return FAILED);
        fusedInputNodes.push_back(preNode);
      }
    } else {
      FUSION_PASS_CHECK(nodePtr->GetOutAllNodes().size() != 1,
              OP_LOGD(FUSED_OP_TYPE.c_str(), "%s 's output should be 1, can,t fuss.",
  ...
  FUSION_PASS_CHECK(!concatNodes.size(),
          OP_LOGD(FUSED_OP_TYPE.c_str(), "Singel concat, no need fusion."),
          return FAILED);
  return SUCCESS;
}

Status Concatv2dFusionPass::Fusion(ge::ComputeGraph &graph,
                              Mapping &mapping,
                              vector<ge::NodePtr> &fusionNodes) {
  ge::NodePtr concatv2dNode = GetNodeFromMapping(PATTERN_FUSEDNODE, mapping);

  FUSION_PASS_CHECK(concatv2dNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new a pattern object failed"),
  ...
  ge::OpDescPtr fusedConcatv2dOpDesc =
      AttrUtils::CloneOpDesc(concatv2dNode->GetOpDesc());
  OP_LOGD(FUSED_OP_TYPE.c_str(), "fusedConcatv2dOpDesc %s, optye %s, input %ld, output %ld",
          fusedConcatv2dOpDesc->GetName().c_str(),
          fusedConcatv2dOpDesc->GetType().c_str(),
          fusedConcatv2dOpDesc->GetAllInputsDesc().size(),
          fusedConcatv2dOpDesc->GetAllOutputsDesc().size());
  FUSION_PASS_CHECK(fusedConcatv2dOpDesc == nullptr,
  ...
  for (auto inputNode : fusedInputNodes) {
    uint32_t index =
        inputNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().at(0)->GetIdx();
    ge::GeTensorDesc desc =
        inputNode->GetOutAllNodes().at(0)->GetOpDesc()->GetInputDesc(index);
  ...
  for (auto inputNode : fusedInputNodes) {
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(inputNode->GetOutDataAnchor(0),
             ...
             OP_LOGI(FUSED_OP_TYPE.c_str(), "add input edge between %s and %s %d th failed.",
                     ...
                     fusedConcatv2dNode->GetName().c_str(), index),
             return NOT_CHANGED);
    index++;
  }
  ...
  for (auto outAnchor : concatv2dNode->GetAllOutDataAnchors()) {
    for (InDataAnchorPtr inAnchorPtr : outAnchor->GetPeerInDataAnchors()){
      ...
      FUSION_PASS_CHECK(SUCCESS !=
          ge::GraphUtils::AddEdge(fusedConcatv2dNode->GetOutDataAnchor(index),
                                  inAnchorPtr),
  ...
  for (auto concatNode : concatNodes) {
    FUSION_PASS_CHECK(graph.RemoveNode(concatNode) == ge::GRAPH_FAILED,
            OP_LOGE(FUSED_OP_TYPE.c_str(), "remove node %s failed.", concatNode->GetName().c_str()),
            return FAILED);
  }
  return SUCCESS;
}
