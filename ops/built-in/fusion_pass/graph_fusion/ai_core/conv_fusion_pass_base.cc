...
namespace fe {
Status ConvFusionPassBase::DoFusion(ge::ComputeGraph &graph,
                                    ge::NodePtr convNode, ge::NodePtr destNode,
  ...
  FUSION_PASS_CHECK(removeNodeRet != SUCCESS,
           OP_LOGE(convNode->GetType().c_str(), "ConvNode[%s]: remove the destNode failed.",
                   convNodeName.c_str()),
...
Status ConvFusionPassBase::GetConvFilterInputIndex(const ge::NodePtr &convNode,
                                                   int &filterInputIdx) {
  for (ge::InDataAnchorPtr inDataAnchorPtr : convNode->GetAllInDataAnchors()) {
    ...
    if (nodePtr->GetType() == CONSTANT || nodePtr->GetType() == CONSTANTOP ||
        nodePtr->GetType() == CONVBNFILTERHOST ||
        nodePtr->GetType() == GROUPPADDING ||
...
Status ConvFusionPassBase::GetAllConstInput(
    const ge::NodePtr &node, vector<ge::GeTensorDesc> &conv2dInputs,
  ...
  for (ge::InDataAnchorPtr inDataAnchorPtr : node->GetAllInDataAnchors()) {
    ...
    ge::NodePtr nodePtr = inDataAnchorPtr->GetPeerOutAnchor()->GetOwnerNode();
    bool checkConstNode = nodePtr->GetType() == CONSTANT ||
                          nodePtr->GetType() == CONSTANTOP ||
                          nodePtr->GetType() == CONVBNFILTERHOST ||
                          nodePtr->GetType() == CONVBNBIASHOST ||
                          nodePtr->GetType() == GROUPPADDING ||
                          nodePtr->GetType() == CONCATHOSTOP;
    if (checkConstNode) {
      if (inDataAnchorPtr->GetIdx() == 0) {
      ...
      conv2dInputs.push_back(
          node->GetOpDesc()->GetInputDesc(inDataAnchorPtr->GetIdx()));
      string inputName =
          node->GetOpDesc()->GetInputNameByIndex(inDataAnchorPtr->GetIdx());
      ...
      if (node->GetType() == CONV2D || node->GetType() == DEPTHWISECONV2D ||
          node->GetType() == CONV3D) {
        conv2dInputsName.push_back("Conv_" + inputName);
      } else {
        conv2dInputsName.push_back(node->GetOpDesc()->GetType() + "_" +
                                   inputName);
      }
      ...
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(inDataAnchorPtr->GetPeerOutAnchor(),
                                          inDataAnchorPtr) != SUCCESS,
...
Status ConvFusionPassBase::AddBiasNode(ge::ComputeGraph &graph,
                                       ge::NodePtr &convNode) {
  // if the former conv has no bias, newBiasData = transBias
  ge::OpDescPtr constOpDesc = std::make_shared<ge::OpDesc>(
      convNode->GetName() + "_" + "bias", CONSTANT);
  ...
  FUSION_PASS_MAKE_SHARED((biasPtr = std::make_shared<ge::GeTensor>(
                      constOutDesc, (uint8_t *)0, sizeof(float))),
                 biasPtr = nullptr;
                 return PARAM_INVALID);
  ge::GeShape biasShape({1});
  biasPtr->MutableTensorDesc().SetShape(biasShape);
  FUSION_PASS_CHECK(constNode == nullptr, OP_LOGE("AddBiasNode", "constNode is nullptr"),
           return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(res != SUCCESS,
           OP_LOGE(convNode->GetType().c_str(), "ConvNode[%s]: add edge between new const node and conv "
                   "node failed!",
                   convNode->GetName().c_str()),
           return res);
...
Status ConvFusionPassBase::GetConvKernelIndex(ge::OpDescPtr convOpdesc,
    const ge::GeTensorDesc &constInputDesc, ge::Format &filterFormat,
    size_t &kernerlIndex) {
