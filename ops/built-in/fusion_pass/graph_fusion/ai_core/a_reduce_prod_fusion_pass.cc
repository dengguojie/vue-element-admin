...
Status CheckProdFussionOrNot(vector<int64_t> tensor_info,
...
vector<FusionPattern *> AReduceProdFusionPass::DefinePatterns() {
  ...
  new (std::nothrow) FusionPattern("AReduceProdFusionPass");
...
Status AReduceProdFusionPass::Fusion(ge::ComputeGraph &graph,
  ...
  FUSION_PASS_CHECK(prodNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "prodNode is null, fusion failed."),
           return PARAM_INVALID);

  ge::GeTensorDesc tensor_input = \
  prodNode->GetOpDesc()->GetInputDesc(0);
  ge::GeTensorDesc axis_input = \
  prodNode->GetOpDesc()->GetInputDesc(1);
  ...
  for (auto inDataAnchor :
       prodNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(prodNode->GetOutDataAnchor(0),
                                        inDataAnchor) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove prod and outnode edge failed."), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(prodNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                                     inDataAnchor) != SUCCESS,
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(prodNode) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove prodNode failed."), return FAILED);
...
REGISTER_PASS("AReduceProdFusionPass", BUILT_IN_GRAPH_PASS,
              AReduceProdFusionPass);
}
