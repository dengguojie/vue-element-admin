...
namespace fe {
const std::string MinToMinimumPass::PATTERN_FUSEDNODE = "MinN";
const int MinToMinimumPass::MIN_INPUT_SIZE = 2;
vector<FusionPattern*> MinToMinimumPass::DefinePatterns() {
  vector<FusionPattern*> patterns;
  FusionPattern* pattern =
      (new (std::nothrow) FusionPattern("MinToMinimumPass"));
  FUSION_PASS_CHECK(pattern == nullptr,
                    OP_LOGE("MinToMinimumPass", "new pattern error"),
                    return patterns);
  pattern->AddOpDesc(PATTERN_FUSEDNODE, {"MinN"})
      .SetOutput(PATTERN_FUSEDNODE);
  patterns.push_back(pattern);
  return patterns;
}

Status MinToMinimumPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
                                vector<ge::NodePtr>& fusion_nodes) {
  ge::NodePtr fused_node = GetNodeFromMapping(PATTERN_FUSEDNODE, mapping);
  FUSION_PASS_CHECK(fused_node == nullptr,
                    OP_LOGE("MinToMinimumPass", "Fusion GetNode Error"),
                    return PARAM_INVALID);
  ge::OpDescPtr op_desc = fused_node->GetOpDesc();
  FUSION_PASS_CHECK(op_desc == nullptr,
                    OP_LOGE("MinToMinimumPass", "Fusion GetOpdesc Error"),
                    return PARAM_INVALID);

  int input_size = op_desc->GetInputsSize();
  if (input_size < MIN_INPUT_SIZE) {
    OP_LOGW("MinToMinimumPass", "input_size less %d", MIN_INPUT_SIZE);
    return PARAM_INVALID;
  }
  GetDataFormatAndType(fused_node);
  auto ret = FusionNode(graph, fused_node, fusion_nodes, input_size);
  FUSION_PASS_CHECK(ret != SUCCESS,
                    OP_LOGE("MinToMinimumPass", "FusionNode FAIL"),
                    return FAILED);
  ret = RemoveFusedNode(graph, fused_node);
  FUSION_PASS_CHECK(ret != SUCCESS,
                    OP_LOGE("MinToMinimumPass", "AddEdgeForList FAIL"),
                    return FAILED);
  return SUCCESS;
}

Status MinToMinimumPass::FusionNode(ge::ComputeGraph& graph,
  ...
  FUSION_PASS_CHECK(
      root_node == nullptr,
      OP_LOGE("MinToMinimumPass", "Fusion Create root node Error"),
      return PARAM_INVALID);
  fusion_nodes.push_back(root_node);
  auto ret = AddEdgeForRoot(fused_node, root_node);
  FUSION_PASS_CHECK(ret != SUCCESS,
                    OP_LOGE("MinToMinimumPass", "AddEdgeForRoot FAIL"),
                    return PARAM_INVALID);
  curr_node = root_node;
  for (int i = 2; i < input_size; ++i) {
    ...
    FUSION_PASS_CHECK(
        new_node == nullptr,
        OP_LOGE("MinToMinimumPass", "Fusion Create new node Error"),
        return PARAM_INVALID);
    fusion_nodes.push_back(new_node);
    ret = UpdateDescForList(fused_node, new_node, curr_node, i);
    FUSION_PASS_CHECK(ret != SUCCESS,
                      OP_LOGE("MinToMinimumPass", "UpdateDescForList FAIL"),
                      return PARAM_INVALID);
...
Status MinToMinimumPass::CreateNode(ge::ComputeGraph& graph,
                                    ge::NodePtr& new_node,
                                    const int index) const {
  ge::OpDescPtr new_desc = nullptr;
  std::string name = "Minimum_Fuss_" + to_string(index);
  FUSION_PASS_MAKE_SHARED(
      (new_desc = std::make_shared<ge::OpDesc>(name, "Minimum")),
      return INTERNAL_ERROR);
  auto ret =
      new_desc->AddInputDesc(GeTensorDesc(GeShape(), data_format, data_type));
  FUSION_PASS_CHECK(ret != SUCCESS,
                    OP_LOGE("MinToMinimumPass", "AddInputDesc first FAIL"),
                    return FAILED);
  ret = new_desc->AddInputDesc(GeTensorDesc(GeShape(), data_format, data_type));
  FUSION_PASS_CHECK(ret != SUCCESS,
                    OP_LOGE("MinToMinimumPass", "AddInputDesc second FAIL"),
                    return FAILED);

  ret =
      new_desc->AddOutputDesc(GeTensorDesc(GeShape(), data_format, data_type));
  FUSION_PASS_CHECK(ret != SUCCESS,
                    OP_LOGE("MinToMinimumPass", "AddOutputDesc FAIL"),
                    return FAILED);
  new_node = graph.AddNode(new_desc);
  return SUCCESS;
}

Status MinToMinimumPass::AddEdgeForRoot(ge::NodePtr& fused_node,
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE("MinToMinimumPass", "AddEdgeForRoot first error"), return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE("MinToMinimumPass", "AddEdgeForRoot second error"),
      return FAILED);
  return SUCCESS;
}

Status MinToMinimumPass::AddEdgeForList(ge::NodePtr& fused_node,
                                        ge::NodePtr& new_node,
                                        ge::NodePtr& curr_node,
                                        const int index) const {
  if (index < 0) {
    OP_LOGE("MinToMinimumPass", "AddEdgeForList index less 0");
    return FAILED;
  }
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE("MinToMinimumPass", "AddEdgeForList first error"), return FAILED);
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(curr_outdata_anchor,
                    ...
                    OP_LOGE("MinToMinimumPass", "AddEdgeForList second error"),
                    return FAILED);
  return SUCCESS;
}

Status MinToMinimumPass::AddEdgeForEnd(ge::NodePtr& fused_node,
  ...
  for (auto in_data_anchor : out_data_anchor->GetPeerInDataAnchors()) {
    if (in_data_anchor != nullptr) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(out_data_anchor,
                        ...
                        OP_LOGE("MinToMinimumPass", "RemoveEdge fail"),
                        return FAILED);
      FUSION_PASS_CHECK(
          ge::GraphUtils::AddEdge(out_data_anchor_end, in_data_anchor) !=
              SUCCESS,
          OP_LOGE("MinToMinimumPass", "AddEdgeForEnd second error"),
          return FAILED);
    }
  }
  return SUCCESS;
...
Status MinToMinimumPass::RemoveFusedNode(ge::ComputeGraph& graph,
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(fused_node) != SUCCESS,
                    OP_LOGE("MinToMinimumPass", "RemoveFusedNode error"),
                    return FAILED);
  return SUCCESS;
}

void MinToMinimumPass::GetDataFormatAndType(ge::NodePtr& fused_node) {
...
Status MinToMinimumPass::UpdateDescForRoot(ge::NodePtr& fused_node,
                                           ge::NodePtr& new_node) const {
  Operator op = ge::OpDescUtils::CreateOperatorFromNode(fused_node);
  auto input_desc = op.GetDynamicInputDesc("x", 0);
  ...
  FUSION_PASS_CHECK(
      op_desc == nullptr,
      OP_LOGE("MinToMinimumPass", "UpdateDescForRoot GetOpDesc error"),
      return FAILED);
...
Status MinToMinimumPass::UpdateDescForList(ge::NodePtr& fused_node,
                                           ge::NodePtr& new_node,
                                           ge::NodePtr& curr_node,
                                           const int index) const {
  Operator op = ge::OpDescUtils::CreateOperatorFromNode(fused_node);
  ...
  FUSION_PASS_CHECK(
      op_desc == nullptr,
      OP_LOGE("MinToMinimumPass", "UpdateDescForList GetOpDesc error"),
      return FAILED);
  ge::GeTensorDesc new_input_desc = op_desc->GetInputDesc(0);
  ge::GeTensorDesc new_input_desc1 = op_desc->GetInputDesc(1);
  ge::GeTensorDesc new_output_desc = op_desc->GetOutputDesc(0);
...
void MinToMinimumPass::GetOutputShape(std::vector<int64_t>& dims,
...
REGISTER_PASS("MinToMinimumPass", BUILT_IN_GRAPH_PASS, MinToMinimumPass);
}  //  namespace fe
