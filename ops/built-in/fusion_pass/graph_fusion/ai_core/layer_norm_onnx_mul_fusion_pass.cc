...
Status LayerNormONNXMULFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  if ((mul1_node == nullptr) && (add1_node == nullptr)) {
    ...
    OP_LOGW(FUSED_OP_TYPE.c_str(), "Either mul1_node or add1_node is null, fusion failed.");
    return NOT_CHANGED;
  }
  ...
  DataType dtype = op_add0.GetInputDesc("x2").GetDataType();
  std::vector<float> add0_vec;
  const uint8_t* add0_data = add0_const.GetData();
  if (add0_data == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "const data of add0 node is null.");
    return GRAPH_FAILED;
  }
  ...
  if (!with_affine) {
    FUSION_PASS_CHECK(SUCCESS != CreateMulAndAddNode(graph, div0_node, mul1_node,
                                                      add1_node, fusionNodes),
                      OP_LOGW(FUSED_OP_TYPE.c_str(), "Fail to create mul1_node and add1_node, fusion failed."),
                      return NOT_CHANGED);
  }
  // copy Opdesc
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(mul0_node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove mul0_node failed."),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(mean1_node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove mean1_node failed."),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(add0_node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove add0_node failed."),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(rsqrt0_node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove rsqrt0_node failed."),
                    return FAILED);
