...
namespace fe {
static const string PATTERN_PASSTHROUGH = "PassThroughSecondFusion";
static const char *PASS_THROUGH_NODE = "PassThrough";
static const char *SPACE_TO_DEPTH_NODE = "SpaceToDepth";

Status PassThroughSecondFusionPass::SetNodeAttrAndOpDesc(ge::OpDescPtr &curOpDesc,
                            const ge::OpDescPtr &oriOpDesc) {
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::GetInt(oriOpDesc, "stride", stride),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Can not get passthrough stride attr failed."), return FAILED);
...
Status PassThroughSecondFusionPass::InsertCurNode(ge::ComputeGraph &graph, ge::NodePtr oriNode) {
  ...
  FUSION_PASS_CHECK(SUCCESS != UnlinkEdge(oriNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Unlink OriNode Edge failed."), return FAILED);
  ...
  FUSION_PASS_CHECK(SUCCESS != SetNodeAttrAndOpDesc(curOpDesc, oriOpDesc),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Set Node[%s] Attr and OpDesc failed.", curNodeName.c_str()), return FAILED);

  curNode = graph.AddNode(curOpDesc);

  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(oriBottomPeerAnchorPtr0,
                                              curNode->GetInDataAnchor(0)),
            OP_LOGE(FUSED_OP_TYPE.c_str(), "add edge from src node[%s] to dst node[%s] anchor 0 failed.",
                    inputNode->GetName().c_str(), curNodeName.c_str()),
            return FAILED);

  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(oriBottomPeerAnchorPtr1,
                                              curNode->GetInDataAnchor(1)),
            OP_LOGE(FUSED_OP_TYPE.c_str(), "add edge from src node[%s] to dst node[%s] anchor 1 failed.",
  ...
  for (uint64_t i = 0; i < oriTopPeerAnchors.size(); i++) {
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(curNode->GetOutDataAnchor(0),
...
Status PassThroughSecondFusionPass::RemoveThisNode(ge::ComputeGraph &graph, ge::NodePtr thisNode) {
  ...
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(thisNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "remove origin node[%s] node failed",
                   thisNode->GetName().c_str()), return FAILED);
...
Status PassThroughSecondFusionPass::RemoveWeightNode(ge::ComputeGraph &graph, ge::NodePtr oriNode) {
  ...
  while(curNode->GetType() != "Constant" && curNode->GetType() != "Const") {
    ...
    FUSION_PASS_CHECK(SUCCESS != UnlinkEdge(curNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Unlink bottomNode Edge failed."), return FAILED);
    FUSION_PASS_CHECK(SUCCESS != RemoveThisNode(graph, curNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove bottomNode failed."), return FAILED);
    curNode = bottomNode;
  }
  FUSION_PASS_CHECK(SUCCESS != UnlinkEdge(curNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Unlink Weight Node Edge failed."), return FAILED);
  FUSION_PASS_CHECK(SUCCESS != RemoveThisNode(graph, curNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove Weight Node failed."), return FAILED);
...
vector<FusionPattern *> PassThroughSecondFusionPass::DefinePatterns() {
  ...
  FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new a pattern object failed."),
           return patterns);
  // define origin graph
  pattern->AddOpDesc(PATTERN_PASSTHROUGH, {PASS_THROUGH_NODE})
          .SetOutput(PATTERN_PASSTHROUGH);
...
Status PassThroughSecondFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping,
                                           vector<ge::NodePtr> &newNodes)
{
  ...
  FUSION_PASS_CHECK(passThroughNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "passThroughNode is null, fusion failed."),
  ...
  FUSION_PASS_CHECK(passThroughDesc->GetInputsSize() == 1,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Passthrough ops not changed, dot not fusion."),
           return NOT_CHANGED);
  ...
  if (passThroughDesc->GetInputDesc(0).GetDataType() != DT_FLOAT16) {
    ...
    FUSION_PASS_CHECK(SUCCESS != RemoveThisNode(graph, passThroughNode),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove OriNode failed."), return FAILED);
  }
