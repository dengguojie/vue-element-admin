...
Status PassThroughSecondFusionPass::InsertCurNode(ge::ComputeGraph& graph, ge::NodePtr oriNode) {
  ...
  FUSION_PASS_CHECK(SUCCESS != UnlinkEdge(oriNode), OP_LOGE(FUSED_OP_TYPE.c_str(), "Unlink OriNode Edge failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(SUCCESS != SetNodeAttrAndOpDesc(curOpDesc, oriOpDesc),
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "Set Node[%s] Attr and OpDesc failed.", curNodeName.c_str()),
                    return FAILED);
...
Status PassThroughSecondFusionPass::RemoveThisNode(ge::ComputeGraph& graph, ge::NodePtr thisNode) {
  ...
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(thisNode),
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "remove origin node[%s] node failed", thisNode->GetName().c_str()),
                    return FAILED);
...
Status PassThroughSecondFusionPass::RemoveWeightNode(ge::ComputeGraph& graph, ge::NodePtr oriNode) {
  ...
  while (nodeType != "Constant" && nodeType != "Const") {
    ...
    FUSION_PASS_CHECK(SUCCESS != UnlinkEdge(curNode), OP_LOGE(FUSED_OP_TYPE.c_str(), "Unlink bottomNode Edge failed."),
                      return FAILED);
    FUSION_PASS_CHECK(SUCCESS != RemoveThisNode(graph, curNode),
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove bottomNode failed."), return FAILED);
    curNode = bottomNode;
  }
  FUSION_PASS_CHECK(SUCCESS != UnlinkEdge(curNode), OP_LOGE(FUSED_OP_TYPE.c_str(), "Unlink Weight Node Edge failed."),
                    return FAILED);
  FUSION_PASS_CHECK(SUCCESS != RemoveThisNode(graph, curNode),
