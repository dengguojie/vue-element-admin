...
static const std::string PATTERN_FUSEDNODE = "Permute";
...
Status PermuteFusionPass::Fusion(ge::ComputeGraph &graph,
                              Mapping &mapping,
  ...
  if(inputShape.GetDimNum() == NUM_4){
    ...
    if(permList.size() == NUM_4 && permList[0] == NUM_0 && permList[1] == NUM_3 && permList[2] == NUM_2 && permList[3] == NUM_1) {
      checkPass = true;
    }
    ...
    if(checkPass){
      ...
      ge::NodePtr permuteNodeNew = graph.AddNode(permuteOpDescNew);
      ...
      if (permuteNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
        for (InDataAnchorPtr inAnchorPtr : permuteNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
          ...
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(permuteNodeNew->GetOutDataAnchor(0),
            inAnchorPtr),
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0] failed.", permuteNodeNew->GetName().c_str(), permuteNode->GetName().c_str()),
            return FAILED);
          OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0].", permuteNodeNew->GetName().c_str(), permuteNode->GetName().c_str());
        }
      }

      // connect the output 0 of permuteNode to input 0 of permuteNodeNew
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(permuteNode->GetOutDataAnchor(0), permuteNodeNew->GetInDataAnchor(0)),
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[%d] to fusion node:%s's input[%d] failed.", permuteNode->GetName().c_str(), 0, permuteNodeNew->GetName().c_str(), 0),
        return FAILED);
      OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[%d] to fusion node:%s's input[%d].", permuteNode->GetName().c_str(), 0, permuteNodeNew->GetName().c_str(), 0);
      ...
      if (permuteNode->GetOutControlAnchor() != nullptr) {
        ...
        for (unsigned int i = 0; i < permuteNode->GetOutControlAnchor()->GetPeerInControlAnchors().size(); i++) {
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(permuteNodeNew->GetOutControlAnchor(),
            permuteNode->GetOutControlAnchor()->GetPeerInControlAnchors().at(i)),
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's control index to fusion node:%s's control index[%d] failed.", permuteNodeNew->GetName().c_str(), permuteNode->GetName().c_str(), i),
            return FAILED);
          OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's control index to fusion node:%s's control index[%d].", permuteNodeNew->GetName().c_str(), permuteNode->GetName().c_str(), i);
        }
        for (auto inControlAnchor : permuteNode->GetOutControlAnchor()->GetPeerInControlAnchors()){
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::RemoveEdge(permuteNode->GetOutControlAnchor(),
            inControlAnchor),
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove edge from fused node:%s's output control failed.", permuteNode->GetName().c_str()),
            return FAILED);
          OP_LOGD(FUSED_OP_TYPE.c_str(), "Remove edge from fused node:%s's output control index.", permuteNode->GetName().c_str());
        }
      }
  ...
  std::string PermutePassName = "PermuteFusionPass";
