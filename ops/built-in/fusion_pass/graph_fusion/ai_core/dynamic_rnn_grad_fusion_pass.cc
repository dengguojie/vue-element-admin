...
vector<vector<ge::NodePtr>> DynamicRNNGradFusionPass::AddTLoopNode(ge::NodePtr dynamicRNNGradNode,
  ...
  for (int64_t i = 0; i < t_size; i++) {
    ...
    if (i != 0) {
      ...
      ge::GeTensorDesc dh_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(9).GetShape(),
    ...
    ge::GeTensorDesc dc_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(10).GetShape(),
                                                             ge::FORMAT_ND, ge::DT_FLOAT);
    ...
    // add reshape
    if (n_value % 16 != 0) {
      auto reshapeOp = ge::OperatorFactory::CreateOperator(dynamicRNNGradNode->GetName() + "/cellReshape" +
                                                           std::to_string(i), "Reshape");
      ...
      vector<int64_t> outputTensorDescCellDgateDims = {1, (output_dims[1] + 15) / 16, (output_dims[0] + 15) / 16,
                                                       16, 16};
    ...
    FUSION_PASS_CHECK(basicLstmCellStateGradNode == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.",
                                                     basicLstmCellStateGradDesc->GetName().c_str()), failStatus=true);
    ...
    FUSION_PASS_CHECK(matmulNode == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.",
                                                     lstmBatchMatMulDesc->GetName().c_str()), failStatus=true);
    ...
    FUSION_PASS_CHECK(splitNode == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(
                        FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.",
                        lstmSplitDesc->GetName().c_str()), failStatus=true);
...
Status DynamicRNNGradFusionPass::AddEdgeForCell(ge::NodePtr dynamicRNNGradNode, ge::ComputeGraph& graph,
  ...
  FUSION_PASS_CHECK(resultNode.empty(),
                    ...
                    failStatus=true);
  FUSION_PASS_CHECK(resultNode.size() != 4,
  ...
  for (int64_t i = 0; i < num_split_x; i++){
    ...
    if (i == num_split_x - 1) {
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(
                                     dynamicRNNGradNode->GetInDataAnchor(5)->GetPeerOutAnchor(),
...
ge::NodePtr DynamicRNNGradFusionPass::AddLSTMInputGradNode(ge::NodePtr dynamicRNNGradNode, ge::ComputeGraph& graph,
  ...
  ge::GeTensorDesc c_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(7).GetShape(),
                                                          ge::FORMAT_ND, ge::DT_FLOAT);
  ...
  FUSION_PASS_MAKE_SHARED(
      (lstmSplitDyDesc = std::make_shared<ge::OpDesc>(dynamicRNNGradNode->GetName() + "/LstmInputGrad/SplitVDdy",
                                                      "SplitVD")),
      failStatus=true; return nullptr);
  ge::GeTensorDesc dy_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(8).GetShape(),
                                                           ge::FORMAT_ND, ge::DT_FLOAT);
  ...
  if (tSizeJudge != 1) {
    FUSION_PASS_MAKE_SHARED(
          (lstmSplitIDesc = std::make_shared<ge::OpDesc>(dynamicRNNGradNode->GetName() + "/LstmInputGrad/SplitVDI",
                                                         "SplitVD")),
          failStatus=true; return nullptr);
    ge::GeTensorDesc i_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(11).GetShape(),
                                                            ge::FORMAT_ND, ge::DT_FLOAT);
    ...
    FUSION_PASS_MAKE_SHARED(
        (lstmSplitJDesc = std::make_shared<ge::OpDesc>(dynamicRNNGradNode->GetName() + "/LstmInputGrad/SplitVDJ",
                                                       "SplitVD")),
         failStatus=true; return nullptr);
    ge::GeTensorDesc j_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(12).GetShape(),
                                                            ge::FORMAT_ND, ge::DT_FLOAT);
    ...
    FUSION_PASS_MAKE_SHARED(
        (lstmSplitFDesc = std::make_shared<ge::OpDesc>(dynamicRNNGradNode->GetName() + "/LstmInputGrad/SplitVDF",
                                                       "SplitVD")),
         failStatus=true; return nullptr);
    ge::GeTensorDesc f_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(13).GetShape(),
                                                            ge::FORMAT_ND, ge::DT_FLOAT);
    ...
    FUSION_PASS_MAKE_SHARED(
        (lstmSplitODesc = std::make_shared<ge::OpDesc>(dynamicRNNGradNode->GetName() + "/LstmInputGrad/SplitVDO",
                                                       "SplitVD")),
         failStatus=true; return nullptr);
    ge::GeTensorDesc o_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(14).GetShape(),
                                                            ge::FORMAT_ND, ge::DT_FLOAT);
    ...
    FUSION_PASS_MAKE_SHARED(
        (lstmSplitTanhDesc = std::make_shared<ge::OpDesc>(dynamicRNNGradNode->GetName() + "/LstmInputGrad/SplitVDTanh",
                                                          "SplitVD")),
         failStatus=true; return nullptr);
    ge::GeTensorDesc tan_input_tensor_desc = ge::GeTensorDesc(dynamicRNNGradDesc->GetInputDesc(15).GetShape(),
                                                              ge::FORMAT_ND, ge::DT_FLOAT);
...
ge::NodePtr DynamicRNNGradFusionPass::AddHConcatNode(ge::NodePtr dynamicRNNGradNode, ge::NodePtr splitNode,
  ...
  if ((n_value % 16) != 0) {
    ...
    outputDims.push_back((dynamicRNNGradNode->GetOpDesc()->GetInputDesc(6).GetShape().GetDim(0)) *
                         dynamicRNNGradNode->GetOpDesc()->GetInputDesc(6).GetShape().GetDim(1));
...
ge::NodePtr DynamicRNNGradFusionPass::AddConcatNode(ge::NodePtr dynamicRNNGradNode, ge::NodePtr hConcatNode,
  ...
  if ((n_value % 16) != 0) {
    ...
    outputDims.push_back(dynamicRNNGradNode->GetOpDesc()->GetInputDesc(6).GetShape().GetDim(0) *
                         dynamicRNNGradNode->GetOpDesc()->GetInputDesc(6).GetShape().GetDim(1));
    outputDims.push_back(dynamicRNNGradNode->GetOpDesc()->GetInputDesc(0).GetShape().GetDim(2) +
...
Status DynamicRNNGradFusionPass::AddDbReduceSumNode(ge::NodePtr dynamicRNNGradNode, ge::NodePtr lstmInputGradNode,
  ...
  if ((n_value % 16) != 0) {
    ...
    reduce_output_dims.push_back((lstmInputGradNode->GetOpDesc()->GetOutputDesc(0).GetOriginShape().GetDim(2) + 15) /
                                 16);
    reduce_output_dims.push_back(1);
