...
Status ConcatExt2FusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(
      inputs_num <= max_inputs,
      OP_LOGD(FUSED_OP_TYPE.c_str(), "The amount of input of ConcatV2D node is less than %lld.", max_inputs);
      fusionNodes.emplace_back(fused_node), return SUCCESS);

  if (inputs_num > max_inputs) {
    int64_t nodes_num = 0;
    int64_t nodes_num1 = 0;
    ...
    for (int64_t i = 0; i < nodes_num; i++) {
      if (i < nodes_num - 1) {
        int64_t inputs_size = fusedDesc->GetInputsSize();
        ...
        if (i == 0) {
          for (int64_t a = inputs_size - 1; a >= max_inputs; a--) {
          ...
          for (int64_t a = inputs_size - (max_inputs * i + 1); a >= max_inputs; a--) {
            RemoveInputDesc(ConcatExt2Desc, a);
          }
        }
        ...
        int64_t concat_dim_axis = 0;
        ge::AttrUtils::GetInt(concatext2_node->GetOpDesc(), "concat_dim", concat_dim_axis);
        ...
        for (int64_t n = 0; n < num_concat; n++) {
          if (zero_num > 0) {
            ...
            int64_t dim_axis_value = ConcatExt2InputShape_1.GetDim(axis_1);
            ...
            auto ConcatExt2InputTensor_1 = op1.GetDynamicInputDesc("x", (inputs_size - 1) - (max_inputs * i + n));
        ...
        ConcatExt2OutputShape_1.SetDim(axis_1, size);
        ...
        int64_t concat_dim_axis = 0;
        ge::AttrUtils::GetInt(last_concatext2_node->GetOpDesc(), "concat_dim", concat_dim_axis);
        ...
        for (int32_t n = 0; n < num_concat; n++) {
          if (zero_num > 0) {
            ...
            int64_t dim_axis_value = ConcatExt2InputShape_2.GetDim(axis_2);
            if (PatternFusionUtil::IsUnknownShape(size)) {
              continue;
            }
            ...
            auto ConcatExt2InputTensor_2 = op2.GetDynamicInputDesc("x", (inputs_size - 1) - (n + max_inputs * i));
        ...
        ConcatExt2OutputShape_2.SetDim(axis_2, size);
