 ...
 *
 * @file  concat_ext2_fusion_pass.cpp
 *
...
Status ConcatExt2FusionPass::Fusion(ge::ComputeGraph &graph,
                              Mapping &mapping,
  ...
  Status ret = PatternFusionUtil::ConstToAttrWithNode(graph, fused_node1,
                 fusionOpType, concatv2AttrInfo, fused_node);
  if (ret != SUCCESS) {
...
//A maximum of 63 tensors are supported in mini mode.
  ...
  if (inputs_num > 63) {
    ...
    FUSION_PASS_CHECK(concatext2_base_node == nullptr,
             ...
             concatext2_base_node->GetName().c_str()), return PARAM_INVALID);
...
//infershape end
        ...
        FUSION_PASS_CHECK(concatext2_node == nullptr,
                ...
                concatext2_node->GetName().c_str()), return PARAM_INVALID);
        ...
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(concatext2_node->GetOutDataAnchor(0),
                 ...
                 concatext2_base_node->GetName().c_str(), i,
                 concatext2_node->GetName().c_str(), i),  return FAILED);
        ...
        for (size_t m=0; m<auto_num; m++) {
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(m+i*auto_num)->GetPeerOutAnchor(),
                   ...
                   concatext2_node->GetName().c_str(), m), return FAILED);
        }
      }else {
...
//infershape end
        FUSION_PASS_CHECK(last_concatext2_node == nullptr,
                 ...
                 last_concatext2_node->GetName().c_str()), return PARAM_INVALID);
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(last_concatext2_node->GetOutDataAnchor(0),
                 concatext2_base_node->GetInDataAnchor(i)),
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                 concatext2_base_node->GetName().c_str(), i,
                 last_concatext2_node->GetName().c_str(), i), return FAILED);
        ...
        for (size_t n=0; n< last_node_inputs_num; n++) {
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(n+i*auto_num)->GetPeerOutAnchor(),
                   ...
                   last_concatext2_node->GetName().c_str(), n), return FAILED);
        }
      }
    }
  }
