...
Status LayerNormBetaGammaBackpropFusionPass::Run(ge::ComputeGraph& graph) {
  ...
  OP_LOGD(FUSED_OP_TYPE.c_str(), "GraphFusionPass[LayerNormGradFusionPassBetaGamma]: pattern=undefined, matchedTimes=%zu.",
  ...
  for (LayerNormMatchResult& matchResult : passMatchResultVec) {
    ...
    OP_LOGI(FUSED_OP_TYPE.c_str(), "LayerNormBetaGammaBackprop's 1st output datatype is %d.", lnOpDesc->GetOutputDesc(0).GetDataType());
    OP_LOGI(FUSED_OP_TYPE.c_str(), "LayerNormBetaGammaBackprop's 1st output origin datatype is %d.", lnOpDesc->GetOutputDesc(0).GetOriginDataType());
    OP_LOGI(FUSED_OP_TYPE.c_str(), "LayerNormBetaGammaBackprop's 2nd output datatype is %d.", lnOpDesc->GetOutputDesc(1).GetDataType());
    OP_LOGI(FUSED_OP_TYPE.c_str(), "LayerNormBetaGammaBackprop's 2nd output origin datatype is %d.", lnOpDesc->GetOutputDesc(1).GetOriginDataType());
  }
  return SUCCESS;
}

Status LayerNormBetaGammaBackpropFusionPass::MatchPass(ge::ComputeGraph& graph, vector<LayerNormMatchResult>& passMatchResultVec) {
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "This graph has [%d] LayerNormBetaGammaBackprop node.", layerNormBetaGammaBackpropNodeVec.size());
...
Status LayerNormBetaGammaBackpropFusionPass::GetAllLayerNormBetaGammaBackpropNodes(ge::ComputeGraph& graph, vector<NodePtr>& layerNormBetaGammaBackpropNodeVec) {
...
Status LayerNormBetaGammaBackpropFusionPass::MatchLayerNormBetaGammaBackpropNode(NodePtr lnNodePtr, LayerNormMatchResult &matchResult) {
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "The LayerNormBetaGammaBackpropFusionPass has been matched, the name of LayerNormBetaGammaBackprop node is [%s]", lnNodePtr->GetName().c_str());
  return SUCCESS;
}

Status LayerNormBetaGammaBackpropFusionPass::FusionGraphWithPass(ge::ComputeGraph& graph, LayerNormMatchResult &matchResult) {
  NodePtr lnNodePtr = matchResult.layerNormBetaGammaBackpropPtr;
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Begin to fusion pass, the name of LayerNormBetaGammaBackprop is [%s].", lnNodePtr->GetName().c_str());

  OpDescPtr layerNormBetaGammaDesc = lnNodePtr->GetOpDesc();
  int index = 0;
  for (NodePtr castNode : matchResult.castNodeVec) {
    FUSION_PASS_CHECK(graph.RemoveNode(castNode) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove cast node[%s] failed.", castNode->GetName().c_str()), return FAILED);
...
Status LayerNormBetaGammaBackpropFusionPass::Fusion(
