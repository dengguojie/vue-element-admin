...
vector<FusionPattern *> MulMaximumFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("MulMaximumFusion");
...
Status MulMaximumFusionPass::IsMatch(ge::NodePtr &mulNode,
  ...
  for (uint32_t index = 0; index < mulNode->GetAllInDataAnchors().size(); index++) {
      ...
      if (input_node_const->GetType() != "Const" && input_node_const->GetType() != "Constant") {
...
Status MulMaximumFusionPass::Fusion(ge::ComputeGraph &graph,
                                    Mapping &mapping,
  ...
  if (alpha->GetTensorDesc().GetDataType() != ge::DT_FLOAT &&
      alpha->GetTensorDesc().GetDataType() != ge::DT_FLOAT16) {
  ...
  if (alphaType == ge::DT_FLOAT) {
      ...
      FUSION_PASS_CHECK(!ge::AttrUtils::SetFloat(mulNode->GetOpDesc(), ge::ATTR_NAME_NEGATIVE_SLOPE, negative_slope),
              OP_LOGE(FUSED_OP_TYPE.c_str(), "Set negative slope failed, exit MulMaximumFusionPass with status: failed."), return FAILED);
  ...
  for (auto inAnchor : maximumNode->GetAllInDataAnchors()) {
      if (inAnchor == nullptr ||
          inAnchor->GetPeerOutAnchor() == nullptr ||
  ...
  if (maximumNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
      for (ge::InDataAnchorPtr inAnchorPtr : maximumNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
          ...
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(mulNode->GetOutDataAnchor(0), inAnchorPtr),
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index to fusion node:%s's 1st index failed.",
                          mulNode->GetName().c_str(), maximumNode->GetName().c_str()), return FAILED);
