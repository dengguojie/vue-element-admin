...
/*
 * 本文件的目的：
 *     构造一个kernel大小的辅助矩阵，假设kernel为 2 * 3 * 4
 *     则构造的数据为NDC1HWC0为(1,2,1,3,4,16)
 *     值为24,23,22,...2,1 C0方向值相同
 */
...
vector<FusionPattern*> MaxPool3DGradGradFusionPass::DefinePatterns() {
  ...
  pattern->AddOpDesc(PATTERN_MAX_POOL3D_GRAD_GRAD, {MAX_POOL3D_GRAD_GRAD})
    .SetOutput(PATTERN_MAX_POOL3D_GRAD_GRAD);
  patterns.push_back(pattern);

  return patterns;
}

int MaxPool3DGradGradFusionPass::GetDHW(const std::vector<int32_t> & ksize , int32_t & D, int32_t & H, int32_t & W, Format format) {
    if (ksize.size() != 1 && ksize.size() != 3 && ksize.size() != 5){
        OP_LOGE(FUSED_OP_TYPE.c_str(), "ksize.size() is %d, should be 1 3 or 5", ksize.size());
        return -1;
    }
    ...
    if (ksize.size() == 1) {
        ...
        if(format == ge::FORMAT_NDHWC) {
            ...
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Unsupported shape for MaxPool3DGradGrad. format = %d", format);
            return -1;
        }
    }
    return 0;
...
Status MaxPool3DGradGradFusionPass::Fusion(ge::ComputeGraph &graph,
                                  Mapping &mapping,
                                  vector<ge::NodePtr> &fusionNodes)
{
    ...
    FUSION_PASS_CHECK(maxpool3dGradGradVNode == nullptr,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "maxpool3dGradGradVNode is null, fusion failed."),
             return PARAM_INVALID);

    ge::OpDescPtr maxpool3dGradGradDesc = maxpool3dGradGradVNode->GetOpDesc();
    FUSION_PASS_CHECK(maxpool3dGradGradDesc == nullptr,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "maxpool3dGradGradVNode's OpDesc is null, fusion failed."),
             return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(!CheckOpSupported(fusionDesc), OP_LOGI(FUSED_OP_TYPE.c_str(), "Op DiagPart Not Supported."),return NOT_CHANGED);
    ...
    FUSION_PASS_CHECK(constInputNodes.size() == 0,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "constInputNodes is empty, fusion failed."),
             return PARAM_INVALID);
    NodePtr constInput = constInputNodes[0];
    FUSION_PASS_CHECK(constInput == nullptr,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "constInput is null, fusion failed."),
             return PARAM_INVALID);
