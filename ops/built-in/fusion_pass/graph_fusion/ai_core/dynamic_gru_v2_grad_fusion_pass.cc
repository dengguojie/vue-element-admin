...
static map<std::string, int> INPUT_INDEX = { { "x", 0 },           { "weight_input", 1 }, { "weight_hidden", 2 },
                                             { "y", 3 },           { "init_h", 4 },       { "h", 5 },
                                             { "dy", 6 },          { "dh", 7 },           { "update", 8 },
                                             { "reset", 9 },       { "new", 10 },         { "hidden_new", 11 },
                                             { "seq_length", 12 }, { "mask", 13 } };
static map<std::string, int> HIDDENGRAD_INPUT_INDEX = {
    { "weight_hidden", 0 }, { "init_h", 1 }, { "h", 2 },   { "dy", 3 },        { "dh", 4 },
    { "update", 5 },        { "reset", 6 },  { "new", 7 }, { "hidden_new", 8 }
};
static map<std::string, int> HIDDENGRAD_INPUT_INDEX_CELL = {
    { "dh_pre_t", 0 }, { "h", 1 },      { "dy", 2 },  { "dh", 3 },
...
void DynamicGRUV2GradFusionPass::AddInputNodeDesc(ge::OpDescPtr opDesc, const std::string &name,
    const vector<int64_t> &dims, const ge::Format &format, const vector<int64_t> &originDims,
...
void DynamicGRUV2GradFusionPass::AddOutputNodeDesc(ge::OpDescPtr opDesc, const std::string &name,
    const vector<int64_t> &dims, const ge::DataType &dtype, const ge::Format &format)
{
...
void DynamicGRUV2GradFusionPass::AddHiddenGradNodeEdge(map<std::string, ge::NodePtr>& inputNodes,
                                                       ge::NodePtr hiddenGradNode,
                                                       ge::NodePtr matmulGradNode,
                                                       ge::NodePtr lastHiddenGradNode,
                                                       ge::NodePtr lastMatmulNode,
                                                        ...
                                                        const std::string &name)
{
    ...
    FUSION_PASS_CHECK(dims.empty(), OP_LOGE(FUSED_OP_TYPE.c_str(),
                      "AddSplitTInputNodeDesc:check failed, fusion failed."),
                      return);
    dims[0] = 1;
    vector<int64_t> originDims = inputDesc.GetOriginShape().GetDims();
    FUSION_PASS_CHECK(originDims.empty(), OP_LOGE(FUSED_OP_TYPE.c_str(),
                      "AddSplitTInputNodeDesc:check failed, fusion failed."),
                      return);
    originDims[0] = 1;
                                                             ...
                                                             vector<ge::NodePtr>& newNodes, bool& failStatus)
{
    ...
    ge::OpDescPtr hiddenGradDesc = std::make_shared<ge::OpDesc>(
            dynamicGRUGradNode->GetName() + "/GRUV2Grad/GRUV2HiddenGrad_"+ std::to_string(curT), "GRUV2HiddenGrad");
    ...
    AddOutputNodeDesc(hiddenGradDesc, "dgate_h", dgateHNzDim, ge::FORMAT_FRACTAL_NZ,
                      dgateHNzDim, ge::FORMAT_FRACTAL_NZ, inputHType);
    AddOutputNodeDesc(hiddenGradDesc, "dnt_x", singleGateNzDim, ge::FORMAT_FRACTAL_NZ,
                      singleGateNzDim, ge::FORMAT_FRACTAL_NZ, inputHType);
                                                                   ...
                                                                   bool &failStatus)
{
    ...
    ge::OpDescPtr matmulDesc =
            std::make_shared<ge::OpDesc>(dynamicGRUGradNode->GetName() + "/GRUV2Grad/Matmul_" + to_string(curT),
                                         "BatchMatMul");
    // input
...
vector<vector<ge::NodePtr>> DynamicGRUV2GradFusionPass::AddTLoopNode(map<std::string, ge::NodePtr>& inputNodes,
    ...
    for (int64_t i = 0; i < t_size; i++) {
        ge::NodePtr hiddenGradNode = AddOneHiddenGradNode(gateOrder, i, dynamicGRUGradNode, graph, newNodes,
                                                          failStatus);
        ge::NodePtr matmulNode = AddOneHiddenGradMatmulNode(i, hiddenGradNode, dynamicGRUGradNode, graph, newNodes,
                                                            failStatus);
        // add input edge
    ...
    // last hiddenGradNode
    ge::NodePtr hiddenGradNode = AddOneHiddenGradNode(gateOrder, t_size, dynamicGRUGradNode, graph, newNodes,
		                                      failStatus);
    AddHiddenGradNodeEdge(inputNodes, hiddenGradNode, nullptr, lastHiddenGradNode, lastMatmulNode,
                                                      ...
                                                      vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    ge::OpDescPtr splitDesc =
            std::make_shared<ge::OpDesc>(dynamicGRUGradNode->GetName() + nodeName, "SplitVD");
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddTConcatNode(const string& nodeName, const string& inputName,
                                                       vector<int64_t> fzDims,
                                                       ge::NodePtr dynamicGRUGradNode, vector<ge::NodePtr> &srcNodes,
...
map<std::string, ge::NodePtr> DynamicGRUV2GradFusionPass::AddGRUHiddenGradNodeCell(ge::NodePtr dynamicGRUGradNode,
    ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    if (t_size > 1) {
        // add h split node
        ge::NodePtr hSplitNode = AddTSplitNode("/GRUV2Grad/SplitH", "h", dynamicGRUGradNode, graph, newNodes,
                                               failStatus);
        ...
        // add dy split node
        ge::NodePtr dySplitNode = AddTSplitNode("/GRUV2Grad/SplitY", "dy", dynamicGRUGradNode, graph, newNodes,
                                                failStatus);
        ...
        ge::NodePtr updateSplitNode = AddTSplitNode("/GRUV2Grad/SplitUpdate", "update", dynamicGRUGradNode, graph,
                                                    newNodes,
                                                    failStatus);
        ...
        ge::NodePtr resetSplitNode = AddTSplitNode("/GRUV2Grad/SplitReset", "reset", dynamicGRUGradNode, graph,
                                                   newNodes,
                                                   failStatus);
        ...
        // add new split node
        ge::NodePtr newSplitNode = AddTSplitNode("/GRUV2Grad/SplitNew", "new", dynamicGRUGradNode, graph, newNodes,
                                                 failStatus);
        ...
        // add hidden_new node
        ge::NodePtr hiddenNewSplit = AddTSplitNode("/GRUV2Grad/SplitHiddenNew", "hidden_new", dynamicGRUGradNode, graph,
                                                   newNodes, failStatus);
        FUSION_PASS_CHECK(failStatus,
                          OP_LOGE(FUSED_OP_TYPE.c_str(), "AddHiddenNewSplitNode:check failed, fusion failed."),
                          return result);
        ...
        ge::NodePtr dgateHConcatTNode = AddTConcatNode("/GRUV2Grad/ConcatDgateH", "dgate_h", fzDims, dynamicGRUGradNode,
                                                       result_node[0], graph, newNodes, failStatus);
        FUSION_PASS_CHECK(failStatus,
                          OP_LOGE(FUSED_OP_TYPE.c_str(), "AddDgateHConcatTNode:check failed, fusion failed."),
                          return result);
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddHSplitNodeCell(ge::NodePtr dynamicGRUGradNode, ge::ComputeGraph &graph,
    vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    // create split desc
    ge::OpDescPtr splitDesc =
    ...
    AddOutputNodeDesc(splitDesc, "split_t_1", output1NzDim, ge::FORMAT_FRACTAL_NZ,
                      output1NzDim, ge::FORMAT_FRACTAL_NZ, inputHType); // split_t_1
    ...
    AddOutputNodeDesc(splitDesc, "split_1", output2NzDim, ge::FORMAT_FRACTAL_NZ,
                      output2NzDim, ge::FORMAT_FRACTAL_NZ, inputHType); // split_1
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddHConcatNodeCell(ge::NodePtr dynamicGRUGradNode, ge::NodePtr splitNode,
    ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    // create concat desc
    ...
    ge::GeTensorDesc inputTensorDescInitH =
	    dynamicGRUGradNode->GetOpDesc()->GetInputDesc(INPUT_INDEX["init_h"]).Clone();
    ge::GeTensorDesc inputTensorDescSplitH = splitNode->GetOpDesc()->GetOutputDesc(0).Clone();

    vector<int64_t> inputNzDim = { 1, nzHiddenDim, nzBatch, 16, 16 };
    inputTensorDescInitH.SetFormat(ge::FORMAT_FRACTAL_NZ);
    inputTensorDescInitH.SetOriginFormat(ge::FORMAT_FRACTAL_NZ);
    inputTensorDescInitH.SetShape(GeShape(inputNzDim));
    inputTensorDescInitH.SetOriginShape(GeShape(inputNzDim));
    ...
    AddOutputNodeDesc(concatDesc, "concat_h", outputNzDim, ge::FORMAT_FRACTAL_NZ,
                      outputNzDim, ge::FORMAT_FRACTAL_NZ, inputHType);
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddDwhMatmulNodeCell(ge::NodePtr dynamicGRUGradNode, ge::NodePtr hConcatNode,
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddDwhMatmulNodeCell(ge::NodePtr dynamicGRUGradNode,
    ge::NodePtr gruHiddenGradNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    // create matmul desc
    ge::OpDescPtr matmulDesc =
    ...
    vector<int64_t> inputHDim = {1, batch, hidden_dim};
    vector<int64_t> inputHNzDim = {1, nzHiddenDim, nzBatch, 16, 16};
    inputTensorDescH.SetShape(GeShape(inputHNzDim));
    inputTensorDescH.SetOriginShape(GeShape(inputHNzDim));
    inputTensorDescH.SetFormat(ge::FORMAT_FRACTAL_NZ);
    inputTensorDescH.SetOriginFormat(ge::FORMAT_FRACTAL_NZ);
    inputTensorDescH.SetDataType(ge::DT_FLOAT16);
    inputTensorDescDgate.SetDataType(ge::DT_FLOAT16);
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddDgateHSplitNodeCell(ge::NodePtr dynamicGRUGradNode,
    ge::NodePtr gruHiddenGradNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    // create split desc
    ge::OpDescPtr splitDesc =
    ...
    if (t_size == 1) {
        inputDgateH =
                gruHiddenGradNode->GetOpDesc()->GetOutputDesc(HIDDENGRAD_OUTPUT_INDEX["dgate_h"]).Clone();
    } else {
    ...
    AddOutputNodeDesc(splitDesc, "split_ir", output1NzDim, ge::FORMAT_FRACTAL_NZ,
                      output1NzDim, ge::FORMAT_FRACTAL_NZ, inputHType); // split_didr

    // add output2 split_1, shape:{t, batch, hidden_size}
    vector<int64_t> output2NzDim {t_size, nzHiddenDim, nzBatch, 16, 16};
    AddOutputNodeDesc(splitDesc, "split_n", output2NzDim, ge::FORMAT_FRACTAL_NZ,
                      output2NzDim, ge::FORMAT_FRACTAL_NZ, inputHType); // split_dn_h
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddDgateXConcatNodeCell(ge::NodePtr dynamicGRUGradNode,
    ge::NodePtr dgateHSplitNode, ge::NodePtr gruHiddenGradNode, ge::ComputeGraph &graph,
    vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    // create concat desc
    ge::OpDescPtr concatDesc =
    ...
    AddOutputNodeDesc(concatDesc, "dgate_x", outputNzDim, ge::FORMAT_FRACTAL_NZ,
                      outputDim, ge::FORMAT_ND, inputHType);
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddWxBroadcastNodeCell(ge::NodePtr dynamicGRUGradNode, ge::ComputeGraph &graph,
    vector<ge::NodePtr> &newNodes, bool &failStatus)
{
...
Status DynamicGRUV2GradFusionPass::AddDxtMatmulNodeCell(ge::NodePtr dynamicGRUGradNode, ge::NodePtr dgateXConcatNode,
    ge::NodePtr wxBoadcastNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes)
{
    // create matmul desc
    ...
    ge::GeTensorDesc outputTensorDesc =
        dynamicGRUGradNode->GetOpDesc()->GetOutputDesc(OUTPUT_INDEX["dx"]).Clone(); // dw
    ...
    // output Edge
    for (InDataAnchorPtr inAnchorPtr :
        dynamicGRUGradNode->GetOutDataAnchor(OUTPUT_INDEX["dx"])->GetPeerInDataAnchors())
    {
        // dxt
        inAnchorPtr->UnlinkAll();
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddDwxMatmulNodeCell(ge::NodePtr dynamicGRUGradNode,
    ge::NodePtr dgateXConcatNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    // create matmul desc
    ge::OpDescPtr matmulDesc =
...
Status DynamicGRUV2GradFusionPass::AddReduceSumNodeCell(ge::NodePtr dynamicGRUGradNode, ge::NodePtr inputNode,
    int anchorIndex, const vector<int64_t> &axis, const string &nodeName, const string &indexName,
    ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes)
{
    ...
    inputTensorDesc.SetDataType(inputTensorDesc.GetDataType());
...
Status DynamicGRUV2GradFusionPass::FusionCell(ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes)
{
    ...
    map<std::string, ge::NodePtr> hiddenGradNodes = AddGRUHiddenGradNodeCell(gruV2GradNode, graph, newNodes, isFailure);
    ...
    if (t_size != 1) {
        ...
        dwhMatmulNode = AddDwhMatmulNode(gruV2GradNode, hConcatNode, hiddenGradNodes["dgate_h"], graph, newNodes,
                                         isFailure);
    ...
    ge::NodePtr dgateHSplitNode = AddDgateHSplitNode(gruV2GradNode, hiddenGradNodes["dgate_h"], graph, newNodes,
                                                     isFailure);
    ...
    isFailure = AddReduceSumNode(gruV2GradNode, dwxMatmulNode, anchorOutputIndex, reduceDwAxis, "dwx", "dw_input",
                                 graph, newNodes);
    FUSION_PASS_CHECK(isFailure, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddDwxReduceSumNode:check failed, fusion failed."),
                      return FAILED);

    isFailure = AddReduceSumNode(gruV2GradNode, dwhMatmulNode, anchorOutputIndex, reduceDwAxis, "dwh", "dw_hidden",
                                 graph, newNodes);
    ...
    isFailure = AddReduceSumNode(gruV2GradNode, gateConcatNode, anchorOutputIndex, reduceDbAxis, "dbx", "db_input",
                                 graph, newNodes);
    ...
    isFailure = AddReduceSumNode(gruV2GradNode, hiddenGradNodes["dgate_h"], anchorDgateIndex, reduceDbAxis, "dbh",
                                 "db_hidden", graph, newNodes);
    ...
    FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(gruV2GradNode),
...
ge::NodePtr DynamicGRUV2GradFusionPass::AddGRUHiddenGradNode(ge::NodePtr dynamicGRUGradNode, ge::ComputeGraph &graph,
    vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ge::OpDescPtr dynamicGRUGradDesc = dynamicGRUGradNode->GetOpDesc();
    // create gru_input_grad desc
    ge::OpDescPtr hiddenGradDesc = AttrUtils::CloneOpDesc(dynamicGRUGradDesc);
    hiddenGradDesc->SetName(dynamicGRUGradDesc->GetName() + "/GRUV2HiddenGrad");
    hiddenGradDesc->SetType("GRUV2HiddenGrad");
    // clear input
    OpDescUtils::ClearInputDesc(hiddenGradDesc, INPUT_INDEX["mask"]);
    OpDescUtils::ClearInputDesc(hiddenGradDesc, INPUT_INDEX["seq_length"]);
    OpDescUtils::ClearInputDesc(hiddenGradDesc, INPUT_INDEX["y"]);
    OpDescUtils::ClearInputDesc(hiddenGradDesc, INPUT_INDEX["weight_input"]);
    OpDescUtils::ClearInputDesc(hiddenGradDesc, INPUT_INDEX["x"]);

    // clear output
    OpDescUtils::ClearOutputDesc(hiddenGradDesc, OUTPUT_INDEX["dx"]);
    OpDescUtils::ClearOutputDesc(hiddenGradDesc, OUTPUT_INDEX["db_hidden"]);
    OpDescUtils::ClearOutputDesc(hiddenGradDesc, OUTPUT_INDEX["db_input"]);
    OpDescUtils::ClearOutputDesc(hiddenGradDesc, OUTPUT_INDEX["dw_hidden"]);
    OpDescUtils::ClearOutputDesc(hiddenGradDesc, OUTPUT_INDEX["dw_input"]);

    vector<int64_t> dgateHDim { this->t_size, this->batch, 3 * this->hidden_dim };
    this->AddOutputNodeDesc(hiddenGradDesc, "dgate_h", dgateHDim, this->inputHType, ge::FORMAT_ND); // dgate_h

    vector<int64_t> singleGateDim { this->t_size, this->batch, this->hidden_dim };
    this->AddOutputNodeDesc(hiddenGradDesc, "dnt_x", singleGateDim, this->inputHType, ge::FORMAT_ND); // dnt_x
    ...
    for (iter = HIDDENGRAD_INPUT_INDEX.begin(); iter != HIDDENGRAD_INPUT_INDEX.end(); ++iter) {
        ge::GraphUtils::AddEdge(dynamicGRUGradNode->GetInDataAnchor(INPUT_INDEX[iter->first])->GetPeerOutAnchor(),
            hiddenGradNode->GetInDataAnchor(iter->second));
    }
    ...
    vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    // add output1 split_t_1, shape:{t-1,batch_size,hidden_size}
    vector<int64_t> output1Dim { this->t_size - 1, this->batch, this->hidden_dim };
    this->AddOutputNodeDesc(splitDesc, "split_t_1", output1Dim, this->inputHType, ge::FORMAT_ND); // split_t_1

    // add output2 split_1, shape:{1,batch_size,hidden_size}
    vector<int64_t> output2Dim { 1, this->batch, this->hidden_dim };
    this->AddOutputNodeDesc(splitDesc, "split_1", output2Dim, this->inputHType, ge::FORMAT_ND); // split_1

    // attr
    vector<int64_t> size_splits { this->t_size - 1, 1 };
    ge::AttrUtils::SetListInt(splitDesc, "size_splits", size_splits);
    ge::AttrUtils::SetInt(splitDesc, "split_dim", 0);
    ge::AttrUtils::SetInt(splitDesc, "num_split", 2);
    ...
    ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    vector<int64_t> input_h = { 1, this->batch, this->hidden_dim };
    ...
    // output concat_h, shape:{t,batch_size,hidden_size}
    vector<int64_t> outputDim { this->t_size, this->batch, this->hidden_dim };
    this->AddOutputNodeDesc(concatDesc, "concat_h", outputDim, this->inputHType, ge::FORMAT_ND);
    ...
    // create concat node
    ge::NodePtr concatNode = this->AddNewNode(graph, concatDesc, newNodes, failStatus);
    ...
    ge::NodePtr gruHiddenGradNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    // add output dwt_h shape:{t, hidden_size, 3 * hide_size}
    vector<int64_t> outputDim { this->t_size, this->hidden_dim, 3 * this->hidden_dim };
    this->AddOutputNodeDesc(matmulDesc, "dwt_h", outputDim, ge::DT_FLOAT16, ge::FORMAT_ND);
    ...
    ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    vector<int64_t> input_h = { 1, this->batch, this->hidden_dim };
    ge::GeShape init_hShape(input_h);
    inputTensorDescH.SetShape(init_hShape);
    inputTensorDescH.SetOriginShape(init_hShape);
    inputTensorDescH.SetDataType(ge::DT_FLOAT16);
    inputTensorDescDgate.SetDataType(ge::DT_FLOAT16);
    matmulDesc->AddInputDesc("input_h", inputTensorDescH);
    matmulDesc->AddInputDesc("input_dgate", inputTensorDescDgate);
    ...
    ge::NodePtr gruHiddenGradNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    // add output1 dgate_ir, shape:{t, batch, 2 * hidden_size}
    vector<int64_t> output1Dim { this->t_size, this->batch, 2 * this->hidden_dim };
    this->AddOutputNodeDesc(splitDesc, "split_ir", output1Dim, this->inputHType, ge::FORMAT_ND); // split_didr

    // add output2 split_1, shape:{t, batch, hidden_size}
    vector<int64_t> output2Dim { this->t_size, this->batch, this->hidden_dim };
    this->AddOutputNodeDesc(splitDesc, "split_n", output2Dim, this->inputHType, ge::FORMAT_ND); // split_dn_h

    // attr
    vector<int64_t> size_splits = { 2 * this->hidden_dim, this->hidden_dim };
    ge::AttrUtils::SetListInt(splitDesc, "size_splits", size_splits);
    ge::AttrUtils::SetInt(splitDesc, "split_dim", 2);
    ge::AttrUtils::SetInt(splitDesc, "num_split", 2);
    ...
    ge::NodePtr gruHiddenGradNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    // input
    ge::GeTensorDesc dirDesc = dgateHSplitNode->GetOpDesc()->GetOutputDesc(0).Clone(); // [dit, drt]
    ge::GeTensorDesc dnxDesc =
        gruHiddenGradNode->GetOpDesc()->GetOutputDesc(HIDDENGRAD_OUTPUT_INDEX["dnt_x"]).Clone(); // [dnt_x]
    concatDesc->AddInputDesc("input_dirt", dirDesc);
    concatDesc->AddInputDesc("input_dnt_x", dnxDesc);

    // output shape:{t,batch,3*hidden_size}
    vector<int64_t> outputDim { this->t_size, this->batch, 3 * this->hidden_dim };
    this->AddOutputNodeDesc(concatDesc, "dgate_x", outputDim, dirDesc.GetDataType(), ge::FORMAT_ND);
    ...
    vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    ge::GeTensorDesc inputWxDesc =
        dynamicGRUGradNode->GetOpDesc()->GetInputDesc(INPUT_INDEX["weight_input"]).Clone(); // weight_x
    vector<int64_t> inputWx { 1, this->input_dim, 3 * this->hidden_dim };
    ge::GeShape inputWxShape(inputWx);
    inputWxDesc.SetShape(inputWxShape);
    inputWxDesc.SetOriginShape(inputWxShape);
    broadcastDesc->AddInputDesc("wx_2d", inputWxDesc);
    ...
    // output shape:{t, input_size, 3*hidden_size}
    vector<int64_t> outputDim { this->t_size, this->input_dim, 3 * this->hidden_dim };
    this->AddOutputNodeDesc(broadcastDesc, "wx_3d", outputDim, inputWxDesc.GetDataType(), ge::FORMAT_ND);

    // attr
    ge::AttrUtils::SetListInt(broadcastDesc, "shape", outputDim);

    // create broadcast node
    ge::NodePtr broadcastNode = this->AddNewNode(graph, broadcastDesc, newNodes, failStatus);
    ...
    ge::NodePtr wxBoadcastNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes)
{
    ...
    // output Edge
    for (InDataAnchorPtr inAnchorPtr :
        dynamicGRUGradNode->GetOutDataAnchor(OUTPUT_INDEX["dx"])->GetPeerInDataAnchors()) { // dxt
        inAnchorPtr->UnlinkAll();
        ge::GraphUtils::AddEdge(matmulNode->GetOutDataAnchor(0), inAnchorPtr);
    }
    return failStatus;
    ...
    ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    // add output dwx, shape:{t, input_dim, 3 * hidden_dim}
    vector<int64_t> outputDim { this->t_size, this->input_dim, 3 * this->hidden_dim };
    this->AddOutputNodeDesc(matmulDesc, "dwt_x", outputDim, ge::DT_FLOAT16, ge::FORMAT_ND);
...
Status DynamicGRUV2GradFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes)
{
    ...
    // add gruHiddenGrad
    ge::NodePtr hiddenGradNode = AddGRUHiddenGradNode(gruV2GradNode, graph, newNodes, isFailure);
    FUSION_PASS_CHECK(isFailure, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddGRUHiddenGradNode:check failed, fusion failed."),
        return FAILED);
    ...
    if (this->t_size != 1) {
        ...
        // add dw_h = matmul(h_prev.T, dgate_h)
        dwhMatmulNode = AddDwhMatmulNode(gruV2GradNode, hConcatNode, hiddenGradNode, graph, newNodes, isFailure);
        FUSION_PASS_CHECK(isFailure, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddDwhMatmulNode:check failed, fusion failed."),
            return FAILED);
    } else {
        // add dw_h = matmul(h_prev.T, dgate_h)
        dwhMatmulNode = AddDwhMatmulNode(gruV2GradNode, hiddenGradNode, graph, newNodes, isFailure);
        FUSION_PASS_CHECK(isFailure, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddDwhMatmulNode:check failed, fusion failed."),
            return FAILED);
    }

    // split dgate_h to [dit, drt] and [dnt_h]
    ge::NodePtr dgateHSplitNode = AddDgateHSplitNode(gruV2GradNode, hiddenGradNode, graph, newNodes, isFailure);
    FUSION_PASS_CHECK(isFailure, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddDgateHSplitNode:check failed, fusion failed."),
        return FAILED);

    // concat [dit, drt] with [dnt_x] to dgate_x
    ge::NodePtr gateConcatNode =
        AddDgateXConcatNode(gruV2GradNode, dgateHSplitNode, hiddenGradNode, graph, newNodes, isFailure);
    FUSION_PASS_CHECK(isFailure, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddDgateXConcatNode:check failed, fusion failed."),
        return FAILED);
    ...
    int anchorDgateIndex = 1;
    isFailure = AddReduceSumNode(gruV2GradNode, hiddenGradNode, anchorDgateIndex, reduceDbAxis, "dbh", "db_hidden",
        graph, newNodes);
    FUSION_PASS_CHECK(isFailure, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddDbhReduceSumNode:check failed, fusion failed."),
        return FAILED);
