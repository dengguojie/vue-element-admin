...
Status SplitVFusionPass::Fusion(ge::ComputeGraph &graph,
                              Mapping &mapping,
  ...
  Status ret = PatternFusionUtil::ConstToAttrWithNode(graph, fused_node1,
                 fusionOpType, splitvAttrInfo, fused_node);
  if (ret != SUCCESS) {
  ...
  if (num_split > 63) {
    ...
    ge::AttrUtils::SetListInt(splitvd_base_node->GetOpDesc(), "size_splits",
                              size_splits_new);
    ...
    FUSION_PASS_CHECK(splitvd_base_node == nullptr,
            ...
            splitvd_base_node->GetName().c_str()), return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(0)->GetPeerOutAnchor(),
            ...
            splitvd_base_node->GetName().c_str(), 0), return FAILED);
    ...
    for (size_t i = 0; i < nodes_num; i++) {
      if (i<nodes_num-1) {
        ...
        ge::AttrUtils::SetListInt(splitvd_node->GetOpDesc(), "size_splits",
                                  size_splits_new2);
        ...
        FUSION_PASS_CHECK(splitvd_node == nullptr,
                OP_LOGE(FUSED_OP_TYPE.c_str(), "splitvd_node:%s is null, fusion failed.",
                splitvd_node->GetName().c_str()), return PARAM_INVALID);
        ...
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(splitvd_base_node->GetOutDataAnchor(i),
                ...
                splitvd_node->GetName().c_str(), i),  return FAILED);
        ...
        if (last_node_num_split != 1) {
          ...
          ge::AttrUtils::SetInt(last_splitvd_node->GetOpDesc(), "num_split",
                                last_node_num_split);
          ge::AttrUtils::SetListInt(last_splitvd_node->GetOpDesc(), "size_splits",
                                    size_splits_new3);
          ...
          FUSION_PASS_CHECK(last_splitvd_node == nullptr,
                    ...
                    last_splitvd_node->GetName().c_str()), return PARAM_INVALID);
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(splitvd_base_node->GetOutDataAnchor(i),
                  last_splitvd_node->GetInDataAnchor(0)),
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                  splitvd_base_node->GetName().c_str(), i,
                  last_splitvd_node->GetName().c_str(), i),  return FAILED);
