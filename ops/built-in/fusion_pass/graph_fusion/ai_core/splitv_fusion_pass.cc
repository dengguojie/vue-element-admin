...
Status SplitVFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(fused_node1 == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "new a pattern object failed"),
                    return PARAM_INVALID);

  ge::OpDescPtr fuseDesc1 = fused_node1->GetOpDesc();
  FUSION_PASS_CHECK(fuseDesc1 == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "OpDesc of node1 is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(fusion_desc == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "fusion op desc is nullptr."),
                    return NOT_CHANGED);
  ...
  if (num_split > 63) {
    ...
    FUSION_PASS_CHECK(splitvd_base_node == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                     "splitvd_base_node:%s is null, fusion failed.",
    ...
    for (size_t i = 0; i < nodes_num; i++) {
      if (i < nodes_num - 1) {
        ...
        FUSION_PASS_CHECK(
            ...
            VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "splitvd_node:%s is null, fusion failed.",
                                           splitvd_node->GetName().c_str()),
            return PARAM_INVALID);
        ...
        if (last_node_num_split != 1) {
          ...
          FUSION_PASS_CHECK(last_splitvd_node == nullptr,
                            VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                                                           "last_splitvd_node:%s is null, fusion failed.",
