...
Status TransdataTransdataPass::RemoveNode(ge::NodePtr node, ge::ComputeGraph &graph) {
  ...
  Status TransdataTransdataPass::Fusion(ge::ComputeGraph &graph,
                                                 Mapping &mapping,
                                                 vector<ge::NodePtr> &fusionNodes)
  {
    ...
    FUSION_PASS_CHECK(transData_1 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "transData_1 is null"), return PARAM_INVALID);
    FUSION_PASS_CHECK(transData_2 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "transData_2 is null"), return PARAM_INVALID);
    FUSION_PASS_CHECK(fullyConnection == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fullyConnection is null"), return PARAM_INVALID);

    int64_t axis;
    FUSION_PASS_CHECK(!ge::AttrUtils::GetInt(fullyConnection->GetOpDesc(), "axis", axis),
             OP_LOGI(FUSED_OP_TYPE.c_str(), "Get node[%s]'s axis attr not success.",
                     fullyConnection->GetName().c_str()), return false);
    if (axis != 1) {
      OP_LOGI(FUSED_OP_TYPE.c_str(), "node[FullyConnection]'s axis is not 1, not support fusion, TransdataTransdataPass fusion end");
      return SUCCESS;
    }
    ...
    if (secondTransDataOutputTensor.GetFormat() != ge::FORMAT_FRACTAL_Z) {
      OP_LOGI(FUSED_OP_TYPE.c_str(), "node[FullyConnection]'s input format is not FRACTAL_Z , not support fusion, TransdataTransdataPass fusion end");
      return SUCCESS;
    }
    ge::OpDescPtr firstTransDataOpDesc = transData_1->GetOpDesc();
    FUSION_PASS_CHECK(firstTransDataOpDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "transData_1 opdesc is null"), return PARAM_INVALID);
    ...
    if (firstTransDataInputTensor.GetFormat() != ge::FORMAT_NC1HWC0) {
      OP_LOGI(FUSED_OP_TYPE.c_str(), "firstTransDataInputTensor format is not NC1HWC0 , not support fusion, TransdataTransdataPass fusion end");
      return SUCCESS;
    }
    ...
    FUSION_PASS_CHECK(firstTransDataOutDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "firstTransDataOutDataAnchor is null"), return FAILED);

    auto secondTransDataOutDataAnchor = transData_2->GetOutDataAnchor(0);
    FUSION_PASS_CHECK(secondTransDataOutDataAnchor == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "secondTransDataOutDataAnchor is null"), return FAILED);
    auto secondTransDataPeerInDataAnchor = secondTransDataOutDataAnchor->GetPeerInDataAnchors();

    // delete transData_2 node
    FUSION_PASS_CHECK(RemoveNode(transData_2, graph) == FAILED, OP_LOGE(FUSED_OP_TYPE.c_str(), "remove transData_2 node failed"), return FAILED);

    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(firstTransDataOutDataAnchor,
                                     secondTransDataPeerInDataAnchor.at(0)) != ge::GRAPH_SUCCESS,
