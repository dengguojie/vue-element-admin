...
Status PReluAbsFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(reluNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reluNode is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(absNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "absNode is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(reluNode) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove reluNode failed."),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(subNode) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove subNode failed."),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(mul1Node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove mul1Node failed."),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(absNode) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove absNode failed."),
                    return FAILED);
