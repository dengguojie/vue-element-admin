...
Status PReluAbsFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes) {
    ...
    FUSION_PASS_CHECK(absNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(),"absNode is null, fusion failed."), return PARAM_INVALID);
    FUSION_PASS_CHECK(subNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(),"subNode is null, fusion failed."), return PARAM_INVALID);
    FUSION_PASS_CHECK(mul1Node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(),"mul1Node is null, fusion failed."), return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(inputSize != 1, OP_LOGI(FUSED_OP_TYPE.c_str(),"abs node size is [%lu], which not equal to 1.", inputSize), return NOT_CHANGED);

    inputSize = mul1Node->GetInDataNodes().size();
    FUSION_PASS_CHECK(inputSize != 2, OP_LOGI(FUSED_OP_TYPE.c_str(),"mul1 node size is [%lu], which not equal to 1.", inputSize), return NOT_CHANGED);

    inputSize = subNode->GetInDataNodes().size();
    FUSION_PASS_CHECK(inputSize != 2, OP_LOGI(FUSED_OP_TYPE.c_str(),"sub node size is [%lu], which not equal to 1.", inputSize), return NOT_CHANGED);
    ...
    FUSION_PASS_CHECK(preluOpdesc->AddInputDesc(0, reluInputDesc) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(),"add reluNode input desc failed."), return FAILED);

    ge::GeTensorDesc mulInputDesc = mulNode->GetOpDesc()->GetInputDesc(0);
    FUSION_PASS_CHECK(preluOpdesc->AddInputDesc(1, mulInputDesc) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(),"add mulNode input desc failed."), return FAILED);
    ...
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
        reluNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
        preluNode->GetInDataAnchor(0)) != SUCCESS,
        OP_LOGE(FUSED_OP_TYPE.c_str(),"Add edge between reluNode and preluNode failed."), return FAILED);

    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
        mulNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
        preluNode->GetInDataAnchor(1)) != SUCCESS,
        OP_LOGE(FUSED_OP_TYPE.c_str(),"Add edge between mulNode and preluNode failed."), return FAILED);
    ...
    FUSION_PASS_CHECK(graph.RemoveNode(subNode) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(),"Remove subNode failed."), return FAILED);
    FUSION_PASS_CHECK(graph.RemoveNode(mul1Node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(),"Remove mul1Node failed."), return FAILED);
    FUSION_PASS_CHECK(graph.RemoveNode(absNode) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(),"Remove absNode failed."), return FAILED);
