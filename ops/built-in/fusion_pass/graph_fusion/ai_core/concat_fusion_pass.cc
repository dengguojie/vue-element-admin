...
namespace fe {
    ...
    Status ConcatFusionPass::Fusion(ge::ComputeGraph &graph,
                                    Mapping &mapping,
        ...
        Status ret = PatternFusionUtil::ConstToAttrWithNode(graph, fused_node1,
                                                            fusionOpType, concatAttrInfo, fused_node);
        if (ret != SUCCESS) {
...
//A maximum of 63 tensors are supported in mini mode.
        ...
        if (inputs_num > 63) {
            ...
            int64_t last_node_inputs_num = inputs_num - (63*(nodes_num-1));
            ...
            for (int64_t i = 0; i < nodes_num; i++) {
                if (i < nodes_num-1) {
                    ...
                    if (i == 0){
                        for (int64_t a = inputs_num-1; a >= 63; a--) {
                            OpDescUtils::ClearInputDesc(ConcatdDesc, a);
                        }
                    }
                    else {
                        for (int64_t a = i*63 -1; a >= 0; a--) {
                            OpDescUtils::ClearInputDesc(ConcatdDesc, a);
                        }
                        for (int64_t a = inputs_num - (63*i + 1); a >= 63; a--) {
                            OpDescUtils::ClearInputDesc(ConcatdDesc, a);
                        }
                    }
                    ge::NodePtr concatd_node = graph.AddNode(ConcatdDesc);
                    newNodes.push_back(concatd_node);
                    ge::AttrUtils::SetInt(concatd_node->GetOpDesc(), "N", 63);
//infershape begin
                    int64_t size = 0;
...
//infershape end
                    ...
                    for (int64_t m=0; m<63; m++) {
                        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(m+i*63)->GetPeerOutAnchor(),
                                                                    concatd_node->GetInDataAnchor(m)),
                                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                                         fused_node->GetName().c_str(), (m+i*63),
...
//the last_node infershape end
                    ...
                    for (int64_t n=0; n< last_node_inputs_num; n++) {
                        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fused_node->GetInDataAnchor(n+i*63)->GetPeerOutAnchor(),
                                                                    last_concatd_node->GetInDataAnchor(n)),
                                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                                         fused_node->GetName().c_str(), (n+i*63),
