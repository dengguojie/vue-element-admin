 ...
 *
 * @brief TensorScatterUpdate fusion pass(TensorScatterUpdate --> TensorMove & ScatterNdUpdate)
 *
 */
...
Status TensorScatterUpdateFusionPass::Fusion(ge::ComputeGraph &graph,
  ...
  tensorMoveOpdesc =
      std::make_shared<ge::OpDesc>(fusedNode->GetName(), TENSORMOVE);
  FUSION_PASS_CHECK(tensorMoveOpdesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "tensorMoveOpdesc is null, fusion failed."),
           return PARAM_INVALID);
  std::shared_ptr<ge::OpDesc> scatterNdUpdatedesc = nullptr;
  scatterNdUpdatedesc =
      std::make_shared<ge::OpDesc>(fusedNode->GetName(), SCATTERNDUPDATE);
  FUSION_PASS_CHECK(scatterNdUpdatedesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "scatterNdUpdatedesc is null, fusion failed."),
           return PARAM_INVALID);

  // add input and output
  ge::GeTensorDesc input_x =
      fusedNode->GetOpDesc()->GetInputDesc(0);
  FUSION_PASS_CHECK(tensorMoveOpdesc->AddInputDesc(input_x) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input x failed."), return FAILED);
  ge::GeTensorDesc output_y =
      fusedNode->GetOpDesc()->GetOutputDesc(0);
  FUSION_PASS_CHECK(tensorMoveOpdesc->AddOutputDesc(output_y) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add output y failed."), return FAILED);
  ge::NodePtr tensorMoveNode = graph.AddNode(tensorMoveOpdesc);

  ge::GeTensorDesc input_var =
      tensorMoveNode->GetOpDesc()->GetOutputDesc(0);
  FUSION_PASS_CHECK(scatterNdUpdatedesc->AddInputDesc("var", input_var) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input var failed."), return FAILED);
  ge::GeTensorDesc input_indices =
      fusedNode->GetOpDesc()->GetInputDesc(1);
  FUSION_PASS_CHECK(scatterNdUpdatedesc->AddInputDesc("indices", input_indices) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input indices failed."), return FAILED);
  ge::GeTensorDesc input_updates =
      fusedNode->GetOpDesc()->GetInputDesc(2);
  FUSION_PASS_CHECK(scatterNdUpdatedesc->AddInputDesc("updates", input_updates) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add input updates failed."), return FAILED);
  ge::GeTensorDesc output_var =
      fusedNode->GetOpDesc()->GetOutputDesc(0);
  FUSION_PASS_CHECK(scatterNdUpdatedesc->AddOutputDesc("var", output_var) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add output var failed."), return FAILED);
  ...
  FUSION_PASS_CHECK(tensorMoveNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "tensorMoveNode fusionNode:%s is null, fusion failed.",
                   tensorMoveNode->GetName().c_str()),
                   return PARAM_INVALID);
  FUSION_PASS_CHECK(scatterNdUpdateNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "scatterNdUpdateNode fusionNode:%s is null, fusion failed.",
  ...
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
               fusedNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
               tensorMoveNode->GetInDataAnchor(0)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge0 failed."),
           return FAILED);
  // connect tensor_move's output with scatter_nd_update
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
               tensorMoveNode->GetOutDataAnchor(0),
               scatterNdUpdateNode->GetInDataAnchor(0)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge1 failed."),
           return FAILED);
  // connect indices with scatter_nd_update
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
               fusedNode->GetInDataAnchor(1)->GetPeerOutAnchor(),
               scatterNdUpdateNode->GetInDataAnchor(1)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge2 failed."),
           return FAILED);
  // connect updates with scatter_nd_update
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(
               fusedNode->GetInDataAnchor(2)->GetPeerOutAnchor(),
               scatterNdUpdateNode->GetInDataAnchor(2)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge3 failed."),
           return FAILED);
  ...
  for (auto inDataAnchor :
    ...
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(scatterNdUpdateNode->GetOutDataAnchor(0),
                                     inDataAnchor) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge4 failed."), return FAILED);
  }

  FUSION_PASS_CHECK(graph.RemoveNode(fusedNode) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove fusedNode failed."), return FAILED);
