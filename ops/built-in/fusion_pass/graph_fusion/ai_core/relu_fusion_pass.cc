...
static const std::set<string> add_relu_fusion_input_op = {
    "Convolution", "Activation", "FusionBatchNorm",
...
Status ReluFusionPass::Fusion(ge::ComputeGraph &graph,
                              Mapping &mapping,
  ...
  int64_t mode = 0;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetInt(relu_node->GetOpDesc(),
                                  ge::ACTIVATION_ATTR_MODE, mode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "get mode fail."), return FAILED);

  FUSION_PASS_CHECK(src_node->GetOutDataNodes().size() > 1 ||
               mode != ge::DOMI_ACTIVATION_RELU ||
               graph.GetGraphOutNodes().find(relu_node->GetName()) !=
                   graph.GetGraphOutNodes().end() ||
  ...
  FUSION_PASS_CHECK(ret != SUCCESS && ret != NOT_CHANGED,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Fail to do fusion between node[%s] and node[%s]: the reason"
  ...
  FUSION_PASS_CHECK(ret == NOT_CHANGED,
           OP_LOGD(FUSED_OP_TYPE.c_str(), "Can't do fusion between node[%s] and node[%s]: the reason"
  ...
  FUSION_PASS_CHECK(in_edges_size < 0, OP_LOGE(FUSED_OP_TYPE.c_str(), "inEdges size is invalid"),
           return FAILED);
...
Status ReluFusionPass::DoFusion(ge::NodePtr src_node, ge::NodePtr relu_node) {
  ...
  if (CONV2D == src_op->GetType()) {
    ...
    FUSION_PASS_CHECK(
        !ge::AttrUtils::SetBool(src_op, ge::CONV_ATTR_NAME_RELU_FLAG, true),
        OP_LOGD(FUSED_OP_TYPE.c_str(), "set CONV_ATTR_NAME_RELU_FLAG fail."), return NOT_CHANGED);
  } else if (FULLCONNECTION == src_op->GetType()) {
    // set relu flag to true
    FUSION_PASS_CHECK(!ge::AttrUtils::SetBool(src_op, ge::FULL_CONNECTION_ATTR_RELU_FLAG,
                                     true),
             OP_LOGD(FUSED_OP_TYPE.c_str(), "set FULLCONNECTION_ATTR_RELU_FLAG fail."),
             return NOT_CHANGED);
    ...
    for (auto in_anchor : src_node->GetAllInDataAnchors()) {
      FUSION_PASS_CHECK(in_anchor == nullptr,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "in_anchor is null, fusion failed."),
               return PARAM_INVALID);
      if (in_anchor->GetPeerOutAnchor() == nullptr ||
          in_anchor->GetPeerOutAnchor()->GetOwnerNode() == nullptr ||
          in_anchor->GetPeerOutAnchor()->GetOwnerNode()->GetOpDesc() ==
              nullptr) {
        continue;
      }
      string input_op_type =
          in_anchor->GetPeerOutAnchor()->GetOwnerNode()->GetOpDesc()->GetType();
      if (add_relu_fusion_input_op.find(input_op_type) ==
          add_relu_fusion_input_op.end()) {
        return NOT_CHANGED;
      }
    }
    src_op->SetType(ELTWISE);
    FUSION_PASS_CHECK(
     !ge::AttrUtils::SetInt(src_op, ge::ELTWISE_ATTR_MODE, cce::CC_ELTWISE_SUM),
