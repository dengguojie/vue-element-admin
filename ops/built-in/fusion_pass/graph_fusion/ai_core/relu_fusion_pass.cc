...
Status ReluFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(src_node == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "src_node is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(relu_node == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "relu_node is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(in_edges_size < 0,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "inEdges size is invalid"), return FAILED);
  Status ret2 = PatternFusionUtil::LinkControlEdge(relu_node, src_node);
  FUSION_PASS_CHECK(ret2 != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "LinkControlEdge failed."), return ret2);
  // delete relu
  Status ret3 = graph.RemoveNode(relu_node);
  FUSION_PASS_CHECK(ret3 != SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Isolate relu node failed"), return ret3);
...
Status ReluFusionPass::DoFusion(ge::NodePtr src_node, ge::NodePtr relu_node) {
  ...
  FUSION_PASS_CHECK(src_op == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "src_op is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(relu_op == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "relu_op is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (CONV2D == src_op->GetType()) {
    ...
    for (auto &in_anchor : src_node->GetAllInDataAnchors()) {
      FUSION_PASS_CHECK(in_anchor == nullptr,
                        VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "in_anchor is null, fusion failed."),
                        return PARAM_INVALID);
