...
namespace fe {
  ...
  Status PackFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping,
...
//A maximum of 63 tensors are supported in mini mode.
    FUSION_PASS_CHECK(inputs_num <= 63,
             OP_LOGD(FUSED_OP_TYPE.c_str(), "The amount of input of Pack node is less than 63."),
             return NOT_CHANGED);

    if (inputs_num > 63) {
      size_t nodes_num, nodes_num1;
      nodes_num1 = inputs_num % 63;
      if (nodes_num1 == 0) {
        nodes_num = inputs_num / 63;
      } else {
        nodes_num = inputs_num / 63 + 1;
      }
      size_t final_clear_node_num = inputs_num - (63 * (nodes_num-1));
      ...
      FUSION_PASS_CHECK(pack_base_node == nullptr,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "fusedNode:%s is null, fusion failed.",
               pack_base_node->GetName().c_str()), return PARAM_INVALID);
    ...
    for (size_t i = 0; i < nodes_num; i++) {
      if(i < nodes_num-1){
        ...
        FUSION_PASS_CHECK(pack_node == nullptr,
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "fusedNode:%s is null, fusion failed.",
                 pack_node->GetName().c_str()), return PARAM_INVALID);

        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(pack_node->GetOutDataAnchor(0),
                 pack_base_node->GetInDataAnchor(i)),
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                 pack_base_node->GetName().c_str(), i,
                 pack_node->GetName().c_str(), i),  return FAILED);

        for (size_t m = 0; m < 63; m++) {
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(m+i*63)->GetPeerOutAnchor(), pack_node->GetInDataAnchor(m)),
                   OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                   fusedNode->GetName().c_str(), (m+i*63), pack_node->GetName().c_str(), m),
                   return FAILED);
        }
      } else {
        ...
        FUSION_PASS_CHECK(last_pack_node == nullptr,
                 ...
                 return PARAM_INVALID);
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(last_pack_node->GetOutDataAnchor(0), pack_base_node->GetInDataAnchor(i)),
                  OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                  pack_base_node->GetName().c_str(), i, last_pack_node->GetName().c_str(), i),
                  return FAILED);

        for (size_t n = 0; n < final_clear_node_num; n++) {
          FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(n+i*63)->GetPeerOutAnchor(), last_pack_node->GetInDataAnchor(n)),
                   OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.",
                   fusedNode->GetName().c_str(), (n+i*63), last_pack_node->GetName().c_str(), n),
                   return FAILED);
        }
      }
    }
  ...
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(fusedNode),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove Node [%s] failed", fusedNode->GetName().c_str()), return FAILED);
