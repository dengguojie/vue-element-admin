...
vector<FusionPattern *> DecodeBboxV2InsertTransposePass::DefinePatterns() {
  ...
  new (std::nothrow) FusionPattern("DecodeBboxV2InsertTransposePass");
...
Status DecodeBboxV2InsertTransposePass::Fusion(ge::ComputeGraph &graph,
                                               Mapping &mapping,
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::GetBool(fuseDesc, "reversed_box", reversed_box),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "get DecodeBboxV2's attr reversed_box failed."),
           return FAILED);
  if (reversed_box == false) {
    ...
    FUSION_PASS_CHECK(!ge::AttrUtils::SetBool(fuseDesc, "reversed_box", true),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Set DecodeBboxV2's attr reversed_box True failed."),
             return FAILED);
  }
  else {
...
REGISTER_PASS("DecodeBboxV2InsertTransposePass", BUILT_IN_GRAPH_PASS,
              DecodeBboxV2InsertTransposePass);
}
