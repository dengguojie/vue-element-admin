  ...
  if (batchNormNode->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
    for (auto inDataAnchor : batchNormNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(
          ...
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: remove out data edge failed, index=[0].", batchNormNodeName.c_str()),
          return FAILED);
      FUSION_PASS_CHECK(
          ge::GraphUtils::AddEdge(updateNode->GetOutDataAnchor(0), inDataAnchor) != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add out data edge failed, index=[0].", batchNormNodeName.c_str()),
          return FAILED);
    }
  }
  if (batchNormNode->GetOutDataAnchor(1)->GetPeerInDataAnchors().size() > 0) {
    for (auto inDataAnchor : batchNormNode->GetOutDataAnchor(1)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(
          ...
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: remove out data edge failed, index=[1].", batchNormNodeName.c_str()),
          return FAILED);
      FUSION_PASS_CHECK(
          ge::GraphUtils::AddEdge(updateNode->GetOutDataAnchor(1), inDataAnchor) != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add out data edge failed, index=[1].", batchNormNodeName.c_str()),
          return FAILED);
    }
  }
  ...
  if (batchNormNode->GetOutDataAnchor(2)->GetPeerInDataAnchors().size() > 0) {
    for (auto inDataAnchor : batchNormNode->GetOutDataAnchor(2)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(
          ...
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: remove out data edge failed, index=[2].", batchNormNodeName.c_str()),
          return FAILED);
      FUSION_PASS_CHECK(
          ge::GraphUtils::AddEdge(updateNode->GetOutDataAnchor(2), inDataAnchor) != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add out data edge failed, index=[2].", batchNormNodeName.c_str()),
          return FAILED);
    }
  }
  ...
  if (batchNormNode->GetOutDataAnchor(3)->GetPeerInDataAnchors().size() > 0) {
    for (auto inDataAnchor : batchNormNode->GetOutDataAnchor(3)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(
          ...
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: remove out data edge failed, index=[3].", batchNormNodeName.c_str()),
          return FAILED);
      FUSION_PASS_CHECK(
          ge::GraphUtils::AddEdge(updateNode->GetOutDataAnchor(3), inDataAnchor) != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add out data edge failed, index=[3].", batchNormNodeName.c_str()),
          return FAILED);
    }
  }
  ...
  if (batchNormNode->GetOutDataAnchor(4)->GetPeerInDataAnchors().size() > 0) {
    for (auto inDataAnchor : batchNormNode->GetOutDataAnchor(4)->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(
          ...
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: remove out data edge failed, index=[4].", batchNormNodeName.c_str()),
          return FAILED);
      FUSION_PASS_CHECK(
          ge::GraphUtils::AddEdge(updateNode->GetOutDataAnchor(4), inDataAnchor) != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add out data edge failed, index=[4].", batchNormNodeName.c_str()),
          return FAILED);
    }
  }
  ...
  if (batchNormNode->GetOutControlAnchor()) {
    for (auto inControlAnchor : batchNormNode->GetOutControlAnchor()->GetPeerInControlAnchors()) {
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(batchNormNode->GetOutControlAnchor(), inControlAnchor) != SUCCESS,
                        OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: remove out control edge failed, index=[0].",
                                batchNormNodeName.c_str()),
                        return FAILED);
      FUSION_PASS_CHECK(
          ge::GraphUtils::AddEdge(updateNode->GetOutControlAnchor(), inControlAnchor) != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add out control edge failed, index=[0].", batchNormNodeName.c_str()),
          return FAILED);
    }
  }
