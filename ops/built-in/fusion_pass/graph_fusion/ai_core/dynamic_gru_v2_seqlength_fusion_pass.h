...
namespace fe {
class DynamicGRUV2SeqFusionPass : public PatternFusionBasePass {
 protected:
  vector<FusionPattern *> DefinePatterns() override;
  Status Fusion(ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes) override;
 private:
  Status AddRNNMaskNode(ge::NodePtr fusedNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes);
  const string FUSED_OP_TYPE = "DynamicGRUV2Seq";
};
}  // namespace fe

#endif  // OPS_BUILT_IN_FUSION_PASS_GRAPH_FUSION_AI_CORE_DYNAMIC_GRU_V2_SEQLENGTH_FUSION_PASS_H