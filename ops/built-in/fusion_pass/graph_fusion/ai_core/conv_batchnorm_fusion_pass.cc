...
vector<FusionPattern *> ConvBatchnormFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("ConvBatchnomFusion");
  FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new an object not success."),
           return patterns);
  ...
  */
  ...
  FusionPattern *pattern1 =
      new (std::nothrow) FusionPattern("ConvBatchnomFusion1");
  FUSION_PASS_CHECK(
...
Status ConvBatchnormFusionPass::CheckWeights(const ge::NodePtr bnNode) {
  ...
  FUSION_PASS_CHECK(destOpDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "destOpDesc is null."),
...
Status ConvBatchnormFusionPass::Fusion(ge::ComputeGraph &graph,
                                       Mapping &mapping,
  ...
  FUSION_PASS_CHECK(ret != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "ConvNode[%s]: LinkControlEdge not success.",
                   convNodeName.c_str()),
           return ret);

  // 1. get the filterFormat and kernelNum
  int filterInputIndex = -1;
  FUSION_PASS_CHECK(GetConvFilterInputIndex(convNode, filterInputIndex) != SUCCESS,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "ConvNode[%s]: GetConvFilterInputIndex not success.",
                   convNodeName.c_str()),
           return NOT_CHANGED);
  ge::GeTensorDesc filterInputDesc =
      convNode->GetOpDesc()->GetInputDesc(filterInputIndex);
  size_t kernerlIndex = 0;
  ge::Format filterFormat;
  FUSION_PASS_CHECK(GetConvKernelIndex(convNode->GetOpDesc(), filterInputDesc,
                              filterFormat, kernerlIndex) != SUCCESS,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "ConvNode[%s]: GetConvKernelIndex not succeess, "
                   "filterInputIndex=[%d].",
  ...
  // create host op
  bool hasBias = true;
  if (convNode->GetAllInDataAnchors().size() <= BIAS_INDEX) {
    AddBiasNode(graph, convNode);
    hasBias = false;
  }
  std::shared_ptr<ge::OpDesc> filterHostOpdesc = nullptr;
  filterHostOpdesc = std::make_shared<ge::OpDesc>(
      convNode->GetName() + destNode->GetName() + "_filter_host",
      FILTER_HOST_TYPE);
  std::shared_ptr<ge::OpDesc> biasHostOpdesc = std::make_shared<ge::OpDesc>(
      convNode->GetName() + destNode->GetName() + "_bias_host", BIAS_HOST_TYPE);
  ...
  ge::GeTensorDesc filterOutputDesc =
      convNode->GetInDataAnchor(filterInputIndex)
          ->GetPeerOutAnchor()
          ->GetOwnerNode()
          ->GetOpDesc()
          ->GetOutputDesc(0);
  ge::OutDataAnchorPtr filterOutputAnchor =
      convNode->GetInDataAnchor(filterInputIndex)->GetPeerOutAnchor();
  ...
  FUSION_PASS_CHECK(!conv2dInputsName.empty() && !conv2dInputsName[0].empty() &&
           ...
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add const input of ConvBnFilterHost failed."),
           return FAILED);
  FUSION_PASS_CHECK(filterHostOpdesc->AddOutputDesc("y", filterOutputDesc) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add const output of ConvBnFilterHost failed."),
           return FAILED);
  ...
  FUSION_PASS_CHECK(biasHostOpdesc->AddOutputDesc("y", biasOutputDesc) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "add const output of ConvBnBiasHost failed."),
           return FAILED);
  ...
  FUSION_PASS_CHECK(
      ge::GraphUtils::AddEdge(filterHostNode->GetOutDataAnchor(0),
                              convNode->GetInDataAnchor(filterInputIndex)) !=
          SUCCESS,
  ...
  float eps = 0.0;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetFloat(destNode->GetOpDesc(),
                                    ge::BATCHNORM_ATTR_EPSILON, eps),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "DestNode[%s]: get epsilon attr %s not success.",
                   destNode->GetName().c_str(),
                   ge::BATCHNORM_ATTR_EPSILON.c_str()),
           return FAILED);
  if (eps < EPS) {
    OP_LOGI(FUSED_OP_TYPE.c_str(),
        "DestNode[%s]: the epsilon is less than %f, set the default value %f.",
        destNode->GetName().c_str(), EPS, EPS_DEFAULT_FLOAT);
    eps = EPS_DEFAULT_FLOAT;
  }
  (void)ge::AttrUtils::SetFloat(filterHostNode->GetOpDesc(),
                                ge::BATCHNORM_ATTR_EPSILON, eps);
  (void)ge::AttrUtils::SetFloat(biasHostNode->GetOpDesc(),
                                ge::BATCHNORM_ATTR_EPSILON, eps);
   ...
   * conv+bn fusion */
  if (!ge::AttrUtils::GetBool(destNode->GetOpDesc(),
                              NEED_ADD_AND_RSQRT, needAddAndRsqrt)) {
    needAddAndRsqrt = true;
  }

  /* here else is ommited because the needAddAndRsqrt is got from the func
   * GetBool */
  (void)ge::AttrUtils::SetBool(filterHostNode->GetOpDesc(), NEED_ADD_AND_RSQRT,
                               needAddAndRsqrt);
  (void)ge::AttrUtils::SetBool(biasHostNode->GetOpDesc(), NEED_ADD_AND_RSQRT,
                               needAddAndRsqrt);
