...
ExtremumGradFusionPass::ExtremumGradFusionPass() {}

ExtremumGradFusionPass::~ExtremumGradFusionPass() {}

vector<FusionPattern *> ExtremumGradFusionPass::DefinePatterns() {
  vector<FusionPattern *> patterns;
  return patterns;
}

Status ExtremumGradFusionPass::Fusion(
...
Status
ExtremumGradFusionPass::Run(ge::ComputeGraph &graph,
                            OpsKernelInfoStorePtr opsKernelInfoStorePtr) {
  // Step1: Check there must exist MaximumGrad/MinimumGrad op, otherwise exit
  if (CheckImplyType() == false) {
    return NOT_CHANGED;
  }
  // Step2: Record all GreaterEqual, LessEqual op in graph
  ...
  for (ge::NodePtr n : graph.GetDirectNode()) {
    if (n->GetOpDesc()->GetType() == GREATER_EQUAL ||
        n->GetOpDesc()->GetType() == LESS_EQUAL) {
      nodeEquals.push_back(n);
    }
  }
  // Step3: For each equal op, do fusion operation
  ...
  for (ge::NodePtr nodeEqual : nodeEquals) {
    ...
    if (ret == SUCCESS) {
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Run one pattern fusion failed.");
      return FAILED;
    }
  }
  return finalRet;
 ...
 *
 */
bool ExtremumGradFusionPass::CheckImplyType() {
  domi::ImplyType greaterEqualImplyType =
      domi::OpRegistry::Instance()->GetImplyType(GREATER_EQUAL);
  domi::ImplyType lessEqualImplyType =
      domi::OpRegistry::Instance()->GetImplyType(LESS_EQUAL);
  if (greaterEqualImplyType != domi::ImplyType::TVM &&
      lessEqualImplyType != domi::ImplyType::TVM) {
    OP_LOGD(FUSED_OP_TYPE.c_str(), "There is no GreaterEqual or LessEqual op,"
 ...
 */
bool ExtremumGradFusionPass::CheckNameScope(const string &nameA, const string &nameB) {
...
bool ExtremumGradFusionPass::CheckZeroConstantOp(ge::NodePtr nodeZeros) {
  if (nodeZeros == nullptr) {
      OP_LOGW(FUSED_OP_TYPE.c_str(), "nodeZeros is nullptr");
      return false;
  }

  if (nodeZeros->GetOpDesc()->GetType() != "Constant") {
    return false;
  }
...
bool ExtremumGradFusionPass::CheckSumOp(ge::NodePtr nodeSum, ge::NodePtr nodeEqual) {
  ...
  if (nodeSum->GetOpDesc()->GetType() != SUM &&
      nodeSum->GetOpDesc()->GetType() != REDUCESUMD) {
    return false;
  }
...
bool ExtremumGradFusionPass::CheckSameZeroNode(ge::NodePtr nodeZeros,
...
bool ExtremumGradFusionPass::MatchDx(ge::NodePtr nodeSelect,
  ...
  if ((recordMap.find(PATTERN_SELECT_DX) != recordMap.end()) ||
      (recordMap.find(PATTERN_SUM_DX) != recordMap.end())) {
    return false;
  }
  ...
  ge::OutDataAnchorPtr input0PeerAnchor =
      nodeSelect->GetInDataAnchor(SELECT_INPUT_0)->GetPeerOutAnchor();
  ...
  ge::OutDataAnchorPtr input1PeerAnchor =
      nodeSelect->GetInDataAnchor(SELECT_INPUT_1)->GetPeerOutAnchor();
  ...
  ge::OutDataAnchorPtr input2PeerAnchor =
      nodeSelect->GetInDataAnchor(SELECT_INPUT_2)->GetPeerOutAnchor();
  ...
  if (outputNodes.size() == 1) {
    ...
    if (dataType == ge::DT_INT32 &&
        nodeSum->GetOpDesc()->GetType() == REDUCESUM){
        return false;
    }
  }
  return true;
...
bool ExtremumGradFusionPass::MatchDy(ge::NodePtr nodeSelect,
                                     map<string, ge::NodePtr> &recordMap) {
  // Step1: Get GreaterEqual/LessEqual node from record map
  ge::NodePtr nodeEqual = nullptr;
  auto iterEqual = recordMap.find(PATTERN_EQUAL);
  ...
  if ((recordMap.find(PATTERN_SELECT_DY) != recordMap.end()) ||
      (recordMap.find(PATTERN_SUM_DY) != recordMap.end())) {
    return false;
  }

  // Step3: Check nodeSelect's validity (3 input anchor, 1 output anchor)
  if (CheckSelectOp(nodeSelect, nodeEqual) == false) {
      return false;
  }
...
bool ExtremumGradFusionPass::CheckEqualOp(ge::NodePtr nodeEqual) {
  ...
  if (nodeEqual->GetOpDesc()->GetType() == GREATER_EQUAL &&
      scopeUpper != SCOPE_MAXIMUM_GRAD) {
    return false;
  } else if (nodeEqual->GetOpDesc()->GetType() == LESS_EQUAL &&
             scopeUpper != SCOPE_MINIMUM_GRAD) {
    return false;
  }
...
Status ExtremumGradFusionPass::RunOnePatternFusion(ge::ComputeGraph &graph,
                                                   ge::NodePtr nodeEqual) {
  map<string, ge::NodePtr> recordMap;
  // Step1: check nodeEqual's validity, two input data anchor, one output data
  // anchor
  if (CheckEqualOp(nodeEqual) == false) {
      return NOT_CHANGED;
  }
  ...
  if (equalOutAnchor == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[equalOutAnchor] must not be null.");
  ...
  if (equalOutAnchor->GetPeerInDataAnchors().size() ==
      EQUAL_WITH_TWO_OUTPUT_NODE) {
    auto peerAnchor0 = equalOutAnchor->GetPeerInDataAnchors().at(0);
    if (peerAnchor0 == nullptr) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[peerAnchor0] must not be null.");
      return fe::PARAM_INVALID;
    }
    ge::NodePtr node0 = peerAnchor0->GetOwnerNode();
    if (node0 == nullptr) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[node0] must not be null.");
      return fe::PARAM_INVALID;
    }

    auto peerAnchor1 = equalOutAnchor->GetPeerInDataAnchors().at(1);
    if (peerAnchor1 == nullptr) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[peerAnchor1] must not be null.");
      return fe::PARAM_INVALID;
    }
    ge::NodePtr node1 = peerAnchor1->GetOwnerNode();
    if (node1 == nullptr) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[node1] must not be null.");
      return fe::PARAM_INVALID;
    }
  ...
  } else if (equalOutAnchor->GetPeerInDataAnchors().size() ==
             EQUAL_WITH_ONE_OUTPUT_NODE) {
    auto peerAnchor0 = equalOutAnchor->GetPeerInDataAnchors().at(0);
    if (peerAnchor0 == nullptr) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[peerAnchor0] must not be null.");
      return fe::PARAM_INVALID;
...
void ExtremumGradFusionPass::SetExtemDataDumpAttr(
    const std::map<string, ge::NodePtr> &recordMap,
    vector<ge::NodePtr> &fusionNodes) {
...
ge::NodePtr ExtremumGradFusionPass::FindNodeInRecordMap(const map<string, ge::NodePtr> &recordMap,
                                string key) {
...
bool ExtremumGradFusionPass::CheckAttrMatch(
    const map<string, ge::NodePtr> &recordMap) {
  // attr _stream_label of fusion nodes must be equal
  string streamLabel = "";
  for (auto iter = recordMap.begin(); iter != recordMap.end(); iter++) {
    string streamLabelTmp = "";
    if (!ge::AttrUtils::GetStr(iter->second->GetOpDesc(), STREAM_LABEL,
                               streamLabelTmp)) {
      streamLabelTmp = "null";
...
Status
ExtremumGradFusionPass::DoFusion(ge::ComputeGraph &graph,
                                 const map<string, ge::NodePtr> &recordMap,
                                 vector<ge::NodePtr> &fusionNodes) {
  ...
  if (nodeEqual == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[nodeEqual] must not be null.");
    return fe::PARAM_INVALID;
  }
  ...
  if (dzInputNode == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[dzInputNode] must not be null.");
    return fe::PARAM_INVALID;
  }
  ...
  for (auto anchor : dzInputNode->GetAllOutDataAnchors()) {
    ...
    for (auto peerAnchor : anchor->GetPeerInDataAnchors()) {
      if (peerAnchor == nullptr) {
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[peerAnchor] must not be null.");
        return fe::PARAM_INVALID;
      }
      if (peerAnchor->GetOwnerNode() == selectDxNode ||
          peerAnchor->GetOwnerNode() == selectDyNode) {
  ...
  extreGradNode = CreateExtremumGradNode(graph, nodeEqual, selectDxNode,
                                         selectDyNode, recordMap);
  if (extreGradNode == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[extreGradNode] must not be null.");
    return fe::PARAM_INVALID;
  }
  // Step4: Relink outerX, outerY, outerDz to MaximumGrad/MinimumGrad intput0,
  // input1, input2,
  Status adjustAnchorRes = AdjustAnchor(dzInputAnchor, nodeEqual, extreGradNode, outputDxNode,
                               outputDyNode);
  FUSION_PASS_CHECK(adjustAnchorRes != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Replace ExtremumGrad node to ComputeGraph failed."),
  ...
  for (auto anchor : extreGradNode->GetAllInDataAnchors()) {
    ...
    FUSION_PASS_CHECK(extreGradNode->GetOpDesc()->UpdateInputDesc(
                 anchor->GetIdx(), outputTensor) != SUCCESS,
...
Status ExtremumGradFusionPass::RemoveInputEdges(ge::ComputeGraph &graph, ge::NodePtr node) {
  if (node == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[node] must not be null.");
    return fe::PARAM_INVALID;
  }
  for (ge::InDataAnchorPtr anchor : node->GetAllInDataAnchors()) {
    ...
    if (ge::GraphUtils::RemoveEdge(srcAnchor, anchor) != ge::GRAPH_SUCCESS) {
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Disconnect %s input data anchor failed.",
                node->GetName().c_str());
        return FAILED;
    }
    ...
    if (srcNode == nullptr) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[srcNode] must not be null.");
      return fe::PARAM_INVALID;
    }
    if (srcNode->GetInAllNodes().size() == 0 &&
        srcNode->GetOutAllNodes().size() == 0) {
      // Delete isolated const node from graph
      if (graph.RemoveNode(srcNode) != ge::GRAPH_SUCCESS) {
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove node from graph failed.");
          return FAILED;
      }
    }
  }
  ...
  if (inCtrlAnchor != nullptr) {
    for (ge::OutControlAnchorPtr srcAnchor :
         inCtrlAnchor->GetPeerOutControlAnchors()) {
      if (ge::GraphUtils::RemoveEdge(srcAnchor, inCtrlAnchor) !=
              ge::GRAPH_SUCCESS) {
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Disconnect %s input control anchor failed.",
                  node->GetName().c_str());
          return FAILED;
      }
    }
...
Status ExtremumGradFusionPass::RemoveOutputEdges(ge::NodePtr node) {
  if (node == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[node] must not be null.");
    return fe::PARAM_INVALID;
  }
  for (ge::OutDataAnchorPtr anchor : node->GetAllOutDataAnchors()) {
    ...
    for (ge::InDataAnchorPtr dstAnchor : anchor->GetPeerInDataAnchors()) {
      if (ge::GraphUtils::RemoveEdge(anchor, dstAnchor) != ge::GRAPH_SUCCESS) {
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Disconnect %s output data anchor failed.",
                  node->GetName().c_str());
          return FAILED;
      }
    }
  ...
  if (outControlAnchor != nullptr) {
    for (ge::InControlAnchorPtr dstAnchor :
         outControlAnchor->GetPeerInControlAnchors()) {
        if (ge::GraphUtils::RemoveEdge(outControlAnchor, dstAnchor) !=
              ge::GRAPH_SUCCESS) {
            OP_LOGE(FUSED_OP_TYPE.c_str(), "Disconnect %s output control anchor failed.",
                    node->GetName().c_str());
            return FAILED;
        }
    }
...
Status
ExtremumGradFusionPass::RemoveNode(ge::ComputeGraph &graph,
                                   const map<string, ge::NodePtr> &recordMap,
                                   string patternName) {
  ...
  if (removeInputEdges != SUCCESS) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove node:%s input edges failed.",
                                   node->GetName().c_str());
  ...
  if (removeOutputEdges != SUCCESS) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove node:%s output edges failed.",
              node->GetName().c_str());
...
ge::NodePtr ExtremumGradFusionPass::CreateExtremumGradNode(
    ge::ComputeGraph &graph, ge::NodePtr nodeEqual, ge::NodePtr selectDxNode,
    ge::NodePtr selectDyNode, const map<string, ge::NodePtr> &recordMap) {
  if (nodeEqual == nullptr) {
  ...
  if (SetExtreMumGradOpDesc(equalOpDesc, selectOpDesc, extreGradOpDesc) !=
          SUCCESS) {
  ...
  ge::GeTensorDesc inputTensorDesc3 =
      iterDz->second->GetOpDesc()->GetOutputDesc(0);
  if (extreGradOpDesc->AddInputDesc(inputTensorDesc1) !=
                                  ge::GRAPH_SUCCESS) {
      OP_LOGW(FUSED_OP_TYPE.c_str(), "add input desc[0] to %s not success",
              extreGradOpDesc->GetName().c_str());
      return nullptr;
  }
  if (extreGradOpDesc->AddInputDesc(inputTensorDesc2) !=
                                  ge::GRAPH_SUCCESS) {
      OP_LOGW(FUSED_OP_TYPE.c_str(), "add input desc[1] to %s not success",
              extreGradOpDesc->GetName().c_str());
      return nullptr;
  }

  if (extreGradOpDesc->AddInputDesc(inputTensorDesc3) !=
                                  ge::GRAPH_SUCCESS) {
      OP_LOGW(FUSED_OP_TYPE.c_str(), "add input desc[2] to %s not success",
              extreGradOpDesc->GetName().c_str());
      return nullptr;
  }
  ...
  if (recordMap.find(PATTERN_SELECT_DX) != recordMap.end()) {
    if (recordMap.find(PATTERN_SUM_DX) != recordMap.end()) {
      ...
      ge::GeTensorDesc sumOutputTensorDesc =
          sumNode->GetOpDesc()->GetOutputDesc(0);
      if (extreGradOpDesc->AddOutputDesc(sumOutputTensorDesc) !=
              ge::GRAPH_SUCCESS) {
          OP_LOGW(FUSED_OP_TYPE.c_str(), "add output desc with sumDx output not success");
          return nullptr;
      }
    } else {
      ge::GeTensorDesc dxTensorDesc =
          selectDxNode->GetOpDesc()->GetOutputDesc(0);
  ...
  if (recordMap.find(PATTERN_SELECT_DY) != recordMap.end()) {
    if (recordMap.find(PATTERN_SUM_DY) != recordMap.end()) {
      ...
      ge::GeTensorDesc dxTensorDesc =
          selectDyNode->GetOpDesc()->GetOutputDesc(0);
      if (extreGradOpDesc->AddOutputDesc(dxTensorDesc) != ge::GRAPH_SUCCESS) {
  ...
  if (recordMap.find(PATTERN_SELECT_DX) == recordMap.end() ||
      recordMap.find(PATTERN_SELECT_DY) == recordMap.end()) {
  ...
  string streamLabel = "";
  if (!ge::AttrUtils::GetStr(nodeEqual->GetOpDesc(), STREAM_LABEL,
                             streamLabel)) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "Fusion nodes not have _stream_label attr.");
  } else {
    if (!ge::AttrUtils::SetStr(extreGradOpDesc, STREAM_LABEL, streamLabel)) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "newNode set _stream_label error, fusion failed.");
      return nullptr;
    }
  }
  // Step5: Create node
...
Status
ExtremumGradFusionPass::SetExtreMumGradOpDesc(ge::OpDescPtr equalOpDesc,
                                              ge::OpDescPtr selectOpDesc,
                                              ge::OpDescPtr extreGradOpDesc) {
  if (equalOpDesc == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[equalOpDesc] must not be null.");
    return fe::PARAM_INVALID;
  }
  if (selectOpDesc == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[selectOpDesc] must not be null.");
    return fe::PARAM_INVALID;
  }
  if (extreGradOpDesc == nullptr) {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[extreGradOpDesc] must not be null.");
    return fe::PARAM_INVALID;
  }
  ...
  OP_LOGD(FUSED_OP_TYPE.c_str(), "add %s Op:%s in ExtremumGradFusionPass",
          extreGradOpDesc->GetType().c_str(),
          extreGradOpDesc->GetName().c_str());
  ++extremumGradCount;
  ...
  if (ge::AttrUtils::HasAttr(selectOpDesc, ATTR_DATA_TYPE)) {
    ...
    if (ge::AttrUtils::GetDataType(selectOpDesc, ATTR_DATA_TYPE,
                                   selectDataType) == false) {
        OP_LOGE(FUSED_OP_TYPE.c_str(), "get datatype from select node failed");
        return FAILED;
    }

    if(ge::AttrUtils::SetDataType(extreGradOpDesc, ATTR_DATA_TYPE,
                                   selectDataType) == false) {
        OP_LOGE(FUSED_OP_TYPE.c_str(), "set datatype to extremum_grad node failed");
        return FAILED;
    }
  } else {
    OP_LOGE(FUSED_OP_TYPE.c_str(), "Cannot find attr:T in node:%s", selectOpDesc->GetName().c_str());
    return FAILED;
  }
  // Init "grad_x", "grad_y" attr to ExtremumGrad OP
  if (ge::AttrUtils::SetBool(extreGradOpDesc, ATTR_GRAD_X, false) == false) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "set Grad_X to extremum_grad node failed");
      return FAILED;
  }

  if (ge::AttrUtils::SetBool(extreGradOpDesc, ATTR_GRAD_Y, false) == false) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "set Grad_Y to extremum_grad node failed");
      return FAILED;
  }
  return SUCCESS;
}

Status ExtremumGradFusionPass::AdjustAnchor(ge::OutDataAnchorPtr dzInputAnchor,
                                            ge::NodePtr nodeEqual,
                                            ge::NodePtr extreGradNode,
  ...
  if (ge::GraphUtils::AddEdge(dzInputAnchor, extreNodeInput0) !=
          ge::GRAPH_SUCCESS) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge between DzInput to ExtremumGrad input0 failed.");
      return FAILED;
  }
  ...
  for (auto inputAnchor : nodeEqual->GetAllInDataAnchors()) {
    if (inputAnchor == nullptr) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[inputAnchor] must not be null.");
      return fe::PARAM_INVALID;
    }
    ...
    if (replaceEdgeDstRes != SUCCESS) {
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Replace equal_inputAnchors[%d] to extreGradNode_inputAnchors[%d] failed",
  ...
  if (outputDxNode != nullptr) {
    ge::OutDataAnchorPtr extreGradOutAnchor0 =
        extreGradNode->GetOutDataAnchor(outputIndex);
    ge::OutDataAnchorPtr oldSrcAnchor = outputDxNode->GetOutDataAnchor(0);
    if (oldSrcAnchor == nullptr) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Parameter[oldSrcAnchor] must not be null.");
      return fe::PARAM_INVALID;
    }
    for (ge::InDataAnchorPtr dstAnchor : oldSrcAnchor->GetPeerInDataAnchors()) {
      ...
      if (replaceEdgeSrcRes != SUCCESS) {
          OP_LOGE(FUSED_OP_TYPE.c_str(), "Replace edge failed.");
          return replaceEdgeSrcRes;
      }
    }

    if (ge::AttrUtils::SetBool(extreGradNode->GetOpDesc(), ATTR_GRAD_X, true) ==
            false) {
        OP_LOGE(FUSED_OP_TYPE.c_str(), "set Grad_X = true to extremum_grad node failed");
        return FAILED;
    }
    outputIndex++;
  }
  ...
  if (outputDyNode != nullptr) {
    ...
    if (ge::AttrUtils::SetBool(extreGradNode->GetOpDesc(), ATTR_GRAD_Y, true) ==
            false) {
        OP_LOGE(FUSED_OP_TYPE.c_str(), "set Grad_Y = true to extremum_grad node failed");
        return FAILED;
    }
  }
...
Status ExtremumGradFusionPass::ReplaceEdgeDst(ge::OutDataAnchorPtr src,
  ...
  if (ge::GraphUtils::RemoveEdge(src, dst) != ge::GRAPH_SUCCESS ||
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Replace edge dst Failed.");
      return FAILED;
  }
  return SUCCESS;
}

Status ExtremumGradFusionPass::ReplaceEdgeSrc(ge::OutDataAnchorPtr src,
