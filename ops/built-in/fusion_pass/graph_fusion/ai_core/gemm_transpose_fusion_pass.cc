...
const string PATTERN_GEMM = "GEMM";
}
...
Status GemmTransFusionPass::Relink(ge::NodePtr aNode, ge::NodePtr transposeANode,
                     ge::NodePtr gemmNode, const int Anchor) {
  FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(
               aNode->GetOutDataAnchor(0),
               gemmNode->GetInDataAnchor(Anchor)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "fail to remove edge between aNode and gemmNode"),
           return FAILED);

  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(aNode->GetOutDataAnchor(0),
                                    transposeANode->GetInDataAnchor(0)) !=
                SUCCESS,
            OP_LOGE(FUSED_OP_TYPE.c_str(), "fail to add edge between aNode and transposeANode"),
            return FAILED);

  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(transposeANode->GetOutDataAnchor(0),
                                    gemmNode->GetInDataAnchor(Anchor)) !=
                SUCCESS,
            OP_LOGE(FUSED_OP_TYPE.c_str(), "fail to add edge between transposeANode and gemmNode"),
            return FAILED);

  FUSION_PASS_CHECK(gemmNode->GetOpDesc()->UpdateInputDesc(
               Anchor, transposeANode->GetOpDesc()->GetOutputDesc(0)) !=
               SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "fail to update input description of transdataANode"),
           return FAILED);

  return SUCCESS;
}

Status GemmTransFusionPass::Fusion(ge::ComputeGraph &graph,
                                           Mapping &mapping,
  ...
  if (op.GetAttr("transpose_a", transposeA) != GRAPH_SUCCESS) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "op gemm get attribute transpose_a failed or transpose_a not exist");
  }

  if (op.GetAttr("transpose_b", transposeB) != GRAPH_SUCCESS) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "op gemm get attribute transpose_b failed or transpose_b not exist");
  }

  // prerequisite
  ge::NodePtr aNode = gemmNode->GetInDataAnchor(aAnchor)->GetPeerOutAnchor()->GetOwnerNode();
  int aIdx = gemmNode->GetInDataAnchor(aAnchor)->GetPeerOutAnchor()->GetIdx();

  ge::NodePtr bNode = gemmNode->GetInDataAnchor(bAnchor)->GetPeerOutAnchor()->GetOwnerNode();
  int bIdx = gemmNode->GetInDataAnchor(bAnchor)->GetPeerOutAnchor()->GetIdx();

  ge::NodePtr cNode = gemmNode->GetInDataAnchor(cAnchor)->GetPeerOutAnchor()->GetOwnerNode();
  ...
  if (aFormat == ge::FORMAT_ND && bFormat == ge::FORMAT_ND && cFormat == ge::FORMAT_ND && (nDirectionLength % 16 == 0)) {
    needTranspose = false;
  }
  ...
  if (transposeA && needTranspose) {
    // transpose a
    FUSION_PASS_CHECK(GenerateTransposeNode(graph, aOutDesc, gemmAInDesc,
                                    transPerm, transposeANode, basenameA) != SUCCESS,
              OP_LOGE(FUSED_OP_TYPE.c_str(), "fail to generate transpose node A"), return FAILED);
    // relink a
    FUSION_PASS_CHECK(Relink(aNode, transposeANode, gemmNode, aAnchor) != SUCCESS,
  ...
  if (transposeB && needTranspose) {
    // transpose b
    FUSION_PASS_CHECK(GenerateTransposeNode(graph, bOutDesc, gemmBInDesc,
                                    transPerm, transposeBNode, basenameB) != SUCCESS,
              OP_LOGE(FUSED_OP_TYPE.c_str(), "fail to generate transpose node B"), return FAILED);
    // relink b
    FUSION_PASS_CHECK(Relink(bNode, transposeBNode, gemmNode, bAnchor) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "fail to relink nodes"), return FAILED);
...
REGISTER_PASS("GemmTransFusionPass", BUILT_IN_GRAPH_PASS,
              GemmTransFusionPass);
} // namespace fe
