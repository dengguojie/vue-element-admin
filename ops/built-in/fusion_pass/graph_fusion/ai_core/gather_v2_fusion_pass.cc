...
Status ConstToAttrGatherV2Pass::Fusion(ge::ComputeGraph &graph,
                                       Mapping &mapping,
                                       vector<ge::NodePtr> &fusionNodes)
{
  ...
  FUSION_PASS_CHECK(fusedDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node:%s's OpDesc is null, fusion failed.", fusedNode->GetName().c_str()),return PARAM_INVALID);

  // check op support for dynamic gather_v2
  FUSION_PASS_CHECK(CheckOpSupported(fusedDesc),
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Op Gatherv2 Supported."),
           return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(!CheckOpSupported(fusionDescPtr),
            OP_LOGI(FUSED_OP_TYPE.c_str(), "Op Gatherv2 Not Supported."),
            return NOT_CHANGED);
