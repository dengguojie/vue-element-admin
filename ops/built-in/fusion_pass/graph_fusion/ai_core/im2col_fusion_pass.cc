...
Status Im2colFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(im2colNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "im2col node is null."),
                    return PARAM_INVALID);              
  ge::OpDescPtr im2colOpDesc = im2colNode->GetOpDesc();
  FUSION_PASS_CHECK(im2colOpDesc == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "im2col is null."),
                    return PARAM_INVALID);
  ...
  if (outputOriginFormat == ge::FORMAT_NCHW) {
    ...
    FUSION_PASS_CHECK(ksizes.empty(), VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "ksizes is null, please check!"),
                      return FAILED);
    ...
    FUSION_PASS_CHECK(input_assist_1.get() == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "input_assist is NULL"),
                      return PARAM_INVALID);
    unique_ptr<int32_t[]> input_assist_2(new (nothrow) int32_t[4]());
    FUSION_PASS_CHECK(input_assist_2.get() == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "input_assist is NULL"),
                      return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "reshape2 UpdateOutputDesc failed."),
                      return FAILED);
