...
namespace fe
{
    ...
    vector<FusionPattern*> ScatterNdFusionPass::DefinePatterns()
    {
        ...
        pattern->AddOpDesc(PATTERN_SCATTERND, {SCATTERND})
            .SetOutput(PATTERN_SCATTERND);
...
Status ScatterNdFusionPass::Fusion(ge::ComputeGraph &graph,
                                   Mapping &mapping,
                                   vector<ge::NodePtr> &fusionNodes)
{
    ge::NodePtr ScatterNdNode = GetNodeFromMapping(PATTERN_SCATTERND, mapping);
    FUSION_PASS_CHECK(ScatterNdNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "ScatterNdNode is null, fusion failed."), return PARAM_INVALID);
    ge::OpDescPtr ScatterNdDesc = ScatterNdNode->GetOpDesc();
    FUSION_PASS_CHECK(ScatterNdDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "ScatterNdNode's OpDesc is null, fusion failed."),return PARAM_INVALID);

    // check op support for dynamic scatter_nd
    FUSION_PASS_CHECK(CheckOpSupported(ScatterNdDesc),
             OP_LOGI(FUSED_OP_TYPE.c_str(), "Op ScatterNd Supported."),
             return NOT_CHANGED);
    ...
    ge::OpDescPtr fusionDescPtr =
        PatternFusionUtil::GetFusionOpDesc(ScatterNdNode, fusionOpType, attrInfos);
