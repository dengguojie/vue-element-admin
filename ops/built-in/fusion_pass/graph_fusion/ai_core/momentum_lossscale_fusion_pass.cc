...
vector<FusionPattern*> MomentumLossscaleFusionPass::DefinePatterns() {
  ...
  pattern->AddOpDesc(MOMENTUM_LOSSSCALE_PATTERN_ID_MOMENTUM, {APPLYMOMENTUM})
      ...
      .SetInputs(MOMENTUM_LOSSSCALE_PATTERN_ID_MOMENTUM, {MOMENTUM_LOSSSCALE_PATTERN_ID_MUL})
...
Status MomentumLossscaleFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  ge::NodePtr momentum_node = GetNodeFromMapping(MOMENTUM_LOSSSCALE_PATTERN_ID_MOMENTUM, mapping);
  ge::NodePtr mul_node = GetNodeFromMapping(MOMENTUM_LOSSSCALE_PATTERN_ID_MUL, mapping);

  FUSION_PASS_CHECK(momentum_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "momentum_node is null, fusion failed."),
  ...
  ge::AttrUtils::GetStr(momentum_node->GetOpDesc(), STREAM_LABEL, momentum_stream_label);
  ...
  FUSION_PASS_CHECK(momentum_node->GetType() != APPLYMOMENTUM,
                    OP_LOGI(FUSED_OP_TYPE.c_str(), "Get the momentum_node type is [%s], which is not apply momentum.",
  ...
  for (auto out_data_anchor : out_dataanchor_list) {
    for (auto peer_indata_anchor : out_data_anchor->GetPeerInDataAnchors()) {
      if (peer_indata_anchor->GetOwnerNode()->GetType() == APPLYMOMENTUM) {
        ...
        if (grad_index != 3) {
          OP_LOGI(FUSED_OP_TYPE.c_str(),
                  "The input mul node %s of ResourceApplyMomentum is not "
  ...
  for (auto index_name : momentum_input_names) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "momentum_node's input name is :%s, input index is %u", index_name.first.c_str(),
            index_name.second);
  }
  ...
  if (doFusionFlag) {
    ...
    FUSION_PASS_CHECK(
        ...
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Fail to update output desc[0] of op[%s].", copy_desc_ptr->GetName().c_str()),
        return FAILED);
    ...
    FUSION_PASS_CHECK(old_desc == nullptr,
                      OP_LOGE(FUSED_OP_TYPE.c_str(), "momentum_node's opDesc is null, fusion failed."), return FAILED);
    ...
    for (uint32_t i = 0; i < momentum_node->GetAllInDataAnchors().size(); i++) {
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(momentum_node->GetInDataAnchor(i)->GetPeerOutAnchor(),
                        ...
                        OP_LOGE(FUSED_OP_TYPE.c_str(), "Fail to add input edge for new momentum_node."), return FAILED);
    }
    ...
    FUSION_PASS_CHECK(
        ...
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Fail to add output[1] edge for new momentum_node."), return FAILED);
    ...
    for (auto in_anchor : var_outanchor->GetPeerInDataAnchors()) {
      if (in_anchor->GetOwnerNode()->GetOpDesc()->GetType() == "Conv2D" ||
        ...
        FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(copy_var_node->GetOutDataAnchor(0), in_anchor) != SUCCESS,
                          OP_LOGE(FUSED_OP_TYPE.c_str(), "Fail to add output edge for copy_var_node."), return FAILED);
      }
    }
