...
Status HostBNFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(bnNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node bninfenced is null, fusion failed."),
...
Status HostBNFusionPass::CheckParameter(ge::NodePtr& bnNodePtr) {
  ...
  FUSION_PASS_CHECK((inNodes.size() != 4 && inNodes.size() != 6),
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "BNInference input nodes num(%d) != 4/6", inNodes.size()),
                    return PARAM_INVALID);
  return SUCCESS;
}
...
Status HostBNFusionPass::BNFuison(ge::ComputeGraph& graph, ge::NodePtr& bnNodePtr, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(
      bnOpDescPtr == nullptr,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Node:%s's OpDesc is null, fusion failed.", bnOpDescPtr->GetName().c_str()),
      return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove input edge from fused data node:%s.", bnNodePtr->GetName().c_str()),
      return FAILED);
  FUSION_PASS_CHECK(
      SUCCESS != ge::GraphUtils::RemoveEdge(meanNodePtr->GetOutAnchor(0), bnNodePtr->GetInDataAnchor(1)),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove input edge from fused mean node:%s.", bnNodePtr->GetName().c_str()),
      return FAILED);
  // delete edge of prenode and psroinode
  FUSION_PASS_CHECK(
      SUCCESS != ge::GraphUtils::RemoveEdge(varNodePtr->GetOutAnchor(0), bnNodePtr->GetInDataAnchor(2)),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove input edge from fused var node:%s.", bnNodePtr->GetName().c_str()),
      return FAILED);
  FUSION_PASS_CHECK(
      SUCCESS != ge::GraphUtils::RemoveEdge(momentumNodePtr->GetOutAnchor(0), bnNodePtr->GetInDataAnchor(3)),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove input edge from fused mu node:%s.", bnNodePtr->GetName().c_str()),
      return FAILED);
  if (inputNodes.size() == 6) {
    FUSION_PASS_CHECK(
        SUCCESS != ge::GraphUtils::RemoveEdge(scaleNodePtr->GetOutAnchor(0), bnNodePtr->GetInDataAnchor(4)),
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove input edge from fused scale node:%s.", bnNodePtr->GetName().c_str()),
        return FAILED);
    FUSION_PASS_CHECK(
        SUCCESS != ge::GraphUtils::RemoveEdge(offsetNodePtr->GetOutAnchor(0), bnNodePtr->GetInDataAnchor(5)),
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove input edge from fused offset node:%s.", bnNodePtr->GetName().c_str()),
        return FAILED);
  }
...
Status HostBNFusionPass::SetAttrValue(const ge::OpDescPtr& preOpDescPtr, ge::OpDescPtr& newOpDescPtr) {
  ...
  FUSION_PASS_CHECK(preOpDescPtr->GetAttr(USE_GLOBAL_STATS, use_global_stats) == ge::GRAPH_FAILED,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "Get attr %s from node %s error", USE_GLOBAL_STATS.c_str(),
                            preOpDescPtr->GetName().c_str()),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Get attr %s from node %s error", MODE.c_str(), preOpDescPtr->GetName().c_str()),
      return FAILED);

  FUSION_PASS_CHECK(
      newOpDescPtr->SetAttr(MODE, mode) == ge::GRAPH_FAILED,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Set attr %s to node %s error", MODE.c_str(), newOpDescPtr->GetName().c_str()),
      return FAILED);
  ...
  if (preOpDescPtr->GetAttr(DTLIST, outputDtList) == ge::GRAPH_SUCCESS) {
    FUSION_PASS_CHECK(
        ...
        OP_LOGE(FUSED_OP_TYPE.c_str(), "Set attr %s to node %s error", DTLIST.c_str(), newOpDescPtr->GetName().c_str()),
        return FAILED);
  }
