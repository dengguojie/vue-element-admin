...
ge::GeTensorDesc RNNFusionPass::ProcessStatic(ge::NodePtr fusedNode, int32_t num_output, ge::NodePtr &innerproductNode, ge::ComputeGraph &graph, vector<ge::NodePtr> &newNodes, bool &failStatus)
{
    ...
    ge::OpDescPtr innerProductStaticDesc = std::make_shared<ge::OpDesc>(fusedDesc->GetName() + "/W_xh_x_static", "FullyConnection");
...
vector<ge::NodePtr> RNNFusionPass::ProcessRnnCell(ge::NodePtr fusedNode, ge::ComputeGraph &graph, ge::GeTensorDesc &outInnerProductTensorDesc, vector<ge::NodePtr> &newNodes, bool &failStatus, const bool has_static)
{
    ...
    bool expose_hidden = false;
    ge::AttrUtils::GetBool(fusedDesc, "expose_hidden", expose_hidden);
    ge::GeTensorDesc biashTensorDesc;
    ge::GeTensorDesc biasoTensorDesc;
    ge::GeTensorDesc wxhTensorDesc;
    ge::GeTensorDesc whohhTensorDesc;
    // x->cont->x_static->h_0->w_xh->bias_h->w_sh->w_hh->w_ho->bias_o
    if (has_static && expose_hidden)
    {
        biashTensorDesc = fusedDesc->GetInputDesc("bias_h");
        biasoTensorDesc = fusedDesc->GetInputDesc("bias_o");
        wxhTensorDesc = fusedDesc->GetInputDesc("w_xh");
        whohhTensorDesc = fusedDesc->GetInputDesc("w_hh");
    }
    // x->cont->x_static->w_xh->bias_h->w_sh->w_hh->w_ho->bias_o
    else if (has_static && !expose_hidden)
    {
        biashTensorDesc = fusedDesc->GetInputDesc(4);
        biasoTensorDesc = fusedDesc->GetInputDesc(8);
        wxhTensorDesc = fusedDesc->GetInputDesc(3);
        whohhTensorDesc = fusedDesc->GetInputDesc(6);
    }
    // x->cont->h_0->w_xh->bias_h->w_hh->w_ho->bias_o
    else if (!has_static && expose_hidden)
    {
        biashTensorDesc = fusedDesc->GetInputDesc(4);
        biasoTensorDesc = fusedDesc->GetInputDesc(7);
        wxhTensorDesc = fusedDesc->GetInputDesc(3);
        whohhTensorDesc = fusedDesc->GetInputDesc(5);
    }
    // x->cont->w_xh->bias_h->w_hh->w_ho->bias_o
    else
    {
        biashTensorDesc = fusedDesc->GetInputDesc(3);
        biasoTensorDesc = fusedDesc->GetInputDesc(6);
        wxhTensorDesc = fusedDesc->GetInputDesc(2);
        whohhTensorDesc = fusedDesc->GetInputDesc(4);
    }
    ...
    ge::GeTensorDesc xInputTensorDesc = ge::GeTensorDesc(shape_input_0, ge::FORMAT_FRACTAL_NZ, inputTensorDesc0.GetDataType());
    ...
    {
        ge::OpDescPtr basicRNNDesc2 = std::make_shared<ge::OpDesc>(fusedDesc->GetName() + "/BasicRNNCell" + std::to_string(i), "BasicRNNCell");
        ...
        if (has_static)
        {
            ...
            ge::GeTensorDesc staticInputTensorDesc = ge::GeTensorDesc(outInnerProductShape, ge::FORMAT_NCHW, outInnerProductTensorDesc.GetDataType());
        ...
        FUSION_PASS_CHECK(basicRNNNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", basicRNNDesc2->GetName().c_str()), failStatus = true);
...
Status RNNFusionPass::Fusion(ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &newNodes)
{
    ...
    if (expose_hidden && input_size == 10)
    {
        has_static = true;
    }
    else if (!expose_hidden && input_size == 9)
    {
        has_static = true;
    }
    ...
    ge::GeTensorDesc splitNodeXDesc = ge::GeTensorDesc(inputTensorDesc0.GetShape(), inputTensorDesc0.GetFormat(), ge::DT_FLOAT16);
    ...
    ge::GeTensorDesc splitNodeContDesc = ge::GeTensorDesc(inputContTensorDesc.GetShape(), inputContTensorDesc.GetFormat(), ge::DT_FLOAT16);
    ...
    // add rnnCellNode
    vector<ge::NodePtr> rnnCellNode = ProcessRnnCell(fusedNode, graph, outInnerProductTensorDesc, newNodes, failStatus, has_static);
    FUSION_PASS_CHECK(failStatus, OP_LOGE(FUSED_OP_TYPE.c_str(), "ProcessRnnCell:check failed, fusion failed."), return PARAM_INVALID);

    if (rnnCellNode.empty())
    {
        OP_LOGE(FUSED_OP_TYPE.c_str(), "ProcessRnnCell:rnnCellNode is empty, fusion failed.");
    ...
    if (has_static)
    {
        ...
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(wshIndex)->GetPeerOutAnchor(),
    ...
    for (int64_t i = 0; i < numSplitX; i++)
    {
        ...
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(splitXNode->GetOutDataAnchor(i),
                 ...
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", splitXNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 0),
                 return FAILED);
        OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", splitXNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 0);
        index = index + 1;
        ...
        if (expose_hidden || (!expose_hidden && i > 0))
        {
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(splitContNode->GetOutDataAnchor(i),
                                                      rnnCellNode[i]->GetInDataAnchor(index)),
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", splitContNode->GetName().c_str(), i, rnnCellNode[i]->GetName().c_str(), 0),
                    return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", splitContNode->GetName().c_str(), i, rnnCellNode[i]->GetName().c_str(), 0);
            index = index + 1;
        }
        ...
        if (has_static)
        {
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(innerproductNode->GetOutDataAnchor(0),
                                                        rnnCellNode[i]->GetInDataAnchor(index)),
                     OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", innerproductNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 0),
                     return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", innerproductNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 0);
        ...
        if (i == 0 && expose_hidden)
        {
            ...
            int hIndex = has_static ? 3 : 2;
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(hIndex)->GetPeerOutAnchor(),
                                                        rnnCellNode[i]->GetInDataAnchor(index)),
                     OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 0),
                     return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 0);
            index = index + 1;
        }
        else if (i > 0)
        {
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(rnnCellNode[i - 1]->GetOutDataAnchor(1),
                                                        rnnCellNode[i]->GetInDataAnchor(index)),
                     OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", rnnCellNode[i - 1]->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 0),
                     return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", rnnCellNode[i - 1]->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 0);
            index = index + 1;
        }

        // edge for w_xh
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(wxhIndex)->GetPeerOutAnchor(),
                                                    rnnCellNode[i]->GetInDataAnchor(index)),
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 1),
                 return FAILED);
        OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), wxhIndex, rnnCellNode[i]->GetName().c_str(), index);
        index = index + 1;

        // edge for bias_h
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(biasHIndex)->GetPeerOutAnchor(),
                                                    rnnCellNode[i]->GetInDataAnchor(index)),
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", rnnCellNode[i]->GetName().c_str(), biasHIndex, fusedNode->GetName().c_str(), index),
                 return FAILED);
        OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), biasHIndex, fusedNode->GetName().c_str(), index);
        index = index + 1;
        ...
        if (expose_hidden || i > 0) {
            ...
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(whhIndex)->GetPeerOutAnchor(),
                                                        rnnCellNode[i]->GetInDataAnchor(index)),
                     OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), whhIndex, rnnCellNode[i]->GetName().c_str(), index),
                     return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 1);
            index = index + 1;
        }

        // edge for w_ho
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(whoIndex)->GetPeerOutAnchor(),
                                                    rnnCellNode[i]->GetInDataAnchor(index)),
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", fusedNode->GetName().c_str(), 0, rnnCellNode[i]->GetName().c_str(), 1),
                 return FAILED);
        OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), whoIndex, rnnCellNode[i]->GetName().c_str(), index);
        index = index + 1;

        // edge for bias_o
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(biasOIndex)->GetPeerOutAnchor(),
                                                    rnnCellNode[i]->GetInDataAnchor(index)),
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", rnnCellNode[i]->GetName().c_str(), biasOIndex, fusedNode->GetName().c_str(), index),
                 return FAILED);
        OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d].", fusedNode->GetName().c_str(), biasOIndex, fusedNode->GetName().c_str(), index);

        // edge for concat
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(rnnCellNode[i]->GetOutDataAnchor(0),
                                                    concatNode->GetInDataAnchor(i)),
                 OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's input[%d] to fusion node:%s's input[%d] failed.", rnnCellNode[i]->GetName().c_str(), 0, concatNode->GetName().c_str(), 0),
                 return FAILED);
    ...
    if (fusedNode->GetOutDataAnchor(1)->GetPeerInDataAnchors().size() > 0)
    {
        ...
        {
            inAnchorPtr->UnlinkAll();
            FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(rnnCellNode[numSplitX - 1]->GetOutDataAnchor(1), inAnchorPtr),
                     OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0] failed.", rnnCellNode[numSplitX - 1]->GetName().c_str(), fusedNode->GetName().c_str()),
                     return FAILED);
            OP_LOGD(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's output[0] to fusion node:%s's output[0].", rnnCellNode[numSplitX - 1]->GetName().c_str(), fusedNode->GetName().c_str());
        }
    }
