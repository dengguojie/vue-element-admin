...
Status FusedBatchnormFusionPass::Run(ge::ComputeGraph &graph) {
  ...
  OP_LOGD(FUSED_OP_TYPE.c_str(), "GraphFusionPass[FusedBatchnormFusionPass]: pattern=undefined, matchedTimes=%zu.",
            passMatchResultVec.size());
  // Record anchor map
...
Status FusedBatchnormFusionPass::MatchPass(
    ge::ComputeGraph &graph, vector<PassMatchResult> &passMatchResultVec) {
...
Status FusedBatchnormFusionPass::GetAllBatchNormNodes(
    ge::ComputeGraph &graph, vector<NodePtr> &batchNormNodeVec) {
...
Status
FusedBatchnormFusionPass::MatchBatchNormNode(NodePtr bnNodePtr,
                                             PassMatchResult &matchResult) {
  ...
  for (InDataAnchorPtr inAnchorPtr : bnNodePtr->GetAllInDataAnchors()) {
    if (inAnchorPtr != nullptr && inAnchorPtr->GetPeerOutAnchor() != nullptr &&
      ...
      matchResult.dataNodeOutAnchorVec.push_back(
          inAnchorPtr->GetPeerOutAnchor());
      matchResult.removeEdgeVec.push_back(
          {inAnchorPtr, inAnchorPtr->GetPeerOutAnchor()});
      inputDataCount++;
    }
  }
  if (inputDataCount < 3) {
  ...
  for (size_t i = 0; i < bnNodePtr->GetAllOutDataAnchors().size(); i++) {
    ...
    if (outAnchorPtr != nullptr &&
        outAnchorPtr->GetPeerInDataAnchors().size() > 0) {
      InDataAnchorPtr inDataAnchorPtr =
          outAnchorPtr->GetPeerInDataAnchors().at(0);
      if (inDataAnchorPtr != nullptr &&
          inDataAnchorPtr->GetOwnerNode() != nullptr) {
        if (i == 0) {
          matchResult.outNodePtr = inDataAnchorPtr->GetOwnerNode();
          OP_LOGI(FUSED_OP_TYPE.c_str(), "outNodePtr Node name is [%s]",
                  matchResult.outNodePtr->GetName().c_str());
        } else if (i == 1 || i == 2) {
          if (inDataAnchorPtr->GetOwnerNode()->GetType() == PASS_OP_TYPE_SUB) {
            subNodes.push_back(inDataAnchorPtr->GetOwnerNode());
            matchResult.removeEdgeVec.push_back(
                {inDataAnchorPtr, outAnchorPtr});
          }
        } else {
  ...
  if (outputDataCount < 3 || subNodes.size() != 2) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "The BatchNorm node must have at least 3 output and two of them "
            "should be sub node.");
    return FAILED;
  }
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "The fused batch norm pass has been matched, the name of batchnorm "
          "node is [%s]",
          bnNodePtr->GetName().c_str());
  return SUCCESS;
}

Status FusedBatchnormFusionPass::MatchSubNode(NodePtr subNodePtr,
                                              PassMatchResult &matchResult) {
  ...
  if (castInPtr != nullptr) {
    ...
    NodePtr assignSubNodePtr =
        FindOutputNode(mulNodePtr, "AssignSub", matchResult, true);
    if (assignSubNodePtr == nullptr) {
      OP_LOGI(FUSED_OP_TYPE.c_str(), "mul Node does not have a AssignSub output node.");
      return FAILED;
    }
    ...
    // find switch node
    NodePtr switchNodePtr =
            FindInputNode(subNodePtr, "Switch", matchResult, true);
...
Status
FusedBatchnormFusionPass::FusionGraphWithPass(ge::ComputeGraph &graph,
                                              PassMatchResult &matchResult) {
  string bnStreamLabel = "";
  if (!ge::AttrUtils::GetStr(matchResult.batchNormPtr->GetOpDesc(),
                             STREAM_LABEL, bnStreamLabel)) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "BatchNorm not have _stream_label attr.");
  }

  for (NodePtr subNodePtr : matchResult.subNodeVec) {
    string streamLabel;
    if (ge::AttrUtils::GetStr(subNodePtr->GetOpDesc(), STREAM_LABEL,
                              streamLabel)) {
      if ((bnStreamLabel.empty() && !streamLabel.empty()) ||
          bnStreamLabel != streamLabel) {
        OP_LOGI(FUSED_OP_TYPE.c_str(),
            "_stream_label(attr) not equal in all nodes and can not fusion.");
        return NOT_CHANGED;
      }
  ...
  for (NodePtr mulNodePtr : matchResult.mulNodeVec) {
    string streamLabel;
    if (ge::AttrUtils::GetStr(mulNodePtr->GetOpDesc(), STREAM_LABEL,
                              streamLabel)) {
      if ((bnStreamLabel.empty() && !streamLabel.empty()) ||
          bnStreamLabel != streamLabel) {
        OP_LOGI(FUSED_OP_TYPE.c_str(),
  ...
  for (NodePtr assignSubNodePtr : matchResult.assignSubNodeVec) {
    string streamLabel;
    if (ge::AttrUtils::GetStr(assignSubNodePtr->GetOpDesc(), STREAM_LABEL,
                              streamLabel)) {
      if ((bnStreamLabel.empty() && !streamLabel.empty()) ||
          bnStreamLabel != streamLabel) {
        OP_LOGI(FUSED_OP_TYPE.c_str(),
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Begin to fusion pass, the name of batch norm is [%s].",
          bnNodePtr->GetName().c_str());
  std::string fusedBatchNormName = bnNodePtr->GetOpDesc()->GetName();
  // BNTrainingReduce
  OpDescPtr bnReduceOp;
  FUSION_PASS_MAKE_SHARED(bnReduceOp = std::make_shared<ge::OpDesc>(
                     fusedBatchNormName + "_Reduce", PASS_OP_TYPE_BNREDUCE),
                 return INTERNAL_ERROR);
  bnReduceOp->AddInputDesc(
      "x", matchResult.batchNormPtr->GetOpDesc()->GetInputDesc(0));
  bnReduceOp->AddOutputDesc(
      "sum", matchResult.batchNormPtr->GetOpDesc()->GetInputDesc(1));
  bnReduceOp->AddOutputDesc(
      "square_sum", matchResult.batchNormPtr->GetOpDesc()->GetInputDesc(2));

  ge::NodePtr bnReduceNode = graph.AddNode(bnReduceOp);
  FUSION_PASS_CHECK(bnReduceNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "bnReduceNode is null, fusion failed."),
           return PARAM_INVALID);
  // add edge for bnreduce node
  GraphUtils::AddEdge(matchResult.dataNodeOutAnchorVec[0],
                      bnReduceNode->GetInDataAnchor(0));

  // BNTrainingUpdate
  OpDescPtr bnUpdateOp;
  FUSION_PASS_MAKE_SHARED(bnUpdateOp = std::make_shared<ge::OpDesc>(
                     fusedBatchNormName + "_Update", PASS_OP_TYPE_BNUPDATE),
                 return INTERNAL_ERROR);
  bnUpdateOp->AddInputDesc(
      "x", matchResult.batchNormPtr->GetOpDesc()->GetInputDesc(0));
  bnUpdateOp->AddInputDesc(
      "sum", matchResult.batchNormPtr->GetOpDesc()->GetInputDesc(1));
  bnUpdateOp->AddInputDesc(
      "square_sum", matchResult.batchNormPtr->GetOpDesc()->GetInputDesc(2));
  bnUpdateOp->AddInputDesc(
      "scale", matchResult.batchNormPtr->GetOpDesc()->GetInputDesc(1));
  bnUpdateOp->AddInputDesc(
      "offset", matchResult.batchNormPtr->GetOpDesc()->GetInputDesc(2));
  bnUpdateOp->AddInputDesc(
      "mean", matchResult.variableNodeVec[0]->GetOpDesc()->GetOutputDesc(0));
  bnUpdateOp->AddInputDesc(
      "variance",
      matchResult.variableNodeVec[1]->GetOpDesc()->GetOutputDesc(0));

  bnUpdateOp->AddOutputDesc("y", bnNodePtr->GetOpDesc()->GetOutputDesc(0));
  bnUpdateOp->AddOutputDesc(
      "mean", matchResult.variableNodeVec[0]->GetOpDesc()->GetOutputDesc(0));
  bnUpdateOp->AddOutputDesc(
      "variance",
      matchResult.variableNodeVec[1]->GetOpDesc()->GetOutputDesc(0));
  bnUpdateOp->AddOutputDesc("batch_mean",
                            bnNodePtr->GetOpDesc()->GetOutputDesc(1));
  bnUpdateOp->AddOutputDesc("batch_variance",
                            bnNodePtr->GetOpDesc()->GetOutputDesc(2));

  ge::NodePtr bnUpdateNode = graph.AddNode(bnUpdateOp);

  FUSION_PASS_CHECK(bnUpdateNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "bnUpdateNode is null, fusion failed."),
           return PARAM_INVALID);

  // add edge for input of bnupdate
  GraphUtils::AddEdge(matchResult.dataNodeOutAnchorVec[0],
                      bnUpdateNode->GetInDataAnchor(0));
  GraphUtils::AddEdge(bnReduceNode->GetOutDataAnchor(0),
                      bnUpdateNode->GetInDataAnchor(1));
  GraphUtils::AddEdge(bnReduceNode->GetOutDataAnchor(1),
                      bnUpdateNode->GetInDataAnchor(2));
  GraphUtils::AddEdge(matchResult.dataNodeOutAnchorVec[1],
                      bnUpdateNode->GetInDataAnchor(3));
  GraphUtils::AddEdge(matchResult.dataNodeOutAnchorVec[2],
                      bnUpdateNode->GetInDataAnchor(4));
  if (matchResult.fwkNodeVec.size() == 2) {
    ...
    GraphUtils::AddEdge(matchResult.variableNodeVec[0]->GetOutDataAnchor(0),
                        bnUpdateNode->GetInDataAnchor(5));
    GraphUtils::AddEdge(matchResult.variableNodeVec[1]->GetOutDataAnchor(0),
                        bnUpdateNode->GetInDataAnchor(6));
  }
  ...
  for (InDataAnchorPtr inAnchorPtr :
       matchResult.batchNormPtr->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
  ...
  if (matchResult.assignSubNodeVec[0]->GetAllOutDataAnchors().size() > 0) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "The size of inAnFromAssignSubZero is [%d].",
            matchResult.assignSubNodeVec[0]
                ->GetOutDataAnchor(0)
                ->GetPeerInDataAnchors()
                .size());
    OutDataAnchorPtr assignSubOutAnchorPtr =
        matchResult.assignSubNodeVec[0]->GetOutDataAnchor(0);
    if (assignSubOutAnchorPtr != nullptr &&
        assignSubOutAnchorPtr->GetPeerInDataAnchors().size() > 0) {
      for (InDataAnchorPtr inAnchorPtr :
           assignSubOutAnchorPtr->GetPeerInDataAnchors()) {
        inAnchorPtr->UnlinkAll();
  ...
  if (matchResult.assignSubNodeVec[1]->GetAllOutDataAnchors().size() > 0) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "The size of inAnFromAssignSubOne is [%d].",
            matchResult.assignSubNodeVec[1]
                ->GetOutDataAnchor(0)
                ->GetPeerInDataAnchors()
                .size());
    OutDataAnchorPtr assignSubOutAnchorPtr =
        matchResult.assignSubNodeVec[1]->GetOutDataAnchor(0);
    if (assignSubOutAnchorPtr != nullptr &&
        assignSubOutAnchorPtr->GetPeerInDataAnchors().size() > 0) {
      for (InDataAnchorPtr inAnchorPtr :
           assignSubOutAnchorPtr->GetPeerInDataAnchors()) {
  ...
  for (NodePtr assignSubNodePtr : matchResult.assignSubNodeVec) {
    if (assignSubNodePtr->GetOutControlAnchor() != nullptr) {
      OutControlAnchorPtr assignSubOutCtrlAnchorPtr =
          assignSubNodePtr->GetOutControlAnchor();
      if (assignSubOutCtrlAnchorPtr->GetPeerInControlAnchors().size() > 0) {
        for (InControlAnchorPtr inCtrlAnchorPtr :
             assignSubOutCtrlAnchorPtr->GetPeerInControlAnchors()) {
  ...
  if (!matchResult.bnOtherOutAnchorVec.empty()) {
    for (InDataAnchorPtr otherInAnchor :
         matchResult.bnOtherOutAnchorVec[0]->GetPeerInDataAnchors()) {
    ...
    if (matchResult.bnOtherOutAnchorVec.size() == 2) {  // tf1.12 bn has 5 output
      for (InDataAnchorPtr otherInAnchor :
           matchResult.bnOtherOutAnchorVec[1]->GetPeerInDataAnchors()) {
        otherInAnchor->UnlinkAll();
      ...
      for (InDataAnchorPtr otherInAnchor :
        matchResult.bnOtherOutAnchorVec[2]->GetPeerInDataAnchors()) {
        ...
        ge::OpDescUtils::ClearInputDesc(bnOtherNodePtr->GetOpDesc(),
                                        (uint32_t)otherIdx);
      }
  ...
  if (ge::AttrUtils::GetFloat(bnNodePtr->GetOpDesc(), "epsilon", epsilonVal)) {
    if (!ge::AttrUtils::SetFloat(bnUpdateNode->GetOpDesc(), "epsilon",
                                 epsilonVal)) {
  ...
  if (!bnStreamLabel.empty()) {
    if (!ge::AttrUtils::SetStr(bnUpdateNode->GetOpDesc(), STREAM_LABEL,
                               bnStreamLabel)) {
      OP_LOGE(FUSED_OP_TYPE.c_str(), "bnUpdateNode set _stream_label error, fusion failed.");
      return FAILED;
    }
    if (!ge::AttrUtils::SetStr(bnReduceNode->GetOpDesc(), STREAM_LABEL,
                               bnStreamLabel)) {
  ...
  for (PassRemoveEdge passRemoveEdge : matchResult.removeEdgeVec) {
    GraphUtils::RemoveEdge(passRemoveEdge.inAnchorPtr,
                           passRemoveEdge.outAnchorPtr);
  }
  ...
  for (NodePtr constNodePtr : matchResult.constNodeVec) {
    if (PatternFusionUtil::GetOutEdgeSize(constNodePtr) == 0) {
      FUSION_PASS_CHECK(
          ge::GRAPH_SUCCESS != graph.RemoveNode(constNodePtr),
      ...
      OP_LOGD(FUSED_OP_TYPE.c_str(), "Node:[%s] have output link to other node.",
              constNodePtr->GetName().c_str());
    }
  }
  ...
  FUSION_PASS_CHECK(matchResult.batchNormPtr == nullptr,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "BatchNormPtr is null"), return FAILED);
  ...
  if (matchResult.assignSubNodeVec.size() == 2) {
    SetOutputTensorDescAttr(0, 1,
            matchResult.assignSubNodeVec[0], bnUpdateNode);
    SetOutputTensorDescAttr(0, 2,
            matchResult.assignSubNodeVec[1], bnUpdateNode);
  }
  ...
  for (GeTensorDesc &tensorDesc : bnReduceOp->GetAllInputsDesc()) {
    reduceOSS << endl
              << "Format:" << tensorDesc.GetFormat() << " OriFormat"
              << tensorDesc.GetOriginFormat();
    reduceOSS << " Dim:{";
  ...
  for (GeTensorDesc &tensorDesc : bnUpdateOp->GetAllInputsDesc()) {
    updateOSS << "Format:" << tensorDesc.GetFormat() << " OriFormat"
              << tensorDesc.GetOriginFormat();
    updateOSS << " Dim:{";
    for (int64_t dim : tensorDesc.GetShape().GetDims()) {
      updateOSS << dim << " ";
...
NodePtr FusedBatchnormFusionPass::FindInputNode(NodePtr nodePtr, string opType,
                                                PassMatchResult &matchResult,
                                                bool isRemoveEdge) {
  ...
  for (InDataAnchorPtr inDataAnchorPtr : nodePtr->GetAllInDataAnchors()) {
    ...
    if (ownerNode->GetType() == opType) {
      if (isRemoveEdge) {
        matchResult.removeEdgeVec.push_back(
            {inDataAnchorPtr, outDataAnchorPtr});
      }
      return ownerNode;
    }
  }
...
NodePtr FusedBatchnormFusionPass::FindInputNodeByIndex(
    NodePtr nodePtr, unsigned int index, PassMatchResult &matchResult,
    bool isRemoveEdge) {
  if (nodePtr->GetAllInDataAnchors().empty()) {
    return nullptr;
  }
...
NodePtr FusedBatchnormFusionPass::FindOutputNode(NodePtr nodePtr, string opType,
                                                 PassMatchResult &matchResult,
                                                 bool isRemoveEdge) {
  ...
  for (OutDataAnchorPtr outDataAnchor : nodePtr->GetAllOutDataAnchors()) {
    ...
    for (InDataAnchorPtr inDataAnchorPtr :
         outDataAnchor->GetPeerInDataAnchors()) {
      if (inDataAnchorPtr == nullptr) {
...
NodePtr FusedBatchnormFusionPass::FindOutputNodeByName(NodePtr nodePtr,
                                                       string opName,
                                                       PassMatchResult &matchResult,
                                                       bool isRemoveEdge) {
  if (nodePtr->GetAllOutDataAnchors().empty()) {
...
Status FusedBatchnormFusionPass::Fusion(ge::ComputeGraph &graph,
                                        Mapping &mapping,
...
Status FusedBatchnormFusionPass::SetOutputTensorDescAttr(
                                 uint16_t originOutputIndex,
                                 uint16_t fuseOutputIndex,
                                 ge::NodePtr originNode,
                                 ge::NodePtr fuseNode) {
  FUSION_PASS_CHECK(fuseNode == nullptr,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "fuseNode is null"), return FAILED);
  FUSION_PASS_CHECK(originNode == nullptr,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "originNode is null"), return FAILED);
  FUSION_PASS_CHECK(fuseNode->GetOpDesc() == nullptr,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "fuseNode OpDesc is null"), return FAILED);
  ...
  FUSION_PASS_CHECK(originNode->GetOpDesc() == nullptr,
           OP_LOGW(FUSED_OP_TYPE.c_str(), "originNode OpDesc is null"), return FAILED);
  ge::AttrUtils::SetStr(
          fuseNode->GetOpDesc()->MutableOutputDesc(fuseOutputIndex),
          ge::ATTR_NAME_DATA_DUMP_ORIGIN_NAME, originNode->GetName());
  ge::AttrUtils::SetInt(
          fuseNode->GetOpDesc()->MutableOutputDesc(fuseOutputIndex),
          ge::ATTR_NAME_DATA_DUMP_ORIGIN_OUTPUT_INDEX, originOutputIndex);
  GraphPassUtil::SetDataDumpOriginDataType(
          originNode->GetOpDesc()
                  ->GetOutputDesc(originOutputIndex)
                  .GetOriginDataType(),
          fuseNode->GetOpDesc()->MutableOutputDesc(fuseOutputIndex));
...
REGISTER_PASS("FusedBatchnormFusionPass", BUILT_IN_GRAPH_PASS,
