...
static const std::string PATTERN_FUSEDNODE = "PassThroughFusion";
...
Status passThroughAssistHelpFP16(const int32_t n, Dtype& output1,
    const vector<int64_t> passThroughDInputDimInfo){
  ...
  for (int32_t windowIdx = 0; windowIdx < windowSize; windowIdx++) {
    for (int32_t channelIdx = 0; channelIdx < channelSize; channelIdx++) {
      ...
      OP_LOGI("passThroughhAssitHelpFP16",
             "=== Idx %d ===",
             fillAssistIdx);
    }
  }
...
vector<FusionPattern*> PassThroughFusionPass::DefinePatterns() {
  ...
  FUSION_PASS_CHECK(pattern == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "new a pattern object failed."),
           return patterns);
  pattern->AddOpDesc(PATTERN_FUSEDNODE, {PASS_THROUGH_NODE})
      .SetOutput(PATTERN_FUSEDNODE);
  patterns.push_back(pattern);
  return patterns;
}

Status PassThroughFusionPass::Fusion(ge::ComputeGraph &graph,
                                      Mapping &mapping,
  ...
  FUSION_PASS_CHECK(passThroughNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(),
                   "passThroughNode is null, fusion failed."),
           return PARAM_INVALID);
  ge::OpDescPtr passThroughDesc = passThroughNode->GetOpDesc();
  FUSION_PASS_CHECK(passThroughDesc == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(),
                   "passThroughDesc is null, fusion failed."),
           return PARAM_INVALID);
  std::string passThroughName = passThroughNode->GetName();

  bool reverse = true;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetBool(passThroughDesc, "reverse", reverse),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]: Can not get passthrough reverse attr.", passThroughName.c_str()),
           return FAILED);
  FUSION_PASS_CHECK(reverse, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]: passthrough reverse is true, do not fusion.",
          passThroughName.c_str()), return NOT_CHANGED);
  // get attr stride of fused node
  int32_t stride = -1;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetInt(passThroughDesc, "stride", stride),
           OP_LOGI(FUSED_OP_TYPE.c_str(),
                   "Node[%s]: Cannot get attr:stride from node.", passThroughName.c_str()),
           return NOT_CHANGED);

  OP_LOGI(FUSED_OP_TYPE.c_str(),
          "Node[%s]: The OriginDataType of node's first input is %d,",
          passThroughName.c_str(), passThroughDesc->GetInputDesc(0).GetDataType());
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]: Get father node[Name:%s Type:%s]",
          passThroughName.c_str(),
          passThroughInNode->GetName().c_str(),
          passThroughInNode->GetType().c_str());
  ...
  FUSION_PASS_CHECK(SUCCESS != PatternFusionUtil::ParseChannelIdx(passThroughInput, inChannelIdx),
           OP_LOGE(FUSED_OP_TYPE.c_str(),
                   "Node[%s]: The original format of node's input0 is %s, which is unsupportable.",
                   passThroughName.c_str(),
                   ge::TypeUtils::FormatToSerialString(assitMatrixFormat).c_str()),
           return FAILED);
  OP_LOGI(FUSED_OP_TYPE.c_str(),
          "Node[%s]: The original format of node's input0 is %s.",
          passThroughName.c_str(),
          ge::TypeUtils::FormatToSerialString(assitMatrixFormat).c_str());
  size_t inNumberIdx = -1;
  FUSION_PASS_CHECK(SUCCESS != ParseNumberIdx(passThroughInput, inNumberIdx),
           OP_LOGE(FUSED_OP_TYPE.c_str(),
                   "Node[%s]: The original format of node's input0 is %s, which is unsupportable.",
                   passThroughName.c_str(),
                   ge::TypeUtils::FormatToSerialString(assitMatrixFormat).c_str()),
  ...
  FUSION_PASS_CHECK(passThroughInputDimInfo[inChannelIdx] > CIN_THESHOLD,
           OP_LOGI(FUSED_OP_TYPE.c_str(),
                   "Node[%s]: Cin[%d] > 64, keep PassThrough for performance, not fusion.",
                   passThroughName.c_str(),
                   passThroughInputDimInfo[inChannelIdx]), return NOT_CHANGED);

  int64_t passThroughDChannel = passThroughInputDimInfo[inChannelIdx] * stride * stride;
  vector<int64_t> passThroughDInputDimInfo = {passThroughDChannel,
                                              passThroughInputDimInfo[inChannelIdx],
                                              stride, stride};
  for (auto printInfo : passThroughDInputDimInfo) {
    OP_LOGI(FUSED_OP_TYPE.c_str(),
            "Node[%s]: info of node's input1 is %d.", passThroughName.c_str(), printInfo);
  }
  ...
  FUSION_PASS_CHECK(inputAssit.get() == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]: inputAssit is NULL", passThroughName.c_str())
          , return PARAM_INVALID);
  ...
  ret = passThroughAssistHelpFP16(destSize, *inputAssit.get(),
                                  passThroughDInputDimInfo);
  FUSION_PASS_CHECK(ret != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Node[%s]: Generate assist matrix failed.", passThroughName.c_str()),
          return ret);
  ...
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Node[%s]: PassThrough has %d input.", passThroughName.c_str(), passThroughNode->GetInNodes().size());
