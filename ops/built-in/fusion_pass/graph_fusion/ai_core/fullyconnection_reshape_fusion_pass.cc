 ...
 */
  ...
  Status FullyConnectionReshapePass::Fusion(ge::ComputeGraph &graph,
                                                 Mapping &mapping,
                                                 vector<ge::NodePtr> &fusionNodes)
  {
    ...
    FUSION_PASS_CHECK(reshapeNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshapeNode is null"), return PARAM_INVALID);
    FUSION_PASS_CHECK(fullyConnectionNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fullyConnectionNode is null"), return PARAM_INVALID);

    int64_t axis = 0;
    FUSION_PASS_CHECK(!ge::AttrUtils::GetInt(fullyConnectionNode->GetOpDesc(), "axis", axis),
             OP_LOGI(FUSED_OP_TYPE.c_str(), "Get node[%s]'s axis attr not success.",
                     fullyConnectionNode->GetName().c_str()), return false);
    if (axis != 1) {
      OP_LOGI(FUSED_OP_TYPE.c_str(), "node[FullyConnection]'s axis is not 1, not support fusion, FullyConnectionReshapePass fusion end");
      return SUCCESS;
    }
    ge::OpDescPtr reshapeDesc = reshapeNode->GetOpDesc();
    FUSION_PASS_CHECK(reshapeDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "reshapeNode's OpDesc is null."), return PARAM_INVALID);

    ge::OpDescPtr fullyConnectionDesc = fullyConnectionNode->GetOpDesc();
    FUSION_PASS_CHECK(fullyConnectionDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "fullyConnectionNode's OpDesc is null."), return PARAM_INVALID);
    ...
    if ((reshapeInputDim0Value != reshapeOutputDim0Value) && (reshapeInputDim0Value != 0)){
      OP_LOGI(FUSED_OP_TYPE.c_str(), "reshapeInputDim0Value is not same with reshapeOutputDim0Value fusion, FullyConnectionReshapePass fusion end");
      return SUCCESS;
    }
    ...
    FUSION_PASS_CHECK(!ge::AttrUtils::GetBool(fullyConnectionNode->GetOpDesc(), "transpose", transposeAttr),
             OP_LOGI(FUSED_OP_TYPE.c_str(), "Get node[%s]'s transpose attr not success.",
                     fullyConnectionNode->GetName().c_str()), return PARAM_INVALID);
    ...
    if (wShape.size() < 2) {
      OP_LOGI(FUSED_OP_TYPE.c_str(), "node[FullyConnection]'s weigth sizeis less 2, FullyConnectionReshapePass fusion end");
    ...
    FUSION_PASS_CHECK(graph.RemoveNode(reshapeNode) != ge::GRAPH_SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "remove node failed"), return FAILED);
