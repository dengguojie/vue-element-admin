...
Status MakeHelperTensor(const vector<int64_t>& offsets_shape,
                        const vector<int64_t>& kernel_sizes, const vector<int64_t>& strides,
                        const vector<int64_t>& pads, const vector<int64_t>& dilations, float& grid_base) {
      ...
      for(int64_t g = 0; g < group; g++) {
        for(int64_t k_h = 0; k_h < K_H; k_h++) {
          for(int64_t k_w = 0; k_w < K_W; k_w++) {
            w_index = h * W_OUT * 3 * group * K_H * K_W + w * 3 * group * K_H * K_W
                      + 0 * group * K_H * K_W + g * K_H * K_W + k_h * K_W + k_w;
            h_index = h * W_OUT * 3 * group * K_H * K_W + w * 3 * group * K_H * K_W
...
Status DeformableOffsetsGradFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_CHECK(deformableOffsetsGradNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(),
                    "Get DeformableOffsetsGrad Node Failed, fusion failed."), return PARAM_INVALID);
  // get desc
  ...
  FUSION_PASS_CHECK(!AttrUtils::GetInt(deformableOffsetsGradDesc, "deformable_groups", dfm_groups),
                    OP_LOGI(FUSED_OP_TYPE.c_str(), "get deformable_groups attr failed."),
                    return PARAM_INVALID);
  vector<int64_t> ksize;
  FUSION_PASS_CHECK(!AttrUtils::GetListInt(deformableOffsetsGradDesc, "ksize", ksize),
                    OP_LOGI(FUSED_OP_TYPE.c_str(), "get ksize attr failed."),
                    return PARAM_INVALID);
  vector<int64_t> strides;
  FUSION_PASS_CHECK(!AttrUtils::GetListInt(deformableOffsetsGradDesc, "strides", strides),
                    OP_LOGI(FUSED_OP_TYPE.c_str(), "get strides attr failed."),
                    return PARAM_INVALID);
  vector<int64_t> pads;
  FUSION_PASS_CHECK(!AttrUtils::GetListInt(deformableOffsetsGradDesc, "pads", pads),
                    OP_LOGI(FUSED_OP_TYPE.c_str(), "get pads attr failed."),
                    return PARAM_INVALID);
  vector<int64_t> dilations;
  FUSION_PASS_CHECK(!AttrUtils::GetListInt(deformableOffsetsGradDesc, "dilations", dilations),
                    OP_LOGI(FUSED_OP_TYPE.c_str(), "get dilations attr failed."),
                    return PARAM_INVALID);

  Format helperMatrixFormat = x_desc.GetFormat();
  FUSION_PASS_CHECK(helperMatrixFormat != FORMAT_NHWC, OP_LOGI(FUSED_OP_TYPE.c_str(),
                    "The format of input is %d, not NHWC, cannot do fusion.",
                    helperMatrixFormat), return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(const_node == nullptr,
                    OP_LOGI(FUSED_OP_TYPE.c_str(), "Fail to add const node."),
                    return NOT_CHANGED);
