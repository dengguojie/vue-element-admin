...
Status BatchNormGradBnInferGradFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping,
  ...
  FUSION_PASS_MAKE_SHARED((newOpdesc = std::make_shared<ge::OpDesc>(batchNormGradNode->GetName(), BNINFERGRAD)),
                          return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(
          FUSED_OP_TYPE.c_str(), "Op[%s]: add the input desc for the input batch_variance failed.", newOpName.c_str()),
      return FAILED);

  // add output
  ge::GeTensorDesc tensor1 = batchNormGradNode->GetOpDesc()->GetOutputDesc(0);
  FUSION_PASS_CHECK(
      newOpdesc->AddOutputDesc(tensor1) != SUCCESS,
      VECTOR_FUSION_INNER_ERR_REPORT(
          FUSED_OP_TYPE.c_str(), "Op[%s]: add the output desc for the output x_backprop failed.", newOpName.c_str()),
      return FAILED);

  ge::NodePtr newNode = graph.AddNode(newOpdesc);
  FUSION_PASS_CHECK(newNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "newNode is null, fusion failed."),
                    return PARAM_INVALID);

  // copy attr
  float epsilon;
  FUSION_PASS_CHECK(!ge::AttrUtils::GetFloat(batchNormGradNode->GetOpDesc(), EPSILON, epsilon),
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "Get epsilon attr failed."), return FAILED);
  ...
  FUSION_PASS_CHECK(peer_out_data_anchor == nullptr,
                    ...
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(batchNormGradNode->GetInDataAnchor(0)->GetPeerOutAnchor(),
                    ...
                    return FAILED);
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(batchNormGradNode->GetInDataAnchor(2)->GetPeerOutAnchor(),
                    ...
                    VECTOR_FUSION_INNER_ERR_REPORT(
                        ...
                        newNode->GetName().c_str()),
                    return FAILED);
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(batchNormGradNode->GetInDataAnchor(4)->GetPeerOutAnchor(),
