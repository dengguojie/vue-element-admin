...
vector<FusionPattern *> BatchNormGradBnInferGradFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("BatchNormGradBnInferGradFusionPass");
...
Status BatchNormGradBnInferGradFusionPass::Fusion(
    ge::ComputeGraph &graph, Mapping &mapping, vector<ge::NodePtr> &fusionNodes)
{
  ...
  FUSION_PASS_CHECK(
      batchNormGradNode->GetOutDataAnchor(1)->GetPeerInDataAnchors().size() !=
              0 ||
          batchNormGradNode->GetOutDataAnchor(2)
  ...
  newOpdesc =
      std::make_shared<ge::OpDesc>(batchNormGradNode->GetName(), BNINFERGRAD);
  ...
  FUSION_PASS_CHECK(newOpdesc->AddOutputDesc(tensor1) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[%s]: add the output desc for the output x_backprop failed.", newOpName.c_str()), return FAILED);
...
REGISTER_PASS("BatchNormGradBnInferGradFusion", BUILT_IN_GRAPH_PASS,
              BatchNormGradBnInferGradFusionPass);
}  // namespace fe
