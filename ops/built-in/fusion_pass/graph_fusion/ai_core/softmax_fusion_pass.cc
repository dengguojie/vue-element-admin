...
Status SoftmaxFusionPass::Fusion(ge::ComputeGraph &graph,
                                 Mapping &mapping,
    ...
    FUSION_PASS_CHECK(softmaxOpDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "softmax is null"),
             return PARAM_INVALID);
    ...
    FUSION_PASS_CHECK(axes.empty(), OP_LOGE(FUSED_OP_TYPE.c_str(), "axes is null, please check!"),
             return FAILED);
    ...
    if (axes[0] == 2 && softmaxOriginFormat == ge::FORMAT_NCHW && isUsePattern) {
        ...
        FUSION_PASS_CHECK(!ge::AttrUtils::SetListInt(softmaxNode->GetOpDesc(), "axes", axes),
                 OP_LOGI(FUSED_OP_TYPE.c_str(), "Set axes attr failed."),
                 return FAILED);
        ...
        transposeDDesc =
            std::make_shared<ge::OpDesc>(softmaxNode->GetName() + "_transposeD_layer", "TransposeD");
        FUSION_PASS_CHECK(transposeDDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "transposeDDesc is null, TransposeD failed."),
                 return PARAM_INVALID);
        ...
        transposeDDesc1 =
            std::make_shared<ge::OpDesc>(softmaxNode->GetName() + "_transposeD1_layer", "TransposeD");
        FUSION_PASS_CHECK(transposeDDesc1 == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "transposeD is null, TransposeD failed."),
                 return PARAM_INVALID);
