...
Status LogSoftmaxFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(logsoftmaxNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "logsoftmax node is null."),
                    return PARAM_INVALID);           
  ge::OpDescPtr logsoftmaxOpDesc = logsoftmaxNode->GetOpDesc();
  FUSION_PASS_CHECK(logsoftmaxOpDesc == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "logsoftmax is null."),
                    return PARAM_INVALID);
