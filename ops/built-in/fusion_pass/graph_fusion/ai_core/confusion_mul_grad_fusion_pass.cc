...
Status MulGradFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(sum_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "sum node is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(mul1_node == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "mul1 node is null, fusion failed."),
                    return PARAM_INVALID);
  FUSION_PASS_CHECK(
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(sum_node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove sum node failed."),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(mul1_node) != SUCCESS, OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove mul1 node failed."),
                    return FAILED);
