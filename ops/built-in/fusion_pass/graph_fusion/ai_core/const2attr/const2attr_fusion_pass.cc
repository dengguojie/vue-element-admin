...
Status Const2AttrFusionPass::Fusion(ge::ComputeGraph& graph,
                                    Mapping& mapping,
                                    vector<ge::NodePtr> &newNodes) {
  return SUCCESS;
}
Status Const2AttrFusionPass::Run(ge::ComputeGraph& graph,
                                 OpsKernelInfoStorePtr opsKernelInfoStorePtr) {
  FUSION_PASS_CHECK(opsKernelInfoStorePtr == nullptr,
           OP_LOGE("Const2AttrFusion", "opsKernelInfoStorePtr is nullptr"), return FAILED);

  for (ge::NodePtr& node : graph.GetDirectNode()) {
    string oriOpType = node->GetOpDesc()->GetType();
    FusionConst2AttrOpRegister reg("");
    if (FusionConst2AttrOpRegistry::Instance()->GetRegisterByOriType(
            oriOpType, reg) != SUCCESS) {
      continue;
    }
    ...
    if (preCheckFunc != nullptr) {
      if (!preCheckFunc(node)) {
        OP_LOGD(opType.c_str(), "node:%s type:%s const to attr preCheck failed.",
    ...
    if (needCheck) {
      ...
      if (ge::NodeUtils::GetNodeUnknownShapeStatus(*(node.get()), unknownShape)
          == ge::GRAPH_SUCCESS && unknownShape) {
        std::string oriUnSupportedReason;
        bool isOriSupported = opsKernelInfoStorePtr->CheckSupported(
            node->GetOpDesc(), oriUnSupportedReason);
        if (isOriSupported) {
          OP_LOGD(opType.c_str(), "Op[name:%s,type:%s] is supported",
                  node->GetOpDesc()->GetName().c_str(), opType.c_str());
          continue;
        } else {
      ...
      ge::OpDescPtr opDescPtr =
          PatternFusionUtil::GetFusionOpDesc(node, opType, attrVec);
      ...
      bool isSupported =
          opsKernelInfoStorePtr->CheckSupported(opDescPtr, unSupportedReason);
      if (!isSupported) {
        OP_LOGI(opType.c_str(), "%s not supported, %s", opType.c_str(),
    ...
    if (dumpret != SUCCESS) {
      OP_LOGW(opType.c_str(), "node:%s RecordOriginalNames for DataDump not success.",
              node->GetName().c_str());
    }
    ...
    if (ret != SUCCESS) {
      OP_LOGI(opType.c_str(), "%s has input which is not a constant, graph not changed.",
              oriOpType.c_str());
      continue;
    }
    ...
    if (PatternFusionUtil::SetOutputDescAttrForDataDump(node, fusionNode) != SUCCESS) {
      OP_LOGW(opType.c_str(), "node:%s SetAttr for DataDump not success.",
...
REGISTER_CONST2ATTR("ApplyAdaMaxD")
    .OriginOpType("ApplyAdaMax");

REGISTER_CONST2ATTR("ApplyAdadeltaD")
    .OriginOpType("ApplyAdadelta");

REGISTER_CONST2ATTR("ApplyAdagradD")
    .OriginOpType("ApplyAdagrad");

REGISTER_CONST2ATTR("ApplyAdagradDAD")
    .OriginOpType("ApplyAdagradDA");

REGISTER_CONST2ATTR("ApplyFtrlD")
    .OriginOpType("ApplyFtrl");

REGISTER_CONST2ATTR("ApplyCenteredRMSPropD")
    .OriginOpType("ApplyCenteredRMSProp");

REGISTER_CONST2ATTR("ApplyAdamD")
    .NeedCheckSupported(false)
    .OriginOpType("ApplyAdam");

REGISTER_CONST2ATTR("SparseApplyProximalAdagradD")
    .OriginOpType("SparseApplyProximalAdagrad");

REGISTER_CONST2ATTR("ApplyAddSignD")
    .OriginOpType("ApplyAddSign");

REGISTER_CONST2ATTR("ApplyPowerSignD")
    .OriginOpType("ApplyPowerSign");

REGISTER_CONST2ATTR("ApplyProximalAdagradD")
    .OriginOpType("ApplyProximalAdagrad");
...
REGISTER_CONST2ATTR("InTopKD")
    .OriginOpType("InTopK")
    .NeedCheckSupported(true)
    .SetConstToAttr(2, "k", "SetInt");
...
REGISTER_CONST2ATTR("ReduceSumD")
    .OriginOpType("ReduceSum")
    .NeedCheckSupported(true)
    .SetConstToAttr(1, "axes", "SetListInt");

REGISTER_CONST2ATTR("CumprodD")
    .OriginOpType("Cumprod")
    .NeedCheckSupported(true)
    .SetConstToAttr(1, "axis", "SetInt");

REGISTER_CONST2ATTR("CumsumD")
    .OriginOpType("Cumsum")
    .NeedCheckSupported(true)
    .SetConstToAttr(1, "axis", "SetInt");
...
REGISTER_CONST2ATTR("SparseApplyAdagradD")
    .OriginOpType("SparseApplyAdagrad")
...
REGISTER_CONST2ATTR("ApplyFtrlV2D")
    .OriginOpType("ApplyFtrlV2");

REGISTER_CONST2ATTR("ApplyMomentumD")
    .OriginOpType("ApplyMomentum");

REGISTER_CONST2ATTR("ApplyKerasMomentumD")
    .OriginOpType("ApplyKerasMomentum");
