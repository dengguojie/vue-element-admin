...
vector<FusionPattern *> MulAddNL2LossFusionPass::DefinePatterns() {
  ...
  FusionPattern *pattern =
      new (std::nothrow) FusionPattern("MulAddNL2LossFusionPass");
...
Status MulAddNL2LossFusionPass::Fusion(ge::ComputeGraph &graph,
                                       Mapping &mapping,
                                       vector<ge::NodePtr> &newNodes) {
  OP_LOGD(FUSED_OP_TYPE.c_str(), "Enter graph fusion MulAddNFusionPass!");
  ge::NodePtr addn_node =
      GetNodeFromMapping(FUSION_NODE_PATTERN_ID_ADDN, mapping);
  ge::NodePtr mul_node =
      GetNodeFromMapping(FUSION_NODE_PATTERN_ID_MUL, mapping);
  ...
  ge::NodePtr variable_node =
      mul_node->GetInDataAnchor(0)->GetPeerOutAnchor()->GetOwnerNode();
  FUSION_PASS_CHECK(variable_node->GetType() != "Variable" && variable_node->GetType() != "TransData",
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Mul node[%s]'s 1st input must be a variable,"
                   "actuall is [%s].",
                   mul_node->GetName().c_str(), variable_node->GetType().c_str()),
           return NOT_CHANGED);
  if (variable_node->GetType() == "TransData") {
    FUSION_PASS_CHECK(variable_node->GetInDataNodes().at(0)->GetType() != "Variable",
             OP_LOGI(FUSED_OP_TYPE.c_str(), "TransData node[%s]'s 1st input must be a variable,"
                     "actuall is [%s].",
                     variable_node->GetName().c_str(),
                     variable_node->GetInDataNodes().at(0)->GetType().c_str()),
             return NOT_CHANGED);
  }
  ...
  FUSION_PASS_CHECK(l2loss_node == nullptr,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "Variable node[%s] don't have l2loss out node.",
                   variable_node->GetName().c_str()),
           return NOT_CHANGED);
  ...
  FUSION_PASS_CHECK(l2loss_node->GetOutAllNodes().size() != 1,
           OP_LOGI(FUSED_OP_TYPE.c_str(), "L2Loss node[%s] should only have one output node, but "
                   "actually is [%u].",
                   l2loss_node->GetName().c_str(),
                   l2loss_node->GetOutAllNodes().size()),
           return NOT_CHANGED);

  ge::NodePtr l2loss_out_node =
      l2loss_node->GetOutDataAnchor(0)->GetFirstPeerAnchor()->GetOwnerNode();
  FUSION_PASS_CHECK(l2loss_out_node->GetType() != "AddN",
           OP_LOGI(FUSED_OP_TYPE.c_str(), "L2Loss node[%s]'s output node must be AddN.",
                   l2loss_out_node->GetName().c_str()),
           return NOT_CHANGED);

  string addn_stream_label;
  string mul_stream_label;
  string l2loss_stream_label;
  ge::AttrUtils::GetStr(addn_node->GetOpDesc(), STREAM_LABEL,
                        addn_stream_label);
  ge::AttrUtils::GetStr(mul_node->GetOpDesc(), STREAM_LABEL, mul_stream_label);
  ge::AttrUtils::GetStr(l2loss_node->GetOpDesc(), STREAM_LABEL,
                        l2loss_stream_label);
  ...
  if (!((addn_stream_label == mul_stream_label) &&
        (mul_stream_label == l2loss_stream_label))) {
  ...
  for (auto mulInNode : mul_node->GetInAllNodes()) {
    ...
    if (mulInNode->GetInControlAnchor() == nullptr ||
        mulInNode->GetInControlAnchor()->GetPeerOutControlAnchors().size() ==
            0) {
      continue;
    }
  ...
  if (domi::OpRegistry::Instance()->GetImplyType(mul_node->GetType()) ==
      domi::ImplyType::CCE) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "Op %s is CCE, no need to fusion.", mul_node->GetName().c_str());
    return NOT_CHANGED;
  }
  if (domi::OpRegistry::Instance()->GetImplyType(addn_node->GetType()) ==
      domi::ImplyType::CCE) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "Op %s is CCE, no need to fusion.", addn_node->GetName().c_str());
    return NOT_CHANGED;
  }
  if (domi::OpRegistry::Instance()->GetImplyType(l2loss_node->GetType()) ==
      domi::ImplyType::CCE) {
  ...
  std::map<string, uint32_t> addn_input_names =
      addn_node->GetOpDesc()->GetAllInputName();
  for (auto index_name : addn_input_names) {
    OP_LOGI(FUSED_OP_TYPE.c_str(), "addn_node's input name is :%s, input index is %u",
  ...
  fusionDesc->SetName(addn_node->GetOpDesc()->GetName() +
                      "/FusedMulAddNL2loss");
  ...
  if (l2loss_node->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
    for (ge::InDataAnchorPtr inAnchorPtr :
         l2loss_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
      inAnchorPtr->UnlinkAll();
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(
                              fusionNode->GetOutDataAnchor(1), inAnchorPtr),
               OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from node:%s's 2nd index to node:%s's 1st "
                       "index failed.",
  ...
  for (unsigned int i = 0; i < addn_node->GetAllInDataAnchors().size(); i++) {
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(
                            addn_node->GetInDataAnchor(i)->GetPeerOutAnchor(),
                            fusionNode->GetInDataAnchor(i)),
             OP_LOGE(FUSED_OP_TYPE.c_str(), "Add edge from fused node:%s's index[%d] to fusion "
                     "node:%s's index[%d] failed.",
                     addn_node->GetName().c_str(), i,
                     fusionNode->GetName().c_str(), i),
             return FAILED);
    OP_LOGI(FUSED_OP_TYPE.c_str(),
  ...
  for (unsigned int i = 0;
       i < addn_node->GetInControlAnchor()->GetPeerOutControlAnchors().size();
       i++) {
    FUSION_PASS_CHECK(
        SUCCESS !=
            ge::GraphUtils::AddEdge(
                addn_node->GetInControlAnchor()->GetPeerOutControlAnchors().at(
                    i),
  ...
  if (addn_node->GetOutDataAnchor(0)->GetPeerInDataAnchors().size() > 0) {
    ...
    for (InDataAnchorPtr inAnchorPtr :
         addn_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
      inAnchorPtr->UnlinkAll();
      FUSION_PASS_CHECK(
          SUCCESS != ge::GraphUtils::AddEdge(fusionNode->GetOutDataAnchor(0),
                                             inAnchorPtr),
  ...
  FUSION_PASS_CHECK(
      ge::GRAPH_SUCCESS != graph.RemoveNode(addn_node),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "remove addn_node node[%s] failed", addn_node->GetName().c_str()),
      return FAILED);
  FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != graph.RemoveNode(l2loss_node),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "remove l2loss_node node[%s] failed",
                   l2loss_node->GetName().c_str()),
           return FAILED);
...
REGISTER_PASS("MulAddNL2LossFusionPass", BUILT_IN_GRAPH_PASS,
