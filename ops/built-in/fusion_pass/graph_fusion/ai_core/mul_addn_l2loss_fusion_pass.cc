...
Status MulAddNL2LossFusionPass::Fusion(ComputeGraph& graph, Mapping& mapping, vector<NodePtr>& new_nodes) {
  ...
  FUSION_PASS_CHECK(addn_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The addn_node is null, fusion failed."),
  ...
  FUSION_PASS_CHECK(in_data_anchor0 == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "The in_data_anchor0 is null, fusion failed."),
                                                   return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(peer_out_anchor0 == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                                   "The peer_out_anchor0 is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  if (variable_node->GetType() == "TransData") {
    FUSION_PASS_CHECK(variable_node->GetInDataNodes().empty(),
                      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                                     "The size of in_data_node is 0, fusion failed."),
                      return PARAM_INVALID);
    FUSION_PASS_CHECK(
  ...
  for (uint32_t index = 0; index < mul_node->GetAllInDataAnchors().size(); index++) {
    ...
    FUSION_PASS_CHECK(input_node_mul_out == nullptr,
                      VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                                     "The input_node_mul_out is null, fusion failed."),
                      return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(mul_node_in_data_anchor == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                                   "The mul_node_in_data_anchor is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(fusion_desc == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                    "The fusion_desc is null."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(fusion_node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "AddNode failed."),
                    return PARAM_INVALID);
  ...
  if (out_data_anchor0->GetPeerInDataAnchors().size() > 0) {
    for (InDataAnchorPtr in_anchor_ptr : out_data_anchor0->GetPeerInDataAnchors()) {
      FUSION_PASS_CHECK(in_anchor_ptr == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                        "The in_anchor_ptr is nullptr."),
                        return PARAM_INVALID);
      in_anchor_ptr->UnlinkAll();
      FUSION_PASS_CHECK(
          GraphUtils::AddEdge(fusion_node->GetOutDataAnchor(1), in_anchor_ptr) != GRAPH_SUCCESS,
          VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                         "Add edge from node:%s's 2nd index to node:%s's 1st index failed.",
  ...
  FUSION_PASS_CHECK(addn_node->GetInControlAnchor() == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(),
                                                   "The GetInControlAnchor of addn_node is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(addn_node_out_data_anchor_0 == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "The addn_node_out_data_anchor_0 is null."),
                                                   return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(addn_node) != GRAPH_SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "Remove addn_node node[%s] failed",
                                                   addn_node->GetName().c_str()),
                    return FAILED);
  FUSION_PASS_CHECK(graph.RemoveNode(l2loss_node) != GRAPH_SUCCESS,
                    VECTOR_FUSION_INNER_ERR_REPORT(kFusedOpType.c_str(), "Remove l2loss_node node[%s] failed",
