...
vector<FusionPattern*> MeanGradFusionPass::DefinePatterns() {
  ...
  FUSION_PASS_CHECK(pattern == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "new an object failed."), return patterns);
...
Status MeanGradFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& fusionNodes) {
  ...
  FUSION_PASS_CHECK(reshapeNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "reshapeNode is nullptr."), return FAILED);
  FUSION_PASS_CHECK(tileNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "tileNode is nullptr."), return FAILED);
  FUSION_PASS_CHECK(truedivNode == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "truedivNode is nullptr."), return FAILED);
  int32_t outputN = 0;
  int32_t outputH = 0;
  int32_t outputW = 0;
  int32_t outputC = 0;
  ...
  for (auto inNode : reshapeNode->GetInDataNodes()) {
    ...
    if ((nodeType == "Constant") || (nodeType == "Const")) {
      ...
      FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                        "get reshape output batch failed."), return ret);
      ret = ParseParaFromConst(inNode, outputC, INPUT_DESC_INDEX_THREE);
      FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
  ...
  FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "Remove tileNode failed."), return ret);
  ret = graph.RemoveNode(truedivNode);
  FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                    "Remove truedivNode failed."), return ret);
...
Status MeanGradFusionPass::RemoveConstOpInput(ge::ComputeGraph& graph, const ge::NodePtr node) {
  FUSION_PASS_CHECK(node == nullptr, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
  ...
  for (auto inDataAnchor : node->GetAllInDataAnchors()) {
    ...
    if (nodeType == "Constant" || nodeType == "Const") {
      ...
      if (outDataAnchor->GetOwnerNode()->GetOutDataNodes().size() == 1) {
        ...
        FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                          "Remove const node failed"), return ret);
      } else {
        ret = ge::GraphUtils::RemoveEdge(outDataAnchor, inDataAnchor);
        ge::NodeUtils::ClearInDataAnchor(node, inDataAnchor);
        ge::OpDescUtils::ClearInputDesc(node->GetOpDesc(), (uint32_t)idx);
        FUSION_PASS_CHECK(ret != SUCCESS, VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                          "Remove edge from const op failed"), return ret);
      }
    }
  }
  return SUCCESS;
...
Status MeanGradFusionPass::ParseParaFromConst(const ge::NodePtr node, int32_t& param, const int index) {
  ...
  if (nodeType == "Const" || nodeType == "Constant") {
    ...
    FUSION_PASS_CHECK(weights_vec.empty(), VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
                      "get weights failed"), return PARAM_INVALID);
