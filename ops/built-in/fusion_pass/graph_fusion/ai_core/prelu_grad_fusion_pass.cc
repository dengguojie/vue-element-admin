...
Status PReluGradFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(prelugradNode == nullptr,
                    VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(), "prelugradNode is null, fusion failed."),
                    return PARAM_INVALID);

  ge::OpDescPtr prelugradDesc = prelugradNode->GetOpDesc();
  FUSION_PASS_CHECK(prelugradDesc == nullptr,
  ...
  FUSION_PASS_CHECK(
      ...
      VECTOR_FUSION_INNER_ERR_REPORT(FUSED_OP_TYPE.c_str(),
        "remove fusedNode node[%s] failed", prelugradNode->GetName().c_str()),
      return FAILED);
