...
ge::NodePtr PReluGradFusionPass::AddPReluGradNoneNode(ge::NodePtr prelugradNode, ge::ComputeGraph& graph,
  ...
  FUSION_PASS_CHECK(prelugradNoneDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "prelugradNone's OpDesc is null, fusion failed."),
                    failStatus = true);
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", prelugradNoneNode->GetName().c_str()),
      failStatus = true);
  ...
  FUSION_PASS_CHECK(prelugradNode->GetOutDataAnchor(0) == nullptr,
                    OP_LOGE(FUSED_OP_TYPE.c_str(), "prelugrad node output anchor is null"), failStatus = true);
...
ge::NodePtr PReluGradFusionPass::AddReduceNode(ge::NodePtr prelugradNode, ge::NodePtr prelugradNoneNode,
  ...
  FUSION_PASS_CHECK(
      reduceNode == nullptr,
      OP_LOGE(FUSED_OP_TYPE.c_str(), "fusionNode:%s is null, fusion failed.", reduceNode->GetName().c_str()),
      failStatus = true);
...
Status PReluGradFusionPass::Fusion(ge::ComputeGraph& graph, Mapping& mapping, vector<ge::NodePtr>& newNodes) {
  ...
  FUSION_PASS_CHECK(prelugradNode == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "prelugradNode is null, fusion failed."),
                    return PARAM_INVALID);

  ge::OpDescPtr prelugradDesc = prelugradNode->GetOpDesc();
  FUSION_PASS_CHECK(prelugradDesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "prelugrad's OpDesc is null, fusion failed."),
                    return PARAM_INVALID);
  ...
  FUSION_PASS_CHECK(failStatus, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddPReluGradNoneNode:check failed, fusion failed."),
                    return FAILED);

  AddReduceNode(prelugradNode, prelugradNoneNode, graph, newNodes, failStatus);
  FUSION_PASS_CHECK(failStatus, OP_LOGE(FUSED_OP_TYPE.c_str(), "AddReduceNode:check failed, fusion failed."),
                    return FAILED);
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGE(FUSED_OP_TYPE.c_str(), "remove fusedNode node[%s] failed", prelugradNode->GetName().c_str()),
      return FAILED);
