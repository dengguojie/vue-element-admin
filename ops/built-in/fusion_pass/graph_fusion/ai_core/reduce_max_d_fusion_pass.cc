...
namespace fe {
static const string PATTERN_MAXD = "maxd";
static const string MAXD = "ReduceMaxD";
static const string MAXDLAST = "ReduceMaxD";
...
vector<FusionPattern *> ReduceMaxDFusionPass::DefinePatterns() {
  ...
  new (std::nothrow) FusionPattern("ReduceMaxDFusionPass");
  pattern->AddOpDesc(PATTERN_MAXD, {MAXD})
          .SetOutput(PATTERN_MAXD);
  patterns.push_back(pattern);
  return patterns;
}

Status ReduceMaxDFusionPass::Fusion(ge::ComputeGraph &graph,
    Mapping &mapping, vector<ge::NodePtr> &newNodes) {
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Define ReduceMaxDFusionPass fusion begin");
  ge::NodePtr max_node =
      GetNodeFromMapping(PATTERN_MAXD, mapping);
  FUSION_PASS_CHECK(max_node == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "max_node is null, fusion failed."),
           return PARAM_INVALID);
  // validation
  ge::GeTensorDesc tensor_input =
      max_node->GetOpDesc()->GetInputDesc(0);
  ...
  minNewOpdesc =
      std::make_shared<ge::OpDesc>(max_node->GetName(), MAXD);
  ...
  minLastOpdesc =
      std::make_shared<ge::OpDesc>(max_node->GetName() + "_Tik", MAXDLAST);
  ...
  ge::GeTensorDesc input_tensor1 =
      max_node->GetOpDesc()->GetOutputDesc(0);
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::SetBool(minLastNode->GetOpDesc(), KEEPDIMS,
           keepdims),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Set keepdims attr failed"), return FAILED);
  ...
  for (auto inDataAnchor :
       max_node->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(max_node->GetOutDataAnchor(0),
  ...
  if (max_node->GetOutControlAnchor()) {
    for (auto inControlAnchor :
         max_node->GetOutControlAnchor()->GetPeerInControlAnchors()) {
      FUSION_PASS_CHECK(
          ge::GraphUtils::RemoveEdge(max_node->GetOutControlAnchor(),
...
REGISTER_PASS("ReduceMaxDFusionPass", BUILT_IN_GRAPH_PASS,
              ReduceMaxDFusionPass);
}
