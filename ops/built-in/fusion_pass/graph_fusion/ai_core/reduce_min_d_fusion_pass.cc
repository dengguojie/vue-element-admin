...
namespace fe {
static const string PATTERN_MIND = "mind";
static const string MIND = "ReduceMinD";
static const string MINDLAST = "ReduceMinD";
...
vector<FusionPattern *> ReduceMinDFusionPass::DefinePatterns() {
  ...
  new (std::nothrow) FusionPattern("ReduceMinDFusionPass");
  pattern->AddOpDesc(PATTERN_MIND, {MIND})
          .SetOutput(PATTERN_MIND);
  patterns.push_back(pattern);
  return patterns;
}

Status ReduceMinDFusionPass::Fusion(ge::ComputeGraph &graph,
    Mapping &mapping, vector<ge::NodePtr> &newNodes) {
  OP_LOGI(FUSED_OP_TYPE.c_str(), "Define ReduceMinDFusionPass fusion begin");
  ge::NodePtr minDNode = \
  GetNodeFromMapping(PATTERN_MIND, mapping);
  FUSION_PASS_CHECK(minDNode == nullptr,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "minDNode is null, fusion failed."),
           return PARAM_INVALID);
  // validation
  ge::GeTensorDesc tensor_input = \
  minDNode->GetOpDesc()->GetInputDesc(0);
  ...
  minNewOpdesc = \
  std::make_shared<ge::OpDesc>(minDNode->GetName(), MIND);
  FUSION_PASS_CHECK(minNewOpdesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "minNewOpdesc is null,"
  "fusion failed."), return PARAM_INVALID);

  // Create a new min_d_last node description
  std::shared_ptr<ge::OpDesc> minLastOpdesc = nullptr;
  minLastOpdesc = \
  std::make_shared<ge::OpDesc>(minDNode->GetName() + "_Tik", MINDLAST);
  FUSION_PASS_CHECK(minLastOpdesc == nullptr, OP_LOGE(FUSED_OP_TYPE.c_str(), "minLastOpdesc is null,"
  "fusion failed."), return PARAM_INVALID);

  // add input for minnew
  ge::GeTensorDesc input_tensor1_new = \
  minDNode->GetOpDesc()->GetInputDesc(0);
  ...
  ge::GeTensorDesc input_tensor1 =
      minDNode->GetOpDesc()->GetOutputDesc(0);
  ...
  // output的tensor描述需要计算，shape参考infershape推导
  bool keepdims;
  ...
  ge::GeTensorDesc out_tensor1_new =
      minDNode->GetOpDesc()->GetOutputDesc(0);
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::SetBool(minNewNode->GetOpDesc(), KEEPDIMS, keepdims),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[name=%s]: Set attr %s failed",
                   minNewNode->GetName().c_str(), KEEPDIMS.c_str()),
           return FAILED);
  FUSION_PASS_CHECK(
      !ge::AttrUtils::SetBool(minLastNode->GetOpDesc(), KEEPDIMS, keepdims),
      OP_LOGE(FUSED_OP_TYPE.c_str(), "Op[name=%s]: Set attr %s failed", minLastNode->GetName().c_str(),
              KEEPDIMS.c_str()),
      return FAILED);
  ...
  FUSION_PASS_CHECK(!ge::AttrUtils::SetListInt(minLastNode->GetOpDesc(), AXIS,
           axis_last),
           OP_LOGE(FUSED_OP_TYPE.c_str(), "minLast Set axis attr failed"), return FAILED);
  // connect output edge for minnew
  FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(minNewNode->GetOutDataAnchor(0),
                                   minLastNode->GetInDataAnchor(0)) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Add out data edge failed."), return FAILED);

  // copy output edge for minlast
  for (auto inDataAnchor :
       minDNode->GetOutDataAnchor(0)->GetPeerInDataAnchors()) {
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(minDNode->GetOutDataAnchor(0),
                                        inDataAnchor) != SUCCESS,
             OP_LOGE(FUSED_OP_TYPE.c_str(), "minDNode Remove out data edge failed."), return FAILED);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(minLastNode->GetOutDataAnchor(0),
                                     inDataAnchor) != SUCCESS,
  ...
  if (minDNode->GetOutControlAnchor()) {
    for (auto inControlAnchor :
         minDNode->GetOutControlAnchor()->GetPeerInControlAnchors()) {
      FUSION_PASS_CHECK(
          ge::GraphUtils::RemoveEdge(minDNode->GetOutControlAnchor(),
                                     inControlAnchor) != SUCCESS,
          OP_LOGE(FUSED_OP_TYPE.c_str(), "minDNode Remove out control edge failed."), return FAILED);
      FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(minLastNode->GetOutControlAnchor(),
                                       inControlAnchor) != SUCCESS,
               OP_LOGE(FUSED_OP_TYPE.c_str(), "minLastNode Add out control edge failed."),
               return FAILED);
    }
  }
  ...
  FUSION_PASS_CHECK(graph.RemoveNode(minDNode) != SUCCESS,
           OP_LOGE(FUSED_OP_TYPE.c_str(), "Remove minDNode node failed."), return FAILED);
...
REGISTER_PASS("ReduceMinDFusionPass", BUILT_IN_GRAPH_PASS,
              ReduceMinDFusionPass);
}
