 ...
 *
 * @brief Provide some basic methods for fusion pass (include change a const anchor to attr)
 *
 */
...
void PatternFusionUtil::SetConstValueToAttr(ge::OpDescPtr op_desc,
  ...
  if (dtype == ge::DT_INT32) {
    ...
    if (size > 1) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        OP_LOGD(curOpType.c_str(), "Node:%s const data int32 proto %d", op_desc->GetName().c_str(), (int32_t)(*(const_data_ptr + i)));
      }
    ...
    if (size > 1) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        OP_LOGD(curOpType.c_str(), "Node:%s const data int64 proto %d", op_desc->GetName().c_str(), (int64_t)(*(const_data_ptr + i)));
      }
      ge::AttrUtils::SetListInt(op_desc, attr_name, const_data);
    }
    ...
    if (size == 1) {
      ...
      ge::AttrUtils::SetFloat(op_desc, attr_name, const_data);
    }
    if (size > 1) {
      std::vector <float> const_data;
      for (size_t i = 0; i < size; ++i) {
        const_data.push_back(((float)(*(const_data_ptr + i))));
        OP_LOGD(curOpType.c_str(),"Node:%s const data float proto %f", op_desc->GetName().c_str(), (float)(*(const_data_ptr + i)));
      }
      ge::AttrUtils::SetListFloat(op_desc, attr_name, const_data);
    }
    ...
    if (size == 1) {
      ...
      ge::AttrUtils::SetFloat(op_desc, attr_name, const_data_fp32);
    }
    if (size > 1) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        OP_LOGD(curOpType.c_str(), "Node:%s const data float proto %f",
                op_desc->GetName().c_str(), const_data_fp32);
      }
      ge::AttrUtils::SetListFloat(op_desc, attr_name, const_data_vec);
    }
  } else {
    OP_LOGW(curOpType.c_str(), "not support this node %s", op_desc->GetName().c_str());
  }
}
...
Status PatternFusionUtil::AddInputToOutput(ge::NodePtr node, std::vector<PassInputInfo> &inputInfoVec) {
  ...
  FUSION_PASS_CHECK(node == nullptr, OP_LOGE(curOpType.c_str(), "node is null, add input to output failed."), return PARAM_INVALID);
  FUSION_PASS_CHECK(inputInfoVec.empty(), OP_LOGE(curOpType.c_str(), "inputInfoVec is empty, add input to output failed."), return PARAM_INVALID);
  ...
  for (auto &inputInfo : inputInfoVec) {
    ...
    if (inputInfo.inputOpDescName != node->GetOpDesc()->GetInputNameByIndex(inputInfo.inputOpDescIndex)) {
      OP_LOGE(curOpType.c_str(), "node name:%s, input name:%s is not same to opdesc input name:%s.",
              node->GetName().c_str(), inputInfo.inputOpDescName.c_str(), node->GetOpDesc()->GetInputNameByIndex(inputInfo.inputOpDescIndex).c_str());
      return FAILED;
    }
    ...
    OP_LOGD(curOpType.c_str(), "node name:%s, input name:%s, inputIdx:%u, add output success.",
            node->GetName().c_str(), inputInfo.inputOpDescName.c_str(), inputInfo.inputOpDescIndex);
  }
...
Status PatternFusionUtil::ConstToAttr(ge::ComputeGraph& graph,
                                      ge::NodePtr fusedNode,
                                      std::string fusionOpType,
                                      std::map<int16_t , std::string> attrInfo) {
  FUSION_PASS_CHECK(fusedNode == nullptr, OP_LOGE(fusionOpType.c_str(), "fusedNode is null, fusion failed."), return PARAM_INVALID);
  ge::OpDescPtr fusedDesc = fusedNode->GetOpDesc();
  FUSION_PASS_CHECK(fusedDesc == nullptr, OP_LOGE(fusionOpType.c_str(), "Node:%s's OpDesc is null, fusion failed.", fusedNode->GetName().c_str()),
           return PARAM_INVALID);
  Operator op = ge::OpDescUtils::CreateOperatorFromNode(fusedNode);
  Tensor constTensor;
  vector<int16_t> attrIndex;
  for (auto getAttrInfo : attrInfo) {
    attrIndex.push_back(getAttrInfo.first);
  }
  std::sort(attrIndex.begin(), attrIndex.end());
  ge::OpDescPtr fusionDesc = AttrUtils::CloneOpDesc(fusedDesc);
  fusionDesc->SetType(fusionOpType);
  FUSION_PASS_CHECK(fusionDesc == nullptr, OP_LOGE(fusionOpType.c_str(), "Node:%s's OpDesc is null, fusion failed.", fusedNode->GetName().c_str()),
           return PARAM_INVALID);
  for(int i = attrIndex.size() - 1; i >= 0 ; i--) {
    unsigned int index = attrIndex[i];
    if (index>=fusionDesc->GetInputsSize()) {
      OP_LOGI(fusionOpType.c_str(), "Index[%d] is beyond the size[%d] of input desc", index, fusionDesc->GetInputsSize());
      continue;
    }
  ...
  FUSION_PASS_CHECK(fusionNode == nullptr, OP_LOGE(fusionOpType.c_str(), "fusionNode:%s is null, fusion failed.", fusionNode->GetName().c_str()), return PARAM_INVALID);
  for (unsigned int i = 0, j = 0; i < fusedNode->GetAllInDataAnchors().size(); i++) {
    if (std::find(attrIndex.begin(), attrIndex.end(), i) != attrIndex.end()) {
      continue;
    }
    FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusedNode->GetInDataAnchor(i)->GetPeerOutAnchor(),
                                                fusionNode->GetInDataAnchor(j)),
             OP_LOGE(fusionOpType.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d] failed.", fusedNode->GetName().c_str(), i, fusionNode->GetName().c_str(), j),
             return FAILED);
    OP_LOGD(fusionOpType.c_str(), "Add edge from fused node:%s's index[%d] to fusion node:%s's index[%d].", fusedNode->GetName().c_str(), i, fusionNode->GetName().c_str(), j);
    j++;
  }

  for (unsigned int i = 0; i < fusedNode->GetAllInDataAnchors().size(); i++) {
    map<int16_t , std::string>::iterator iter = attrInfo.find(i);
    if (attrInfo.end() != iter) {
      ge::InDataAnchorPtr fused_anchor_ptr = fusedNode->GetInDataAnchor(iter->first);
      ge::OutDataAnchorPtr const_anchor_ptr = fused_anchor_ptr->GetPeerOutAnchor();
      ge::NodePtr constNode = const_anchor_ptr->GetOwnerNode();
      if (constNode->GetType() != "Constant" && constNode->GetType() != "Const") {
        OP_LOGW(fusionOpType.c_str(), "Node %s's %dth input is not a constant.", fusedNode->GetName().c_str(), iter->first);
        return NOT_CHANGED;
      }
      OP_LOGI(fusionOpType.c_str(), "Input name is %s", fusedDesc->GetInputNameByIndex(iter->first).c_str());
      op.GetInputConstData(fusedDesc->GetInputNameByIndex(iter->first), constTensor);
      SetConstValueToAttr(fusionDesc, constTensor, fusedDesc->GetInputDesc(iter->first).GetDataType(), iter->second);
    }
  }
  ...
  for (unsigned int i = 0; i < fusedNode->GetAllOutDataAnchors().size(); i++) {
    ...
    for (unsigned int j = 0; j < inAnchorVec.size(); j++) {
      FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusionNode->GetOutDataAnchor(i), inAnchorVec[j]),
               OP_LOGE(fusionOpType.c_str(), "Add edge from fusion node:%s's index[%d] to fused node:%s's index[%d] failed.", fusionNode->GetName().c_str(), i, fusedNode->GetName().c_str(), j),
               return FAILED);
      OP_LOGD(fusionOpType.c_str(), "Add edge from fusion node:%s's index[%d] to fused node:%s's index[%d].", fusionNode->GetName().c_str(), i, fusedNode->GetName().c_str(), j);
    }
  }
  ...
  if (fusedNode->GetInControlAnchor() != nullptr) {
    if (!fusedNode->GetInControlAnchor()->GetPeerOutControlAnchors().empty() && fusionNode->GetInControlAnchor() != nullptr) {
      OP_LOGI(fusionOpType.c_str(), "The PeerOutControlAnchors of fused node[%s] input control anchor is empty.", fusedNode->GetName().c_str());
      for (OutControlAnchorPtr outCtrlAnchorPtr : fusedNode->GetInControlAnchor()->GetPeerOutControlAnchors()) {
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(outCtrlAnchorPtr, fusionNode->GetInControlAnchor()),
                 OP_LOGE(fusionOpType.c_str(), "Fail to add input control edge for fusion node:%s.", fusionNode->GetName().c_str()),
                 return FAILED);
      }
    }
  ...
  if (fusedNode->GetOutControlAnchor() != nullptr) {
    if (!fusedNode->GetOutControlAnchor()->GetPeerInControlAnchors().empty() && fusionNode->GetOutControlAnchor() != nullptr) {
      OP_LOGI(fusionOpType.c_str(), "The PeerInControlAnchors of fused node[%s] output control anchor is empty.", fusedNode->GetName().c_str());
      for (InControlAnchorPtr inCtrlAnchorPtr : fusedNode->GetOutControlAnchor()->GetPeerInControlAnchors()) {
        FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(fusionNode->GetOutControlAnchor(), inCtrlAnchorPtr),
                 OP_LOGE(fusionOpType.c_str(), "Fail to add output control edge for fusion node:%s.", fusionNode->GetName().c_str()),
                 return FAILED);
      }
    }
...
void PatternFusionUtil::SetConstValueToAttrWithType(ge::OpDescPtr op_desc,
                                                    const ge::Tensor& const_tensor,
                                                    const DataType& dtype,
                                                    PassAttrInfo& attrInfo) {
  string curOpType = op_desc->GetType();
  OP_LOGD(curOpType.c_str(), "Begin to convert const data of node[%s] to attribute value.",
          op_desc->GetName().c_str());
  size_t size = 0;
  if (dtype == ge::DT_INT32) {
    int32_t* const_data_ptr = (int32_t*) const_tensor.GetData();
    FUSION_PASS_CHECK(const_data_ptr == nullptr,
             OP_LOGE(curOpType.c_str(), "const_data_ptr is null, fusion failed."), return);
    size = const_tensor.GetSize() / sizeof(int32_t);
    ...
    OP_LOGW(curOpType.c_str(), "Data type of const node of [%s] is not supported.",
            op_desc->GetName().c_str());
  }
}

Status PatternFusionUtil::ConstToAttrWithType(ge::ComputeGraph& graph,
                                              ge::NodePtr& fusedNode,
                                              std::string fusionOpType,
                                              std::vector<PassAttrInfo>& attrInfos) {
  ge::NodePtr fusionNode = nullptr;
  Status ret = ConstToAttrWithNode(graph, fusedNode, fusionOpType, attrInfos, fusionNode);
  return ret;
}

Status PatternFusionUtil::ConstToAttrWithNode(ge::ComputeGraph& graph,
                                              ge::NodePtr& fusedNode,
                                              std::string fusionOpType,
                                              std::vector<PassAttrInfo>& attrInfos,
  ...
  for (size_t i = 0, j = 0; i < fusedNode->GetAllInDataAnchors().size(); i++) {
    if (!FindAttrInfoByIndex(attrInfos, i, attr)) {
      if (fusedDesc->GetInputNameByIndex(i).empty()) {
        OP_LOGE(fusionOpType.c_str(), "Get fuseNode:[%s]'s inputName by index [%d] failed.", fusedNode->GetName().c_str(), i);
        return FAILED;
      }
    ...
    ge::NodePtr constNode = const_anchor_ptr->GetOwnerNode();
    if (constNode->GetType() != "Constant" && constNode->GetType() != "Const") {
      OP_LOGW(fusionOpType.c_str(), "Node %s's %dth input is not a constant.", fusedNode->GetName().c_str(), attr.attrIndex);
      return NOT_CHANGED;
    } else {
    ...
    if (fusedDesc->GetInputNameByIndex(attr.attrIndex).empty()) {
      OP_LOGE(fusionOpType.c_str(), "Get fuseNode:[%s]'s inputName by index [%d] failed.", fusedNode->GetName().c_str(), attr.attrIndex);
      return FAILED;
    }
...
Status PatternFusionUtil::RecordOriginalNamesForConstToAttr(
        ge::NodePtr& fusedNode, std::vector<PassAttrInfo>& attrInfos,
        std::vector<ge::NodePtr> &originalNodes) {
  FUSION_PASS_CHECK(fusedNode == nullptr, OP_LOGE("RecordOriginalNamesForConstToAttr", "fusedNode is null, fusion failed."),
           return PARAM_INVALID);
  ge::OpDescPtr fusedDesc = fusedNode->GetOpDesc();
  ...
  for (size_t i = 0; i < fusedNode->GetAllInDataAnchors().size(); i++) {
    ...
    ge::InDataAnchorPtr fused_anchor_ptr =
            fusedNode->GetInDataAnchor(attr.attrIndex);
    ge::OutDataAnchorPtr const_anchor_ptr =
            fused_anchor_ptr->GetPeerOutAnchor();
    ge::NodePtr constNode = const_anchor_ptr->GetOwnerNode();
    if (constNode->GetType() != "Constant" && constNode->GetType() != "Const") {
      OP_LOGW(curOpType.c_str(), "Node %s's %dth input is not a constant.",
              fusedNode->GetName().c_str(), attr.attrIndex);
      return NOT_CHANGED;
    } else {
      if (std::find(constNodes.begin(), constNodes.end(), constNode) ==
          constNodes.end()) {
        constNodes.push_back(constNode);
      }
    }
...
{
    for (unsigned int i = 0; i < fusedNode->GetAllOutDataAnchors().size(); i++) {
        ge::AttrUtils::SetStr(fusionNode->GetOpDesc()->MutableOutputDesc(i),
                              ge::ATTR_NAME_DATA_DUMP_ORIGIN_NAME,
                              fusedNode->GetName());
        ge::AttrUtils::SetInt(fusionNode->GetOpDesc()->MutableOutputDesc(i),
                              ge::ATTR_NAME_DATA_DUMP_ORIGIN_OUTPUT_INDEX, i);
        GraphPassUtil::SetDataDumpOriginDataType(
                fusedNode->GetOpDesc()->GetOutputDesc(i).GetOriginDataType(),
                fusionNode->GetOpDesc()->MutableOutputDesc(i));
        GraphPassUtil::SetDataDumpOriginFormat(
                fusedNode->GetOpDesc()->GetOutputDesc(i).GetOriginFormat(),
                fusionNode->GetOpDesc()->MutableOutputDesc(i));
    }
    return SUCCESS;
}
...
ge::OpDescPtr PatternFusionUtil::GetFusionOpDesc(ge::NodePtr fusedNodePtr,
                                     std::string fusionOpType,
                                     std::vector<PassAttrInfo>& attrInfos) {

  FUSION_PASS_CHECK(fusedNodePtr == nullptr, OP_LOGE(fusionOpType.c_str(), "fusedNode is null, fusion failed."), return nullptr);
  ...
  for(int i = attrIndexVec.size() - 1; i >= 0 ; i--) {
    ...
    if (index >= fusionDescPtr->GetInputsSize()) {
      OP_LOGI(fusionOpType.c_str(), "Index[%d] is beyond the size[%d] of input desc", index, fusionDescPtr->GetInputsSize());
      continue;
    }
  ...
  for (size_t i = 0; i < fusedNodePtr->GetAllInDataAnchors().size(); i++) {
    ...
    if (!FindAttrInfoByIndex(attrInfos, i, attr)) {
      if (fusionDescPtr->GetInputNameByIndex(i).empty()) {
        OP_LOGE(fusionOpType.c_str(), "Get fuseNode:[%s]'s inputName by index [%d] failed.",
                fusedNodePtr->GetName().c_str(), i);
        return nullptr;
      }
      fusionDescInputName.emplace(fusionDescPtr->GetInputNameByIndex(i),
              newIndex);
      newIndex++;
      continue;
    }
    ...
    ge::NodePtr constNode = const_anchor_ptr->GetOwnerNode();
    if (constNode->GetType() != "Constant" && constNode->GetType() != "Const") {
      OP_LOGW(fusionOpType.c_str(), "Node %s's %dth input is not a constant.", fusedNodePtr->GetName().c_str(), attr.attrIndex);
      return nullptr;
    }
    OP_LOGI(fusionOpType.c_str(), "Input name is %s", fusionDescPtr->GetInputNameByIndex(attr.attrIndex).c_str());
    Tensor constTensor;
    op.GetInputConstData(fusionDescPtr->GetInputNameByIndex(attr.attrIndex), constTensor);
    SetConstValueToAttrWithType(fusionDescPtr, constTensor, opDescPtr->GetInputDesc(attr.attrIndex).GetDataType(), attr);
  }
...
Status PatternFusionUtil::InsertSliceDNodes(ComputeGraph &graph,
                                            NodePtr srcNode,
  ...
  for (auto convInAnchor : srcNode->GetInAllNodes().at(weightIdx)->GetOutDataAnchor(
          0)->GetPeerInDataAnchors()) {
  ...
  if (newSlicedNodes.empty()) {
    for (unsigned int newSrcNodeIdx = 0; newSrcNodeIdx < group; newSrcNodeIdx++) {
      ...
      OpDescPtr sliceDesc = std::make_shared<ge::OpDesc>(
              srcNode->GetName() + to_string(weightIdx) + "_slice" + to_string(newSrcNodeIdx),
              "SliceD");
      FUSION_PASS_CHECK(sliceDesc == nullptr,
               OP_LOGE(curOpType.c_str(), "sliceNdoe's OpDesc is null, fusion failed."),
               return FAILED);
      ...
      FUSION_PASS_CHECK(inputShape.GetDimNum() < 1,
               OP_LOGE(curOpType.c_str(), "sliceNdoe's dim:[%ld] less than one, fusion failed.",
      ...
      FUSION_PASS_CHECK(slicedNode == nullptr,
               OP_LOGE(curOpType.c_str(), "sliceNdoe is null, fusion failed."), return FAILED);
      newSlicedNodes.push_back(slicedNode);
      // connect weight and slice node
      FUSION_PASS_CHECK(ge::GRAPH_SUCCESS != GraphUtils::AddEdge(
              srcNode->GetInAllNodes().at(weightIdx)->GetOutDataAnchor(0),
      ...
      FUSION_PASS_CHECK(ge::GRAPH_SUCCESS !=
               GraphUtils::AddEdge(newSlicedNodes[newSrcNodeIdx]->GetOutDataAnchor(0),
    ...
    for (int newSrcNodeIdx = 0; newSrcNodeIdx < group; newSrcNodeIdx++) {
      for (auto sliceNode : newSlicedNodes) {
        ...
        if (idx == newSrcNodeIdx) {
          // connect slice's output to each new conv node
          FUSION_PASS_CHECK(ge::GRAPH_SUCCESS !=
                   GraphUtils::AddEdge(sliceNode->GetOutDataAnchor(0),
                                       newConvNodes[newSrcNodeIdx]->GetInDataAnchor(weightIdx)),
                   OP_LOGE(curOpType.c_str(), "add concat to output edge fail"), return FAILED);
          newConvNodes[newSrcNodeIdx]->GetOpDesc()->UpdateInputDesc(weightIdx, sliceNode->GetOpDesc()->GetOutputDesc(0));
          continue;
        }
      }
    }
...
Status PatternFusionUtil::GenGroupPaddingTensor(ge::GeTensorDesc &inTensor,
  ...
  FUSION_PASS_CHECK(SUCCESS != ParseChannelIdx(weightOutputDesc, outChannelIdx),
           OP_LOGE(weightOpDesc->GetType().c_str(), "The original format of node[%s, %s]'s output0 is %s, which is unsupportable.",
...
NodePtr PatternFusionUtil::AddGroupPaddingNode(ComputeGraph &graph,
  ...
  FUSION_PASS_MAKE_SHARED(opdesc = std::make_shared<ge::OpDesc>(nodeName,
                                                        "GroupPadding"), return nullptr);
...
Status PatternFusionUtil::ProcessGroupPadding(ComputeGraph &graph, const NodePtr& groupConvNode, int64_t groups) {
  FUSION_PASS_CHECK(groupConvNode == nullptr,
                      OP_LOGE("ProcessGroupPadding", "The groupConvNode is nullptr."), return FAILED);

  ge::GeTensorDesc inTensor;
  ge::GeTensorDesc outTensor;
  string curOpType = groupConvNode->GetType();
  FUSION_PASS_CHECK(groupConvNode->GetInAllNodes().size() < 2,
           OP_LOGE(curOpType.c_str(), "The number of input of the node[name=%s, type=%s] is less than 2, there is no weight input.",
  ...
  FUSION_PASS_CHECK(SUCCESS != GenGroupPaddingTensor(inTensor, outTensor, groups, weightNode),
           OP_LOGE(curOpType.c_str(), "Generate group padding tensor failed."),
           return FAILED);
  FUSION_PASS_CHECK(groupConvNode->GetOpDesc()->UpdateInputDesc(1, outTensor),
           OP_LOGE(curOpType.c_str(), "Update node[%s]'s first input failed.",
  ...
  FUSION_PASS_CHECK(paddingNode == nullptr,
           OP_LOGE(curOpType.c_str(), "Create group padding node failed."),
           return FAILED);

  ge::GraphUtils::RemoveEdge(weightNode->GetOutDataAnchor(0), groupConvNode->GetInDataAnchor(1));

  // link weight->groupPadding->conv
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(weightNode->GetOutDataAnchor(0),
                                              paddingNode->GetInDataAnchor(0)),
           OP_LOGE(curOpType.c_str(), "Add edge from src node[%s] to dst node[%s] failed.",
                   weightNode->GetName().c_str(), paddingNode->GetName().c_str()),
           return FAILED);
  FUSION_PASS_CHECK(SUCCESS != ge::GraphUtils::AddEdge(paddingNode->GetOutDataAnchor(0),
                                              groupConvNode->GetInDataAnchor(1)),
           OP_LOGE(curOpType.c_str(), "Add edge from src node[%s] to dst node[%s] failed.",
                   paddingNode->GetName().c_str(), groupConvNode->GetName().c_str()),
           return FAILED);
  return SUCCESS;
}
 ...
 */
Status PatternFusionUtil::LinkControlEdge(ge::NodePtr srcNode,
                                             ge::NodePtr dstNode) {
 FUSION_PASS_CHECK(srcNode == nullptr,
          OP_LOGE("LinkControlEdge", "srcNode is null, LinkControlEdge failed."), return FAILED);
 FUSION_PASS_CHECK(dstNode == nullptr,
          OP_LOGE("LinkControlEdge", "dstNode is null, LinkControlEdge failed."), return FAILED);
 auto destInAnchor = srcNode->GetInControlAnchor();
 FUSION_PASS_CHECK(destInAnchor == nullptr,
          OP_LOGE("LinkControlEdge", "destInAnchor is null, LinkControlEdge failed."),
          return FAILED);
 for (auto peerOutControlAnchor : destInAnchor->GetPeerOutControlAnchors()) {
   if (peerOutControlAnchor->GetOwnerNode()->GetOpDesc()->GetType() ==
       STREAMSWITCH) {
     // unlink the out anchor of STREAMSWITCH and the in anchor of dest node
     FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(peerOutControlAnchor, destInAnchor) !=
              SUCCESS,
              OP_LOGE(STREAMSWITCH, "remove edge failed, LinkControlEdge failed."),
              return FAILED);
     // add a control edge of the out anchor of STREAMSWITCH and the in control
     // anchor of dest node
     FUSION_PASS_CHECK(
             ge::GraphUtils::AddEdge(peerOutControlAnchor,
                                     dstNode->GetInControlAnchor()) != SUCCESS,
             OP_LOGE(STREAMSWITCH, "add anchor to in control anchor failed, LinkControlEdge "
                     "failed."),
             return FAILED);
   }
 }
...
Status PatternFusionUtil::RemoveInputEdge(ge::NodePtr node) {
  ...
  for (size_t i = 0; i < node->GetAllInDataAnchors().size(); ++i) {
    ...
    FUSION_PASS_CHECK(inDataAnchor == nullptr, OP_LOGE(curOpType.c_str(), "Node[%s]: indataAnchor is null",
                                                       curOpName.c_str()), return FAILED);
    auto preOutDataAnchor = inDataAnchor->GetPeerOutAnchor();
    FUSION_PASS_CHECK(preOutDataAnchor == nullptr, OP_LOGE(curOpType.c_str(), "Node[%s]: outdataAnchor is null",
                                                           curOpName.c_str()), return FAILED);
  ...
  if (inControlAnchor != nullptr) {
    for (ge::OutControlAnchorPtr srcAnchor :
            inControlAnchor->GetPeerOutControlAnchors()) {
        if(ge::GraphUtils::RemoveEdge(srcAnchor, inControlAnchor) != ge::GRAPH_SUCCESS) {
            OP_LOGW(curOpType.c_str(), "Node[%s]: disconnect node input control anchor failed.",
                    curOpName.c_str());
            return FAILED;
        }
    }
...
Status PatternFusionUtil::CopyMultiReferenceConstNode(ge::ComputeGraph &graph,
                                                      ge::NodePtr nodePtr) {
  for (size_t index = 0; index < nodePtr->GetAllInDataAnchors().size();
       index++) {
    ge::NodePtr inputNode =
        nodePtr->GetInDataAnchor(index)->GetPeerOutAnchor()->GetOwnerNode();
    // 1. find const node
    if (inputNode->GetType() == "Const") {
      ...
      if (ge::GraphUtils::AddEdge(newConstNode->GetOutDataAnchor(0),
                                  nodePtr->GetInDataAnchor(index)) != SUCCESS) {
