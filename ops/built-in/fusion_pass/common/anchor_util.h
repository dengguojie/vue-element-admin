...
inline ge::OutDataAnchorPtr GetPeerOutAnchorWithInDataAnchor(ge::NodePtr curr_node, int idx) {
  FUSION_PASS_CHECK(curr_node == nullptr, OP_LOGD("", "node is null."), return nullptr);

  auto in_anchor = curr_node->GetInDataAnchor(idx);
  FUSION_PASS_CHECK(in_anchor == nullptr, OP_LOGD(curr_node, "in data anchor is null."), return nullptr);
...
inline ge::NodePtr GetPeerOutNodeWithInDataAnchor(ge::NodePtr curr_node, int idx) {
  ...
  FUSION_PASS_CHECK(peer_out_anchor == nullptr, OP_LOGD("", "peer out anchor is null."),
                    return nullptr);
...
inline ge::InDataAnchorPtr GetPeerInAnchorByOutDataAnchor(ge::OutDataAnchorPtr out_anchor_ptr, size_t idx) {
  FUSION_PASS_CHECK(out_anchor_ptr == nullptr, OP_LOGD("", "out anchor is null."), return nullptr);

  auto peer_in_anchors = out_anchor_ptr->GetPeerInDataAnchors();
  FUSION_PASS_CHECK(peer_in_anchors.size() <= idx, OP_LOGD("", "idx out of range."), return nullptr);
...
inline ge::NodePtr GetPeerInNodeByOutDataAnchor(ge::OutDataAnchorPtr out_anchor_ptr, size_t idx) {
  ...
  FUSION_PASS_CHECK(peer_in_anchor == nullptr, OP_LOGD("", "peer in anchor is null."), return nullptr);
...
inline ge::ConstGeTensorDescPtr GetCurrNodeInputDesc(ge::NodePtr curr_node, size_t idx) {
  ...
  FUSION_PASS_CHECK(curr_desc == nullptr, OP_LOGD("", "current opdesc is null."), return nullptr);
