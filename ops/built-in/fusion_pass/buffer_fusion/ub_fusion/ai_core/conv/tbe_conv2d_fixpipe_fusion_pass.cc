 ...
 */
vector<BufferFusionPattern *> TbeConv2DFixpipeFusionPass::DefinePatterns() {
  vector<BufferFusionPattern *> patterns;
  string pass_name = "TbeConv2DFixpipeFusionPass";
  ...
  pattern->AddOpDesc(kTypeTransData1, {kOpTypeTransData},
          ...
          .AddOpDesc(kPatternCube, {OP_PATTERN_CONV},
...
bool TbeConv2DFixpipeFusionPass::Conv2DInOutSupportTrans(const ge::NodePtr &node, const bool &is_input) {
  if(is_input) {
...
bool TbeConv2DFixpipeFusionPass::Conv2DWeightSupportTrans(const ge::NodePtr &node, const bool &is_input) {
  if(is_input) {
    return node->GetOpDesc()->GetInputDesc(0).GetFormat() == ge::FORMAT_NHWC &&
...
void TbeConv2DFixpipeFusionPass::CheckCubeSupportTransNodes(const vector<ge::NodePtr> &cube_nodes,
  ...
  if (cube_node->GetType() == "Conv2D") {
    if (!transdata1_nodes.empty()) {
      ...
      if (no_group && weight_trans && cube_node->GetInDataNodes().size() >= 2) {
        ...
        if (weight_trans_node->GetType() == kOpTypeTransData && Conv2DWeightSupportTrans(weight_trans_node, true)) {
          fusion_nodes.push_back(weight_trans_node);
        }
      }
    }
    if (!transdata2_nodes.empty()) {
      if (!Conv2DInOutSupportTrans(transdata2_nodes[0], false)) {
 ...
 */
Status TbeConv2DFixpipeFusionPass::GetFusionNodes(const BufferFusionMapping &mapping,
                                                  vector<ge::NodePtr> &fusion_nodes) {
...
REGISTER_BUFFER_FUSION_PASS("TbeConv2DFixpipeFusionPass", BUILT_IN_AI_CORE_BUFFER_FUSION_PASS,
                            TbeConv2DFixpipeFusionPass);
}  // namespace fe
