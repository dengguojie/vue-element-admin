...
#include "tbe_dx_fixpipe_fusion_pass.h"
#include <string>
#include <vector>
#include "op_log.h"
 ...
 */
vector<BufferFusionPattern *> TbeDxFixpipeFusionPass::DefinePatterns() {
  vector<BufferFusionPattern *> patterns;
  string pass_name = "TbeDxFixpipeFusionPass";
  ...
  pattern->AddOpDesc(kTypeTransData1, {kOpTypeTransData},
          ...
          .AddOpDesc(kPatternCube, {OP_PATTERN_CONV_BACKPROP_INPUT},
...
bool TbeDxFixpipeFusionPass::IsInWhiteListOfElemwiseOp(const vector<ge::NodePtr> &elemwise_nodes) {
...
void TbeDxFixpipeFusionPass::CheckCubeSupportTransNodes(const vector<ge::NodePtr> &cube_nodes,
  ...
  // dx
  if (cube_node->GetType() == "Conv2DBackpropInputD") {
    if (!transdata1_nodes.empty()) {
      bool weight_trans = true;
      if (!Conv2DWeightSupportTrans(transdata1_nodes[0], true)) {
...
Status TbeDxFixpipeFusionPass::GetFusionNodes(const BufferFusionMapping &mapping,
  ...
  OP_LOGD(kFusedOpType.c_str(), "End to do TbeDxFixpipeFusionPass!");
  return SUCCESS;
}
REGISTER_BUFFER_FUSION_PASS("TbeDxFixpipeFusionPass", BUILT_IN_AI_CORE_BUFFER_FUSION_PASS,
