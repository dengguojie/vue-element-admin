...
vector<BufferFusionPattern *> MatmulTransdataFusionPass::DefinePatterns() {
  ...
  FUSION_PASS_CHECK(pattern == nullptr, OP_LOGW(kFusedOpType, "can not new a pattern."), return patterns);

  pattern
      ->AddOpDesc(kPatternMatmul, {OP_PATTERN_MATMUL, OP_PATTERN_BATCH_MATMUL}, TBE_PATTERN_NUM_DEFAULT,
                  TBE_PATTERN_NUM_DEFAULT)
      .SetHead({kPatternMatmul});
  patterns.push_back(pattern);
  OP_LOGD(kFusedOpType, "Define pattern MatmulTransdataFusion success.");
...
void MatmulTransdataFusionPass::IsOutTransdataCorrect(const ge::Node::Vistor<ge::NodePtr> &out_node_matmuls) {
  ...
  if (out_transdata_ptr != nullptr && out_node_matmuls.size() != 1) {
    OP_LOGW(out_transdata_ptr, "Get out of matmul should only has one transdata.");
...
bool MatmulTransdataFusionPass::CheckFormatOfTransData(const ge::NodePtr &node_ptr_transdata,
  ...
  string src_format;
  string dst_format;
  FUSION_PASS_CHECK(node_ptr_transdata == nullptr, OP_LOGD(node_ptr_transdata, "Skip this node."), return true);
  FUSION_PASS_CHECK(!AttrUtils::GetStr(node_ptr_transdata->GetOpDesc(), "src_format", src_format),
                    OP_LOGW(node_ptr_transdata, "Unable to get the attribute src_format from TransData."),
                    return false);
  FUSION_PASS_CHECK(!AttrUtils::GetStr(node_ptr_transdata->GetOpDesc(), "dst_format", dst_format),
                    OP_LOGW(node_ptr_transdata, "Unable to get the attribute dst_format from TransData."),
                    return false);
...
bool MatmulTransdataFusionPass::IsLinkRelationshipCorrect() {
  FUSION_PASS_CHECK(matmul_node_ptr == nullptr, OP_LOGW(matmul_node_ptr, "Get node matmul failed"), return false);

  auto in_node_matmuls = matmul_node_ptr->GetInDataNodes();
  FUSION_PASS_CHECK(in_node_matmuls.size() < 2, OP_LOGW(matmul_node_ptr, "Matmul at least has 2 inputs"), return false);

  auto in_node_matmul_ptr_0 = in_node_matmuls.at(0);
  FUSION_PASS_CHECK(in_node_matmul_ptr_0 == nullptr, OP_LOGW(in_node_matmul_ptr_0, "Get input node 0 of matmul failed"),
                    return false);
  ...
  FUSION_PASS_CHECK(in_node_matmul_ptr_1 == nullptr, OP_LOGW(in_node_matmul_ptr_1, "Get input node 1 of matmul failed"),
                    return false);
  ...
  FUSION_PASS_CHECK(!CheckFormatOfTransData(transdata_ptr_0, kFormatNd, kFormatFractalNz),
                    OP_LOGW(kFusedOpType, "TransData(idx:0) format before MatMul node does not match."), return false);

  FUSION_PASS_CHECK(!CheckFormatOfTransData(transdata_ptr_1, kFormatNd, kFormatFractalNz),
                    OP_LOGW(kFusedOpType, "TransData(idx:1) format before Matmul node does not match."), return false);

  FUSION_PASS_CHECK(!CheckFormatOfTransData(out_transdata_ptr, kFormatFractalNz, kFormatNd),
                    OP_LOGW(kFusedOpType, "TransData format after Matmul node does not match."), return false);
  return true;
}
...
bool MatmulTransdataFusionPass::IsOutOfInTransdataCorrect() {
  if (transdata_ptr_0 != nullptr) {
    FUSION_PASS_CHECK(transdata_ptr_0->GetOutDataNodesSize() != 1,
                      OP_LOGW(transdata_ptr_0, "Transdata(index: 0) before matmul node has %u outputs.",
  ...
  if (transdata_ptr_1 != nullptr) {
    FUSION_PASS_CHECK(transdata_ptr_1->GetOutDataNodesSize() != 1,
                      OP_LOGW(transdata_ptr_1, "Transdata(index: 1) before matmul node has %u outputs.",
...
bool MatmulTransdataFusionPass::IsStaticShape() const { return !HasUnKnowShape(in_ptr_0) && !HasUnKnowShape(in_ptr_1); }
...
bool MatmulTransdataFusionPass::SupportOut2L1Nd2nz() {
  ...
  FUSION_PASS_CHECK(
      ...
      OP_LOGW(kFusedOpType, "Get platform info failed, not fusion."), return false);
  ...
  FUSION_PASS_CHECK(!support_out2l1_nd2nz, OP_LOGD(kFusedOpType, "Current version not support out2l1 nd2nz"),
                    return false);
  return true;
}
...
bool MatmulTransdataFusionPass::NeedFusion() {
  FUSION_PASS_CHECK(!IsLinkRelationshipCorrect(),
                    OP_LOGW(kFusedOpType, "The connection relationship does not meet expectations."), return false);
  ...
  FUSION_PASS_CHECK(matmul_node_ptr->GetOpDesc()->MutableInputDesc(0)->GetDataType() != DT_FLOAT16 ||
                    ...
                    OP_LOGW(kFusedOpType, "Only support input and output data types as float16."), return false);

  FUSION_PASS_CHECK(!IsStaticShape() || !IsAligned(),
                    OP_LOGW(kFusedOpType, "Static shape and unaligned scenes are not supported."), return false);

  FUSION_PASS_CHECK(
      SupportOut2L1Nd2nz(),
      OP_LOGW(kFusedOpType, "Transdata can be implemented during data load from Out to L1, not supported."),
      return false);
  return true;
}
...
bool MatmulTransdataFusionPass::ModifyTransdataInControlEdge(const ge::NodePtr &node_ptr_transdata) const {
  for (auto &control_transdata_node : node_ptr_transdata->GetInControlNodes()) {
    FUSION_PASS_CHECK(control_transdata_node == nullptr,
                      OP_LOGD(node_ptr_transdata, "In Transdata has no in control node."), return true);
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(control_transdata_node->GetOutControlAnchor(),
                      ...
                      OP_LOGW(node_ptr_transdata, "remove control edge between input and transdata failed."),
                      return false);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(control_transdata_node->GetOutControlAnchor(),
                      ...
                      OP_LOGW(node_ptr_transdata, "link control edge between input and matmul failed."), return false);
  }
  return true;
}
...
bool MatmulTransdataFusionPass::ModifyTransdataOutControlEdge(const ge::NodePtr &node_ptr_transdata) const {
  for (auto &transdata_control_node : node_ptr_transdata->GetOutControlNodes()) {
    FUSION_PASS_CHECK(transdata_control_node == nullptr,
                      OP_LOGD(node_ptr_transdata, "In Transdata has no out control node."), return true);
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(node_ptr_transdata->GetOutControlAnchor(),
                      ...
                      OP_LOGW(node_ptr_transdata, "remove control edge between transdata and output failed."),
                      return false);
    FUSION_PASS_CHECK(ge::GraphUtils::AddEdge(matmul_node_ptr->GetOutControlAnchor(),
                                              transdata_control_node->GetInControlAnchor()) == ge::GRAPH_FAILED,
                      OP_LOGW(node_ptr_transdata, "link control edge between matmul and output failed."), return false);
  }
  return true;
}
...
bool MatmulTransdataFusionPass::DelInputTransdata(ge::NodePtr &node_ptr_transdata, const uint32_t idx) {
  if (node_ptr_transdata != nullptr) {
    ...
    FUSION_PASS_CHECK(in_transdata_desc->GetShapeRange(range_data) == ge::GRAPH_FAILED,
                      OP_LOGW(node_ptr_transdata, "Failed to get input shape range of in_transdata:%u", idx),
                      return false);
    FUSION_PASS_CHECK(
        matmul_node_ptr->GetOpDesc()->MutableInputDesc(idx)->SetShapeRange(range_data) == ge::GRAPH_FAILED,
        OP_LOGW(node_ptr_transdata, "Failed to set input shape range of in_transdata:%u", idx), return false);
    ...
    FUSION_PASS_CHECK(in_transdata_peer_out == nullptr,
                      OP_LOGW(node_ptr_transdata, "Failed to get peer out anchor of in_transdata:%u", idx),
                      return false);
    FUSION_PASS_CHECK(
        ge::GraphUtils::RemoveEdge(in_transdata_peer_out, node_ptr_transdata->GetInDataAnchor(0)) == ge::GRAPH_FAILED,
        OP_LOGW(node_ptr_transdata, "Failed to remove edge between data and transdata."), return false);
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(node_ptr_transdata->GetOutDataAnchor(0),
                      ...
                      OP_LOGW(node_ptr_transdata, "Failed to remove edge between input and transdata."), return false);
    FUSION_PASS_CHECK(
        ge::GraphUtils::AddEdge(in_transdata_peer_out, matmul_node_ptr->GetInDataAnchor(idx)) == ge::GRAPH_FAILED,
        OP_LOGW(node_ptr_transdata, "Failed to add edge between input and matmul."), return false);

    // step2: modify control edge
    FUSION_PASS_CHECK(
        !ModifyTransdataInControlEdge(node_ptr_transdata),
        OP_LOGW(node_ptr_transdata, "Failed to modify control edge between input and intransdata:%u.", idx),
        return false);
    FUSION_PASS_CHECK(
        !ModifyTransdataOutControlEdge(node_ptr_transdata),
        OP_LOGW(node_ptr_transdata, "Failed to modify control edge between intransdata:%u. and out.", idx),
        return false);

    // step3: remove transdata
    auto graph = matmul_node_ptr->GetOwnerComputeGraph();
    FUSION_PASS_CHECK(graph == nullptr, OP_LOGW(matmul_node_ptr, "Failed to get compute graph of matmul."),
                      return false);
    FUSION_PASS_CHECK(graph->RemoveNode(node_ptr_transdata) == ge::GRAPH_FAILED,
                      OP_LOGW(matmul_node_ptr, "Failed to remove transdata:%u.", idx), return false);
  }
  return true;
}
...
bool MatmulTransdataFusionPass::DelOutputTransdata() const {
  if (out_transdata_ptr != nullptr) {
    ...
    FUSION_PASS_CHECK(out_transdata_desc->GetShapeRange(range_data) == ge::GRAPH_FAILED,
                      OP_LOGW(out_transdata_ptr, "Failed to get output shape range of out_transdata."), return false);
    FUSION_PASS_CHECK(matmul_node_ptr->GetOpDesc()->MutableOutputDesc(0)->SetShapeRange(range_data) == ge::GRAPH_FAILED,
                      OP_LOGW(out_transdata_ptr, "Failed to set out shape range of out_transdata."), return false);
    ...
    FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(matmul_node_ptr->GetOutDataAnchor(0),
                      ...
                      OP_LOGW(matmul_node_ptr, "Failed to remove edge between matmul and out transdata."),
                      return false);
    ...
    for (uint32_t idx = 0; idx < in_nodes_size; ++idx) {
      ...
      FUSION_PASS_CHECK(out_transdata_peer_in == nullptr,
                        OP_LOGW(matmul_node_ptr, "Can not get peer out anchor of in_transdata."), return false);
      FUSION_PASS_CHECK(ge::GraphUtils::RemoveEdge(out_transdata_out_anchor, out_transdata_peer_in) == ge::GRAPH_FAILED,
                        OP_LOGW(matmul_node_ptr, "Can not remove edge between out transdata and data."),
                        return false);
      FUSION_PASS_CHECK(
          ge::GraphUtils::AddEdge(matmul_node_ptr->GetOutDataAnchor(0), out_transdata_peer_in) == ge::GRAPH_FAILED,
          OP_LOGW(matmul_node_ptr, "Can not add edge between matmul and out data."), return false);
    }

    // step2: modify control edge
    FUSION_PASS_CHECK(!ModifyTransdataInControlEdge(out_transdata_ptr),
                      OP_LOGW(out_transdata_ptr, "Can not modify control edge between input and out transdata."),
                      return false);
    FUSION_PASS_CHECK(!ModifyTransdataOutControlEdge(out_transdata_ptr),
                      OP_LOGW(out_transdata_ptr, "Can not modify control edge between out transdata and out."),
                      return false);

    // step3: remove transdata
    auto graph = matmul_node_ptr->GetOwnerComputeGraph();
    FUSION_PASS_CHECK(graph == nullptr, OP_LOGW(out_transdata_ptr, "Can not get compute graph of matmul."),
                      return false);
    FUSION_PASS_CHECK(graph->RemoveNode(out_transdata_ptr) == ge::GRAPH_FAILED,
                      OP_LOGW(out_transdata_ptr, "Can not remove out transdata."), return false);
  }
  return true;
}
...
bool MatmulTransdataFusionPass::DoFusion() {
  ...
  FUSION_PASS_CHECK(!DelInputTransdata(transdata_ptr_0, 0), OP_LOGW(transdata_ptr_0, "Can not delete transdata:0."),
                    return false);
  FUSION_PASS_CHECK(!DelInputTransdata(transdata_ptr_1, 1), OP_LOGW(transdata_ptr_1, "Can not delete transdata:1."),
                    return false);

  // delete output transdata
  FUSION_PASS_CHECK(!DelOutputTransdata(), OP_LOGW(out_transdata_ptr, "Can not delete output transdata"), return false);
  return true;
}
...
Status MatmulTransdataFusionPass::GetFusionNodes(const BufferFusionMapping &mapping,
  ...
  FUSION_PASS_CHECK(matmul_nodes.empty(), OP_LOGW(kFusedOpType, "MatMul node is not matched."), return SUCCESS);

  matmul_node_ptr = matmul_nodes.at(0);

  FUSION_PASS_CHECK(
      std::find(kOpTypeMatmulList.begin(), kOpTypeMatmulList.end(), matmul_node_ptr->GetType()) ==
          kOpTypeMatmulList.end(),
      OP_LOGW(matmul_node_ptr, "Failed to match matmul node. It's %s node.", matmul_node_ptr->GetType().c_str()),
      return SUCCESS);

  FUSION_PASS_CHECK(!NeedFusion(), OP_LOGW(matmul_node_ptr, "no need do ub fusion"), return SUCCESS);

  FUSION_PASS_CHECK(!DoFusion(), OP_LOGW(matmul_node_ptr, "can not delete transdata"), return SUCCESS);
