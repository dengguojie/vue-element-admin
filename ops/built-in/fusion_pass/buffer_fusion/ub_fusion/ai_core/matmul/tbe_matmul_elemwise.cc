...
vector<BufferFusionPattern *> TbeMatmulElemwiseFusionPass::DefinePatterns() {
  ...
  pattern1
          ...
          .AddOpDesc(PATTERN_MATMUL, {OP_PATTERN_MATMUL}, TBE_PATTERN_NUM_DEFAULT,
                     TBE_PATTERN_NUM_DEFAULT)
          .AddOpDesc(PATTERN_OUTPUT, {TBE_PATTERN_OUTPUT_NODE},
                     TBE_PATTERN_NUM_DEFAULT, TBE_PATTERN_NUM_DEFAULT)
          .SetHead({PATTERN_MATMUL})
          .SetOutputs(PATTERN_MATMUL, {PATTERN_ELTWISE, PATTERN_OUTPUT},
...
Status TbeMatmulElemwiseFusionPass::GetFusionNodes(const BufferFusionMapping &mapping,
  ...
  for (auto &item : mapping) {
    auto elemOpdesc = find(item.first->types.begin(), item.first->types.end(),
                           OP_PATTERN_ELEMWISE);
    if (elemOpdesc == item.first->types.end()) {
      continue;
    }
    for (auto &node : item.second) {
      if (node->GetOpDesc()->GetAllOutputsDesc().size() !=
          NODE_OUTPUT_SIZE) {
        OP_LOGI(FUSED_OP_TYPE.c_str(), "The number of node[%s] output is [%zu],"
                "which is not equal to one, no need to do fusion.",
                node->GetName().c_str(),
                node->GetOpDesc()->GetAllOutputsDesc().size());
        return SUCCESS;
      }
    }
  }
...
REGISTER_BUFFER_FUSION_PASS("ATbeMatmulElemwiseFusionPass",
