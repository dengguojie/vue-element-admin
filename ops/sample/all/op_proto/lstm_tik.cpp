...
bool InferShapeAndTypeLSTMTik(Operator& op, const string& output_h_name, const string& output_c_name) {
  ...
  TensorDesc vOutputHDesc = op.GetOutputDesc(output_h_name);
  ...
  op.UpdateOutputDesc(output_h_name, vOutputHDesc);

  TensorDesc vOutputCDesc = op.GetOutputDesc(output_c_name);
  ...
  op.UpdateOutputDesc(output_c_name, vOutputCDesc);
