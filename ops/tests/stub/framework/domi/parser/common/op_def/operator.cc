...
Operator &Operator::Input(const Operator &in_op, uint32_t index) {
  if (index == 0) {
...
FMK_FUNC_HOST_VISIBILITY FMK_FUNC_DEV_VISIBILITY Operator &Operator::Name(const std::string &name) {
  name_ = name;
  return *this;
}

FMK_FUNC_HOST_VISIBILITY FMK_FUNC_DEV_VISIBILITY Operator &Operator::Type(const std::string &type) {
  type_ = type;
  return *this;
}

FMK_FUNC_HOST_VISIBILITY FMK_FUNC_DEV_VISIBILITY Operator &Operator::InputTensorDesc(
  const ge::GeTensorDesc &input_tensordesc) {
  input_descs_.push_back(input_tensordesc);
  return *this;
}

FMK_FUNC_HOST_VISIBILITY FMK_FUNC_DEV_VISIBILITY Operator &Operator::OutputTensorDesc(
  const ge::GeTensorDesc &output_tensordesc) {
  output_descs_.push_back(output_tensordesc);
  return *this;
}

FMK_FUNC_HOST_VISIBILITY FMK_FUNC_DEV_VISIBILITY Operator &Operator::AttrVector(std::string key,
...
FMK_FUNC_DEV_VISIBILITY FMK_FUNC_DEV_VISIBILITY Operator &Operator::AttrVector(std::string key,
...
Operator &Operator::Attr(const OpAttribute &attr) {
...
Operator &Operator::Attr_bt(const std::string &name, const std::string &value) {
  domi::AttrDef a;
  a.set_bt(value);
...
#define ATTR_SETTER_WITH_SINGLE_VALUE(type, field)                       \
  Operator &Operator::Attr(const std::string &name, const type &value) { \
    domi::AttrDef a;                                                     \
    a.set_##field(value);                                                \
    Attr(OpAttribute(name, a));                                          \
    return *this;                                                        \
...
#define ATTR_SETTER_WITH_LIST_VALUE(type, field)                                      \
  Operator &Operator::Attr(const std::string &name, const std::vector<type> &value) { \
    domi::AttrDef a;                                                                  \
    for (size_t i = 0; i < value.size(); ++i) {                                       \
      a.mutable_list()->add_##field(value[i]);                                        \
    }                                                                                 \
    Attr(OpAttribute(name, a));                                                       \
    return *this;                                                                     \
  }                                                                                   \
  Operator &Operator::Attr(const std::string &name, const ge::Tuple<type> &value) {   \
    domi::AttrDef a;                                                                  \
...
#define ATTR_GET_SINGLE_VALUE(type, field, type_name)                  \
  type Operator::Get##type_name##Attr(const std::string &name) const { \
...
#define ATTR_GET_TUPLE_VALUE(type, field, tuple_type_name)                              \
  tuple_type_name Operator::Get##tuple_type_name##Attr(const std::string &name) const { \
    domi::AttrDef value;                                                                \
    auto it = op_attrs_.find(name);                                                     \
    if (it != op_attrs_.end()) {                                                        \
