...
class TensorFlowModelParser : public ModelParser {
 public:
 ...
 private:
  ...
  Status AddFmkNode(ge::ComputeGraphPtr &graph, shared_ptr<ge::ScopeGraph> &scope_graph,
                    const vector<string> &op_node_name_list, bool is_dataset_init = false);
  ...
  /**
  * @ingroup domi_omg
  * @brief Resolve whether input of opnode is a variable or a constant，record it in opdef.is_input_const
  * @param [in] node_def Nodedef information
  * @param [in|out] op_def save op after parsing
  * @return SUCCESS get successfully
  * @return FAILED get failed
  */
  Status GetOpNodeInputMode(const domi::tensorflow::NodeDef *node_def, ge::OpDescPtr &op_def);
  ...
  */
  ...
  Status GraphDefOptimizeTranspose(domi::tensorflow::GraphDef *graph_def);
  ...
  */
  ...
  Status GenerateGraphByInputShape(const domi::tensorflow::GraphDef &input_graph_def,
  ...
  /**
   * @ingroup domi_omg
   * @brief Nodedef of new fusion operator
   * @param op_node_name，op name of the fusion operator
   * @return SUCCESS
   * @return FAILED

   */
  Status AddFusionNodeDef(const string &op_node_name);
 ...
 */
class TensorFlowWeightsParser : public WeightsParser {
 public:
  /**
   * @ingroup domi_omg
