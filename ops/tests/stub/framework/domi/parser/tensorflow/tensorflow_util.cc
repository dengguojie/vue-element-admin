    ...
    if (attr_value.has_list()) {                                                                                \
      if (attr_value.list().name##_size() > 0) {                                                                \
        if (type != "list(" type_string ")") {                                                                  \
          DOMI_LOGE("GeAttrValue had value with type 'list(" type_string ")'when '%s' expected", type.c_str()); \
          return FAILED;                                                                                        \
        }                                                                                                       \
        ++num_set;                                                                                              \
      }                                                                                                         \
      ...
      if (type != type_string) {                                                                                \
        DOMI_LOGE("GeAttrValue had value with type '" type_string "' when '%s' expected", type.c_str());        \
        return FAILED;                                                                                          \
      }                                                                                                         \
      ++num_set;                                                                                                \
    }                                                                                                           \
...
#undef VALIDATE_FIELD
  ...
  if (attr_value.value_case() == domi::tensorflow::AttrValue::kPlaceholder) {
    DOMI_LOGE("GeAttrValue had value with unexpected type 'placeholder'");
    return FAILED;
  }

  // Okay to have an empty list, but not to be missing a non-list value.
  if ((num_set == 0) && (!ge::StringUtils::StartWith(type, "list("))) {
    DOMI_LOGE("GeAttrValue missing value with expected type '%s'", type.c_str());
    return FAILED;
  }
  ...
  if (type == "type") {
    if (!domi::tensorflow::DataType_IsValid(attr_value.type())) {
      DOMI_LOGE("GeAttrValue has invalid DataType enum: %d", attr_value.type());
      return FAILED;
    }
    if (attr_value.type() == DT_INVALID) {
      DOMI_LOGE("GeAttrValue has invalid DataType");
      return FAILED;
    }
    ...
    for (auto &as_int : attr_value.list().type()) {
      ...
      if (!domi::tensorflow::DataType_IsValid(dtype)) {
        DOMI_LOGE("GeAttrValue has invalid DataType enum: %d", as_int);
        return FAILED;
      }
      if (dtype == DT_INVALID) {
        DOMI_LOGE("GeAttrValue contains invalid DataType");
        return FAILED;
      }
    }
  }
...
FMK_FUNC_HOST_VISIBILITY FMK_FUNC_DEV_VISIBILITY domi::Status TensorFlowUtil::TransTensorDescriptor(
  const domi::tensorflow::AttrValue &attr_value, domi::Operator *op, const uint32_t io, const string &type) {
  GE_CHECK_NOTNULL(op);
  if (!attr_value.has_list()) {
