...
class resize_bilinear_v2_grad_infer_test : public testing::Test {
protected:
  static void SetUpTestCase() {
    std::cout << "resize_bilinear_v2_grad_infer_test SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "resize_bilinear_v2_grad_infer_test TearDown" << std::endl;
  }
};

TEST_F(resize_bilinear_v2_grad_infer_test, resize_bilinear_v2_grad_infer_test_1) {
...
TEST_F(resize_bilinear_v2_grad_infer_test, resize_bilinear_v2_grad_infer_test_2) {
  // input grads shape {-1, 5}
  // input grads range {{1, 60}, {5, 5}}
  // input original_image shape {6, -1}
  // input original_image range {{6, 6}, {5, 90}}
  ...
  std::vector<std::pair<int64_t,int64_t>> range_x2 = {{3, 8}, {1, -1}, {2, 20}, {3, 83}};

  // expect result
  std::vector<int64_t> expected_shape = {3, -1, -1, 5};
  std::vector<std::pair<int64_t,int64_t>> expected_range = {{3, 3}, {1, -1},{2, 20}, {5, 5}};
  // create desc
  auto tensor_desc_x1 = create_desc_shape_range(shape_x1, ge::DT_FLOAT, ge::FORMAT_ND,
                                                shape_x1, ge::FORMAT_NHWC, range_x1);
  auto tensor_desc_x2 = create_desc_shape_range(shape_x2, ge::DT_FLOAT, ge::FORMAT_ND,
