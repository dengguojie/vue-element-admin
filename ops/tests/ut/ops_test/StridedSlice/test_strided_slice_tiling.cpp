...
TEST_F(stried_slice_tiling, stried_slice_tiling_no_mask) {
  ...
  auto iter =
      optiling::OpTilingInterf::RegisteredOpInterf().find("StridedSlice");
  ...
  opParas.const_inputs["begin"] =
      std::tuple<const uint8_t *, size_t>((const uint8_t *) begin.data(),
                                          begin.size() * sizeof(int32_t));
  opParas.const_inputs["end"] =
      std::tuple<const uint8_t *, size_t>((const uint8_t *) end.data(),
                                          end.size() * sizeof(int32_t));
  opParas.const_inputs["strides"] =
      std::tuple<const uint8_t *, size_t>((const uint8_t *) strides.data(),
                                          strides.size() * sizeof(int32_t));
...
// do tilling, get runInfo
  ...
  iter->second("StridedSlice", opParas, op_info, runInfo);
  EXPECT_EQ(to_string(runInfo.tiling_data),
            "10 10 3 1 2 10 10 3 1 1 0 0 0 0 0 10 10 3 1 1 1 1 1 1 1 ");
}
