...
def calc_expect_func(condition, x1, x2, y):
    ...
    output_data = np.where(x1_value, x2_value, x3_value)
    return output_data
...
ut_case.add_precision_case("all", {"params": [{"shape": (1, 24, 1, 256), "dtype": "int8", "format": "NCHW", "ori_shape": (1, 24, 1, 256),"ori_format": "NCHW", "param_type": "input", "value_range": [-1,2]},
                                              {"shape": (1, 24, 1, 256), "dtype": "float32", "format": "NCHW", "ori_shape": (1, 24, 1, 256),"ori_format": "NCHW", "param_type": "input"},
                                              {"shape": (1, 24, 1, 256), "dtype": "float32", "format": "NCHW", "ori_shape": (1, 24, 1, 256),"ori_format": "NCHW", "param_type": "input"},
                                              {"shape": (1, 24, 1, 256), "dtype": "float32", "format": "NCHW", "ori_shape": (1, 24, 1, 256),"ori_format": "NCHW", "param_type": "output"},
                                              ],
                                   "calc_expect_func": calc_expect_func,
                                   "precision_standard": precision_info.PrecisionStandard(0.001, 0.001)
                                   })
