...
def calc_expect_func(input, output, threshold, value):
    shape = input["shape"]
    data = input["value"]
    dtype = input["dtype"]

    input_x = tf.constant(data, dtype=dtype, shape=shape)
    data_thr = tf.constant(threshold, dtype=dtype, shape=shape)
    data_value = tf.constant(value, dtype=dtype, shape=shape)
    greater = tf.greater(input_x, data_thr)
    tmp_data = tf.where(greater, input_x, data_value)

    session = tf.Session()
    output_data = session.run(tmp_data)
    return output_data
