...
class BatchToSpaceNdFive():
    ...
    def kernel_ir(self, dst, src):
        ...
        if tile_axis == 1 and output_w * (block_h - 1) * burst <= 65535 and \
            ...
            padded_data = self.new_alloc(i_b, [size],
                                         name="padded_data",
                                         scope=tbe_platform.scope_ubuf)

            padded_data2 = self.new_alloc(i_b, [size],
                                          name="padded_data2",
                                          scope=tbe_platform.scope_ubuf)
                    ...
                    with i_b.for_range(0, input_h) as i_h:
                        ...
                        with i_b.for_range(0, block_w) as b_w:
                            ...
                            i_b.emit(
                                tvm.call_extern(
                                    dtype, "copy_gm_to_ubuf",
                                    padded_data.access_ptr("w",
                                                           offset=b_w * c_0),
                                    input_data.access_ptr("r",
                                                          offset=offset_src), 0,
                                    input_w, burst, 0, (block_w - 1) * burst))
                        ...
                        with i_b.if_scope(
                            ...
                            i_b.emit(
                                tvm.call_extern(
                                    ...
                                    padded_data.access_ptr("r",
                                                           offset=crop_l * c_0),
                                    0, 1, output_w * burst, 0, 0))

        elif tile_axis == 3 and (block_w - 1) * burst <= 65535 and (input_w <=
                                                                    4095):
            size = input_w * c_0
            padded_data = self.new_alloc(i_b, [size],
                                         name="padded_data",
                            ...
                            with i_b.for_range(0, input_w) as i_w:
                                ...
                                with i_b.if_scope(
                                    ...
                                    offset_base = (var * num_inner + n_i
                                                  ) * output_h * output_w * c_0
                                    offset_out = (i_h * block_h + b_h - crop_t
                                                 ) * output_w * c_0 + (
                                                     i_w * block_w + b_w -
                                                     crop_l) * c_0 + offset_base
                                    i_b.emit(
...
class BatchToSpaceNdSix():
    ...
    def __init__(self, shape, dtype, block_shape, crops, kernel_name):
        ...
        self.output_b = self.input_b // block_shape[0] // block_shape[
            1] // block_shape[2]
        # input batch and output batch
        ...
        # output shape
        self.out_shape = [
            self.output_b, self.output_d, self.c_1, self.output_h,
        ...
        self.input_tensor = self.tik_instance.Tensor(self.dtype,
                                                     ...
                                                     name="gm_in",
                                                     scope=tik.scope_gm)
        self.output_tensor = self.tik_instance.Tensor(self.dtype,
                                                      self.out_shape,
                                                      name="gm_out",
                                                      scope=tik.scope_gm)
    ...
    def run_depth(self):
        ...
        with self.tik_instance.for_range(0, self.output_b) as idx_b:
            ...
            ub_tensor_out = self.tik_instance.Tensor(self.dtype, [ub_size],
                                                     ...
                                                     scope=tik.scope_ubuf)
            # move in
            with self.tik_instance.for_range(0, self.input_d) as idx_d:
                with self.tik_instance.for_range(
                        0, self.block_shape[0]) as idx_blk_0:
                    with self.tik_instance.for_range(0, self.c_1) as idx_c1:
                        with self.tik_instance.for_range(0,
                                                         self.input_h) as idx_h:
                            with self.tik_instance.for_range(
                                    0, self.block_shape[1]) as idx_blk_1:
                                with self.tik_instance.for_range(
                                        0, self.block_shape[2]) as idx_blk_2:
                                    offset_gm_in = (
                                        (((((idx_blk_0 * self.block_shape[1] +
                                             idx_blk_1) * self.block_shape[2] +
                                            idx_blk_2) * self.output_b + idx_b)
                                          * self.input_d + idx_d) * self.c_1 +
                                         idx_c1) * self.input_h +
                                        idx_h) * self.input_w * self.c_0
                                    offset_ub_in = (
                                        (((idx_d * self.block_shape[0] +
                                           idx_blk_0) * self.c_1 + idx_c1) *
                                         self.input_h + idx_h) *
                                        self.block_shape[1] + idx_blk_1
                                    ) * self.input_w * self.block_shape[
                                        2] * self.c_0 + idx_blk_2 * self.c_0
                                    dst_stride = self.block_shape[
                                        2] * self.burst_len - self.burst_len
                                    self.tik_instance.data_move(
                                        ub_tensor_in[offset_ub_in],
                                        self.input_tensor[offset_gm_in], 0,
                                        self.input_w, self.burst_len, 0,
                                        dst_stride)
            # crops permute
            offset_crops = self.crops[0][
                0] * self.c_1 * self.permute_h * self.permute_w * self.c_0 + self.crops[
                    1][0] * self.permute_w * self.c_0 + self.crops[2][
                        0] * self.c_0
            with self.tik_instance.for_range(0, self.output_d) as idx_d:
                with self.tik_instance.for_range(0, self.c_1) as idx_c1:
                    offset_ub_out = (idx_d * self.c_1 + idx_c1
                                    ) * self.output_h * self.output_w * self.c_0
                    offset_ub_in = offset_crops + (
                        idx_d * self.c_1 +
                        idx_c1) * self.permute_h * self.permute_w * self.c_0
                    src_stride = (self.crops[2][1] +
                                  self.crops[2][0]) * self.burst_len
                    self.tik_instance.data_move(ub_tensor_out[offset_ub_out],
                                                ub_tensor_in[offset_ub_in], 0,
                                                self.output_h,
                                                self.output_w * self.burst_len,
                                                src_stride, 0)
            # move out
            ...
            self.tik_instance.data_move(self.output_tensor[offset_gm_out],
                                        ub_tensor_out, 0, 1, burst_len, 0, 0)
        ...
        with self.tik_instance.for_range(0, self.output_b) as idx_b:
            with self.tik_instance.for_range(0, self.input_d) as idx_d:
                with self.tik_instance.for_range(
                    ...
                    ub_tensor_in = self.tik_instance.Tensor(
                        self.dtype, [ub_size],
                        name="ub_tensor_in",
                        scope=tik.scope_ubuf)
                    ub_tensor_out = self.tik_instance.Tensor(
                        self.dtype, [ub_size],
                        name="ub_tensor_out",
                        scope=tik.scope_ubuf)
                    ...
                    with self.tik_instance.if_scope(
                        ...
                        with self.tik_instance.for_range(0, self.c_1) as idx_c1:
                            with self.tik_instance.for_range(
                                    0, self.input_h) as idx_h:
                                with self.tik_instance.for_range(
                                        0, self.block_shape[1]) as idx_blk_1:
                                    with self.tik_instance.for_range(
                                            0,
                                            self.block_shape[2]) as idx_blk_2:
                                        offset_gm_in = ((((
                                            ((idx_blk_0 * self.block_shape[1] +
                                              idx_blk_1) * self.block_shape[2] +
                                             idx_blk_2) * self.output_b + idx_b
                                        ...
                                        self.tik_instance.data_move(
                                            ...
                                            dst_stride)
                        # crops permute
                        offset_crops = self.crops[1][
                            0] * self.permute_w * self.c_0 + self.crops[2][
                                0] * self.c_0
                        with self.tik_instance.for_range(0, self.c_1) as idx_c1:
                    ...
                    with self.tik_instance.for_range(0, self.c_1) as idx_c1:
                        ...
                        with self.tik_instance.if_scope(
                            ...
                            src_stride = (self.crops[2][1] +
                                          self.crops[2][0]) * self.burst_len
                            self.tik_instance.data_move(
                                ub_tensor_out, ub_tensor_in[offset_crops], 0,
                                self.output_h, self.output_w * self.burst_len,
                                src_stride, 0)
                            # move out
                            ...
                            self.tik_instance.data_move(
                                self.output_tensor[offset_gm_out],
                                ub_tensor_out, 0, 1, burst_len, 0, 0)
                    ...
                    with self.tik_instance.for_range(0, self.c_1) as idx_c1:
                        with self.tik_instance.for_range(0,
                            ...
                            with self.tik_instance.for_range(
                                ...
                                with self.tik_instance.if_scope(
                                        tik.all(
                                            flag_d >= self.crops[0][0], flag_d <
                                            self.crops[0][0] + self.output_d,
                                            flag_h >= self.crops[1][0], flag_h <
                                            self.crops[1][0] + self.output_h)):
                                    # move in
                                    with self.tik_instance.for_range(
                                            0,
                                            self.block_shape[2]) as idx_blk_2:
                                    ...
                                    offset_d = (
                                        idx_d * self.block_shape[0] +
                                        idx_blk_0 - self.crops[0][0]
                                    ) * self.c_1 * self.output_h * self.output_w * self.c_0
                                    offset_c1 = idx_c1 * self.output_h * self.output_w * self.c_0
                                    offset_h = (idx_h * self.block_shape[1] +
                                                idx_blk_1 - self.crops[1][0]
                                               ) * self.output_w * self.c_0
                                    offset_gm_out = offset_b + offset_d + offset_c1 + offset_h
                                    burst_len = self.output_w * self.burst_len
                                    self.tik_instance.data_move(
                                        self.output_tensor[offset_gm_out],
                                        ub_tensor[self.crops[2][0] * self.c_0],
                                        0, 1, burst_len, 0, 0)
                                ...
                                with self.tik_instance.for_range(
                                    ...
                                    with self.tik_instance.for_range(
                                        ...
                                        ub_tensor = self.tik_instance.Tensor(
                                            ...
                                            name="ub_tensor",
                                            scope=tik.scope_ubuf)
                                        flag_d = idx_d * self.block_shape[
                                            0] + idx_blk_0
                                        flag_h = idx_h * self.block_shape[
                                            1] + idx_blk_1
                                        flag_w = idx_w * self.block_shape[
                                            2] + idx_blk_2
                                        with self.tik_instance.if_scope(
                                                tik.all(
                                                    flag_d >= self.crops[0][0],
                                            ...
                                            offset_gm_in = (((((
                                                ((idx_blk_0 * self.
                                                  block_shape[1] + idx_blk_1) *
                                                 self.block_shape[2] +
                                                 idx_blk_2) * self.output_b +
                                                idx_b) * self.input_d + idx_d) *
                                                              self.c_1 + idx_c1)
                                                             * self.input_h +
                                                             idx_h) *
                                                            self.input_w +
                                            ...
                                            self.tik_instance.data_move(
                                                ub_tensor,
                                                self.input_tensor[offset_gm_in],
    ...
    def run(self):
        ...
        if self.permute_d * self.c_1 * self.permute_h * self.permute_w * self.c_0 <= self.ub_half_ele:
            ...
            self.run_height()
        elif self.permute_w * self.c_0 <= self.ub_half_ele:
            self.run_width()
        else:
...
def get_op_support_info(x,
    ...
    if format_x == "NC1HWC0":
        ...
        axis_reduce_list = None
    op_cal_info_in_json = get_op_cal_info(axis_split_matrix, axis_reduce_list,
                                          0, 0)
...
def check_supported(x,
    ...
    if ori_format not in ("NHWC", "NCHW", "NDHWC",
                          "NCDHW") or input_format not in ("NHWC", "NCHW",
                                                           "NDHWC", "NCDHW",
                                                           "NC1HWC0",
                                                           "NDC1HWC0"):
        return False
    if ori_format in ("NHWC", "NCHW") and input_format in (
        ...
        return False
    if ori_format in ("NDHWC", "NCDHW") and input_format in (
            "NDHWC", "NDHWC") and len(input_format) != 5:
        return False
    if ori_format in ("NHWC",):
        return len(block_shape) == 2 and len(crops) == 4
    elif ori_format in ("NCHW",):
        return len(block_shape) == 3 and len(crops) == 6 and block_shape[
            0] == 1 and crops[0] == 0 and crops[1] == 0
    elif ori_format in ("NDHWC",):
        return len(block_shape) == 3 and len(crops) == 6
    else:
        return len(block_shape) == 4 and len(crops) == 8 and block_shape[
            0] == 1 and crops[0] == 0 and crops[1] == 0
...
def check_parms_6hd(shape, dtype, block_shape, crops, kernel_name):
    ...
    if len(shape) != 6:
        error_detail = "the shape'rank of x should be 6 bug got: %d" % len(
            shape)
        error_manager_vector.raise_err_input_shape_invalid(
            kernel_name, "x", error_detail)

    if len(block_shape) != 3:
        error_detail = "the shape'rank of block_shape should be 3 bug got: %d" % len(
            block_shape)
        error_manager_vector.raise_err_input_shape_invalid(
            kernel_name, "block_shape", error_detail)

    if len(crops) != 3 or len(crops[0]) != 2 or len(crops[1]) != 2 or len(
            crops[2]) != 2:
        error_detail = "the shape of crops should be 3x2"
        error_manager_vector.raise_err_input_shape_invalid(
            kernel_name, "crops", error_detail)

    if not (isinstance(block_shape[0], int) and isinstance(
            block_shape[1], int) and isinstance(block_shape[2], int) and
    ...
    if not (isinstance(crops[0][0], int) and crops[0][0] >= 0 and
            ...
            isinstance(crops[1][1], int) and crops[1][1] >= 0 and
            isinstance(crops[2][0], int) and crops[2][0] >= 0 and
            isinstance(crops[2][1], int) and crops[2][1] >= 0):
...
def batch_to_space_nd_d_6hd(x,
                            y,
                            block_shape,
                            crops,
                            kernel_name="batch_to_space_nd_d_6hd"):
    ...
    if ori_format in ("NDHWC",):
        ...
        if len(crops) == 8 and crops[0] == 0 and crops[1] == 0:
            crops = [[crops[2], crops[3]], [crops[4], crops[5]],
                     [crops[6], crops[7]]]
        elif len(crops) == 4 and len(crops[0]) == 2 and len(
                crops[1]) == 2 and len(crops[2]) == 2 and len(
                    crops[3]) == 2 and crops[0][0] == 0 and crops[0][1] == 0:
            crops = [[crops[1][0], crops[1][1]], [crops[2][0], crops[2][1]],
                     [crops[3][0], crops[3][1]]]
        else:
            error_detail = "The value of first crops must be 0"
            error_manager_vector.raise_err_input_shape_invalid(
                kernel_name, "crops", error_detail)
    else:
        error_manager_vector.raise_err_input_format_invalid(
            kernel_name, "x", "NDHWC,NCDHW", ori_format)

    # check input params
    check_parms_6hd(shape, dtype, block_shape, crops, kernel_name)

    # if output is the same as input, call the function 'copy_only'
    if block_shape[0] == 1 and block_shape[1] == 1 and block_shape[
            2] == 1 and crops[0][0] == 0 and crops[0][1] == 0 and crops[1][
                0] == 0 and crops[1][1] == 0 and crops[2][0] == 0 and crops[2][
                    1] == 0:
        copy_only(x, x, kernel_name)
        return

    # run tik
    batch_to_space_nd = BatchToSpaceNdSix(shape, dtype, block_shape, crops,
                                          kernel_name)
...
def batch_to_space_nd_d_5hd(x,
                            y,
                            block_shape,
                            crops,
                            kernel_name="batch_to_space_nd_d_5hd"):
    """BatchToSpace for N-D tensors.
...
def batch_to_space_nd_d(x,
    ...
    crops: list or tuple
        2-D with shape [2, 2] or [3, 2], crops[i] = [crop_start, crop_end].
    kernel_name: str
    ...
    if input_format not in ("NC1HWC0", "NDC1HWC0"):
        error_manager_vector.raise_err_input_format_invalid(
            kernel_name, "x", "NC1HWC0,NDC1HWC0", input_format)
