...
def _gen_outputs(input_var, input_m, input_v, input_beta1_power, input_lr, 
                 input_beta1, input_beta2, input_epsilon, input_grad, dtype):
    if dtype == 'float16':
    ...
    output_var = input_var - input_lr / (1 - input_beta1_power) * (
            output_m / (output_v + input_epsilon))

    if dtype == 'float16':
        output_m = output_m.astype(dtype)
        output_v = output_v.astype(dtype)
        output_var = output_var.astype(dtype)
    output_res = output_var
