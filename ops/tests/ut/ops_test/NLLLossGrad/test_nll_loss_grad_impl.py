...
def calc_expect_func(inputArr, y_grad, target, weight, total_weight, output_y, reduction):

    shape_out = list(inputArr["shape"])
    outputArr = np.zeros(shape_out).astype(inputArr["dtype"])
    if len(shape_out[0]==1):
        outputArr[target["value"][i]] = y_grad["value"][0]
    else:
        for i in range(0,shape_out[0]):
            if reduction == "none":
                outputArr[i][target["value"][i]] = y_grad["value"][i]
            elif reduction == "sum":
                outputArr[i][target["value"][i]] = y_grad["value"][i]
                outputArr[i][target["value"][i]] =  weight["value"][target[i]] * outputArr[i][target["value"][i]] * -1
            elif reduction == "mean":
                outputArr[i][target["value"][i]] = y_grad["value"][i]
                outputArr[i][target["value"][i]] =  weight["value"][target[i]] * outputArr[i][target["value"][i]] * -1
                outputArr[i][target["value"][i]] = outputArr[i][target["value"][i]]/total_weight["value"][0]
...
ut_case.add_precision_case("all", {
    "params": [
        ...
        {"shape": (1, ), "dtype": "float32", "format": "ND", "ori_shape": (3, 4), "ori_format": "ND", "param_type": "input", "value_range": [0.0, 1.0]},
        ...
        {"shape": (3, 4), "dtype": "float32", "format": "ND", "ori_shape": (1, ), "ori_format": "ND", "param_type": "output"}, "mean",],
