...
ut_case = OpUT("AtanGrad", None, None)

def calc_expect_func(inputA, inputB, output):
    input_A_Arr = inputA['value']
    input_B_Arr = inputB['value']
    output_Arr = 1 / (1 + np.square(input_A_Arr)) * input_B_Arr
    return output_Arr

case1 = {"params": [{"shape": (100,1), "dtype": "float16", "format": "ND", "ori_shape": (100,1),"ori_format": "ND"},
                    {"shape": (100,1), "dtype": "float16", "format": "ND", "ori_shape": (100,1),"ori_format": "ND"},
                    {"shape": (100,1), "dtype": "float16", "format": "ND", "ori_shape": (100,1),"ori_format": "ND"}],
         "case_name": "atan_grad_1",
         "expect": "success",
         "format_expect": [],
         "support_expect": True}
