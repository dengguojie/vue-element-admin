...
class CAST_KERNEL_UTest : public testing::Test {
protected:
    virtual void SetUp() {}
...
TEST_F(CAST_KERNEL_UTest, Host_FLOAT_INT)
{
...
TEST_F(CAST_KERNEL_UTest, Host_FLOAT_INT1)
{
    auto nodeDef = CpuKernelUtils::CpuKernelUtils::CreateNodeDef();
    nodeDef->SetOpType("Cast");
    ...
    EXPECT_EQ(output[0], 22);
    EXPECT_EQ(output[1], 32);
   // EXPECT_EQ(output[19], 22);
    //EXPECT_EQ(output[18], 32);
    cout << "Test Kernel " << nodeDef->GetOpType() << " Finish. " << endl;
}

TEST_F(CAST_KERNEL_UTest, Host_FLOAT_INT2)
{
    auto nodeDef = CpuKernelUtils::CpuKernelUtils::CreateNodeDef();
    nodeDef->SetOpType("Cast");
    ...
    int expect_out[2][2] = {22, 32, -78, -28};
    NodeDefBuilder(nodeDef.get(), "Cast", "Cast")
        .Input({ "x", DT_FLOAT, { 0}, (void *)input })
        .Output({ "y", DT_INT32, { 0 }, (void *)output });
...
TEST_F(CAST_KERNEL_UTest, Host_FLOAT_INT3)
{
    auto nodeDef = CpuKernelUtils::CpuKernelUtils::CreateNodeDef();
    nodeDef->SetOpType("Cast");
    ...
    int expect_out[2][2] = {22, 32, -78, -28};
    NodeDefBuilder(nodeDef.get(), "Cast", "Cast")
        .Input({ "x", DT_STRING, { 10,2}, (void *)input })
        .Output({ "y", DT_INT32, { 10 }, (void *)output });
...
TEST_F(CAST_KERNEL_UTest, Host_INT_BOOL)
{
    auto nodeDef = CpuKernelUtils::CpuKernelUtils::CreateNodeDef();
    nodeDef->SetOpType("Cast");
