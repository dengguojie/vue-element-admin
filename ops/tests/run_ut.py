...
repo_root = os.path.sep.join(cur_dir.split(os.path.sep)[:-3])
...
def get_file_change_info_from_ci(changed_file_info_from_ci):
    ...
    for line in lines:
        line = line.strip()
        if line.startswith(os.path.join("ops", "built-in")):
            ops_changed_files.append(line)
        elif line.startswith(os.path.join("ops", "tests")):
    ...
    def _get_relate_ut_list_by_file_change():
        ...
        def _deal_test_file_change():
            ...
            for test_changed_file in test_changed_files:
                ...
                test_case_dir = os.path.join("ops", "tests", "ut", "ops_test")
        ...
        def _deal_op_test_frame_change():
            if not relate_ut_dir_list:
                relate_ut_dir_list.append(os.path.join(repo_root, "ops", "tests", "ut", "ops_test", "Add"))
