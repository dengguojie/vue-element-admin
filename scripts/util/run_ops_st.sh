...
modify_for_cov() {
  ...
  do
    if [[ `grep "st_cover.start()" $manager_file|wc -l` -eq 0 ]]; then
      ...
      sed -i 's#opm = importlib.import_module(op_module)#import coverage,sys,time;cov_file = os.path.join(os.getenv("CANN_ROOT") or "", "cov_result", ".coverage.%s.%s" % (op_module, time.time()));st_cover = coverage.Coverage(source=[op_module, "impl"], data_file=cov_file);st_cover.start();opm = importlib.import_module(op_module);st_cover.stop;st_cover.save();#g' $manager_file
