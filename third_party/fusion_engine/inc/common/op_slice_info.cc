/**
 * @file op_slice_info.cc
 *
 * Copyright (c) Huawei Technologies Co., Ltd. 2019-2020. All rights reserved.
 *
 * @brief provide methods to set/get op slice info
 *
 * @version 1.0
 *
 */
...
InputSplitInfo::InputSplitInfo() {
  split_impl_ = std::make_shared<InputSplitInfoImpl>();
}
...
OutputSplitInfo::OutputSplitInfo() {
  split_impl_ = std::make_shared<OutputSplitInfoImpl>();
}
...
InputReduceInfo::InputReduceInfo() {
  reduce_impl_ = std::make_shared<InputReduceInfoImpl>();
}
...
class OutputReduceInfoImpl {
 public:
  size_t GetIndex() { return idx_; }
  OpReduceType GetReduceType() { return reduce_type_; }
  bool GetIsAtomic() { return is_atomic_; }
  void SetIndex(const size_t& idx) { idx_ = idx; }
...
OutputReduceInfo::OutputReduceInfo() {
  reduce_impl_ = std::make_shared<OutputReduceInfoImpl>();
}
...
class AxisSplitMapImpl {
 public:
  ...
  void AddInputSplitInfo(InputSplitInfo& input_split_info) {
    InputSplitInfoPtr input_split_info_ptr = std::make_shared<InputSplitInfo>(input_split_info);
  ...
  void AddOutputSplitInfo(OutputSplitInfo& output_split_info) {
    OutputSplitInfoPtr output_split_info_ptr = std::make_shared<OutputSplitInfo>(output_split_info);
...
AxisSplitMap::AxisSplitMap() {
  aixs_split_impl_ = std::make_shared<AxisSplitMapImpl>();
}
...
class AxisReduceMapImpl {
 public:
  ...
  void AddInputReduceInfo(InputReduceInfo& input_reduce_info) {
    InputReduceInfoPtr input_reduce_info_ptr = std::make_shared<InputReduceInfo>(input_reduce_info);
  ...
  void AddOutputReduceInfo(OutputReduceInfo& output_reduce_info) {
    OutputReduceInfoPtr output_reduce_info_ptr = std::make_shared<OutputReduceInfo>(output_reduce_info);
...
AxisReduceMap::AxisReduceMap() {
  aixs_reduce_impl_ = std::make_shared<AxisReduceMapImpl>();
}
...
class OpCalcInfoImpl {
 public:
  ...
  OpL1FusionType GetL1FusionEnable() { return l1_fusion_enable_; }
  int64_t GetMinTbeL1Space() { return min_tbe_l1_space_; }

  void AddAxisSplitMap(AxisSplitMap& axis_split_map) {
    AxisSplitMapPtr axis_split_map_ptr = std::make_shared<AxisSplitMap>(axis_split_map);
  ...
  void AddAxisReduceMap(AxisReduceMap& axis_reduce_map) {
    AxisReduceMapPtr axis_reduce_map_ptr = std::make_shared<AxisReduceMap>(axis_reduce_map);
 ...
 private:
  ...
  OpL1FusionType l1_fusion_enable_;
  int64_t min_tbe_l1_space_ = 0;
};

OpCalcInfo::OpCalcInfo() {
  op_calc_info_impl_ = std::make_shared<OpCalcInfoImpl>();
}
